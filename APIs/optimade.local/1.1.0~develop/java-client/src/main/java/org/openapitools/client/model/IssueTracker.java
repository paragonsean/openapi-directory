...
public class IssueTracker extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(IssueTracker.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!IssueTracker.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'IssueTracker' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<IssueTracker>() {
                @Override
                public void write(JsonWriter out, IssueTracker value) throws IOException {
                ...
                @Override
                public IssueTracker read(JsonReader in) throws IOException {
                    ...
                    try {
                        ...
                        IssueTracker ret = new IssueTracker();
                    ...
                    throw new IOException(String.format("Failed deserialization for IssueTracker: no class matches result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to IssueTracker
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        throw new IOException(String.format("The JSON string is invalid for IssueTracker with anyOf schemas: Link, URI. no class match the result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
    }

    /**
     * Create an instance of IssueTracker given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of IssueTracker
     * @throws IOException if the JSON string is invalid with respect to IssueTracker
     */
    public static IssueTracker fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, IssueTracker.class);
    }

    /**
     * Convert an instance of IssueTracker to an JSON string
     *
     * @return JSON string
     */
