...
public class Prev extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(Prev.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!Prev.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'Prev' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<Prev>() {
                @Override
                public void write(JsonWriter out, Prev value) throws IOException {
                ...
                @Override
                public Prev read(JsonReader in) throws IOException {
                    ...
                    throw new IOException(String.format("Failed deserialization for Prev: no class matches result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to Prev
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        throw new IOException(String.format("The JSON string is invalid for Prev with anyOf schemas: Link, URI. no class match the result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
    }

    /**
     * Create an instance of Prev given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of Prev
     * @throws IOException if the JSON string is invalid with respect to Prev
     */
    public static Prev fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, Prev.class);
    }

    /**
     * Convert an instance of Prev to an JSON string
     *
     * @return JSON string
     */
