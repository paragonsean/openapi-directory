...
public class Last extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(Last.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!Last.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'Last' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<Last>() {
                @Override
                public void write(JsonWriter out, Last value) throws IOException {
                ...
                @Override
                public Last read(JsonReader in) throws IOException {
                    ...
                    throw new IOException(String.format("Failed deserialization for Last: no class matches result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to Last
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        throw new IOException(String.format("The JSON string is invalid for Last with anyOf schemas: Link, URI. no class match the result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
    }

    /**
     * Create an instance of Last given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of Last
     * @throws IOException if the JSON string is invalid with respect to Last
     */
    public static Last fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, Last.class);
    }

    /**
     * Convert an instance of Last to an JSON string
     *
     * @return JSON string
     */
