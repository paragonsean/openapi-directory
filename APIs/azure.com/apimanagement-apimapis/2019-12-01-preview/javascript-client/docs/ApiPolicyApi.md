...
> ApiOperationPolicyGet200Response apiPolicyGet(resourceGroupName, serviceName, apiId, policyId, apiVersion, subscriptionId, opts)
...
apiInstance.apiPolicyGet(resourceGroupName, serviceName, apiId, policyId, apiVersion, subscriptionId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
