...
> IssueCollection apiIssueListByService(resourceGroupName, serviceName, apiId, apiVersion, subscriptionId, opts)
...
> apiIssueUpdate(resourceGroupName, serviceName, apiId, issueId, apiVersion, subscriptionId, parameters, opts)
...
let apiVersion = "apiVersion_example"; // String | Version of the API to be used with the client request.
let subscriptionId = "subscriptionId_example"; // String | Subscription credentials which uniquely identify Microsoft Azure subscription. The subscription ID forms part of the URI for every service call.
let parameters = new ApiManagementClient.IssueUpdateContract(); // IssueUpdateContract | Update parameters.
let opts = {
  'ifMatch': "ifMatch_example" // String | ETag of the Issue Entity. ETag should match the current entity state from the header response of the GET request or it should be * for unconditional update.
};
apiInstance.apiIssueUpdate(resourceGroupName, serviceName, apiId, issueId, apiVersion, subscriptionId, parameters, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully.');
