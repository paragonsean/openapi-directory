...
> IssueAttachmentCollection apiIssueAttachmentsListByService(resourceGroupName, serviceName, apiId, apiVersion, issueId, subscriptionId, opts)
...
apiInstance.apiIssueAttachmentsListByService(resourceGroupName, serviceName, apiId, apiVersion, issueId, subscriptionId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
