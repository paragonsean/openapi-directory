...
async def test_policy_assignments_list_for_management_group(client):
    ...
    response = await client.request(
        method='GET',
        path='/providers/Microsoft.Management/managementgroups/{management_group_id}/providers/Microsoft.Authorization/policyAssignments'.format(management_group_id='management_group_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
