

# ApiManagementServiceResource

A single API Management service resource in List or Get response.

## Properties

| Name | Type | Description | Notes |
|------------ | ------------- | ------------- | -------------|
|**etag** | **String** | ETag of the resource. |  [optional] [readonly] |
|**properties** | [**ApiManagementServiceProperties**](ApiManagementServiceProperties.md) |  |  |
|**sku** | [**ApiManagementServiceSkuProperties**](ApiManagementServiceSkuProperties.md) |  |  |
|**id** | **String** | Resource ID. |  [optional] [readonly] |
|**location** | **String** | Resource location. |  |
|**name** | **String** | Resource name. |  [optional] |
|**tags** | **Map&lt;String, String&gt;** | Resource tags. |  [optional] |
|**type** | **String** | Resource type for API Management resource is set to Microsoft.ApiManagement. |  [optional] [readonly] |



