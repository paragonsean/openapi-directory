# StorageCacheMgmtClient.StorageTarget

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**id** | **String** | Resource ID of the Storage Target. | [optional] [readonly] 
**name** | **String** | Schema for the name of resources served by this provider. Note that objects will contain an odata @id annotation as appropriate. This will contain the complete URL of the object. These names are case-preserving, but not case sensitive. | [optional] 
**properties** | [**StorageTargetProperties**](StorageTargetProperties.md) |  | [optional] 
**type** | **String** | Type of the Storage Target; Microsoft.StorageCache/Cache/StorageTarget | [optional] [readonly] 


