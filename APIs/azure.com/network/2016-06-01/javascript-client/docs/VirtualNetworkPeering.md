# NetworkManagementClient.VirtualNetworkPeering

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**etag** | **String** | A unique read-only string that changes whenever the resource is updated | [optional] 
**name** | **String** | Gets or sets the name of the resource that is unique within a resource group. This name can be used to access the resource | [optional] 
**properties** | [**VirtualNetworkPeeringPropertiesFormat**](VirtualNetworkPeeringPropertiesFormat.md) |  | [optional] 
**id** | **String** | Resource Id | [optional] 


