# NetworkManagementClient.SubnetsApi
...
let apiInstance = new NetworkManagementClient.SubnetsApi();
...
let subnetParameters = new NetworkManagementClient.Subnet(); // Subnet | Parameters supplied to the create/update Subnet operation
...
> Subnet subnetsGet(resourceGroupName, virtualNetworkName, subnetName, apiVersion, subscriptionId, opts)
...
apiInstance.subnetsGet(resourceGroupName, virtualNetworkName, subnetName, apiVersion, subscriptionId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
