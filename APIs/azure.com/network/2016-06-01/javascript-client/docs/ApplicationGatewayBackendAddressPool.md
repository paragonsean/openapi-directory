# NetworkManagementClient.ApplicationGatewayBackendAddressPool

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**etag** | **String** | A unique read-only string that changes whenever the resource is updated | [optional] 
**name** | **String** | Resource that is unique within a resource group. This name can be used to access the resource | [optional] 
**properties** | [**ApplicationGatewayBackendAddressPoolPropertiesFormat**](ApplicationGatewayBackendAddressPoolPropertiesFormat.md) |  | [optional] 
**id** | **String** | Resource Id | [optional] 


