...
> [Advisor] databaseAdvisorsListByDatabase(resourceGroupName, serverName, databaseName, subscriptionId, apiVersion)
...
apiInstance.databaseAdvisorsUpdate(resourceGroupName, serverName, databaseName, advisorName, subscriptionId, apiVersion, parameters, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
