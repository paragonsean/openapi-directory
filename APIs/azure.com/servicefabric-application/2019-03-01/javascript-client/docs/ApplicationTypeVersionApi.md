...
apiInstance.applicationTypeVersionsCreateOrUpdate(subscriptionId, resourceGroupName, clusterName, applicationTypeName, version, apiVersion, parameters, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
