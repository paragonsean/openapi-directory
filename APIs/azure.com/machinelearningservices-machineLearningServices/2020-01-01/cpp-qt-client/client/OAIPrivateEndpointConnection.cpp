/**
 * Azure Machine Learning Workspaces
 * These APIs allow end users to operate on Azure Machine Learning Workspace resources.
 *
 * The version of the OpenAPI document: 2020-01-01
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIPrivateEndpointConnection.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIPrivateEndpointConnection::OAIPrivateEndpointConnection(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIPrivateEndpointConnection::OAIPrivateEndpointConnection() {
    this->initializeModel();
}

OAIPrivateEndpointConnection::~OAIPrivateEndpointConnection() {}

void OAIPrivateEndpointConnection::initializeModel() {

    m_properties_isSet = false;
    m_properties_isValid = false;

    m_id_isSet = false;
    m_id_isValid = false;

    m_identity_isSet = false;
    m_identity_isValid = false;

    m_location_isSet = false;
    m_location_isValid = false;

    m_name_isSet = false;
    m_name_isValid = false;

    m_sku_isSet = false;
    m_sku_isValid = false;

    m_tags_isSet = false;
    m_tags_isValid = false;

    m_type_isSet = false;
    m_type_isValid = false;
}

void OAIPrivateEndpointConnection::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAIPrivateEndpointConnection::fromJsonObject(QJsonObject json) {

    m_properties_isValid = ::OpenAPI::fromJsonValue(m_properties, json[QString("properties")]);
    m_properties_isSet = !json[QString("properties")].isNull() && m_properties_isValid;

    m_id_isValid = ::OpenAPI::fromJsonValue(m_id, json[QString("id")]);
    m_id_isSet = !json[QString("id")].isNull() && m_id_isValid;

    m_identity_isValid = ::OpenAPI::fromJsonValue(m_identity, json[QString("identity")]);
    m_identity_isSet = !json[QString("identity")].isNull() && m_identity_isValid;

    m_location_isValid = ::OpenAPI::fromJsonValue(m_location, json[QString("location")]);
    m_location_isSet = !json[QString("location")].isNull() && m_location_isValid;

    m_name_isValid = ::OpenAPI::fromJsonValue(m_name, json[QString("name")]);
    m_name_isSet = !json[QString("name")].isNull() && m_name_isValid;

    m_sku_isValid = ::OpenAPI::fromJsonValue(m_sku, json[QString("sku")]);
    m_sku_isSet = !json[QString("sku")].isNull() && m_sku_isValid;

    m_tags_isValid = ::OpenAPI::fromJsonValue(m_tags, json[QString("tags")]);
    m_tags_isSet = !json[QString("tags")].isNull() && m_tags_isValid;

    m_type_isValid = ::OpenAPI::fromJsonValue(m_type, json[QString("type")]);
    m_type_isSet = !json[QString("type")].isNull() && m_type_isValid;
}

QString OAIPrivateEndpointConnection::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAIPrivateEndpointConnection::asJsonObject() const {
    QJsonObject obj;
    if (m_properties.isSet()) {
        obj.insert(QString("properties"), ::OpenAPI::toJsonValue(m_properties));
    }
    if (m_id_isSet) {
        obj.insert(QString("id"), ::OpenAPI::toJsonValue(m_id));
    }
    if (m_identity.isSet()) {
        obj.insert(QString("identity"), ::OpenAPI::toJsonValue(m_identity));
    }
    if (m_location_isSet) {
        obj.insert(QString("location"), ::OpenAPI::toJsonValue(m_location));
    }
    if (m_name_isSet) {
        obj.insert(QString("name"), ::OpenAPI::toJsonValue(m_name));
    }
    if (m_sku.isSet()) {
        obj.insert(QString("sku"), ::OpenAPI::toJsonValue(m_sku));
    }
    if (m_tags.size() > 0) {
        obj.insert(QString("tags"), ::OpenAPI::toJsonValue(m_tags));
    }
    if (m_type_isSet) {
        obj.insert(QString("type"), ::OpenAPI::toJsonValue(m_type));
    }
    return obj;
}

OAIPrivateEndpointConnectionProperties OAIPrivateEndpointConnection::getProperties() const {
    return m_properties;
}
void OAIPrivateEndpointConnection::setProperties(const OAIPrivateEndpointConnectionProperties &properties) {
    m_properties = properties;
    m_properties_isSet = true;
}

bool OAIPrivateEndpointConnection::is_properties_Set() const{
    return m_properties_isSet;
}

bool OAIPrivateEndpointConnection::is_properties_Valid() const{
    return m_properties_isValid;
}

QString OAIPrivateEndpointConnection::getId() const {
    return m_id;
}
void OAIPrivateEndpointConnection::setId(const QString &id) {
    m_id = id;
    m_id_isSet = true;
}

bool OAIPrivateEndpointConnection::is_id_Set() const{
    return m_id_isSet;
}

bool OAIPrivateEndpointConnection::is_id_Valid() const{
    return m_id_isValid;
}

OAIIdentity OAIPrivateEndpointConnection::getIdentity() const {
    return m_identity;
}
void OAIPrivateEndpointConnection::setIdentity(const OAIIdentity &identity) {
    m_identity = identity;
    m_identity_isSet = true;
}

bool OAIPrivateEndpointConnection::is_identity_Set() const{
    return m_identity_isSet;
}

bool OAIPrivateEndpointConnection::is_identity_Valid() const{
    return m_identity_isValid;
}

QString OAIPrivateEndpointConnection::getLocation() const {
    return m_location;
}
void OAIPrivateEndpointConnection::setLocation(const QString &location) {
    m_location = location;
    m_location_isSet = true;
}

bool OAIPrivateEndpointConnection::is_location_Set() const{
    return m_location_isSet;
}

bool OAIPrivateEndpointConnection::is_location_Valid() const{
    return m_location_isValid;
}

QString OAIPrivateEndpointConnection::getName() const {
    return m_name;
}
void OAIPrivateEndpointConnection::setName(const QString &name) {
    m_name = name;
    m_name_isSet = true;
}

bool OAIPrivateEndpointConnection::is_name_Set() const{
    return m_name_isSet;
}

bool OAIPrivateEndpointConnection::is_name_Valid() const{
    return m_name_isValid;
}

OAISku OAIPrivateEndpointConnection::getSku() const {
    return m_sku;
}
void OAIPrivateEndpointConnection::setSku(const OAISku &sku) {
    m_sku = sku;
    m_sku_isSet = true;
}

bool OAIPrivateEndpointConnection::is_sku_Set() const{
    return m_sku_isSet;
}

bool OAIPrivateEndpointConnection::is_sku_Valid() const{
    return m_sku_isValid;
}

QMap<QString, QString> OAIPrivateEndpointConnection::getTags() const {
    return m_tags;
}
void OAIPrivateEndpointConnection::setTags(const QMap<QString, QString> &tags) {
    m_tags = tags;
    m_tags_isSet = true;
}

bool OAIPrivateEndpointConnection::is_tags_Set() const{
    return m_tags_isSet;
}

bool OAIPrivateEndpointConnection::is_tags_Valid() const{
    return m_tags_isValid;
}

QString OAIPrivateEndpointConnection::getType() const {
    return m_type;
}
void OAIPrivateEndpointConnection::setType(const QString &type) {
    m_type = type;
    m_type_isSet = true;
}

bool OAIPrivateEndpointConnection::is_type_Set() const{
    return m_type_isSet;
}

bool OAIPrivateEndpointConnection::is_type_Valid() const{
    return m_type_isValid;
}

bool OAIPrivateEndpointConnection::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_properties.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_identity.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_location_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_name_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_sku.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_tags.size() > 0) {
            isObjectUpdated = true;
            break;
        }

        if (m_type_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAIPrivateEndpointConnection::isValid() const {
    // only required properties are required for the object to be considered valid
    return true;
}

} // namespace OpenAPI
