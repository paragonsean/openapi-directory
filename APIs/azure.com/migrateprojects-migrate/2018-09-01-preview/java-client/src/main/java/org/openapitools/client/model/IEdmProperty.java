...
public class IEdmProperty {
  ...
  public IEdmProperty declaringType(IEdmStructuredType declaringType) {
  ...
  public IEdmProperty type(IEdmTypeReference type) {
    this.type = type;
    return this;
  }
  ...
  public boolean equals(Object o) {
    ...
    IEdmProperty iedmProperty = (IEdmProperty) o;
    return Objects.equals(this.declaringType, iedmProperty.declaringType) &&
        Objects.equals(this.name, iedmProperty.name) &&
        Objects.equals(this.propertyKind, iedmProperty.propertyKind) &&
        Objects.equals(this.type, iedmProperty.type);
  }

  @Override
  public int hashCode() {
    return Objects.hash(declaringType, name, propertyKind, type);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to IEdmProperty
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!IEdmProperty.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in IEdmProperty is not found in the empty JSON string", IEdmProperty.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!IEdmProperty.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `IEdmProperty` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!IEdmProperty.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'IEdmProperty' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<IEdmProperty> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(IEdmProperty.class));

       return (TypeAdapter<T>) new TypeAdapter<IEdmProperty>() {
           @Override
           public void write(JsonWriter out, IEdmProperty value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public IEdmProperty read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of IEdmProperty given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of IEdmProperty
   * @throws IOException if the JSON string is invalid with respect to IEdmProperty
   */
  public static IEdmProperty fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, IEdmProperty.class);
  }

  /**
   * Convert an instance of IEdmProperty to an JSON string
   *
   * @return JSON string
   */
