/**
 * Azure Migrate Hub
 * Migrate your workloads to Azure.
 *
 * The version of the OpenAPI document: 2018-09-01-preview
 * Contact: azuremigratehub@microsoft.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIIEdmStructuralProperty.h
 *
 * 
 */

#ifndef OAIIEdmStructuralProperty_H
#define OAIIEdmStructuralProperty_H

#include <QJsonObject>

#include "OAIIEdmStructuredType.h"
#include "OAIIEdmTypeReference.h"
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {
class OAIIEdmStructuredType;
class OAIIEdmTypeReference;

class OAIIEdmStructuralProperty : public OAIObject {
public:
    OAIIEdmStructuralProperty();
    OAIIEdmStructuralProperty(QString json);
    ~OAIIEdmStructuralProperty() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    OAIIEdmStructuredType getDeclaringType() const;
    void setDeclaringType(const OAIIEdmStructuredType &declaring_type);
    bool is_declaring_type_Set() const;
    bool is_declaring_type_Valid() const;

    QString getDefaultValueString() const;
    void setDefaultValueString(const QString &default_value_string);
    bool is_default_value_string_Set() const;
    bool is_default_value_string_Valid() const;

    QString getName() const;
    void setName(const QString &name);
    bool is_name_Set() const;
    bool is_name_Valid() const;

    QString getPropertyKind() const;
    void setPropertyKind(const QString &property_kind);
    bool is_property_kind_Set() const;
    bool is_property_kind_Valid() const;

    OAIIEdmTypeReference getType() const;
    void setType(const OAIIEdmTypeReference &type);
    bool is_type_Set() const;
    bool is_type_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    OAIIEdmStructuredType m_declaring_type;
    bool m_declaring_type_isSet;
    bool m_declaring_type_isValid;

    QString m_default_value_string;
    bool m_default_value_string_isSet;
    bool m_default_value_string_isValid;

    QString m_name;
    bool m_name_isSet;
    bool m_name_isValid;

    QString m_property_kind;
    bool m_property_kind_isSet;
    bool m_property_kind_isValid;

    OAIIEdmTypeReference m_type;
    bool m_type_isSet;
    bool m_type_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIIEdmStructuralProperty)

#endif // OAIIEdmStructuralProperty_H
