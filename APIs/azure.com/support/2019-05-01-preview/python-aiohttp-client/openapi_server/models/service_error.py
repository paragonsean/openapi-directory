# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.service_error_detail import ServiceErrorDetail
from openapi_server import util


class ServiceError(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, code: str=None, details: List[ServiceErrorDetail]=None, message: str=None, target: str=None):
        """ServiceError - a model defined in OpenAPI

        :param code: The code of this ServiceError.
        :param details: The details of this ServiceError.
        :param message: The message of this ServiceError.
        :param target: The target of this ServiceError.
        """
        self.openapi_types = {
            'code': str,
            'details': List[ServiceErrorDetail],
            'message': str,
            'target': str
        }

        self.attribute_map = {
            'code': 'code',
            'details': 'details',
            'message': 'message',
            'target': 'target'
        }

        self._code = code
        self._details = details
        self._message = message
        self._target = target

    @classmethod
    def from_dict(cls, dikt: dict) -> 'ServiceError':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The ServiceError of this ServiceError.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def code(self):
        """Gets the code of this ServiceError.

        The error code.

        :return: The code of this ServiceError.
        :rtype: str
        """
        return self._code

    @code.setter
    def code(self, code):
        """Sets the code of this ServiceError.

        The error code.

        :param code: The code of this ServiceError.
        :type code: str
        """

        self._code = code

    @property
    def details(self):
        """Gets the details of this ServiceError.

        The list of error details.

        :return: The details of this ServiceError.
        :rtype: List[ServiceErrorDetail]
        """
        return self._details

    @details.setter
    def details(self, details):
        """Sets the details of this ServiceError.

        The list of error details.

        :param details: The details of this ServiceError.
        :type details: List[ServiceErrorDetail]
        """

        self._details = details

    @property
    def message(self):
        """Gets the message of this ServiceError.

        The error message.

        :return: The message of this ServiceError.
        :rtype: str
        """
        return self._message

    @message.setter
    def message(self, message):
        """Sets the message of this ServiceError.

        The error message.

        :param message: The message of this ServiceError.
        :type message: str
        """

        self._message = message

    @property
    def target(self):
        """Gets the target of this ServiceError.

        The target of the error.

        :return: The target of this ServiceError.
        :rtype: str
        """
        return self._target

    @target.setter
    def target(self, target):
        """Sets the target of this ServiceError.

        The target of the error.

        :param target: The target of this ServiceError.
        :type target: str
        """

        self._target = target
