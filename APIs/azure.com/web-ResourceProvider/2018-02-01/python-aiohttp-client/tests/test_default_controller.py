...
async def test_validate(client):
    ...
    """
    validate_request = {"name":"name","location":"location","type":"ServerFarm","properties":{"serverFarmId":"serverFarmId","skuName":"skuName","isSpot":True,"isXenon":True,"needLinuxWorkers":True,"capacity":1,"hostingEnvironment":"hostingEnvironment"}}
    params = [('api-version', 'api_version_example')]
    headers = { 
        'Accept': 'application/json',
        'Content-Type': 'application/json',
...
async def test_validate_container_settings(client):
    """Test case for validate_container_settings

    Validate if the container settings are correct.
    """
    validate_container_settings_request = {"baseUrl":"baseUrl","password":"password","tag":"tag","repository":"repository","platform":"platform","username":"username"}
    params = [('api-version', 'api_version_example')]
    headers = { 
        'Accept': 'application/json',
        'Content-Type': 'application/json',
    ...
    response = await client.request(
        method='POST',
        path='/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Web/validateContainerSettings'.format(resource_group_name='resource_group_name_example', subscription_id='subscription_id_example'),
        headers=headers,
        json=validate_container_settings_request,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
