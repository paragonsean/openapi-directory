# DevTestLabsClient.GalleryImagesApi
...
> ResponseWithContinuationGalleryImage galleryImagesList(subscriptionId, resourceGroupName, labName, apiVersion, opts)
...
let apiInstance = new DevTestLabsClient.GalleryImagesApi();
...
apiInstance.galleryImagesList(subscriptionId, resourceGroupName, labName, apiVersion, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
