/**
 * DevTestLabsClient
 * The DevTest Labs Client.
 *
 * The version of the OpenAPI document: 2016-05-15
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';

/**
 * The UserIdentityFragment model module.
 * @module model/UserIdentityFragment
 * @version 2016-05-15
 */
class UserIdentityFragment {
    /**
     * Constructs a new <code>UserIdentityFragment</code>.
     * Identity attributes of a lab user.
     * @alias module:model/UserIdentityFragment
     */
    constructor() { 
        
        UserIdentityFragment.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>UserIdentityFragment</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/UserIdentityFragment} obj Optional instance to populate.
     * @return {module:model/UserIdentityFragment} The populated <code>UserIdentityFragment</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new UserIdentityFragment();

            if (data.hasOwnProperty('appId')) {
                obj['appId'] = ApiClient.convertToType(data['appId'], 'String');
            }
            if (data.hasOwnProperty('objectId')) {
                obj['objectId'] = ApiClient.convertToType(data['objectId'], 'String');
            }
            if (data.hasOwnProperty('principalId')) {
                obj['principalId'] = ApiClient.convertToType(data['principalId'], 'String');
            }
            if (data.hasOwnProperty('principalName')) {
                obj['principalName'] = ApiClient.convertToType(data['principalName'], 'String');
            }
            if (data.hasOwnProperty('tenantId')) {
                obj['tenantId'] = ApiClient.convertToType(data['tenantId'], 'String');
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>UserIdentityFragment</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>UserIdentityFragment</code>.
     */
    static validateJSON(data) {
        // ensure the json data is a string
        if (data['appId'] && !(typeof data['appId'] === 'string' || data['appId'] instanceof String)) {
            throw new Error("Expected the field `appId` to be a primitive type in the JSON string but got " + data['appId']);
        }
        // ensure the json data is a string
        if (data['objectId'] && !(typeof data['objectId'] === 'string' || data['objectId'] instanceof String)) {
            throw new Error("Expected the field `objectId` to be a primitive type in the JSON string but got " + data['objectId']);
        }
        // ensure the json data is a string
        if (data['principalId'] && !(typeof data['principalId'] === 'string' || data['principalId'] instanceof String)) {
            throw new Error("Expected the field `principalId` to be a primitive type in the JSON string but got " + data['principalId']);
        }
        // ensure the json data is a string
        if (data['principalName'] && !(typeof data['principalName'] === 'string' || data['principalName'] instanceof String)) {
            throw new Error("Expected the field `principalName` to be a primitive type in the JSON string but got " + data['principalName']);
        }
        // ensure the json data is a string
        if (data['tenantId'] && !(typeof data['tenantId'] === 'string' || data['tenantId'] instanceof String)) {
            throw new Error("Expected the field `tenantId` to be a primitive type in the JSON string but got " + data['tenantId']);
        }

        return true;
    }


}



/**
 * Set to the app Id of the client JWT making the request.
 * @member {String} appId
 */
UserIdentityFragment.prototype['appId'] = undefined;

/**
 * Set to the object Id of the client JWT making the request. Not all users have object Id. For CSP (reseller) scenarios for example, object Id is not available.
 * @member {String} objectId
 */
UserIdentityFragment.prototype['objectId'] = undefined;

/**
 * Set to the principal Id of the client JWT making the request. Service principal will not have the principal Id.
 * @member {String} principalId
 */
UserIdentityFragment.prototype['principalId'] = undefined;

/**
 * Set to the principal name / UPN of the client JWT making the request.
 * @member {String} principalName
 */
UserIdentityFragment.prototype['principalName'] = undefined;

/**
 * Set to the tenant ID of the client JWT making the request.
 * @member {String} tenantId
 */
UserIdentityFragment.prototype['tenantId'] = undefined;






export default UserIdentityFragment;

