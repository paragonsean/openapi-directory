# NetworkManagementClient.PacketCaptureResult

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**etag** | **String** | A unique read-only string that changes whenever the resource is updated. | [optional] [readonly] 
**id** | **String** | ID of the packet capture operation. | [optional] [readonly] 
**name** | **String** | Name of the packet capture session. | [optional] [readonly] 
**properties** | [**PacketCaptureResultProperties**](PacketCaptureResultProperties.md) |  | [optional] 


