...
async def test_network_watchers_create_or_update(client):
    ...
    """
    parameters = {"name":"name","etag":"A unique read-only string that changes whenever the resource is updated.","id":"id","type":"type","properties":{"provisioningState":"Succeeded"}}
    params = [('api-version', 'api_version_example')]
    headers = { 
        'Accept': 'application/json',
        'Content-Type': 'application/json',
