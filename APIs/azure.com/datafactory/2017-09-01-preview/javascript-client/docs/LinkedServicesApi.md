...
> LinkedServiceResource linkedServicesGet(subscriptionId, resourceGroupName, factoryName, linkedServiceName, apiVersion)
...
let linkedServiceName = "linkedServiceName_example"; // String | The linked service name.
let apiVersion = "apiVersion_example"; // String | The API version.
apiInstance.linkedServicesGet(subscriptionId, resourceGroupName, factoryName, linkedServiceName, apiVersion, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
