...
async def test_billing_subscriptions_get_by_customer_name(client):
...
async def test_billing_subscriptions_list_by_billing_account_name(client):
...
async def test_billing_subscriptions_list_by_billing_profile_name(client):
...
async def test_billing_subscriptions_list_by_customer_name(client):
...
async def test_billing_subscriptions_list_by_invoice_section_name(client):
    ...
    response = await client.request(
        method='GET',
        path='/providers/Microsoft.Billing/billingAccounts/{billing_account_name}/invoiceSections/{invoice_section_name}/billingSubscriptions'.format(billing_account_name='billing_account_name_example', invoice_section_name='invoice_section_name_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
