...
async def test_billing_subscriptions_validate_transfer(client):
    ...
    response = await client.request(
        method='POST',
        path='/providers/Microsoft.Billing/billingAccounts/{billing_account_name}/invoiceSections/{invoice_section_name}/billingSubscriptions/{billing_subscription_name}/validateTransferEligibility'.format(billing_account_name='billing_account_name_example', invoice_section_name='invoice_section_name_example', billing_subscription_name='billing_subscription_name_example'),
        headers=headers,
        json=parameters,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_products_validate_transfer(client):
    ...
    response = await client.request(
        method='POST',
        path='/providers/Microsoft.Billing/billingAccounts/{billing_account_name}/invoiceSections/{invoice_section_name}/products/{product_name}/validateTransferEligibility'.format(billing_account_name='billing_account_name_example', invoice_section_name='invoice_section_name_example', product_name='product_name_example'),
        headers=headers,
        json=parameters,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
