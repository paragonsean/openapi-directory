# BillingManagementClient.LineOfCreditsApi
...
let apiInstance = new BillingManagementClient.LineOfCreditsApi();
...
apiInstance.lineOfCreditsGet(apiVersion, subscriptionId, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
let apiVersion = "apiVersion_example"; // String | Version of the API to be used with the client request. The current version is 2018-11-01-preview.
let subscriptionId = "subscriptionId_example"; // String | Azure Subscription Id.
let parameters = new BillingManagementClient.LineOfCredit(); // LineOfCredit | Parameters supplied to the increase line of credit operation.
apiInstance.lineOfCreditsUpdate(apiVersion, subscriptionId, parameters, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
