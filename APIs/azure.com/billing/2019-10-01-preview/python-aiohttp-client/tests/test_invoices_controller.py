...
async def test_invoices_get_by_id(client):
    ...
    response = await client.request(
        method='GET',
        path='/providers/Microsoft.Billing/billingAccounts/{billing_account_name}/billingSubscriptions/{billing_subscription_name}/invoices/{invoice_name}'.format(billing_account_name='billing_account_name_example', billing_subscription_name='billing_subscription_name_example', invoice_name='invoice_name_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_invoices_list_by_billing_account(client):
...
async def test_invoices_list_by_billing_subscription(client):
    ...
    response = await client.request(
        method='GET',
        path='/providers/Microsoft.Billing/billingAccounts/{billing_account_name}/billingSubscriptions/{billing_subscription_name}/invoices'.format(billing_account_name='billing_account_name_example', billing_subscription_name='billing_subscription_name_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
