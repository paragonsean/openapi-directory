...
async def test_recipient_transfers_accept(client):
    ...
    """
    parameters = {"properties":{"productDetails":[{"productId":"productId","productType":"AzureSubscription"},{"productId":"productId","productType":"AzureSubscription"}]}}
    headers = { 
...
async def test_recipient_transfers_validate(client):
    """Test case for recipient_transfers_validate
    ...
    response = await client.request(
        method='POST',
        path='/providers/Microsoft.Billing/transfers/{transfer_name}/validateTransfer'.format(transfer_name='transfer_name_example'),
        headers=headers,
        json=parameters,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
