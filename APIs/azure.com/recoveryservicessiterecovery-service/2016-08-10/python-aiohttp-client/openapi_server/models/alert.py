# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.alert_properties import AlertProperties
from openapi_server import util


class Alert(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, properties: AlertProperties=None, id: str=None, location: str=None, name: str=None, type: str=None):
        """Alert - a model defined in OpenAPI

        :param properties: The properties of this Alert.
        :param id: The id of this Alert.
        :param location: The location of this Alert.
        :param name: The name of this Alert.
        :param type: The type of this Alert.
        """
        self.openapi_types = {
            'properties': AlertProperties,
            'id': str,
            'location': str,
            'name': str,
            'type': str
        }

        self.attribute_map = {
            'properties': 'properties',
            'id': 'id',
            'location': 'location',
            'name': 'name',
            'type': 'type'
        }

        self._properties = properties
        self._id = id
        self._location = location
        self._name = name
        self._type = type

    @classmethod
    def from_dict(cls, dikt: dict) -> 'Alert':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The Alert of this Alert.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def properties(self):
        """Gets the properties of this Alert.


        :return: The properties of this Alert.
        :rtype: AlertProperties
        """
        return self._properties

    @properties.setter
    def properties(self, properties):
        """Sets the properties of this Alert.


        :param properties: The properties of this Alert.
        :type properties: AlertProperties
        """

        self._properties = properties

    @property
    def id(self):
        """Gets the id of this Alert.

        Resource Id

        :return: The id of this Alert.
        :rtype: str
        """
        return self._id

    @id.setter
    def id(self, id):
        """Sets the id of this Alert.

        Resource Id

        :param id: The id of this Alert.
        :type id: str
        """

        self._id = id

    @property
    def location(self):
        """Gets the location of this Alert.

        Resource Location

        :return: The location of this Alert.
        :rtype: str
        """
        return self._location

    @location.setter
    def location(self, location):
        """Sets the location of this Alert.

        Resource Location

        :param location: The location of this Alert.
        :type location: str
        """

        self._location = location

    @property
    def name(self):
        """Gets the name of this Alert.

        Resource Name

        :return: The name of this Alert.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name):
        """Sets the name of this Alert.

        Resource Name

        :param name: The name of this Alert.
        :type name: str
        """

        self._name = name

    @property
    def type(self):
        """Gets the type of this Alert.

        Resource Type

        :return: The type of this Alert.
        :rtype: str
        """
        return self._type

    @type.setter
    def type(self, type):
        """Sets the type of this Alert.

        Resource Type

        :param type: The type of this Alert.
        :type type: str
        """

        self._type = type
