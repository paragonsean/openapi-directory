# SiteRecoveryManagementClient.MasterTargetServer

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**agentExpiryDate** | **Date** | Agent expiry date. | [optional] 
**agentVersion** | **String** | The version of the scout component on the server. | [optional] 
**agentVersionDetails** | [**VersionDetails**](VersionDetails.md) |  | [optional] 
**dataStores** | [**[DataStore]**](DataStore.md) | The list of data stores in the fabric. | [optional] 
**diskCount** | **Number** | Disk count of the master target. | [optional] 
**healthErrors** | [**[HealthError]**](HealthError.md) | Health errors. | [optional] 
**id** | **String** | The server Id. | [optional] 
**ipAddress** | **String** | The IP address of the server. | [optional] 
**lastHeartbeat** | **Date** | The last heartbeat received from the server. | [optional] 
**marsAgentExpiryDate** | **Date** | MARS agent expiry date. | [optional] 
**marsAgentVersion** | **String** | MARS agent version. | [optional] 
**marsAgentVersionDetails** | [**VersionDetails**](VersionDetails.md) |  | [optional] 
**name** | **String** | The server name. | [optional] 
**osType** | **String** | The OS type of the server. | [optional] 
**osVersion** | **String** | OS Version of the master target. | [optional] 
**retentionVolumes** | [**[RetentionVolume]**](RetentionVolume.md) | The retention volumes of Master target Server. | [optional] 
**validationErrors** | [**[HealthError]**](HealthError.md) | Validation errors. | [optional] 
**versionStatus** | **String** | Version status | [optional] 


