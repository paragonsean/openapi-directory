# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.certificate_properties import CertificateProperties
from openapi_server import util


class Certificate(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, properties: CertificateProperties=None, id: str=None, name: str=None, type: str=None):
        """Certificate - a model defined in OpenAPI

        :param properties: The properties of this Certificate.
        :param id: The id of this Certificate.
        :param name: The name of this Certificate.
        :param type: The type of this Certificate.
        """
        self.openapi_types = {
            'properties': CertificateProperties,
            'id': str,
            'name': str,
            'type': str
        }

        self.attribute_map = {
            'properties': 'properties',
            'id': 'id',
            'name': 'name',
            'type': 'type'
        }

        self._properties = properties
        self._id = id
        self._name = name
        self._type = type

    @classmethod
    def from_dict(cls, dikt: dict) -> 'Certificate':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The Certificate of this Certificate.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def properties(self):
        """Gets the properties of this Certificate.


        :return: The properties of this Certificate.
        :rtype: CertificateProperties
        """
        return self._properties

    @properties.setter
    def properties(self, properties):
        """Sets the properties of this Certificate.


        :param properties: The properties of this Certificate.
        :type properties: CertificateProperties
        """

        self._properties = properties

    @property
    def id(self):
        """Gets the id of this Certificate.

        Fully qualified resource Id for the resource

        :return: The id of this Certificate.
        :rtype: str
        """
        return self._id

    @id.setter
    def id(self, id):
        """Sets the id of this Certificate.

        Fully qualified resource Id for the resource

        :param id: The id of this Certificate.
        :type id: str
        """

        self._id = id

    @property
    def name(self):
        """Gets the name of this Certificate.

        The name of the resource

        :return: The name of this Certificate.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name):
        """Sets the name of this Certificate.

        The name of the resource

        :param name: The name of this Certificate.
        :type name: str
        """

        self._name = name

    @property
    def type(self):
        """Gets the type of this Certificate.

        The type of the resource.

        :return: The type of this Certificate.
        :rtype: str
        """
        return self._type

    @type.setter
    def type(self, type):
        """Sets the type of this Certificate.

        The type of the resource.

        :param type: The type of this Certificate.
        :type type: str
        """

        self._type = type
