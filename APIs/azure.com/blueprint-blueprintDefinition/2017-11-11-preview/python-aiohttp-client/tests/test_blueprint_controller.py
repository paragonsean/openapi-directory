...
async def test_blueprints_create_or_update(client):
    ...
    response = await client.request(
        method='PUT',
        path='/providers/Microsoft.Management/managementGroups/{management_group_name}/providers/Microsoft.Blueprint/blueprints/{blueprint_name}'.format(management_group_name='management_group_name_example', blueprint_name='blueprint_name_example'),
        headers=headers,
        json=blueprint,
        params=params,
        )
...
async def test_blueprints_list(client):
    ...
    response = await client.request(
        method='GET',
        path='/providers/Microsoft.Management/managementGroups/{management_group_name}/providers/Microsoft.Blueprint/blueprints'.format(management_group_name='management_group_name_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
