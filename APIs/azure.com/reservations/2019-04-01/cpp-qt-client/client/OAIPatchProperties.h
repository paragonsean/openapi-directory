/**
 * Azure Reservation
 * This API describe Azure Reservation
 *
 * The version of the OpenAPI document: 2019-04-01
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIPatchProperties.h
 *
 * 
 */

#ifndef OAIPatchProperties_H
#define OAIPatchProperties_H

#include <QJsonObject>

#include "OAIAppliedScopeType.h"
#include "OAIInstanceFlexibility.h"
#include "OAIPatchProperties_renewProperties.h"
#include <QList>
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {
class OAIPatchProperties_renewProperties;

class OAIPatchProperties : public OAIObject {
public:
    OAIPatchProperties();
    OAIPatchProperties(QString json);
    ~OAIPatchProperties() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    OAIAppliedScopeType getAppliedScopeType() const;
    void setAppliedScopeType(const OAIAppliedScopeType &applied_scope_type);
    bool is_applied_scope_type_Set() const;
    bool is_applied_scope_type_Valid() const;

    QList<QString> getAppliedScopes() const;
    void setAppliedScopes(const QList<QString> &applied_scopes);
    bool is_applied_scopes_Set() const;
    bool is_applied_scopes_Valid() const;

    OAIInstanceFlexibility getInstanceFlexibility() const;
    void setInstanceFlexibility(const OAIInstanceFlexibility &instance_flexibility);
    bool is_instance_flexibility_Set() const;
    bool is_instance_flexibility_Valid() const;

    QString getName() const;
    void setName(const QString &name);
    bool is_name_Set() const;
    bool is_name_Valid() const;

    bool isRenew() const;
    void setRenew(const bool &renew);
    bool is_renew_Set() const;
    bool is_renew_Valid() const;

    OAIPatchProperties_renewProperties getRenewProperties() const;
    void setRenewProperties(const OAIPatchProperties_renewProperties &renew_properties);
    bool is_renew_properties_Set() const;
    bool is_renew_properties_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    OAIAppliedScopeType m_applied_scope_type;
    bool m_applied_scope_type_isSet;
    bool m_applied_scope_type_isValid;

    QList<QString> m_applied_scopes;
    bool m_applied_scopes_isSet;
    bool m_applied_scopes_isValid;

    OAIInstanceFlexibility m_instance_flexibility;
    bool m_instance_flexibility_isSet;
    bool m_instance_flexibility_isValid;

    QString m_name;
    bool m_name_isSet;
    bool m_name_isValid;

    bool m_renew;
    bool m_renew_isSet;
    bool m_renew_isValid;

    OAIPatchProperties_renewProperties m_renew_properties;
    bool m_renew_properties_isSet;
    bool m_renew_properties_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIPatchProperties)

#endif // OAIPatchProperties_H
