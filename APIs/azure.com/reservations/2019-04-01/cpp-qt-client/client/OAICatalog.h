/**
 * Azure Reservation
 * This API describe Azure Reservation
 *
 * The version of the OpenAPI document: 2019-04-01
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAICatalog.h
 *
 * 
 */

#ifndef OAICatalog_H
#define OAICatalog_H

#include <QJsonObject>

#include "OAIReservationBillingPlan.h"
#include "OAIReservationTerm.h"
#include "OAISkuProperty.h"
#include "OAISkuRestriction.h"
#include <QList>
#include <QMap>
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {
class OAISkuRestriction;
class OAISkuProperty;

class OAICatalog : public OAIObject {
public:
    OAICatalog();
    OAICatalog(QString json);
    ~OAICatalog() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QMap<QString, QList<OAIReservationBillingPlan>> getBillingPlans() const;
    void setBillingPlans(const QMap<QString, QList<OAIReservationBillingPlan>> &billing_plans);
    bool is_billing_plans_Set() const;
    bool is_billing_plans_Valid() const;

    QList<QString> getLocations() const;
    void setLocations(const QList<QString> &locations);
    bool is_locations_Set() const;
    bool is_locations_Valid() const;

    QString getName() const;
    void setName(const QString &name);
    bool is_name_Set() const;
    bool is_name_Valid() const;

    QString getResourceType() const;
    void setResourceType(const QString &resource_type);
    bool is_resource_type_Set() const;
    bool is_resource_type_Valid() const;

    QList<OAISkuRestriction> getRestrictions() const;
    void setRestrictions(const QList<OAISkuRestriction> &restrictions);
    bool is_restrictions_Set() const;
    bool is_restrictions_Valid() const;

    QList<OAISkuProperty> getSkuProperties() const;
    void setSkuProperties(const QList<OAISkuProperty> &sku_properties);
    bool is_sku_properties_Set() const;
    bool is_sku_properties_Valid() const;

    QList<OAIReservationTerm> getTerms() const;
    void setTerms(const QList<OAIReservationTerm> &terms);
    bool is_terms_Set() const;
    bool is_terms_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QMap<QString, QList<OAIReservationBillingPlan>> m_billing_plans;
    bool m_billing_plans_isSet;
    bool m_billing_plans_isValid;

    QList<QString> m_locations;
    bool m_locations_isSet;
    bool m_locations_isValid;

    QString m_name;
    bool m_name_isSet;
    bool m_name_isValid;

    QString m_resource_type;
    bool m_resource_type_isSet;
    bool m_resource_type_isValid;

    QList<OAISkuRestriction> m_restrictions;
    bool m_restrictions_isSet;
    bool m_restrictions_isValid;

    QList<OAISkuProperty> m_sku_properties;
    bool m_sku_properties_isSet;
    bool m_sku_properties_isValid;

    QList<OAIReservationTerm> m_terms;
    bool m_terms_isSet;
    bool m_terms_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAICatalog)

#endif // OAICatalog_H
