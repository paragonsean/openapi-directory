# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.policy_evaluation_details import PolicyEvaluationDetails
from openapi_server import util


class PolicyState(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, odata_context: str=None, odata_id: str=None, compliance_state: str=None, effective_parameters: str=None, is_compliant: bool=None, management_group_ids: str=None, policy_assignment_id: str=None, policy_assignment_name: str=None, policy_assignment_owner: str=None, policy_assignment_parameters: str=None, policy_assignment_scope: str=None, policy_definition_action: str=None, policy_definition_category: str=None, policy_definition_id: str=None, policy_definition_name: str=None, policy_definition_reference_id: str=None, policy_evaluation_details: PolicyEvaluationDetails=None, policy_set_definition_category: str=None, policy_set_definition_id: str=None, policy_set_definition_name: str=None, policy_set_definition_owner: str=None, policy_set_definition_parameters: str=None, resource_group: str=None, resource_id: str=None, resource_location: str=None, resource_tags: str=None, resource_type: str=None, subscription_id: str=None, timestamp: datetime=None):
        """PolicyState - a model defined in OpenAPI

        :param odata_context: The odata_context of this PolicyState.
        :param odata_id: The odata_id of this PolicyState.
        :param compliance_state: The compliance_state of this PolicyState.
        :param effective_parameters: The effective_parameters of this PolicyState.
        :param is_compliant: The is_compliant of this PolicyState.
        :param management_group_ids: The management_group_ids of this PolicyState.
        :param policy_assignment_id: The policy_assignment_id of this PolicyState.
        :param policy_assignment_name: The policy_assignment_name of this PolicyState.
        :param policy_assignment_owner: The policy_assignment_owner of this PolicyState.
        :param policy_assignment_parameters: The policy_assignment_parameters of this PolicyState.
        :param policy_assignment_scope: The policy_assignment_scope of this PolicyState.
        :param policy_definition_action: The policy_definition_action of this PolicyState.
        :param policy_definition_category: The policy_definition_category of this PolicyState.
        :param policy_definition_id: The policy_definition_id of this PolicyState.
        :param policy_definition_name: The policy_definition_name of this PolicyState.
        :param policy_definition_reference_id: The policy_definition_reference_id of this PolicyState.
        :param policy_evaluation_details: The policy_evaluation_details of this PolicyState.
        :param policy_set_definition_category: The policy_set_definition_category of this PolicyState.
        :param policy_set_definition_id: The policy_set_definition_id of this PolicyState.
        :param policy_set_definition_name: The policy_set_definition_name of this PolicyState.
        :param policy_set_definition_owner: The policy_set_definition_owner of this PolicyState.
        :param policy_set_definition_parameters: The policy_set_definition_parameters of this PolicyState.
        :param resource_group: The resource_group of this PolicyState.
        :param resource_id: The resource_id of this PolicyState.
        :param resource_location: The resource_location of this PolicyState.
        :param resource_tags: The resource_tags of this PolicyState.
        :param resource_type: The resource_type of this PolicyState.
        :param subscription_id: The subscription_id of this PolicyState.
        :param timestamp: The timestamp of this PolicyState.
        """
        self.openapi_types = {
            'odata_context': str,
            'odata_id': str,
            'compliance_state': str,
            'effective_parameters': str,
            'is_compliant': bool,
            'management_group_ids': str,
            'policy_assignment_id': str,
            'policy_assignment_name': str,
            'policy_assignment_owner': str,
            'policy_assignment_parameters': str,
            'policy_assignment_scope': str,
            'policy_definition_action': str,
            'policy_definition_category': str,
            'policy_definition_id': str,
            'policy_definition_name': str,
            'policy_definition_reference_id': str,
            'policy_evaluation_details': PolicyEvaluationDetails,
            'policy_set_definition_category': str,
            'policy_set_definition_id': str,
            'policy_set_definition_name': str,
            'policy_set_definition_owner': str,
            'policy_set_definition_parameters': str,
            'resource_group': str,
            'resource_id': str,
            'resource_location': str,
            'resource_tags': str,
            'resource_type': str,
            'subscription_id': str,
            'timestamp': datetime
        }

        self.attribute_map = {
            'odata_context': '@odata.context',
            'odata_id': '@odata.id',
            'compliance_state': 'complianceState',
            'effective_parameters': 'effectiveParameters',
            'is_compliant': 'isCompliant',
            'management_group_ids': 'managementGroupIds',
            'policy_assignment_id': 'policyAssignmentId',
            'policy_assignment_name': 'policyAssignmentName',
            'policy_assignment_owner': 'policyAssignmentOwner',
            'policy_assignment_parameters': 'policyAssignmentParameters',
            'policy_assignment_scope': 'policyAssignmentScope',
            'policy_definition_action': 'policyDefinitionAction',
            'policy_definition_category': 'policyDefinitionCategory',
            'policy_definition_id': 'policyDefinitionId',
            'policy_definition_name': 'policyDefinitionName',
            'policy_definition_reference_id': 'policyDefinitionReferenceId',
            'policy_evaluation_details': 'policyEvaluationDetails',
            'policy_set_definition_category': 'policySetDefinitionCategory',
            'policy_set_definition_id': 'policySetDefinitionId',
            'policy_set_definition_name': 'policySetDefinitionName',
            'policy_set_definition_owner': 'policySetDefinitionOwner',
            'policy_set_definition_parameters': 'policySetDefinitionParameters',
            'resource_group': 'resourceGroup',
            'resource_id': 'resourceId',
            'resource_location': 'resourceLocation',
            'resource_tags': 'resourceTags',
            'resource_type': 'resourceType',
            'subscription_id': 'subscriptionId',
            'timestamp': 'timestamp'
        }

        self._odata_context = odata_context
        self._odata_id = odata_id
        self._compliance_state = compliance_state
        self._effective_parameters = effective_parameters
        self._is_compliant = is_compliant
        self._management_group_ids = management_group_ids
        self._policy_assignment_id = policy_assignment_id
        self._policy_assignment_name = policy_assignment_name
        self._policy_assignment_owner = policy_assignment_owner
        self._policy_assignment_parameters = policy_assignment_parameters
        self._policy_assignment_scope = policy_assignment_scope
        self._policy_definition_action = policy_definition_action
        self._policy_definition_category = policy_definition_category
        self._policy_definition_id = policy_definition_id
        self._policy_definition_name = policy_definition_name
        self._policy_definition_reference_id = policy_definition_reference_id
        self._policy_evaluation_details = policy_evaluation_details
        self._policy_set_definition_category = policy_set_definition_category
        self._policy_set_definition_id = policy_set_definition_id
        self._policy_set_definition_name = policy_set_definition_name
        self._policy_set_definition_owner = policy_set_definition_owner
        self._policy_set_definition_parameters = policy_set_definition_parameters
        self._resource_group = resource_group
        self._resource_id = resource_id
        self._resource_location = resource_location
        self._resource_tags = resource_tags
        self._resource_type = resource_type
        self._subscription_id = subscription_id
        self._timestamp = timestamp

    @classmethod
    def from_dict(cls, dikt: dict) -> 'PolicyState':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The PolicyState of this PolicyState.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def odata_context(self):
        """Gets the odata_context of this PolicyState.

        OData context string; used by OData clients to resolve type information based on metadata.

        :return: The odata_context of this PolicyState.
        :rtype: str
        """
        return self._odata_context

    @odata_context.setter
    def odata_context(self, odata_context):
        """Sets the odata_context of this PolicyState.

        OData context string; used by OData clients to resolve type information based on metadata.

        :param odata_context: The odata_context of this PolicyState.
        :type odata_context: str
        """

        self._odata_context = odata_context

    @property
    def odata_id(self):
        """Gets the odata_id of this PolicyState.

        OData entity ID; always set to null since policy state records do not have an entity ID.

        :return: The odata_id of this PolicyState.
        :rtype: str
        """
        return self._odata_id

    @odata_id.setter
    def odata_id(self, odata_id):
        """Sets the odata_id of this PolicyState.

        OData entity ID; always set to null since policy state records do not have an entity ID.

        :param odata_id: The odata_id of this PolicyState.
        :type odata_id: str
        """

        self._odata_id = odata_id

    @property
    def compliance_state(self):
        """Gets the compliance_state of this PolicyState.

        Compliance state of the resource.

        :return: The compliance_state of this PolicyState.
        :rtype: str
        """
        return self._compliance_state

    @compliance_state.setter
    def compliance_state(self, compliance_state):
        """Sets the compliance_state of this PolicyState.

        Compliance state of the resource.

        :param compliance_state: The compliance_state of this PolicyState.
        :type compliance_state: str
        """

        self._compliance_state = compliance_state

    @property
    def effective_parameters(self):
        """Gets the effective_parameters of this PolicyState.

        Effective parameters for the policy assignment.

        :return: The effective_parameters of this PolicyState.
        :rtype: str
        """
        return self._effective_parameters

    @effective_parameters.setter
    def effective_parameters(self, effective_parameters):
        """Sets the effective_parameters of this PolicyState.

        Effective parameters for the policy assignment.

        :param effective_parameters: The effective_parameters of this PolicyState.
        :type effective_parameters: str
        """

        self._effective_parameters = effective_parameters

    @property
    def is_compliant(self):
        """Gets the is_compliant of this PolicyState.

        Flag which states whether the resource is compliant against the policy assignment it was evaluated against.

        :return: The is_compliant of this PolicyState.
        :rtype: bool
        """
        return self._is_compliant

    @is_compliant.setter
    def is_compliant(self, is_compliant):
        """Sets the is_compliant of this PolicyState.

        Flag which states whether the resource is compliant against the policy assignment it was evaluated against.

        :param is_compliant: The is_compliant of this PolicyState.
        :type is_compliant: bool
        """

        self._is_compliant = is_compliant

    @property
    def management_group_ids(self):
        """Gets the management_group_ids of this PolicyState.

        Comma separated list of management group IDs, which represent the hierarchy of the management groups the resource is under.

        :return: The management_group_ids of this PolicyState.
        :rtype: str
        """
        return self._management_group_ids

    @management_group_ids.setter
    def management_group_ids(self, management_group_ids):
        """Sets the management_group_ids of this PolicyState.

        Comma separated list of management group IDs, which represent the hierarchy of the management groups the resource is under.

        :param management_group_ids: The management_group_ids of this PolicyState.
        :type management_group_ids: str
        """

        self._management_group_ids = management_group_ids

    @property
    def policy_assignment_id(self):
        """Gets the policy_assignment_id of this PolicyState.

        Policy assignment ID.

        :return: The policy_assignment_id of this PolicyState.
        :rtype: str
        """
        return self._policy_assignment_id

    @policy_assignment_id.setter
    def policy_assignment_id(self, policy_assignment_id):
        """Sets the policy_assignment_id of this PolicyState.

        Policy assignment ID.

        :param policy_assignment_id: The policy_assignment_id of this PolicyState.
        :type policy_assignment_id: str
        """

        self._policy_assignment_id = policy_assignment_id

    @property
    def policy_assignment_name(self):
        """Gets the policy_assignment_name of this PolicyState.

        Policy assignment name.

        :return: The policy_assignment_name of this PolicyState.
        :rtype: str
        """
        return self._policy_assignment_name

    @policy_assignment_name.setter
    def policy_assignment_name(self, policy_assignment_name):
        """Sets the policy_assignment_name of this PolicyState.

        Policy assignment name.

        :param policy_assignment_name: The policy_assignment_name of this PolicyState.
        :type policy_assignment_name: str
        """

        self._policy_assignment_name = policy_assignment_name

    @property
    def policy_assignment_owner(self):
        """Gets the policy_assignment_owner of this PolicyState.

        Policy assignment owner.

        :return: The policy_assignment_owner of this PolicyState.
        :rtype: str
        """
        return self._policy_assignment_owner

    @policy_assignment_owner.setter
    def policy_assignment_owner(self, policy_assignment_owner):
        """Sets the policy_assignment_owner of this PolicyState.

        Policy assignment owner.

        :param policy_assignment_owner: The policy_assignment_owner of this PolicyState.
        :type policy_assignment_owner: str
        """

        self._policy_assignment_owner = policy_assignment_owner

    @property
    def policy_assignment_parameters(self):
        """Gets the policy_assignment_parameters of this PolicyState.

        Policy assignment parameters.

        :return: The policy_assignment_parameters of this PolicyState.
        :rtype: str
        """
        return self._policy_assignment_parameters

    @policy_assignment_parameters.setter
    def policy_assignment_parameters(self, policy_assignment_parameters):
        """Sets the policy_assignment_parameters of this PolicyState.

        Policy assignment parameters.

        :param policy_assignment_parameters: The policy_assignment_parameters of this PolicyState.
        :type policy_assignment_parameters: str
        """

        self._policy_assignment_parameters = policy_assignment_parameters

    @property
    def policy_assignment_scope(self):
        """Gets the policy_assignment_scope of this PolicyState.

        Policy assignment scope.

        :return: The policy_assignment_scope of this PolicyState.
        :rtype: str
        """
        return self._policy_assignment_scope

    @policy_assignment_scope.setter
    def policy_assignment_scope(self, policy_assignment_scope):
        """Sets the policy_assignment_scope of this PolicyState.

        Policy assignment scope.

        :param policy_assignment_scope: The policy_assignment_scope of this PolicyState.
        :type policy_assignment_scope: str
        """

        self._policy_assignment_scope = policy_assignment_scope

    @property
    def policy_definition_action(self):
        """Gets the policy_definition_action of this PolicyState.

        Policy definition action, i.e. effect.

        :return: The policy_definition_action of this PolicyState.
        :rtype: str
        """
        return self._policy_definition_action

    @policy_definition_action.setter
    def policy_definition_action(self, policy_definition_action):
        """Sets the policy_definition_action of this PolicyState.

        Policy definition action, i.e. effect.

        :param policy_definition_action: The policy_definition_action of this PolicyState.
        :type policy_definition_action: str
        """

        self._policy_definition_action = policy_definition_action

    @property
    def policy_definition_category(self):
        """Gets the policy_definition_category of this PolicyState.

        Policy definition category.

        :return: The policy_definition_category of this PolicyState.
        :rtype: str
        """
        return self._policy_definition_category

    @policy_definition_category.setter
    def policy_definition_category(self, policy_definition_category):
        """Sets the policy_definition_category of this PolicyState.

        Policy definition category.

        :param policy_definition_category: The policy_definition_category of this PolicyState.
        :type policy_definition_category: str
        """

        self._policy_definition_category = policy_definition_category

    @property
    def policy_definition_id(self):
        """Gets the policy_definition_id of this PolicyState.

        Policy definition ID.

        :return: The policy_definition_id of this PolicyState.
        :rtype: str
        """
        return self._policy_definition_id

    @policy_definition_id.setter
    def policy_definition_id(self, policy_definition_id):
        """Sets the policy_definition_id of this PolicyState.

        Policy definition ID.

        :param policy_definition_id: The policy_definition_id of this PolicyState.
        :type policy_definition_id: str
        """

        self._policy_definition_id = policy_definition_id

    @property
    def policy_definition_name(self):
        """Gets the policy_definition_name of this PolicyState.

        Policy definition name.

        :return: The policy_definition_name of this PolicyState.
        :rtype: str
        """
        return self._policy_definition_name

    @policy_definition_name.setter
    def policy_definition_name(self, policy_definition_name):
        """Sets the policy_definition_name of this PolicyState.

        Policy definition name.

        :param policy_definition_name: The policy_definition_name of this PolicyState.
        :type policy_definition_name: str
        """

        self._policy_definition_name = policy_definition_name

    @property
    def policy_definition_reference_id(self):
        """Gets the policy_definition_reference_id of this PolicyState.

        Reference ID for the policy definition inside the policy set, if the policy assignment is for a policy set.

        :return: The policy_definition_reference_id of this PolicyState.
        :rtype: str
        """
        return self._policy_definition_reference_id

    @policy_definition_reference_id.setter
    def policy_definition_reference_id(self, policy_definition_reference_id):
        """Sets the policy_definition_reference_id of this PolicyState.

        Reference ID for the policy definition inside the policy set, if the policy assignment is for a policy set.

        :param policy_definition_reference_id: The policy_definition_reference_id of this PolicyState.
        :type policy_definition_reference_id: str
        """

        self._policy_definition_reference_id = policy_definition_reference_id

    @property
    def policy_evaluation_details(self):
        """Gets the policy_evaluation_details of this PolicyState.


        :return: The policy_evaluation_details of this PolicyState.
        :rtype: PolicyEvaluationDetails
        """
        return self._policy_evaluation_details

    @policy_evaluation_details.setter
    def policy_evaluation_details(self, policy_evaluation_details):
        """Sets the policy_evaluation_details of this PolicyState.


        :param policy_evaluation_details: The policy_evaluation_details of this PolicyState.
        :type policy_evaluation_details: PolicyEvaluationDetails
        """

        self._policy_evaluation_details = policy_evaluation_details

    @property
    def policy_set_definition_category(self):
        """Gets the policy_set_definition_category of this PolicyState.

        Policy set definition category, if the policy assignment is for a policy set.

        :return: The policy_set_definition_category of this PolicyState.
        :rtype: str
        """
        return self._policy_set_definition_category

    @policy_set_definition_category.setter
    def policy_set_definition_category(self, policy_set_definition_category):
        """Sets the policy_set_definition_category of this PolicyState.

        Policy set definition category, if the policy assignment is for a policy set.

        :param policy_set_definition_category: The policy_set_definition_category of this PolicyState.
        :type policy_set_definition_category: str
        """

        self._policy_set_definition_category = policy_set_definition_category

    @property
    def policy_set_definition_id(self):
        """Gets the policy_set_definition_id of this PolicyState.

        Policy set definition ID, if the policy assignment is for a policy set.

        :return: The policy_set_definition_id of this PolicyState.
        :rtype: str
        """
        return self._policy_set_definition_id

    @policy_set_definition_id.setter
    def policy_set_definition_id(self, policy_set_definition_id):
        """Sets the policy_set_definition_id of this PolicyState.

        Policy set definition ID, if the policy assignment is for a policy set.

        :param policy_set_definition_id: The policy_set_definition_id of this PolicyState.
        :type policy_set_definition_id: str
        """

        self._policy_set_definition_id = policy_set_definition_id

    @property
    def policy_set_definition_name(self):
        """Gets the policy_set_definition_name of this PolicyState.

        Policy set definition name, if the policy assignment is for a policy set.

        :return: The policy_set_definition_name of this PolicyState.
        :rtype: str
        """
        return self._policy_set_definition_name

    @policy_set_definition_name.setter
    def policy_set_definition_name(self, policy_set_definition_name):
        """Sets the policy_set_definition_name of this PolicyState.

        Policy set definition name, if the policy assignment is for a policy set.

        :param policy_set_definition_name: The policy_set_definition_name of this PolicyState.
        :type policy_set_definition_name: str
        """

        self._policy_set_definition_name = policy_set_definition_name

    @property
    def policy_set_definition_owner(self):
        """Gets the policy_set_definition_owner of this PolicyState.

        Policy set definition owner, if the policy assignment is for a policy set.

        :return: The policy_set_definition_owner of this PolicyState.
        :rtype: str
        """
        return self._policy_set_definition_owner

    @policy_set_definition_owner.setter
    def policy_set_definition_owner(self, policy_set_definition_owner):
        """Sets the policy_set_definition_owner of this PolicyState.

        Policy set definition owner, if the policy assignment is for a policy set.

        :param policy_set_definition_owner: The policy_set_definition_owner of this PolicyState.
        :type policy_set_definition_owner: str
        """

        self._policy_set_definition_owner = policy_set_definition_owner

    @property
    def policy_set_definition_parameters(self):
        """Gets the policy_set_definition_parameters of this PolicyState.

        Policy set definition parameters, if the policy assignment is for a policy set.

        :return: The policy_set_definition_parameters of this PolicyState.
        :rtype: str
        """
        return self._policy_set_definition_parameters

    @policy_set_definition_parameters.setter
    def policy_set_definition_parameters(self, policy_set_definition_parameters):
        """Sets the policy_set_definition_parameters of this PolicyState.

        Policy set definition parameters, if the policy assignment is for a policy set.

        :param policy_set_definition_parameters: The policy_set_definition_parameters of this PolicyState.
        :type policy_set_definition_parameters: str
        """

        self._policy_set_definition_parameters = policy_set_definition_parameters

    @property
    def resource_group(self):
        """Gets the resource_group of this PolicyState.

        Resource group name.

        :return: The resource_group of this PolicyState.
        :rtype: str
        """
        return self._resource_group

    @resource_group.setter
    def resource_group(self, resource_group):
        """Sets the resource_group of this PolicyState.

        Resource group name.

        :param resource_group: The resource_group of this PolicyState.
        :type resource_group: str
        """

        self._resource_group = resource_group

    @property
    def resource_id(self):
        """Gets the resource_id of this PolicyState.

        Resource ID.

        :return: The resource_id of this PolicyState.
        :rtype: str
        """
        return self._resource_id

    @resource_id.setter
    def resource_id(self, resource_id):
        """Sets the resource_id of this PolicyState.

        Resource ID.

        :param resource_id: The resource_id of this PolicyState.
        :type resource_id: str
        """

        self._resource_id = resource_id

    @property
    def resource_location(self):
        """Gets the resource_location of this PolicyState.

        Resource location.

        :return: The resource_location of this PolicyState.
        :rtype: str
        """
        return self._resource_location

    @resource_location.setter
    def resource_location(self, resource_location):
        """Sets the resource_location of this PolicyState.

        Resource location.

        :param resource_location: The resource_location of this PolicyState.
        :type resource_location: str
        """

        self._resource_location = resource_location

    @property
    def resource_tags(self):
        """Gets the resource_tags of this PolicyState.

        List of resource tags.

        :return: The resource_tags of this PolicyState.
        :rtype: str
        """
        return self._resource_tags

    @resource_tags.setter
    def resource_tags(self, resource_tags):
        """Sets the resource_tags of this PolicyState.

        List of resource tags.

        :param resource_tags: The resource_tags of this PolicyState.
        :type resource_tags: str
        """

        self._resource_tags = resource_tags

    @property
    def resource_type(self):
        """Gets the resource_type of this PolicyState.

        Resource type.

        :return: The resource_type of this PolicyState.
        :rtype: str
        """
        return self._resource_type

    @resource_type.setter
    def resource_type(self, resource_type):
        """Sets the resource_type of this PolicyState.

        Resource type.

        :param resource_type: The resource_type of this PolicyState.
        :type resource_type: str
        """

        self._resource_type = resource_type

    @property
    def subscription_id(self):
        """Gets the subscription_id of this PolicyState.

        Subscription ID.

        :return: The subscription_id of this PolicyState.
        :rtype: str
        """
        return self._subscription_id

    @subscription_id.setter
    def subscription_id(self, subscription_id):
        """Sets the subscription_id of this PolicyState.

        Subscription ID.

        :param subscription_id: The subscription_id of this PolicyState.
        :type subscription_id: str
        """

        self._subscription_id = subscription_id

    @property
    def timestamp(self):
        """Gets the timestamp of this PolicyState.

        Timestamp for the policy state record.

        :return: The timestamp of this PolicyState.
        :rtype: datetime
        """
        return self._timestamp

    @timestamp.setter
    def timestamp(self, timestamp):
        """Sets the timestamp of this PolicyState.

        Timestamp for the policy state record.

        :param timestamp: The timestamp of this PolicyState.
        :type timestamp: datetime
        """

        self._timestamp = timestamp
