# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server import util


class SshPublicKey(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, certificate_data: str=None):
        """SshPublicKey - a model defined in OpenAPI

        :param certificate_data: The certificate_data of this SshPublicKey.
        """
        self.openapi_types = {
            'certificate_data': str
        }

        self.attribute_map = {
            'certificate_data': 'certificateData'
        }

        self._certificate_data = certificate_data

    @classmethod
    def from_dict(cls, dikt: dict) -> 'SshPublicKey':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The SshPublicKey of this SshPublicKey.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def certificate_data(self):
        """Gets the certificate_data of this SshPublicKey.

        The certificate for SSH.

        :return: The certificate_data of this SshPublicKey.
        :rtype: str
        """
        return self._certificate_data

    @certificate_data.setter
    def certificate_data(self, certificate_data):
        """Sets the certificate_data of this SshPublicKey.

        The certificate for SSH.

        :param certificate_data: The certificate_data of this SshPublicKey.
        :type certificate_data: str
        """

        self._certificate_data = certificate_data
