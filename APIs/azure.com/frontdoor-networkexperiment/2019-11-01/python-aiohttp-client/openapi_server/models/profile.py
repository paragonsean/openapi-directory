# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.profile_properties import ProfileProperties
from openapi_server import util


class Profile(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, etag: str=None, name: str=None, properties: ProfileProperties=None, id: str=None, location: str=None, tags: Dict[str, str]=None, type: str=None):
        """Profile - a model defined in OpenAPI

        :param etag: The etag of this Profile.
        :param name: The name of this Profile.
        :param properties: The properties of this Profile.
        :param id: The id of this Profile.
        :param location: The location of this Profile.
        :param tags: The tags of this Profile.
        :param type: The type of this Profile.
        """
        self.openapi_types = {
            'etag': str,
            'name': str,
            'properties': ProfileProperties,
            'id': str,
            'location': str,
            'tags': Dict[str, str],
            'type': str
        }

        self.attribute_map = {
            'etag': 'etag',
            'name': 'name',
            'properties': 'properties',
            'id': 'id',
            'location': 'location',
            'tags': 'tags',
            'type': 'type'
        }

        self._etag = etag
        self._name = name
        self._properties = properties
        self._id = id
        self._location = location
        self._tags = tags
        self._type = type

    @classmethod
    def from_dict(cls, dikt: dict) -> 'Profile':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The Profile of this Profile.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def etag(self):
        """Gets the etag of this Profile.

        Gets a unique read-only string that changes whenever the resource is updated.

        :return: The etag of this Profile.
        :rtype: str
        """
        return self._etag

    @etag.setter
    def etag(self, etag):
        """Sets the etag of this Profile.

        Gets a unique read-only string that changes whenever the resource is updated.

        :param etag: The etag of this Profile.
        :type etag: str
        """

        self._etag = etag

    @property
    def name(self):
        """Gets the name of this Profile.

        The name of the Profile

        :return: The name of this Profile.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name):
        """Sets the name of this Profile.

        The name of the Profile

        :param name: The name of this Profile.
        :type name: str
        """

        self._name = name

    @property
    def properties(self):
        """Gets the properties of this Profile.


        :return: The properties of this Profile.
        :rtype: ProfileProperties
        """
        return self._properties

    @properties.setter
    def properties(self, properties):
        """Sets the properties of this Profile.


        :param properties: The properties of this Profile.
        :type properties: ProfileProperties
        """

        self._properties = properties

    @property
    def id(self):
        """Gets the id of this Profile.

        Resource ID.

        :return: The id of this Profile.
        :rtype: str
        """
        return self._id

    @id.setter
    def id(self, id):
        """Sets the id of this Profile.

        Resource ID.

        :param id: The id of this Profile.
        :type id: str
        """

        self._id = id

    @property
    def location(self):
        """Gets the location of this Profile.

        Resource location.

        :return: The location of this Profile.
        :rtype: str
        """
        return self._location

    @location.setter
    def location(self, location):
        """Sets the location of this Profile.

        Resource location.

        :param location: The location of this Profile.
        :type location: str
        """

        self._location = location

    @property
    def tags(self):
        """Gets the tags of this Profile.

        Resource tags.

        :return: The tags of this Profile.
        :rtype: Dict[str, str]
        """
        return self._tags

    @tags.setter
    def tags(self, tags):
        """Sets the tags of this Profile.

        Resource tags.

        :param tags: The tags of this Profile.
        :type tags: Dict[str, str]
        """

        self._tags = tags

    @property
    def type(self):
        """Gets the type of this Profile.

        Resource type.

        :return: The type of this Profile.
        :rtype: str
        """
        return self._type

    @type.setter
    def type(self, type):
        """Sets the type of this Profile.

        Resource type.

        :param type: The type of this Profile.
        :type type: str
        """

        self._type = type
