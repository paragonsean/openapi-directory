...
async def test_virtual_machines_list_all(client):
    ...
    """
    params = [('api-version', 'api_version_example'),
                    ('statusOnly', 'status_only_example')]
    headers = { 
        'Accept': 'application/json',
        'Authorization': 'Bearer special-key',
    }
...
async def test_virtual_machines_reapply(client):
    """Test case for virtual_machines_reapply
    ...
    response = await client.request(
        method='POST',
        path='/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Compute/virtualMachines/{vm_name}/reapply'.format(resource_group_name='resource_group_name_example', vm_name='vm_name_example', subscription_id='subscription_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
