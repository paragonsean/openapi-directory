...
> ProductPolicyGet200Response productPolicyGet(resourceGroupName, serviceName, productId, policyId, apiVersion, subscriptionId, opts)
...
apiInstance.productPolicyGet(resourceGroupName, serviceName, productId, policyId, apiVersion, subscriptionId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
