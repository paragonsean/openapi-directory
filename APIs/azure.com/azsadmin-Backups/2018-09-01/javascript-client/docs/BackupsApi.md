...
let backup = "backup_example"; // String | Name of the backup.
let apiVersion = "'2018-09-01'"; // String | Client API version.
let restoreOptions = new BackupManagementClient.RestoreOptions(); // RestoreOptions | Restore options.
apiInstance.backupsRestore(subscriptionId, location, resourceGroupName, backup, apiVersion, restoreOptions, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
