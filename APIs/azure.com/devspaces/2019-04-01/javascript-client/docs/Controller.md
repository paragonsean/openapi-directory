# DevSpacesManagement.Controller

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**properties** | [**ControllerProperties**](ControllerProperties.md) |  | 
**sku** | [**Sku**](Sku.md) |  | 
**location** | **String** | Region where the Azure resource is located. | [optional] 
**tags** | **{String: String}** | Tags for the Azure resource. | [optional] 
**id** | **String** | Fully qualified resource Id for the resource. | [optional] [readonly] 
**name** | **String** | The name of the resource. | [optional] [readonly] 
**type** | **String** | The type of the resource. | [optional] [readonly] 


