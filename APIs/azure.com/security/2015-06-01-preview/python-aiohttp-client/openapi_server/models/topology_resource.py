# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.topology_resource_properties import TopologyResourceProperties
from openapi_server import util


class TopologyResource(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, properties: TopologyResourceProperties=None, id: str=None, name: str=None, type: str=None, location: str=None):
        """TopologyResource - a model defined in OpenAPI

        :param properties: The properties of this TopologyResource.
        :param id: The id of this TopologyResource.
        :param name: The name of this TopologyResource.
        :param type: The type of this TopologyResource.
        :param location: The location of this TopologyResource.
        """
        self.openapi_types = {
            'properties': TopologyResourceProperties,
            'id': str,
            'name': str,
            'type': str,
            'location': str
        }

        self.attribute_map = {
            'properties': 'properties',
            'id': 'id',
            'name': 'name',
            'type': 'type',
            'location': 'location'
        }

        self._properties = properties
        self._id = id
        self._name = name
        self._type = type
        self._location = location

    @classmethod
    def from_dict(cls, dikt: dict) -> 'TopologyResource':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The TopologyResource of this TopologyResource.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def properties(self):
        """Gets the properties of this TopologyResource.


        :return: The properties of this TopologyResource.
        :rtype: TopologyResourceProperties
        """
        return self._properties

    @properties.setter
    def properties(self, properties):
        """Sets the properties of this TopologyResource.


        :param properties: The properties of this TopologyResource.
        :type properties: TopologyResourceProperties
        """

        self._properties = properties

    @property
    def id(self):
        """Gets the id of this TopologyResource.

        Resource Id

        :return: The id of this TopologyResource.
        :rtype: str
        """
        return self._id

    @id.setter
    def id(self, id):
        """Sets the id of this TopologyResource.

        Resource Id

        :param id: The id of this TopologyResource.
        :type id: str
        """

        self._id = id

    @property
    def name(self):
        """Gets the name of this TopologyResource.

        Resource name

        :return: The name of this TopologyResource.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name):
        """Sets the name of this TopologyResource.

        Resource name

        :param name: The name of this TopologyResource.
        :type name: str
        """

        self._name = name

    @property
    def type(self):
        """Gets the type of this TopologyResource.

        Resource type

        :return: The type of this TopologyResource.
        :rtype: str
        """
        return self._type

    @type.setter
    def type(self, type):
        """Sets the type of this TopologyResource.

        Resource type

        :param type: The type of this TopologyResource.
        :type type: str
        """

        self._type = type

    @property
    def location(self):
        """Gets the location of this TopologyResource.

        Location where the resource is stored

        :return: The location of this TopologyResource.
        :rtype: str
        """
        return self._location

    @location.setter
    def location(self, location):
        """Sets the location of this TopologyResource.

        Location where the resource is stored

        :param location: The location of this TopologyResource.
        :type location: str
        """

        self._location = location
