/**
 * BatchService
 * A client for issuing REST requests to the Azure Batch service.
 *
 * The version of the OpenAPI document: 2018-12-01.8.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import PoolEndpointConfiguration from './PoolEndpointConfiguration';

/**
 * The NetworkConfiguration model module.
 * @module model/NetworkConfiguration
 * @version 2018-12-01.8.0
 */
class NetworkConfiguration {
    /**
     * Constructs a new <code>NetworkConfiguration</code>.
     * The network configuration for a pool.
     * @alias module:model/NetworkConfiguration
     */
    constructor() { 
        
        NetworkConfiguration.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>NetworkConfiguration</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/NetworkConfiguration} obj Optional instance to populate.
     * @return {module:model/NetworkConfiguration} The populated <code>NetworkConfiguration</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new NetworkConfiguration();

            if (data.hasOwnProperty('dynamicVNetAssignmentScope')) {
                obj['dynamicVNetAssignmentScope'] = ApiClient.convertToType(data['dynamicVNetAssignmentScope'], 'String');
            }
            if (data.hasOwnProperty('endpointConfiguration')) {
                obj['endpointConfiguration'] = PoolEndpointConfiguration.constructFromObject(data['endpointConfiguration']);
            }
            if (data.hasOwnProperty('subnetId')) {
                obj['subnetId'] = ApiClient.convertToType(data['subnetId'], 'String');
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>NetworkConfiguration</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>NetworkConfiguration</code>.
     */
    static validateJSON(data) {
        // ensure the json data is a string
        if (data['dynamicVNetAssignmentScope'] && !(typeof data['dynamicVNetAssignmentScope'] === 'string' || data['dynamicVNetAssignmentScope'] instanceof String)) {
            throw new Error("Expected the field `dynamicVNetAssignmentScope` to be a primitive type in the JSON string but got " + data['dynamicVNetAssignmentScope']);
        }
        // validate the optional field `endpointConfiguration`
        if (data['endpointConfiguration']) { // data not null
          PoolEndpointConfiguration.validateJSON(data['endpointConfiguration']);
        }
        // ensure the json data is a string
        if (data['subnetId'] && !(typeof data['subnetId'] === 'string' || data['subnetId'] instanceof String)) {
            throw new Error("Expected the field `subnetId` to be a primitive type in the JSON string but got " + data['subnetId']);
        }

        return true;
    }


}



/**
 * @member {module:model/NetworkConfiguration.DynamicVNetAssignmentScopeEnum} dynamicVNetAssignmentScope
 */
NetworkConfiguration.prototype['dynamicVNetAssignmentScope'] = undefined;

/**
 * @member {module:model/PoolEndpointConfiguration} endpointConfiguration
 */
NetworkConfiguration.prototype['endpointConfiguration'] = undefined;

/**
 * This is of the form /subscriptions/{subscription}/resourceGroups/{group}/providers/{provider}/virtualNetworks/{network}/subnets/{subnet}. The virtual network must be in the same region and subscription as the Azure Batch account. The specified subnet should have enough free IP addresses to accommodate the number of nodes in the pool. If the subnet doesn't have enough free IP addresses, the pool will partially allocate compute nodes, and a resize error will occur. For pools created with virtualMachineConfiguration only ARM virtual networks ('Microsoft.Network/virtualNetworks') are supported, but for pools created with cloudServiceConfiguration both ARM and classic virtual networks are supported. For more details, see: https://docs.microsoft.com/en-us/azure/batch/batch-api-basics#virtual-network-vnet-and-firewall-configuration
 * @member {String} subnetId
 */
NetworkConfiguration.prototype['subnetId'] = undefined;





/**
 * Allowed values for the <code>dynamicVNetAssignmentScope</code> property.
 * @enum {String}
 * @readonly
 */
NetworkConfiguration['DynamicVNetAssignmentScopeEnum'] = {

    /**
     * value: "none"
     * @const
     */
    "none": "none",

    /**
     * value: "job"
     * @const
     */
    "job": "job"
};



export default NetworkConfiguration;

