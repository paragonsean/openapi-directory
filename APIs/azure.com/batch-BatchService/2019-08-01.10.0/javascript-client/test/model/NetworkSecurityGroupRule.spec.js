/**
 * BatchService
 * A client for issuing REST requests to the Azure Batch service.
 *
 * The version of the OpenAPI document: 2019-08-01.10.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', process.cwd()+'/src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require(process.cwd()+'/src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.BatchService);
  }
}(this, function(expect, BatchService) {
  'use strict';

  var instance;

  beforeEach(function() {
    instance = new BatchService.NetworkSecurityGroupRule();
  });

  var getProperty = function(object, getter, property) {
    // Use getter method if present; otherwise, get the property directly.
    if (typeof object[getter] === 'function')
      return object[getter]();
    else
      return object[property];
  }

  var setProperty = function(object, setter, property, value) {
    // Use setter method if present; otherwise, set the property directly.
    if (typeof object[setter] === 'function')
      object[setter](value);
    else
      object[property] = value;
  }

  describe('NetworkSecurityGroupRule', function() {
    it('should create an instance of NetworkSecurityGroupRule', function() {
      // uncomment below and update the code to test NetworkSecurityGroupRule
      //var instance = new BatchService.NetworkSecurityGroupRule();
      //expect(instance).to.be.a(BatchService.NetworkSecurityGroupRule);
    });

    it('should have the property access (base name: "access")', function() {
      // uncomment below and update the code to test the property access
      //var instance = new BatchService.NetworkSecurityGroupRule();
      //expect(instance).to.be();
    });

    it('should have the property priority (base name: "priority")', function() {
      // uncomment below and update the code to test the property priority
      //var instance = new BatchService.NetworkSecurityGroupRule();
      //expect(instance).to.be();
    });

    it('should have the property sourceAddressPrefix (base name: "sourceAddressPrefix")', function() {
      // uncomment below and update the code to test the property sourceAddressPrefix
      //var instance = new BatchService.NetworkSecurityGroupRule();
      //expect(instance).to.be();
    });

    it('should have the property sourcePortRanges (base name: "sourcePortRanges")', function() {
      // uncomment below and update the code to test the property sourcePortRanges
      //var instance = new BatchService.NetworkSecurityGroupRule();
      //expect(instance).to.be();
    });

  });

}));
