...
> Zone zonesUpdate(resourceGroupName, zoneName, apiVersion, subscriptionId, parameters, opts)
...
let apiVersion = "apiVersion_example"; // String | The API version to use for this operation.
let subscriptionId = "subscriptionId_example"; // String | The ID of the target subscription.
let parameters = new DnsManagementClient.ZoneUpdate(); // ZoneUpdate | Parameters supplied to the Update operation.
let opts = {
  'ifMatch': "ifMatch_example" // String | The etag of the DNS zone. Omit this value to always overwrite the current zone. Specify the last-seen etag value to prevent accidentally overwriting any concurrent changes.
};
apiInstance.zonesUpdate(resourceGroupName, zoneName, apiVersion, subscriptionId, parameters, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
