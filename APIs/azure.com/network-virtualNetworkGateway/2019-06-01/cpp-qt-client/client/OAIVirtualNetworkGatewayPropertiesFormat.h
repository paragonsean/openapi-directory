/**
 * NetworkManagementClient
 * The Microsoft Azure Network management API provides a RESTful set of web services that interact with Microsoft Azure Networks service to manage your network resources. The API has entities that capture the relationship between an end user and the Microsoft Azure Networks service.
 *
 * The version of the OpenAPI document: 2019-06-01
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIVirtualNetworkGatewayPropertiesFormat.h
 *
 * VirtualNetworkGateway properties.
 */

#ifndef OAIVirtualNetworkGatewayPropertiesFormat_H
#define OAIVirtualNetworkGatewayPropertiesFormat_H

#include <QJsonObject>

#include "OAIBgpSettings.h"
#include "OAILocalNetworkGatewayPropertiesFormat_localNetworkAddressSpace.h"
#include "OAIVirtualNetworkGatewayConnectionListEntityPropertiesFormat_peer.h"
#include "OAIVirtualNetworkGatewayIPConfiguration.h"
#include "OAIVirtualNetworkGatewaySku.h"
#include "OAIVpnClientConfiguration.h"
#include <QList>
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {
class OAIBgpSettings;
class OAILocalNetworkGatewayPropertiesFormat_localNetworkAddressSpace;
class OAIVirtualNetworkGatewayConnectionListEntityPropertiesFormat_peer;
class OAIVirtualNetworkGatewayIPConfiguration;
class OAIVirtualNetworkGatewaySku;
class OAIVpnClientConfiguration;

class OAIVirtualNetworkGatewayPropertiesFormat : public OAIObject {
public:
    OAIVirtualNetworkGatewayPropertiesFormat();
    OAIVirtualNetworkGatewayPropertiesFormat(QString json);
    ~OAIVirtualNetworkGatewayPropertiesFormat() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    bool isActiveActive() const;
    void setActiveActive(const bool &active_active);
    bool is_active_active_Set() const;
    bool is_active_active_Valid() const;

    OAIBgpSettings getBgpSettings() const;
    void setBgpSettings(const OAIBgpSettings &bgp_settings);
    bool is_bgp_settings_Set() const;
    bool is_bgp_settings_Valid() const;

    OAILocalNetworkGatewayPropertiesFormat_localNetworkAddressSpace getCustomRoutes() const;
    void setCustomRoutes(const OAILocalNetworkGatewayPropertiesFormat_localNetworkAddressSpace &custom_routes);
    bool is_custom_routes_Set() const;
    bool is_custom_routes_Valid() const;

    bool isEnableBgp() const;
    void setEnableBgp(const bool &enable_bgp);
    bool is_enable_bgp_Set() const;
    bool is_enable_bgp_Valid() const;

    OAIVirtualNetworkGatewayConnectionListEntityPropertiesFormat_peer getGatewayDefaultSite() const;
    void setGatewayDefaultSite(const OAIVirtualNetworkGatewayConnectionListEntityPropertiesFormat_peer &gateway_default_site);
    bool is_gateway_default_site_Set() const;
    bool is_gateway_default_site_Valid() const;

    QString getGatewayType() const;
    void setGatewayType(const QString &gateway_type);
    bool is_gateway_type_Set() const;
    bool is_gateway_type_Valid() const;

    QList<OAIVirtualNetworkGatewayIPConfiguration> getIpConfigurations() const;
    void setIpConfigurations(const QList<OAIVirtualNetworkGatewayIPConfiguration> &ip_configurations);
    bool is_ip_configurations_Set() const;
    bool is_ip_configurations_Valid() const;

    QString getProvisioningState() const;
    void setProvisioningState(const QString &provisioning_state);
    bool is_provisioning_state_Set() const;
    bool is_provisioning_state_Valid() const;

    QString getResourceGuid() const;
    void setResourceGuid(const QString &resource_guid);
    bool is_resource_guid_Set() const;
    bool is_resource_guid_Valid() const;

    OAIVirtualNetworkGatewaySku getSku() const;
    void setSku(const OAIVirtualNetworkGatewaySku &sku);
    bool is_sku_Set() const;
    bool is_sku_Valid() const;

    OAIVpnClientConfiguration getVpnClientConfiguration() const;
    void setVpnClientConfiguration(const OAIVpnClientConfiguration &vpn_client_configuration);
    bool is_vpn_client_configuration_Set() const;
    bool is_vpn_client_configuration_Valid() const;

    QString getVpnType() const;
    void setVpnType(const QString &vpn_type);
    bool is_vpn_type_Set() const;
    bool is_vpn_type_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    bool m_active_active;
    bool m_active_active_isSet;
    bool m_active_active_isValid;

    OAIBgpSettings m_bgp_settings;
    bool m_bgp_settings_isSet;
    bool m_bgp_settings_isValid;

    OAILocalNetworkGatewayPropertiesFormat_localNetworkAddressSpace m_custom_routes;
    bool m_custom_routes_isSet;
    bool m_custom_routes_isValid;

    bool m_enable_bgp;
    bool m_enable_bgp_isSet;
    bool m_enable_bgp_isValid;

    OAIVirtualNetworkGatewayConnectionListEntityPropertiesFormat_peer m_gateway_default_site;
    bool m_gateway_default_site_isSet;
    bool m_gateway_default_site_isValid;

    QString m_gateway_type;
    bool m_gateway_type_isSet;
    bool m_gateway_type_isValid;

    QList<OAIVirtualNetworkGatewayIPConfiguration> m_ip_configurations;
    bool m_ip_configurations_isSet;
    bool m_ip_configurations_isValid;

    QString m_provisioning_state;
    bool m_provisioning_state_isSet;
    bool m_provisioning_state_isValid;

    QString m_resource_guid;
    bool m_resource_guid_isSet;
    bool m_resource_guid_isValid;

    OAIVirtualNetworkGatewaySku m_sku;
    bool m_sku_isSet;
    bool m_sku_isValid;

    OAIVpnClientConfiguration m_vpn_client_configuration;
    bool m_vpn_client_configuration_isSet;
    bool m_vpn_client_configuration_isValid;

    QString m_vpn_type;
    bool m_vpn_type_isSet;
    bool m_vpn_type_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIVirtualNetworkGatewayPropertiesFormat)

#endif // OAIVirtualNetworkGatewayPropertiesFormat_H
