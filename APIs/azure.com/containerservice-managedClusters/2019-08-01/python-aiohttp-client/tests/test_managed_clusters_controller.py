...
async def test_managed_clusters_create_or_update(client):
    ...
    """
    parameters = {"identity":{"tenantId":"tenantId","principalId":"principalId","type":"SystemAssigned"},"name":"name","location":"location","id":"id","type":"type","properties":{"agentPoolProfiles":[{"osDiskSizeGB":576,"count":0,"vnetSubnetID":"vnetSubnetID","availabilityZones":["availabilityZones","availabilityZones"],"enableAutoScaling":True,"orchestratorVersion":"orchestratorVersion","provisioningState":"provisioningState","type":"VirtualMachineScaleSets","maxCount":6,"enableNodePublicIP":True,"scaleSetEvictionPolicy":"Delete","nodeTaints":["nodeTaints","nodeTaints"],"scaleSetPriority":"Regular","osType":"Linux","name":"name","minCount":5,"vmSize":"Standard_A1","maxPods":1},{"osDiskSizeGB":576,"count":0,"vnetSubnetID":"vnetSubnetID","availabilityZones":["availabilityZones","availabilityZones"],"enableAutoScaling":True,"orchestratorVersion":"orchestratorVersion","provisioningState":"provisioningState","type":"VirtualMachineScaleSets","maxCount":6,"enableNodePublicIP":True,"scaleSetEvictionPolicy":"Delete","nodeTaints":["nodeTaints","nodeTaints"],"scaleSetPriority":"Regular","osType":"Linux","name":"name","minCount":5,"vmSize":"Standard_A1","maxPods":1}],"apiServerAccessProfile":{"enablePrivateCluster":True,"authorizedIPRanges":["authorizedIPRanges","authorizedIPRanges"]},"enableRBAC":True,"fqdn":"fqdn","servicePrincipalProfile":{"clientId":"clientId","secret":"secret"},"provisioningState":"provisioningState","nodeResourceGroup":"nodeResourceGroup","windowsProfile":{"adminUsername":"adminUsername","adminPassword":"adminPassword"},"addonProfiles":"{}","dnsPrefix":"dnsPrefix","networkProfile":{"loadBalancerSku":"standard","networkPlugin":"kubenet","dockerBridgeCidr":"172.17.0.1/16","dnsServiceIP":"10.0.0.10","networkPolicy":"calico","podCidr":"10.244.0.0/16","loadBalancerProfile":{"managedOutboundIPs":{"count":70},"outboundIPPrefixes":{"publicIPPrefixes":[{"id":"id"},{"id":"id"}]},"outboundIPs":{"publicIPs":[{"id":"id"},{"id":"id"}]},"effectiveOutboundIPs":[{"id":"id"},{"id":"id"}]},"serviceCidr":"10.0.0.0/16"},"aadProfile":{"serverAppSecret":"serverAppSecret","tenantID":"tenantID","serverAppID":"serverAppID","clientAppID":"clientAppID"},"enablePodSecurityPolicy":True,"maxAgentPools":2,"linuxProfile":{"adminUsername":"adminUsername","ssh":{"publicKeys":[{"keyData":"keyData"},{"keyData":"keyData"}]}},"kubernetesVersion":"kubernetesVersion"},"tags":{"key":"tags"}}
...
async def test_managed_clusters_rotate_cluster_certificates(client):
    ...
    response = await client.request(
        method='POST',
        path='/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.ContainerService/managedClusters/{resource_name}/rotateClusterCertificates'.format(subscription_id='subscription_id_example', resource_group_name='resource_group_name_example', resource_name='resource_name_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
