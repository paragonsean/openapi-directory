...
class ContainerServiceLinuxProfile(Model):
    ...
    def admin_username(self, admin_username):
        ...
        if admin_username is not None and not re.search(r'^[A-Za-z][-A-Za-z0-9_]*$', admin_username):
            raise ValueError("Invalid value for `admin_username`, must be a follow pattern or equal to `/^[A-Za-z][-A-Za-z0-9_]*$/`")
