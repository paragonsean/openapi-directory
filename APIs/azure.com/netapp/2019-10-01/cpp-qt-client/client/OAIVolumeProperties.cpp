/**
 * Microsoft NetApp
 * Microsoft NetApp Azure Resource Provider specification
 *
 * The version of the OpenAPI document: 2019-10-01
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIVolumeProperties.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIVolumeProperties::OAIVolumeProperties(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIVolumeProperties::OAIVolumeProperties() {
    this->initializeModel();
}

OAIVolumeProperties::~OAIVolumeProperties() {}

void OAIVolumeProperties::initializeModel() {

    m_baremetal_tenant_id_isSet = false;
    m_baremetal_tenant_id_isValid = false;

    m_creation_token_isSet = false;
    m_creation_token_isValid = false;

    m_data_protection_isSet = false;
    m_data_protection_isValid = false;

    m_export_policy_isSet = false;
    m_export_policy_isValid = false;

    m_file_system_id_isSet = false;
    m_file_system_id_isValid = false;

    m_is_restoring_isSet = false;
    m_is_restoring_isValid = false;

    m_mount_targets_isSet = false;
    m_mount_targets_isValid = false;

    m_protocol_types_isSet = false;
    m_protocol_types_isValid = false;

    m_provisioning_state_isSet = false;
    m_provisioning_state_isValid = false;

    m_service_level_isSet = false;
    m_service_level_isValid = false;

    m_snapshot_id_isSet = false;
    m_snapshot_id_isValid = false;

    m_subnet_id_isSet = false;
    m_subnet_id_isValid = false;

    m_usage_threshold_isSet = false;
    m_usage_threshold_isValid = false;

    m_volume_type_isSet = false;
    m_volume_type_isValid = false;
}

void OAIVolumeProperties::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAIVolumeProperties::fromJsonObject(QJsonObject json) {

    m_baremetal_tenant_id_isValid = ::OpenAPI::fromJsonValue(m_baremetal_tenant_id, json[QString("baremetalTenantId")]);
    m_baremetal_tenant_id_isSet = !json[QString("baremetalTenantId")].isNull() && m_baremetal_tenant_id_isValid;

    m_creation_token_isValid = ::OpenAPI::fromJsonValue(m_creation_token, json[QString("creationToken")]);
    m_creation_token_isSet = !json[QString("creationToken")].isNull() && m_creation_token_isValid;

    m_data_protection_isValid = ::OpenAPI::fromJsonValue(m_data_protection, json[QString("dataProtection")]);
    m_data_protection_isSet = !json[QString("dataProtection")].isNull() && m_data_protection_isValid;

    m_export_policy_isValid = ::OpenAPI::fromJsonValue(m_export_policy, json[QString("exportPolicy")]);
    m_export_policy_isSet = !json[QString("exportPolicy")].isNull() && m_export_policy_isValid;

    m_file_system_id_isValid = ::OpenAPI::fromJsonValue(m_file_system_id, json[QString("fileSystemId")]);
    m_file_system_id_isSet = !json[QString("fileSystemId")].isNull() && m_file_system_id_isValid;

    m_is_restoring_isValid = ::OpenAPI::fromJsonValue(m_is_restoring, json[QString("isRestoring")]);
    m_is_restoring_isSet = !json[QString("isRestoring")].isNull() && m_is_restoring_isValid;

    m_mount_targets_isValid = ::OpenAPI::fromJsonValue(m_mount_targets, json[QString("mountTargets")]);
    m_mount_targets_isSet = !json[QString("mountTargets")].isNull() && m_mount_targets_isValid;

    m_protocol_types_isValid = ::OpenAPI::fromJsonValue(m_protocol_types, json[QString("protocolTypes")]);
    m_protocol_types_isSet = !json[QString("protocolTypes")].isNull() && m_protocol_types_isValid;

    m_provisioning_state_isValid = ::OpenAPI::fromJsonValue(m_provisioning_state, json[QString("provisioningState")]);
    m_provisioning_state_isSet = !json[QString("provisioningState")].isNull() && m_provisioning_state_isValid;

    m_service_level_isValid = ::OpenAPI::fromJsonValue(m_service_level, json[QString("serviceLevel")]);
    m_service_level_isSet = !json[QString("serviceLevel")].isNull() && m_service_level_isValid;

    m_snapshot_id_isValid = ::OpenAPI::fromJsonValue(m_snapshot_id, json[QString("snapshotId")]);
    m_snapshot_id_isSet = !json[QString("snapshotId")].isNull() && m_snapshot_id_isValid;

    m_subnet_id_isValid = ::OpenAPI::fromJsonValue(m_subnet_id, json[QString("subnetId")]);
    m_subnet_id_isSet = !json[QString("subnetId")].isNull() && m_subnet_id_isValid;

    m_usage_threshold_isValid = ::OpenAPI::fromJsonValue(m_usage_threshold, json[QString("usageThreshold")]);
    m_usage_threshold_isSet = !json[QString("usageThreshold")].isNull() && m_usage_threshold_isValid;

    m_volume_type_isValid = ::OpenAPI::fromJsonValue(m_volume_type, json[QString("volumeType")]);
    m_volume_type_isSet = !json[QString("volumeType")].isNull() && m_volume_type_isValid;
}

QString OAIVolumeProperties::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAIVolumeProperties::asJsonObject() const {
    QJsonObject obj;
    if (m_baremetal_tenant_id_isSet) {
        obj.insert(QString("baremetalTenantId"), ::OpenAPI::toJsonValue(m_baremetal_tenant_id));
    }
    if (m_creation_token_isSet) {
        obj.insert(QString("creationToken"), ::OpenAPI::toJsonValue(m_creation_token));
    }
    if (m_data_protection.isSet()) {
        obj.insert(QString("dataProtection"), ::OpenAPI::toJsonValue(m_data_protection));
    }
    if (m_export_policy.isSet()) {
        obj.insert(QString("exportPolicy"), ::OpenAPI::toJsonValue(m_export_policy));
    }
    if (m_file_system_id_isSet) {
        obj.insert(QString("fileSystemId"), ::OpenAPI::toJsonValue(m_file_system_id));
    }
    if (m_is_restoring_isSet) {
        obj.insert(QString("isRestoring"), ::OpenAPI::toJsonValue(m_is_restoring));
    }
    if (m_mount_targets.size() > 0) {
        obj.insert(QString("mountTargets"), ::OpenAPI::toJsonValue(m_mount_targets));
    }
    if (m_protocol_types.size() > 0) {
        obj.insert(QString("protocolTypes"), ::OpenAPI::toJsonValue(m_protocol_types));
    }
    if (m_provisioning_state_isSet) {
        obj.insert(QString("provisioningState"), ::OpenAPI::toJsonValue(m_provisioning_state));
    }
    if (m_service_level_isSet) {
        obj.insert(QString("serviceLevel"), ::OpenAPI::toJsonValue(m_service_level));
    }
    if (m_snapshot_id_isSet) {
        obj.insert(QString("snapshotId"), ::OpenAPI::toJsonValue(m_snapshot_id));
    }
    if (m_subnet_id_isSet) {
        obj.insert(QString("subnetId"), ::OpenAPI::toJsonValue(m_subnet_id));
    }
    if (m_usage_threshold_isSet) {
        obj.insert(QString("usageThreshold"), ::OpenAPI::toJsonValue(m_usage_threshold));
    }
    if (m_volume_type_isSet) {
        obj.insert(QString("volumeType"), ::OpenAPI::toJsonValue(m_volume_type));
    }
    return obj;
}

QString OAIVolumeProperties::getBaremetalTenantId() const {
    return m_baremetal_tenant_id;
}
void OAIVolumeProperties::setBaremetalTenantId(const QString &baremetal_tenant_id) {
    m_baremetal_tenant_id = baremetal_tenant_id;
    m_baremetal_tenant_id_isSet = true;
}

bool OAIVolumeProperties::is_baremetal_tenant_id_Set() const{
    return m_baremetal_tenant_id_isSet;
}

bool OAIVolumeProperties::is_baremetal_tenant_id_Valid() const{
    return m_baremetal_tenant_id_isValid;
}

QString OAIVolumeProperties::getCreationToken() const {
    return m_creation_token;
}
void OAIVolumeProperties::setCreationToken(const QString &creation_token) {
    m_creation_token = creation_token;
    m_creation_token_isSet = true;
}

bool OAIVolumeProperties::is_creation_token_Set() const{
    return m_creation_token_isSet;
}

bool OAIVolumeProperties::is_creation_token_Valid() const{
    return m_creation_token_isValid;
}

OAIDataProtection OAIVolumeProperties::getDataProtection() const {
    return m_data_protection;
}
void OAIVolumeProperties::setDataProtection(const OAIDataProtection &data_protection) {
    m_data_protection = data_protection;
    m_data_protection_isSet = true;
}

bool OAIVolumeProperties::is_data_protection_Set() const{
    return m_data_protection_isSet;
}

bool OAIVolumeProperties::is_data_protection_Valid() const{
    return m_data_protection_isValid;
}

OAIExportPolicy OAIVolumeProperties::getExportPolicy() const {
    return m_export_policy;
}
void OAIVolumeProperties::setExportPolicy(const OAIExportPolicy &export_policy) {
    m_export_policy = export_policy;
    m_export_policy_isSet = true;
}

bool OAIVolumeProperties::is_export_policy_Set() const{
    return m_export_policy_isSet;
}

bool OAIVolumeProperties::is_export_policy_Valid() const{
    return m_export_policy_isValid;
}

QString OAIVolumeProperties::getFileSystemId() const {
    return m_file_system_id;
}
void OAIVolumeProperties::setFileSystemId(const QString &file_system_id) {
    m_file_system_id = file_system_id;
    m_file_system_id_isSet = true;
}

bool OAIVolumeProperties::is_file_system_id_Set() const{
    return m_file_system_id_isSet;
}

bool OAIVolumeProperties::is_file_system_id_Valid() const{
    return m_file_system_id_isValid;
}

bool OAIVolumeProperties::isIsRestoring() const {
    return m_is_restoring;
}
void OAIVolumeProperties::setIsRestoring(const bool &is_restoring) {
    m_is_restoring = is_restoring;
    m_is_restoring_isSet = true;
}

bool OAIVolumeProperties::is_is_restoring_Set() const{
    return m_is_restoring_isSet;
}

bool OAIVolumeProperties::is_is_restoring_Valid() const{
    return m_is_restoring_isValid;
}

QList<OAIMountTargetList> OAIVolumeProperties::getMountTargets() const {
    return m_mount_targets;
}
void OAIVolumeProperties::setMountTargets(const QList<OAIMountTargetList> &mount_targets) {
    m_mount_targets = mount_targets;
    m_mount_targets_isSet = true;
}

bool OAIVolumeProperties::is_mount_targets_Set() const{
    return m_mount_targets_isSet;
}

bool OAIVolumeProperties::is_mount_targets_Valid() const{
    return m_mount_targets_isValid;
}

QList<QString> OAIVolumeProperties::getProtocolTypes() const {
    return m_protocol_types;
}
void OAIVolumeProperties::setProtocolTypes(const QList<QString> &protocol_types) {
    m_protocol_types = protocol_types;
    m_protocol_types_isSet = true;
}

bool OAIVolumeProperties::is_protocol_types_Set() const{
    return m_protocol_types_isSet;
}

bool OAIVolumeProperties::is_protocol_types_Valid() const{
    return m_protocol_types_isValid;
}

QString OAIVolumeProperties::getProvisioningState() const {
    return m_provisioning_state;
}
void OAIVolumeProperties::setProvisioningState(const QString &provisioning_state) {
    m_provisioning_state = provisioning_state;
    m_provisioning_state_isSet = true;
}

bool OAIVolumeProperties::is_provisioning_state_Set() const{
    return m_provisioning_state_isSet;
}

bool OAIVolumeProperties::is_provisioning_state_Valid() const{
    return m_provisioning_state_isValid;
}

QString OAIVolumeProperties::getServiceLevel() const {
    return m_service_level;
}
void OAIVolumeProperties::setServiceLevel(const QString &service_level) {
    m_service_level = service_level;
    m_service_level_isSet = true;
}

bool OAIVolumeProperties::is_service_level_Set() const{
    return m_service_level_isSet;
}

bool OAIVolumeProperties::is_service_level_Valid() const{
    return m_service_level_isValid;
}

QString OAIVolumeProperties::getSnapshotId() const {
    return m_snapshot_id;
}
void OAIVolumeProperties::setSnapshotId(const QString &snapshot_id) {
    m_snapshot_id = snapshot_id;
    m_snapshot_id_isSet = true;
}

bool OAIVolumeProperties::is_snapshot_id_Set() const{
    return m_snapshot_id_isSet;
}

bool OAIVolumeProperties::is_snapshot_id_Valid() const{
    return m_snapshot_id_isValid;
}

QString OAIVolumeProperties::getSubnetId() const {
    return m_subnet_id;
}
void OAIVolumeProperties::setSubnetId(const QString &subnet_id) {
    m_subnet_id = subnet_id;
    m_subnet_id_isSet = true;
}

bool OAIVolumeProperties::is_subnet_id_Set() const{
    return m_subnet_id_isSet;
}

bool OAIVolumeProperties::is_subnet_id_Valid() const{
    return m_subnet_id_isValid;
}

qint64 OAIVolumeProperties::getUsageThreshold() const {
    return m_usage_threshold;
}
void OAIVolumeProperties::setUsageThreshold(const qint64 &usage_threshold) {
    m_usage_threshold = usage_threshold;
    m_usage_threshold_isSet = true;
}

bool OAIVolumeProperties::is_usage_threshold_Set() const{
    return m_usage_threshold_isSet;
}

bool OAIVolumeProperties::is_usage_threshold_Valid() const{
    return m_usage_threshold_isValid;
}

QString OAIVolumeProperties::getVolumeType() const {
    return m_volume_type;
}
void OAIVolumeProperties::setVolumeType(const QString &volume_type) {
    m_volume_type = volume_type;
    m_volume_type_isSet = true;
}

bool OAIVolumeProperties::is_volume_type_Set() const{
    return m_volume_type_isSet;
}

bool OAIVolumeProperties::is_volume_type_Valid() const{
    return m_volume_type_isValid;
}

bool OAIVolumeProperties::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_baremetal_tenant_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_creation_token_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_data_protection.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_export_policy.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_file_system_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_is_restoring_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_mount_targets.size() > 0) {
            isObjectUpdated = true;
            break;
        }

        if (m_protocol_types.size() > 0) {
            isObjectUpdated = true;
            break;
        }

        if (m_provisioning_state_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_service_level_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_snapshot_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_subnet_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_usage_threshold_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_volume_type_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAIVolumeProperties::isValid() const {
    // only required properties are required for the object to be considered valid
    return m_creation_token_isValid && m_subnet_id_isValid && m_usage_threshold_isValid && true;
}

} // namespace OpenAPI
