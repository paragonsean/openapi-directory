/**
 * LogicManagementClient
 * REST API for Azure Logic Apps.
 *
 * The version of the OpenAPI document: 2015-02-01-preview
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIWorkflowProperties.h
 *
 * 
 */

#ifndef OAIWorkflowProperties_H
#define OAIWorkflowProperties_H

#include <QJsonObject>

#include "OAIContentLink.h"
#include "OAIObject.h"
#include "OAISku.h"
#include "OAIWorkflowParameter.h"
#include "OAIWorkflowProvisioningState.h"
#include "OAIWorkflowState.h"
#include <QDateTime>
#include <QMap>
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {
class OAIContentLink;
class OAIWorkflowParameter;
class OAISku;

class OAIWorkflowProperties : public OAIObject {
public:
    OAIWorkflowProperties();
    OAIWorkflowProperties(QString json);
    ~OAIWorkflowProperties() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QString getAccessEndpoint() const;
    void setAccessEndpoint(const QString &access_endpoint);
    bool is_access_endpoint_Set() const;
    bool is_access_endpoint_Valid() const;

    QDateTime getChangedTime() const;
    void setChangedTime(const QDateTime &changed_time);
    bool is_changed_time_Set() const;
    bool is_changed_time_Valid() const;

    QDateTime getCreatedTime() const;
    void setCreatedTime(const QDateTime &created_time);
    bool is_created_time_Set() const;
    bool is_created_time_Valid() const;

    OAIObject getDefinition() const;
    void setDefinition(const OAIObject &definition);
    bool is_definition_Set() const;
    bool is_definition_Valid() const;

    OAIContentLink getDefinitionLink() const;
    void setDefinitionLink(const OAIContentLink &definition_link);
    bool is_definition_link_Set() const;
    bool is_definition_link_Valid() const;

    QMap<QString, OAIWorkflowParameter> getParameters() const;
    void setParameters(const QMap<QString, OAIWorkflowParameter> &parameters);
    bool is_parameters_Set() const;
    bool is_parameters_Valid() const;

    OAIContentLink getParametersLink() const;
    void setParametersLink(const OAIContentLink &parameters_link);
    bool is_parameters_link_Set() const;
    bool is_parameters_link_Valid() const;

    OAIWorkflowProvisioningState getProvisioningState() const;
    void setProvisioningState(const OAIWorkflowProvisioningState &provisioning_state);
    bool is_provisioning_state_Set() const;
    bool is_provisioning_state_Valid() const;

    OAISku getSku() const;
    void setSku(const OAISku &sku);
    bool is_sku_Set() const;
    bool is_sku_Valid() const;

    OAIWorkflowState getState() const;
    void setState(const OAIWorkflowState &state);
    bool is_state_Set() const;
    bool is_state_Valid() const;

    QString getVersion() const;
    void setVersion(const QString &version);
    bool is_version_Set() const;
    bool is_version_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString m_access_endpoint;
    bool m_access_endpoint_isSet;
    bool m_access_endpoint_isValid;

    QDateTime m_changed_time;
    bool m_changed_time_isSet;
    bool m_changed_time_isValid;

    QDateTime m_created_time;
    bool m_created_time_isSet;
    bool m_created_time_isValid;

    OAIObject m_definition;
    bool m_definition_isSet;
    bool m_definition_isValid;

    OAIContentLink m_definition_link;
    bool m_definition_link_isSet;
    bool m_definition_link_isValid;

    QMap<QString, OAIWorkflowParameter> m_parameters;
    bool m_parameters_isSet;
    bool m_parameters_isValid;

    OAIContentLink m_parameters_link;
    bool m_parameters_link_isSet;
    bool m_parameters_link_isValid;

    OAIWorkflowProvisioningState m_provisioning_state;
    bool m_provisioning_state_isSet;
    bool m_provisioning_state_isValid;

    OAISku m_sku;
    bool m_sku_isSet;
    bool m_sku_isValid;

    OAIWorkflowState m_state;
    bool m_state_isSet;
    bool m_state_isValid;

    QString m_version;
    bool m_version_isSet;
    bool m_version_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIWorkflowProperties)

#endif // OAIWorkflowProperties_H
