...
> WorkflowTriggerCallbackUrl workflowVersionsListCallbackUrl(subscriptionId, resourceGroupName, workflowName, versionId, triggerName, apiVersion, opts)
...
apiInstance.workflowVersionsListCallbackUrl(subscriptionId, resourceGroupName, workflowName, versionId, triggerName, apiVersion, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
