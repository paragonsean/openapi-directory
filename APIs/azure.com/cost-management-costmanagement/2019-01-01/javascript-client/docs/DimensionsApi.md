...
> DimensionsListResult dimensionsListBySubscription(scope, apiVersion, opts)
...
apiInstance.dimensionsListBySubscription(scope, apiVersion, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
