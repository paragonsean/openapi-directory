# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.report_properties import ReportProperties
from openapi_server import util


class Report(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, properties: ReportProperties=None, id: str=None, name: str=None, tags: Dict[str, str]=None, type: str=None):
        """Report - a model defined in OpenAPI

        :param properties: The properties of this Report.
        :param id: The id of this Report.
        :param name: The name of this Report.
        :param tags: The tags of this Report.
        :param type: The type of this Report.
        """
        self.openapi_types = {
            'properties': ReportProperties,
            'id': str,
            'name': str,
            'tags': Dict[str, str],
            'type': str
        }

        self.attribute_map = {
            'properties': 'properties',
            'id': 'id',
            'name': 'name',
            'tags': 'tags',
            'type': 'type'
        }

        self._properties = properties
        self._id = id
        self._name = name
        self._tags = tags
        self._type = type

    @classmethod
    def from_dict(cls, dikt: dict) -> 'Report':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The Report of this Report.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def properties(self):
        """Gets the properties of this Report.


        :return: The properties of this Report.
        :rtype: ReportProperties
        """
        return self._properties

    @properties.setter
    def properties(self, properties):
        """Sets the properties of this Report.


        :param properties: The properties of this Report.
        :type properties: ReportProperties
        """

        self._properties = properties

    @property
    def id(self):
        """Gets the id of this Report.

        Resource Id.

        :return: The id of this Report.
        :rtype: str
        """
        return self._id

    @id.setter
    def id(self, id):
        """Sets the id of this Report.

        Resource Id.

        :param id: The id of this Report.
        :type id: str
        """

        self._id = id

    @property
    def name(self):
        """Gets the name of this Report.

        Resource name.

        :return: The name of this Report.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name):
        """Sets the name of this Report.

        Resource name.

        :param name: The name of this Report.
        :type name: str
        """

        self._name = name

    @property
    def tags(self):
        """Gets the tags of this Report.

        Resource tags.

        :return: The tags of this Report.
        :rtype: Dict[str, str]
        """
        return self._tags

    @tags.setter
    def tags(self, tags):
        """Sets the tags of this Report.

        Resource tags.

        :param tags: The tags of this Report.
        :type tags: Dict[str, str]
        """

        self._tags = tags

    @property
    def type(self):
        """Gets the type of this Report.

        Resource type.

        :return: The type of this Report.
        :rtype: str
        """
        return self._type

    @type.setter
    def type(self, type):
        """Sets the type of this Report.

        Resource type.

        :param type: The type of this Report.
        :type type: str
        """

        self._type = type
