...
> userConfirmationPasswordSend(resourceGroupName, serviceName, userId, apiVersion, subscriptionId)
...
apiInstance.userConfirmationPasswordSend(resourceGroupName, serviceName, userId, apiVersion, subscriptionId, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
