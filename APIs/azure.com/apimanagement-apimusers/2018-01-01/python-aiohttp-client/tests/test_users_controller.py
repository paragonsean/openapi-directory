...
async def test_user_create_or_update(client):
    ...
    """
    parameters = {"properties":{"firstName":"firstName","lastName":"lastName","password":"password","confirmation":"signup","email":"email"}}
...
async def test_user_get_identity(client):
    """Test case for user_get_identity
    ...
    response = await client.request(
        method='GET',
        path='/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.ApiManagement/service/{service_name}/identity'.format(resource_group_name='resource_group_name_example', service_name='service_name_example', subscription_id='subscription_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_user_update(client):
    ...
    """
    parameters = {"properties":{"firstName":"firstName","lastName":"lastName","password":"password","email":"email"}}
    params = [('api-version', 'api_version_example')]
    headers = { 
        'Accept': 'application/json',
        'Content-Type': 'application/json',
