...
let galleryName = "galleryName_example"; // String | The name of the Shared Image Gallery. The allowed characters are alphabets and numbers with dots and periods allowed in the middle. The maximum length is 80 characters.
let apiVersion = "apiVersion_example"; // String | Client Api Version.
let gallery = new SharedImageGalleryServiceClient.GalleryUpdate(); // GalleryUpdate | Parameters supplied to the update Shared Image Gallery operation.
apiInstance.galleriesUpdate(subscriptionId, resourceGroupName, galleryName, apiVersion, gallery, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
