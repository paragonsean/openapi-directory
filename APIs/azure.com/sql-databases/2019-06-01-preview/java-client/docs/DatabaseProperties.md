...
|**readReplicaCount** | **Integer** | The number of readonly secondary replicas associated with the database. |  [optional] |
|**readScale** | [**ReadScaleEnum**](#ReadScaleEnum) | The state of read-only routing. If enabled, connections that have application intent set to readonly in their connection string may be routed to a readonly secondary replica in the same region. |  [optional] |
...
|**storageAccountType** | [**StorageAccountTypeEnum**](#StorageAccountTypeEnum) | The storage account type used to store backups for this database. Currently the only supported option is GRS (GeoRedundantStorage). |  [optional] |
