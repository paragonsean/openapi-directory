...
async def test_role_definitions_create_or_update(client):
    ...
    """
    role_definition = {"name":"name","id":"id","type":"type","properties":{"permissions":[{"notActions":["notActions","notActions"],"actions":["actions","actions"]},{"notActions":["notActions","notActions"],"actions":["actions","actions"]}],"roleName":"roleName","description":"description","type":"type","assignableScopes":["assignableScopes","assignableScopes"]}}
...
async def test_role_definitions_get_by_id(client):
    ...
    response = await client.request(
        method='GET',
        path='/{role_definition_id}'.format(role_definition_id='role_definition_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
