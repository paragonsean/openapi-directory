...
[**jobsUpdate**](DefaultApi.md#jobsUpdate) | **PATCH** /subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Media/mediaServices/{accountName}/transforms/{transformName}/jobs/{jobName} | Update Job
...
let opts = {
  ...
  'orderby': "orderby_example" // String | Specifies the by which the result collection should be ordered.
};
 ...
 **orderby** | **String**| Specifies the by which the result collection should be ordered. | [optional] 
...
> Job jobsUpdate(subscriptionId, resourceGroupName, accountName, transformName, jobName, apiVersion, parameters)
...
let apiVersion = "apiVersion_example"; // String | The Version of the API to be used with the client request.
let parameters = new AzureMediaServices.Job(); // Job | The request parameters
apiInstance.jobsUpdate(subscriptionId, resourceGroupName, accountName, transformName, jobName, apiVersion, parameters, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
