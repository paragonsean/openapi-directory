# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.build_properties import BuildProperties
from openapi_server import util


class Build(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, properties: BuildProperties=None, id: str=None, name: str=None, type: str=None):
        """Build - a model defined in OpenAPI

        :param properties: The properties of this Build.
        :param id: The id of this Build.
        :param name: The name of this Build.
        :param type: The type of this Build.
        """
        self.openapi_types = {
            'properties': BuildProperties,
            'id': str,
            'name': str,
            'type': str
        }

        self.attribute_map = {
            'properties': 'properties',
            'id': 'id',
            'name': 'name',
            'type': 'type'
        }

        self._properties = properties
        self._id = id
        self._name = name
        self._type = type

    @classmethod
    def from_dict(cls, dikt: dict) -> 'Build':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The Build of this Build.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def properties(self):
        """Gets the properties of this Build.


        :return: The properties of this Build.
        :rtype: BuildProperties
        """
        return self._properties

    @properties.setter
    def properties(self, properties):
        """Sets the properties of this Build.


        :param properties: The properties of this Build.
        :type properties: BuildProperties
        """

        self._properties = properties

    @property
    def id(self):
        """Gets the id of this Build.

        The resource ID.

        :return: The id of this Build.
        :rtype: str
        """
        return self._id

    @id.setter
    def id(self, id):
        """Sets the id of this Build.

        The resource ID.

        :param id: The id of this Build.
        :type id: str
        """

        self._id = id

    @property
    def name(self):
        """Gets the name of this Build.

        The name of the resource.

        :return: The name of this Build.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name):
        """Sets the name of this Build.

        The name of the resource.

        :param name: The name of this Build.
        :type name: str
        """

        self._name = name

    @property
    def type(self):
        """Gets the type of this Build.

        The type of the resource.

        :return: The type of this Build.
        :rtype: str
        """
        return self._type

    @type.setter
    def type(self, type):
        """Sets the type of this Build.

        The type of the resource.

        :param type: The type of this Build.
        :type type: str
        """

        self._type = type
