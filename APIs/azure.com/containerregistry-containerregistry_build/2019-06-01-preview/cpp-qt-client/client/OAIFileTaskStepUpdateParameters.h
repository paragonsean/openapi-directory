/**
 * ContainerRegistryManagementClient
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 2019-06-01-preview
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIFileTaskStepUpdateParameters.h
 *
 * The properties of updating a task step.
 */

#ifndef OAIFileTaskStepUpdateParameters_H
#define OAIFileTaskStepUpdateParameters_H

#include <QJsonObject>

#include "OAISetValue.h"
#include "OAITaskStepUpdateParameters.h"
#include <QList>
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {
class OAISetValue;

class OAIFileTaskStepUpdateParameters : public OAIObject {
public:
    OAIFileTaskStepUpdateParameters();
    OAIFileTaskStepUpdateParameters(QString json);
    ~OAIFileTaskStepUpdateParameters() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QString getTaskFilePath() const;
    void setTaskFilePath(const QString &task_file_path);
    bool is_task_file_path_Set() const;
    bool is_task_file_path_Valid() const;

    QList<OAISetValue> getValues() const;
    void setValues(const QList<OAISetValue> &values);
    bool is_values_Set() const;
    bool is_values_Valid() const;

    QString getValuesFilePath() const;
    void setValuesFilePath(const QString &values_file_path);
    bool is_values_file_path_Set() const;
    bool is_values_file_path_Valid() const;

    QString getContextAccessToken() const;
    void setContextAccessToken(const QString &context_access_token);
    bool is_context_access_token_Set() const;
    bool is_context_access_token_Valid() const;

    QString getContextPath() const;
    void setContextPath(const QString &context_path);
    bool is_context_path_Set() const;
    bool is_context_path_Valid() const;

    QString getType() const;
    void setType(const QString &type);
    bool is_type_Set() const;
    bool is_type_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString m_task_file_path;
    bool m_task_file_path_isSet;
    bool m_task_file_path_isValid;

    QList<OAISetValue> m_values;
    bool m_values_isSet;
    bool m_values_isValid;

    QString m_values_file_path;
    bool m_values_file_path_isSet;
    bool m_values_file_path_isValid;

    QString m_context_access_token;
    bool m_context_access_token_isSet;
    bool m_context_access_token_isValid;

    QString m_context_path;
    bool m_context_path_isSet;
    bool m_context_path_isValid;

    QString m_type;
    bool m_type_isSet;
    bool m_type_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIFileTaskStepUpdateParameters)

#endif // OAIFileTaskStepUpdateParameters_H
