...
async def test_bastion_hosts_update_tags(client):
    """Test case for bastion_hosts_update_tags

    
    """
    bastion_host_parameters = openapi_server.BastionHostsUpdateTagsRequest()
    params = [('api-version', 'api_version_example')]
    headers = { 
        'Accept': 'application/json',
        'Content-Type': 'application/json',
    ...
    response = await client.request(
        method='PATCH',
        path='/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Network/bastionHosts/{bastion_host_name}'.format(subscription_id='subscription_id_example', resource_group_name='resource_group_name_example', bastion_host_name='bastion_host_name_example'),
        headers=headers,
        json=bastion_host_parameters,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
