...
async def test_environments_create_or_update(client):
    ...
    """
    parameters = {"sku":{"name":"S1","capacity":1},"properties":{"partitionKeyProperties":[{"name":"name","type":"String"},{"name":"name","type":"String"}],"storageLimitExceededBehavior":"PurgeOldData","dataRetentionTime":"dataRetentionTime"}}
...
async def test_environments_update(client):
    ...
    """
    environment_update_parameters = {"sku":{"name":"S1","capacity":1},"properties":{"partitionKeyProperties":[{"name":"name","type":"String"},{"name":"name","type":"String"}],"storageLimitExceededBehavior":"PurgeOldData","dataRetentionTime":"dataRetentionTime"},"tags":{"key":"tags"}}
    params = [('api-version', 'api_version_example')]
    headers = { 
        'Accept': 'application/json',
        'Content-Type': 'application/json',
    ...
    response = await client.request(
        ...
        path='/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.TimeSeriesInsights/environments/{environment_name}'.format(subscription_id='subscription_id_example', resource_group_name='resource_group_name_example', environment_name='environment_name_example'),
        headers=headers,
        json=environment_update_parameters,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
