/**
 * RecoveryServicesBackupClient
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 2016-12-01
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIMabContainer.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIMabContainer::OAIMabContainer(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIMabContainer::OAIMabContainer() {
    this->initializeModel();
}

OAIMabContainer::~OAIMabContainer() {}

void OAIMabContainer::initializeModel() {

    m_agent_version_isSet = false;
    m_agent_version_isValid = false;

    m_can_re_register_isSet = false;
    m_can_re_register_isValid = false;

    m_container_health_state_isSet = false;
    m_container_health_state_isValid = false;

    m_container_id_isSet = false;
    m_container_id_isValid = false;

    m_extended_info_isSet = false;
    m_extended_info_isValid = false;

    m_mab_container_health_details_isSet = false;
    m_mab_container_health_details_isValid = false;

    m_protected_item_count_isSet = false;
    m_protected_item_count_isValid = false;

    m_backup_management_type_isSet = false;
    m_backup_management_type_isValid = false;

    m_container_type_isSet = false;
    m_container_type_isValid = false;

    m_friendly_name_isSet = false;
    m_friendly_name_isValid = false;

    m_health_status_isSet = false;
    m_health_status_isValid = false;

    m_registration_status_isSet = false;
    m_registration_status_isValid = false;
}

void OAIMabContainer::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAIMabContainer::fromJsonObject(QJsonObject json) {

    m_agent_version_isValid = ::OpenAPI::fromJsonValue(m_agent_version, json[QString("agentVersion")]);
    m_agent_version_isSet = !json[QString("agentVersion")].isNull() && m_agent_version_isValid;

    m_can_re_register_isValid = ::OpenAPI::fromJsonValue(m_can_re_register, json[QString("canReRegister")]);
    m_can_re_register_isSet = !json[QString("canReRegister")].isNull() && m_can_re_register_isValid;

    m_container_health_state_isValid = ::OpenAPI::fromJsonValue(m_container_health_state, json[QString("containerHealthState")]);
    m_container_health_state_isSet = !json[QString("containerHealthState")].isNull() && m_container_health_state_isValid;

    m_container_id_isValid = ::OpenAPI::fromJsonValue(m_container_id, json[QString("containerId")]);
    m_container_id_isSet = !json[QString("containerId")].isNull() && m_container_id_isValid;

    m_extended_info_isValid = ::OpenAPI::fromJsonValue(m_extended_info, json[QString("extendedInfo")]);
    m_extended_info_isSet = !json[QString("extendedInfo")].isNull() && m_extended_info_isValid;

    m_mab_container_health_details_isValid = ::OpenAPI::fromJsonValue(m_mab_container_health_details, json[QString("mabContainerHealthDetails")]);
    m_mab_container_health_details_isSet = !json[QString("mabContainerHealthDetails")].isNull() && m_mab_container_health_details_isValid;

    m_protected_item_count_isValid = ::OpenAPI::fromJsonValue(m_protected_item_count, json[QString("protectedItemCount")]);
    m_protected_item_count_isSet = !json[QString("protectedItemCount")].isNull() && m_protected_item_count_isValid;

    m_backup_management_type_isValid = ::OpenAPI::fromJsonValue(m_backup_management_type, json[QString("backupManagementType")]);
    m_backup_management_type_isSet = !json[QString("backupManagementType")].isNull() && m_backup_management_type_isValid;

    m_container_type_isValid = ::OpenAPI::fromJsonValue(m_container_type, json[QString("containerType")]);
    m_container_type_isSet = !json[QString("containerType")].isNull() && m_container_type_isValid;

    m_friendly_name_isValid = ::OpenAPI::fromJsonValue(m_friendly_name, json[QString("friendlyName")]);
    m_friendly_name_isSet = !json[QString("friendlyName")].isNull() && m_friendly_name_isValid;

    m_health_status_isValid = ::OpenAPI::fromJsonValue(m_health_status, json[QString("healthStatus")]);
    m_health_status_isSet = !json[QString("healthStatus")].isNull() && m_health_status_isValid;

    m_registration_status_isValid = ::OpenAPI::fromJsonValue(m_registration_status, json[QString("registrationStatus")]);
    m_registration_status_isSet = !json[QString("registrationStatus")].isNull() && m_registration_status_isValid;
}

QString OAIMabContainer::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAIMabContainer::asJsonObject() const {
    QJsonObject obj;
    if (m_agent_version_isSet) {
        obj.insert(QString("agentVersion"), ::OpenAPI::toJsonValue(m_agent_version));
    }
    if (m_can_re_register_isSet) {
        obj.insert(QString("canReRegister"), ::OpenAPI::toJsonValue(m_can_re_register));
    }
    if (m_container_health_state_isSet) {
        obj.insert(QString("containerHealthState"), ::OpenAPI::toJsonValue(m_container_health_state));
    }
    if (m_container_id_isSet) {
        obj.insert(QString("containerId"), ::OpenAPI::toJsonValue(m_container_id));
    }
    if (m_extended_info.isSet()) {
        obj.insert(QString("extendedInfo"), ::OpenAPI::toJsonValue(m_extended_info));
    }
    if (m_mab_container_health_details.size() > 0) {
        obj.insert(QString("mabContainerHealthDetails"), ::OpenAPI::toJsonValue(m_mab_container_health_details));
    }
    if (m_protected_item_count_isSet) {
        obj.insert(QString("protectedItemCount"), ::OpenAPI::toJsonValue(m_protected_item_count));
    }
    if (m_backup_management_type_isSet) {
        obj.insert(QString("backupManagementType"), ::OpenAPI::toJsonValue(m_backup_management_type));
    }
    if (m_container_type_isSet) {
        obj.insert(QString("containerType"), ::OpenAPI::toJsonValue(m_container_type));
    }
    if (m_friendly_name_isSet) {
        obj.insert(QString("friendlyName"), ::OpenAPI::toJsonValue(m_friendly_name));
    }
    if (m_health_status_isSet) {
        obj.insert(QString("healthStatus"), ::OpenAPI::toJsonValue(m_health_status));
    }
    if (m_registration_status_isSet) {
        obj.insert(QString("registrationStatus"), ::OpenAPI::toJsonValue(m_registration_status));
    }
    return obj;
}

QString OAIMabContainer::getAgentVersion() const {
    return m_agent_version;
}
void OAIMabContainer::setAgentVersion(const QString &agent_version) {
    m_agent_version = agent_version;
    m_agent_version_isSet = true;
}

bool OAIMabContainer::is_agent_version_Set() const{
    return m_agent_version_isSet;
}

bool OAIMabContainer::is_agent_version_Valid() const{
    return m_agent_version_isValid;
}

bool OAIMabContainer::isCanReRegister() const {
    return m_can_re_register;
}
void OAIMabContainer::setCanReRegister(const bool &can_re_register) {
    m_can_re_register = can_re_register;
    m_can_re_register_isSet = true;
}

bool OAIMabContainer::is_can_re_register_Set() const{
    return m_can_re_register_isSet;
}

bool OAIMabContainer::is_can_re_register_Valid() const{
    return m_can_re_register_isValid;
}

QString OAIMabContainer::getContainerHealthState() const {
    return m_container_health_state;
}
void OAIMabContainer::setContainerHealthState(const QString &container_health_state) {
    m_container_health_state = container_health_state;
    m_container_health_state_isSet = true;
}

bool OAIMabContainer::is_container_health_state_Set() const{
    return m_container_health_state_isSet;
}

bool OAIMabContainer::is_container_health_state_Valid() const{
    return m_container_health_state_isValid;
}

qint64 OAIMabContainer::getContainerId() const {
    return m_container_id;
}
void OAIMabContainer::setContainerId(const qint64 &container_id) {
    m_container_id = container_id;
    m_container_id_isSet = true;
}

bool OAIMabContainer::is_container_id_Set() const{
    return m_container_id_isSet;
}

bool OAIMabContainer::is_container_id_Valid() const{
    return m_container_id_isValid;
}

OAIMabContainerExtendedInfo OAIMabContainer::getExtendedInfo() const {
    return m_extended_info;
}
void OAIMabContainer::setExtendedInfo(const OAIMabContainerExtendedInfo &extended_info) {
    m_extended_info = extended_info;
    m_extended_info_isSet = true;
}

bool OAIMabContainer::is_extended_info_Set() const{
    return m_extended_info_isSet;
}

bool OAIMabContainer::is_extended_info_Valid() const{
    return m_extended_info_isValid;
}

QList<OAIMABContainerHealthDetails> OAIMabContainer::getMabContainerHealthDetails() const {
    return m_mab_container_health_details;
}
void OAIMabContainer::setMabContainerHealthDetails(const QList<OAIMABContainerHealthDetails> &mab_container_health_details) {
    m_mab_container_health_details = mab_container_health_details;
    m_mab_container_health_details_isSet = true;
}

bool OAIMabContainer::is_mab_container_health_details_Set() const{
    return m_mab_container_health_details_isSet;
}

bool OAIMabContainer::is_mab_container_health_details_Valid() const{
    return m_mab_container_health_details_isValid;
}

qint64 OAIMabContainer::getProtectedItemCount() const {
    return m_protected_item_count;
}
void OAIMabContainer::setProtectedItemCount(const qint64 &protected_item_count) {
    m_protected_item_count = protected_item_count;
    m_protected_item_count_isSet = true;
}

bool OAIMabContainer::is_protected_item_count_Set() const{
    return m_protected_item_count_isSet;
}

bool OAIMabContainer::is_protected_item_count_Valid() const{
    return m_protected_item_count_isValid;
}

QString OAIMabContainer::getBackupManagementType() const {
    return m_backup_management_type;
}
void OAIMabContainer::setBackupManagementType(const QString &backup_management_type) {
    m_backup_management_type = backup_management_type;
    m_backup_management_type_isSet = true;
}

bool OAIMabContainer::is_backup_management_type_Set() const{
    return m_backup_management_type_isSet;
}

bool OAIMabContainer::is_backup_management_type_Valid() const{
    return m_backup_management_type_isValid;
}

QString OAIMabContainer::getContainerType() const {
    return m_container_type;
}
void OAIMabContainer::setContainerType(const QString &container_type) {
    m_container_type = container_type;
    m_container_type_isSet = true;
}

bool OAIMabContainer::is_container_type_Set() const{
    return m_container_type_isSet;
}

bool OAIMabContainer::is_container_type_Valid() const{
    return m_container_type_isValid;
}

QString OAIMabContainer::getFriendlyName() const {
    return m_friendly_name;
}
void OAIMabContainer::setFriendlyName(const QString &friendly_name) {
    m_friendly_name = friendly_name;
    m_friendly_name_isSet = true;
}

bool OAIMabContainer::is_friendly_name_Set() const{
    return m_friendly_name_isSet;
}

bool OAIMabContainer::is_friendly_name_Valid() const{
    return m_friendly_name_isValid;
}

QString OAIMabContainer::getHealthStatus() const {
    return m_health_status;
}
void OAIMabContainer::setHealthStatus(const QString &health_status) {
    m_health_status = health_status;
    m_health_status_isSet = true;
}

bool OAIMabContainer::is_health_status_Set() const{
    return m_health_status_isSet;
}

bool OAIMabContainer::is_health_status_Valid() const{
    return m_health_status_isValid;
}

QString OAIMabContainer::getRegistrationStatus() const {
    return m_registration_status;
}
void OAIMabContainer::setRegistrationStatus(const QString &registration_status) {
    m_registration_status = registration_status;
    m_registration_status_isSet = true;
}

bool OAIMabContainer::is_registration_status_Set() const{
    return m_registration_status_isSet;
}

bool OAIMabContainer::is_registration_status_Valid() const{
    return m_registration_status_isValid;
}

bool OAIMabContainer::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_agent_version_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_can_re_register_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_container_health_state_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_container_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_extended_info.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_mab_container_health_details.size() > 0) {
            isObjectUpdated = true;
            break;
        }

        if (m_protected_item_count_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_backup_management_type_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_container_type_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_friendly_name_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_health_status_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_registration_status_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAIMabContainer::isValid() const {
    // only required properties are required for the object to be considered valid
    return true;
}

} // namespace OpenAPI
