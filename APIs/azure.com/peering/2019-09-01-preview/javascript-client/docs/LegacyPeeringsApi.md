...
> PeeringListResult legacyPeeringsList(peeringLocation, kind, subscriptionId, apiVersion)
...
apiInstance.legacyPeeringsList(peeringLocation, kind, subscriptionId, apiVersion, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
