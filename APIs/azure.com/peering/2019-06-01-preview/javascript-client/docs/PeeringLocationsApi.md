...
> PeeringLocationListResult peeringLocationsList(kind, subscriptionId, apiVersion)
...
apiInstance.peeringLocationsList(kind, subscriptionId, apiVersion, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
