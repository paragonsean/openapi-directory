

# Profile

The object containing the job profile information.

## Properties

| Name | Type | Description | Notes |
|------------ | ------------- | ------------- | -------------|
|**jobFile** | **String** | The job configuration file. |  [optional] |
|**jobID** | [**JobID**](JobID.md) |  |  [optional] |
|**jobId** | **String** | The full ID of the job. |  [optional] |
|**jobName** | **String** | The user-specified job name. |  [optional] |
|**queueName** | **String** | The name of the queue to which the job is submitted. |  [optional] |
|**url** | **String** | The link to the web-ui for details of the job. |  [optional] |
|**user** | **String** | The userid of the person who submitted the job. |  [optional] |



