# NetworkManagementClient.AzureFirewallNatRuleCollection

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**etag** | **String** | A unique read-only string that changes whenever the resource is updated. | [optional] [readonly] 
**name** | **String** | The name of the resource that is unique within the Azure firewall. This name can be used to access the resource. | [optional] 
**properties** | [**AzureFirewallNatRuleCollectionProperties**](AzureFirewallNatRuleCollectionProperties.md) |  | [optional] 
**id** | **String** | Resource ID. | [optional] 


