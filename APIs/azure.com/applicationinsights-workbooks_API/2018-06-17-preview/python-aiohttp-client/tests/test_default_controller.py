...
async def test_workbooks_create_or_update(client):
    ...
    """
    workbook_properties = {"properties":{"serializedData":"serializedData","displayName":"displayName","category":"category","timeModified":"timeModified","userId":"userId","version":"version","tags":["tags","tags"]}}
...
async def test_workbooks_update(client):
    ...
    """
    workbook_update_parameters = {"kind":"user","properties":{"serializedData":"serializedData","displayName":"displayName","category":"category","tags":["tags","tags"]},"tags":{"key":"tags"}}
    params = [('sourceId', 'source_id_example'),
                    ('api-version', 'api_version_example')]
    headers = { 
        'Accept': 'application/json',
    ...
    response = await client.request(
        ...
        path='/subscriptions/{subscription_id}/resourceGroup/{resource_group_name}/providers/microsoft.insights/workbooks/{resource_name}'.format(subscription_id='subscription_id_example', resource_group_name='resource_group_name_example', resource_name='resource_name_example'),
        headers=headers,
        json=workbook_update_parameters,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
