...
async def test_dsc_node_configuration_create_or_update(client):
    ...
    """
    parameters = {"name":"name","properties":{"incrementNodeConfigurationBuild":True,"configuration":{"name":"name"},"source":{"type":"embeddedContent","value":"value","version":"version","hash":{"value":"value","algorithm":"algorithm"}}},"tags":{"key":"tags"}}
...
async def test_dsc_node_configuration_delete(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Automation/automationAccounts/{automation_account_name}/nodeConfigurations/{node_configuration_name}'.format(subscription_id='subscription_id_example', resource_group_name='resource_group_name_example', automation_account_name='automation_account_name_example', node_configuration_name='node_configuration_name_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
