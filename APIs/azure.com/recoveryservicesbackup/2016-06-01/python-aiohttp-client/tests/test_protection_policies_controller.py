...
async def test_protection_policies_create_or_update(client):
    ...
    """
    resource_protection_policy = {"properties":{"backupManagementType":"backupManagementType","protectedItemsCount":0}}
    ...
    response = await client.request(
        ...
        headers=headers,
        json=resource_protection_policy,
        params=params,
        )
...
async def test_protection_policies_list(client):
    """Test case for protection_policies_list
    ...
    response = await client.request(
        method='GET',
        path='/Subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.RecoveryServices/vaults/{vault_name}/backupPolicies'.format(vault_name='vault_name_example', resource_group_name='resource_group_name_example', subscription_id='subscription_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
