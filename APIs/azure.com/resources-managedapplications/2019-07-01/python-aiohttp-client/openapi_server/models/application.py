# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.application_properties import ApplicationProperties
from openapi_server.models.identity import Identity
from openapi_server.models.plan import Plan
from openapi_server.models.sku import Sku
import re
from openapi_server import util


class Application(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, identity: Identity=None, kind: str=None, plan: Plan=None, properties: ApplicationProperties=None, managed_by: str=None, sku: Sku=None, id: str=None, location: str=None, name: str=None, tags: Dict[str, str]=None, type: str=None):
        """Application - a model defined in OpenAPI

        :param identity: The identity of this Application.
        :param kind: The kind of this Application.
        :param plan: The plan of this Application.
        :param properties: The properties of this Application.
        :param managed_by: The managed_by of this Application.
        :param sku: The sku of this Application.
        :param id: The id of this Application.
        :param location: The location of this Application.
        :param name: The name of this Application.
        :param tags: The tags of this Application.
        :param type: The type of this Application.
        """
        self.openapi_types = {
            'identity': Identity,
            'kind': str,
            'plan': Plan,
            'properties': ApplicationProperties,
            'managed_by': str,
            'sku': Sku,
            'id': str,
            'location': str,
            'name': str,
            'tags': Dict[str, str],
            'type': str
        }

        self.attribute_map = {
            'identity': 'identity',
            'kind': 'kind',
            'plan': 'plan',
            'properties': 'properties',
            'managed_by': 'managedBy',
            'sku': 'sku',
            'id': 'id',
            'location': 'location',
            'name': 'name',
            'tags': 'tags',
            'type': 'type'
        }

        self._identity = identity
        self._kind = kind
        self._plan = plan
        self._properties = properties
        self._managed_by = managed_by
        self._sku = sku
        self._id = id
        self._location = location
        self._name = name
        self._tags = tags
        self._type = type

    @classmethod
    def from_dict(cls, dikt: dict) -> 'Application':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The Application of this Application.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def identity(self):
        """Gets the identity of this Application.


        :return: The identity of this Application.
        :rtype: Identity
        """
        return self._identity

    @identity.setter
    def identity(self, identity):
        """Sets the identity of this Application.


        :param identity: The identity of this Application.
        :type identity: Identity
        """

        self._identity = identity

    @property
    def kind(self):
        """Gets the kind of this Application.

        The kind of the managed application. Allowed values are MarketPlace and ServiceCatalog.

        :return: The kind of this Application.
        :rtype: str
        """
        return self._kind

    @kind.setter
    def kind(self, kind):
        """Sets the kind of this Application.

        The kind of the managed application. Allowed values are MarketPlace and ServiceCatalog.

        :param kind: The kind of this Application.
        :type kind: str
        """
        if kind is None:
            raise ValueError("Invalid value for `kind`, must not be `None`")
        if kind is not None and not re.search(r'^[-\w\._,\(\)]+$', kind):
            raise ValueError("Invalid value for `kind`, must be a follow pattern or equal to `/^[-\w\._,\(\)]+$/`")

        self._kind = kind

    @property
    def plan(self):
        """Gets the plan of this Application.


        :return: The plan of this Application.
        :rtype: Plan
        """
        return self._plan

    @plan.setter
    def plan(self, plan):
        """Sets the plan of this Application.


        :param plan: The plan of this Application.
        :type plan: Plan
        """

        self._plan = plan

    @property
    def properties(self):
        """Gets the properties of this Application.


        :return: The properties of this Application.
        :rtype: ApplicationProperties
        """
        return self._properties

    @properties.setter
    def properties(self, properties):
        """Sets the properties of this Application.


        :param properties: The properties of this Application.
        :type properties: ApplicationProperties
        """
        if properties is None:
            raise ValueError("Invalid value for `properties`, must not be `None`")

        self._properties = properties

    @property
    def managed_by(self):
        """Gets the managed_by of this Application.

        ID of the resource that manages this resource.

        :return: The managed_by of this Application.
        :rtype: str
        """
        return self._managed_by

    @managed_by.setter
    def managed_by(self, managed_by):
        """Sets the managed_by of this Application.

        ID of the resource that manages this resource.

        :param managed_by: The managed_by of this Application.
        :type managed_by: str
        """

        self._managed_by = managed_by

    @property
    def sku(self):
        """Gets the sku of this Application.


        :return: The sku of this Application.
        :rtype: Sku
        """
        return self._sku

    @sku.setter
    def sku(self, sku):
        """Sets the sku of this Application.


        :param sku: The sku of this Application.
        :type sku: Sku
        """

        self._sku = sku

    @property
    def id(self):
        """Gets the id of this Application.

        Resource ID

        :return: The id of this Application.
        :rtype: str
        """
        return self._id

    @id.setter
    def id(self, id):
        """Sets the id of this Application.

        Resource ID

        :param id: The id of this Application.
        :type id: str
        """

        self._id = id

    @property
    def location(self):
        """Gets the location of this Application.

        Resource location

        :return: The location of this Application.
        :rtype: str
        """
        return self._location

    @location.setter
    def location(self, location):
        """Sets the location of this Application.

        Resource location

        :param location: The location of this Application.
        :type location: str
        """

        self._location = location

    @property
    def name(self):
        """Gets the name of this Application.

        Resource name

        :return: The name of this Application.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name):
        """Sets the name of this Application.

        Resource name

        :param name: The name of this Application.
        :type name: str
        """

        self._name = name

    @property
    def tags(self):
        """Gets the tags of this Application.

        Resource tags

        :return: The tags of this Application.
        :rtype: Dict[str, str]
        """
        return self._tags

    @tags.setter
    def tags(self, tags):
        """Sets the tags of this Application.

        Resource tags

        :param tags: The tags of this Application.
        :type tags: Dict[str, str]
        """

        self._tags = tags

    @property
    def type(self):
        """Gets the type of this Application.

        Resource type

        :return: The type of this Application.
        :rtype: str
        """
        return self._type

    @type.setter
    def type(self, type):
        """Sets the type of this Application.

        Resource type

        :param type: The type of this Application.
        :type type: str
        """

        self._type = type
