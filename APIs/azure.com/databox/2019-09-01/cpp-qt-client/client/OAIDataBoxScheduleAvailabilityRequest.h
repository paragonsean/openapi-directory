/**
 * DataBoxManagementClient
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 2019-09-01
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIDataBoxScheduleAvailabilityRequest.h
 *
 * Request body to get the availability for scheduling data box orders orders.
 */

#ifndef OAIDataBoxScheduleAvailabilityRequest_H
#define OAIDataBoxScheduleAvailabilityRequest_H

#include <QJsonObject>

#include "OAIScheduleAvailabilityRequest.h"
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {

class OAIDataBoxScheduleAvailabilityRequest : public OAIObject {
public:
    OAIDataBoxScheduleAvailabilityRequest();
    OAIDataBoxScheduleAvailabilityRequest(QString json);
    ~OAIDataBoxScheduleAvailabilityRequest() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QString getSkuName() const;
    void setSkuName(const QString &sku_name);
    bool is_sku_name_Set() const;
    bool is_sku_name_Valid() const;

    QString getStorageLocation() const;
    void setStorageLocation(const QString &storage_location);
    bool is_storage_location_Set() const;
    bool is_storage_location_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString m_sku_name;
    bool m_sku_name_isSet;
    bool m_sku_name_isValid;

    QString m_storage_location;
    bool m_storage_location_isSet;
    bool m_storage_location_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIDataBoxScheduleAvailabilityRequest)

#endif // OAIDataBoxScheduleAvailabilityRequest_H
