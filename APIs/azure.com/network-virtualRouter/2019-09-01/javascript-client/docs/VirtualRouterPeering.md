# NetworkManagementClient.VirtualRouterPeering

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**etag** | **String** | A unique read-only string that changes whenever the resource is updated. | [optional] [readonly] 
**name** | **String** | Name of the virtual router peering that is unique within a virtual router. | [optional] 
**properties** | [**VirtualRouterPeeringProperties**](VirtualRouterPeeringProperties.md) |  | [optional] 
**type** | **String** | Peering type. | [optional] [readonly] 
**id** | **String** | Resource ID. | [optional] 


