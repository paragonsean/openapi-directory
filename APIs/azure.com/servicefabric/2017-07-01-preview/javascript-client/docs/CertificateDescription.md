# ServiceFabricManagementClient.CertificateDescription

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**thumbprint** | **String** | Thumbprint of the primary certificate. | 
**thumbprintSecondary** | **String** | Thumbprint of the secondary certificate. | [optional] 
**x509StoreName** | **String** | The local certificate store location. | [optional] 



## Enum: X509StoreNameEnum


* `AddressBook` (value: `"AddressBook"`)

* `AuthRoot` (value: `"AuthRoot"`)

* `CertificateAuthority` (value: `"CertificateAuthority"`)

* `Disallowed` (value: `"Disallowed"`)

* `My` (value: `"My"`)

* `Root` (value: `"Root"`)

* `TrustedPeople` (value: `"TrustedPeople"`)

* `TrustedPublisher` (value: `"TrustedPublisher"`)




