...
public class DiagnosticsSinkProperties {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DiagnosticsSinkProperties.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DiagnosticsSinkProperties` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : DiagnosticsSinkProperties.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!DiagnosticsSinkProperties.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'DiagnosticsSinkProperties' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<DiagnosticsSinkProperties> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(DiagnosticsSinkProperties.class));

       return (TypeAdapter<T>) new TypeAdapter<DiagnosticsSinkProperties>() {
           @Override
           public void write(JsonWriter out, DiagnosticsSinkProperties value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public DiagnosticsSinkProperties read(JsonReader in) throws IOException {
