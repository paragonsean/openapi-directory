...
async def test_add_configuration_parameter_overrides(client):
    """Test case for add_configuration_parameter_overrides

    Adds the list of configuration overrides on the specified node.
    """
    config_parameter_override_list = [openapi_server.ConfigParameterOverride()]
    ...
    response = await client.request(
        method='POST',
        path='/Nodes/{node_name}/$/AddConfigurationParameterOverrides'.format(node_name='node_name_example'),
        headers=headers,
        json=config_parameter_override_list,
        params=params,
        )
...
async def test_get_configuration_overrides(client):
    ...
    response = await client.request(
        method='GET',
        path='/Nodes/{node_name}/$/GetConfigurationOverrides'.format(node_name='node_name_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_remove_configuration_overrides(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/Nodes/{node_name}/$/RemoveConfigurationOverrides'.format(node_name='node_name_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
