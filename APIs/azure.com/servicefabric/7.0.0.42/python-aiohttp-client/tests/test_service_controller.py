...
async def test_get_unplaced_replica_information(client):
    ...
    response = await client.request(
        method='GET',
        path='/Services/{service_id}/$/GetUnplacedReplicaInformation'.format(service_id='service_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
