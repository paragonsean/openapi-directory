...
<a id="movePrimaryReplica"></a>
# **movePrimaryReplica**
> movePrimaryReplica(apiVersion, partitionId, nodeName, ignoreConstraints, timeout)
...
public class Example {
  public static void main(String[] args) {
    ...
    Boolean ignoreConstraints = false; // Boolean | Ignore constraints when moving a replica. If this parameter is not specified, all constraints are honored.
    Long timeout = 60L; // Long | The server timeout for performing the operation in seconds. This timeout specifies the time duration that the client is willing to wait for the requested operation to complete. The default value for this parameter is 60 seconds.
    try {
      apiInstance.movePrimaryReplica(apiVersion, partitionId, nodeName, ignoreConstraints, timeout);
    } catch (ApiException e) {
      System.err.println("Exception when calling PartitionApi#movePrimaryReplica");
      System.err.println("Status code: " + e.getCode());
      System.err.println("Reason: " + e.getResponseBody());
      System.err.println("Response headers: " + e.getResponseHeaders());
      e.printStackTrace();
...
<a id="moveSecondaryReplica"></a>
# **moveSecondaryReplica**
> moveSecondaryReplica(apiVersion, partitionId, currentNodeName, newNodeName, ignoreConstraints, timeout)
...
public class Example {
  public static void main(String[] args) {
    ...
    String apiVersion = "6.5"; // String | The version of the API. This parameter is required and its value must be '6.5'.  Service Fabric REST API version is based on the runtime version in which the API was introduced or was changed. Service Fabric runtime supports more than one version of the API. This version is the latest supported version of the API. If a lower API version is passed, the returned response may be different from the one documented in this specification.  Additionally the runtime accepts any version that is higher than the latest supported version up to the current version of the runtime. So if the latest API version is 6.0 and the runtime is 6.1, the runtime will accept version 6.1 for that API. However the behavior of the API will be as per the documented 6.0 version.
    UUID partitionId = UUID.randomUUID(); // UUID | The identity of the partition.
    String currentNodeName = "currentNodeName_example"; // String | The name of the source node for secondary replica move.
    String newNodeName = "newNodeName_example"; // String | The name of the target node for secondary replica move. If not specified, replica is moved to a random node.
    Boolean ignoreConstraints = false; // Boolean | Ignore constraints when moving a replica. If this parameter is not specified, all constraints are honored.
    Long timeout = 60L; // Long | The server timeout for performing the operation in seconds. This timeout specifies the time duration that the client is willing to wait for the requested operation to complete. The default value for this parameter is 60 seconds.
    try {
      apiInstance.moveSecondaryReplica(apiVersion, partitionId, currentNodeName, newNodeName, ignoreConstraints, timeout);
    } catch (ApiException e) {
      System.err.println("Exception when calling PartitionApi#moveSecondaryReplica");
      System.err.println("Status code: " + e.getCode());
      System.err.println("Reason: " + e.getResponseBody());
      System.err.println("Response headers: " + e.getResponseHeaders());
      e.printStackTrace();
