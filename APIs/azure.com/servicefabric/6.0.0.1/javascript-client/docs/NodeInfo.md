# ServiceFabricClientApis.NodeInfo
...
**nodeDownAt** | **Date** | Date time in UTC when the node went down. If node has never been down then this value will be zero date time. | [optional] 
...
**nodeUpAt** | **Date** | Date time in UTC when the node came up. If the node has never been up then this value will be zero date time. | [optional] 
