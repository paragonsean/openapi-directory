...
async def start_node_transition(request: web.Request, api_version, node_name, operation_id, node_transition_type, node_instance_id, stop_duration_in_seconds, timeout=None) -> web.Response:
    ...
    :type operation_id: str
    :type operation_id: str
    :param node_transition_type: Indicates the type of transition to perform.  NodeTransitionType.Start will start a stopped node.  NodeTransitionType.Stop will stop a node that is up.   - Invalid - Reserved.  Do not pass into API.   - Start - Transition a stopped node to up.   - Stop - Transition an up node to stopped. 
...
async def start_partition_restart(request: web.Request, api_version, service_id, partition_id, operation_id, restart_partition_mode, timeout=None) -> web.Response:
    ...
    :type operation_id: str
    :type operation_id: str
    :param restart_partition_mode: - Invalid - Reserved.  Do not pass into API. - AllReplicasOrInstances - All replicas or instances in the partition are restarted at once. - OnlyActiveSecondaries - Only the secondary replicas are restarted. 
