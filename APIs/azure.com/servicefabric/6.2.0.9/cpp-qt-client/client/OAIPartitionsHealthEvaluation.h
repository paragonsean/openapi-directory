/**
 * Service Fabric Client APIs
 * Service Fabric REST Client APIs allows management of Service Fabric clusters, applications and services.
 *
 * The version of the OpenAPI document: 6.2.0.9
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIPartitionsHealthEvaluation.h
 *
 * Represents health evaluation for the partitions of a service, containing health evaluations for each unhealthy partition that impacts current aggregated health state. Can be returned when evaluating service health and the aggregated health state is either Error or Warning.
 */

#ifndef OAIPartitionsHealthEvaluation_H
#define OAIPartitionsHealthEvaluation_H

#include <QJsonObject>

#include "OAIHealthEvaluation.h"
#include "OAIHealthEvaluationKind.h"
#include "OAIHealthEvaluationWrapper.h"
#include "OAIHealthState.h"
#include <QList>
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {
class OAIHealthEvaluationWrapper;

class OAIPartitionsHealthEvaluation : public OAIObject {
public:
    OAIPartitionsHealthEvaluation();
    OAIPartitionsHealthEvaluation(QString json);
    ~OAIPartitionsHealthEvaluation() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    OAIHealthState getAggregatedHealthState() const;
    void setAggregatedHealthState(const OAIHealthState &aggregated_health_state);
    bool is_aggregated_health_state_Set() const;
    bool is_aggregated_health_state_Valid() const;

    QString getDescription() const;
    void setDescription(const QString &description);
    bool is_description_Set() const;
    bool is_description_Valid() const;

    OAIHealthEvaluationKind getKind() const;
    void setKind(const OAIHealthEvaluationKind &kind);
    bool is_kind_Set() const;
    bool is_kind_Valid() const;

    qint32 getMaxPercentUnhealthyPartitionsPerService() const;
    void setMaxPercentUnhealthyPartitionsPerService(const qint32 &max_percent_unhealthy_partitions_per_service);
    bool is_max_percent_unhealthy_partitions_per_service_Set() const;
    bool is_max_percent_unhealthy_partitions_per_service_Valid() const;

    qint64 getTotalCount() const;
    void setTotalCount(const qint64 &total_count);
    bool is_total_count_Set() const;
    bool is_total_count_Valid() const;

    QList<OAIHealthEvaluationWrapper> getUnhealthyEvaluations() const;
    void setUnhealthyEvaluations(const QList<OAIHealthEvaluationWrapper> &unhealthy_evaluations);
    bool is_unhealthy_evaluations_Set() const;
    bool is_unhealthy_evaluations_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    OAIHealthState m_aggregated_health_state;
    bool m_aggregated_health_state_isSet;
    bool m_aggregated_health_state_isValid;

    QString m_description;
    bool m_description_isSet;
    bool m_description_isValid;

    OAIHealthEvaluationKind m_kind;
    bool m_kind_isSet;
    bool m_kind_isValid;

    qint32 m_max_percent_unhealthy_partitions_per_service;
    bool m_max_percent_unhealthy_partitions_per_service_isSet;
    bool m_max_percent_unhealthy_partitions_per_service_isValid;

    qint64 m_total_count;
    bool m_total_count_isSet;
    bool m_total_count_isValid;

    QList<OAIHealthEvaluationWrapper> m_unhealthy_evaluations;
    bool m_unhealthy_evaluations_isSet;
    bool m_unhealthy_evaluations_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIPartitionsHealthEvaluation)

#endif // OAIPartitionsHealthEvaluation_H
