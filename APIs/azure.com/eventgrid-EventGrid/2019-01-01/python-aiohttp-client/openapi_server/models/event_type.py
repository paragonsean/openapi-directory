# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.event_type_properties import EventTypeProperties
from openapi_server import util


class EventType(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, properties: EventTypeProperties=None, id: str=None, name: str=None, type: str=None):
        """EventType - a model defined in OpenAPI

        :param properties: The properties of this EventType.
        :param id: The id of this EventType.
        :param name: The name of this EventType.
        :param type: The type of this EventType.
        """
        self.openapi_types = {
            'properties': EventTypeProperties,
            'id': str,
            'name': str,
            'type': str
        }

        self.attribute_map = {
            'properties': 'properties',
            'id': 'id',
            'name': 'name',
            'type': 'type'
        }

        self._properties = properties
        self._id = id
        self._name = name
        self._type = type

    @classmethod
    def from_dict(cls, dikt: dict) -> 'EventType':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The EventType of this EventType.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def properties(self):
        """Gets the properties of this EventType.


        :return: The properties of this EventType.
        :rtype: EventTypeProperties
        """
        return self._properties

    @properties.setter
    def properties(self, properties):
        """Sets the properties of this EventType.


        :param properties: The properties of this EventType.
        :type properties: EventTypeProperties
        """

        self._properties = properties

    @property
    def id(self):
        """Gets the id of this EventType.

        Fully qualified identifier of the resource

        :return: The id of this EventType.
        :rtype: str
        """
        return self._id

    @id.setter
    def id(self, id):
        """Sets the id of this EventType.

        Fully qualified identifier of the resource

        :param id: The id of this EventType.
        :type id: str
        """

        self._id = id

    @property
    def name(self):
        """Gets the name of this EventType.

        Name of the resource

        :return: The name of this EventType.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name):
        """Sets the name of this EventType.

        Name of the resource

        :param name: The name of this EventType.
        :type name: str
        """

        self._name = name

    @property
    def type(self):
        """Gets the type of this EventType.

        Type of the resource

        :return: The type of this EventType.
        :rtype: str
        """
        return self._type

    @type.setter
    def type(self, type):
        """Sets the type of this EventType.

        Type of the resource

        :param type: The type of this EventType.
        :type type: str
        """

        self._type = type
