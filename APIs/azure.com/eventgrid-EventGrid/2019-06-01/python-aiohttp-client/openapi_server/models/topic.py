# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.topic_properties import TopicProperties
from openapi_server import util


class Topic(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, properties: TopicProperties=None, location: str=None, tags: Dict[str, str]=None, id: str=None, name: str=None, type: str=None):
        """Topic - a model defined in OpenAPI

        :param properties: The properties of this Topic.
        :param location: The location of this Topic.
        :param tags: The tags of this Topic.
        :param id: The id of this Topic.
        :param name: The name of this Topic.
        :param type: The type of this Topic.
        """
        self.openapi_types = {
            'properties': TopicProperties,
            'location': str,
            'tags': Dict[str, str],
            'id': str,
            'name': str,
            'type': str
        }

        self.attribute_map = {
            'properties': 'properties',
            'location': 'location',
            'tags': 'tags',
            'id': 'id',
            'name': 'name',
            'type': 'type'
        }

        self._properties = properties
        self._location = location
        self._tags = tags
        self._id = id
        self._name = name
        self._type = type

    @classmethod
    def from_dict(cls, dikt: dict) -> 'Topic':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The Topic of this Topic.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def properties(self):
        """Gets the properties of this Topic.


        :return: The properties of this Topic.
        :rtype: TopicProperties
        """
        return self._properties

    @properties.setter
    def properties(self, properties):
        """Sets the properties of this Topic.


        :param properties: The properties of this Topic.
        :type properties: TopicProperties
        """

        self._properties = properties

    @property
    def location(self):
        """Gets the location of this Topic.

        Location of the resource.

        :return: The location of this Topic.
        :rtype: str
        """
        return self._location

    @location.setter
    def location(self, location):
        """Sets the location of this Topic.

        Location of the resource.

        :param location: The location of this Topic.
        :type location: str
        """
        if location is None:
            raise ValueError("Invalid value for `location`, must not be `None`")

        self._location = location

    @property
    def tags(self):
        """Gets the tags of this Topic.

        Tags of the resource.

        :return: The tags of this Topic.
        :rtype: Dict[str, str]
        """
        return self._tags

    @tags.setter
    def tags(self, tags):
        """Sets the tags of this Topic.

        Tags of the resource.

        :param tags: The tags of this Topic.
        :type tags: Dict[str, str]
        """

        self._tags = tags

    @property
    def id(self):
        """Gets the id of this Topic.

        Fully qualified identifier of the resource.

        :return: The id of this Topic.
        :rtype: str
        """
        return self._id

    @id.setter
    def id(self, id):
        """Sets the id of this Topic.

        Fully qualified identifier of the resource.

        :param id: The id of this Topic.
        :type id: str
        """

        self._id = id

    @property
    def name(self):
        """Gets the name of this Topic.

        Name of the resource.

        :return: The name of this Topic.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name):
        """Sets the name of this Topic.

        Name of the resource.

        :param name: The name of this Topic.
        :type name: str
        """

        self._name = name

    @property
    def type(self):
        """Gets the type of this Topic.

        Type of the resource.

        :return: The type of this Topic.
        :rtype: str
        """
        return self._type

    @type.setter
    def type(self, type):
        """Sets the type of this Topic.

        Type of the resource.

        :param type: The type of this Topic.
        :type type: str
        """

        self._type = type
