/**
 * Marketcheck APIs
 * One API serving data spanned across multiple verticals
 *
 * The version of the OpenAPI document: 2.01
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIHeavyEquipmentsListing.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIHeavyEquipmentsListing::OAIHeavyEquipmentsListing(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIHeavyEquipmentsListing::OAIHeavyEquipmentsListing() {
    this->initializeModel();
}

OAIHeavyEquipmentsListing::~OAIHeavyEquipmentsListing() {}

void OAIHeavyEquipmentsListing::initializeModel() {

    m_build_isSet = false;
    m_build_isValid = false;

    m_dealer_isSet = false;
    m_dealer_isValid = false;

    m_dp_url_isSet = false;
    m_dp_url_isValid = false;

    m_exterior_color_isSet = false;
    m_exterior_color_isValid = false;

    m_extra_isSet = false;
    m_extra_isValid = false;

    m_first_seen_at_isSet = false;
    m_first_seen_at_isValid = false;

    m_first_seen_at_date_isSet = false;
    m_first_seen_at_date_isValid = false;

    m_heading_isSet = false;
    m_heading_isValid = false;

    m_id_isSet = false;
    m_id_isValid = false;

    m_interior_color_isSet = false;
    m_interior_color_isValid = false;

    m_inventory_type_isSet = false;
    m_inventory_type_isValid = false;

    m_last_seen_at_isSet = false;
    m_last_seen_at_isValid = false;

    m_last_seen_at_date_isSet = false;
    m_last_seen_at_date_isValid = false;

    m_media_isSet = false;
    m_media_isValid = false;

    m_miles_isSet = false;
    m_miles_isValid = false;

    m_msrp_isSet = false;
    m_msrp_isValid = false;

    m_price_isSet = false;
    m_price_isValid = false;

    m_scraped_at_isSet = false;
    m_scraped_at_isValid = false;

    m_scraped_at_date_isSet = false;
    m_scraped_at_date_isValid = false;

    m_seller_type_isSet = false;
    m_seller_type_isValid = false;

    m_source_isSet = false;
    m_source_isValid = false;

    m_stock_no_isSet = false;
    m_stock_no_isValid = false;

    m_vin_isSet = false;
    m_vin_isValid = false;
}

void OAIHeavyEquipmentsListing::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAIHeavyEquipmentsListing::fromJsonObject(QJsonObject json) {

    m_build_isValid = ::OpenAPI::fromJsonValue(m_build, json[QString("build")]);
    m_build_isSet = !json[QString("build")].isNull() && m_build_isValid;

    m_dealer_isValid = ::OpenAPI::fromJsonValue(m_dealer, json[QString("dealer")]);
    m_dealer_isSet = !json[QString("dealer")].isNull() && m_dealer_isValid;

    m_dp_url_isValid = ::OpenAPI::fromJsonValue(m_dp_url, json[QString("dp_url")]);
    m_dp_url_isSet = !json[QString("dp_url")].isNull() && m_dp_url_isValid;

    m_exterior_color_isValid = ::OpenAPI::fromJsonValue(m_exterior_color, json[QString("exterior_color")]);
    m_exterior_color_isSet = !json[QString("exterior_color")].isNull() && m_exterior_color_isValid;

    m_extra_isValid = ::OpenAPI::fromJsonValue(m_extra, json[QString("extra")]);
    m_extra_isSet = !json[QString("extra")].isNull() && m_extra_isValid;

    m_first_seen_at_isValid = ::OpenAPI::fromJsonValue(m_first_seen_at, json[QString("first_seen_at")]);
    m_first_seen_at_isSet = !json[QString("first_seen_at")].isNull() && m_first_seen_at_isValid;

    m_first_seen_at_date_isValid = ::OpenAPI::fromJsonValue(m_first_seen_at_date, json[QString("first_seen_at_date")]);
    m_first_seen_at_date_isSet = !json[QString("first_seen_at_date")].isNull() && m_first_seen_at_date_isValid;

    m_heading_isValid = ::OpenAPI::fromJsonValue(m_heading, json[QString("heading")]);
    m_heading_isSet = !json[QString("heading")].isNull() && m_heading_isValid;

    m_id_isValid = ::OpenAPI::fromJsonValue(m_id, json[QString("id")]);
    m_id_isSet = !json[QString("id")].isNull() && m_id_isValid;

    m_interior_color_isValid = ::OpenAPI::fromJsonValue(m_interior_color, json[QString("interior_color")]);
    m_interior_color_isSet = !json[QString("interior_color")].isNull() && m_interior_color_isValid;

    m_inventory_type_isValid = ::OpenAPI::fromJsonValue(m_inventory_type, json[QString("inventory_type")]);
    m_inventory_type_isSet = !json[QString("inventory_type")].isNull() && m_inventory_type_isValid;

    m_last_seen_at_isValid = ::OpenAPI::fromJsonValue(m_last_seen_at, json[QString("last_seen_at")]);
    m_last_seen_at_isSet = !json[QString("last_seen_at")].isNull() && m_last_seen_at_isValid;

    m_last_seen_at_date_isValid = ::OpenAPI::fromJsonValue(m_last_seen_at_date, json[QString("last_seen_at_date")]);
    m_last_seen_at_date_isSet = !json[QString("last_seen_at_date")].isNull() && m_last_seen_at_date_isValid;

    m_media_isValid = ::OpenAPI::fromJsonValue(m_media, json[QString("media")]);
    m_media_isSet = !json[QString("media")].isNull() && m_media_isValid;

    m_miles_isValid = ::OpenAPI::fromJsonValue(m_miles, json[QString("miles")]);
    m_miles_isSet = !json[QString("miles")].isNull() && m_miles_isValid;

    m_msrp_isValid = ::OpenAPI::fromJsonValue(m_msrp, json[QString("msrp")]);
    m_msrp_isSet = !json[QString("msrp")].isNull() && m_msrp_isValid;

    m_price_isValid = ::OpenAPI::fromJsonValue(m_price, json[QString("price")]);
    m_price_isSet = !json[QString("price")].isNull() && m_price_isValid;

    m_scraped_at_isValid = ::OpenAPI::fromJsonValue(m_scraped_at, json[QString("scraped_at")]);
    m_scraped_at_isSet = !json[QString("scraped_at")].isNull() && m_scraped_at_isValid;

    m_scraped_at_date_isValid = ::OpenAPI::fromJsonValue(m_scraped_at_date, json[QString("scraped_at_date")]);
    m_scraped_at_date_isSet = !json[QString("scraped_at_date")].isNull() && m_scraped_at_date_isValid;

    m_seller_type_isValid = ::OpenAPI::fromJsonValue(m_seller_type, json[QString("seller_type")]);
    m_seller_type_isSet = !json[QString("seller_type")].isNull() && m_seller_type_isValid;

    m_source_isValid = ::OpenAPI::fromJsonValue(m_source, json[QString("source")]);
    m_source_isSet = !json[QString("source")].isNull() && m_source_isValid;

    m_stock_no_isValid = ::OpenAPI::fromJsonValue(m_stock_no, json[QString("stock_no")]);
    m_stock_no_isSet = !json[QString("stock_no")].isNull() && m_stock_no_isValid;

    m_vin_isValid = ::OpenAPI::fromJsonValue(m_vin, json[QString("vin")]);
    m_vin_isSet = !json[QString("vin")].isNull() && m_vin_isValid;
}

QString OAIHeavyEquipmentsListing::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAIHeavyEquipmentsListing::asJsonObject() const {
    QJsonObject obj;
    if (m_build.isSet()) {
        obj.insert(QString("build"), ::OpenAPI::toJsonValue(m_build));
    }
    if (m_dealer.isSet()) {
        obj.insert(QString("dealer"), ::OpenAPI::toJsonValue(m_dealer));
    }
    if (m_dp_url_isSet) {
        obj.insert(QString("dp_url"), ::OpenAPI::toJsonValue(m_dp_url));
    }
    if (m_exterior_color_isSet) {
        obj.insert(QString("exterior_color"), ::OpenAPI::toJsonValue(m_exterior_color));
    }
    if (m_extra.isSet()) {
        obj.insert(QString("extra"), ::OpenAPI::toJsonValue(m_extra));
    }
    if (m_first_seen_at_isSet) {
        obj.insert(QString("first_seen_at"), ::OpenAPI::toJsonValue(m_first_seen_at));
    }
    if (m_first_seen_at_date_isSet) {
        obj.insert(QString("first_seen_at_date"), ::OpenAPI::toJsonValue(m_first_seen_at_date));
    }
    if (m_heading_isSet) {
        obj.insert(QString("heading"), ::OpenAPI::toJsonValue(m_heading));
    }
    if (m_id_isSet) {
        obj.insert(QString("id"), ::OpenAPI::toJsonValue(m_id));
    }
    if (m_interior_color_isSet) {
        obj.insert(QString("interior_color"), ::OpenAPI::toJsonValue(m_interior_color));
    }
    if (m_inventory_type_isSet) {
        obj.insert(QString("inventory_type"), ::OpenAPI::toJsonValue(m_inventory_type));
    }
    if (m_last_seen_at_isSet) {
        obj.insert(QString("last_seen_at"), ::OpenAPI::toJsonValue(m_last_seen_at));
    }
    if (m_last_seen_at_date_isSet) {
        obj.insert(QString("last_seen_at_date"), ::OpenAPI::toJsonValue(m_last_seen_at_date));
    }
    if (m_media.isSet()) {
        obj.insert(QString("media"), ::OpenAPI::toJsonValue(m_media));
    }
    if (m_miles_isSet) {
        obj.insert(QString("miles"), ::OpenAPI::toJsonValue(m_miles));
    }
    if (m_msrp_isSet) {
        obj.insert(QString("msrp"), ::OpenAPI::toJsonValue(m_msrp));
    }
    if (m_price_isSet) {
        obj.insert(QString("price"), ::OpenAPI::toJsonValue(m_price));
    }
    if (m_scraped_at_isSet) {
        obj.insert(QString("scraped_at"), ::OpenAPI::toJsonValue(m_scraped_at));
    }
    if (m_scraped_at_date_isSet) {
        obj.insert(QString("scraped_at_date"), ::OpenAPI::toJsonValue(m_scraped_at_date));
    }
    if (m_seller_type_isSet) {
        obj.insert(QString("seller_type"), ::OpenAPI::toJsonValue(m_seller_type));
    }
    if (m_source_isSet) {
        obj.insert(QString("source"), ::OpenAPI::toJsonValue(m_source));
    }
    if (m_stock_no_isSet) {
        obj.insert(QString("stock_no"), ::OpenAPI::toJsonValue(m_stock_no));
    }
    if (m_vin_isSet) {
        obj.insert(QString("vin"), ::OpenAPI::toJsonValue(m_vin));
    }
    return obj;
}

OAIRVBuild OAIHeavyEquipmentsListing::getBuild() const {
    return m_build;
}
void OAIHeavyEquipmentsListing::setBuild(const OAIRVBuild &build) {
    m_build = build;
    m_build_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_build_Set() const{
    return m_build_isSet;
}

bool OAIHeavyEquipmentsListing::is_build_Valid() const{
    return m_build_isValid;
}

OAINestDealer OAIHeavyEquipmentsListing::getDealer() const {
    return m_dealer;
}
void OAIHeavyEquipmentsListing::setDealer(const OAINestDealer &dealer) {
    m_dealer = dealer;
    m_dealer_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_dealer_Set() const{
    return m_dealer_isSet;
}

bool OAIHeavyEquipmentsListing::is_dealer_Valid() const{
    return m_dealer_isValid;
}

QString OAIHeavyEquipmentsListing::getDpUrl() const {
    return m_dp_url;
}
void OAIHeavyEquipmentsListing::setDpUrl(const QString &dp_url) {
    m_dp_url = dp_url;
    m_dp_url_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_dp_url_Set() const{
    return m_dp_url_isSet;
}

bool OAIHeavyEquipmentsListing::is_dp_url_Valid() const{
    return m_dp_url_isValid;
}

QString OAIHeavyEquipmentsListing::getExteriorColor() const {
    return m_exterior_color;
}
void OAIHeavyEquipmentsListing::setExteriorColor(const QString &exterior_color) {
    m_exterior_color = exterior_color;
    m_exterior_color_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_exterior_color_Set() const{
    return m_exterior_color_isSet;
}

bool OAIHeavyEquipmentsListing::is_exterior_color_Valid() const{
    return m_exterior_color_isValid;
}

OAIListingNestExtraAttributes OAIHeavyEquipmentsListing::getExtra() const {
    return m_extra;
}
void OAIHeavyEquipmentsListing::setExtra(const OAIListingNestExtraAttributes &extra) {
    m_extra = extra;
    m_extra_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_extra_Set() const{
    return m_extra_isSet;
}

bool OAIHeavyEquipmentsListing::is_extra_Valid() const{
    return m_extra_isValid;
}

qint32 OAIHeavyEquipmentsListing::getFirstSeenAt() const {
    return m_first_seen_at;
}
void OAIHeavyEquipmentsListing::setFirstSeenAt(const qint32 &first_seen_at) {
    m_first_seen_at = first_seen_at;
    m_first_seen_at_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_first_seen_at_Set() const{
    return m_first_seen_at_isSet;
}

bool OAIHeavyEquipmentsListing::is_first_seen_at_Valid() const{
    return m_first_seen_at_isValid;
}

QString OAIHeavyEquipmentsListing::getFirstSeenAtDate() const {
    return m_first_seen_at_date;
}
void OAIHeavyEquipmentsListing::setFirstSeenAtDate(const QString &first_seen_at_date) {
    m_first_seen_at_date = first_seen_at_date;
    m_first_seen_at_date_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_first_seen_at_date_Set() const{
    return m_first_seen_at_date_isSet;
}

bool OAIHeavyEquipmentsListing::is_first_seen_at_date_Valid() const{
    return m_first_seen_at_date_isValid;
}

QString OAIHeavyEquipmentsListing::getHeading() const {
    return m_heading;
}
void OAIHeavyEquipmentsListing::setHeading(const QString &heading) {
    m_heading = heading;
    m_heading_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_heading_Set() const{
    return m_heading_isSet;
}

bool OAIHeavyEquipmentsListing::is_heading_Valid() const{
    return m_heading_isValid;
}

QString OAIHeavyEquipmentsListing::getId() const {
    return m_id;
}
void OAIHeavyEquipmentsListing::setId(const QString &id) {
    m_id = id;
    m_id_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_id_Set() const{
    return m_id_isSet;
}

bool OAIHeavyEquipmentsListing::is_id_Valid() const{
    return m_id_isValid;
}

QString OAIHeavyEquipmentsListing::getInteriorColor() const {
    return m_interior_color;
}
void OAIHeavyEquipmentsListing::setInteriorColor(const QString &interior_color) {
    m_interior_color = interior_color;
    m_interior_color_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_interior_color_Set() const{
    return m_interior_color_isSet;
}

bool OAIHeavyEquipmentsListing::is_interior_color_Valid() const{
    return m_interior_color_isValid;
}

QString OAIHeavyEquipmentsListing::getInventoryType() const {
    return m_inventory_type;
}
void OAIHeavyEquipmentsListing::setInventoryType(const QString &inventory_type) {
    m_inventory_type = inventory_type;
    m_inventory_type_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_inventory_type_Set() const{
    return m_inventory_type_isSet;
}

bool OAIHeavyEquipmentsListing::is_inventory_type_Valid() const{
    return m_inventory_type_isValid;
}

qint32 OAIHeavyEquipmentsListing::getLastSeenAt() const {
    return m_last_seen_at;
}
void OAIHeavyEquipmentsListing::setLastSeenAt(const qint32 &last_seen_at) {
    m_last_seen_at = last_seen_at;
    m_last_seen_at_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_last_seen_at_Set() const{
    return m_last_seen_at_isSet;
}

bool OAIHeavyEquipmentsListing::is_last_seen_at_Valid() const{
    return m_last_seen_at_isValid;
}

QString OAIHeavyEquipmentsListing::getLastSeenAtDate() const {
    return m_last_seen_at_date;
}
void OAIHeavyEquipmentsListing::setLastSeenAtDate(const QString &last_seen_at_date) {
    m_last_seen_at_date = last_seen_at_date;
    m_last_seen_at_date_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_last_seen_at_date_Set() const{
    return m_last_seen_at_date_isSet;
}

bool OAIHeavyEquipmentsListing::is_last_seen_at_date_Valid() const{
    return m_last_seen_at_date_isValid;
}

OAIListingNestMedia OAIHeavyEquipmentsListing::getMedia() const {
    return m_media;
}
void OAIHeavyEquipmentsListing::setMedia(const OAIListingNestMedia &media) {
    m_media = media;
    m_media_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_media_Set() const{
    return m_media_isSet;
}

bool OAIHeavyEquipmentsListing::is_media_Valid() const{
    return m_media_isValid;
}

qint32 OAIHeavyEquipmentsListing::getMiles() const {
    return m_miles;
}
void OAIHeavyEquipmentsListing::setMiles(const qint32 &miles) {
    m_miles = miles;
    m_miles_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_miles_Set() const{
    return m_miles_isSet;
}

bool OAIHeavyEquipmentsListing::is_miles_Valid() const{
    return m_miles_isValid;
}

qint32 OAIHeavyEquipmentsListing::getMsrp() const {
    return m_msrp;
}
void OAIHeavyEquipmentsListing::setMsrp(const qint32 &msrp) {
    m_msrp = msrp;
    m_msrp_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_msrp_Set() const{
    return m_msrp_isSet;
}

bool OAIHeavyEquipmentsListing::is_msrp_Valid() const{
    return m_msrp_isValid;
}

qint32 OAIHeavyEquipmentsListing::getPrice() const {
    return m_price;
}
void OAIHeavyEquipmentsListing::setPrice(const qint32 &price) {
    m_price = price;
    m_price_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_price_Set() const{
    return m_price_isSet;
}

bool OAIHeavyEquipmentsListing::is_price_Valid() const{
    return m_price_isValid;
}

double OAIHeavyEquipmentsListing::getScrapedAt() const {
    return m_scraped_at;
}
void OAIHeavyEquipmentsListing::setScrapedAt(const double &scraped_at) {
    m_scraped_at = scraped_at;
    m_scraped_at_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_scraped_at_Set() const{
    return m_scraped_at_isSet;
}

bool OAIHeavyEquipmentsListing::is_scraped_at_Valid() const{
    return m_scraped_at_isValid;
}

QString OAIHeavyEquipmentsListing::getScrapedAtDate() const {
    return m_scraped_at_date;
}
void OAIHeavyEquipmentsListing::setScrapedAtDate(const QString &scraped_at_date) {
    m_scraped_at_date = scraped_at_date;
    m_scraped_at_date_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_scraped_at_date_Set() const{
    return m_scraped_at_date_isSet;
}

bool OAIHeavyEquipmentsListing::is_scraped_at_date_Valid() const{
    return m_scraped_at_date_isValid;
}

QString OAIHeavyEquipmentsListing::getSellerType() const {
    return m_seller_type;
}
void OAIHeavyEquipmentsListing::setSellerType(const QString &seller_type) {
    m_seller_type = seller_type;
    m_seller_type_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_seller_type_Set() const{
    return m_seller_type_isSet;
}

bool OAIHeavyEquipmentsListing::is_seller_type_Valid() const{
    return m_seller_type_isValid;
}

QString OAIHeavyEquipmentsListing::getSource() const {
    return m_source;
}
void OAIHeavyEquipmentsListing::setSource(const QString &source) {
    m_source = source;
    m_source_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_source_Set() const{
    return m_source_isSet;
}

bool OAIHeavyEquipmentsListing::is_source_Valid() const{
    return m_source_isValid;
}

QString OAIHeavyEquipmentsListing::getStockNo() const {
    return m_stock_no;
}
void OAIHeavyEquipmentsListing::setStockNo(const QString &stock_no) {
    m_stock_no = stock_no;
    m_stock_no_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_stock_no_Set() const{
    return m_stock_no_isSet;
}

bool OAIHeavyEquipmentsListing::is_stock_no_Valid() const{
    return m_stock_no_isValid;
}

QString OAIHeavyEquipmentsListing::getVin() const {
    return m_vin;
}
void OAIHeavyEquipmentsListing::setVin(const QString &vin) {
    m_vin = vin;
    m_vin_isSet = true;
}

bool OAIHeavyEquipmentsListing::is_vin_Set() const{
    return m_vin_isSet;
}

bool OAIHeavyEquipmentsListing::is_vin_Valid() const{
    return m_vin_isValid;
}

bool OAIHeavyEquipmentsListing::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_build.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_dealer.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_dp_url_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_exterior_color_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_extra.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_first_seen_at_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_first_seen_at_date_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_heading_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_interior_color_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_inventory_type_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_last_seen_at_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_last_seen_at_date_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_media.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_miles_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_msrp_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_price_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_scraped_at_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_scraped_at_date_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_seller_type_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_source_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_stock_no_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_vin_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAIHeavyEquipmentsListing::isValid() const {
    // only required properties are required for the object to be considered valid
    return true;
}

} // namespace OpenAPI
