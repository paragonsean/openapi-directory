...
#include <iostream>
#include "../client/OAIApplicationChargeApi.h"
...
class Example : public QObject {
    Q_OBJECT
    QString create();
    OAICreateUsageCharge_request create();
    QString create();
public Q_SLOTS:
...
QString Example::create(){
    QString obj;
OAICreateUsageCharge_request Example::create(){
    OAICreateUsageCharge_request obj;
QString Example::create(){
    QString obj;
 return obj;
}
...
void Example::exampleFunction1(){
     OAIApplicationChargeApi apiInstance;
     
      //OAuth Authentication supported right now

      QEventLoop loop;
      connect(&apiInstance, &OAIApplicationChargeApi::createUsageChargeSignal, [&]() {
          loop.quit();
      });
      connect(&apiInstance, &OAIApplicationChargeApi::createUsageChargeSignalE, [&](QNetworkReply::NetworkError, QString error_str) {
          qDebug() << "Error happened while issuing request : " << error_str;
          loop.quit();
      });

      QString recurring_application_charge_id = create(); // QString | 課金契約ID
      ...
      OAICreateUsageCharge_request oai_create_usage_charge_request = create(); // OAICreateUsageCharge_request | 従量課金データ
      ...
      QString x_appstore_usage_charge_token = create(); // QString | アンインストール後の従量課金の精算をする際に、 `Authorization` ヘッダへアクセストークンを指定する代わりにこのヘッダを指定することで、このAPIを実行することができます。 インストール中は指定不要で、アンインストール後のみ必須となります。 アンインストールフックで通知される `usage_charge.api_token` の値を指定してください。 このヘッダは、アンインストールフックで通知される `usage_charge.closing_on` まで有効です。この期間を過ぎると従量課金を精算できなくなりますのでご注意ください。詳しくは [アプリのアンインストール](#section/API/アプリのアンインストール) をご確認ください。
      apiInstance.createUsageCharge(recurring_application_charge_idoai_create_usage_charge_requestx_appstore_usage_charge_token);
