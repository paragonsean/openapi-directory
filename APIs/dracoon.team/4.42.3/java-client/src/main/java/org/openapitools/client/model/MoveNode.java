...
public class MoveNode {
  ...
  public MoveNode timestampCreation(OffsetDateTime timestampCreation) {
  ...
  public MoveNode timestampModification(OffsetDateTime timestampModification) {
   ...
   * @throws IOException if the JSON Element is invalid with respect to MoveNode
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!MoveNode.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in MoveNode is not found in the empty JSON string", MoveNode.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!MoveNode.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `MoveNode` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : MoveNode.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!MoveNode.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'MoveNode' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<MoveNode> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(MoveNode.class));

       return (TypeAdapter<T>) new TypeAdapter<MoveNode>() {
           @Override
           public void write(JsonWriter out, MoveNode value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public MoveNode read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of MoveNode given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of MoveNode
   * @throws IOException if the JSON string is invalid with respect to MoveNode
   */
  public static MoveNode fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, MoveNode.class);
  }

  /**
   * Convert an instance of MoveNode to an JSON string
   *
   * @return JSON string
   */
