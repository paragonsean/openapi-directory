...
public class CopyNode {
  ...
  public CopyNode timestampCreation(OffsetDateTime timestampCreation) {
  ...
  public CopyNode timestampModification(OffsetDateTime timestampModification) {
   ...
   * @throws IOException if the JSON Element is invalid with respect to CopyNode
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CopyNode.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in CopyNode is not found in the empty JSON string", CopyNode.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CopyNode.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CopyNode` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : CopyNode.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!CopyNode.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'CopyNode' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<CopyNode> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(CopyNode.class));

       return (TypeAdapter<T>) new TypeAdapter<CopyNode>() {
           @Override
           public void write(JsonWriter out, CopyNode value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public CopyNode read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of CopyNode given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of CopyNode
   * @throws IOException if the JSON string is invalid with respect to CopyNode
   */
  public static CopyNode fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, CopyNode.class);
  }

  /**
   * Convert an instance of CopyNode to an JSON string
   *
   * @return JSON string
   */
