/**
 * DRACOON API
 * REST Web Services for DRACOON<br><br>This page provides an overview of all available and documented DRACOON APIs, which are grouped by tags.<br>Each tag provides a collection of APIs that are intended for a specific area of the DRACOON.<br><br><a title='Developer Information' href='https://developer.dracoon.com'>Developer Information</a>&emsp;&emsp;<a title='Get SDKs on GitHub' href='https://github.com/dracoon'>Get SDKs on GitHub</a><br><br><a title='Terms of service' href='https://www.dracoon.com/terms/general-terms-and-conditions/'>Terms of service</a>
 *
 * The version of the OpenAPI document: 4.42.3
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAICreateActiveDirectoryConfigRequest.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAICreateActiveDirectoryConfigRequest::OAICreateActiveDirectoryConfigRequest(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAICreateActiveDirectoryConfigRequest::OAICreateActiveDirectoryConfigRequest() {
    this->initializeModel();
}

OAICreateActiveDirectoryConfigRequest::~OAICreateActiveDirectoryConfigRequest() {}

void OAICreateActiveDirectoryConfigRequest::initializeModel() {

    m_ad_export_group_isSet = false;
    m_ad_export_group_isValid = false;

    m_alias_isSet = false;
    m_alias_isValid = false;

    m_create_home_folder_isSet = false;
    m_create_home_folder_isValid = false;

    m_home_folder_parent_isSet = false;
    m_home_folder_parent_isValid = false;

    m_ldap_users_domain_isSet = false;
    m_ldap_users_domain_isValid = false;

    m_sds_import_group_isSet = false;
    m_sds_import_group_isValid = false;

    m_server_admin_name_isSet = false;
    m_server_admin_name_isValid = false;

    m_server_admin_password_isSet = false;
    m_server_admin_password_isValid = false;

    m_server_ip_isSet = false;
    m_server_ip_isValid = false;

    m_server_port_isSet = false;
    m_server_port_isValid = false;

    m_ssl_finger_print_isSet = false;
    m_ssl_finger_print_isValid = false;

    m_use_ldaps_isSet = false;
    m_use_ldaps_isValid = false;

    m_user_filter_isSet = false;
    m_user_filter_isValid = false;

    m_user_import_isSet = false;
    m_user_import_isValid = false;
}

void OAICreateActiveDirectoryConfigRequest::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAICreateActiveDirectoryConfigRequest::fromJsonObject(QJsonObject json) {

    m_ad_export_group_isValid = ::OpenAPI::fromJsonValue(m_ad_export_group, json[QString("adExportGroup")]);
    m_ad_export_group_isSet = !json[QString("adExportGroup")].isNull() && m_ad_export_group_isValid;

    m_alias_isValid = ::OpenAPI::fromJsonValue(m_alias, json[QString("alias")]);
    m_alias_isSet = !json[QString("alias")].isNull() && m_alias_isValid;

    m_create_home_folder_isValid = ::OpenAPI::fromJsonValue(m_create_home_folder, json[QString("createHomeFolder")]);
    m_create_home_folder_isSet = !json[QString("createHomeFolder")].isNull() && m_create_home_folder_isValid;

    m_home_folder_parent_isValid = ::OpenAPI::fromJsonValue(m_home_folder_parent, json[QString("homeFolderParent")]);
    m_home_folder_parent_isSet = !json[QString("homeFolderParent")].isNull() && m_home_folder_parent_isValid;

    m_ldap_users_domain_isValid = ::OpenAPI::fromJsonValue(m_ldap_users_domain, json[QString("ldapUsersDomain")]);
    m_ldap_users_domain_isSet = !json[QString("ldapUsersDomain")].isNull() && m_ldap_users_domain_isValid;

    m_sds_import_group_isValid = ::OpenAPI::fromJsonValue(m_sds_import_group, json[QString("sdsImportGroup")]);
    m_sds_import_group_isSet = !json[QString("sdsImportGroup")].isNull() && m_sds_import_group_isValid;

    m_server_admin_name_isValid = ::OpenAPI::fromJsonValue(m_server_admin_name, json[QString("serverAdminName")]);
    m_server_admin_name_isSet = !json[QString("serverAdminName")].isNull() && m_server_admin_name_isValid;

    m_server_admin_password_isValid = ::OpenAPI::fromJsonValue(m_server_admin_password, json[QString("serverAdminPassword")]);
    m_server_admin_password_isSet = !json[QString("serverAdminPassword")].isNull() && m_server_admin_password_isValid;

    m_server_ip_isValid = ::OpenAPI::fromJsonValue(m_server_ip, json[QString("serverIp")]);
    m_server_ip_isSet = !json[QString("serverIp")].isNull() && m_server_ip_isValid;

    m_server_port_isValid = ::OpenAPI::fromJsonValue(m_server_port, json[QString("serverPort")]);
    m_server_port_isSet = !json[QString("serverPort")].isNull() && m_server_port_isValid;

    m_ssl_finger_print_isValid = ::OpenAPI::fromJsonValue(m_ssl_finger_print, json[QString("sslFingerPrint")]);
    m_ssl_finger_print_isSet = !json[QString("sslFingerPrint")].isNull() && m_ssl_finger_print_isValid;

    m_use_ldaps_isValid = ::OpenAPI::fromJsonValue(m_use_ldaps, json[QString("useLdaps")]);
    m_use_ldaps_isSet = !json[QString("useLdaps")].isNull() && m_use_ldaps_isValid;

    m_user_filter_isValid = ::OpenAPI::fromJsonValue(m_user_filter, json[QString("userFilter")]);
    m_user_filter_isSet = !json[QString("userFilter")].isNull() && m_user_filter_isValid;

    m_user_import_isValid = ::OpenAPI::fromJsonValue(m_user_import, json[QString("userImport")]);
    m_user_import_isSet = !json[QString("userImport")].isNull() && m_user_import_isValid;
}

QString OAICreateActiveDirectoryConfigRequest::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAICreateActiveDirectoryConfigRequest::asJsonObject() const {
    QJsonObject obj;
    if (m_ad_export_group_isSet) {
        obj.insert(QString("adExportGroup"), ::OpenAPI::toJsonValue(m_ad_export_group));
    }
    if (m_alias_isSet) {
        obj.insert(QString("alias"), ::OpenAPI::toJsonValue(m_alias));
    }
    if (m_create_home_folder_isSet) {
        obj.insert(QString("createHomeFolder"), ::OpenAPI::toJsonValue(m_create_home_folder));
    }
    if (m_home_folder_parent_isSet) {
        obj.insert(QString("homeFolderParent"), ::OpenAPI::toJsonValue(m_home_folder_parent));
    }
    if (m_ldap_users_domain_isSet) {
        obj.insert(QString("ldapUsersDomain"), ::OpenAPI::toJsonValue(m_ldap_users_domain));
    }
    if (m_sds_import_group_isSet) {
        obj.insert(QString("sdsImportGroup"), ::OpenAPI::toJsonValue(m_sds_import_group));
    }
    if (m_server_admin_name_isSet) {
        obj.insert(QString("serverAdminName"), ::OpenAPI::toJsonValue(m_server_admin_name));
    }
    if (m_server_admin_password_isSet) {
        obj.insert(QString("serverAdminPassword"), ::OpenAPI::toJsonValue(m_server_admin_password));
    }
    if (m_server_ip_isSet) {
        obj.insert(QString("serverIp"), ::OpenAPI::toJsonValue(m_server_ip));
    }
    if (m_server_port_isSet) {
        obj.insert(QString("serverPort"), ::OpenAPI::toJsonValue(m_server_port));
    }
    if (m_ssl_finger_print_isSet) {
        obj.insert(QString("sslFingerPrint"), ::OpenAPI::toJsonValue(m_ssl_finger_print));
    }
    if (m_use_ldaps_isSet) {
        obj.insert(QString("useLdaps"), ::OpenAPI::toJsonValue(m_use_ldaps));
    }
    if (m_user_filter_isSet) {
        obj.insert(QString("userFilter"), ::OpenAPI::toJsonValue(m_user_filter));
    }
    if (m_user_import_isSet) {
        obj.insert(QString("userImport"), ::OpenAPI::toJsonValue(m_user_import));
    }
    return obj;
}

QString OAICreateActiveDirectoryConfigRequest::getAdExportGroup() const {
    return m_ad_export_group;
}
void OAICreateActiveDirectoryConfigRequest::setAdExportGroup(const QString &ad_export_group) {
    m_ad_export_group = ad_export_group;
    m_ad_export_group_isSet = true;
}

bool OAICreateActiveDirectoryConfigRequest::is_ad_export_group_Set() const{
    return m_ad_export_group_isSet;
}

bool OAICreateActiveDirectoryConfigRequest::is_ad_export_group_Valid() const{
    return m_ad_export_group_isValid;
}

QString OAICreateActiveDirectoryConfigRequest::getAlias() const {
    return m_alias;
}
void OAICreateActiveDirectoryConfigRequest::setAlias(const QString &alias) {
    m_alias = alias;
    m_alias_isSet = true;
}

bool OAICreateActiveDirectoryConfigRequest::is_alias_Set() const{
    return m_alias_isSet;
}

bool OAICreateActiveDirectoryConfigRequest::is_alias_Valid() const{
    return m_alias_isValid;
}

bool OAICreateActiveDirectoryConfigRequest::isCreateHomeFolder() const {
    return m_create_home_folder;
}
void OAICreateActiveDirectoryConfigRequest::setCreateHomeFolder(const bool &create_home_folder) {
    m_create_home_folder = create_home_folder;
    m_create_home_folder_isSet = true;
}

bool OAICreateActiveDirectoryConfigRequest::is_create_home_folder_Set() const{
    return m_create_home_folder_isSet;
}

bool OAICreateActiveDirectoryConfigRequest::is_create_home_folder_Valid() const{
    return m_create_home_folder_isValid;
}

qint64 OAICreateActiveDirectoryConfigRequest::getHomeFolderParent() const {
    return m_home_folder_parent;
}
void OAICreateActiveDirectoryConfigRequest::setHomeFolderParent(const qint64 &home_folder_parent) {
    m_home_folder_parent = home_folder_parent;
    m_home_folder_parent_isSet = true;
}

bool OAICreateActiveDirectoryConfigRequest::is_home_folder_parent_Set() const{
    return m_home_folder_parent_isSet;
}

bool OAICreateActiveDirectoryConfigRequest::is_home_folder_parent_Valid() const{
    return m_home_folder_parent_isValid;
}

QString OAICreateActiveDirectoryConfigRequest::getLdapUsersDomain() const {
    return m_ldap_users_domain;
}
void OAICreateActiveDirectoryConfigRequest::setLdapUsersDomain(const QString &ldap_users_domain) {
    m_ldap_users_domain = ldap_users_domain;
    m_ldap_users_domain_isSet = true;
}

bool OAICreateActiveDirectoryConfigRequest::is_ldap_users_domain_Set() const{
    return m_ldap_users_domain_isSet;
}

bool OAICreateActiveDirectoryConfigRequest::is_ldap_users_domain_Valid() const{
    return m_ldap_users_domain_isValid;
}

qint64 OAICreateActiveDirectoryConfigRequest::getSdsImportGroup() const {
    return m_sds_import_group;
}
void OAICreateActiveDirectoryConfigRequest::setSdsImportGroup(const qint64 &sds_import_group) {
    m_sds_import_group = sds_import_group;
    m_sds_import_group_isSet = true;
}

bool OAICreateActiveDirectoryConfigRequest::is_sds_import_group_Set() const{
    return m_sds_import_group_isSet;
}

bool OAICreateActiveDirectoryConfigRequest::is_sds_import_group_Valid() const{
    return m_sds_import_group_isValid;
}

QString OAICreateActiveDirectoryConfigRequest::getServerAdminName() const {
    return m_server_admin_name;
}
void OAICreateActiveDirectoryConfigRequest::setServerAdminName(const QString &server_admin_name) {
    m_server_admin_name = server_admin_name;
    m_server_admin_name_isSet = true;
}

bool OAICreateActiveDirectoryConfigRequest::is_server_admin_name_Set() const{
    return m_server_admin_name_isSet;
}

bool OAICreateActiveDirectoryConfigRequest::is_server_admin_name_Valid() const{
    return m_server_admin_name_isValid;
}

QString OAICreateActiveDirectoryConfigRequest::getServerAdminPassword() const {
    return m_server_admin_password;
}
void OAICreateActiveDirectoryConfigRequest::setServerAdminPassword(const QString &server_admin_password) {
    m_server_admin_password = server_admin_password;
    m_server_admin_password_isSet = true;
}

bool OAICreateActiveDirectoryConfigRequest::is_server_admin_password_Set() const{
    return m_server_admin_password_isSet;
}

bool OAICreateActiveDirectoryConfigRequest::is_server_admin_password_Valid() const{
    return m_server_admin_password_isValid;
}

QString OAICreateActiveDirectoryConfigRequest::getServerIp() const {
    return m_server_ip;
}
void OAICreateActiveDirectoryConfigRequest::setServerIp(const QString &server_ip) {
    m_server_ip = server_ip;
    m_server_ip_isSet = true;
}

bool OAICreateActiveDirectoryConfigRequest::is_server_ip_Set() const{
    return m_server_ip_isSet;
}

bool OAICreateActiveDirectoryConfigRequest::is_server_ip_Valid() const{
    return m_server_ip_isValid;
}

qint32 OAICreateActiveDirectoryConfigRequest::getServerPort() const {
    return m_server_port;
}
void OAICreateActiveDirectoryConfigRequest::setServerPort(const qint32 &server_port) {
    m_server_port = server_port;
    m_server_port_isSet = true;
}

bool OAICreateActiveDirectoryConfigRequest::is_server_port_Set() const{
    return m_server_port_isSet;
}

bool OAICreateActiveDirectoryConfigRequest::is_server_port_Valid() const{
    return m_server_port_isValid;
}

QString OAICreateActiveDirectoryConfigRequest::getSslFingerPrint() const {
    return m_ssl_finger_print;
}
void OAICreateActiveDirectoryConfigRequest::setSslFingerPrint(const QString &ssl_finger_print) {
    m_ssl_finger_print = ssl_finger_print;
    m_ssl_finger_print_isSet = true;
}

bool OAICreateActiveDirectoryConfigRequest::is_ssl_finger_print_Set() const{
    return m_ssl_finger_print_isSet;
}

bool OAICreateActiveDirectoryConfigRequest::is_ssl_finger_print_Valid() const{
    return m_ssl_finger_print_isValid;
}

bool OAICreateActiveDirectoryConfigRequest::isUseLdaps() const {
    return m_use_ldaps;
}
void OAICreateActiveDirectoryConfigRequest::setUseLdaps(const bool &use_ldaps) {
    m_use_ldaps = use_ldaps;
    m_use_ldaps_isSet = true;
}

bool OAICreateActiveDirectoryConfigRequest::is_use_ldaps_Set() const{
    return m_use_ldaps_isSet;
}

bool OAICreateActiveDirectoryConfigRequest::is_use_ldaps_Valid() const{
    return m_use_ldaps_isValid;
}

QString OAICreateActiveDirectoryConfigRequest::getUserFilter() const {
    return m_user_filter;
}
void OAICreateActiveDirectoryConfigRequest::setUserFilter(const QString &user_filter) {
    m_user_filter = user_filter;
    m_user_filter_isSet = true;
}

bool OAICreateActiveDirectoryConfigRequest::is_user_filter_Set() const{
    return m_user_filter_isSet;
}

bool OAICreateActiveDirectoryConfigRequest::is_user_filter_Valid() const{
    return m_user_filter_isValid;
}

bool OAICreateActiveDirectoryConfigRequest::isUserImport() const {
    return m_user_import;
}
void OAICreateActiveDirectoryConfigRequest::setUserImport(const bool &user_import) {
    m_user_import = user_import;
    m_user_import_isSet = true;
}

bool OAICreateActiveDirectoryConfigRequest::is_user_import_Set() const{
    return m_user_import_isSet;
}

bool OAICreateActiveDirectoryConfigRequest::is_user_import_Valid() const{
    return m_user_import_isValid;
}

bool OAICreateActiveDirectoryConfigRequest::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_ad_export_group_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_alias_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_create_home_folder_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_home_folder_parent_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_ldap_users_domain_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_sds_import_group_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_server_admin_name_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_server_admin_password_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_server_ip_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_server_port_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_ssl_finger_print_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_use_ldaps_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_user_filter_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_user_import_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAICreateActiveDirectoryConfigRequest::isValid() const {
    // only required properties are required for the object to be considered valid
    return m_alias_isValid && m_ldap_users_domain_isValid && m_server_admin_name_isValid && m_server_admin_password_isValid && m_server_ip_isValid && m_server_port_isValid && m_user_filter_isValid && true;
}

} // namespace OpenAPI
