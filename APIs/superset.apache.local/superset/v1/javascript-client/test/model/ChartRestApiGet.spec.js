/**
 * Superset
 * Superset
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', process.cwd()+'/src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require(process.cwd()+'/src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.Superset);
  }
}(this, function(expect, Superset) {
  'use strict';

  var instance;

  beforeEach(function() {
    instance = new Superset.ChartRestApiGet();
  });

  var getProperty = function(object, getter, property) {
    // Use getter method if present; otherwise, get the property directly.
    if (typeof object[getter] === 'function')
      return object[getter]();
    else
      return object[property];
  }

  var setProperty = function(object, setter, property, value) {
    // Use setter method if present; otherwise, set the property directly.
    if (typeof object[setter] === 'function')
      object[setter](value);
    else
      object[property] = value;
  }

  describe('ChartRestApiGet', function() {
    it('should create an instance of ChartRestApiGet', function() {
      // uncomment below and update the code to test ChartRestApiGet
      //var instance = new Superset.ChartRestApiGet();
      //expect(instance).to.be.a(Superset.ChartRestApiGet);
    });

    it('should have the property cacheTimeout (base name: "cache_timeout")', function() {
      // uncomment below and update the code to test the property cacheTimeout
      //var instance = new Superset.ChartRestApiGet();
      //expect(instance).to.be();
    });

    it('should have the property dashboards (base name: "dashboards")', function() {
      // uncomment below and update the code to test the property dashboards
      //var instance = new Superset.ChartRestApiGet();
      //expect(instance).to.be();
    });

    it('should have the property description (base name: "description")', function() {
      // uncomment below and update the code to test the property description
      //var instance = new Superset.ChartRestApiGet();
      //expect(instance).to.be();
    });

    it('should have the property owners (base name: "owners")', function() {
      // uncomment below and update the code to test the property owners
      //var instance = new Superset.ChartRestApiGet();
      //expect(instance).to.be();
    });

    it('should have the property params (base name: "params")', function() {
      // uncomment below and update the code to test the property params
      //var instance = new Superset.ChartRestApiGet();
      //expect(instance).to.be();
    });

    it('should have the property queryContext (base name: "query_context")', function() {
      // uncomment below and update the code to test the property queryContext
      //var instance = new Superset.ChartRestApiGet();
      //expect(instance).to.be();
    });

    it('should have the property sliceName (base name: "slice_name")', function() {
      // uncomment below and update the code to test the property sliceName
      //var instance = new Superset.ChartRestApiGet();
      //expect(instance).to.be();
    });

    it('should have the property vizType (base name: "viz_type")', function() {
      // uncomment below and update the code to test the property vizType
      //var instance = new Superset.ChartRestApiGet();
      //expect(instance).to.be();
    });

  });

}));
