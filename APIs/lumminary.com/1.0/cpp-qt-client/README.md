...
#include <iostream>
#include "../client/OAILumminaryAPISpecApi.h"
...
void Example::exampleFunction1(){
     OAILumminaryAPISpecApi apiInstance;
     
      // Configure API key authorization: Bearer
      apiInstance.setApiKey("YOUR API KEY NAME","YOUR API KEY");

      QEventLoop loop;
      connect(&apiInstance, &OAILumminaryAPISpecApi::getAuthorizationsQueueSignal, [&]() {
          loop.quit();
      });
      connect(&apiInstance, &OAILumminaryAPISpecApi::getAuthorizationsQueueSignalE, [&](QNetworkReply::NetworkError, QString error_str) {
          qDebug() << "Error happened while issuing request : " << error_str;
          loop.quit();
      });

      QString product_id = create(); // QString | The UUID of the product
      ...
      QString seq_num_start = create(); // QString | The first sequence number from which to fetch (the sequence number of the last processed authorization)
      ...
      QString x_fields = create(); // QString | An optional fields mask
      apiInstance.getAuthorizationsQueue(product_idseq_num_startx_fields);
