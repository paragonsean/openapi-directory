...
void OAIMessagesApi::initializeServerConfigs() {
    ...
    "No description provided",
    QMap<QString, OAIServerVariable>()));
    _serverConfigs.insert("markMessageAsRead", defaultConf);
    _serverIndices.insert("markMessageAsRead", 0);
...
void OAIMessagesApi::markMessageAsRead(const QString &message_id, const OAIMark_Message_As_Read_Request_Body &oai_mark_message_as_read_request_body) {
    QString fullPath = QString(_serverConfigs["markMessageAsRead"][_serverIndices.value("markMessageAsRead")].URL()+"/messages/{MessageID}");
...

    {
        ...
        QByteArray output = oai_mark_message_as_read_request_body.asJson().toUtf8();
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIMessagesApi::markMessageAsReadCallback);
...
void OAIMessagesApi::markMessageAsReadCallback(OAIHttpRequestWorker *worker) {
    ...
    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT markMessageAsReadSignal();
        Q_EMIT markMessageAsReadSignalFull(worker);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT markMessageAsReadSignalE(error_type, error_str);
        Q_EMIT markMessageAsReadSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT markMessageAsReadSignalError(error_type, error_str);
        Q_EMIT markMessageAsReadSignalErrorFull(worker, error_type, error_str);
    }
}

void OAIMessagesApi::sendMessage(const OAISend_Message_Request_Body &oai_send_message_request_body) {
...

    {
        ...
        QByteArray output = oai_send_message_request_body.asJson().toUtf8();
        input.request_body.append(output);
    }
    for (auto keyValueIt = _defaultHeaders.keyValueBegin(); keyValueIt != _defaultHeaders.keyValueEnd(); keyValueIt++) {
        input.headers.insert(keyValueIt->first, keyValueIt->second);
...
void OAIMessagesApi::sendMessageCallback(OAIHttpRequestWorker *worker) {
    ...
    OAIMessage_Response output(QString(worker->response));
    worker->deleteLater();
