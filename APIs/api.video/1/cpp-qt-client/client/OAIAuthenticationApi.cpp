...
void OAIAuthenticationApi::initializeServerConfigs() {
    ...
    _serverConfigs.insert("pOSTAuthApiKey", defaultConf);
    _serverIndices.insert("pOSTAuthApiKey", 0);
    _serverConfigs.insert("pOSTAuthRefresh", defaultConf);
    _serverIndices.insert("pOSTAuthRefresh", 0);
}
...
void OAIAuthenticationApi::pOSTAuthApiKey(const ::OpenAPI::OptionalParam<OAIAuthenticate_payload> &oai_authenticate_payload) {
    QString fullPath = QString(_serverConfigs["pOSTAuthApiKey"][_serverIndices.value("pOSTAuthApiKey")].URL()+"/auth/api-key");
    ...
    if (oai_authenticate_payload.hasValue()){

        
        QByteArray output = oai_authenticate_payload.value().asJson().toUtf8();
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIAuthenticationApi::pOSTAuthApiKeyCallback);
...
void OAIAuthenticationApi::pOSTAuthApiKeyCallback(OAIHttpRequestWorker *worker) {
    ...
    OAIAccess_token output(QString(worker->response));
    worker->deleteLater();

    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT pOSTAuthApiKeySignal(output);
        Q_EMIT pOSTAuthApiKeySignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT pOSTAuthApiKeySignalE(output, error_type, error_str);
        Q_EMIT pOSTAuthApiKeySignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT pOSTAuthApiKeySignalError(output, error_type, error_str);
        Q_EMIT pOSTAuthApiKeySignalErrorFull(worker, error_type, error_str);
    }
}

void OAIAuthenticationApi::pOSTAuthRefresh(const ::OpenAPI::OptionalParam<OAIRefresh_token_payload> &oai_refresh_token_payload) {
    QString fullPath = QString(_serverConfigs["pOSTAuthRefresh"][_serverIndices.value("pOSTAuthRefresh")].URL()+"/auth/refresh");
    ...
    if (oai_refresh_token_payload.hasValue()){

        
        QByteArray output = oai_refresh_token_payload.value().asJson().toUtf8();
        input.request_body.append(output);
    }
    for (auto keyValueIt = _defaultHeaders.keyValueBegin(); keyValueIt != _defaultHeaders.keyValueEnd(); keyValueIt++) {
        input.headers.insert(keyValueIt->first, keyValueIt->second);
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIAuthenticationApi::pOSTAuthRefreshCallback);
    connect(this, &OAIAuthenticationApi::abortRequestsSignal, worker, &QObject::deleteLater);
    connect(worker, &QObject::destroyed, this, [this]() {
        if (findChildren<OAIHttpRequestWorker*>().count() == 0) {
            Q_EMIT allPendingRequestsCompleted();
...
void OAIAuthenticationApi::pOSTAuthRefreshCallback(OAIHttpRequestWorker *worker) {
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
    ...
    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT pOSTAuthRefreshSignal(output);
        Q_EMIT pOSTAuthRefreshSignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT pOSTAuthRefreshSignalE(output, error_type, error_str);
        Q_EMIT pOSTAuthRefreshSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT pOSTAuthRefreshSignalError(output, error_type, error_str);
        Q_EMIT pOSTAuthRefreshSignalErrorFull(worker, error_type, error_str);
    }
}
