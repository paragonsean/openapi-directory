...
public class BaseGistFilesValue {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(filename, language, rawUrl, size, type);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to BaseGistFilesValue
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!BaseGistFilesValue.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!BaseGistFilesValue.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BaseGistFilesValue` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
