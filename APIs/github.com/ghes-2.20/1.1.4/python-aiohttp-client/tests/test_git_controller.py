...
async def test_git_get_ref(client):
    ...
    response = await client.request(
        method='GET',
        path='/api/v3/repos/{owner}/{repo}/git/ref/{ref}'.format(owner='owner_example', repo='repo_example', ref='ref_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_git_list_matching_refs(client):
    ...
    response = await client.request(
        method='GET',
        path='/api/v3/repos/{owner}/{repo}/git/matching-refs/{ref}'.format(owner='owner_example', repo='repo_example', ref='ref_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
