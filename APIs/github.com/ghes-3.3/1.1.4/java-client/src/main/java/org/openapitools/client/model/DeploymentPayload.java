...
public class DeploymentPayload extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(DeploymentPayload.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!DeploymentPayload.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'DeploymentPayload' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<DeploymentPayload>() {
                @Override
                public void write(JsonWriter out, DeploymentPayload value) throws IOException {
                ...
                @Override
                public DeploymentPayload read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        DeploymentPayload ret = new DeploymentPayload();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonElement));
                        return ret;
                    }

                    throw new IOException(String.format("Failed deserialization for DeploymentPayload: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public DeploymentPayload(Object o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }
    ...
    public Map<String, Class<?>> getSchemas() {
        return DeploymentPayload.schemas;
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to DeploymentPayload
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            throw new IOException(String.format("The JSON string is invalid for DeploymentPayload with oneOf schemas: Map<String, Object>, String. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }

    /**
     * Create an instance of DeploymentPayload given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of DeploymentPayload
     * @throws IOException if the JSON string is invalid with respect to DeploymentPayload
     */
    public static DeploymentPayload fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, DeploymentPayload.class);
    }

    /**
     * Convert an instance of DeploymentPayload to an JSON string
     *
     * @return JSON string
     */
