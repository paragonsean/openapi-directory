...
async def test_apps_check_token(client):
    ...
    Check a token
    """
    body = openapi_server.AppsCheckTokenRequest()
    headers = { 
        'Accept': 'application/json',
    ...
    response = await client.request(
        method='POST',
        path='/api/v3/applications/{client_id}/token'.format(client_id='client_id_example'),
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_apps_delete_authorization(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/api/v3/applications/{client_id}/grant'.format(client_id='client_id_example'),
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
