...
[**gitGetAllRefs**](GitApi.md#gitGetAllRefs) | **GET** /repos/{owner}/{repo}/git/refs/{namespace} | Get all references
...
> [GitRef] gitGetAllRefs(owner, repo, namespace, opts)
...
let owner = "owner_example"; // String | 
let repo = "repo_example"; // String | 
let namespace = "namespace_example"; // String | Automatically added
let opts = {
  'perPage': 30, // Number | Results per page (max 100)
  'page': 1 // Number | Page number of the results to fetch.
};
apiInstance.gitGetAllRefs(owner, repo, namespace, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
 ...
 **owner** | **String**|  | 
 **repo** | **String**|  | 
 **namespace** | **String**| Automatically added | 
