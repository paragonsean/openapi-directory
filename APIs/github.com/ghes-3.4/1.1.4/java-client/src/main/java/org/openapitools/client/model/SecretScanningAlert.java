...
public class SecretScanningAlert {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(createdAt, htmlUrl, locationsUrl, number, resolution, resolvedAt, resolvedBy, secret, secretType, state, updatedAt, url);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to SecretScanningAlert
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SecretScanningAlert.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SecretScanningAlert.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SecretScanningAlert` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
