...
public class IssuesLockRequest {
   ...
   * @throws IOException if the JSON Element is invalid with respect to IssuesLockRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!IssuesLockRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in IssuesLockRequest is not found in the empty JSON string", IssuesLockRequest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!IssuesLockRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `IssuesLockRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      // validate the optional field `lock_reason`
      if (jsonObj.get("lock_reason") != null && !jsonObj.get("lock_reason").isJsonNull()) {
        LockReasonEnum.validateJsonElement(jsonObj.get("lock_reason"));
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!IssuesLockRequest.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'IssuesLockRequest' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<IssuesLockRequest> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(IssuesLockRequest.class));

       return (TypeAdapter<T>) new TypeAdapter<IssuesLockRequest>() {
           @Override
           public void write(JsonWriter out, IssuesLockRequest value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public IssuesLockRequest read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of IssuesLockRequest given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of IssuesLockRequest
   * @throws IOException if the JSON string is invalid with respect to IssuesLockRequest
   */
  public static IssuesLockRequest fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, IssuesLockRequest.class);
  }

  /**
   * Convert an instance of IssuesLockRequest to an JSON string
   *
   * @return JSON string
   */
