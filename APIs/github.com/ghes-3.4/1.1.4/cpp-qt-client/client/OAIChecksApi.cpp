...
void OAIChecksApi::initializeServerConfigs() {
    ...
    _serverConfigs.insert("checks_rerequestRun", defaultConf);
    _serverIndices.insert("checks_rerequestRun", 0);
...
void OAIChecksApi::checks_rerequestRun(const QString &owner, const QString &repo, const qint32 &check_run_id) {
    QString fullPath = QString(_serverConfigs["checks_rerequestRun"][_serverIndices.value("checks_rerequestRun")].URL()+"/repos/{owner}/{repo}/check-runs/{check_run_id}/rerequest");
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIChecksApi::checks_rerequestRunCallback);
    connect(this, &OAIChecksApi::abortRequestsSignal, worker, &QObject::deleteLater);
    connect(worker, &QObject::destroyed, this, [this]() {
        if (findChildren<OAIHttpRequestWorker*>().count() == 0) {
            Q_EMIT allPendingRequestsCompleted();
...
void OAIChecksApi::checks_rerequestRunCallback(OAIHttpRequestWorker *worker) {
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
    ...
    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT checks_rerequestRunSignal(output);
        Q_EMIT checks_rerequestRunSignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT checks_rerequestRunSignalE(output, error_type, error_str);
        Q_EMIT checks_rerequestRunSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT checks_rerequestRunSignalError(output, error_type, error_str);
        Q_EMIT checks_rerequestRunSignalErrorFull(worker, error_type, error_str);
    }
}
