...
async def test_apps_scope_token(client):
    ...
    """
    body = openapi_server.AppsScopeTokenRequest()
    headers = { 
        'Accept': 'application/json',
        'Content-Type': 'application/json',
    }
    response = await client.request(
        method='POST',
        path='/api/v3/applications/{client_id}/token/scoped'.format(client_id='client_id_example'),
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_apps_suspend_installation(client):
    ...
    response = await client.request(
        method='PUT',
        path='/api/v3/app/installations/{installation_id}/suspended'.format(installation_id=56),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_apps_update_webhook_config_for_app(client):
    ...
    """
    body = openapi_server.AppsUpdateWebhookConfigForAppRequest()
    headers = { 
        'Accept': 'application/json',
        'Content-Type': 'application/json',
    }
