...
import RunnerNoLabels from '../model/RunnerNoLabels';
...
export default class ActionsApi {
    ...
    */
     ...
     * @param {String} error Error message, if any.
     * @param {module:model/RunnerNoLabels} data The data returned by the service call.
     * @param {String} response The complete HTTP response.
     */
     ...
     * data is of type: {@link module:model/RunnerNoLabels}
     */
     ...
     * @param {Number} runId The id of the workflow run.
     * @param {Object} opts Optional parameters
     * @param {module:model/String} [filter = 'latest')] Filters jobs by their `completed_at` timestamp. Can be one of:   \\* `latest`: Returns jobs from the most recent execution of the workflow run.   \\* `all`: Returns all jobs for a workflow run, including from old executions of the workflow run.
     * @param {Number} [perPage = 30)] Results per page (max 100)
     * @param {Number} [page = 1)] Page number of the results to fetch.
     ...
     * @param {String} [event] Returns workflow run triggered by the event you specify. For example, `push`, `pull_request` or `issue`. For more information, see \"[Events that trigger workflows](https://help.github.com/en/actions/automating-your-workflow-with-github-actions/events-that-trigger-workflows).\"
     * @param {module:model/String} [status] Returns workflow runs with the check run `status` or `conclusion` that you specify. For example, a conclusion can be `success` or a status can be `in_progress`. Only GitHub can set a status of `waiting` or `requested`. For a list of the possible `status` and `conclusion` options, see \"[Create a check run](https://docs.github.com/enterprise-server@2.22/rest/reference/checks#create-a-check-run).\"
     * @param {Number} [perPage = 30)] Results per page (max 100)
     * @param {Number} [page = 1)] Page number of the results to fetch.
     * @param {Date} [created] 
     * @param {Boolean} [excludePullRequests = false)] If `true` pull requests are omitted from the response (empty array).
     ...
     * data is of type: {@link Object}
     */
    actionsReRunWorkflow(owner, repo, runId, callback) {
      let postBody = null;
      // verify the required parameter 'owner' is set
      if (owner === undefined || owner === null) {
