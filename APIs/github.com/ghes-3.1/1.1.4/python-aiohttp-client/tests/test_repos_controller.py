...
async def test_repos_add_app_access_restrictions(client):
    ...
    """
    body = openapi_server.ReposSetAppAccessRestrictionsRequest()
    headers = { 
        'Accept': 'application/json',
    ...
    response = await client.request(
        method='POST',
        path='/api/v3/repos/{owner}/{repo}/branches/{branch}/protection/restrictions/apps'.format(owner='owner_example', repo='repo_example', branch='branch_example'),
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_repos_create_dispatch_event(client):
    ...
    response = await client.request(
        method='POST',
        path='/api/v3/repos/{owner}/{repo}/dispatches'.format(owner='owner_example', repo='repo_example'),
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_repos_get_webhook_config_for_repo(client):
    ...
    response = await client.request(
        method='GET',
        path='/api/v3/repos/{owner}/{repo}/hooks/{hook_id}/config'.format(owner='owner_example', repo='repo_example', hook_id=56),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_repos_rename_branch(client):
    ...
    response = await client.request(
        method='POST',
        path='/api/v3/repos/{owner}/{repo}/branches/{branch}/rename'.format(owner='owner_example', repo='repo_example', branch='branch_example'),
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_repos_update_webhook_config_for_repo(client):
    ...
    """
    body = openapi_server.ReposUpdateWebhookConfigForRepoRequest()
    headers = { 
        'Accept': 'application/json',
        'Content-Type': 'application/json',
    }
