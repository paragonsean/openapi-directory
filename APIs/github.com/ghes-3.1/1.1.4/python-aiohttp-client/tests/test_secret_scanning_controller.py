...
async def test_secret_scanning_get_alert(client):
    ...
    response = await client.request(
        method='GET',
        path='/api/v3/repos/{owner}/{repo}/secret-scanning/alerts/{alert_number}'.format(owner='owner_example', repo='repo_example', alert_number=56),
        headers=headers,
        )
...
async def test_secret_scanning_list_alerts_for_repo(client):
    ...
    response = await client.request(
        method='GET',
        path='/api/v3/repos/{owner}/{repo}/secret-scanning/alerts'.format(owner='owner_example', repo='repo_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
