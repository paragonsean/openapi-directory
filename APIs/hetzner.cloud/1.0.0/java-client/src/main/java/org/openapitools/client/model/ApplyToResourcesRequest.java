...
import org.openapitools.client.model.FirewallApplyToResources;
...
public class ApplyToResourcesRequest {
  ...
  private List<FirewallApplyToResources> applyTo = new ArrayList<>();

  public ApplyToResourcesRequest() {
  }

  public ApplyToResourcesRequest applyTo(List<FirewallApplyToResources> applyTo) {
    this.applyTo = applyTo;
    return this;
  }

  public ApplyToResourcesRequest addApplyToItem(FirewallApplyToResources applyToItem) {
  ...
  public List<FirewallApplyToResources> getApplyTo() {
    return applyTo;
  }

  public void setApplyTo(List<FirewallApplyToResources> applyTo) {
    this.applyTo = applyTo;
  }
  ...
  public boolean equals(Object o) {
    ...
    ApplyToResourcesRequest applyToResourcesRequest = (ApplyToResourcesRequest) o;
    return Objects.equals(this.applyTo, applyToResourcesRequest.applyTo);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to ApplyToResourcesRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ApplyToResourcesRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ApplyToResourcesRequest is not found in the empty JSON string", ApplyToResourcesRequest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ApplyToResourcesRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ApplyToResourcesRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ApplyToResourcesRequest.openapiRequiredFields) {
      ...
      if (!jsonObj.get("apply_to").isJsonArray()) {
      ...
      JsonArray jsonArrayapplyTo = jsonObj.getAsJsonArray("apply_to");
      // validate the required field `apply_to` (array)
      for (int i = 0; i < jsonArrayapplyTo.size(); i++) {
        FirewallApplyToResources.validateJsonElement(jsonArrayapplyTo.get(i));
      };
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ApplyToResourcesRequest.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ApplyToResourcesRequest' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ApplyToResourcesRequest> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ApplyToResourcesRequest.class));

       return (TypeAdapter<T>) new TypeAdapter<ApplyToResourcesRequest>() {
           @Override
           public void write(JsonWriter out, ApplyToResourcesRequest value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ApplyToResourcesRequest read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of ApplyToResourcesRequest given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of ApplyToResourcesRequest
   * @throws IOException if the JSON string is invalid with respect to ApplyToResourcesRequest
   */
  public static ApplyToResourcesRequest fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ApplyToResourcesRequest.class);
  }

  /**
   * Convert an instance of ApplyToResourcesRequest to an JSON string
   *
   * @return JSON string
   */
