...
public class LoadBalancerAlgorithm {
  ...
  public LoadBalancerAlgorithm() {
  }

  public LoadBalancerAlgorithm type(TypeEnum type) {
    this.type = type;
    return this;
  }
  ...
  public boolean equals(Object o) {
    ...
    LoadBalancerAlgorithm loadBalancerAlgorithm = (LoadBalancerAlgorithm) o;
    return Objects.equals(this.type, loadBalancerAlgorithm.type);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to LoadBalancerAlgorithm
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!LoadBalancerAlgorithm.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in LoadBalancerAlgorithm is not found in the empty JSON string", LoadBalancerAlgorithm.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!LoadBalancerAlgorithm.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `LoadBalancerAlgorithm` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : LoadBalancerAlgorithm.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!LoadBalancerAlgorithm.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'LoadBalancerAlgorithm' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<LoadBalancerAlgorithm> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(LoadBalancerAlgorithm.class));

       return (TypeAdapter<T>) new TypeAdapter<LoadBalancerAlgorithm>() {
           @Override
           public void write(JsonWriter out, LoadBalancerAlgorithm value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public LoadBalancerAlgorithm read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of LoadBalancerAlgorithm given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of LoadBalancerAlgorithm
   * @throws IOException if the JSON string is invalid with respect to LoadBalancerAlgorithm
   */
  public static LoadBalancerAlgorithm fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, LoadBalancerAlgorithm.class);
  }

  /**
   * Convert an instance of LoadBalancerAlgorithm to an JSON string
   *
   * @return JSON string
   */
