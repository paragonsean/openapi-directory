...
public class ActionError {
  ...
  public ActionError code(String code) {
    this.code = code;
    return this;
  }
  ...
  public ActionError message(String message) {
  ...
  public boolean equals(Object o) {
    ...
    ActionError actionError = (ActionError) o;
    return Objects.equals(this.code, actionError.code) &&
        Objects.equals(this.message, actionError.message);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to ActionError
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ActionError.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ActionError is not found in the empty JSON string", ActionError.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ActionError.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ActionError` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ActionError.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ActionError.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ActionError' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ActionError> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ActionError.class));

       return (TypeAdapter<T>) new TypeAdapter<ActionError>() {
           @Override
           public void write(JsonWriter out, ActionError value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ActionError read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of ActionError given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of ActionError
   * @throws IOException if the JSON string is invalid with respect to ActionError
   */
  public static ActionError fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ActionError.class);
  }

  /**
   * Convert an instance of ActionError to an JSON string
   *
   * @return JSON string
   */
