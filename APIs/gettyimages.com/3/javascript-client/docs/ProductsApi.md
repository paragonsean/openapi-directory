...
> ProductsResult v3ProductsGet(opts)
...
let apiInstance = new GettyImages.ProductsApi();
let opts = {
  'acceptLanguage': "acceptLanguage_example", // String | Provide a header to specify the language of result values. Supported values: cs (iStock only), de, en-GB, en-US, es, fi (iStock only), fr, hu (iStock only), id (iStock only), it, ja, ko (creative assets only), nl, pl (creative assets only), pt-BR, pt-PT, ro (iStock only), ru (creative assets only), sv, th (iStock only), tr, uk (iStock only), vi (iStock only), zh-HK (creative assets only).
  'fields': [new GettyImages.ProductFieldValues()] // [ProductFieldValues] | Comma separated list of fields. Allows product download requirements to be returned.
};
apiInstance.v3ProductsGet(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
 ...
 **fields** | [**[ProductFieldValues]**](ProductFieldValues.md)| Comma separated list of fields. Allows product download requirements to be returned. | [optional] 
