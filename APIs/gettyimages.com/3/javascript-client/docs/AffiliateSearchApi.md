# GettyImages.AffiliateSearchApi
...
> AffiliateImageSearchResponse v3AffiliatesSearchImagesGet(opts)
...
let apiInstance = new GettyImages.AffiliateSearchApi();
let opts = {
...
apiInstance.v3AffiliatesSearchImagesGet(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
 ...
 **style** | [**AffiliateSearchStyle**](.md)| Filter based on graphical style of the image. | [optional] 
...
> AffiliateVideoSearchResponse v3AffiliatesSearchVideosGet(opts)
...
let opts = {
  'acceptLanguage': "acceptLanguage_example", // String | Provide a header to specify the language of result values. Supported values: cs (iStock only), de, en-GB, en-US, es, fi (iStock only), fr, hu (iStock only), id (iStock only), it, ja, ko (creative assets only), nl, pl (creative assets only), pt-BR, pt-PT, ro (iStock only), ru (creative assets only), sv, th (iStock only), tr, uk (iStock only), vi (iStock only), zh-HK (creative assets only).
  'phrase': "phrase_example" // String | 
};
apiInstance.v3AffiliatesSearchVideosGet(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
