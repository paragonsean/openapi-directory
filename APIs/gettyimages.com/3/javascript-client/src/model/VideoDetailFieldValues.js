/**
 * Getty Images
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 3
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
/**
* Enum class VideoDetailFieldValues.
* @enum {}
* @readonly
*/
export default class VideoDetailFieldValues {
    
        /**
         * value: "id"
         * @const
         */
        "id" = "id";

    
        /**
         * value: "allowed_use"
         * @const
         */
        "allowed_use" = "allowed_use";

    
        /**
         * value: "artist"
         * @const
         */
        "artist" = "artist";

    
        /**
         * value: "asset_family"
         * @const
         */
        "asset_family" = "asset_family";

    
        /**
         * value: "call_for_image"
         * @const
         */
        "call_for_image" = "call_for_image";

    
        /**
         * value: "caption"
         * @const
         */
        "caption" = "caption";

    
        /**
         * value: "city"
         * @const
         */
        "city" = "city";

    
        /**
         * value: "clip_length"
         * @const
         */
        "clip_length" = "clip_length";

    
        /**
         * value: "collection_code"
         * @const
         */
        "collection_code" = "collection_code";

    
        /**
         * value: "collection_id"
         * @const
         */
        "collection_id" = "collection_id";

    
        /**
         * value: "collection_name"
         * @const
         */
        "collection_name" = "collection_name";

    
        /**
         * value: "color_type"
         * @const
         */
        "color_type" = "color_type";

    
        /**
         * value: "comp"
         * @const
         */
        "comp" = "comp";

    
        /**
         * value: "copyright"
         * @const
         */
        "copyright" = "copyright";

    
        /**
         * value: "country"
         * @const
         */
        "country" = "country";

    
        /**
         * value: "date_created"
         * @const
         */
        "date_created" = "date_created";

    
        /**
         * value: "date_submitted"
         * @const
         */
        "date_submitted" = "date_submitted";

    
        /**
         * value: "detail_set"
         * @const
         */
        "detail_set" = "detail_set";

    
        /**
         * value: "display_set"
         * @const
         */
        "display_set" = "display_set";

    
        /**
         * value: "download_sizes"
         * @const
         */
        "download_sizes" = "download_sizes";

    
        /**
         * value: "download_product"
         * @const
         */
        "download_product" = "download_product";

    
        /**
         * value: "downloads"
         * @const
         */
        "downloads" = "downloads";

    
        /**
         * value: "editorial_segments"
         * @const
         */
        "editorial_segments" = "editorial_segments";

    
        /**
         * value: "entity_details"
         * @const
         */
        "entity_details" = "entity_details";

    
        /**
         * value: "era"
         * @const
         */
        "era" = "era";

    
        /**
         * value: "event_ids"
         * @const
         */
        "event_ids" = "event_ids";

    
        /**
         * value: "istock_collection"
         * @const
         */
        "istock_collection" = "istock_collection";

    
        /**
         * value: "istock_licenses"
         * @const
         */
        "istock_licenses" = "istock_licenses";

    
        /**
         * value: "key_frames"
         * @const
         */
        "key_frames" = "key_frames";

    
        /**
         * value: "keywords"
         * @const
         */
        "keywords" = "keywords";

    
        /**
         * value: "license_model"
         * @const
         */
        "license_model" = "license_model";

    
        /**
         * value: "mastered_to"
         * @const
         */
        "mastered_to" = "mastered_to";

    
        /**
         * value: "object_name"
         * @const
         */
        "object_name" = "object_name";

    
        /**
         * value: "orientation"
         * @const
         */
        "orientation" = "orientation";

    
        /**
         * value: "originally_shot_on"
         * @const
         */
        "originally_shot_on" = "originally_shot_on";

    
        /**
         * value: "preview"
         * @const
         */
        "preview" = "preview";

    
        /**
         * value: "product_types"
         * @const
         */
        "product_types" = "product_types";

    
        /**
         * value: "quality_rank"
         * @const
         */
        "quality_rank" = "quality_rank";

    
        /**
         * value: "referral_destinations"
         * @const
         */
        "referral_destinations" = "referral_destinations";

    
        /**
         * value: "shot_speed"
         * @const
         */
        "shot_speed" = "shot_speed";

    
        /**
         * value: "source"
         * @const
         */
        "source" = "source";

    
        /**
         * value: "state_province"
         * @const
         */
        "state_province" = "state_province";

    
        /**
         * value: "summary_set"
         * @const
         */
        "summary_set" = "summary_set";

    
        /**
         * value: "thumb"
         * @const
         */
        "thumb" = "thumb";

    
        /**
         * value: "title"
         * @const
         */
        "title" = "title";

    

    /**
    * Returns a <code>VideoDetailFieldValues</code> enum value from a Javascript object name.
    * @param {Object} data The plain JavaScript object containing the name of the enum value.
    * @return {module:model/VideoDetailFieldValues} The enum <code>VideoDetailFieldValues</code> value.
    */
    static constructFromObject(object) {
        return object;
    }
}

