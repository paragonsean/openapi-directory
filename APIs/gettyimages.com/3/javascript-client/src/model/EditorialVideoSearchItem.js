/**
 * Getty Images
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 3
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import AllowedUse from './AllowedUse';
import Download from './Download';
import IStockLicense from './IStockLicense';
import Keyword from './Keyword';
import ReferralDestination from './ReferralDestination';
import VideoSearchItemDisplaySize from './VideoSearchItemDisplaySize';

/**
 * The EditorialVideoSearchItem model module.
 * @module model/EditorialVideoSearchItem
 * @version 3
 */
class EditorialVideoSearchItem {
    /**
     * Constructs a new <code>EditorialVideoSearchItem</code>.
     * @alias module:model/EditorialVideoSearchItem
     */
    constructor() { 
        
        EditorialVideoSearchItem.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>EditorialVideoSearchItem</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/EditorialVideoSearchItem} obj Optional instance to populate.
     * @return {module:model/EditorialVideoSearchItem} The populated <code>EditorialVideoSearchItem</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new EditorialVideoSearchItem();

            if (data.hasOwnProperty('allowed_use')) {
                obj['allowed_use'] = AllowedUse.constructFromObject(data['allowed_use']);
            }
            if (data.hasOwnProperty('artist')) {
                obj['artist'] = ApiClient.convertToType(data['artist'], 'String');
            }
            if (data.hasOwnProperty('asset_family')) {
                obj['asset_family'] = ApiClient.convertToType(data['asset_family'], 'String');
            }
            if (data.hasOwnProperty('caption')) {
                obj['caption'] = ApiClient.convertToType(data['caption'], 'String');
            }
            if (data.hasOwnProperty('clip_length')) {
                obj['clip_length'] = ApiClient.convertToType(data['clip_length'], 'String');
            }
            if (data.hasOwnProperty('collection_code')) {
                obj['collection_code'] = ApiClient.convertToType(data['collection_code'], 'String');
            }
            if (data.hasOwnProperty('collection_id')) {
                obj['collection_id'] = ApiClient.convertToType(data['collection_id'], 'Number');
            }
            if (data.hasOwnProperty('collection_name')) {
                obj['collection_name'] = ApiClient.convertToType(data['collection_name'], 'String');
            }
            if (data.hasOwnProperty('color_type')) {
                obj['color_type'] = ApiClient.convertToType(data['color_type'], 'String');
            }
            if (data.hasOwnProperty('copyright')) {
                obj['copyright'] = ApiClient.convertToType(data['copyright'], 'String');
            }
            if (data.hasOwnProperty('date_created')) {
                obj['date_created'] = ApiClient.convertToType(data['date_created'], 'Date');
            }
            if (data.hasOwnProperty('display_sizes')) {
                obj['display_sizes'] = ApiClient.convertToType(data['display_sizes'], [VideoSearchItemDisplaySize]);
            }
            if (data.hasOwnProperty('download_product')) {
                obj['download_product'] = ApiClient.convertToType(data['download_product'], 'String');
            }
            if (data.hasOwnProperty('era')) {
                obj['era'] = ApiClient.convertToType(data['era'], 'String');
            }
            if (data.hasOwnProperty('event_ids')) {
                obj['event_ids'] = ApiClient.convertToType(data['event_ids'], ['Number']);
            }
            if (data.hasOwnProperty('id')) {
                obj['id'] = ApiClient.convertToType(data['id'], 'String');
            }
            if (data.hasOwnProperty('istock_licenses')) {
                obj['istock_licenses'] = ApiClient.convertToType(data['istock_licenses'], [IStockLicense]);
            }
            if (data.hasOwnProperty('keywords')) {
                obj['keywords'] = ApiClient.convertToType(data['keywords'], [Keyword]);
            }
            if (data.hasOwnProperty('largest_downloads')) {
                obj['largest_downloads'] = ApiClient.convertToType(data['largest_downloads'], [Download]);
            }
            if (data.hasOwnProperty('license_model')) {
                obj['license_model'] = ApiClient.convertToType(data['license_model'], 'String');
            }
            if (data.hasOwnProperty('mastered_to')) {
                obj['mastered_to'] = ApiClient.convertToType(data['mastered_to'], 'String');
            }
            if (data.hasOwnProperty('originally_shot_on')) {
                obj['originally_shot_on'] = ApiClient.convertToType(data['originally_shot_on'], 'String');
            }
            if (data.hasOwnProperty('product_types')) {
                obj['product_types'] = ApiClient.convertToType(data['product_types'], ['String']);
            }
            if (data.hasOwnProperty('referral_destinations')) {
                obj['referral_destinations'] = ApiClient.convertToType(data['referral_destinations'], [ReferralDestination]);
            }
            if (data.hasOwnProperty('shot_speed')) {
                obj['shot_speed'] = ApiClient.convertToType(data['shot_speed'], 'String');
            }
            if (data.hasOwnProperty('source')) {
                obj['source'] = ApiClient.convertToType(data['source'], 'String');
            }
            if (data.hasOwnProperty('title')) {
                obj['title'] = ApiClient.convertToType(data['title'], 'String');
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>EditorialVideoSearchItem</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>EditorialVideoSearchItem</code>.
     */
    static validateJSON(data) {
        // validate the optional field `allowed_use`
        if (data['allowed_use']) { // data not null
          AllowedUse.validateJSON(data['allowed_use']);
        }
        // ensure the json data is a string
        if (data['artist'] && !(typeof data['artist'] === 'string' || data['artist'] instanceof String)) {
            throw new Error("Expected the field `artist` to be a primitive type in the JSON string but got " + data['artist']);
        }
        // ensure the json data is a string
        if (data['asset_family'] && !(typeof data['asset_family'] === 'string' || data['asset_family'] instanceof String)) {
            throw new Error("Expected the field `asset_family` to be a primitive type in the JSON string but got " + data['asset_family']);
        }
        // ensure the json data is a string
        if (data['caption'] && !(typeof data['caption'] === 'string' || data['caption'] instanceof String)) {
            throw new Error("Expected the field `caption` to be a primitive type in the JSON string but got " + data['caption']);
        }
        // ensure the json data is a string
        if (data['clip_length'] && !(typeof data['clip_length'] === 'string' || data['clip_length'] instanceof String)) {
            throw new Error("Expected the field `clip_length` to be a primitive type in the JSON string but got " + data['clip_length']);
        }
        // ensure the json data is a string
        if (data['collection_code'] && !(typeof data['collection_code'] === 'string' || data['collection_code'] instanceof String)) {
            throw new Error("Expected the field `collection_code` to be a primitive type in the JSON string but got " + data['collection_code']);
        }
        // ensure the json data is a string
        if (data['collection_name'] && !(typeof data['collection_name'] === 'string' || data['collection_name'] instanceof String)) {
            throw new Error("Expected the field `collection_name` to be a primitive type in the JSON string but got " + data['collection_name']);
        }
        // ensure the json data is a string
        if (data['color_type'] && !(typeof data['color_type'] === 'string' || data['color_type'] instanceof String)) {
            throw new Error("Expected the field `color_type` to be a primitive type in the JSON string but got " + data['color_type']);
        }
        // ensure the json data is a string
        if (data['copyright'] && !(typeof data['copyright'] === 'string' || data['copyright'] instanceof String)) {
            throw new Error("Expected the field `copyright` to be a primitive type in the JSON string but got " + data['copyright']);
        }
        if (data['display_sizes']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['display_sizes'])) {
                throw new Error("Expected the field `display_sizes` to be an array in the JSON data but got " + data['display_sizes']);
            }
            // validate the optional field `display_sizes` (array)
            for (const item of data['display_sizes']) {
                VideoSearchItemDisplaySize.validateJSON(item);
            };
        }
        // ensure the json data is a string
        if (data['download_product'] && !(typeof data['download_product'] === 'string' || data['download_product'] instanceof String)) {
            throw new Error("Expected the field `download_product` to be a primitive type in the JSON string but got " + data['download_product']);
        }
        // ensure the json data is a string
        if (data['era'] && !(typeof data['era'] === 'string' || data['era'] instanceof String)) {
            throw new Error("Expected the field `era` to be a primitive type in the JSON string but got " + data['era']);
        }
        // ensure the json data is an array
        if (!Array.isArray(data['event_ids'])) {
            throw new Error("Expected the field `event_ids` to be an array in the JSON data but got " + data['event_ids']);
        }
        // ensure the json data is a string
        if (data['id'] && !(typeof data['id'] === 'string' || data['id'] instanceof String)) {
            throw new Error("Expected the field `id` to be a primitive type in the JSON string but got " + data['id']);
        }
        if (data['istock_licenses']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['istock_licenses'])) {
                throw new Error("Expected the field `istock_licenses` to be an array in the JSON data but got " + data['istock_licenses']);
            }
            // validate the optional field `istock_licenses` (array)
            for (const item of data['istock_licenses']) {
                IStockLicense.validateJSON(item);
            };
        }
        if (data['keywords']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['keywords'])) {
                throw new Error("Expected the field `keywords` to be an array in the JSON data but got " + data['keywords']);
            }
            // validate the optional field `keywords` (array)
            for (const item of data['keywords']) {
                Keyword.validateJSON(item);
            };
        }
        if (data['largest_downloads']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['largest_downloads'])) {
                throw new Error("Expected the field `largest_downloads` to be an array in the JSON data but got " + data['largest_downloads']);
            }
            // validate the optional field `largest_downloads` (array)
            for (const item of data['largest_downloads']) {
                Download.validateJSON(item);
            };
        }
        // ensure the json data is a string
        if (data['license_model'] && !(typeof data['license_model'] === 'string' || data['license_model'] instanceof String)) {
            throw new Error("Expected the field `license_model` to be a primitive type in the JSON string but got " + data['license_model']);
        }
        // ensure the json data is a string
        if (data['mastered_to'] && !(typeof data['mastered_to'] === 'string' || data['mastered_to'] instanceof String)) {
            throw new Error("Expected the field `mastered_to` to be a primitive type in the JSON string but got " + data['mastered_to']);
        }
        // ensure the json data is a string
        if (data['originally_shot_on'] && !(typeof data['originally_shot_on'] === 'string' || data['originally_shot_on'] instanceof String)) {
            throw new Error("Expected the field `originally_shot_on` to be a primitive type in the JSON string but got " + data['originally_shot_on']);
        }
        // ensure the json data is an array
        if (!Array.isArray(data['product_types'])) {
            throw new Error("Expected the field `product_types` to be an array in the JSON data but got " + data['product_types']);
        }
        if (data['referral_destinations']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['referral_destinations'])) {
                throw new Error("Expected the field `referral_destinations` to be an array in the JSON data but got " + data['referral_destinations']);
            }
            // validate the optional field `referral_destinations` (array)
            for (const item of data['referral_destinations']) {
                ReferralDestination.validateJSON(item);
            };
        }
        // ensure the json data is a string
        if (data['shot_speed'] && !(typeof data['shot_speed'] === 'string' || data['shot_speed'] instanceof String)) {
            throw new Error("Expected the field `shot_speed` to be a primitive type in the JSON string but got " + data['shot_speed']);
        }
        // ensure the json data is a string
        if (data['source'] && !(typeof data['source'] === 'string' || data['source'] instanceof String)) {
            throw new Error("Expected the field `source` to be a primitive type in the JSON string but got " + data['source']);
        }
        // ensure the json data is a string
        if (data['title'] && !(typeof data['title'] === 'string' || data['title'] instanceof String)) {
            throw new Error("Expected the field `title` to be a primitive type in the JSON string but got " + data['title']);
        }

        return true;
    }


}



/**
 * @member {module:model/AllowedUse} allowed_use
 */
EditorialVideoSearchItem.prototype['allowed_use'] = undefined;

/**
 * @member {String} artist
 */
EditorialVideoSearchItem.prototype['artist'] = undefined;

/**
 * @member {String} asset_family
 */
EditorialVideoSearchItem.prototype['asset_family'] = undefined;

/**
 * @member {String} caption
 */
EditorialVideoSearchItem.prototype['caption'] = undefined;

/**
 * @member {String} clip_length
 */
EditorialVideoSearchItem.prototype['clip_length'] = undefined;

/**
 * @member {String} collection_code
 */
EditorialVideoSearchItem.prototype['collection_code'] = undefined;

/**
 * @member {Number} collection_id
 */
EditorialVideoSearchItem.prototype['collection_id'] = undefined;

/**
 * @member {String} collection_name
 */
EditorialVideoSearchItem.prototype['collection_name'] = undefined;

/**
 * @member {String} color_type
 */
EditorialVideoSearchItem.prototype['color_type'] = undefined;

/**
 * @member {String} copyright
 */
EditorialVideoSearchItem.prototype['copyright'] = undefined;

/**
 * @member {Date} date_created
 */
EditorialVideoSearchItem.prototype['date_created'] = undefined;

/**
 * @member {Array.<module:model/VideoSearchItemDisplaySize>} display_sizes
 */
EditorialVideoSearchItem.prototype['display_sizes'] = undefined;

/**
 * @member {String} download_product
 */
EditorialVideoSearchItem.prototype['download_product'] = undefined;

/**
 * @member {String} era
 */
EditorialVideoSearchItem.prototype['era'] = undefined;

/**
 * @member {Array.<Number>} event_ids
 */
EditorialVideoSearchItem.prototype['event_ids'] = undefined;

/**
 * @member {String} id
 */
EditorialVideoSearchItem.prototype['id'] = undefined;

/**
 * @member {Array.<module:model/IStockLicense>} istock_licenses
 */
EditorialVideoSearchItem.prototype['istock_licenses'] = undefined;

/**
 * @member {Array.<module:model/Keyword>} keywords
 */
EditorialVideoSearchItem.prototype['keywords'] = undefined;

/**
 * @member {Array.<module:model/Download>} largest_downloads
 */
EditorialVideoSearchItem.prototype['largest_downloads'] = undefined;

/**
 * @member {String} license_model
 */
EditorialVideoSearchItem.prototype['license_model'] = undefined;

/**
 * @member {String} mastered_to
 */
EditorialVideoSearchItem.prototype['mastered_to'] = undefined;

/**
 * @member {String} originally_shot_on
 */
EditorialVideoSearchItem.prototype['originally_shot_on'] = undefined;

/**
 * @member {Array.<String>} product_types
 */
EditorialVideoSearchItem.prototype['product_types'] = undefined;

/**
 * @member {Array.<module:model/ReferralDestination>} referral_destinations
 */
EditorialVideoSearchItem.prototype['referral_destinations'] = undefined;

/**
 * @member {String} shot_speed
 */
EditorialVideoSearchItem.prototype['shot_speed'] = undefined;

/**
 * @member {String} source
 */
EditorialVideoSearchItem.prototype['source'] = undefined;

/**
 * @member {String} title
 */
EditorialVideoSearchItem.prototype['title'] = undefined;






export default EditorialVideoSearchItem;

