/**
 * GeoDB Cities API
 * The GeoDB API focuses on getting global city and region data. Easily obtain country, region, and city data for use in your apps!  <ul>   <li>Filter cities by name prefix, country, location, time-zone, and even minimum population.</li>   <li>Sort cities by name, country code, elevation, and population - or any combination of these.</li>    <li>Get all country regions.</li> <li>Get all cities in a given region.</li>   <li>     Display results in multiple languages.</li> <li>RESTful API adheres to industry best-practices, including     HATEOAS-style links to facilitate paging results.   </li>    <li>Backed by cloud-based load-balanced infrastructure for resiliency and performance!</li>   <li>Data is periodically refreshed from GeoNames and WikiData.</li> </ul> <p>Notes: <ul>   <li>     Since the database is periodically updated, this may <strong>very rarely</strong> result in certain cities     being marked deleted (e.g., duplicates removed). By default, endpoints returning city data will exclude     cities marked deleted. However, in the unlikely event that this occurs while your app is paging through a set     of affected results - and you care about the paged results suddenly changing underneath - specify      <tt>includeDeleted=SINCE_YESTERDAY</tt> (or <tt>SINCE_LAST_WEEK</tt> if you're really paranoid!).   </li> </ul> <hr/> <h3>Useful Resources</h3> <ul>   <li>     SDKs     <ul>       <li>         <a href='https://www.npmjs.com/package/wft-geodb-angular-client'>Angular</a>,          <a href='https://github.com/wirefreethought/geodb-sample-angular-app'>Sample App</a>       </li>       <li><a href='https://github.com/wirefreethought/geodb-java-client'>Java</a></li>       <li><a href='https://www.npmjs.com/package/wft-geodb-js-client'>JavaScript</a></li>     </ul>     <li><a href='swagger.json'>Swagger Docs</a></li>     <li><a href='http://creativecommons.org/licenses/by/3.0/'>Usage License</a></li>   </li>       </ul> 
 *
 * The version of the OpenAPI document: 1.0.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';

/**
 * The RegionDetails model module.
 * @module model/RegionDetails
 * @version 1.0.0
 */
class RegionDetails {
    /**
     * Constructs a new <code>RegionDetails</code>.
     * Full country region details
     * @alias module:model/RegionDetails
     */
    constructor() { 
        
        RegionDetails.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>RegionDetails</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/RegionDetails} obj Optional instance to populate.
     * @return {module:model/RegionDetails} The populated <code>RegionDetails</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new RegionDetails();

            if (data.hasOwnProperty('capital')) {
                obj['capital'] = ApiClient.convertToType(data['capital'], 'String');
            }
            if (data.hasOwnProperty('countryCode')) {
                obj['countryCode'] = ApiClient.convertToType(data['countryCode'], 'String');
            }
            if (data.hasOwnProperty('fipsCode')) {
                obj['fipsCode'] = ApiClient.convertToType(data['fipsCode'], 'String');
            }
            if (data.hasOwnProperty('isoCode')) {
                obj['isoCode'] = ApiClient.convertToType(data['isoCode'], 'String');
            }
            if (data.hasOwnProperty('name')) {
                obj['name'] = ApiClient.convertToType(data['name'], 'String');
            }
            if (data.hasOwnProperty('numCities')) {
                obj['numCities'] = ApiClient.convertToType(data['numCities'], 'Number');
            }
            if (data.hasOwnProperty('wikiDataId')) {
                obj['wikiDataId'] = ApiClient.convertToType(data['wikiDataId'], 'String');
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>RegionDetails</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>RegionDetails</code>.
     */
    static validateJSON(data) {
        // ensure the json data is a string
        if (data['capital'] && !(typeof data['capital'] === 'string' || data['capital'] instanceof String)) {
            throw new Error("Expected the field `capital` to be a primitive type in the JSON string but got " + data['capital']);
        }
        // ensure the json data is a string
        if (data['countryCode'] && !(typeof data['countryCode'] === 'string' || data['countryCode'] instanceof String)) {
            throw new Error("Expected the field `countryCode` to be a primitive type in the JSON string but got " + data['countryCode']);
        }
        // ensure the json data is a string
        if (data['fipsCode'] && !(typeof data['fipsCode'] === 'string' || data['fipsCode'] instanceof String)) {
            throw new Error("Expected the field `fipsCode` to be a primitive type in the JSON string but got " + data['fipsCode']);
        }
        // ensure the json data is a string
        if (data['isoCode'] && !(typeof data['isoCode'] === 'string' || data['isoCode'] instanceof String)) {
            throw new Error("Expected the field `isoCode` to be a primitive type in the JSON string but got " + data['isoCode']);
        }
        // ensure the json data is a string
        if (data['name'] && !(typeof data['name'] === 'string' || data['name'] instanceof String)) {
            throw new Error("Expected the field `name` to be a primitive type in the JSON string but got " + data['name']);
        }
        // ensure the json data is a string
        if (data['wikiDataId'] && !(typeof data['wikiDataId'] === 'string' || data['wikiDataId'] instanceof String)) {
            throw new Error("Expected the field `wikiDataId` to be a primitive type in the JSON string but got " + data['wikiDataId']);
        }

        return true;
    }


}



/**
 * The region's capital city (varies by languageCode)
 * @member {String} capital
 */
RegionDetails.prototype['capital'] = undefined;

/**
 * An ISO-3166 country code
 * @member {String} countryCode
 */
RegionDetails.prototype['countryCode'] = undefined;

/**
 * The FIPS 10-4 region code
 * @member {String} fipsCode
 */
RegionDetails.prototype['fipsCode'] = undefined;

/**
 * The ISO region code
 * @member {String} isoCode
 */
RegionDetails.prototype['isoCode'] = undefined;

/**
 * The region name (varies by languageCode)
 * @member {String} name
 */
RegionDetails.prototype['name'] = undefined;

/**
 * The number of cities in this region
 * @member {Number} numCities
 */
RegionDetails.prototype['numCities'] = undefined;

/**
 * The region WikiData id
 * @member {String} wikiDataId
 */
RegionDetails.prototype['wikiDataId'] = undefined;






export default RegionDetails;

