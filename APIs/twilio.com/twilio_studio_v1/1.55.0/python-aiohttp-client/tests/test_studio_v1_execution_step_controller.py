...
async def test_fetch_execution_step(client):
    ...
    response = await client.request(
        method='GET',
        path='/v1/Flows/{flow_sid}/Executions/{execution_sid}/Steps/{sid}'.format(flow_sid='flow_sid_example', execution_sid='execution_sid_example', sid='sid_example'),
        headers=headers,
        )
...
async def test_list_execution_step(client):
    ...
    response = await client.request(
        method='GET',
        path='/v1/Flows/{flow_sid}/Executions/{execution_sid}/Steps'.format(flow_sid='flow_sid_example', execution_sid='execution_sid_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
