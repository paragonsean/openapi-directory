# TwilioAutopilot.AutopilotV1QueryApi
...
> AutopilotV1AssistantQuery createQuery(assistantSid, language, query, opts)
...
let apiInstance = new TwilioAutopilot.AutopilotV1QueryApi();
...
let language = "language_example"; // String | The [ISO language-country](https://docs.oracle.com/cd/E13214_01/wli/docs92/xref/xqisocodes.html) string that specifies the language used for the new query. For example: `en-US`.
let query = "query_example"; // String | The end-user's natural language input. It can be up to 2048 characters long.
let opts = {
  'modelBuild': "modelBuild_example", // String | The SID or unique name of the [Model Build](https://www.twilio.com/docs/autopilot/api/model-build) to be queried.
  'tasks': "tasks_example" // String | The list of tasks to limit the new query to. Tasks are expressed as a comma-separated list of task `unique_name` values. For example, `task-unique_name-1, task-unique_name-2`. Listing specific tasks is useful to constrain the paths that a user can take.
};
apiInstance.createQuery(assistantSid, language, query, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
let sid = "sid_example"; // String | The Twilio-provided string that uniquely identifies the Query resource to delete.
apiInstance.deleteQuery(assistantSid, sid, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
let sid = "sid_example"; // String | The Twilio-provided string that uniquely identifies the Query resource to fetch.
apiInstance.fetchQuery(assistantSid, sid, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
...
> ListQueryResponse listQuery(assistantSid, opts)
...
let opts = {
  'language': "language_example", // String | The [ISO language-country](https://docs.oracle.com/cd/E13214_01/wli/docs92/xref/xqisocodes.html) string that specifies the language used by the Query resources to read. For example: `en-US`.
  'modelBuild': "modelBuild_example", // String | The SID or unique name of the [Model Build](https://www.twilio.com/docs/autopilot/api/model-build) to be queried.
  'status': "status_example", // String | The status of the resources to read. Can be: `pending-review`, `reviewed`, or `discarded`
  'dialogueSid': "dialogueSid_example", // String | The SID of the [Dialogue](https://www.twilio.com/docs/autopilot/api/dialogue).
  'pageSize': 56, // Number | How many resources to return in each list page. The default is 50, and the maximum is 1000.
  'page': 56, // Number | The page index. This value is simply for client state.
  'pageToken': "pageToken_example" // String | The page token. This is provided by the API.
};
apiInstance.listQuery(assistantSid, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
...
> AutopilotV1AssistantQuery updateQuery(assistantSid, sid, opts)
...
let sid = "sid_example"; // String | The Twilio-provided string that uniquely identifies the Query resource to update.
let opts = {
  'sampleSid': "sampleSid_example", // String | The SID of an optional reference to the [Sample](https://www.twilio.com/docs/autopilot/api/task-sample) created from the query.
  'status': "status_example" // String | The new status of the resource. Can be: `pending-review`, `reviewed`, or `discarded`
};
apiInstance.updateQuery(assistantSid, sid, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
