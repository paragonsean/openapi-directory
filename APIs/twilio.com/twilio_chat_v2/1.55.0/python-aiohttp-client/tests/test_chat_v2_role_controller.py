...
async def test_create_role(client):
    ...
    response = await client.request(
        method='POST',
        path='/v2/Services/{service_sid}/Roles'.format(service_sid='service_sid_example'),
        headers=headers,
        data=data,
        )
...
async def test_delete_role(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/v2/Services/{service_sid}/Roles/{sid}'.format(service_sid='service_sid_example', sid='sid_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
