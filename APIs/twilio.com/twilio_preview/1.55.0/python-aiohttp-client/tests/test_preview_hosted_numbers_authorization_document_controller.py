...
async def test_create_hosted_numbers_authorization_document(client):
    """Test case for create_hosted_numbers_authorization_document
...
async def test_fetch_hosted_numbers_authorization_document(client):
    """Test case for fetch_hosted_numbers_authorization_document
    ...
    response = await client.request(
        method='GET',
        path='/HostedNumbers/AuthorizationDocuments/{sid}'.format(sid='sid_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_list_hosted_numbers_authorization_document(client):
    """Test case for list_hosted_numbers_authorization_document
...
@pytest.mark.skip("application/x-www-form-urlencoded not supported by Connexion")
async def test_update_hosted_numbers_authorization_document(client):
    """Test case for update_hosted_numbers_authorization_document
