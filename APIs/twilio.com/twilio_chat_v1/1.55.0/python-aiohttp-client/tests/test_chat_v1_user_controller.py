...
async def test_create_user(client):
    ...
    response = await client.request(
        method='POST',
        path='/v1/Services/{service_sid}/Users'.format(service_sid='service_sid_example'),
        headers=headers,
        data=data,
        )
...
async def test_delete_user(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/v1/Services/{service_sid}/Users/{sid}'.format(service_sid='service_sid_example', sid='sid_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
