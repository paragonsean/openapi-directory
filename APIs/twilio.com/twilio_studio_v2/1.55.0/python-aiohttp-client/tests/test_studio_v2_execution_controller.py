...
async def test_create_execution(client):
    ...
    response = await client.request(
        method='POST',
        path='/v2/Flows/{flow_sid}/Executions'.format(flow_sid='flow_sid_example'),
        headers=headers,
        data=data,
        )
...
async def test_delete_execution(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/v2/Flows/{flow_sid}/Executions/{sid}'.format(flow_sid='flow_sid_example', sid='sid_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
