...
async def test_create_asset(client):
    ...
    response = await client.request(
        method='POST',
        path='/v1/Services/{service_sid}/Assets'.format(service_sid='service_sid_example'),
        headers=headers,
        data=data,
        )
...
async def test_delete_asset(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/v1/Services/{service_sid}/Assets/{sid}'.format(service_sid='service_sid_example', sid='sid_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_fetch_asset(client):
...
async def test_list_asset(client):
    """Test case for list_asset
