...
async def test_create_phone_number(client):
    ...
    response = await client.request(
        method='POST',
        path='/v1/Services/{service_sid}/PhoneNumbers'.format(service_sid='service_sid_example'),
        headers=headers,
        data=data,
        )
...
async def test_delete_phone_number(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/v1/Services/{service_sid}/PhoneNumbers/{sid}'.format(service_sid='service_sid_example', sid='sid_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
