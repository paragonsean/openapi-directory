/**
 * Twilio - Api
 * This is the public Twilio REST API.
 *
 * The version of the OpenAPI document: 1.55.0
 * Contact: support@twilio.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';

/**
 * The ApiV2010AccountSipSipDomainSipCredentialListMapping model module.
 * @module model/ApiV2010AccountSipSipDomainSipCredentialListMapping
 * @version 1.55.0
 */
class ApiV2010AccountSipSipDomainSipCredentialListMapping {
    /**
     * Constructs a new <code>ApiV2010AccountSipSipDomainSipCredentialListMapping</code>.
     * @alias module:model/ApiV2010AccountSipSipDomainSipCredentialListMapping
     */
    constructor() { 
        
        ApiV2010AccountSipSipDomainSipCredentialListMapping.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>ApiV2010AccountSipSipDomainSipCredentialListMapping</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/ApiV2010AccountSipSipDomainSipCredentialListMapping} obj Optional instance to populate.
     * @return {module:model/ApiV2010AccountSipSipDomainSipCredentialListMapping} The populated <code>ApiV2010AccountSipSipDomainSipCredentialListMapping</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new ApiV2010AccountSipSipDomainSipCredentialListMapping();

            if (data.hasOwnProperty('account_sid')) {
                obj['account_sid'] = ApiClient.convertToType(data['account_sid'], 'String');
            }
            if (data.hasOwnProperty('date_created')) {
                obj['date_created'] = ApiClient.convertToType(data['date_created'], 'String');
            }
            if (data.hasOwnProperty('date_updated')) {
                obj['date_updated'] = ApiClient.convertToType(data['date_updated'], 'String');
            }
            if (data.hasOwnProperty('domain_sid')) {
                obj['domain_sid'] = ApiClient.convertToType(data['domain_sid'], 'String');
            }
            if (data.hasOwnProperty('friendly_name')) {
                obj['friendly_name'] = ApiClient.convertToType(data['friendly_name'], 'String');
            }
            if (data.hasOwnProperty('sid')) {
                obj['sid'] = ApiClient.convertToType(data['sid'], 'String');
            }
            if (data.hasOwnProperty('uri')) {
                obj['uri'] = ApiClient.convertToType(data['uri'], 'String');
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>ApiV2010AccountSipSipDomainSipCredentialListMapping</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>ApiV2010AccountSipSipDomainSipCredentialListMapping</code>.
     */
    static validateJSON(data) {
        // ensure the json data is a string
        if (data['account_sid'] && !(typeof data['account_sid'] === 'string' || data['account_sid'] instanceof String)) {
            throw new Error("Expected the field `account_sid` to be a primitive type in the JSON string but got " + data['account_sid']);
        }
        // ensure the json data is a string
        if (data['date_created'] && !(typeof data['date_created'] === 'string' || data['date_created'] instanceof String)) {
            throw new Error("Expected the field `date_created` to be a primitive type in the JSON string but got " + data['date_created']);
        }
        // ensure the json data is a string
        if (data['date_updated'] && !(typeof data['date_updated'] === 'string' || data['date_updated'] instanceof String)) {
            throw new Error("Expected the field `date_updated` to be a primitive type in the JSON string but got " + data['date_updated']);
        }
        // ensure the json data is a string
        if (data['domain_sid'] && !(typeof data['domain_sid'] === 'string' || data['domain_sid'] instanceof String)) {
            throw new Error("Expected the field `domain_sid` to be a primitive type in the JSON string but got " + data['domain_sid']);
        }
        // ensure the json data is a string
        if (data['friendly_name'] && !(typeof data['friendly_name'] === 'string' || data['friendly_name'] instanceof String)) {
            throw new Error("Expected the field `friendly_name` to be a primitive type in the JSON string but got " + data['friendly_name']);
        }
        // ensure the json data is a string
        if (data['sid'] && !(typeof data['sid'] === 'string' || data['sid'] instanceof String)) {
            throw new Error("Expected the field `sid` to be a primitive type in the JSON string but got " + data['sid']);
        }
        // ensure the json data is a string
        if (data['uri'] && !(typeof data['uri'] === 'string' || data['uri'] instanceof String)) {
            throw new Error("Expected the field `uri` to be a primitive type in the JSON string but got " + data['uri']);
        }

        return true;
    }


}



/**
 * The unique id of the Account that is responsible for this resource.
 * @member {String} account_sid
 */
ApiV2010AccountSipSipDomainSipCredentialListMapping.prototype['account_sid'] = undefined;

/**
 * The date that this resource was created, given as GMT in [RFC 2822](https://www.php.net/manual/en/class.datetime.php#datetime.constants.rfc2822) format.
 * @member {String} date_created
 */
ApiV2010AccountSipSipDomainSipCredentialListMapping.prototype['date_created'] = undefined;

/**
 * The date that this resource was last updated, given as GMT in [RFC 2822](https://www.php.net/manual/en/class.datetime.php#datetime.constants.rfc2822) format.
 * @member {String} date_updated
 */
ApiV2010AccountSipSipDomainSipCredentialListMapping.prototype['date_updated'] = undefined;

/**
 * The unique string that is created to identify the SipDomain resource.
 * @member {String} domain_sid
 */
ApiV2010AccountSipSipDomainSipCredentialListMapping.prototype['domain_sid'] = undefined;

/**
 * A human readable descriptive text for this resource, up to 64 characters long.
 * @member {String} friendly_name
 */
ApiV2010AccountSipSipDomainSipCredentialListMapping.prototype['friendly_name'] = undefined;

/**
 * A 34 character string that uniquely identifies this resource.
 * @member {String} sid
 */
ApiV2010AccountSipSipDomainSipCredentialListMapping.prototype['sid'] = undefined;

/**
 * The URI for this resource, relative to `https://api.twilio.com`
 * @member {String} uri
 */
ApiV2010AccountSipSipDomainSipCredentialListMapping.prototype['uri'] = undefined;






export default ApiV2010AccountSipSipDomainSipCredentialListMapping;

