...
> ListAvailablePhoneNumberVoipResponse listAvailablePhoneNumberVoip(accountSid, countryCode, opts)
...
let apiInstance = new TwilioApi.Api20100401VoipApi();
...
apiInstance.listAvailablePhoneNumberVoip(accountSid, countryCode, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
