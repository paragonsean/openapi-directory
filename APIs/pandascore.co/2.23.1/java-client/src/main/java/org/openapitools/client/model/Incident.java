...
public class Incident extends AbstractOpenApiSchema {
    ...
    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            ...
            final TypeAdapter<DeletionIncident> adapterDeletionIncident = gson.getDelegateAdapter(this, TypeToken.get(DeletionIncident.class));
            ...
            return (TypeAdapter<T>) new TypeAdapter<Incident>() {
                ...
                public void write(JsonWriter out, Incident value) throws IOException {
                    ...
                    // check if the actual instance is of the type `DeletionIncident`
                    if (value.getActualInstance() instanceof DeletionIncident) {
                        JsonElement element = adapterDeletionIncident.toJsonTree((DeletionIncident)value.getActualInstance());
                        elementAdapter.write(out, element);
                        return;
                    }
                    throw new IOException("Failed to serialize as the type doesn't match oneOf schemas: DeletionIncident, IncidentOfTypeLeague, IncidentOfTypeMatch, IncidentOfTypePlayer, IncidentOfTypeSerie, IncidentOfTypeTeam, IncidentOfTypeTournament");
                }
                ...
                public Incident read(JsonReader in) throws IOException {
                    ...
                    // deserialize DeletionIncident
                    try {
                        // validate the JSON object to see if any exception is thrown
                        DeletionIncident.validateJsonElement(jsonElement);
                        actualAdapter = adapterDeletionIncident;
                        match++;
                        log.log(Level.FINER, "Input data matches schema 'DeletionIncident'");
                    } catch (Exception e) {
                        // deserialization failed, continue
                        errorMessages.add(String.format("Deserialization for DeletionIncident failed with `%s`.", e.getMessage()));
                        log.log(Level.FINER, "Input data does not match schema 'DeletionIncident'", e);
                    }
                    ...
                    throw new IOException(String.format("Failed deserialization for Incident: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    static {
        ...
        schemas.put("DeletionIncident", DeletionIncident.class);
    }

    @Override
    public Map<String, Class<?>> getSchemas() {
        return Incident.schemas;
    }

    /**
     * Set the instance that matches the oneOf child schema, check
     * the instance parameter is valid against the oneOf child schemas:
     * DeletionIncident, IncidentOfTypeLeague, IncidentOfTypeMatch, IncidentOfTypePlayer, IncidentOfTypeSerie, IncidentOfTypeTeam, IncidentOfTypeTournament
     *
    ...
    public void setActualInstance(Object instance) {
        ...
        if (instance instanceof DeletionIncident) {
            super.setActualInstance(instance);
            return;
        }

        throw new RuntimeException("Invalid instance type. Must be DeletionIncident, IncidentOfTypeLeague, IncidentOfTypeMatch, IncidentOfTypePlayer, IncidentOfTypeSerie, IncidentOfTypeTeam, IncidentOfTypeTournament");
    }
     ...
     * DeletionIncident, IncidentOfTypeLeague, IncidentOfTypeMatch, IncidentOfTypePlayer, IncidentOfTypeSerie, IncidentOfTypeTeam, IncidentOfTypeTournament
     *
     * @return The actual instance (DeletionIncident, IncidentOfTypeLeague, IncidentOfTypeMatch, IncidentOfTypePlayer, IncidentOfTypeSerie, IncidentOfTypeTeam, IncidentOfTypeTournament)
     */
    @SuppressWarnings("unchecked")
    @Override
    ...
    /**
     * Get the actual instance of `DeletionIncident`. If the actual instance is not `DeletionIncident`,
     * the ClassCastException will be thrown.
     *
     * @return The actual instance of `DeletionIncident`
     * @throws ClassCastException if the instance is not `DeletionIncident`
     */
    public DeletionIncident getDeletionIncident() throws ClassCastException {
        return (DeletionIncident)super.getActualInstance();
    }
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        // validate the json string with DeletionIncident
        try {
            DeletionIncident.validateJsonElement(jsonElement);
            validCount++;
        } catch (Exception e) {
        ...
        if (validCount != 1) {
            throw new IOException(String.format("The JSON string is invalid for Incident with oneOf schemas: DeletionIncident, IncidentOfTypeLeague, IncidentOfTypeMatch, IncidentOfTypePlayer, IncidentOfTypeSerie, IncidentOfTypeTeam, IncidentOfTypeTournament. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }
