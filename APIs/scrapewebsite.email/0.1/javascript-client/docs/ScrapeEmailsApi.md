# ScrapeWebsiteEmailApi.ScrapeEmailsApi
...
> gETV1ScrapeEmailsFormat(website, opts)
...
let apiInstance = new ScrapeWebsiteEmailApi.ScrapeEmailsApi();
let website = "website_example"; // String | <p>The website (ie. www.soundflair.com)</p> 
let opts = {
  'mustInclude': "mustInclude_example" // String | <table>   <tbody>     <tr>       <td>Optional. The word(s) that the webpage must include (otherwise it will skip scraping that page). Good if you want to scrape only contact pages. Takes regex (ie. about</td>       <td>contact).</td>     </tr>   </tbody> </table> 
};
apiInstance.gETV1ScrapeEmailsFormat(website, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
