...
public class ForecastHourly {
  ...
  private List<ForecastHour> data = new ArrayList<>();
   ...
   * @throws IOException if the JSON Element is invalid with respect to ForecastHourly
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ForecastHourly.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ForecastHourly is not found in the empty JSON string", ForecastHourly.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ForecastHourly.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ForecastHourly` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("data") != null && !jsonObj.get("data").isJsonNull()) {
        ...
        if (jsonArraydata != null) {
          ...
          for (int i = 0; i < jsonArraydata.size(); i++) {
            ForecastHour.validateJsonElement(jsonArraydata.get(i));
          };
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ForecastHourly.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ForecastHourly' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ForecastHourly> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ForecastHourly.class));

       return (TypeAdapter<T>) new TypeAdapter<ForecastHourly>() {
           @Override
           public void write(JsonWriter out, ForecastHourly value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ForecastHourly read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of ForecastHourly given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of ForecastHourly
   * @throws IOException if the JSON string is invalid with respect to ForecastHourly
   */
  public static ForecastHourly fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ForecastHourly.class);
  }

  /**
   * Convert an instance of ForecastHourly to an JSON string
   *
   * @return JSON string
   */
