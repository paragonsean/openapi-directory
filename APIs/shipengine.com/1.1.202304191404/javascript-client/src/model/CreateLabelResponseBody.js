...
/**
 * The CreateLabelResponseBody model module.
 * @module model/CreateLabelResponseBody
 * @version 1.1.202304191404
 */
class CreateLabelResponseBody {
    /**
     * Constructs a new <code>CreateLabelResponseBody</code>.
     * A create label response body
     * @alias module:model/CreateLabelResponseBody
     * @implements module:model/Label
     * @param batchId {String} If this label was created as part of a [batch](https://www.shipengine.com/docs/labels/bulk/), then this is the unique ID of that batch. 
     * @param carrierCode {String} The [shipping carrier](https://www.shipengine.com/docs/carriers/setup/) who will ship the package, such as `fedex`, `dhl_express`, `stamps_com`, etc. 
     * @param carrierId {String} The unique ID of the [carrier account](https://www.shipengine.com/docs/carriers/setup/) that was used to create this label 
     * @param chargeEvent {module:model/LabelChargeEvent} The label charge event. 
     * @param createdAt {Date} The date and time that the label was created in ShipEngine.
     * @param displayScheme {module:model/DisplayScheme} The display format that the label should be shown in.
     * @param formDownload {module:model/Link} The link to download the customs form (a.k.a. commercial invoice) for this shipment, if any.  Forms are in PDF format. This field is null if the shipment does not require a customs form, or if the carrier does not support it. 
     * @param insuranceClaim {module:model/Link} The link to submit an insurance claim for the shipment.  This field is null if the shipment is not insured or if the insurance provider does not support online claim submission. 
     * @param insuranceCost {module:model/MonetaryValue} The insurance cost for this package.  Add this to the `shipment_cost` field to get the total cost. 
     * @param isInternational {Boolean} Indicates whether this is an international shipment.  That is, the originating country and destination country are different. 
     * @param isReturnLabel {Boolean} Indicates whether this is a return label.  You may also want to set the `rma_number` so you know what is being returned. 
     * @param labelDownload {module:model/LabelDownload} 
     * @param labelFormat {module:model/LabelFormat} The file format that you want the label to be in.  We recommend `pdf` format because it is supported by all carriers, whereas some carriers do not support the `png` or `zpl` formats. 
     * @param labelId {String} A string that uniquely identifies the label. This ID is generated by ShipEngine when the label is created. 
     * @param labelImageId {String} The label image resource that was used to create a custom label image.
     * @param labelLayout {module:model/LabelLayout} The layout (size) that you want the label to be in.  The `label_format` determines which sizes are allowed.  `4x6` is supported for all label formats, whereas `letter` (8.5\" x 11\") is only supported for `pdf` format. 
     * @param packageCode {String} The [package type](https://www.shipengine.com/docs/reference/list-carrier-packages/), such as `thick_envelope`, `small_flat_rate_box`, `large_package`, etc.  The code `package` indicates a custom or unknown package type. 
     * @param packages {Array.<module:model/Package>} The label's package(s).  > **Note:** Some carriers only allow one package per label.  If you attempt to create a multi-package label for a carrier that doesn't allow it, an error will be returned. 
     * @param rmaNumber {String} An optional Return Merchandise Authorization number.  This field is useful for return labels.  You can set it to any string value. 
     ...
     * @param shipDate {Date} The date that the package was (or will be) shippped.  ShipEngine will take the day of week into consideration. For example, if the carrier does not operate on Sundays, then a package that would have shipped on Sunday will ship on Monday instead. 
     * @param shipmentCost {module:model/MonetaryValue} The cost of shipping, delivery confirmation, and other carrier charges.  This amount **does not** include insurance costs. 
     * @param shipmentId {String} The shipment that this label is for.  ShipEngine can create a shipment for you automatically when you [create a label](https://www.shipengine.com/docs/labels/create-a-label/), or you can [create your own shipment](https://www.shipengine.com/docs/shipping/create-a-shipment/) and then [use it to print a label](https://www.shipengine.com/docs/labels/create-from-shipment/) 
     * @param status {module:model/LabelStatus} 
     * @param trackable {Boolean} Indicates whether the shipment is trackable, in which case the `tracking_status` field will reflect the current status and each package will have a `tracking_number`. 
     * @param trackingNumber {String} The tracking number for the package. Tracking number formats vary across carriers.
     * @param trackingStatus {module:model/TrackingStatus} The current status of the package, such as `in_transit` or `delivered`
     * @param voided {Boolean} Indicates whether the label has been [voided](https://www.shipengine.com/docs/labels/voiding/) 
     * @param voidedAt {Date} The date and time that the label was [voided](https://www.shipengine.com/docs/labels/voiding/), or `null` if the label has not been voided 
     */
    constructor(batchId, carrierCode, carrierId, chargeEvent, createdAt, displayScheme, formDownload, insuranceClaim, insuranceCost, isInternational, isReturnLabel, labelDownload, labelFormat, labelId, labelImageId, labelLayout, packageCode, packages, rmaNumber, serviceCode, shipDate, shipmentCost, shipmentId, status, trackable, trackingNumber, trackingStatus, voided, voidedAt) { 
        Label.initialize(this);
        CreateLabelResponseBody.initialize(this, batchId, carrierCode, carrierId, chargeEvent, createdAt, displayScheme, formDownload, insuranceClaim, insuranceCost, isInternational, isReturnLabel, labelDownload, labelFormat, labelId, labelImageId, labelLayout, packageCode, packages, rmaNumber, serviceCode, shipDate, shipmentCost, shipmentId, status, trackable, trackingNumber, trackingStatus, voided, voidedAt);
    }
     ...
     */
    static initialize(obj, batchId, carrierCode, carrierId, chargeEvent, createdAt, displayScheme, formDownload, insuranceClaim, insuranceCost, isInternational, isReturnLabel, labelDownload, labelFormat, labelId, labelImageId, labelLayout, packageCode, packages, rmaNumber, serviceCode, shipDate, shipmentCost, shipmentId, status, trackable, trackingNumber, trackingStatus, voided, voidedAt) { 
        obj['batch_id'] = batchId;
        obj['carrier_code'] = carrierCode;
        obj['carrier_id'] = carrierId;
        obj['charge_event'] = chargeEvent;
        obj['created_at'] = createdAt;
        obj['display_scheme'] = displayScheme;
        obj['form_download'] = formDownload;
        obj['insurance_claim'] = insuranceClaim;
        obj['insurance_cost'] = insuranceCost;
        obj['is_international'] = isInternational;
        obj['is_return_label'] = isReturnLabel;
        obj['label_download'] = labelDownload;
        obj['label_format'] = labelFormat;
        obj['label_id'] = labelId;
        obj['label_image_id'] = labelImageId;
        obj['label_layout'] = labelLayout;
        obj['package_code'] = packageCode;
        obj['packages'] = packages;
        obj['rma_number'] = rmaNumber;
        obj['service_code'] = serviceCode;
        obj['ship_date'] = shipDate;
        obj['shipment_cost'] = shipmentCost;
        obj['shipment_id'] = shipmentId;
        obj['status'] = status;
        obj['test_label'] = false;
        obj['trackable'] = trackable;
        obj['tracking_number'] = trackingNumber;
        obj['tracking_status'] = trackingStatus;
        obj['voided'] = voided;
        obj['voided_at'] = voidedAt;
    }

    /**
     * Constructs a <code>CreateLabelResponseBody</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/CreateLabelResponseBody} obj Optional instance to populate.
     * @return {module:model/CreateLabelResponseBody} The populated <code>CreateLabelResponseBody</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new CreateLabelResponseBody();
    ...
    /**
     * Validates the JSON data with respect to <code>CreateLabelResponseBody</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>CreateLabelResponseBody</code>.
     */
    static validateJSON(data) {
        // check to make sure all required properties are present in the JSON string
        for (const property of CreateLabelResponseBody.RequiredProperties) {
 ...
 */
CreateLabelResponseBody.prototype['alternative_identifiers'] = undefined;
 ...
 */
CreateLabelResponseBody.prototype['batch_id'] = undefined;
 ...
 */
CreateLabelResponseBody.prototype['carrier_code'] = undefined;
 ...
 */
CreateLabelResponseBody.prototype['carrier_id'] = undefined;

/**
 * The label charge event. 
 * @member {module:model/LabelChargeEvent} charge_event
 */
CreateLabelResponseBody.prototype['charge_event'] = undefined;

/**
 * The date and time that the label was created in ShipEngine.
 * @member {Date} created_at
 */
CreateLabelResponseBody.prototype['created_at'] = undefined;

/**
 * The display format that the label should be shown in.
 * @member {module:model/DisplayScheme} display_scheme
 */
CreateLabelResponseBody.prototype['display_scheme'] = undefined;
 ...
 */
CreateLabelResponseBody.prototype['form_download'] = undefined;
 ...
 */
CreateLabelResponseBody.prototype['insurance_claim'] = undefined;
 ...
 */
CreateLabelResponseBody.prototype['insurance_cost'] = undefined;
 ...
 */
CreateLabelResponseBody.prototype['is_international'] = undefined;
 ...
 */
CreateLabelResponseBody.prototype['is_return_label'] = undefined;

/**
 * @member {module:model/LabelDownload} label_download
 */
CreateLabelResponseBody.prototype['label_download'] = undefined;

/**
 * @member {module:model/LabelDownloadType} label_download_type
 */
CreateLabelResponseBody.prototype['label_download_type'] = undefined;

/**
 * The file format that you want the label to be in.  We recommend `pdf` format because it is supported by all carriers, whereas some carriers do not support the `png` or `zpl` formats. 
 * @member {module:model/LabelFormat} label_format
 */
CreateLabelResponseBody.prototype['label_format'] = undefined;
 ...
 */
CreateLabelResponseBody.prototype['label_id'] = undefined;

/**
 * The label image resource that was used to create a custom label image.
 * @member {String} label_image_id
 */
CreateLabelResponseBody.prototype['label_image_id'] = undefined;
 ...
 */
CreateLabelResponseBody.prototype['label_layout'] = undefined;
 ...
 */
CreateLabelResponseBody.prototype['outbound_label_id'] = undefined;
 ...
 */
CreateLabelResponseBody.prototype['package_code'] = undefined;
 ...
 */
CreateLabelResponseBody.prototype['packages'] = undefined;

/**
 * An optional Return Merchandise Authorization number.  This field is useful for return labels.  You can set it to any string value. 
 * @member {String} rma_number
 */
CreateLabelResponseBody.prototype['rma_number'] = undefined;
 ...
 */
CreateLabelResponseBody.prototype['service_code'] = undefined;
 ...
 */
CreateLabelResponseBody.prototype['ship_date'] = undefined;

/**
 * The shipment information used to generate the label
 * @member {module:model/Shipment} shipment
 */
CreateLabelResponseBody.prototype['shipment'] = undefined;
 ...
 */
CreateLabelResponseBody.prototype['shipment_cost'] = undefined;
 ...
 */
CreateLabelResponseBody.prototype['shipment_id'] = undefined;

/**
 * @member {module:model/LabelStatus} status
 */
CreateLabelResponseBody.prototype['status'] = undefined;
 ...
 * @default false
 */
CreateLabelResponseBody.prototype['test_label'] = false;
 ...
 */
CreateLabelResponseBody.prototype['voided_at'] = undefined;
...
export default CreateLabelResponseBody;
