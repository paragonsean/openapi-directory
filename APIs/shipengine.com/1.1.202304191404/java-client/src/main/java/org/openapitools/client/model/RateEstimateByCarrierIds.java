...
public class RateEstimateByCarrierIds {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(carrierIds);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to RateEstimateByCarrierIds
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!RateEstimateByCarrierIds.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!RateEstimateByCarrierIds.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `RateEstimateByCarrierIds` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // ensure the optional json data is an array if present
      if (jsonObj.get("carrier_ids") != null && !jsonObj.get("carrier_ids").isJsonNull() && !jsonObj.get("carrier_ids").isJsonArray()) {
