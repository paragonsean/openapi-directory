...
public class ShipmentRequest {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ShipmentRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ShipmentRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ShipmentRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ShipmentRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // validate the optional field `shipment`
      if (jsonObj.get("shipment") != null && !jsonObj.get("shipment").isJsonNull()) {
        AddressValidatingShipment.validateJsonElement(jsonObj.get("shipment"));
      }
  }
