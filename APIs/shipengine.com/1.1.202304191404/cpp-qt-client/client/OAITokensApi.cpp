...
void OAITokensApi::initializeServerConfigs() {
    ...
    _serverConfigs.insert("tokensGetEphemeralToken", defaultConf);
    _serverIndices.insert("tokensGetEphemeralToken", 0);
}
...
void OAITokensApi::tokensGetEphemeralToken(const ::OpenAPI::OptionalParam<OAIRedirect> &redirect) {
    QString fullPath = QString(_serverConfigs["tokensGetEphemeralToken"][_serverIndices.value("tokensGetEphemeralToken")].URL()+"/v1/tokens/ephemeral");
    ...
    if (redirect.hasValue())
    {
        ...
        if (fullPath.indexOf("?") > 0)
            ...
            fullPath.append("?");
        QString paramString = (queryStyle == "form" && true) ? "" : (queryStyle == "form" && !(true)) ? "redirect"+querySuffix : "";
        QJsonObject parameter = redirect.value().asJsonObject();
        qint32 count = 0;
        for(const QString& key : parameter.keys()) {
            ...
            switch(parameter.value(key).type()) {
                ...
                {
                    paramString.append(((queryStyle == "form") ? key : QString("redirect").append("[").append(key).append("]"))+assignOperator+parameter.value(key).toString());
                    break;
                }
                case QJsonValue::Double:
                {
                    paramString.append(((queryStyle == "form") ? key : QString("redirect").append("[").append(key).append("]"))+assignOperator+QString::number(parameter.value(key).toDouble()));
                    break;
                }
                case QJsonValue::Bool:
                {
                    paramString.append(((queryStyle == "form") ? key : QString("redirect").append("[").append(key).append("]"))+assignOperator+QVariant(parameter.value(key).toBool()).toString());
                    break;
                }
                case QJsonValue::Array:
                {
                    paramString.append(((queryStyle == "form") ? key : QString("redirect").append("[").append(key).append("]"))+assignOperator+QVariant(parameter.value(key).toArray()).toString());
                    break;
                }
                case QJsonValue::Object:
                {
                    paramString.append(((queryStyle == "form") ? key : QString("redirect").append("[").append(key).append("]"))+assignOperator+QVariant(parameter.value(key).toObject()).toString());
                    break;
                }
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAITokensApi::tokensGetEphemeralTokenCallback);
...
void OAITokensApi::tokensGetEphemeralTokenCallback(OAIHttpRequestWorker *worker) {
    ...
    OAITokens_get_ephemeral_token_response_body_yaml output(QString(worker->response));
    worker->deleteLater();

    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT tokensGetEphemeralTokenSignal(output);
        Q_EMIT tokensGetEphemeralTokenSignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT tokensGetEphemeralTokenSignalE(output, error_type, error_str);
        Q_EMIT tokensGetEphemeralTokenSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT tokensGetEphemeralTokenSignalError(output, error_type, error_str);
        Q_EMIT tokensGetEphemeralTokenSignalErrorFull(worker, error_type, error_str);
    }
}
