...
public class DateFrom extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(DateFrom.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!DateFrom.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'DateFrom' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<DateFrom>() {
                @Override
                public void write(JsonWriter out, DateFrom value) throws IOException {
                ...
                @Override
                public DateFrom read(JsonReader in) throws IOException {
                    ...
                    throw new IOException(String.format("Failed deserialization for DateFrom: no class matches result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to DateFrom
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        throw new IOException(String.format("The JSON string is invalid for DateFrom with anyOf schemas: LocalDate, OffsetDateTime. no class match the result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
    }

    /**
     * Create an instance of DateFrom given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of DateFrom
     * @throws IOException if the JSON string is invalid with respect to DateFrom
     */
    public static DateFrom fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, DateFrom.class);
    }

    /**
     * Convert an instance of DateFrom to an JSON string
     *
     * @return JSON string
     */
