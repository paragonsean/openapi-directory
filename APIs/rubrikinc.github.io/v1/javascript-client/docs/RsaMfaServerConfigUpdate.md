# RubrikRestApi.RsaMfaServerConfigUpdate
...
**baseUrl** | **String** | The base url for RSA REST API server including the host name and port number. A valid input looks like https://&lt;SECURID_ACCESS_HOST &gt;:&lt;REST_API_PORT&gt;/mfa/v1_1.  | [optional] 
...
**name** | **String** | Unique name to identify MFA server configuration. | [optional] 
...
**restApiKey** | **String** | A unique string that RSA SecurID Access generates and uses as a shared secret with REST API clients.  | [optional] 
**timeout** | **Number** | Specifies a number of seconds to wait for server response to a given authentication method.  | [optional] 
