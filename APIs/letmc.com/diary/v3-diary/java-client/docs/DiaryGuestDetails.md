

# DiaryGuestDetails

Submission Model - Structure to submit appointment guest details

## Properties

| Name | Type | Description | Notes |
|------------ | ------------- | ------------- | -------------|
|**allowMarketingCorrespondence** | **Boolean** | Marketing Correspondence Preferences |  [optional] |
|**emailAddress** | **String** | Email address |  [optional] |
|**forename** | **String** | Forename |  [optional] |
|**mobilePhone** | **String** | Mobile phone |  [optional] |
|**OID** | **String** | The unique identifier of a user, only submit this is known |  [optional] |
|**surname** | **String** | Surname |  [optional] |



