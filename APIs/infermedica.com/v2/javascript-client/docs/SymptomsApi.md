# InfermedicaApi.SymptomsApi

All URIs are relative to *https://api.infermedica.com/v2*

Method | HTTP request | Description
------------- | ------------- | -------------
[**getAllSymptoms**](SymptomsApi.md#getAllSymptoms) | **GET** /symptoms | List all symptoms
[**getSymptom**](SymptomsApi.md#getSymptom) | **GET** /symptoms/{id} | Get symptoms by id



## getAllSymptoms

> [SymptomPublic] getAllSymptoms(opts)

List all symptoms

Returns a list of all available symptoms.

### Example

```javascript
import InfermedicaApi from 'infermedica_api';

let apiInstance = new InfermedicaApi.SymptomsApi();
let opts = {
  'ageValue': 18, // Number | age value
  'ageUnit': "year", // String | unit in which age value was provided
  'enableTriage5': true // Boolean | enable 5-level triage values
};
apiInstance.getAllSymptoms(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
  }
});
```

### Parameters


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **ageValue** | **Number**| age value | [optional] 
 **ageUnit** | **String**| unit in which age value was provided | [optional] [default to &#39;year&#39;]
 **enableTriage5** | **Boolean**| enable 5-level triage values | [optional] 

### Return type

[**[SymptomPublic]**](SymptomPublic.md)

### Authorization

No authorization required

### HTTP request headers

- **Content-Type**: Not defined
- **Accept**: application/json


## getSymptom

> SymptomDetails getSymptom(id, opts)

Get symptoms by id

Returns details of a single symptom specified by id parameter.

### Example

```javascript
import InfermedicaApi from 'infermedica_api';

let apiInstance = new InfermedicaApi.SymptomsApi();
let id = "id_example"; // String | symptoms id
let opts = {
  'ageValue': 18, // Number | age value
  'ageUnit': "year", // String | unit in which age value was provided
  'enableTriage5': true // Boolean | enable 5-level triage values
};
apiInstance.getSymptom(id, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
  }
});
```

### Parameters


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **id** | **String**| symptoms id | 
 **ageValue** | **Number**| age value | [optional] 
 **ageUnit** | **String**| unit in which age value was provided | [optional] [default to &#39;year&#39;]
 **enableTriage5** | **Boolean**| enable 5-level triage values | [optional] 

### Return type

[**SymptomDetails**](SymptomDetails.md)

### Authorization

No authorization required

### HTTP request headers

- **Content-Type**: Not defined
- **Accept**: application/json

