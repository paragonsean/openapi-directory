...
async def test_create_environment(client):
    ...
    Create Environment
    """
    body = {"color":"color","name":"name","description":"description"}
    headers = { 
    ...
    response = await client.request(
        method='POST',
        path='/v1/products/{product_id}/environments'.format(product_id='product_id_example'),
        headers=headers,
        json=body,
        )
...
async def test_delete_environment(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/v1/environments/{environment_id}'.format(environment_id='environment_id_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
