...
import org.openapitools.client.model.FluxAtEnergyFlux;
...
public class FluxAtEnergy {
  ...
  private FluxAtEnergyFlux flux;

  public FluxAtEnergy() {
  }

  public FluxAtEnergy flux(FluxAtEnergyFlux flux) {
    this.flux = flux;
    return this;
  }
  ...
  public FluxAtEnergyFlux getFlux() {
    return flux;
  }

  public void setFlux(FluxAtEnergyFlux flux) {
    this.flux = flux;
  }
  ...
  public boolean equals(Object o) {
    ...
    FluxAtEnergy fluxAtEnergy = (FluxAtEnergy) o;
    return Objects.equals(this.flux, fluxAtEnergy.flux);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to FluxAtEnergy
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!FluxAtEnergy.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in FluxAtEnergy is not found in the empty JSON string", FluxAtEnergy.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!FluxAtEnergy.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `FluxAtEnergy` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // validate the optional field `flux`
      if (jsonObj.get("flux") != null && !jsonObj.get("flux").isJsonNull()) {
        FluxAtEnergyFlux.validateJsonElement(jsonObj.get("flux"));
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!FluxAtEnergy.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'FluxAtEnergy' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<FluxAtEnergy> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(FluxAtEnergy.class));

       return (TypeAdapter<T>) new TypeAdapter<FluxAtEnergy>() {
           @Override
           public void write(JsonWriter out, FluxAtEnergy value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public FluxAtEnergy read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of FluxAtEnergy given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of FluxAtEnergy
   * @throws IOException if the JSON string is invalid with respect to FluxAtEnergy
   */
  public static FluxAtEnergy fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, FluxAtEnergy.class);
  }

  /**
   * Convert an instance of FluxAtEnergy to an JSON string
   *
   * @return JSON string
   */
