/**
 * The Plaid API
 * The Plaid REST API. Please see https://plaid.com/docs/api for more details.
 *
 * The version of the OpenAPI document: 2020-09-14_1.345.1
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIRiskSignalDocumentReference.h
 *
 * Object containing metadata for the document
 */

#ifndef OAIRiskSignalDocumentReference_H
#define OAIRiskSignalDocumentReference_H

#include <QJsonObject>

#include <QJsonValue>
#include <QMap>
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {

class OAIRiskSignalDocumentReference : public OAIObject {
public:
    OAIRiskSignalDocumentReference();
    OAIRiskSignalDocumentReference(QString json);
    ~OAIRiskSignalDocumentReference() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QString getDocumentId() const;
    void setDocumentId(const QString &document_id);
    bool is_document_id_Set() const;
    bool is_document_id_Valid() const;

    QString getDocumentName() const;
    void setDocumentName(const QString &document_name);
    bool is_document_name_Set() const;
    bool is_document_name_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString m_document_id;
    bool m_document_id_isSet;
    bool m_document_id_isValid;

    QString m_document_name;
    bool m_document_name_isSet;
    bool m_document_name_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIRiskSignalDocumentReference)

#endif // OAIRiskSignalDocumentReference_H
