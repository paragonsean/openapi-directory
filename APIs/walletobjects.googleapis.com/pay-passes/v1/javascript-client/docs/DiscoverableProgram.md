# GooglePayPassesApi.DiscoverableProgram

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**merchantSigninInfo** | [**DiscoverableProgramMerchantSigninInfo**](DiscoverableProgramMerchantSigninInfo.md) |  | [optional] 
**merchantSignupInfo** | [**DiscoverableProgramMerchantSignupInfo**](DiscoverableProgramMerchantSignupInfo.md) |  | [optional] 
**state** | **String** | Visibility state of the discoverable program. | [optional] 



## Enum: StateEnum


* `STATE_UNSPECIFIED` (value: `"STATE_UNSPECIFIED"`)

* `TRUSTED_TESTERS` (value: `"TRUSTED_TESTERS"`)

* `trustedTesters` (value: `"trustedTesters"`)

* `LIVE` (value: `"LIVE"`)

* `live` (value: `"live"`)

* `DISABLED` (value: `"DISABLED"`)

* `disabled` (value: `"disabled"`)




