...
void Example::exampleFunction1(){
     OAIAddressesApi apiInstance;
      ...
      QEventLoop loop;
      connect(&apiInstance, &OAIAddressesApi::createClientAddressSignal, [&]() {
          loop.quit();
      });
      connect(&apiInstance, &OAIAddressesApi::createClientAddressSignalE, [&](QNetworkReply::NetworkError, QString error_str) {
      ...
      QString profile_id = create(); // QString | ID of the client's profile as returned by the Create profile endpoint's response, in the `id` field. It can also be an `alternativeKey` according to your custom profile schema. In this case, this request should also send the `alternativeKey` parameter.
      ...
      OAIAddress oai_address = create(); // OAIAddress | 
      apiInstance.createClientAddress(content_typeacceptprofile_idalternative_keyoai_address);
