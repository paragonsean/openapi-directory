/**
 * Cloud Datastore API
 * Accesses the schemaless NoSQL database to provide fully managed, robust, scalable storage for your application. 
 *
 * The version of the OpenAPI document: v1beta3
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import Key from './Key';
import ReadOptions from './ReadOptions';

/**
 * The LookupRequest model module.
 * @module model/LookupRequest
 * @version v1beta3
 */
class LookupRequest {
    /**
     * Constructs a new <code>LookupRequest</code>.
     * The request for Datastore.Lookup.
     * @alias module:model/LookupRequest
     */
    constructor() { 
        
        LookupRequest.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>LookupRequest</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/LookupRequest} obj Optional instance to populate.
     * @return {module:model/LookupRequest} The populated <code>LookupRequest</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new LookupRequest();

            if (data.hasOwnProperty('keys')) {
                obj['keys'] = ApiClient.convertToType(data['keys'], [Key]);
            }
            if (data.hasOwnProperty('readOptions')) {
                obj['readOptions'] = ReadOptions.constructFromObject(data['readOptions']);
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>LookupRequest</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>LookupRequest</code>.
     */
    static validateJSON(data) {
        if (data['keys']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['keys'])) {
                throw new Error("Expected the field `keys` to be an array in the JSON data but got " + data['keys']);
            }
            // validate the optional field `keys` (array)
            for (const item of data['keys']) {
                Key.validateJSON(item);
            };
        }
        // validate the optional field `readOptions`
        if (data['readOptions']) { // data not null
          ReadOptions.validateJSON(data['readOptions']);
        }

        return true;
    }


}



/**
 * Required. Keys of entities to look up.
 * @member {Array.<module:model/Key>} keys
 */
LookupRequest.prototype['keys'] = undefined;

/**
 * @member {module:model/ReadOptions} readOptions
 */
LookupRequest.prototype['readOptions'] = undefined;






export default LookupRequest;

