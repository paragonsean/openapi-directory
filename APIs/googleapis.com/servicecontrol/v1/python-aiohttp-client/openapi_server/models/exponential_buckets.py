# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server import util


class ExponentialBuckets(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, growth_factor: float=None, num_finite_buckets: int=None, scale: float=None):
        """ExponentialBuckets - a model defined in OpenAPI

        :param growth_factor: The growth_factor of this ExponentialBuckets.
        :param num_finite_buckets: The num_finite_buckets of this ExponentialBuckets.
        :param scale: The scale of this ExponentialBuckets.
        """
        self.openapi_types = {
            'growth_factor': float,
            'num_finite_buckets': int,
            'scale': float
        }

        self.attribute_map = {
            'growth_factor': 'growthFactor',
            'num_finite_buckets': 'numFiniteBuckets',
            'scale': 'scale'
        }

        self._growth_factor = growth_factor
        self._num_finite_buckets = num_finite_buckets
        self._scale = scale

    @classmethod
    def from_dict(cls, dikt: dict) -> 'ExponentialBuckets':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The ExponentialBuckets of this ExponentialBuckets.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def growth_factor(self):
        """Gets the growth_factor of this ExponentialBuckets.

        The i'th exponential bucket covers the interval [scale * growth_factor^(i-1), scale * growth_factor^i) where i ranges from 1 to num_finite_buckets inclusive. Must be larger than 1.0.

        :return: The growth_factor of this ExponentialBuckets.
        :rtype: float
        """
        return self._growth_factor

    @growth_factor.setter
    def growth_factor(self, growth_factor):
        """Sets the growth_factor of this ExponentialBuckets.

        The i'th exponential bucket covers the interval [scale * growth_factor^(i-1), scale * growth_factor^i) where i ranges from 1 to num_finite_buckets inclusive. Must be larger than 1.0.

        :param growth_factor: The growth_factor of this ExponentialBuckets.
        :type growth_factor: float
        """

        self._growth_factor = growth_factor

    @property
    def num_finite_buckets(self):
        """Gets the num_finite_buckets of this ExponentialBuckets.

        The number of finite buckets. With the underflow and overflow buckets, the total number of buckets is `num_finite_buckets` + 2. See comments on `bucket_options` for details.

        :return: The num_finite_buckets of this ExponentialBuckets.
        :rtype: int
        """
        return self._num_finite_buckets

    @num_finite_buckets.setter
    def num_finite_buckets(self, num_finite_buckets):
        """Sets the num_finite_buckets of this ExponentialBuckets.

        The number of finite buckets. With the underflow and overflow buckets, the total number of buckets is `num_finite_buckets` + 2. See comments on `bucket_options` for details.

        :param num_finite_buckets: The num_finite_buckets of this ExponentialBuckets.
        :type num_finite_buckets: int
        """

        self._num_finite_buckets = num_finite_buckets

    @property
    def scale(self):
        """Gets the scale of this ExponentialBuckets.

        The i'th exponential bucket covers the interval [scale * growth_factor^(i-1), scale * growth_factor^i) where i ranges from 1 to num_finite_buckets inclusive. Must be > 0.

        :return: The scale of this ExponentialBuckets.
        :rtype: float
        """
        return self._scale

    @scale.setter
    def scale(self, scale):
        """Sets the scale of this ExponentialBuckets.

        The i'th exponential bucket covers the interval [scale * growth_factor^(i-1), scale * growth_factor^i) where i ranges from 1 to num_finite_buckets inclusive. Must be > 0.

        :param scale: The scale of this ExponentialBuckets.
        :type scale: float
        """

        self._scale = scale
