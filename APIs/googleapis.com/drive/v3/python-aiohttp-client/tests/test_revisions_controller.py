...
async def test_drive_revisions_delete(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/drive/v3/files/{file_id}/revisions/{revision_id}'.format(file_id='file_id_example', revision_id='revision_id_example'),
        headers=headers,
        params=params,
        )
...
async def test_drive_revisions_list(client):
    ...
    response = await client.request(
        method='GET',
        path='/drive/v3/files/{file_id}/revisions'.format(file_id='file_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_drive_revisions_update(client):
    ...
    """
    body = {"modifiedTime":"2000-01-23T04:56:07.000+00:00","publishedLink":"publishedLink","kind":"drive#revision","keepForever":True,"publishAuto":True,"mimeType":"mimeType","published":True,"lastModifyingUser":{"permissionId":"permissionId","emailAddress":"emailAddress","displayName":"displayName","kind":"drive#user","me":True,"photoLink":"photoLink"},"md5Checksum":"md5Checksum","size":"size","exportLinks":{"key":"exportLinks"},"publishedOutsideDomain":True,"id":"id","originalFilename":"originalFilename"}
    params = [('$.xgafv', 'xgafv_example'),
                    ('access_token', 'access_token_example'),
                    ('alt', 'alt_example'),
                    ('callback', 'param_callback_example'),
