...
async def test_drive_drives_delete(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/drive/v3/drives/{drive_id}'.format(drive_id='drive_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_drive_drives_hide(client):
    ...
    response = await client.request(
        method='POST',
        path='/drive/v3/drives/{drive_id}/hide'.format(drive_id='drive_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_drive_drives_unhide(client):
    ...
    response = await client.request(
        method='POST',
        path='/drive/v3/drives/{drive_id}/unhide'.format(drive_id='drive_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
