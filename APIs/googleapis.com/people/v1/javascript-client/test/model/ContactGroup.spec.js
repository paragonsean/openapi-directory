/**
 * People API
 * Provides access to information about profiles and contacts.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', process.cwd()+'/src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require(process.cwd()+'/src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.PeopleApi);
  }
}(this, function(expect, PeopleApi) {
  'use strict';

  var instance;

  beforeEach(function() {
    instance = new PeopleApi.ContactGroup();
  });

  var getProperty = function(object, getter, property) {
    // Use getter method if present; otherwise, get the property directly.
    if (typeof object[getter] === 'function')
      return object[getter]();
    else
      return object[property];
  }

  var setProperty = function(object, setter, property, value) {
    // Use setter method if present; otherwise, set the property directly.
    if (typeof object[setter] === 'function')
      object[setter](value);
    else
      object[property] = value;
  }

  describe('ContactGroup', function() {
    it('should create an instance of ContactGroup', function() {
      // uncomment below and update the code to test ContactGroup
      //var instance = new PeopleApi.ContactGroup();
      //expect(instance).to.be.a(PeopleApi.ContactGroup);
    });

    it('should have the property clientData (base name: "clientData")', function() {
      // uncomment below and update the code to test the property clientData
      //var instance = new PeopleApi.ContactGroup();
      //expect(instance).to.be();
    });

    it('should have the property etag (base name: "etag")', function() {
      // uncomment below and update the code to test the property etag
      //var instance = new PeopleApi.ContactGroup();
      //expect(instance).to.be();
    });

    it('should have the property formattedName (base name: "formattedName")', function() {
      // uncomment below and update the code to test the property formattedName
      //var instance = new PeopleApi.ContactGroup();
      //expect(instance).to.be();
    });

    it('should have the property groupType (base name: "groupType")', function() {
      // uncomment below and update the code to test the property groupType
      //var instance = new PeopleApi.ContactGroup();
      //expect(instance).to.be();
    });

    it('should have the property memberCount (base name: "memberCount")', function() {
      // uncomment below and update the code to test the property memberCount
      //var instance = new PeopleApi.ContactGroup();
      //expect(instance).to.be();
    });

    it('should have the property memberResourceNames (base name: "memberResourceNames")', function() {
      // uncomment below and update the code to test the property memberResourceNames
      //var instance = new PeopleApi.ContactGroup();
      //expect(instance).to.be();
    });

    it('should have the property metadata (base name: "metadata")', function() {
      // uncomment below and update the code to test the property metadata
      //var instance = new PeopleApi.ContactGroup();
      //expect(instance).to.be();
    });

    it('should have the property name (base name: "name")', function() {
      // uncomment below and update the code to test the property name
      //var instance = new PeopleApi.ContactGroup();
      //expect(instance).to.be();
    });

    it('should have the property resourceName (base name: "resourceName")', function() {
      // uncomment below and update the code to test the property resourceName
      //var instance = new PeopleApi.ContactGroup();
      //expect(instance).to.be();
    });

  });

}));
