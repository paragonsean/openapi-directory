/**
 * Apigee API
 * Use the Apigee API to programmatically develop and manage APIs with a set of RESTful operations. Develop and secure API proxies, deploy and undeploy API proxy revisions, monitor APIs, configure environments, manage users, and more. Note: This product is available as a free trial for a time period of 60 days.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import GoogleCloudApigeeV1ApiProductRef from './GoogleCloudApigeeV1ApiProductRef';
import GoogleCloudApigeeV1Attribute from './GoogleCloudApigeeV1Attribute';
import GoogleCloudApigeeV1Credential from './GoogleCloudApigeeV1Credential';

/**
 * The GoogleCloudApigeeV1App model module.
 * @module model/GoogleCloudApigeeV1App
 * @version v1
 */
class GoogleCloudApigeeV1App {
    /**
     * Constructs a new <code>GoogleCloudApigeeV1App</code>.
     * @alias module:model/GoogleCloudApigeeV1App
     */
    constructor() { 
        
        GoogleCloudApigeeV1App.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>GoogleCloudApigeeV1App</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/GoogleCloudApigeeV1App} obj Optional instance to populate.
     * @return {module:model/GoogleCloudApigeeV1App} The populated <code>GoogleCloudApigeeV1App</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new GoogleCloudApigeeV1App();

            if (data.hasOwnProperty('apiProducts')) {
                obj['apiProducts'] = ApiClient.convertToType(data['apiProducts'], [GoogleCloudApigeeV1ApiProductRef]);
            }
            if (data.hasOwnProperty('appGroup')) {
                obj['appGroup'] = ApiClient.convertToType(data['appGroup'], 'String');
            }
            if (data.hasOwnProperty('appId')) {
                obj['appId'] = ApiClient.convertToType(data['appId'], 'String');
            }
            if (data.hasOwnProperty('attributes')) {
                obj['attributes'] = ApiClient.convertToType(data['attributes'], [GoogleCloudApigeeV1Attribute]);
            }
            if (data.hasOwnProperty('callbackUrl')) {
                obj['callbackUrl'] = ApiClient.convertToType(data['callbackUrl'], 'String');
            }
            if (data.hasOwnProperty('companyName')) {
                obj['companyName'] = ApiClient.convertToType(data['companyName'], 'String');
            }
            if (data.hasOwnProperty('createdAt')) {
                obj['createdAt'] = ApiClient.convertToType(data['createdAt'], 'String');
            }
            if (data.hasOwnProperty('credentials')) {
                obj['credentials'] = ApiClient.convertToType(data['credentials'], [GoogleCloudApigeeV1Credential]);
            }
            if (data.hasOwnProperty('developerEmail')) {
                obj['developerEmail'] = ApiClient.convertToType(data['developerEmail'], 'String');
            }
            if (data.hasOwnProperty('developerId')) {
                obj['developerId'] = ApiClient.convertToType(data['developerId'], 'String');
            }
            if (data.hasOwnProperty('keyExpiresIn')) {
                obj['keyExpiresIn'] = ApiClient.convertToType(data['keyExpiresIn'], 'String');
            }
            if (data.hasOwnProperty('lastModifiedAt')) {
                obj['lastModifiedAt'] = ApiClient.convertToType(data['lastModifiedAt'], 'String');
            }
            if (data.hasOwnProperty('name')) {
                obj['name'] = ApiClient.convertToType(data['name'], 'String');
            }
            if (data.hasOwnProperty('scopes')) {
                obj['scopes'] = ApiClient.convertToType(data['scopes'], ['String']);
            }
            if (data.hasOwnProperty('status')) {
                obj['status'] = ApiClient.convertToType(data['status'], 'String');
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>GoogleCloudApigeeV1App</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>GoogleCloudApigeeV1App</code>.
     */
    static validateJSON(data) {
        if (data['apiProducts']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['apiProducts'])) {
                throw new Error("Expected the field `apiProducts` to be an array in the JSON data but got " + data['apiProducts']);
            }
            // validate the optional field `apiProducts` (array)
            for (const item of data['apiProducts']) {
                GoogleCloudApigeeV1ApiProductRef.validateJSON(item);
            };
        }
        // ensure the json data is a string
        if (data['appGroup'] && !(typeof data['appGroup'] === 'string' || data['appGroup'] instanceof String)) {
            throw new Error("Expected the field `appGroup` to be a primitive type in the JSON string but got " + data['appGroup']);
        }
        // ensure the json data is a string
        if (data['appId'] && !(typeof data['appId'] === 'string' || data['appId'] instanceof String)) {
            throw new Error("Expected the field `appId` to be a primitive type in the JSON string but got " + data['appId']);
        }
        if (data['attributes']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['attributes'])) {
                throw new Error("Expected the field `attributes` to be an array in the JSON data but got " + data['attributes']);
            }
            // validate the optional field `attributes` (array)
            for (const item of data['attributes']) {
                GoogleCloudApigeeV1Attribute.validateJSON(item);
            };
        }
        // ensure the json data is a string
        if (data['callbackUrl'] && !(typeof data['callbackUrl'] === 'string' || data['callbackUrl'] instanceof String)) {
            throw new Error("Expected the field `callbackUrl` to be a primitive type in the JSON string but got " + data['callbackUrl']);
        }
        // ensure the json data is a string
        if (data['companyName'] && !(typeof data['companyName'] === 'string' || data['companyName'] instanceof String)) {
            throw new Error("Expected the field `companyName` to be a primitive type in the JSON string but got " + data['companyName']);
        }
        // ensure the json data is a string
        if (data['createdAt'] && !(typeof data['createdAt'] === 'string' || data['createdAt'] instanceof String)) {
            throw new Error("Expected the field `createdAt` to be a primitive type in the JSON string but got " + data['createdAt']);
        }
        if (data['credentials']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['credentials'])) {
                throw new Error("Expected the field `credentials` to be an array in the JSON data but got " + data['credentials']);
            }
            // validate the optional field `credentials` (array)
            for (const item of data['credentials']) {
                GoogleCloudApigeeV1Credential.validateJSON(item);
            };
        }
        // ensure the json data is a string
        if (data['developerEmail'] && !(typeof data['developerEmail'] === 'string' || data['developerEmail'] instanceof String)) {
            throw new Error("Expected the field `developerEmail` to be a primitive type in the JSON string but got " + data['developerEmail']);
        }
        // ensure the json data is a string
        if (data['developerId'] && !(typeof data['developerId'] === 'string' || data['developerId'] instanceof String)) {
            throw new Error("Expected the field `developerId` to be a primitive type in the JSON string but got " + data['developerId']);
        }
        // ensure the json data is a string
        if (data['keyExpiresIn'] && !(typeof data['keyExpiresIn'] === 'string' || data['keyExpiresIn'] instanceof String)) {
            throw new Error("Expected the field `keyExpiresIn` to be a primitive type in the JSON string but got " + data['keyExpiresIn']);
        }
        // ensure the json data is a string
        if (data['lastModifiedAt'] && !(typeof data['lastModifiedAt'] === 'string' || data['lastModifiedAt'] instanceof String)) {
            throw new Error("Expected the field `lastModifiedAt` to be a primitive type in the JSON string but got " + data['lastModifiedAt']);
        }
        // ensure the json data is a string
        if (data['name'] && !(typeof data['name'] === 'string' || data['name'] instanceof String)) {
            throw new Error("Expected the field `name` to be a primitive type in the JSON string but got " + data['name']);
        }
        // ensure the json data is an array
        if (!Array.isArray(data['scopes'])) {
            throw new Error("Expected the field `scopes` to be an array in the JSON data but got " + data['scopes']);
        }
        // ensure the json data is a string
        if (data['status'] && !(typeof data['status'] === 'string' || data['status'] instanceof String)) {
            throw new Error("Expected the field `status` to be a primitive type in the JSON string but got " + data['status']);
        }

        return true;
    }


}



/**
 * List of API products associated with the app.
 * @member {Array.<module:model/GoogleCloudApigeeV1ApiProductRef>} apiProducts
 */
GoogleCloudApigeeV1App.prototype['apiProducts'] = undefined;

/**
 * Name of the AppGroup
 * @member {String} appGroup
 */
GoogleCloudApigeeV1App.prototype['appGroup'] = undefined;

/**
 * ID of the app.
 * @member {String} appId
 */
GoogleCloudApigeeV1App.prototype['appId'] = undefined;

/**
 * List of attributes.
 * @member {Array.<module:model/GoogleCloudApigeeV1Attribute>} attributes
 */
GoogleCloudApigeeV1App.prototype['attributes'] = undefined;

/**
 * Callback URL used by OAuth 2.0 authorization servers to communicate authorization codes back to apps.
 * @member {String} callbackUrl
 */
GoogleCloudApigeeV1App.prototype['callbackUrl'] = undefined;

/**
 * Name of the company that owns the app.
 * @member {String} companyName
 */
GoogleCloudApigeeV1App.prototype['companyName'] = undefined;

/**
 * Output only. Unix time when the app was created.
 * @member {String} createdAt
 */
GoogleCloudApigeeV1App.prototype['createdAt'] = undefined;

/**
 * Output only. Set of credentials for the app. Credentials are API key/secret pairs associated with API products.
 * @member {Array.<module:model/GoogleCloudApigeeV1Credential>} credentials
 */
GoogleCloudApigeeV1App.prototype['credentials'] = undefined;

/**
 * Email of the developer.
 * @member {String} developerEmail
 */
GoogleCloudApigeeV1App.prototype['developerEmail'] = undefined;

/**
 * ID of the developer.
 * @member {String} developerId
 */
GoogleCloudApigeeV1App.prototype['developerId'] = undefined;

/**
 * Duration, in milliseconds, of the consumer key that will be generated for the app. The default value, -1, indicates an infinite validity period. Once set, the expiration can't be updated. json key: keyExpiresIn
 * @member {String} keyExpiresIn
 */
GoogleCloudApigeeV1App.prototype['keyExpiresIn'] = undefined;

/**
 * Output only. Last modified time as milliseconds since epoch.
 * @member {String} lastModifiedAt
 */
GoogleCloudApigeeV1App.prototype['lastModifiedAt'] = undefined;

/**
 * Name of the app.
 * @member {String} name
 */
GoogleCloudApigeeV1App.prototype['name'] = undefined;

/**
 * Scopes to apply to the app. The specified scope names must already exist on the API product that you associate with the app.
 * @member {Array.<String>} scopes
 */
GoogleCloudApigeeV1App.prototype['scopes'] = undefined;

/**
 * Status of the credential.
 * @member {String} status
 */
GoogleCloudApigeeV1App.prototype['status'] = undefined;






export default GoogleCloudApigeeV1App;

