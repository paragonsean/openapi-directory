# GooglePlayAndroidDeveloperApi.RegionalPriceMigrationConfig
...
**priceIncreaseType** | **String** | Optional. The behavior the caller wants users to see when there is a price increase during migration. If left unset, the behavior defaults to PRICE_INCREASE_TYPE_OPT_IN. Note that the first opt-out price increase migration for each app must be initiated in Play Console. | [optional] 
...
* `UNSPECIFIED` (value: `"PRICE_INCREASE_TYPE_UNSPECIFIED"`)

* `OPT_IN` (value: `"PRICE_INCREASE_TYPE_OPT_IN"`)

* `OPT_OUT` (value: `"PRICE_INCREASE_TYPE_OPT_OUT"`)
