/**
 * Google Play Android Developer API
 * Lets Android application developers access their Google Play accounts. At a high level, the expected workflow is to \"insert\" an Edit, make changes as necessary, and then \"commit\" it. 
 *
 * The version of the OpenAPI document: v3
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';

/**
 * The DeactivateSubscriptionOfferRequest model module.
 * @module model/DeactivateSubscriptionOfferRequest
 * @version v3
 */
class DeactivateSubscriptionOfferRequest {
    /**
     * Constructs a new <code>DeactivateSubscriptionOfferRequest</code>.
     * Request message for DeactivateSubscriptionOffer.
     * @alias module:model/DeactivateSubscriptionOfferRequest
     */
    constructor() { 
        
        DeactivateSubscriptionOfferRequest.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>DeactivateSubscriptionOfferRequest</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/DeactivateSubscriptionOfferRequest} obj Optional instance to populate.
     * @return {module:model/DeactivateSubscriptionOfferRequest} The populated <code>DeactivateSubscriptionOfferRequest</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new DeactivateSubscriptionOfferRequest();

            if (data.hasOwnProperty('basePlanId')) {
                obj['basePlanId'] = ApiClient.convertToType(data['basePlanId'], 'String');
            }
            if (data.hasOwnProperty('latencyTolerance')) {
                obj['latencyTolerance'] = ApiClient.convertToType(data['latencyTolerance'], 'String');
            }
            if (data.hasOwnProperty('offerId')) {
                obj['offerId'] = ApiClient.convertToType(data['offerId'], 'String');
            }
            if (data.hasOwnProperty('packageName')) {
                obj['packageName'] = ApiClient.convertToType(data['packageName'], 'String');
            }
            if (data.hasOwnProperty('productId')) {
                obj['productId'] = ApiClient.convertToType(data['productId'], 'String');
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>DeactivateSubscriptionOfferRequest</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>DeactivateSubscriptionOfferRequest</code>.
     */
    static validateJSON(data) {
        // ensure the json data is a string
        if (data['basePlanId'] && !(typeof data['basePlanId'] === 'string' || data['basePlanId'] instanceof String)) {
            throw new Error("Expected the field `basePlanId` to be a primitive type in the JSON string but got " + data['basePlanId']);
        }
        // ensure the json data is a string
        if (data['latencyTolerance'] && !(typeof data['latencyTolerance'] === 'string' || data['latencyTolerance'] instanceof String)) {
            throw new Error("Expected the field `latencyTolerance` to be a primitive type in the JSON string but got " + data['latencyTolerance']);
        }
        // ensure the json data is a string
        if (data['offerId'] && !(typeof data['offerId'] === 'string' || data['offerId'] instanceof String)) {
            throw new Error("Expected the field `offerId` to be a primitive type in the JSON string but got " + data['offerId']);
        }
        // ensure the json data is a string
        if (data['packageName'] && !(typeof data['packageName'] === 'string' || data['packageName'] instanceof String)) {
            throw new Error("Expected the field `packageName` to be a primitive type in the JSON string but got " + data['packageName']);
        }
        // ensure the json data is a string
        if (data['productId'] && !(typeof data['productId'] === 'string' || data['productId'] instanceof String)) {
            throw new Error("Expected the field `productId` to be a primitive type in the JSON string but got " + data['productId']);
        }

        return true;
    }


}



/**
 * Required. The parent base plan (ID) of the offer to deactivate.
 * @member {String} basePlanId
 */
DeactivateSubscriptionOfferRequest.prototype['basePlanId'] = undefined;

/**
 * Optional. The latency tolerance for the propagation of this product update. Defaults to latency-sensitive.
 * @member {module:model/DeactivateSubscriptionOfferRequest.LatencyToleranceEnum} latencyTolerance
 */
DeactivateSubscriptionOfferRequest.prototype['latencyTolerance'] = undefined;

/**
 * Required. The unique offer ID of the offer to deactivate.
 * @member {String} offerId
 */
DeactivateSubscriptionOfferRequest.prototype['offerId'] = undefined;

/**
 * Required. The parent app (package name) of the offer to deactivate.
 * @member {String} packageName
 */
DeactivateSubscriptionOfferRequest.prototype['packageName'] = undefined;

/**
 * Required. The parent subscription (ID) of the offer to deactivate.
 * @member {String} productId
 */
DeactivateSubscriptionOfferRequest.prototype['productId'] = undefined;





/**
 * Allowed values for the <code>latencyTolerance</code> property.
 * @enum {String}
 * @readonly
 */
DeactivateSubscriptionOfferRequest['LatencyToleranceEnum'] = {

    /**
     * value: "PRODUCT_UPDATE_LATENCY_TOLERANCE_UNSPECIFIED"
     * @const
     */
    "UNSPECIFIED": "PRODUCT_UPDATE_LATENCY_TOLERANCE_UNSPECIFIED",

    /**
     * value: "PRODUCT_UPDATE_LATENCY_TOLERANCE_LATENCY_SENSITIVE"
     * @const
     */
    "LATENCY_SENSITIVE": "PRODUCT_UPDATE_LATENCY_TOLERANCE_LATENCY_SENSITIVE",

    /**
     * value: "PRODUCT_UPDATE_LATENCY_TOLERANCE_LATENCY_TOLERANT"
     * @const
     */
    "LATENCY_TOLERANT": "PRODUCT_UPDATE_LATENCY_TOLERANCE_LATENCY_TOLERANT"
};



export default DeactivateSubscriptionOfferRequest;

