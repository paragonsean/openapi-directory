...
async def test_androidpublisher_reviews_get(client):
    ...
    response = await client.request(
        method='GET',
        path='/androidpublisher/v3/applications/{package_name}/reviews/{review_id}'.format(package_name='package_name_example', review_id='review_id_example'),
        headers=headers,
        params=params,
        )
...
async def test_androidpublisher_reviews_list(client):
    ...
    response = await client.request(
        method='GET',
        path='/androidpublisher/v3/applications/{package_name}/reviews'.format(package_name='package_name_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_androidpublisher_reviews_reply(client):
    ...
    response = await client.request(
        method='POST',
        path='/androidpublisher/v3/applications/{package_name}/reviews/{review_idrepl}'.format(package_name='package_name_example', review_id='review_id_example'),
        headers=headers,
        json=body,
        params=params,
        )
