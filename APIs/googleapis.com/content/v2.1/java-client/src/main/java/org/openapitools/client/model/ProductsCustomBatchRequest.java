...
import org.openapitools.client.model.ProductsCustomBatchRequestEntry;
...
public class ProductsCustomBatchRequest {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ProductsCustomBatchRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ProductsCustomBatchRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ProductsCustomBatchRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ProductsCustomBatchRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("entries") != null && !jsonObj.get("entries").isJsonNull()) {
        ...
        if (jsonArrayentries != null) {
          ...
          for (int i = 0; i < jsonArrayentries.size(); i++) {
            ProductsCustomBatchRequestEntry.validateJsonElement(jsonArrayentries.get(i));
          };
        }
      }
  }
