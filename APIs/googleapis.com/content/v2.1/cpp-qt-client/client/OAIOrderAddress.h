/**
 * Content API for Shopping
 * Manage your product listings and accounts for Google Shopping
 *
 * The version of the OpenAPI document: v2.1
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIOrderAddress.h
 *
 * 
 */

#ifndef OAIOrderAddress_H
#define OAIOrderAddress_H

#include <QJsonObject>

#include <QList>
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {

class OAIOrderAddress : public OAIObject {
public:
    OAIOrderAddress();
    OAIOrderAddress(QString json);
    ~OAIOrderAddress() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QString getCountry() const;
    void setCountry(const QString &country);
    bool is_country_Set() const;
    bool is_country_Valid() const;

    QList<QString> getFullAddress() const;
    void setFullAddress(const QList<QString> &full_address);
    bool is_full_address_Set() const;
    bool is_full_address_Valid() const;

    bool isIsPostOfficeBox() const;
    void setIsPostOfficeBox(const bool &is_post_office_box);
    bool is_is_post_office_box_Set() const;
    bool is_is_post_office_box_Valid() const;

    QString getLocality() const;
    void setLocality(const QString &locality);
    bool is_locality_Set() const;
    bool is_locality_Valid() const;

    QString getPostalCode() const;
    void setPostalCode(const QString &postal_code);
    bool is_postal_code_Set() const;
    bool is_postal_code_Valid() const;

    QString getRecipientName() const;
    void setRecipientName(const QString &recipient_name);
    bool is_recipient_name_Set() const;
    bool is_recipient_name_Valid() const;

    QString getRegion() const;
    void setRegion(const QString &region);
    bool is_region_Set() const;
    bool is_region_Valid() const;

    QList<QString> getStreetAddress() const;
    void setStreetAddress(const QList<QString> &street_address);
    bool is_street_address_Set() const;
    bool is_street_address_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString m_country;
    bool m_country_isSet;
    bool m_country_isValid;

    QList<QString> m_full_address;
    bool m_full_address_isSet;
    bool m_full_address_isValid;

    bool m_is_post_office_box;
    bool m_is_post_office_box_isSet;
    bool m_is_post_office_box_isValid;

    QString m_locality;
    bool m_locality_isSet;
    bool m_locality_isValid;

    QString m_postal_code;
    bool m_postal_code_isSet;
    bool m_postal_code_isValid;

    QString m_recipient_name;
    bool m_recipient_name_isSet;
    bool m_recipient_name_isValid;

    QString m_region;
    bool m_region_isSet;
    bool m_region_isValid;

    QList<QString> m_street_address;
    bool m_street_address_isSet;
    bool m_street_address_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIOrderAddress)

#endif // OAIOrderAddress_H
