...
async def test_content_orderreports_listdisbursements(client):
    ...
    response = await client.request(
        method='GET',
        path='/content/v2/{merchant_id}/orderreports/disbursements'.format(merchant_id='merchant_id_example'),
        headers=headers,
        params=params,
        )
...
async def test_content_orderreports_listtransactions(client):
    ...
    response = await client.request(
        method='GET',
        path='/content/v2/{merchant_id}/orderreports/disbursements/{disbursement_id}/transactions'.format(merchant_id='merchant_id_example', disbursement_id='disbursement_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
