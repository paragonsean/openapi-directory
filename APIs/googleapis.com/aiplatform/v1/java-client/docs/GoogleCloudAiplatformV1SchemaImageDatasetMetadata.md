

# GoogleCloudAiplatformV1SchemaImageDatasetMetadata

The metadata of Datasets that contain Image DataItems.

## Properties

| Name | Type | Description | Notes |
|------------ | ------------- | ------------- | -------------|
|**dataItemSchemaUri** | **String** | Points to a YAML file stored on Google Cloud Storage describing payload of the Image DataItems that belong to this Dataset. |  [optional] |
|**gcsBucket** | **String** | Google Cloud Storage Bucket name that contains the blob data of this Dataset. |  [optional] |



