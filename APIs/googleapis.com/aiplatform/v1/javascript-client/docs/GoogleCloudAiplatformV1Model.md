# VertexAiApi.GoogleCloudAiplatformV1Model
...
**deployedModels** | [**[GoogleCloudAiplatformV1DeployedModelRef]**](GoogleCloudAiplatformV1DeployedModelRef.md) | Output only. The pointers to DeployedModels created from this Model. Note that Model could have been deployed to Endpoints in different Locations. | [optional] [readonly] 
...
**labels** | **{String: String}** | The labels with user-defined metadata to organize your Models. Label keys and values can be no longer than 64 characters (Unicode codepoints), can only contain lowercase letters, numeric characters, underscores and dashes. International characters are allowed. See https://goo.gl/xmQnxf for more information and examples of labels. | [optional] 
...
**supportedExportFormats** | [**[GoogleCloudAiplatformV1ModelExportFormat]**](GoogleCloudAiplatformV1ModelExportFormat.md) | Output only. The formats in which this Model may be exported. If empty, this Model is not available for export. | [optional] [readonly] 
...
## Enum: [SupportedDeploymentResourcesTypesEnum]


* `DEPLOYMENT_RESOURCES_TYPE_UNSPECIFIED` (value: `"DEPLOYMENT_RESOURCES_TYPE_UNSPECIFIED"`)

* `DEDICATED_RESOURCES` (value: `"DEDICATED_RESOURCES"`)

* `AUTOMATIC_RESOURCES` (value: `"AUTOMATIC_RESOURCES"`)

* `SHARED_RESOURCES` (value: `"SHARED_RESOURCES"`)
