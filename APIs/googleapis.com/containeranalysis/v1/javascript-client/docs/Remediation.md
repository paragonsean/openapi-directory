# ContainerAnalysisApi.Remediation

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**details** | **String** | Contains a comprehensive human-readable discussion of the remediation. | [optional] 
**remediationType** | **String** | The type of remediation that can be applied. | [optional] 
**remediationUri** | [**RelatedUrl**](RelatedUrl.md) |  | [optional] 



## Enum: RemediationTypeEnum


* `REMEDIATION_TYPE_UNSPECIFIED` (value: `"REMEDIATION_TYPE_UNSPECIFIED"`)

* `MITIGATION` (value: `"MITIGATION"`)

* `NO_FIX_PLANNED` (value: `"NO_FIX_PLANNED"`)

* `NONE_AVAILABLE` (value: `"NONE_AVAILABLE"`)

* `VENDOR_FIX` (value: `"VENDOR_FIX"`)

* `WORKAROUND` (value: `"WORKAROUND"`)




