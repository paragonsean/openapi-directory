# ContainerAnalysisApi.BuildStep
...
**exitCode** | **Number** | Output only. Return code from running the step. | [optional] 
...
**status** | **String** | Output only. Status of the build step. At this time, build step status is only updated on build completion; step status is not updated in real-time as the build progresses. | [optional] 
