...
> SearchAllResourcesResponse cloudassetResourcesSearchAll(scope, opts)
...
let apiInstance = new CloudAssetApi.ResourcesApi();
...
let opts = {
  ...
  'assetTypes': ["null"], // [String] | Optional. A list of asset types that this request searches for. If empty, it will search all the supported asset types.
  'orderBy': "orderBy_example", // String | Optional. A comma separated list of fields specifying the sorting order of the results. The default order is ascending. Add ` DESC` after the field name to indicate descending order. Redundant space characters are ignored. For example, ` location DESC , name `.
  ...
  'query': "query_example" // String | Optional. The query statement.
};
apiInstance.cloudassetResourcesSearchAll(scope, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
 ...
 **assetTypes** | [**[String]**](String.md)| Optional. A list of asset types that this request searches for. If empty, it will search all the supported asset types. | [optional] 
