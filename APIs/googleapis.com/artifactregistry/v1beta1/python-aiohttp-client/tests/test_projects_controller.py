...
async def test_artifactregistry_projects_locations_repositories_create(client):
    ...
    """
    body = {"createTime":"createTime","format":"FORMAT_UNSPECIFIED","name":"name","description":"description","satisfiesPzs":True,"updateTime":"updateTime","kmsKeyName":"kmsKeyName","labels":{"key":"labels"},"sizeBytes":"sizeBytes"}
    params = [('$.xgafv', 'xgafv_example'),
                    ('access_token', 'access_token_example'),
                    ('alt', 'alt_example'),
                    ('callback', 'param_callback_example'),
    ...
    response = await client.request(
        method='POST',
        path='/v1beta1/{parent}/repositories'.format(parent='parent_example'),
        headers=headers,
        json=body,
        params=params,
        )
...
async def test_artifactregistry_projects_locations_repositories_packages_list(client):
    ...
    response = await client.request(
        method='GET',
        path='/v1beta1/{parent}/packages'.format(parent='parent_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
