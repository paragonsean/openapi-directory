...
async def test_content_orderreturns_get(client):
    ...
    response = await client.request(
        method='GET',
        path='/content/v2/{merchant_id}/orderreturns/{return_id}'.format(merchant_id='merchant_id_example', return_id='return_id_example'),
        headers=headers,
        params=params,
        )
...
async def test_content_orderreturns_list(client):
    ...
    response = await client.request(
        method='GET',
        path='/content/v2/{merchant_id}/orderreturns'.format(merchant_id='merchant_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
