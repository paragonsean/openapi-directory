...
async def test_content_accounttax_get(client):
    ...
    response = await client.request(
        method='GET',
        path='/content/v2/{merchant_id}/accounttax/{account_id}'.format(merchant_id='merchant_id_example', account_id='account_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_content_accounttax_list(client):
    ...
    response = await client.request(
        method='GET',
        path='/content/v2/{merchant_id}/accounttax'.format(merchant_id='merchant_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
