/**
 * On-Demand Scanning API
 * A service to scan container images for vulnerabilities.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import License from './License';
import Location from './Location';
import Version from './Version';

/**
 * The PackageOccurrence model module.
 * @module model/PackageOccurrence
 * @version v1
 */
class PackageOccurrence {
    /**
     * Constructs a new <code>PackageOccurrence</code>.
     * Details on how a particular software package was installed on a system.
     * @alias module:model/PackageOccurrence
     */
    constructor() { 
        
        PackageOccurrence.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>PackageOccurrence</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/PackageOccurrence} obj Optional instance to populate.
     * @return {module:model/PackageOccurrence} The populated <code>PackageOccurrence</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new PackageOccurrence();

            if (data.hasOwnProperty('architecture')) {
                obj['architecture'] = ApiClient.convertToType(data['architecture'], 'String');
            }
            if (data.hasOwnProperty('cpeUri')) {
                obj['cpeUri'] = ApiClient.convertToType(data['cpeUri'], 'String');
            }
            if (data.hasOwnProperty('license')) {
                obj['license'] = License.constructFromObject(data['license']);
            }
            if (data.hasOwnProperty('location')) {
                obj['location'] = ApiClient.convertToType(data['location'], [Location]);
            }
            if (data.hasOwnProperty('name')) {
                obj['name'] = ApiClient.convertToType(data['name'], 'String');
            }
            if (data.hasOwnProperty('packageType')) {
                obj['packageType'] = ApiClient.convertToType(data['packageType'], 'String');
            }
            if (data.hasOwnProperty('version')) {
                obj['version'] = Version.constructFromObject(data['version']);
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>PackageOccurrence</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>PackageOccurrence</code>.
     */
    static validateJSON(data) {
        // ensure the json data is a string
        if (data['architecture'] && !(typeof data['architecture'] === 'string' || data['architecture'] instanceof String)) {
            throw new Error("Expected the field `architecture` to be a primitive type in the JSON string but got " + data['architecture']);
        }
        // ensure the json data is a string
        if (data['cpeUri'] && !(typeof data['cpeUri'] === 'string' || data['cpeUri'] instanceof String)) {
            throw new Error("Expected the field `cpeUri` to be a primitive type in the JSON string but got " + data['cpeUri']);
        }
        // validate the optional field `license`
        if (data['license']) { // data not null
          License.validateJSON(data['license']);
        }
        if (data['location']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['location'])) {
                throw new Error("Expected the field `location` to be an array in the JSON data but got " + data['location']);
            }
            // validate the optional field `location` (array)
            for (const item of data['location']) {
                Location.validateJSON(item);
            };
        }
        // ensure the json data is a string
        if (data['name'] && !(typeof data['name'] === 'string' || data['name'] instanceof String)) {
            throw new Error("Expected the field `name` to be a primitive type in the JSON string but got " + data['name']);
        }
        // ensure the json data is a string
        if (data['packageType'] && !(typeof data['packageType'] === 'string' || data['packageType'] instanceof String)) {
            throw new Error("Expected the field `packageType` to be a primitive type in the JSON string but got " + data['packageType']);
        }
        // validate the optional field `version`
        if (data['version']) { // data not null
          Version.validateJSON(data['version']);
        }

        return true;
    }


}



/**
 * Output only. The CPU architecture for which packages in this distribution channel were built. Architecture will be blank for language packages.
 * @member {module:model/PackageOccurrence.ArchitectureEnum} architecture
 */
PackageOccurrence.prototype['architecture'] = undefined;

/**
 * Output only. The cpe_uri in [CPE format](https://cpe.mitre.org/specification/) denoting the package manager version distributing a package. The cpe_uri will be blank for language packages.
 * @member {String} cpeUri
 */
PackageOccurrence.prototype['cpeUri'] = undefined;

/**
 * @member {module:model/License} license
 */
PackageOccurrence.prototype['license'] = undefined;

/**
 * All of the places within the filesystem versions of this package have been found.
 * @member {Array.<module:model/Location>} location
 */
PackageOccurrence.prototype['location'] = undefined;

/**
 * Required. Output only. The name of the installed package.
 * @member {String} name
 */
PackageOccurrence.prototype['name'] = undefined;

/**
 * Output only. The type of package; whether native or non native (e.g., ruby gems, node.js packages, etc.).
 * @member {String} packageType
 */
PackageOccurrence.prototype['packageType'] = undefined;

/**
 * @member {module:model/Version} version
 */
PackageOccurrence.prototype['version'] = undefined;





/**
 * Allowed values for the <code>architecture</code> property.
 * @enum {String}
 * @readonly
 */
PackageOccurrence['ArchitectureEnum'] = {

    /**
     * value: "ARCHITECTURE_UNSPECIFIED"
     * @const
     */
    "ARCHITECTURE_UNSPECIFIED": "ARCHITECTURE_UNSPECIFIED",

    /**
     * value: "X86"
     * @const
     */
    "X86": "X86",

    /**
     * value: "X64"
     * @const
     */
    "X64": "X64"
};



export default PackageOccurrence;

