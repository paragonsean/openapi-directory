...
async def test_redis_projects_locations_instances_failover(client):
    ...
    response = await client.request(
        method='POST',
        path='/v1beta1/{namefailove}'.format(name='name_example'),
        headers=headers,
        json=body,
        params=params,
        )
...
async def test_redis_projects_locations_instances_get_auth_string(client):
    ...
    response = await client.request(
        method='GET',
        path='/v1beta1/{name}/authString'.format(name='name_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_redis_projects_locations_instances_reschedule_maintenance(client):
    ...
    response = await client.request(
        method='POST',
        path='/v1beta1/{namereschedule_maintenanc}'.format(name='name_example'),
        headers=headers,
        json=body,
        params=params,
        )
