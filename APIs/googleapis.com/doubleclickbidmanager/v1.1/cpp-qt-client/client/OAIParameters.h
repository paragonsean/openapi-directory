/**
 * DoubleClick Bid Manager API
 * DoubleClick Bid Manager API allows users to manage and create campaigns and reports.
 *
 * The version of the OpenAPI document: v1.1
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIParameters.h
 *
 * Parameters of a query or report.
 */

#ifndef OAIParameters_H
#define OAIParameters_H

#include <QJsonObject>

#include "OAIFilterPair.h"
#include "OAIOptions.h"
#include <QList>
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {
class OAIFilterPair;
class OAIOptions;

class OAIParameters : public OAIObject {
public:
    OAIParameters();
    OAIParameters(QString json);
    ~OAIParameters() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QList<OAIFilterPair> getFilters() const;
    void setFilters(const QList<OAIFilterPair> &filters);
    bool is_filters_Set() const;
    bool is_filters_Valid() const;

    QList<QString> getGroupBys() const;
    void setGroupBys(const QList<QString> &group_bys);
    bool is_group_bys_Set() const;
    bool is_group_bys_Valid() const;

    bool isIncludeInviteData() const;
    void setIncludeInviteData(const bool &include_invite_data);
    bool is_include_invite_data_Set() const;
    bool is_include_invite_data_Valid() const;

    QList<QString> getMetrics() const;
    void setMetrics(const QList<QString> &metrics);
    bool is_metrics_Set() const;
    bool is_metrics_Valid() const;

    OAIOptions getOptions() const;
    void setOptions(const OAIOptions &options);
    bool is_options_Set() const;
    bool is_options_Valid() const;

    QString getType() const;
    void setType(const QString &type);
    bool is_type_Set() const;
    bool is_type_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QList<OAIFilterPair> m_filters;
    bool m_filters_isSet;
    bool m_filters_isValid;

    QList<QString> m_group_bys;
    bool m_group_bys_isSet;
    bool m_group_bys_isValid;

    bool m_include_invite_data;
    bool m_include_invite_data_isSet;
    bool m_include_invite_data_isValid;

    QList<QString> m_metrics;
    bool m_metrics_isSet;
    bool m_metrics_isValid;

    OAIOptions m_options;
    bool m_options_isSet;
    bool m_options_isValid;

    QString m_type;
    bool m_type_isSet;
    bool m_type_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIParameters)

#endif // OAIParameters_H
