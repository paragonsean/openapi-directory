# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.source import Source
from openapi_server import util


class DerivedSource(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, derivation_mode: str=None, source: Source=None):
        """DerivedSource - a model defined in OpenAPI

        :param derivation_mode: The derivation_mode of this DerivedSource.
        :param source: The source of this DerivedSource.
        """
        self.openapi_types = {
            'derivation_mode': str,
            'source': Source
        }

        self.attribute_map = {
            'derivation_mode': 'derivationMode',
            'source': 'source'
        }

        self._derivation_mode = derivation_mode
        self._source = source

    @classmethod
    def from_dict(cls, dikt: dict) -> 'DerivedSource':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The DerivedSource of this DerivedSource.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def derivation_mode(self):
        """Gets the derivation_mode of this DerivedSource.

        What source to base the produced source on (if any).

        :return: The derivation_mode of this DerivedSource.
        :rtype: str
        """
        return self._derivation_mode

    @derivation_mode.setter
    def derivation_mode(self, derivation_mode):
        """Sets the derivation_mode of this DerivedSource.

        What source to base the produced source on (if any).

        :param derivation_mode: The derivation_mode of this DerivedSource.
        :type derivation_mode: str
        """
        allowed_values = ["SOURCE_DERIVATION_MODE_UNKNOWN", "SOURCE_DERIVATION_MODE_INDEPENDENT", "SOURCE_DERIVATION_MODE_CHILD_OF_CURRENT", "SOURCE_DERIVATION_MODE_SIBLING_OF_CURRENT"]  # noqa: E501
        if derivation_mode not in allowed_values:
            raise ValueError(
                "Invalid value for `derivation_mode` ({0}), must be one of {1}"
                .format(derivation_mode, allowed_values)
            )

        self._derivation_mode = derivation_mode

    @property
    def source(self):
        """Gets the source of this DerivedSource.


        :return: The source of this DerivedSource.
        :rtype: Source
        """
        return self._source

    @source.setter
    def source(self, source):
        """Sets the source of this DerivedSource.


        :param source: The source of this DerivedSource.
        :type source: Source
        """

        self._source = source
