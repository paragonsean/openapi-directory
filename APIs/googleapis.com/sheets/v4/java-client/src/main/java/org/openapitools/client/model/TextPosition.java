...
public class TextPosition {
  ...
  public TextPosition horizontalAlignment(HorizontalAlignmentEnum horizontalAlignment) {
  ...
  public boolean equals(Object o) {
    ...
    TextPosition textPosition = (TextPosition) o;
    return Objects.equals(this.horizontalAlignment, textPosition.horizontalAlignment);
  }

  @Override
  public int hashCode() {
    return Objects.hash(horizontalAlignment);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to TextPosition
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!TextPosition.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in TextPosition is not found in the empty JSON string", TextPosition.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!TextPosition.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TextPosition` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!TextPosition.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'TextPosition' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<TextPosition> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(TextPosition.class));

       return (TypeAdapter<T>) new TypeAdapter<TextPosition>() {
           @Override
           public void write(JsonWriter out, TextPosition value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public TextPosition read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of TextPosition given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of TextPosition
   * @throws IOException if the JSON string is invalid with respect to TextPosition
   */
  public static TextPosition fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, TextPosition.class);
  }

  /**
   * Convert an instance of TextPosition to an JSON string
   *
   * @return JSON string
   */
