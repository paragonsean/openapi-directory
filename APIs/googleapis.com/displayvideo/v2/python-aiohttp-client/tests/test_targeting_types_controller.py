...
async def test_displayvideo_targeting_types_targeting_options_get(client):
    ...
    response = await client.request(
        method='GET',
        path='/v2/targetingTypes/{targeting_type}/targetingOptions/{targeting_option_id}'.format(targeting_type='targeting_type_example', targeting_option_id='targeting_option_id_example'),
        headers=headers,
        params=params,
        )
...
async def test_displayvideo_targeting_types_targeting_options_list(client):
    ...
    response = await client.request(
        method='GET',
        path='/v2/targetingTypes/{targeting_type}/targetingOptions'.format(targeting_type='targeting_type_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_displayvideo_targeting_types_targeting_options_search(client):
    ...
    response = await client.request(
        method='POST',
        path='/v2/targetingTypes/{targeting_type}/targetingOptions:search'.format(targeting_type='targeting_type_example'),
        headers=headers,
        json=body,
        params=params,
        )
