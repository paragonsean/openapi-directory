...
async def test_displayvideo_first_and_third_party_audiences_edit_customer_match_members(client):
    ...
    response = await client.request(
        method='POST',
        path='/v3/firstAndThirdPartyAudiences/{first_and_third_party_audience_idedit_customer_match_member}'.format(first_and_third_party_audience_id='first_and_third_party_audience_id_example'),
        headers=headers,
        json=body,
        params=params,
        )
...
async def test_displayvideo_first_and_third_party_audiences_get(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/firstAndThirdPartyAudiences/{first_and_third_party_audience_id}'.format(first_and_third_party_audience_id='first_and_third_party_audience_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
