...
async def test_displayvideo_inventory_sources_edit_inventory_source_read_write_accessors(client):
    ...
    response = await client.request(
        method='POST',
        path='/v3/inventorySources/{inventory_source_idedit_inventory_source_read_write_accessor}'.format(inventory_source_id='inventory_source_id_example'),
        headers=headers,
        json=body,
        params=params,
        )
...
async def test_displayvideo_inventory_sources_get(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/inventorySources/{inventory_source_id}'.format(inventory_source_id='inventory_source_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
