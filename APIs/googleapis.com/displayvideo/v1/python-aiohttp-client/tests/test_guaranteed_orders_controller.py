...
async def test_displayvideo_guaranteed_orders_edit_guaranteed_order_read_accessors(client):
    ...
    response = await client.request(
        method='POST',
        path='/v1/guaranteedOrders/{guaranteed_order_idedit_guaranteed_order_read_accessor}'.format(guaranteed_order_id='guaranteed_order_id_example'),
        headers=headers,
        json=body,
        params=params,
        )
...
async def test_displayvideo_guaranteed_orders_get(client):
    ...
    response = await client.request(
        method='GET',
        path='/v1/guaranteedOrders/{guaranteed_order_id}'.format(guaranteed_order_id='guaranteed_order_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
