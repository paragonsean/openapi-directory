...
async def test_dns_dns_keys_get(client):
    ...
    response = await client.request(
        method='GET',
        path='/dns/v1beta2/projects/{project}/managedZones/{managed_zone}/dnsKeys/{dns_key_id}'.format(project='project_example', managed_zone='managed_zone_example', dns_key_id='dns_key_id_example'),
        headers=headers,
        params=params,
        )
...
async def test_dns_dns_keys_list(client):
    ...
    response = await client.request(
        method='GET',
        path='/dns/v1beta2/projects/{project}/managedZones/{managed_zone}/dnsKeys'.format(project='project_example', managed_zone='managed_zone_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
