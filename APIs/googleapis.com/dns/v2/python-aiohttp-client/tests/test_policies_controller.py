...
async def test_dns_policies_create(client):
    ...
    response = await client.request(
        method='POST',
        path='/dns/v2/projects/{project}/locations/{location}/policies'.format(project='project_example', location='location_example'),
        headers=headers,
        json=body,
        params=params,
        )
...
async def test_dns_policies_delete(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/dns/v2/projects/{project}/locations/{location}/policies/{policy}'.format(project='project_example', location='location_example', policy='policy_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
