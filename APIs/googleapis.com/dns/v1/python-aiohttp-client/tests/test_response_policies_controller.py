...
async def test_dns_response_policies_create(client):
    ...
    response = await client.request(
        method='POST',
        path='/dns/v1/projects/{project}/responsePolicies'.format(project='project_example'),
        headers=headers,
        json=body,
        params=params,
        )
...
async def test_dns_response_policies_delete(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/dns/v1/projects/{project}/responsePolicies/{response_policy}'.format(project='project_example', response_policy='response_policy_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
