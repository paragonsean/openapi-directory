...
async def test_dns_managed_zone_operations_get(client):
    ...
    response = await client.request(
        method='GET',
        path='/dns/v1/projects/{project}/managedZones/{managed_zone}/operations/{operation}'.format(project='project_example', managed_zone='managed_zone_example', operation='operation_example'),
        headers=headers,
        params=params,
        )
...
async def test_dns_managed_zone_operations_list(client):
    ...
    response = await client.request(
        method='GET',
        path='/dns/v1/projects/{project}/managedZones/{managed_zone}/operations'.format(project='project_example', managed_zone='managed_zone_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
