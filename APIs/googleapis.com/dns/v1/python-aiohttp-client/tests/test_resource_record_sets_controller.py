...
async def test_dns_resource_record_sets_create(client):
    ...
    response = await client.request(
        method='POST',
        path='/dns/v1/projects/{project}/managedZones/{managed_zone}/rrsets'.format(project='project_example', managed_zone='managed_zone_example'),
        headers=headers,
        json=body,
        params=params,
        )
...
async def test_dns_resource_record_sets_delete(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/dns/v1/projects/{project}/managedZones/{managed_zone}/rrsets/{name}/{type}'.format(project='project_example', managed_zone='managed_zone_example', name='name_example', type='type_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
