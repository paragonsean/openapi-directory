...
|------------ | ------------- | ------------- | -------------|
|**body** | **byte[]** | HTTP request body. A request body is allowed only if the HTTP method is POST, PUT, or PATCH. It is an error to set body on a task with an incompatible HttpMethod. |  [optional] |
...
|**httpMethod** | [**HttpMethodEnum**](#HttpMethodEnum) | The HTTP method to use for the request. The default is POST. |  [optional] |
