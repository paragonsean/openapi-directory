# GooglePlayGameServices.RevisionsApi
...
> RevisionCheckResponse gamesRevisionsCheck(clientRevision, opts)
...
apiInstance.gamesRevisionsCheck(clientRevision, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
