# BooksApi.PersonalizedstreamApi
...
> Discoveryclusters booksPersonalizedstreamGet(opts)
...
let apiInstance = new BooksApi.PersonalizedstreamApi();
let opts = {
...
apiInstance.booksPersonalizedstreamGet(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
