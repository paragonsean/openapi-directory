/**
 * Cloud Dataproc API
 * Manages Hadoop-based clusters and jobs on Google Cloud Platform.
 *
 * The version of the OpenAPI document: v1
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIPrestoJob.h
 *
 * A Dataproc job for running Presto (https://prestosql.io/) queries. IMPORTANT: The Dataproc Presto Optional Component (https://cloud.google.com/dataproc/docs/concepts/components/presto) must be enabled when the cluster is created to submit a Presto job to the cluster.
 */

#ifndef OAIPrestoJob_H
#define OAIPrestoJob_H

#include <QJsonObject>

#include "OAILoggingConfig.h"
#include "OAIQueryList.h"
#include <QList>
#include <QMap>
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {
class OAILoggingConfig;
class OAIQueryList;

class OAIPrestoJob : public OAIObject {
public:
    OAIPrestoJob();
    OAIPrestoJob(QString json);
    ~OAIPrestoJob() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QList<QString> getClientTags() const;
    void setClientTags(const QList<QString> &client_tags);
    bool is_client_tags_Set() const;
    bool is_client_tags_Valid() const;

    bool isContinueOnFailure() const;
    void setContinueOnFailure(const bool &continue_on_failure);
    bool is_continue_on_failure_Set() const;
    bool is_continue_on_failure_Valid() const;

    OAILoggingConfig getLoggingConfig() const;
    void setLoggingConfig(const OAILoggingConfig &logging_config);
    bool is_logging_config_Set() const;
    bool is_logging_config_Valid() const;

    QString getOutputFormat() const;
    void setOutputFormat(const QString &output_format);
    bool is_output_format_Set() const;
    bool is_output_format_Valid() const;

    QMap<QString, QString> getProperties() const;
    void setProperties(const QMap<QString, QString> &properties);
    bool is_properties_Set() const;
    bool is_properties_Valid() const;

    QString getQueryFileUri() const;
    void setQueryFileUri(const QString &query_file_uri);
    bool is_query_file_uri_Set() const;
    bool is_query_file_uri_Valid() const;

    OAIQueryList getQueryList() const;
    void setQueryList(const OAIQueryList &query_list);
    bool is_query_list_Set() const;
    bool is_query_list_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QList<QString> m_client_tags;
    bool m_client_tags_isSet;
    bool m_client_tags_isValid;

    bool m_continue_on_failure;
    bool m_continue_on_failure_isSet;
    bool m_continue_on_failure_isValid;

    OAILoggingConfig m_logging_config;
    bool m_logging_config_isSet;
    bool m_logging_config_isValid;

    QString m_output_format;
    bool m_output_format_isSet;
    bool m_output_format_isValid;

    QMap<QString, QString> m_properties;
    bool m_properties_isSet;
    bool m_properties_isValid;

    QString m_query_file_uri;
    bool m_query_file_uri_isSet;
    bool m_query_file_uri_isValid;

    OAIQueryList m_query_list;
    bool m_query_list_isSet;
    bool m_query_list_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIPrestoJob)

#endif // OAIPrestoJob_H
