...
public class PlaylistItemStatus {
   ...
   * @throws IOException if the JSON Element is invalid with respect to PlaylistItemStatus
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PlaylistItemStatus.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PlaylistItemStatus is not found in the empty JSON string", PlaylistItemStatus.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PlaylistItemStatus.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PlaylistItemStatus` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PlaylistItemStatus.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PlaylistItemStatus' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PlaylistItemStatus> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PlaylistItemStatus.class));

       return (TypeAdapter<T>) new TypeAdapter<PlaylistItemStatus>() {
           @Override
           public void write(JsonWriter out, PlaylistItemStatus value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PlaylistItemStatus read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of PlaylistItemStatus given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of PlaylistItemStatus
   * @throws IOException if the JSON string is invalid with respect to PlaylistItemStatus
   */
  public static PlaylistItemStatus fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PlaylistItemStatus.class);
  }

  /**
   * Convert an instance of PlaylistItemStatus to an JSON string
   *
   * @return JSON string
   */
