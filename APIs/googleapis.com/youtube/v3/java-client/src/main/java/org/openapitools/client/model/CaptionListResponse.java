...
public class CaptionListResponse {
  ...
  private List<Caption> items = new ArrayList<>();
   ...
   * @throws IOException if the JSON Element is invalid with respect to CaptionListResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CaptionListResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in CaptionListResponse is not found in the empty JSON string", CaptionListResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CaptionListResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CaptionListResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("items") != null && !jsonObj.get("items").isJsonNull()) {
        ...
        if (jsonArrayitems != null) {
          ...
          for (int i = 0; i < jsonArrayitems.size(); i++) {
            Caption.validateJsonElement(jsonArrayitems.get(i));
          };
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!CaptionListResponse.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'CaptionListResponse' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<CaptionListResponse> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(CaptionListResponse.class));

       return (TypeAdapter<T>) new TypeAdapter<CaptionListResponse>() {
           @Override
           public void write(JsonWriter out, CaptionListResponse value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public CaptionListResponse read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of CaptionListResponse given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of CaptionListResponse
   * @throws IOException if the JSON string is invalid with respect to CaptionListResponse
   */
  public static CaptionListResponse fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, CaptionListResponse.class);
  }

  /**
   * Convert an instance of CaptionListResponse to an JSON string
   *
   * @return JSON string
   */
