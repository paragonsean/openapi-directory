...
public class ActivityListResponse {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ActivityListResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ActivityListResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ActivityListResponse is not found in the empty JSON string", ActivityListResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ActivityListResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ActivityListResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ActivityListResponse.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ActivityListResponse' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ActivityListResponse> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ActivityListResponse.class));

       return (TypeAdapter<T>) new TypeAdapter<ActivityListResponse>() {
           @Override
           public void write(JsonWriter out, ActivityListResponse value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ActivityListResponse read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of ActivityListResponse given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of ActivityListResponse
   * @throws IOException if the JSON string is invalid with respect to ActivityListResponse
   */
  public static ActivityListResponse fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ActivityListResponse.class);
  }

  /**
   * Convert an instance of ActivityListResponse to an JSON string
   *
   * @return JSON string
   */
