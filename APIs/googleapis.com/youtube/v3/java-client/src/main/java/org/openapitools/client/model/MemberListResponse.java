...
public class MemberListResponse {
   ...
   * @throws IOException if the JSON Element is invalid with respect to MemberListResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!MemberListResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in MemberListResponse is not found in the empty JSON string", MemberListResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!MemberListResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `MemberListResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!MemberListResponse.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'MemberListResponse' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<MemberListResponse> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(MemberListResponse.class));

       return (TypeAdapter<T>) new TypeAdapter<MemberListResponse>() {
           @Override
           public void write(JsonWriter out, MemberListResponse value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public MemberListResponse read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of MemberListResponse given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of MemberListResponse
   * @throws IOException if the JSON string is invalid with respect to MemberListResponse
   */
  public static MemberListResponse fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, MemberListResponse.class);
  }

  /**
   * Convert an instance of MemberListResponse to an JSON string
   *
   * @return JSON string
   */
