...
public class ThumbnailSetResponse {
  ...
  private List<ThumbnailDetails> items = new ArrayList<>();
   ...
   * @throws IOException if the JSON Element is invalid with respect to ThumbnailSetResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ThumbnailSetResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ThumbnailSetResponse is not found in the empty JSON string", ThumbnailSetResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ThumbnailSetResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ThumbnailSetResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("items") != null && !jsonObj.get("items").isJsonNull()) {
        ...
        if (jsonArrayitems != null) {
          ...
          for (int i = 0; i < jsonArrayitems.size(); i++) {
            ThumbnailDetails.validateJsonElement(jsonArrayitems.get(i));
          };
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ThumbnailSetResponse.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ThumbnailSetResponse' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ThumbnailSetResponse> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ThumbnailSetResponse.class));

       return (TypeAdapter<T>) new TypeAdapter<ThumbnailSetResponse>() {
           @Override
           public void write(JsonWriter out, ThumbnailSetResponse value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ThumbnailSetResponse read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of ThumbnailSetResponse given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of ThumbnailSetResponse
   * @throws IOException if the JSON string is invalid with respect to ThumbnailSetResponse
   */
  public static ThumbnailSetResponse fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ThumbnailSetResponse.class);
  }

  /**
   * Convert an instance of ThumbnailSetResponse to an JSON string
   *
   * @return JSON string
   */
