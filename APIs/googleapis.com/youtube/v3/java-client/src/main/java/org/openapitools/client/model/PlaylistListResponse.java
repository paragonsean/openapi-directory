...
public class PlaylistListResponse {
   ...
   * @throws IOException if the JSON Element is invalid with respect to PlaylistListResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PlaylistListResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PlaylistListResponse is not found in the empty JSON string", PlaylistListResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PlaylistListResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PlaylistListResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PlaylistListResponse.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PlaylistListResponse' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PlaylistListResponse> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PlaylistListResponse.class));

       return (TypeAdapter<T>) new TypeAdapter<PlaylistListResponse>() {
           @Override
           public void write(JsonWriter out, PlaylistListResponse value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PlaylistListResponse read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of PlaylistListResponse given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of PlaylistListResponse
   * @throws IOException if the JSON string is invalid with respect to PlaylistListResponse
   */
  public static PlaylistListResponse fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PlaylistListResponse.class);
  }

  /**
   * Convert an instance of PlaylistListResponse to an JSON string
   *
   * @return JSON string
   */
