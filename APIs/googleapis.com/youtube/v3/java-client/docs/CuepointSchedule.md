

# CuepointSchedule

Schedule to insert cuepoints into a broadcast by ads automator.

## Properties

| Name | Type | Description | Notes |
|------------ | ------------- | ------------- | -------------|
|**enabled** | **Boolean** | This field is semantically required. If it is set false or not set, other fields in this message will be ignored. |  [optional] |
|**pauseAdsUntil** | **String** | If set, automatic cuepoint insertion is paused until this timestamp (\&quot;No Ad Zone\&quot;). The value is specified in ISO 8601 format. |  [optional] |
|**repeatIntervalSecs** | **Integer** | Interval frequency in seconds that api uses to insert cuepoints automatically. |  [optional] |
|**scheduleStrategy** | [**ScheduleStrategyEnum**](#ScheduleStrategyEnum) | The strategy to use when scheduling cuepoints. |  [optional] |



## Enum: ScheduleStrategyEnum

| Name | Value |
|---- | -----|
| SCHEDULE_STRATEGY_UNSPECIFIED | &quot;scheduleStrategyUnspecified&quot; |
| CONCURRENT | &quot;concurrent&quot; |
| NON_CONCURRENT | &quot;nonConcurrent&quot; |



