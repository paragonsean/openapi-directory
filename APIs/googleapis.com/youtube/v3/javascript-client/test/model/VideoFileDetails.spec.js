/**
 * YouTube Data API v3
 * The YouTube Data API v3 is an API that provides access to YouTube data, such as videos, playlists, and channels.
 *
 * The version of the OpenAPI document: v3
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', process.cwd()+'/src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require(process.cwd()+'/src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.YouTubeDataApiV3);
  }
}(this, function(expect, YouTubeDataApiV3) {
  'use strict';

  var instance;

  beforeEach(function() {
    instance = new YouTubeDataApiV3.VideoFileDetails();
  });

  var getProperty = function(object, getter, property) {
    // Use getter method if present; otherwise, get the property directly.
    if (typeof object[getter] === 'function')
      return object[getter]();
    else
      return object[property];
  }

  var setProperty = function(object, setter, property, value) {
    // Use setter method if present; otherwise, set the property directly.
    if (typeof object[setter] === 'function')
      object[setter](value);
    else
      object[property] = value;
  }

  describe('VideoFileDetails', function() {
    it('should create an instance of VideoFileDetails', function() {
      // uncomment below and update the code to test VideoFileDetails
      //var instance = new YouTubeDataApiV3.VideoFileDetails();
      //expect(instance).to.be.a(YouTubeDataApiV3.VideoFileDetails);
    });

    it('should have the property audioStreams (base name: "audioStreams")', function() {
      // uncomment below and update the code to test the property audioStreams
      //var instance = new YouTubeDataApiV3.VideoFileDetails();
      //expect(instance).to.be();
    });

    it('should have the property bitrateBps (base name: "bitrateBps")', function() {
      // uncomment below and update the code to test the property bitrateBps
      //var instance = new YouTubeDataApiV3.VideoFileDetails();
      //expect(instance).to.be();
    });

    it('should have the property container (base name: "container")', function() {
      // uncomment below and update the code to test the property container
      //var instance = new YouTubeDataApiV3.VideoFileDetails();
      //expect(instance).to.be();
    });

    it('should have the property creationTime (base name: "creationTime")', function() {
      // uncomment below and update the code to test the property creationTime
      //var instance = new YouTubeDataApiV3.VideoFileDetails();
      //expect(instance).to.be();
    });

    it('should have the property durationMs (base name: "durationMs")', function() {
      // uncomment below and update the code to test the property durationMs
      //var instance = new YouTubeDataApiV3.VideoFileDetails();
      //expect(instance).to.be();
    });

    it('should have the property fileName (base name: "fileName")', function() {
      // uncomment below and update the code to test the property fileName
      //var instance = new YouTubeDataApiV3.VideoFileDetails();
      //expect(instance).to.be();
    });

    it('should have the property fileSize (base name: "fileSize")', function() {
      // uncomment below and update the code to test the property fileSize
      //var instance = new YouTubeDataApiV3.VideoFileDetails();
      //expect(instance).to.be();
    });

    it('should have the property fileType (base name: "fileType")', function() {
      // uncomment below and update the code to test the property fileType
      //var instance = new YouTubeDataApiV3.VideoFileDetails();
      //expect(instance).to.be();
    });

    it('should have the property videoStreams (base name: "videoStreams")', function() {
      // uncomment below and update the code to test the property videoStreams
      //var instance = new YouTubeDataApiV3.VideoFileDetails();
      //expect(instance).to.be();
    });

  });

}));
