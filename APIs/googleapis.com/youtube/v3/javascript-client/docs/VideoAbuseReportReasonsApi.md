# YouTubeDataApiV3.VideoAbuseReportReasonsApi
...
> VideoAbuseReportReasonListResponse youtubeVideoAbuseReportReasonsList(part, opts)
...
let apiInstance = new YouTubeDataApiV3.VideoAbuseReportReasonsApi();
let part = ["null"]; // [String] | The *part* parameter specifies the videoCategory resource parts that the API response will include. Supported values are id and snippet.
let opts = {
...
apiInstance.youtubeVideoAbuseReportReasonsList(part, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
