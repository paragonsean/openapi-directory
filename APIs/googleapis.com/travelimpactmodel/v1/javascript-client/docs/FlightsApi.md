# TravelImpactModelApi.FlightsApi
...
> ComputeFlightEmissionsResponse travelimpactmodelFlightsComputeFlightEmissions(opts)
...
```javascript
import TravelImpactModelApi from 'travel_impact_model_api';

let apiInstance = new TravelImpactModelApi.FlightsApi();
let opts = {
...
apiInstance.travelimpactmodelFlightsComputeFlightEmissions(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
