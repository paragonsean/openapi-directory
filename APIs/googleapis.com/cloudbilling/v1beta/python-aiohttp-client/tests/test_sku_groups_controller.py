...
async def test_cloudbilling_sku_groups_list(client):
...
async def test_cloudbilling_sku_groups_skus_list(client):
    ...
    response = await client.request(
        method='GET',
        path='/v1beta/{parent}/skus'.format(parent='parent_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
