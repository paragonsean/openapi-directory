...
async def test_cloudbilling_skus_price_get(client):
...
async def test_cloudbilling_skus_prices_list(client):
    ...
    response = await client.request(
        method='GET',
        path='/v1beta/{parent}/prices'.format(parent='parent_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
