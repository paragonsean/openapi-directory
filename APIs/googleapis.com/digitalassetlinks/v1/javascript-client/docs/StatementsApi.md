# DigitalAssetLinksApi.StatementsApi
...
> ListResponse digitalassetlinksStatementsList(opts)
...
let apiInstance = new DigitalAssetLinksApi.StatementsApi();
let opts = {
...
apiInstance.digitalassetlinksStatementsList(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
