

# GoogleCloudDialogflowCxV3ListTestCasesResponse

The response message for TestCases.ListTestCases.

## Properties

| Name | Type | Description | Notes |
|------------ | ------------- | ------------- | -------------|
|**nextPageToken** | **String** | Token to retrieve the next page of results, or empty if there are no more results in the list. |  [optional] |
|**testCases** | [**List&lt;GoogleCloudDialogflowCxV3TestCase&gt;**](GoogleCloudDialogflowCxV3TestCase.md) | The list of test cases. There will be a maximum number of items returned based on the page_size field in the request. |  [optional] |



