...
async def test_sql_connect_generate_ephemeral(client):
    ...
    response = await client.request(
        method='POST',
        path='/v1/projects/{project}/instances/{instancegenerate_ephemeral_cer}'.format(project='project_example', instance='instance_example'),
        headers=headers,
        json=body,
        params=params,
        )
...
async def test_sql_connect_get(client):
    ...
    response = await client.request(
        method='GET',
        path='/v1/projects/{project}/instances/{instance}/connectSettings'.format(project='project_example', instance='instance_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
