/**
 * Dataflow Kit Web Scraper
 * Render Javascript driven pages, while we internally manage Headless Chrome and proxies for you.   - Build a custom web scraper with our Visual point-and-click toolkit. - Scrape the most popular Search engines result pages (SERP). - Convert web pages to PDF and capture screenshots. *** ### Authentication Dataflow Kit API require you to sign up for an API key in order to use the API.   The API key can be found in the [DFK Dashboard](https://account.dataflowkit.com) after _free registration_.  Pass a secret API Key to all API requests to the server as the `api_key` query parameter.  
 *
 * The version of the OpenAPI document: 1.3
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIUrl2pdfrequest.h
 *
 * 
 */

#ifndef OAIUrl2pdfrequest_H
#define OAIUrl2pdfrequest_H

#include <QJsonObject>

#include "OAIAction.h"
#include "OAIInitialCookie.h"
#include <QList>
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {
class OAIAction;
class OAIInitialCookie;

class OAIUrl2pdfrequest : public OAIObject {
public:
    OAIUrl2pdfrequest();
    OAIUrl2pdfrequest(QString json);
    ~OAIUrl2pdfrequest() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QList<OAIAction> getActions() const;
    void setActions(const QList<OAIAction> &actions);
    bool is_actions_Set() const;
    bool is_actions_Valid() const;

    bool isIgnoreHttpStatusErrCodes() const;
    void setIgnoreHttpStatusErrCodes(const bool &ignore_http_status_err_codes);
    bool is_ignore_http_status_err_codes_Set() const;
    bool is_ignore_http_status_err_codes_Valid() const;

    QList<OAIInitialCookie> getInitialCookies() const;
    void setInitialCookies(const QList<OAIInitialCookie> &initial_cookies);
    bool is_initial_cookies_Set() const;
    bool is_initial_cookies_Valid() const;

    bool isLandscape() const;
    void setLandscape(const bool &landscape);
    bool is_landscape_Set() const;
    bool is_landscape_Valid() const;

    double getMarginBottom() const;
    void setMarginBottom(const double &margin_bottom);
    bool is_margin_bottom_Set() const;
    bool is_margin_bottom_Valid() const;

    double getMarginLeft() const;
    void setMarginLeft(const double &margin_left);
    bool is_margin_left_Set() const;
    bool is_margin_left_Valid() const;

    double getMarginRight() const;
    void setMarginRight(const double &margin_right);
    bool is_margin_right_Set() const;
    bool is_margin_right_Valid() const;

    double getMarginTop() const;
    void setMarginTop(const double &margin_top);
    bool is_margin_top_Set() const;
    bool is_margin_top_Valid() const;

    QString getOutput() const;
    void setOutput(const QString &output);
    bool is_output_Set() const;
    bool is_output_Valid() const;

    QString getPageRanges() const;
    void setPageRanges(const QString &page_ranges);
    bool is_page_ranges_Set() const;
    bool is_page_ranges_Valid() const;

    QString getPaperSize() const;
    void setPaperSize(const QString &paper_size);
    bool is_paper_size_Set() const;
    bool is_paper_size_Valid() const;

    bool isPrintBackground() const;
    void setPrintBackground(const bool &print_background);
    bool is_print_background_Set() const;
    bool is_print_background_Valid() const;

    bool isPrintHeaderFooter() const;
    void setPrintHeaderFooter(const bool &print_header_footer);
    bool is_print_header_footer_Set() const;
    bool is_print_header_footer_Valid() const;

    QString getProxy() const;
    void setProxy(const QString &proxy);
    bool is_proxy_Set() const;
    bool is_proxy_Valid() const;

    double getScale() const;
    void setScale(const double &scale);
    bool is_scale_Set() const;
    bool is_scale_Valid() const;

    QString getUrl() const;
    void setUrl(const QString &url);
    bool is_url_Set() const;
    bool is_url_Valid() const;

    double getWaitDelay() const;
    void setWaitDelay(const double &wait_delay);
    bool is_wait_delay_Set() const;
    bool is_wait_delay_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QList<OAIAction> m_actions;
    bool m_actions_isSet;
    bool m_actions_isValid;

    bool m_ignore_http_status_err_codes;
    bool m_ignore_http_status_err_codes_isSet;
    bool m_ignore_http_status_err_codes_isValid;

    QList<OAIInitialCookie> m_initial_cookies;
    bool m_initial_cookies_isSet;
    bool m_initial_cookies_isValid;

    bool m_landscape;
    bool m_landscape_isSet;
    bool m_landscape_isValid;

    double m_margin_bottom;
    bool m_margin_bottom_isSet;
    bool m_margin_bottom_isValid;

    double m_margin_left;
    bool m_margin_left_isSet;
    bool m_margin_left_isValid;

    double m_margin_right;
    bool m_margin_right_isSet;
    bool m_margin_right_isValid;

    double m_margin_top;
    bool m_margin_top_isSet;
    bool m_margin_top_isValid;

    QString m_output;
    bool m_output_isSet;
    bool m_output_isValid;

    QString m_page_ranges;
    bool m_page_ranges_isSet;
    bool m_page_ranges_isValid;

    QString m_paper_size;
    bool m_paper_size_isSet;
    bool m_paper_size_isValid;

    bool m_print_background;
    bool m_print_background_isSet;
    bool m_print_background_isValid;

    bool m_print_header_footer;
    bool m_print_header_footer_isSet;
    bool m_print_header_footer_isValid;

    QString m_proxy;
    bool m_proxy_isSet;
    bool m_proxy_isValid;

    double m_scale;
    bool m_scale_isSet;
    bool m_scale_isValid;

    QString m_url;
    bool m_url_isSet;
    bool m_url_isValid;

    double m_wait_delay;
    bool m_wait_delay_isSet;
    bool m_wait_delay_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIUrl2pdfrequest)

#endif // OAIUrl2pdfrequest_H
