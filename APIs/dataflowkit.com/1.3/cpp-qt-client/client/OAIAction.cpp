/**
 * Dataflow Kit Web Scraper
 * Render Javascript driven pages, while we internally manage Headless Chrome and proxies for you.   - Build a custom web scraper with our Visual point-and-click toolkit. - Scrape the most popular Search engines result pages (SERP). - Convert web pages to PDF and capture screenshots. *** ### Authentication Dataflow Kit API require you to sign up for an API key in order to use the API.   The API key can be found in the [DFK Dashboard](https://account.dataflowkit.com) after _free registration_.  Pass a secret API Key to all API requests to the server as the `api_key` query parameter.  
 *
 * The version of the OpenAPI document: 1.3
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIAction.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIAction::OAIAction(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIAction::OAIAction() {
    this->initializeModel();
}

OAIAction::~OAIAction() {}

void OAIAction::initializeModel() {

    m_ignore_if_not_present_isSet = false;
    m_ignore_if_not_present_isValid = false;

    m_selector_isSet = false;
    m_selector_isValid = false;

    m_value_isSet = false;
    m_value_isValid = false;

    m_skip_last_iteration_isSet = false;
    m_skip_last_iteration_isValid = false;

    m_wait_delay_isSet = false;
    m_wait_delay_isValid = false;

    m_script_isSet = false;
    m_script_isValid = false;

    m_actions_isSet = false;
    m_actions_isValid = false;

    m_times_isSet = false;
    m_times_isValid = false;

    m_scroll_by_pixels_isSet = false;
    m_scroll_by_pixels_isValid = false;

    m_scrolling_element_selector_isSet = false;
    m_scrolling_element_selector_isValid = false;
}

void OAIAction::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAIAction::fromJsonObject(QJsonObject json) {

    m_ignore_if_not_present_isValid = ::OpenAPI::fromJsonValue(m_ignore_if_not_present, json[QString("ignoreIfNotPresent")]);
    m_ignore_if_not_present_isSet = !json[QString("ignoreIfNotPresent")].isNull() && m_ignore_if_not_present_isValid;

    m_selector_isValid = ::OpenAPI::fromJsonValue(m_selector, json[QString("selector")]);
    m_selector_isSet = !json[QString("selector")].isNull() && m_selector_isValid;

    m_value_isValid = ::OpenAPI::fromJsonValue(m_value, json[QString("value")]);
    m_value_isSet = !json[QString("value")].isNull() && m_value_isValid;

    m_skip_last_iteration_isValid = ::OpenAPI::fromJsonValue(m_skip_last_iteration, json[QString("skipLastIteration")]);
    m_skip_last_iteration_isSet = !json[QString("skipLastIteration")].isNull() && m_skip_last_iteration_isValid;

    m_wait_delay_isValid = ::OpenAPI::fromJsonValue(m_wait_delay, json[QString("waitDelay")]);
    m_wait_delay_isSet = !json[QString("waitDelay")].isNull() && m_wait_delay_isValid;

    m_script_isValid = ::OpenAPI::fromJsonValue(m_script, json[QString("script")]);
    m_script_isSet = !json[QString("script")].isNull() && m_script_isValid;

    m_actions_isValid = ::OpenAPI::fromJsonValue(m_actions, json[QString("actions")]);
    m_actions_isSet = !json[QString("actions")].isNull() && m_actions_isValid;

    m_times_isValid = ::OpenAPI::fromJsonValue(m_times, json[QString("times")]);
    m_times_isSet = !json[QString("times")].isNull() && m_times_isValid;

    m_scroll_by_pixels_isValid = ::OpenAPI::fromJsonValue(m_scroll_by_pixels, json[QString("scrollByPixels")]);
    m_scroll_by_pixels_isSet = !json[QString("scrollByPixels")].isNull() && m_scroll_by_pixels_isValid;

    m_scrolling_element_selector_isValid = ::OpenAPI::fromJsonValue(m_scrolling_element_selector, json[QString("scrollingElementSelector")]);
    m_scrolling_element_selector_isSet = !json[QString("scrollingElementSelector")].isNull() && m_scrolling_element_selector_isValid;
}

QString OAIAction::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAIAction::asJsonObject() const {
    QJsonObject obj;
    if (m_ignore_if_not_present_isSet) {
        obj.insert(QString("ignoreIfNotPresent"), ::OpenAPI::toJsonValue(m_ignore_if_not_present));
    }
    if (m_selector_isSet) {
        obj.insert(QString("selector"), ::OpenAPI::toJsonValue(m_selector));
    }
    if (m_value_isSet) {
        obj.insert(QString("value"), ::OpenAPI::toJsonValue(m_value));
    }
    if (m_skip_last_iteration_isSet) {
        obj.insert(QString("skipLastIteration"), ::OpenAPI::toJsonValue(m_skip_last_iteration));
    }
    if (m_wait_delay_isSet) {
        obj.insert(QString("waitDelay"), ::OpenAPI::toJsonValue(m_wait_delay));
    }
    if (m_script_isSet) {
        obj.insert(QString("script"), ::OpenAPI::toJsonValue(m_script));
    }
    if (m_actions.size() > 0) {
        obj.insert(QString("actions"), ::OpenAPI::toJsonValue(m_actions));
    }
    if (m_times_isSet) {
        obj.insert(QString("times"), ::OpenAPI::toJsonValue(m_times));
    }
    if (m_scroll_by_pixels_isSet) {
        obj.insert(QString("scrollByPixels"), ::OpenAPI::toJsonValue(m_scroll_by_pixels));
    }
    if (m_scrolling_element_selector_isSet) {
        obj.insert(QString("scrollingElementSelector"), ::OpenAPI::toJsonValue(m_scrolling_element_selector));
    }
    return obj;
}

bool OAIAction::isIgnoreIfNotPresent() const {
    return m_ignore_if_not_present;
}
void OAIAction::setIgnoreIfNotPresent(const bool &ignore_if_not_present) {
    m_ignore_if_not_present = ignore_if_not_present;
    m_ignore_if_not_present_isSet = true;
}

bool OAIAction::is_ignore_if_not_present_Set() const{
    return m_ignore_if_not_present_isSet;
}

bool OAIAction::is_ignore_if_not_present_Valid() const{
    return m_ignore_if_not_present_isValid;
}

QString OAIAction::getSelector() const {
    return m_selector;
}
void OAIAction::setSelector(const QString &selector) {
    m_selector = selector;
    m_selector_isSet = true;
}

bool OAIAction::is_selector_Set() const{
    return m_selector_isSet;
}

bool OAIAction::is_selector_Valid() const{
    return m_selector_isValid;
}

QString OAIAction::getValue() const {
    return m_value;
}
void OAIAction::setValue(const QString &value) {
    m_value = value;
    m_value_isSet = true;
}

bool OAIAction::is_value_Set() const{
    return m_value_isSet;
}

bool OAIAction::is_value_Valid() const{
    return m_value_isValid;
}

bool OAIAction::isSkipLastIteration() const {
    return m_skip_last_iteration;
}
void OAIAction::setSkipLastIteration(const bool &skip_last_iteration) {
    m_skip_last_iteration = skip_last_iteration;
    m_skip_last_iteration_isSet = true;
}

bool OAIAction::is_skip_last_iteration_Set() const{
    return m_skip_last_iteration_isSet;
}

bool OAIAction::is_skip_last_iteration_Valid() const{
    return m_skip_last_iteration_isValid;
}

QString OAIAction::getWaitDelay() const {
    return m_wait_delay;
}
void OAIAction::setWaitDelay(const QString &wait_delay) {
    m_wait_delay = wait_delay;
    m_wait_delay_isSet = true;
}

bool OAIAction::is_wait_delay_Set() const{
    return m_wait_delay_isSet;
}

bool OAIAction::is_wait_delay_Valid() const{
    return m_wait_delay_isValid;
}

QString OAIAction::getScript() const {
    return m_script;
}
void OAIAction::setScript(const QString &script) {
    m_script = script;
    m_script_isSet = true;
}

bool OAIAction::is_script_Set() const{
    return m_script_isSet;
}

bool OAIAction::is_script_Valid() const{
    return m_script_isValid;
}

QList<OAIAction> OAIAction::getActions() const {
    return m_actions;
}
void OAIAction::setActions(const QList<OAIAction> &actions) {
    m_actions = actions;
    m_actions_isSet = true;
}

bool OAIAction::is_actions_Set() const{
    return m_actions_isSet;
}

bool OAIAction::is_actions_Valid() const{
    return m_actions_isValid;
}

qint32 OAIAction::getTimes() const {
    return m_times;
}
void OAIAction::setTimes(const qint32 &times) {
    m_times = times;
    m_times_isSet = true;
}

bool OAIAction::is_times_Set() const{
    return m_times_isSet;
}

bool OAIAction::is_times_Valid() const{
    return m_times_isValid;
}

double OAIAction::getScrollByPixels() const {
    return m_scroll_by_pixels;
}
void OAIAction::setScrollByPixels(const double &scroll_by_pixels) {
    m_scroll_by_pixels = scroll_by_pixels;
    m_scroll_by_pixels_isSet = true;
}

bool OAIAction::is_scroll_by_pixels_Set() const{
    return m_scroll_by_pixels_isSet;
}

bool OAIAction::is_scroll_by_pixels_Valid() const{
    return m_scroll_by_pixels_isValid;
}

QString OAIAction::getScrollingElementSelector() const {
    return m_scrolling_element_selector;
}
void OAIAction::setScrollingElementSelector(const QString &scrolling_element_selector) {
    m_scrolling_element_selector = scrolling_element_selector;
    m_scrolling_element_selector_isSet = true;
}

bool OAIAction::is_scrolling_element_selector_Set() const{
    return m_scrolling_element_selector_isSet;
}

bool OAIAction::is_scrolling_element_selector_Valid() const{
    return m_scrolling_element_selector_isValid;
}

bool OAIAction::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_ignore_if_not_present_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_selector_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_value_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_skip_last_iteration_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_wait_delay_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_script_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_actions.size() > 0) {
            isObjectUpdated = true;
            break;
        }

        if (m_times_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_scroll_by_pixels_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_scrolling_element_selector_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAIAction::isValid() const {
    // only required properties are required for the object to be considered valid
    return true;
}

} // namespace OpenAPI
