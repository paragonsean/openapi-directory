...
> APISchoolListRank rankingsGetRank(st, appID, appKey, opts)
...
let apiInstance = new SchoolDiggerApiV1.RankingsApi();
...
apiInstance.rankingsGetRank(st, appID, appKey, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
> APIDistrictListRank rankingsGetRankDistrict(st, appID, appKey, opts)
