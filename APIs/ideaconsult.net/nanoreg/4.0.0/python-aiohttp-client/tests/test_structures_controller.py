...
async def test_search_by_identifier(client):
    ...
    response = await client.request(
        method='GET',
        path='/nanoreg1/enm/{db}/query/compound/{term}/{representation}'.format(db=nanoreg1, term='term_example', representation='representation_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_search_by_similarity(client):
    ...
    response = await client.request(
        method='GET',
        path='/nanoreg1/enm/{db}/query/similarity'.format(db=nanoreg1),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_search_by_smarts(client):
    ...
    response = await client.request(
        method='GET',
        path='/nanoreg1/enm/{db}/query/smarts'.format(db=nanoreg1),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
