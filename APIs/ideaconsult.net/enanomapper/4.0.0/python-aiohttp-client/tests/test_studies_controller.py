...
async def test_get_investigation_results(client):
    ...
    response = await client.request(
        method='GET',
        path='/enanomapper/enm/{db}/investigation'.format(db=nanoreg1),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_get_substance_study(client):
    ...
    response = await client.request(
        method='GET',
        path='/enanomapper/enm/{db}/substance/{uuid}/study'.format(db=nanoreg1, uuid='uuid_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_get_substance_study_summary(client):
    ...
    response = await client.request(
        method='GET',
        path='/enanomapper/enm/{db}/substance/{uuid}/studySummary'.format(db=nanoreg1, uuid='uuid_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
