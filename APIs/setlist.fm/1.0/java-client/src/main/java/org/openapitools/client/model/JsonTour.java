...
public class JsonTour {
  ...
  public JsonTour name(String name) {
    this.name = name;
    return this;
  }
  ...
  public boolean equals(Object o) {
    ...
    JsonTour jsonTour = (JsonTour) o;
    return Objects.equals(this.name, jsonTour.name);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to JsonTour
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!JsonTour.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in JsonTour is not found in the empty JSON string", JsonTour.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!JsonTour.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `JsonTour` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!JsonTour.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'JsonTour' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<JsonTour> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(JsonTour.class));

       return (TypeAdapter<T>) new TypeAdapter<JsonTour>() {
           @Override
           public void write(JsonWriter out, JsonTour value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public JsonTour read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of JsonTour given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of JsonTour
   * @throws IOException if the JSON string is invalid with respect to JsonTour
   */
  public static JsonTour fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, JsonTour.class);
  }

  /**
   * Convert an instance of JsonTour to an JSON string
   *
   * @return JSON string
   */
