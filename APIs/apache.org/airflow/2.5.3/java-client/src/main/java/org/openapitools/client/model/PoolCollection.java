...
public class PoolCollection {
   ...
   * @throws IOException if the JSON Element is invalid with respect to PoolCollection
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PoolCollection.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PoolCollection is not found in the empty JSON string", PoolCollection.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PoolCollection.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PoolCollection` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PoolCollection.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PoolCollection' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PoolCollection> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PoolCollection.class));

       return (TypeAdapter<T>) new TypeAdapter<PoolCollection>() {
           @Override
           public void write(JsonWriter out, PoolCollection value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PoolCollection read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of PoolCollection given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of PoolCollection
   * @throws IOException if the JSON string is invalid with respect to PoolCollection
   */
  public static PoolCollection fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PoolCollection.class);
  }

  /**
   * Convert an instance of PoolCollection to an JSON string
   *
   * @return JSON string
   */
