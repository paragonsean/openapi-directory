...
public class ConnectionCollection {
  ...
  private List<ConnectionCollectionItem> connections = new ArrayList<>();
   ...
   * @throws IOException if the JSON Element is invalid with respect to ConnectionCollection
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ConnectionCollection.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ConnectionCollection is not found in the empty JSON string", ConnectionCollection.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ConnectionCollection.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ConnectionCollection` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("connections") != null && !jsonObj.get("connections").isJsonNull()) {
        ...
        if (jsonArrayconnections != null) {
          ...
          for (int i = 0; i < jsonArrayconnections.size(); i++) {
            ConnectionCollectionItem.validateJsonElement(jsonArrayconnections.get(i));
          };
        }
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ConnectionCollection.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ConnectionCollection' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ConnectionCollection> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ConnectionCollection.class));

       return (TypeAdapter<T>) new TypeAdapter<ConnectionCollection>() {
           @Override
  ...
  /**
   * Create an instance of ConnectionCollection given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of ConnectionCollection
   * @throws IOException if the JSON string is invalid with respect to ConnectionCollection
   */
  public static ConnectionCollection fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ConnectionCollection.class);
  }

  /**
   * Convert an instance of ConnectionCollection to an JSON string
   *
   * @return JSON string
   */
