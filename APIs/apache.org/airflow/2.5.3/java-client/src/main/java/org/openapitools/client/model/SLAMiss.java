...
public class SLAMiss {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(dagId, description, emailSent, executionDate, notificationSent, taskId, timestamp);
  }
  ...
  static {
    ...
    openapiFields.add("notification_sent");
   ...
   * @throws IOException if the JSON Element is invalid with respect to SLAMiss
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SLAMiss.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in SLAMiss is not found in the empty JSON string", SLAMiss.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SLAMiss.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SLAMiss` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!SLAMiss.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'SLAMiss' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<SLAMiss> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(SLAMiss.class));

       return (TypeAdapter<T>) new TypeAdapter<SLAMiss>() {
           @Override
           public void write(JsonWriter out, SLAMiss value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public SLAMiss read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of SLAMiss given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of SLAMiss
   * @throws IOException if the JSON string is invalid with respect to SLAMiss
   */
  public static SLAMiss fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, SLAMiss.class);
  }

  /**
   * Convert an instance of SLAMiss to an JSON string
   *
   * @return JSON string
   */
