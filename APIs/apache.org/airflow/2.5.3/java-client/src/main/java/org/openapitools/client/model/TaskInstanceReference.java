...
public class TaskInstanceReference {
   ...
   * @throws IOException if the JSON Element is invalid with respect to TaskInstanceReference
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!TaskInstanceReference.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in TaskInstanceReference is not found in the empty JSON string", TaskInstanceReference.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!TaskInstanceReference.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TaskInstanceReference` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!TaskInstanceReference.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'TaskInstanceReference' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<TaskInstanceReference> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(TaskInstanceReference.class));

       return (TypeAdapter<T>) new TypeAdapter<TaskInstanceReference>() {
           @Override
           public void write(JsonWriter out, TaskInstanceReference value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public TaskInstanceReference read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of TaskInstanceReference given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of TaskInstanceReference
   * @throws IOException if the JSON string is invalid with respect to TaskInstanceReference
   */
  public static TaskInstanceReference fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, TaskInstanceReference.class);
  }

  /**
   * Convert an instance of TaskInstanceReference to an JSON string
   *
   * @return JSON string
   */
