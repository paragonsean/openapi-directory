...
public class UpdateTaskInstancesState {
  ...
  public UpdateTaskInstancesState() {
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to UpdateTaskInstancesState
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UpdateTaskInstancesState.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in UpdateTaskInstancesState is not found in the empty JSON string", UpdateTaskInstancesState.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!UpdateTaskInstancesState.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UpdateTaskInstancesState` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!UpdateTaskInstancesState.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'UpdateTaskInstancesState' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<UpdateTaskInstancesState> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(UpdateTaskInstancesState.class));

       return (TypeAdapter<T>) new TypeAdapter<UpdateTaskInstancesState>() {
           @Override
           public void write(JsonWriter out, UpdateTaskInstancesState value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public UpdateTaskInstancesState read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of UpdateTaskInstancesState given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of UpdateTaskInstancesState
   * @throws IOException if the JSON string is invalid with respect to UpdateTaskInstancesState
   */
  public static UpdateTaskInstancesState fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, UpdateTaskInstancesState.class);
  }

  /**
   * Convert an instance of UpdateTaskInstancesState to an JSON string
   *
   * @return JSON string
   */
