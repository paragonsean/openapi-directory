...
public class XCom {
   ...
   * @throws IOException if the JSON Element is invalid with respect to XCom
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!XCom.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in XCom is not found in the empty JSON string", XCom.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!XCom.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `XCom` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!XCom.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'XCom' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<XCom> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(XCom.class));

       return (TypeAdapter<T>) new TypeAdapter<XCom>() {
           @Override
           public void write(JsonWriter out, XCom value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public XCom read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of XCom given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of XCom
   * @throws IOException if the JSON string is invalid with respect to XCom
   */
  public static XCom fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, XCom.class);
  }

  /**
   * Convert an instance of XCom to an JSON string
   *
   * @return JSON string
   */
