...
public class BasicDAGRun {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(dagId, dataIntervalEnd, dataIntervalStart, endDate, logicalDate, runId, startDate, state);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to BasicDAGRun
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!BasicDAGRun.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in BasicDAGRun is not found in the empty JSON string", BasicDAGRun.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!BasicDAGRun.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BasicDAGRun` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!BasicDAGRun.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'BasicDAGRun' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<BasicDAGRun> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(BasicDAGRun.class));

       return (TypeAdapter<T>) new TypeAdapter<BasicDAGRun>() {
           @Override
           public void write(JsonWriter out, BasicDAGRun value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public BasicDAGRun read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of BasicDAGRun given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of BasicDAGRun
   * @throws IOException if the JSON string is invalid with respect to BasicDAGRun
   */
  public static BasicDAGRun fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, BasicDAGRun.class);
  }

  /**
   * Convert an instance of BasicDAGRun to an JSON string
   *
   * @return JSON string
   */
