...
public class VariableCollection {
  ...
  private List<VariableCollectionItem> variables = new ArrayList<>();
   ...
   * @throws IOException if the JSON Element is invalid with respect to VariableCollection
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!VariableCollection.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in VariableCollection is not found in the empty JSON string", VariableCollection.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!VariableCollection.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `VariableCollection` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("variables") != null && !jsonObj.get("variables").isJsonNull()) {
        ...
        if (jsonArrayvariables != null) {
          ...
          for (int i = 0; i < jsonArrayvariables.size(); i++) {
            VariableCollectionItem.validateJsonElement(jsonArrayvariables.get(i));
          };
        }
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!VariableCollection.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'VariableCollection' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<VariableCollection> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(VariableCollection.class));

       return (TypeAdapter<T>) new TypeAdapter<VariableCollection>() {
           @Override
           public void write(JsonWriter out, VariableCollection value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public VariableCollection read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of VariableCollection given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of VariableCollection
   * @throws IOException if the JSON string is invalid with respect to VariableCollection
   */
  public static VariableCollection fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, VariableCollection.class);
  }

  /**
   * Convert an instance of VariableCollection to an JSON string
   *
   * @return JSON string
   */
