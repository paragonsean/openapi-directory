...
public class DatasetCollection {
  ...
  public DatasetCollection datasets(List<Dataset> datasets) {
   ...
   * @throws IOException if the JSON Element is invalid with respect to DatasetCollection
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DatasetCollection.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in DatasetCollection is not found in the empty JSON string", DatasetCollection.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DatasetCollection.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DatasetCollection` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!DatasetCollection.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'DatasetCollection' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<DatasetCollection> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(DatasetCollection.class));

       return (TypeAdapter<T>) new TypeAdapter<DatasetCollection>() {
           @Override
           public void write(JsonWriter out, DatasetCollection value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public DatasetCollection read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of DatasetCollection given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of DatasetCollection
   * @throws IOException if the JSON string is invalid with respect to DatasetCollection
   */
  public static DatasetCollection fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, DatasetCollection.class);
  }

  /**
   * Convert an instance of DatasetCollection to an JSON string
   *
   * @return JSON string
   */
