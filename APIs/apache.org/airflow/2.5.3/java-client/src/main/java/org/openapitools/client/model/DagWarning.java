...
public class DagWarning {
   ...
   * @throws IOException if the JSON Element is invalid with respect to DagWarning
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DagWarning.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in DagWarning is not found in the empty JSON string", DagWarning.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DagWarning.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DagWarning` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("warning_type") != null && !jsonObj.get("warning_type").isJsonNull()) && !jsonObj.get("warning_type").isJsonPrimitive()) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!DagWarning.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'DagWarning' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<DagWarning> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(DagWarning.class));

       return (TypeAdapter<T>) new TypeAdapter<DagWarning>() {
           @Override
           public void write(JsonWriter out, DagWarning value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public DagWarning read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of DagWarning given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of DagWarning
   * @throws IOException if the JSON string is invalid with respect to DagWarning
   */
  public static DagWarning fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, DagWarning.class);
  }

  /**
   * Convert an instance of DagWarning to an JSON string
   *
   * @return JSON string
   */
