...
void OAIEventsApi::initializeServerConfigs() {
    ...
    _serverConfigs.insert("trackJourneyEvent", defaultConf);
    _serverIndices.insert("trackJourneyEvent", 0);
}
...
void OAIEventsApi::trackJourneyEvent(const OAITrackJourneyEvent_request &oai_track_journey_event_request) {
    QString fullPath = QString(_serverConfigs["trackJourneyEvent"][_serverIndices.value("trackJourneyEvent")].URL()+"/events");
...

    {
        ...
        QByteArray output = oai_track_journey_event_request.asJson().toUtf8();
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIEventsApi::trackJourneyEventCallback);
    connect(this, &OAIEventsApi::abortRequestsSignal, worker, &QObject::deleteLater);
    connect(worker, &QObject::destroyed, this, [this]() {
        if (findChildren<OAIHttpRequestWorker*>().count() == 0) {
            Q_EMIT allPendingRequestsCompleted();
...
void OAIEventsApi::trackJourneyEventCallback(OAIHttpRequestWorker *worker) {
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
    ...
    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT trackJourneyEventSignal(output);
        Q_EMIT trackJourneyEventSignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT trackJourneyEventSignalE(output, error_type, error_str);
        Q_EMIT trackJourneyEventSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT trackJourneyEventSignalError(output, error_type, error_str);
        Q_EMIT trackJourneyEventSignalErrorFull(worker, error_type, error_str);
    }
}
