...
void OAIValidationApi::initializeServerConfigs() {
    ...
    _serverConfigs.insert("getValidity", defaultConf);
    _serverIndices.insert("getValidity", 0);
}
...
void OAIValidationApi::getValidity() {
    QString fullPath = QString(_serverConfigs["getValidity"][_serverIndices.value("getValidity")].URL()+"/validate");
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIValidationApi::getValidityCallback);
...
void OAIValidationApi::getValidityCallback(OAIHttpRequestWorker *worker) {
    ...
    OAIGetValidity_200_response output(QString(worker->response));
    worker->deleteLater();

    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT getValiditySignal(output);
        Q_EMIT getValiditySignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT getValiditySignalE(output, error_type, error_str);
        Q_EMIT getValiditySignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT getValiditySignalError(output, error_type, error_str);
        Q_EMIT getValiditySignalErrorFull(worker, error_type, error_str);
    }
}
