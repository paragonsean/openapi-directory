...
void OAIUsersApi::initializeServerConfigs() {
    ...
    _serverConfigs.insert("link", defaultConf);
    _serverIndices.insert("link", 0);
    _serverConfigs.insert("upsertUser", defaultConf);
    _serverIndices.insert("upsertUser", 0);
}
...
void OAIUsersApi::deleteUser(const OAIDeleteUser_request &oai_delete_user_request) {
...
void OAIUsersApi::link(const OAILink_request &oai_link_request) {
    QString fullPath = QString(_serverConfigs["link"][_serverIndices.value("link")].URL()+"/link");
...

    {
        ...
        QByteArray output = oai_link_request.asJson().toUtf8();
        input.request_body.append(output);
    }
    for (auto keyValueIt = _defaultHeaders.keyValueBegin(); keyValueIt != _defaultHeaders.keyValueEnd(); keyValueIt++) {
        input.headers.insert(keyValueIt->first, keyValueIt->second);
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIUsersApi::linkCallback);
    connect(this, &OAIUsersApi::abortRequestsSignal, worker, &QObject::deleteLater);
    connect(worker, &QObject::destroyed, this, [this]() {
        if (findChildren<OAIHttpRequestWorker*>().count() == 0) {
            Q_EMIT allPendingRequestsCompleted();
...
void OAIUsersApi::linkCallback(OAIHttpRequestWorker *worker) {
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
    ...
    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT linkSignal(output);
        Q_EMIT linkSignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT linkSignalE(output, error_type, error_str);
        Q_EMIT linkSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT linkSignalError(output, error_type, error_str);
        Q_EMIT linkSignalErrorFull(worker, error_type, error_str);
    }
}

void OAIUsersApi::upsertUser(const OAIUpsertUser_request &oai_upsert_user_request) {
    QString fullPath = QString(_serverConfigs["upsertUser"][_serverIndices.value("upsertUser")].URL()+"/users/upsert");
...

    {
        ...
        QByteArray output = oai_upsert_user_request.asJson().toUtf8();
        input.request_body.append(output);
    }
    for (auto keyValueIt = _defaultHeaders.keyValueBegin(); keyValueIt != _defaultHeaders.keyValueEnd(); keyValueIt++) {
        input.headers.insert(keyValueIt->first, keyValueIt->second);
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIUsersApi::upsertUserCallback);
    connect(this, &OAIUsersApi::abortRequestsSignal, worker, &QObject::deleteLater);
    connect(worker, &QObject::destroyed, this, [this]() {
        if (findChildren<OAIHttpRequestWorker*>().count() == 0) {
            Q_EMIT allPendingRequestsCompleted();
...
void OAIUsersApi::upsertUserCallback(OAIHttpRequestWorker *worker) {
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
    ...
    OAIUpsertUser_201_response output(QString(worker->response));
    worker->deleteLater();

    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT upsertUserSignal(output);
        Q_EMIT upsertUserSignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT upsertUserSignalE(output, error_type, error_str);
        Q_EMIT upsertUserSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT upsertUserSignalError(output, error_type, error_str);
        Q_EMIT upsertUserSignalErrorFull(worker, error_type, error_str);
    }
}
