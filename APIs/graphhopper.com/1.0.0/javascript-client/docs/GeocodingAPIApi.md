# GraphHopperDirectionsApi.GeocodingAPIApi
...
> GeocodingResponse getGeocode(opts)
...
let apiInstance = new GraphHopperDirectionsApi.GeocodingAPIApi();
let opts = {
  ...
  'limit': 10, // Number | Specify the maximum number of results to return
  'reverse': false, // Boolean | It is `required` to be `true` if you want to do a reverse geocoding request. If it is `true`, `point` must be defined as well, and `q` must not be used.
  'debug': false, // Boolean | If `true`, the output will be formatted.
...
apiInstance.getGeocode(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
 ...
 **limit** | **Number**| Specify the maximum number of results to return | [optional] [default to 10]
