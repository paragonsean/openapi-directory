...
async def test_play_by_play(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/nba/pbp/{format}/PlayByPlay/{gameid}'.format(format=XML, gameid='gameid_example'),
        headers=headers,
        )
...
async def test_play_by_play_delta(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/nba/pbp/{format}/PlayByPlayDelta/{_date}/{minutes}'.format(format=XML, _date='_date_example', minutes='minutes_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
