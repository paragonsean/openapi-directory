...
async def test_premium_news(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/nba/news-rotoballer/{format}/RotoBallerPremiumNews'.format(format=xml),
        headers=headers,
        )
...
async def test_premium_news_by_date(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/nba/news-rotoballer/{format}/RotoBallerPremiumNewsByDate/{_date}'.format(format=xml, _date='_date_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_premium_news_by_player(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/nba/news-rotoballer/{format}/RotoBallerPremiumNewsByPlayerID/{playerid}'.format(format=xml, playerid='playerid_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
