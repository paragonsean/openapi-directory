# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server import util


class Standing(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, conference: str=None, conference_losses: int=None, conference_rank: int=None, conference_ties: int=None, conference_wins: int=None, division: str=None, division_losses: int=None, division_rank: int=None, division_ties: int=None, division_wins: int=None, global_team_id: int=None, losses: int=None, name: str=None, net_points: int=None, percentage: float=None, points_against: int=None, points_for: int=None, season: int=None, season_type: int=None, team: str=None, team_id: int=None, ties: int=None, touchdowns: int=None, wins: int=None):
        """Standing - a model defined in OpenAPI

        :param conference: The conference of this Standing.
        :param conference_losses: The conference_losses of this Standing.
        :param conference_rank: The conference_rank of this Standing.
        :param conference_ties: The conference_ties of this Standing.
        :param conference_wins: The conference_wins of this Standing.
        :param division: The division of this Standing.
        :param division_losses: The division_losses of this Standing.
        :param division_rank: The division_rank of this Standing.
        :param division_ties: The division_ties of this Standing.
        :param division_wins: The division_wins of this Standing.
        :param global_team_id: The global_team_id of this Standing.
        :param losses: The losses of this Standing.
        :param name: The name of this Standing.
        :param net_points: The net_points of this Standing.
        :param percentage: The percentage of this Standing.
        :param points_against: The points_against of this Standing.
        :param points_for: The points_for of this Standing.
        :param season: The season of this Standing.
        :param season_type: The season_type of this Standing.
        :param team: The team of this Standing.
        :param team_id: The team_id of this Standing.
        :param ties: The ties of this Standing.
        :param touchdowns: The touchdowns of this Standing.
        :param wins: The wins of this Standing.
        """
        self.openapi_types = {
            'conference': str,
            'conference_losses': int,
            'conference_rank': int,
            'conference_ties': int,
            'conference_wins': int,
            'division': str,
            'division_losses': int,
            'division_rank': int,
            'division_ties': int,
            'division_wins': int,
            'global_team_id': int,
            'losses': int,
            'name': str,
            'net_points': int,
            'percentage': float,
            'points_against': int,
            'points_for': int,
            'season': int,
            'season_type': int,
            'team': str,
            'team_id': int,
            'ties': int,
            'touchdowns': int,
            'wins': int
        }

        self.attribute_map = {
            'conference': 'Conference',
            'conference_losses': 'ConferenceLosses',
            'conference_rank': 'ConferenceRank',
            'conference_ties': 'ConferenceTies',
            'conference_wins': 'ConferenceWins',
            'division': 'Division',
            'division_losses': 'DivisionLosses',
            'division_rank': 'DivisionRank',
            'division_ties': 'DivisionTies',
            'division_wins': 'DivisionWins',
            'global_team_id': 'GlobalTeamID',
            'losses': 'Losses',
            'name': 'Name',
            'net_points': 'NetPoints',
            'percentage': 'Percentage',
            'points_against': 'PointsAgainst',
            'points_for': 'PointsFor',
            'season': 'Season',
            'season_type': 'SeasonType',
            'team': 'Team',
            'team_id': 'TeamID',
            'ties': 'Ties',
            'touchdowns': 'Touchdowns',
            'wins': 'Wins'
        }

        self._conference = conference
        self._conference_losses = conference_losses
        self._conference_rank = conference_rank
        self._conference_ties = conference_ties
        self._conference_wins = conference_wins
        self._division = division
        self._division_losses = division_losses
        self._division_rank = division_rank
        self._division_ties = division_ties
        self._division_wins = division_wins
        self._global_team_id = global_team_id
        self._losses = losses
        self._name = name
        self._net_points = net_points
        self._percentage = percentage
        self._points_against = points_against
        self._points_for = points_for
        self._season = season
        self._season_type = season_type
        self._team = team
        self._team_id = team_id
        self._ties = ties
        self._touchdowns = touchdowns
        self._wins = wins

    @classmethod
    def from_dict(cls, dikt: dict) -> 'Standing':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The Standing of this Standing.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def conference(self):
        """Gets the conference of this Standing.


        :return: The conference of this Standing.
        :rtype: str
        """
        return self._conference

    @conference.setter
    def conference(self, conference):
        """Sets the conference of this Standing.


        :param conference: The conference of this Standing.
        :type conference: str
        """

        self._conference = conference

    @property
    def conference_losses(self):
        """Gets the conference_losses of this Standing.


        :return: The conference_losses of this Standing.
        :rtype: int
        """
        return self._conference_losses

    @conference_losses.setter
    def conference_losses(self, conference_losses):
        """Sets the conference_losses of this Standing.


        :param conference_losses: The conference_losses of this Standing.
        :type conference_losses: int
        """

        self._conference_losses = conference_losses

    @property
    def conference_rank(self):
        """Gets the conference_rank of this Standing.


        :return: The conference_rank of this Standing.
        :rtype: int
        """
        return self._conference_rank

    @conference_rank.setter
    def conference_rank(self, conference_rank):
        """Sets the conference_rank of this Standing.


        :param conference_rank: The conference_rank of this Standing.
        :type conference_rank: int
        """

        self._conference_rank = conference_rank

    @property
    def conference_ties(self):
        """Gets the conference_ties of this Standing.


        :return: The conference_ties of this Standing.
        :rtype: int
        """
        return self._conference_ties

    @conference_ties.setter
    def conference_ties(self, conference_ties):
        """Sets the conference_ties of this Standing.


        :param conference_ties: The conference_ties of this Standing.
        :type conference_ties: int
        """

        self._conference_ties = conference_ties

    @property
    def conference_wins(self):
        """Gets the conference_wins of this Standing.


        :return: The conference_wins of this Standing.
        :rtype: int
        """
        return self._conference_wins

    @conference_wins.setter
    def conference_wins(self, conference_wins):
        """Sets the conference_wins of this Standing.


        :param conference_wins: The conference_wins of this Standing.
        :type conference_wins: int
        """

        self._conference_wins = conference_wins

    @property
    def division(self):
        """Gets the division of this Standing.


        :return: The division of this Standing.
        :rtype: str
        """
        return self._division

    @division.setter
    def division(self, division):
        """Sets the division of this Standing.


        :param division: The division of this Standing.
        :type division: str
        """

        self._division = division

    @property
    def division_losses(self):
        """Gets the division_losses of this Standing.


        :return: The division_losses of this Standing.
        :rtype: int
        """
        return self._division_losses

    @division_losses.setter
    def division_losses(self, division_losses):
        """Sets the division_losses of this Standing.


        :param division_losses: The division_losses of this Standing.
        :type division_losses: int
        """

        self._division_losses = division_losses

    @property
    def division_rank(self):
        """Gets the division_rank of this Standing.


        :return: The division_rank of this Standing.
        :rtype: int
        """
        return self._division_rank

    @division_rank.setter
    def division_rank(self, division_rank):
        """Sets the division_rank of this Standing.


        :param division_rank: The division_rank of this Standing.
        :type division_rank: int
        """

        self._division_rank = division_rank

    @property
    def division_ties(self):
        """Gets the division_ties of this Standing.


        :return: The division_ties of this Standing.
        :rtype: int
        """
        return self._division_ties

    @division_ties.setter
    def division_ties(self, division_ties):
        """Sets the division_ties of this Standing.


        :param division_ties: The division_ties of this Standing.
        :type division_ties: int
        """

        self._division_ties = division_ties

    @property
    def division_wins(self):
        """Gets the division_wins of this Standing.


        :return: The division_wins of this Standing.
        :rtype: int
        """
        return self._division_wins

    @division_wins.setter
    def division_wins(self, division_wins):
        """Sets the division_wins of this Standing.


        :param division_wins: The division_wins of this Standing.
        :type division_wins: int
        """

        self._division_wins = division_wins

    @property
    def global_team_id(self):
        """Gets the global_team_id of this Standing.


        :return: The global_team_id of this Standing.
        :rtype: int
        """
        return self._global_team_id

    @global_team_id.setter
    def global_team_id(self, global_team_id):
        """Sets the global_team_id of this Standing.


        :param global_team_id: The global_team_id of this Standing.
        :type global_team_id: int
        """

        self._global_team_id = global_team_id

    @property
    def losses(self):
        """Gets the losses of this Standing.


        :return: The losses of this Standing.
        :rtype: int
        """
        return self._losses

    @losses.setter
    def losses(self, losses):
        """Sets the losses of this Standing.


        :param losses: The losses of this Standing.
        :type losses: int
        """

        self._losses = losses

    @property
    def name(self):
        """Gets the name of this Standing.


        :return: The name of this Standing.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name):
        """Sets the name of this Standing.


        :param name: The name of this Standing.
        :type name: str
        """

        self._name = name

    @property
    def net_points(self):
        """Gets the net_points of this Standing.


        :return: The net_points of this Standing.
        :rtype: int
        """
        return self._net_points

    @net_points.setter
    def net_points(self, net_points):
        """Sets the net_points of this Standing.


        :param net_points: The net_points of this Standing.
        :type net_points: int
        """

        self._net_points = net_points

    @property
    def percentage(self):
        """Gets the percentage of this Standing.


        :return: The percentage of this Standing.
        :rtype: float
        """
        return self._percentage

    @percentage.setter
    def percentage(self, percentage):
        """Sets the percentage of this Standing.


        :param percentage: The percentage of this Standing.
        :type percentage: float
        """

        self._percentage = percentage

    @property
    def points_against(self):
        """Gets the points_against of this Standing.


        :return: The points_against of this Standing.
        :rtype: int
        """
        return self._points_against

    @points_against.setter
    def points_against(self, points_against):
        """Sets the points_against of this Standing.


        :param points_against: The points_against of this Standing.
        :type points_against: int
        """

        self._points_against = points_against

    @property
    def points_for(self):
        """Gets the points_for of this Standing.


        :return: The points_for of this Standing.
        :rtype: int
        """
        return self._points_for

    @points_for.setter
    def points_for(self, points_for):
        """Sets the points_for of this Standing.


        :param points_for: The points_for of this Standing.
        :type points_for: int
        """

        self._points_for = points_for

    @property
    def season(self):
        """Gets the season of this Standing.


        :return: The season of this Standing.
        :rtype: int
        """
        return self._season

    @season.setter
    def season(self, season):
        """Sets the season of this Standing.


        :param season: The season of this Standing.
        :type season: int
        """

        self._season = season

    @property
    def season_type(self):
        """Gets the season_type of this Standing.


        :return: The season_type of this Standing.
        :rtype: int
        """
        return self._season_type

    @season_type.setter
    def season_type(self, season_type):
        """Sets the season_type of this Standing.


        :param season_type: The season_type of this Standing.
        :type season_type: int
        """

        self._season_type = season_type

    @property
    def team(self):
        """Gets the team of this Standing.


        :return: The team of this Standing.
        :rtype: str
        """
        return self._team

    @team.setter
    def team(self, team):
        """Sets the team of this Standing.


        :param team: The team of this Standing.
        :type team: str
        """

        self._team = team

    @property
    def team_id(self):
        """Gets the team_id of this Standing.


        :return: The team_id of this Standing.
        :rtype: int
        """
        return self._team_id

    @team_id.setter
    def team_id(self, team_id):
        """Sets the team_id of this Standing.


        :param team_id: The team_id of this Standing.
        :type team_id: int
        """

        self._team_id = team_id

    @property
    def ties(self):
        """Gets the ties of this Standing.


        :return: The ties of this Standing.
        :rtype: int
        """
        return self._ties

    @ties.setter
    def ties(self, ties):
        """Sets the ties of this Standing.


        :param ties: The ties of this Standing.
        :type ties: int
        """

        self._ties = ties

    @property
    def touchdowns(self):
        """Gets the touchdowns of this Standing.


        :return: The touchdowns of this Standing.
        :rtype: int
        """
        return self._touchdowns

    @touchdowns.setter
    def touchdowns(self, touchdowns):
        """Sets the touchdowns of this Standing.


        :param touchdowns: The touchdowns of this Standing.
        :type touchdowns: int
        """

        self._touchdowns = touchdowns

    @property
    def wins(self):
        """Gets the wins of this Standing.


        :return: The wins of this Standing.
        :rtype: int
        """
        return self._wins

    @wins.setter
    def wins(self, wins):
        """Sets the wins of this Standing.


        :param wins: The wins of this Standing.
        :type wins: int
        """

        self._wins = wins
