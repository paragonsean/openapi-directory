...
async def test_depth_charts(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/mlb/projections/{format}/DepthCharts'.format(format=XML),
        headers=headers,
        )
...
async def test_dfs_slates_by_date(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/mlb/projections/{format}/DfsSlatesByDate/{_date}'.format(format=XML, _date='_date_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_injured_players(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/mlb/projections/{format}/InjuredPlayers'.format(format=XML),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_projected_player_game_stats_by_date_w_injuries_dfs_salaries(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/mlb/projections/{format}/PlayerGameProjectionStatsByDate/{_date}'.format(format=XML, _date='_date_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_projected_player_game_stats_by_player_w_injuries_dfs_salaries(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/mlb/projections/{format}/PlayerGameProjectionStatsByPlayer/{_date}/{playerid}'.format(format=XML, _date='_date_example', playerid='playerid_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_projected_player_season_stats_with_adp(client):
    """Test case for projected_player_season_stats_with_adp
    ...
    response = await client.request(
        method='GET',
        path='/v3/mlb/projections/{format}/PlayerSeasonProjectionStats/{season}'.format(format=XML, season='season_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_starting_lineups_by_date(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/mlb/projections/{format}/StartingLineupsByDate/{_date}'.format(format=XML, _date='_date_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
