...
async def test_rotoballer_articles(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/nfl/articles-rotoballer/{format}/RotoBallerArticles'.format(format=xml),
        headers=headers,
        )
...
async def test_rotoballer_articles_by_date(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/nfl/articles-rotoballer/{format}/RotoBallerArticlesByDate/{_date}'.format(format=xml, _date='_date_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_rotoballer_articles_by_player(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/nfl/articles-rotoballer/{format}/RotoBallerArticlesByPlayerID/{playerid}'.format(format=xml, playerid='playerid_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
