...
let apiInstance = new OrgHunter.CharityPremiumApi();
let opts = {
  'ein': "ein_example" // String | ein (Employer Identification Number)
};
apiInstance.v1CharitypremiumPost(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
