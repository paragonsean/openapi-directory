from typing import List, Dict
from aiohttp import web

from openapi_server.models.custom_field import CustomField
from openapi_server import util


async def v2_custom_fields_id_json_delete(request: web.Request, id) -> web.Response:
    """Delete a custom field

    Deletes a custom field. 

    :param id: Custom Field ID
    :type id: str

    """
    return web.Response(status=200)


async def v2_custom_fields_id_json_get(request: web.Request, id) -> web.Response:
    """Fetch a custom field

    Fetches a custom field, by ID only. 

    :param id: Custom Field ID
    :type id: str

    """
    return web.Response(status=200)


async def v2_custom_fields_id_json_put(request: web.Request, id, field_type=None, name=None) -> web.Response:
    """Update a custom field

    Update a custom field. 

    :param id: Custom Field ID
    :type id: str
    :param field_type: The field type of the custom field. Value must be one of: person, company, opportunity
    :type field_type: str
    :param name: The name of the custom field
    :type name: str

    """
    return web.Response(status=200)


async def v2_custom_fields_json_get(request: web.Request, ids=None, field_type=None, sort_by=None, sort_direction=None, per_page=None, page=None, include_paging_counts=None, limit_paging_counts=None) -> web.Response:
    """List custom fields

    Fetches multiple custom field records. The records can be filtered, paged, and sorted according to the respective parameters. 

    :param ids: IDs of custom fields to fetch.
    :type ids: List[int]
    :param field_type: Type of field to fetch. Value must be one of: person, company, opportunity
    :type field_type: str
    :param sort_by: Key to sort on, must be one of: created_at, updated_at, name. Defaults to updated_at
    :type sort_by: str
    :param sort_direction: Direction to sort in, must be one of: ASC, DESC. Defaults to DESC
    :type sort_direction: str
    :param per_page: How many records to show per page in the range [1, 100]. Defaults to 25
    :type per_page: int
    :param page: The current page to fetch results from. Defaults to 1
    :type page: int
    :param include_paging_counts: Whether to include total_pages and total_count in the metadata. Defaults to false
    :type include_paging_counts: bool
    :param limit_paging_counts: Specifies whether the max limit of 10k records should be applied to pagination counts. Affects the total_count and total_pages data
    :type limit_paging_counts: bool

    """
    return web.Response(status=200)


async def v2_custom_fields_json_post(request: web.Request, name, field_type=None) -> web.Response:
    """Create a custom field

    Creates a custom field. 

    :param name: The name of the custom field
    :type name: str
    :param field_type: The field type of the custom field. Value must be one of: person, company, opportunity
    :type field_type: str

    """
    return web.Response(status=200)
