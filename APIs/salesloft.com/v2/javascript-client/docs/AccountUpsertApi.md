# SalesLoftPlatform.AccountUpsertApi
...
> AccountUpsert v2AccountUpsertsJsonPost(domain, name, opts)
...
let apiInstance = new SalesLoftPlatform.AccountUpsertApi();
...
let opts = {
  ...
  'id': 56, // Number | ID of the account to update. Used if the upsert_key=id. When id and another upsert_key are provided, the request will fail if the upsert record id and id parameter don't match. 
...
apiInstance.v2AccountUpsertsJsonPost(domain, name, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
 ...
 **id** | **Number**| ID of the account to update. Used if the upsert_key&#x3D;id. When id and another upsert_key are provided, the request will fail if the upsert record id and id parameter don&#39;t match.  | [optional] 
