# SalesLoftPlatform.CRMActivitiesApi
...
let apiInstance = new SalesLoftPlatform.CRMActivitiesApi();
let id = "id_example"; // String | Crm activity ID
apiInstance.v2CrmActivitiesIdJsonGet(id, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
> [CrmActivity] v2CrmActivitiesJsonGet(opts)
...
let apiInstance = new SalesLoftPlatform.CRMActivitiesApi();
let opts = {
  'ids': [null], // [Number] | IDs of crm activities to fetch.
...
apiInstance.v2CrmActivitiesJsonGet(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
...
Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **ids** | [**[Number]**](Number.md)| IDs of crm activities to fetch. | [optional] 
