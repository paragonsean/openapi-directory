...
public class PreviewEntity {
  ...
  public PreviewEntity size(String size) {
    this.size = size;
    return this;
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to PreviewEntity
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PreviewEntity.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PreviewEntity is not found in the empty JSON string", PreviewEntity.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PreviewEntity.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PreviewEntity` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PreviewEntity.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PreviewEntity' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PreviewEntity> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PreviewEntity.class));

       return (TypeAdapter<T>) new TypeAdapter<PreviewEntity>() {
           @Override
           public void write(JsonWriter out, PreviewEntity value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PreviewEntity read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of PreviewEntity given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of PreviewEntity
   * @throws IOException if the JSON string is invalid with respect to PreviewEntity
   */
  public static PreviewEntity fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PreviewEntity.class);
  }

  /**
   * Convert an instance of PreviewEntity to an JSON string
   *
   * @return JSON string
   */
