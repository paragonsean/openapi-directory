# FilesComApi.As2IncomingMessagesApi
...
> [As2IncomingMessageEntity] getAs2IncomingMessages(opts)
...
let apiInstance = new FilesComApi.As2IncomingMessagesApi();
let opts = {
  ...
  'sortBy': {key: null}, // Object | If set, sort records by the specified field in either `asc` or `desc` direction (e.g. `sort_by[created_at]=desc`). Valid fields are `created_at` and `as2_partner_id`.
  ...
  'as2PartnerId': 56 // Number | As2 Partner ID.  If provided, will return message specific to that partner.
};
apiInstance.getAs2IncomingMessages(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
 ...
 **as2PartnerId** | **Number**| As2 Partner ID.  If provided, will return message specific to that partner. | [optional] 

### Return type

[**[As2IncomingMessageEntity]**](As2IncomingMessageEntity.md)
