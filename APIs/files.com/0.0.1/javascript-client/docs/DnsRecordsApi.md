...
> [DnsRecordEntity] getDnsRecords(opts)
...
let apiInstance = new FilesComApi.DnsRecordsApi();
let opts = {
...
apiInstance.getDnsRecords(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
