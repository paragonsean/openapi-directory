...
> [InboxUploadEntity] getInboxUploads(opts)
...
let apiInstance = new FilesComApi.InboxUploadsApi();
let opts = {
  ...
  'inboxRegistrationId': 56, // Number | InboxRegistration ID
  'inboxId': 56 // Number | Inbox ID
};
apiInstance.getInboxUploads(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
 ...
 **inboxRegistrationId** | **Number**| InboxRegistration ID | [optional] 
 **inboxId** | **Number**| Inbox ID | [optional] 

### Return type

[**[InboxUploadEntity]**](InboxUploadEntity.md)
