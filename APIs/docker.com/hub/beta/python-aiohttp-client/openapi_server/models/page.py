# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server import util


class Page(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, count: int=None, next: str=None, previous: str=None):
        """Page - a model defined in OpenAPI

        :param count: The count of this Page.
        :param next: The next of this Page.
        :param previous: The previous of this Page.
        """
        self.openapi_types = {
            'count': int,
            'next': str,
            'previous': str
        }

        self.attribute_map = {
            'count': 'count',
            'next': 'next',
            'previous': 'previous'
        }

        self._count = count
        self._next = next
        self._previous = previous

    @classmethod
    def from_dict(cls, dikt: dict) -> 'Page':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The page of this Page.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def count(self):
        """Gets the count of this Page.

        total number of results available across all pages

        :return: The count of this Page.
        :rtype: int
        """
        return self._count

    @count.setter
    def count(self, count):
        """Sets the count of this Page.

        total number of results available across all pages

        :param count: The count of this Page.
        :type count: int
        """

        self._count = count

    @property
    def next(self):
        """Gets the next of this Page.

        link to next page of results if any

        :return: The next of this Page.
        :rtype: str
        """
        return self._next

    @next.setter
    def next(self, next):
        """Sets the next of this Page.

        link to next page of results if any

        :param next: The next of this Page.
        :type next: str
        """

        self._next = next

    @property
    def previous(self):
        """Gets the previous of this Page.

        link to previous page of results  if any

        :return: The previous of this Page.
        :rtype: str
        """
        return self._previous

    @previous.setter
    def previous(self, previous):
        """Sets the previous of this Page.

        link to previous page of results  if any

        :param previous: The previous of this Page.
        :type previous: str
        """

        self._previous = previous
