...
public class EpisodeBase {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(audioPreviewUrl, description, durationMs, explicit, externalUrls, href, htmlDescription, id, images, isExternallyHosted, isPlayable, language, languages, name, releaseDate, releaseDatePrecision, restrictions, resumePoint, type, uri);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to EpisodeBase
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!EpisodeBase.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!EpisodeBase.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `EpisodeBase` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
