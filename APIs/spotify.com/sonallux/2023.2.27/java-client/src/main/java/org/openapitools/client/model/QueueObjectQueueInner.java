...
public class QueueObjectQueueInner extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(QueueObjectQueueInner.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!QueueObjectQueueInner.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'QueueObjectQueueInner' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<QueueObjectQueueInner>() {
                @Override
                public void write(JsonWriter out, QueueObjectQueueInner value) throws IOException {
                ...
                @Override
                public QueueObjectQueueInner read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        QueueObjectQueueInner ret = new QueueObjectQueueInner();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonElement));
                        return ret;
                    }

                    throw new IOException(String.format("Failed deserialization for QueueObjectQueueInner: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public QueueObjectQueueInner() {
        super("oneOf", Boolean.FALSE);
    }

    public QueueObjectQueueInner(Object o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }
    ...
    public Map<String, Class<?>> getSchemas() {
        return QueueObjectQueueInner.schemas;
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to QueueObjectQueueInner
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            throw new IOException(String.format("The JSON string is invalid for QueueObjectQueueInner with oneOf schemas: EpisodeObject, TrackObject. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }

    /**
     * Create an instance of QueueObjectQueueInner given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of QueueObjectQueueInner
     * @throws IOException if the JSON string is invalid with respect to QueueObjectQueueInner
     */
    public static QueueObjectQueueInner fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, QueueObjectQueueInner.class);
    }

    /**
     * Convert an instance of QueueObjectQueueInner to an JSON string
     *
     * @return JSON string
     */
