...
public class TuneableTrackObject {
   ...
   * @throws IOException if the JSON Element is invalid with respect to TuneableTrackObject
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!TuneableTrackObject.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in TuneableTrackObject is not found in the empty JSON string", TuneableTrackObject.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!TuneableTrackObject.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TuneableTrackObject` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!TuneableTrackObject.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'TuneableTrackObject' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<TuneableTrackObject> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(TuneableTrackObject.class));

       return (TypeAdapter<T>) new TypeAdapter<TuneableTrackObject>() {
           @Override
           public void write(JsonWriter out, TuneableTrackObject value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public TuneableTrackObject read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of TuneableTrackObject given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of TuneableTrackObject
   * @throws IOException if the JSON string is invalid with respect to TuneableTrackObject
   */
  public static TuneableTrackObject fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, TuneableTrackObject.class);
  }

  /**
   * Convert an instance of TuneableTrackObject to an JSON string
   *
   * @return JSON string
   */
