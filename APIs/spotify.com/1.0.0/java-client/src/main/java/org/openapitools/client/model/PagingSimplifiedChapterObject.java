...
public class PagingSimplifiedChapterObject {
   ...
   * @throws IOException if the JSON Element is invalid with respect to PagingSimplifiedChapterObject
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PagingSimplifiedChapterObject.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PagingSimplifiedChapterObject.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PagingSimplifiedChapterObject` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayitems.size(); i++) {
        SimplifiedChapterObject.validateJsonElement(jsonArrayitems.get(i));
      };
  }
