...
class Status(Model):
    ...
    def info(self, info):
        ...
        :type info: str
        """
        if info is not None and len(info) > 2097152:
            raise ValueError("Invalid value for `info`, length must be less than or equal to `2097152`")
        if info is not None and not re.search(r'^.{1,2097152}', info):
            raise ValueError("Invalid value for `info`, must be a follow pattern or equal to `/^.{1,2097152}/`")
    ...
    def reason(self, reason):
        ...
        :type reason: str
        """
        if reason is not None and len(reason) > 2097152:
            raise ValueError("Invalid value for `reason`, length must be less than or equal to `2097152`")
        if reason is not None and not re.search(r'^.{1,2097152}', reason):
            raise ValueError("Invalid value for `reason`, must be a follow pattern or equal to `/^.{1,2097152}/`")
