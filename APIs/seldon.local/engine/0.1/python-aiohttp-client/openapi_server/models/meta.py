...
class Meta(Model):
    ...
    def metrics(self, metrics):
        ...
        """
        if metrics is not None and len(metrics) > 2097152:
            raise ValueError("Invalid value for `metrics`, number of items must be less than or equal to `2097152`")
    ...
    def puid(self, puid):
        ...
        :type puid: str
        """
        if puid is not None and len(puid) > 2097152:
            raise ValueError("Invalid value for `puid`, length must be less than or equal to `2097152`")
        if puid is not None and not re.search(r'^.{1,2097152}', puid):
            raise ValueError("Invalid value for `puid`, must be a follow pattern or equal to `/^.{1,2097152}/`")
