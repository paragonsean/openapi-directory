...
public class UserV2 {
   ...
   * @throws IOException if the JSON Element is invalid with respect to UserV2
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UserV2.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in UserV2 is not found in the empty JSON string", UserV2.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!UserV2.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UserV2` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : UserV2.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!UserV2.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'UserV2' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<UserV2> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(UserV2.class));

       return (TypeAdapter<T>) new TypeAdapter<UserV2>() {
           @Override
           public void write(JsonWriter out, UserV2 value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public UserV2 read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of UserV2 given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of UserV2
   * @throws IOException if the JSON string is invalid with respect to UserV2
   */
  public static UserV2 fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, UserV2.class);
  }

  /**
   * Convert an instance of UserV2 to an JSON string
   *
   * @return JSON string
   */
