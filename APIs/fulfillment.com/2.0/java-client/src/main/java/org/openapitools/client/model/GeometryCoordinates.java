...
public class GeometryCoordinates extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(GeometryCoordinates.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!GeometryCoordinates.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'GeometryCoordinates' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<GeometryCoordinates>() {
                @Override
                public void write(JsonWriter out, GeometryCoordinates value) throws IOException {
                    ...
                    throw new IOException("Failed to serialize as the type doesn't match oneOf schemas: List<BigDecimal>, List<Object>");
                }

                @Override
                public GeometryCoordinates read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        GeometryCoordinates ret = new GeometryCoordinates();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonElement));
                        return ret;
                    }

                    throw new IOException(String.format("Failed deserialization for GeometryCoordinates: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public GeometryCoordinates(Object o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }
    ...
    public Map<String, Class<?>> getSchemas() {
        return GeometryCoordinates.schemas;
    }
    ...
    public void setActualInstance(Object instance) {
        ...
        throw new RuntimeException("Invalid instance type. Must be List<BigDecimal>, List<Object>");
    }
     ...
     * List<BigDecimal>, List<Object>
     *
     * @return The actual instance (List<BigDecimal>, List<Object>)
     */
    @SuppressWarnings("unchecked")
    @Override
     ...
     * @throws IOException if the JSON Element is invalid with respect to GeometryCoordinates
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            throw new IOException(String.format("The JSON string is invalid for GeometryCoordinates with oneOf schemas: List<BigDecimal>, List<Object>. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }

    /**
     * Create an instance of GeometryCoordinates given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of GeometryCoordinates
     * @throws IOException if the JSON string is invalid with respect to GeometryCoordinates
     */
    public static GeometryCoordinates fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, GeometryCoordinates.class);
    }

    /**
     * Convert an instance of GeometryCoordinates to an JSON string
     *
     * @return JSON string
     */
