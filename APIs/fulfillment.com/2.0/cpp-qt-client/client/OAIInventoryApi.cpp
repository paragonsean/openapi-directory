...
void OAIInventoryApi::getInventory(const ::OpenAPI::OptionalParam<qint32> &page, const ::OpenAPI::OptionalParam<qint32> &limit, const ::OpenAPI::OptionalParam<QList<qint32>> &merchant_ids, const ::OpenAPI::OptionalParam<QList<qint32>> &warehouse_ids, const ::OpenAPI::OptionalParam<QList<QString>> &external_sku_names) {
    QString fullPath = QString(_serverConfigs["getInventory"][_serverIndices.value("getInventory")].URL()+"/inventory");
    ...
    if (external_sku_names.hasValue())
    {
        queryStyle = "form";
        if (queryStyle == "")
            queryStyle = "form";
        queryPrefix = getParamStylePrefix(queryStyle);
        querySuffix = getParamStyleSuffix(queryStyle);
        queryDelimiter = getParamStyleDelimiter(queryStyle, "externalSkuNames", false);
        if (external_sku_names.value().size() > 0) {
            if (QString("csv").indexOf("multi") == 0) {
                for (QString t : external_sku_names.value()) {
                    if (fullPath.indexOf("?") > 0)
                        fullPath.append(queryPrefix);
                    else
                        fullPath.append("?");
                    fullPath.append("externalSkuNames=").append(::OpenAPI::toStringValue(t));
                }
            } else if (QString("csv").indexOf("ssv") == 0) {
                if (fullPath.indexOf("?") > 0)
                    fullPath.append("&");
                else
                    fullPath.append("?").append(queryPrefix).append("externalSkuNames").append(querySuffix);
                qint32 count = 0;
                for (QString t : external_sku_names.value()) {
                    if (count > 0) {
                        fullPath.append((false)? queryDelimiter : QUrl::toPercentEncoding(queryDelimiter));
    ...
    connect(_latestWorker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIInventoryApi::getInventoryCallback);
...
void OAIInventoryApi::getInventoryCallback(OAIHttpRequestWorker *worker) {
    ...
    OAIItemInventoryArray_v2 output(QString(worker->response));
    worker->deleteLater();
