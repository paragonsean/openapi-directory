/**
 * Fulfillment.com APIv2
 * Welcome to our current iteration of our REST API. While we encourage you to upgrade to v2.0 we will continue support for our [SOAP API](https://github.com/fulfillment/soap-integration).  # Versioning  The Fulfillment.com (FDC) REST API is version controlled and backwards compatible. We have many future APIs scheduled for publication within our v2.0 spec so please be prepared for us to add data nodes in our responses, however, we will not remove knowledge from previously published APIs.  #### A Current Response  ```javascript {   id: 123 } ```  #### A Potential Future Response  ```javascript {   id: 123,   reason: \"More Knowledge\" } ```  # Getting Started  We use OAuth v2.0 to authenticate clients, you can choose [implicit](https://oauth.net/2/grant-types/implicit/) or [password](https://oauth.net/2/grant-types/password/) grant type. To obtain an OAuth `client_id` and `client_secret` contact your account executive.  **Tip**: Generate an additional login and use those credentials for your integration so that changes are accredited to that \"user\".  You are now ready to make requests to our other APIs by filling your `Authorization` header with `Bearer {access_token}`.  ## Perpetuating Access  Perpetuating access to FDC without storing your password locally can be achieved using the `refresh_token` returned by [POST /oauth/access_token](#operation/generateToken).  A simple concept to achieve this is outlined below.  1. Your application/script will ask you for your `username` and `password`, your `client_id` and `client_secret` will be accessible via a DB or ENV. 2. [Request an access_token](#operation/generateToken)   + Your function should be capable of formatting your request for both a `grant_type` of \\\"password\\\" (step 1) and \\\"refresh_token\\\" (step 4). 3. Store the `access_token` and `refresh_token` so future requests can skip step 1 4. When the `access_token` expires request anew using your `refresh_token`, replace both tokens in local storage.  + If this fails you will have to revert to step 1.  Alternatively if you choose for your application/script to have access to your `username` and `password` you can skip step 4.  In all scenarios we recommend storing all credentials outside your codebase.  ## Date Time Definitions  We will report all date-time stamps using the [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) standard. When using listing API's where fromDate and toDate are available note that both dates are inclusive while requiring the fromDate to be before or at the toDate.  ### The Fulfillment Process  Many steps are required to fulfill your order we report back to you three fundamental milestones inside the orders model.  * `recordedOn` When we received your order. This will never change.  * `dispatchDate` When the current iteration of your order was scheduled for fulfillment. This may change however it is an indicator that the physical process of fulfillment has begun and a tracking number has been **assigned** to your order. The tracking number **MAY CHANGE**. You will not be able to cancel an order once it has been dispatched. If you need to recall an order that has been dispatched please contact your account executive.  * `departDate` When we recorded your order passing our final inspection and placed with the carrier. At this point it is **safe to inform the consignee** of the tracking number as it will not change.  ## Evaluating Error Responses  We currently return two different error models, with and without context. All errors will include a `message` node while errors with `context` will include additional information designed to save you time when encountering highly probable errors. For example, when you send us a request to create a duplicate order, we will reject your request and the context will include the FDC order `id` so that you may record it for your records.  ### Without Context  New order with missing required fields.  | Header | Response | | ------ | -------- | | Status | `400 Bad Request` |  ```javascript {       \"message\": \"Invalid request body\" } ```  ### With Context  New order with duplicate `merchantOrderId`.  | Header | Response | | ------ | -------- | | Status | `409 Conflict` |  ```javascript {   \"message\": \"Duplicate Order\",   \"context\": {     \"id\": 123   } } ```  ## Status Codes  Codes are a concatenation of State, Stage, and Detail.  `^([0-9]{2})([0-9]{2})([0-9]{2})$`  | Code | State              | Stage    | Detail         | | ---- | ------------------ | -------- | -------------- | | 010101 | Processing Order | Recieved | Customer Order | | 010102 | Processing Order | Recieved | Recieved | | 010201 | Processing Order | Approved | | | 010301 | Processing Order | Hold | Merchant Stock | | 010302 | Processing Order | Hold | Merchant Funds | | 010303 | Processing Order | Hold | For Merchant | | 010304 | Processing Order | Hold | Oversized Shipment | | 010305 | Processing Order | Hold | Invalid Parent Order | | 010306 | Processing Order | Hold | Invalid Address | | 010307 | Processing Order | Hold | By Admin | | 010401 | Processing Order | Address Problem | Incomplete Address | | 010402 | Processing Order | Address Problem | Invalid Locality | | 010403 | Processing Order | Address Problem | Invalid Region | | 010404 | Processing Order | Address Problem | Address Not Found | | 010405 | Processing Order | Address Problem | Many Addresses Found | | 010406 | Processing Order | Address Problem | Invalid Postal Code | | 010407 | Processing Order | Address Problem | Country Not Mapped | | 010408 | Processing Order | Address Problem | Invalid Recipient Name | | 010409 | Processing Order | Address Problem | Bad UK Address | | 010410 | Processing Order | Address Problem | Invalid Address Line 1 or 2 | | 010501 | Processing Order | Sku Problem | Invalid SKU | | 010501 | Processing Order | Sku Problem | Child Order has Invalid SKUs | | 010601 | Processing Order | Facility Problem | Facility Not Mapped | | 010701 | Processing Order | Ship Method Problem | Unmapped Ship Method | | 010702 | Processing Order | Ship Method Problem | Unmapped Ship Cost | | 010703 | Processing Order | Ship Method Problem | Missing Ship Method | | 010704 | Processing Order | Ship Method Problem | Invalid Ship Method | | 010705 | Processing Order | Ship Method Problem | Order Weight Outside of Ship Method Weight | | 010801 | Processing Order | Inventory Problem | Insufficient Inventory In Facility | | 010802 | Processing Order | Inventory Problem | Issue Encountered During Inventory Adjustment | | 010901 | Processing Order | Released To WMS | Released | | 020101 | Fulfillment In Progress | Postage Problem | Address Issue | | 020102 | Fulfillment In Progress | Postage Problem | Postage OK, OMS Issue Occurred | | 020103 | Fulfillment In Progress | Postage Problem | Postage Void Failed | | 020201 | Fulfillment In Progress | Postage Acquired | | | 020301 | Fulfillment In Progress | Postage Voided | Postage Void Failed Gracefully | | 020301 | Fulfillment In Progress | Hold | Departure Hold Requested | | 020401 | Fulfillment In Progress | 4PL Processing | | | 020501 | Fulfillment In Progress | 4PL Problem | Order is Proccessable, Postage Issue Occurred | | 020601 | Fulfillment In Progress | Label Printed | | | 020701 | Fulfillment In Progress | Shipment Cubed | | | 020801 | Fulfillment In Progress | Picking Inventory | | | 020901 | Fulfillment In Progress | Label Print Verified | | | 021001 | Fulfillment In Progress | Passed Final Inspection | | | 030101 | Shipped | Fulfilled By 4PL | | | 030102 | Shipped | Fulfilled By 4PL | Successfully Fulfilled, OMS Encountered Issue During Processing | | 030201 | Shipped | Fulfilled By FDC | | | 040101 | Returned | Returned | | | 050101 | Cancelled | Cancelled | | | 060101 | Test | Test | Test | 
 *
 * The version of the OpenAPI document: 2.0
 * Contact: dev@fulfillment.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIOrderResponse_v2.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIOrderResponse_v2::OAIOrderResponse_v2(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIOrderResponse_v2::OAIOrderResponse_v2() {
    this->initializeModel();
}

OAIOrderResponse_v2::~OAIOrderResponse_v2() {}

void OAIOrderResponse_v2::initializeModel() {

    m_current_status_isSet = false;
    m_current_status_isValid = false;

    m_depart_date_isSet = false;
    m_depart_date_isValid = false;

    m_dispatch_date_isSet = false;
    m_dispatch_date_isValid = false;

    m_id_isSet = false;
    m_id_isValid = false;

    m_merchant_isSet = false;
    m_merchant_isValid = false;

    m_merchant_order_id_isSet = false;
    m_merchant_order_id_isValid = false;

    m_merchant_shipping_method_isSet = false;
    m_merchant_shipping_method_isValid = false;

    m_original_consignee_isSet = false;
    m_original_consignee_isValid = false;

    m_parent_order_isSet = false;
    m_parent_order_isValid = false;

    m_purchase_order_num_isSet = false;
    m_purchase_order_num_isValid = false;

    m_recorded_on_isSet = false;
    m_recorded_on_isValid = false;

    m_tracking_numbers_isSet = false;
    m_tracking_numbers_isValid = false;

    m_validated_consignee_isSet = false;
    m_validated_consignee_isValid = false;

    m_warehouse_isSet = false;
    m_warehouse_isValid = false;
}

void OAIOrderResponse_v2::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAIOrderResponse_v2::fromJsonObject(QJsonObject json) {

    m_current_status_isValid = ::OpenAPI::fromJsonValue(m_current_status, json[QString("currentStatus")]);
    m_current_status_isSet = !json[QString("currentStatus")].isNull() && m_current_status_isValid;

    m_depart_date_isValid = ::OpenAPI::fromJsonValue(m_depart_date, json[QString("departDate")]);
    m_depart_date_isSet = !json[QString("departDate")].isNull() && m_depart_date_isValid;

    m_dispatch_date_isValid = ::OpenAPI::fromJsonValue(m_dispatch_date, json[QString("dispatchDate")]);
    m_dispatch_date_isSet = !json[QString("dispatchDate")].isNull() && m_dispatch_date_isValid;

    m_id_isValid = ::OpenAPI::fromJsonValue(m_id, json[QString("id")]);
    m_id_isSet = !json[QString("id")].isNull() && m_id_isValid;

    m_merchant_isValid = ::OpenAPI::fromJsonValue(m_merchant, json[QString("merchant")]);
    m_merchant_isSet = !json[QString("merchant")].isNull() && m_merchant_isValid;

    m_merchant_order_id_isValid = ::OpenAPI::fromJsonValue(m_merchant_order_id, json[QString("merchantOrderId")]);
    m_merchant_order_id_isSet = !json[QString("merchantOrderId")].isNull() && m_merchant_order_id_isValid;

    m_merchant_shipping_method_isValid = ::OpenAPI::fromJsonValue(m_merchant_shipping_method, json[QString("merchantShippingMethod")]);
    m_merchant_shipping_method_isSet = !json[QString("merchantShippingMethod")].isNull() && m_merchant_shipping_method_isValid;

    m_original_consignee_isValid = ::OpenAPI::fromJsonValue(m_original_consignee, json[QString("originalConsignee")]);
    m_original_consignee_isSet = !json[QString("originalConsignee")].isNull() && m_original_consignee_isValid;

    m_parent_order_isValid = ::OpenAPI::fromJsonValue(m_parent_order, json[QString("parentOrder")]);
    m_parent_order_isSet = !json[QString("parentOrder")].isNull() && m_parent_order_isValid;

    m_purchase_order_num_isValid = ::OpenAPI::fromJsonValue(m_purchase_order_num, json[QString("purchaseOrderNum")]);
    m_purchase_order_num_isSet = !json[QString("purchaseOrderNum")].isNull() && m_purchase_order_num_isValid;

    m_recorded_on_isValid = ::OpenAPI::fromJsonValue(m_recorded_on, json[QString("recordedOn")]);
    m_recorded_on_isSet = !json[QString("recordedOn")].isNull() && m_recorded_on_isValid;

    m_tracking_numbers_isValid = ::OpenAPI::fromJsonValue(m_tracking_numbers, json[QString("trackingNumbers")]);
    m_tracking_numbers_isSet = !json[QString("trackingNumbers")].isNull() && m_tracking_numbers_isValid;

    m_validated_consignee_isValid = ::OpenAPI::fromJsonValue(m_validated_consignee, json[QString("validatedConsignee")]);
    m_validated_consignee_isSet = !json[QString("validatedConsignee")].isNull() && m_validated_consignee_isValid;

    m_warehouse_isValid = ::OpenAPI::fromJsonValue(m_warehouse, json[QString("warehouse")]);
    m_warehouse_isSet = !json[QString("warehouse")].isNull() && m_warehouse_isValid;
}

QString OAIOrderResponse_v2::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAIOrderResponse_v2::asJsonObject() const {
    QJsonObject obj;
    if (m_current_status.isSet()) {
        obj.insert(QString("currentStatus"), ::OpenAPI::toJsonValue(m_current_status));
    }
    if (m_depart_date_isSet) {
        obj.insert(QString("departDate"), ::OpenAPI::toJsonValue(m_depart_date));
    }
    if (m_dispatch_date_isSet) {
        obj.insert(QString("dispatchDate"), ::OpenAPI::toJsonValue(m_dispatch_date));
    }
    if (m_id_isSet) {
        obj.insert(QString("id"), ::OpenAPI::toJsonValue(m_id));
    }
    if (m_merchant.isSet()) {
        obj.insert(QString("merchant"), ::OpenAPI::toJsonValue(m_merchant));
    }
    if (m_merchant_order_id_isSet) {
        obj.insert(QString("merchantOrderId"), ::OpenAPI::toJsonValue(m_merchant_order_id));
    }
    if (m_merchant_shipping_method_isSet) {
        obj.insert(QString("merchantShippingMethod"), ::OpenAPI::toJsonValue(m_merchant_shipping_method));
    }
    if (m_original_consignee.isSet()) {
        obj.insert(QString("originalConsignee"), ::OpenAPI::toJsonValue(m_original_consignee));
    }
    if (m_parent_order.isSet()) {
        obj.insert(QString("parentOrder"), ::OpenAPI::toJsonValue(m_parent_order));
    }
    if (m_purchase_order_num_isSet) {
        obj.insert(QString("purchaseOrderNum"), ::OpenAPI::toJsonValue(m_purchase_order_num));
    }
    if (m_recorded_on_isSet) {
        obj.insert(QString("recordedOn"), ::OpenAPI::toJsonValue(m_recorded_on));
    }
    if (m_tracking_numbers.size() > 0) {
        obj.insert(QString("trackingNumbers"), ::OpenAPI::toJsonValue(m_tracking_numbers));
    }
    if (m_validated_consignee.isSet()) {
        obj.insert(QString("validatedConsignee"), ::OpenAPI::toJsonValue(m_validated_consignee));
    }
    if (m_warehouse.isSet()) {
        obj.insert(QString("warehouse"), ::OpenAPI::toJsonValue(m_warehouse));
    }
    return obj;
}

OAIStatusEvent_v2 OAIOrderResponse_v2::getCurrentStatus() const {
    return m_current_status;
}
void OAIOrderResponse_v2::setCurrentStatus(const OAIStatusEvent_v2 &current_status) {
    m_current_status = current_status;
    m_current_status_isSet = true;
}

bool OAIOrderResponse_v2::is_current_status_Set() const{
    return m_current_status_isSet;
}

bool OAIOrderResponse_v2::is_current_status_Valid() const{
    return m_current_status_isValid;
}

QDateTime OAIOrderResponse_v2::getDepartDate() const {
    return m_depart_date;
}
void OAIOrderResponse_v2::setDepartDate(const QDateTime &depart_date) {
    m_depart_date = depart_date;
    m_depart_date_isSet = true;
}

bool OAIOrderResponse_v2::is_depart_date_Set() const{
    return m_depart_date_isSet;
}

bool OAIOrderResponse_v2::is_depart_date_Valid() const{
    return m_depart_date_isValid;
}

QDateTime OAIOrderResponse_v2::getDispatchDate() const {
    return m_dispatch_date;
}
void OAIOrderResponse_v2::setDispatchDate(const QDateTime &dispatch_date) {
    m_dispatch_date = dispatch_date;
    m_dispatch_date_isSet = true;
}

bool OAIOrderResponse_v2::is_dispatch_date_Set() const{
    return m_dispatch_date_isSet;
}

bool OAIOrderResponse_v2::is_dispatch_date_Valid() const{
    return m_dispatch_date_isValid;
}

qint32 OAIOrderResponse_v2::getId() const {
    return m_id;
}
void OAIOrderResponse_v2::setId(const qint32 &id) {
    m_id = id;
    m_id_isSet = true;
}

bool OAIOrderResponse_v2::is_id_Set() const{
    return m_id_isSet;
}

bool OAIOrderResponse_v2::is_id_Valid() const{
    return m_id_isValid;
}

OAIMerchant_v2 OAIOrderResponse_v2::getMerchant() const {
    return m_merchant;
}
void OAIOrderResponse_v2::setMerchant(const OAIMerchant_v2 &merchant) {
    m_merchant = merchant;
    m_merchant_isSet = true;
}

bool OAIOrderResponse_v2::is_merchant_Set() const{
    return m_merchant_isSet;
}

bool OAIOrderResponse_v2::is_merchant_Valid() const{
    return m_merchant_isValid;
}

QString OAIOrderResponse_v2::getMerchantOrderId() const {
    return m_merchant_order_id;
}
void OAIOrderResponse_v2::setMerchantOrderId(const QString &merchant_order_id) {
    m_merchant_order_id = merchant_order_id;
    m_merchant_order_id_isSet = true;
}

bool OAIOrderResponse_v2::is_merchant_order_id_Set() const{
    return m_merchant_order_id_isSet;
}

bool OAIOrderResponse_v2::is_merchant_order_id_Valid() const{
    return m_merchant_order_id_isValid;
}

QString OAIOrderResponse_v2::getMerchantShippingMethod() const {
    return m_merchant_shipping_method;
}
void OAIOrderResponse_v2::setMerchantShippingMethod(const QString &merchant_shipping_method) {
    m_merchant_shipping_method = merchant_shipping_method;
    m_merchant_shipping_method_isSet = true;
}

bool OAIOrderResponse_v2::is_merchant_shipping_method_Set() const{
    return m_merchant_shipping_method_isSet;
}

bool OAIOrderResponse_v2::is_merchant_shipping_method_Valid() const{
    return m_merchant_shipping_method_isValid;
}

OAIConsignee_v2 OAIOrderResponse_v2::getOriginalConsignee() const {
    return m_original_consignee;
}
void OAIOrderResponse_v2::setOriginalConsignee(const OAIConsignee_v2 &original_consignee) {
    m_original_consignee = original_consignee;
    m_original_consignee_isSet = true;
}

bool OAIOrderResponse_v2::is_original_consignee_Set() const{
    return m_original_consignee_isSet;
}

bool OAIOrderResponse_v2::is_original_consignee_Valid() const{
    return m_original_consignee_isValid;
}

OAIOrderResponse_v2_parentOrder OAIOrderResponse_v2::getParentOrder() const {
    return m_parent_order;
}
void OAIOrderResponse_v2::setParentOrder(const OAIOrderResponse_v2_parentOrder &parent_order) {
    m_parent_order = parent_order;
    m_parent_order_isSet = true;
}

bool OAIOrderResponse_v2::is_parent_order_Set() const{
    return m_parent_order_isSet;
}

bool OAIOrderResponse_v2::is_parent_order_Valid() const{
    return m_parent_order_isValid;
}

QString OAIOrderResponse_v2::getPurchaseOrderNum() const {
    return m_purchase_order_num;
}
void OAIOrderResponse_v2::setPurchaseOrderNum(const QString &purchase_order_num) {
    m_purchase_order_num = purchase_order_num;
    m_purchase_order_num_isSet = true;
}

bool OAIOrderResponse_v2::is_purchase_order_num_Set() const{
    return m_purchase_order_num_isSet;
}

bool OAIOrderResponse_v2::is_purchase_order_num_Valid() const{
    return m_purchase_order_num_isValid;
}

QDateTime OAIOrderResponse_v2::getRecordedOn() const {
    return m_recorded_on;
}
void OAIOrderResponse_v2::setRecordedOn(const QDateTime &recorded_on) {
    m_recorded_on = recorded_on;
    m_recorded_on_isSet = true;
}

bool OAIOrderResponse_v2::is_recorded_on_Set() const{
    return m_recorded_on_isSet;
}

bool OAIOrderResponse_v2::is_recorded_on_Valid() const{
    return m_recorded_on_isValid;
}

QList<OAITrackingNumber_v2> OAIOrderResponse_v2::getTrackingNumbers() const {
    return m_tracking_numbers;
}
void OAIOrderResponse_v2::setTrackingNumbers(const QList<OAITrackingNumber_v2> &tracking_numbers) {
    m_tracking_numbers = tracking_numbers;
    m_tracking_numbers_isSet = true;
}

bool OAIOrderResponse_v2::is_tracking_numbers_Set() const{
    return m_tracking_numbers_isSet;
}

bool OAIOrderResponse_v2::is_tracking_numbers_Valid() const{
    return m_tracking_numbers_isValid;
}

OAIObject OAIOrderResponse_v2::getValidatedConsignee() const {
    return m_validated_consignee;
}
void OAIOrderResponse_v2::setValidatedConsignee(const OAIObject &validated_consignee) {
    m_validated_consignee = validated_consignee;
    m_validated_consignee_isSet = true;
}

bool OAIOrderResponse_v2::is_validated_consignee_Set() const{
    return m_validated_consignee_isSet;
}

bool OAIOrderResponse_v2::is_validated_consignee_Valid() const{
    return m_validated_consignee_isValid;
}

OAIWarehouse_v2 OAIOrderResponse_v2::getWarehouse() const {
    return m_warehouse;
}
void OAIOrderResponse_v2::setWarehouse(const OAIWarehouse_v2 &warehouse) {
    m_warehouse = warehouse;
    m_warehouse_isSet = true;
}

bool OAIOrderResponse_v2::is_warehouse_Set() const{
    return m_warehouse_isSet;
}

bool OAIOrderResponse_v2::is_warehouse_Valid() const{
    return m_warehouse_isValid;
}

bool OAIOrderResponse_v2::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_current_status.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_depart_date_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_dispatch_date_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_merchant.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_merchant_order_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_merchant_shipping_method_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_original_consignee.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_parent_order.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_purchase_order_num_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_recorded_on_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_tracking_numbers.size() > 0) {
            isObjectUpdated = true;
            break;
        }

        if (m_validated_consignee.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_warehouse.isSet()) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAIOrderResponse_v2::isValid() const {
    // only required properties are required for the object to be considered valid
    return m_current_status_isValid && m_id_isValid && m_merchant_isValid && m_merchant_order_id_isValid && m_merchant_shipping_method_isValid && m_original_consignee_isValid && m_recorded_on_isValid && m_validated_consignee_isValid && true;
}

} // namespace OpenAPI
