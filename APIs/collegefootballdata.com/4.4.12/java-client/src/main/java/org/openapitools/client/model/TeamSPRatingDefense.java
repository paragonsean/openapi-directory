...
public class TeamSPRatingDefense {
   ...
   * @throws IOException if the JSON Element is invalid with respect to TeamSPRatingDefense
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!TeamSPRatingDefense.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in TeamSPRatingDefense is not found in the empty JSON string", TeamSPRatingDefense.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!TeamSPRatingDefense.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TeamSPRatingDefense` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!TeamSPRatingDefense.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'TeamSPRatingDefense' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<TeamSPRatingDefense> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(TeamSPRatingDefense.class));

       return (TypeAdapter<T>) new TypeAdapter<TeamSPRatingDefense>() {
           @Override
           public void write(JsonWriter out, TeamSPRatingDefense value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public TeamSPRatingDefense read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of TeamSPRatingDefense given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of TeamSPRatingDefense
   * @throws IOException if the JSON string is invalid with respect to TeamSPRatingDefense
   */
  public static TeamSPRatingDefense fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, TeamSPRatingDefense.class);
  }

  /**
   * Convert an instance of TeamSPRatingDefense to an JSON string
   *
   * @return JSON string
   */
