...
public class TeamSPRatingOffense {
   ...
   * @throws IOException if the JSON Element is invalid with respect to TeamSPRatingOffense
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!TeamSPRatingOffense.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in TeamSPRatingOffense is not found in the empty JSON string", TeamSPRatingOffense.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!TeamSPRatingOffense.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TeamSPRatingOffense` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!TeamSPRatingOffense.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'TeamSPRatingOffense' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<TeamSPRatingOffense> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(TeamSPRatingOffense.class));

       return (TypeAdapter<T>) new TypeAdapter<TeamSPRatingOffense>() {
           @Override
           public void write(JsonWriter out, TeamSPRatingOffense value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public TeamSPRatingOffense read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of TeamSPRatingOffense given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of TeamSPRatingOffense
   * @throws IOException if the JSON string is invalid with respect to TeamSPRatingOffense
   */
  public static TeamSPRatingOffense fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, TeamSPRatingOffense.class);
  }

  /**
   * Convert an instance of TeamSPRatingOffense to an JSON string
   *
   * @return JSON string
   */
