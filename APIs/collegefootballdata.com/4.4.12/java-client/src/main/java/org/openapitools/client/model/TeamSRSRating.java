...
public class TeamSRSRating {
   ...
   * @throws IOException if the JSON Element is invalid with respect to TeamSRSRating
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!TeamSRSRating.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in TeamSRSRating is not found in the empty JSON string", TeamSRSRating.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!TeamSRSRating.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TeamSRSRating` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!TeamSRSRating.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'TeamSRSRating' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<TeamSRSRating> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(TeamSRSRating.class));

       return (TypeAdapter<T>) new TypeAdapter<TeamSRSRating>() {
           @Override
           public void write(JsonWriter out, TeamSRSRating value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public TeamSRSRating read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of TeamSRSRating given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of TeamSRSRating
   * @throws IOException if the JSON string is invalid with respect to TeamSRSRating
   */
  public static TeamSRSRating fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, TeamSRSRating.class);
  }

  /**
   * Convert an instance of TeamSRSRating to an JSON string
   *
   * @return JSON string
   */
