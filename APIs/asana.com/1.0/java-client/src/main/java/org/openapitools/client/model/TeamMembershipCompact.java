...
public class TeamMembershipCompact {
   ...
   * @throws IOException if the JSON Element is invalid with respect to TeamMembershipCompact
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!TeamMembershipCompact.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in TeamMembershipCompact is not found in the empty JSON string", TeamMembershipCompact.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!TeamMembershipCompact.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TeamMembershipCompact` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!TeamMembershipCompact.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'TeamMembershipCompact' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<TeamMembershipCompact> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(TeamMembershipCompact.class));

       return (TypeAdapter<T>) new TypeAdapter<TeamMembershipCompact>() {
           @Override
           public void write(JsonWriter out, TeamMembershipCompact value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public TeamMembershipCompact read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of TeamMembershipCompact given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of TeamMembershipCompact
   * @throws IOException if the JSON string is invalid with respect to TeamMembershipCompact
   */
  public static TeamMembershipCompact fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, TeamMembershipCompact.class);
  }

  /**
   * Convert an instance of TeamMembershipCompact to an JSON string
   *
   * @return JSON string
   */
