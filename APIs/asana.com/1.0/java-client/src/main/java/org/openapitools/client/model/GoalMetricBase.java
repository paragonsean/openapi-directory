...
public class GoalMetricBase {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(gid, resourceType, currencyCode, currentDisplayValue, currentNumberValue, initialNumberValue, precision, progressSource, resourceSubtype, targetNumberValue, unit);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to GoalMetricBase
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!GoalMetricBase.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!GoalMetricBase.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `GoalMetricBase` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
