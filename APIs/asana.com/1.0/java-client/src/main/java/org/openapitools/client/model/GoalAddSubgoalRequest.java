...
public class GoalAddSubgoalRequest {
   ...
   * @throws IOException if the JSON Element is invalid with respect to GoalAddSubgoalRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!GoalAddSubgoalRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in GoalAddSubgoalRequest is not found in the empty JSON string", GoalAddSubgoalRequest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!GoalAddSubgoalRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `GoalAddSubgoalRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : GoalAddSubgoalRequest.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!GoalAddSubgoalRequest.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'GoalAddSubgoalRequest' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<GoalAddSubgoalRequest> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(GoalAddSubgoalRequest.class));

       return (TypeAdapter<T>) new TypeAdapter<GoalAddSubgoalRequest>() {
           @Override
           public void write(JsonWriter out, GoalAddSubgoalRequest value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public GoalAddSubgoalRequest read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of GoalAddSubgoalRequest given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of GoalAddSubgoalRequest
   * @throws IOException if the JSON string is invalid with respect to GoalAddSubgoalRequest
   */
  public static GoalAddSubgoalRequest fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, GoalAddSubgoalRequest.class);
  }

  /**
   * Convert an instance of GoalAddSubgoalRequest to an JSON string
   *
   * @return JSON string
   */
