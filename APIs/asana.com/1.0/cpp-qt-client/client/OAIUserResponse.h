/**
 * Asana
 * This is the interface for interacting with the [Asana Platform](https://developers.asana.com). Our API reference is generated from our [OpenAPI spec] (https://raw.githubusercontent.com/Asana/developer-docs/master/defs/asana_oas.yaml).
 *
 * The version of the OpenAPI document: 1.0
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIUserResponse.h
 *
 * 
 */

#ifndef OAIUserResponse_H
#define OAIUserResponse_H

#include <QJsonObject>

#include "OAIUserBaseResponse_allOf_photo.h"
#include "OAIWorkspaceCompact.h"
#include <QList>
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {
class OAIUserBaseResponse_allOf_photo;
class OAIWorkspaceCompact;

class OAIUserResponse : public OAIObject {
public:
    OAIUserResponse();
    OAIUserResponse(QString json);
    ~OAIUserResponse() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QString getGid() const;
    void setGid(const QString &gid);
    bool is_gid_Set() const;
    bool is_gid_Valid() const;

    QString getResourceType() const;
    void setResourceType(const QString &resource_type);
    bool is_resource_type_Set() const;
    bool is_resource_type_Valid() const;

    QString getName() const;
    void setName(const QString &name);
    bool is_name_Set() const;
    bool is_name_Valid() const;

    QString getEmail() const;
    void setEmail(const QString &email);
    bool is_email_Set() const;
    bool is_email_Valid() const;

    OAIUserBaseResponse_allOf_photo getPhoto() const;
    void setPhoto(const OAIUserBaseResponse_allOf_photo &photo);
    bool is_photo_Set() const;
    bool is_photo_Valid() const;

    QList<OAIWorkspaceCompact> getWorkspaces() const;
    void setWorkspaces(const QList<OAIWorkspaceCompact> &workspaces);
    bool is_workspaces_Set() const;
    bool is_workspaces_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString m_gid;
    bool m_gid_isSet;
    bool m_gid_isValid;

    QString m_resource_type;
    bool m_resource_type_isSet;
    bool m_resource_type_isValid;

    QString m_name;
    bool m_name_isSet;
    bool m_name_isValid;

    QString m_email;
    bool m_email_isSet;
    bool m_email_isValid;

    OAIUserBaseResponse_allOf_photo m_photo;
    bool m_photo_isSet;
    bool m_photo_isValid;

    QList<OAIWorkspaceCompact> m_workspaces;
    bool m_workspaces_isSet;
    bool m_workspaces_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIUserResponse)

#endif // OAIUserResponse_H
