/**
 * Asana
 * This is the interface for interacting with the [Asana Platform](https://developers.asana.com). Our API reference is generated from our [OpenAPI spec] (https://raw.githubusercontent.com/Asana/developer-docs/master/defs/asana_oas.yaml).
 *
 * The version of the OpenAPI document: 1.0
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAITeamMembershipCompact.h
 *
 * 
 */

#ifndef OAITeamMembershipCompact_H
#define OAITeamMembershipCompact_H

#include <QJsonObject>

#include "OAITeamCompact.h"
#include "OAIUserCompact.h"
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {
class OAITeamCompact;
class OAIUserCompact;

class OAITeamMembershipCompact : public OAIObject {
public:
    OAITeamMembershipCompact();
    OAITeamMembershipCompact(QString json);
    ~OAITeamMembershipCompact() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QString getGid() const;
    void setGid(const QString &gid);
    bool is_gid_Set() const;
    bool is_gid_Valid() const;

    QString getResourceType() const;
    void setResourceType(const QString &resource_type);
    bool is_resource_type_Set() const;
    bool is_resource_type_Valid() const;

    bool isIsGuest() const;
    void setIsGuest(const bool &is_guest);
    bool is_is_guest_Set() const;
    bool is_is_guest_Valid() const;

    OAITeamCompact getTeam() const;
    void setTeam(const OAITeamCompact &team);
    bool is_team_Set() const;
    bool is_team_Valid() const;

    OAIUserCompact getUser() const;
    void setUser(const OAIUserCompact &user);
    bool is_user_Set() const;
    bool is_user_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString m_gid;
    bool m_gid_isSet;
    bool m_gid_isValid;

    QString m_resource_type;
    bool m_resource_type_isSet;
    bool m_resource_type_isValid;

    bool m_is_guest;
    bool m_is_guest_isSet;
    bool m_is_guest_isValid;

    OAITeamCompact m_team;
    bool m_team_isSet;
    bool m_team_isValid;

    OAIUserCompact m_user;
    bool m_user_isSet;
    bool m_user_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAITeamMembershipCompact)

#endif // OAITeamMembershipCompact_H
