# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.custom_field_setting_response import CustomFieldSettingResponse
from openapi_server.models.project_base_all_of_workspace import ProjectBaseAllOfWorkspace
from openapi_server.models.project_status_response import ProjectStatusResponse
from openapi_server.models.status_update_compact import StatusUpdateCompact
from openapi_server.models.user_compact import UserCompact
from openapi_server import util


class ProjectBase(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, gid: str=None, resource_type: str=None, name: str=None, archived: bool=None, color: str=None, created_at: datetime=None, current_status: ProjectStatusResponse=None, current_status_update: StatusUpdateCompact=None, custom_field_settings: List[CustomFieldSettingResponse]=None, default_view: str=None, due_date: datetime=None, due_on: datetime=None, html_notes: str=None, is_template: bool=None, members: List[UserCompact]=None, modified_at: datetime=None, notes: str=None, public: bool=None, start_on: date=None, workspace: ProjectBaseAllOfWorkspace=None):
        """ProjectBase - a model defined in OpenAPI

        :param gid: The gid of this ProjectBase.
        :param resource_type: The resource_type of this ProjectBase.
        :param name: The name of this ProjectBase.
        :param archived: The archived of this ProjectBase.
        :param color: The color of this ProjectBase.
        :param created_at: The created_at of this ProjectBase.
        :param current_status: The current_status of this ProjectBase.
        :param current_status_update: The current_status_update of this ProjectBase.
        :param custom_field_settings: The custom_field_settings of this ProjectBase.
        :param default_view: The default_view of this ProjectBase.
        :param due_date: The due_date of this ProjectBase.
        :param due_on: The due_on of this ProjectBase.
        :param html_notes: The html_notes of this ProjectBase.
        :param is_template: The is_template of this ProjectBase.
        :param members: The members of this ProjectBase.
        :param modified_at: The modified_at of this ProjectBase.
        :param notes: The notes of this ProjectBase.
        :param public: The public of this ProjectBase.
        :param start_on: The start_on of this ProjectBase.
        :param workspace: The workspace of this ProjectBase.
        """
        self.openapi_types = {
            'gid': str,
            'resource_type': str,
            'name': str,
            'archived': bool,
            'color': str,
            'created_at': datetime,
            'current_status': ProjectStatusResponse,
            'current_status_update': StatusUpdateCompact,
            'custom_field_settings': List[CustomFieldSettingResponse],
            'default_view': str,
            'due_date': datetime,
            'due_on': datetime,
            'html_notes': str,
            'is_template': bool,
            'members': List[UserCompact],
            'modified_at': datetime,
            'notes': str,
            'public': bool,
            'start_on': date,
            'workspace': ProjectBaseAllOfWorkspace
        }

        self.attribute_map = {
            'gid': 'gid',
            'resource_type': 'resource_type',
            'name': 'name',
            'archived': 'archived',
            'color': 'color',
            'created_at': 'created_at',
            'current_status': 'current_status',
            'current_status_update': 'current_status_update',
            'custom_field_settings': 'custom_field_settings',
            'default_view': 'default_view',
            'due_date': 'due_date',
            'due_on': 'due_on',
            'html_notes': 'html_notes',
            'is_template': 'is_template',
            'members': 'members',
            'modified_at': 'modified_at',
            'notes': 'notes',
            'public': 'public',
            'start_on': 'start_on',
            'workspace': 'workspace'
        }

        self._gid = gid
        self._resource_type = resource_type
        self._name = name
        self._archived = archived
        self._color = color
        self._created_at = created_at
        self._current_status = current_status
        self._current_status_update = current_status_update
        self._custom_field_settings = custom_field_settings
        self._default_view = default_view
        self._due_date = due_date
        self._due_on = due_on
        self._html_notes = html_notes
        self._is_template = is_template
        self._members = members
        self._modified_at = modified_at
        self._notes = notes
        self._public = public
        self._start_on = start_on
        self._workspace = workspace

    @classmethod
    def from_dict(cls, dikt: dict) -> 'ProjectBase':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The ProjectBase of this ProjectBase.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def gid(self):
        """Gets the gid of this ProjectBase.

        Globally unique identifier of the resource, as a string.

        :return: The gid of this ProjectBase.
        :rtype: str
        """
        return self._gid

    @gid.setter
    def gid(self, gid):
        """Sets the gid of this ProjectBase.

        Globally unique identifier of the resource, as a string.

        :param gid: The gid of this ProjectBase.
        :type gid: str
        """

        self._gid = gid

    @property
    def resource_type(self):
        """Gets the resource_type of this ProjectBase.

        The base type of this resource.

        :return: The resource_type of this ProjectBase.
        :rtype: str
        """
        return self._resource_type

    @resource_type.setter
    def resource_type(self, resource_type):
        """Sets the resource_type of this ProjectBase.

        The base type of this resource.

        :param resource_type: The resource_type of this ProjectBase.
        :type resource_type: str
        """

        self._resource_type = resource_type

    @property
    def name(self):
        """Gets the name of this ProjectBase.

        Name of the project. This is generally a short sentence fragment that fits on a line in the UI for maximum readability. However, it can be longer.

        :return: The name of this ProjectBase.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name):
        """Sets the name of this ProjectBase.

        Name of the project. This is generally a short sentence fragment that fits on a line in the UI for maximum readability. However, it can be longer.

        :param name: The name of this ProjectBase.
        :type name: str
        """

        self._name = name

    @property
    def archived(self):
        """Gets the archived of this ProjectBase.

        True if the project is archived, false if not. Archived projects do not show in the UI by default and may be treated differently for queries.

        :return: The archived of this ProjectBase.
        :rtype: bool
        """
        return self._archived

    @archived.setter
    def archived(self, archived):
        """Sets the archived of this ProjectBase.

        True if the project is archived, false if not. Archived projects do not show in the UI by default and may be treated differently for queries.

        :param archived: The archived of this ProjectBase.
        :type archived: bool
        """

        self._archived = archived

    @property
    def color(self):
        """Gets the color of this ProjectBase.

        Color of the project.

        :return: The color of this ProjectBase.
        :rtype: str
        """
        return self._color

    @color.setter
    def color(self, color):
        """Sets the color of this ProjectBase.

        Color of the project.

        :param color: The color of this ProjectBase.
        :type color: str
        """
        allowed_values = [None,"dark-pink", "dark-green", "dark-blue", "dark-red", "dark-teal", "dark-brown", "dark-orange", "dark-purple", "dark-warm-gray", "light-pink", "light-green", "light-blue", "light-red", "light-teal", "light-brown", "light-orange", "light-purple", "light-warm-gray"]  # noqa: E501
        if color not in allowed_values:
            raise ValueError(
                "Invalid value for `color` ({0}), must be one of {1}"
                .format(color, allowed_values)
            )

        self._color = color

    @property
    def created_at(self):
        """Gets the created_at of this ProjectBase.

        The time at which this resource was created.

        :return: The created_at of this ProjectBase.
        :rtype: datetime
        """
        return self._created_at

    @created_at.setter
    def created_at(self, created_at):
        """Sets the created_at of this ProjectBase.

        The time at which this resource was created.

        :param created_at: The created_at of this ProjectBase.
        :type created_at: datetime
        """

        self._created_at = created_at

    @property
    def current_status(self):
        """Gets the current_status of this ProjectBase.

        *Deprecated: new integrations should prefer the `current_status_update` resource.*

        :return: The current_status of this ProjectBase.
        :rtype: ProjectStatusResponse
        """
        return self._current_status

    @current_status.setter
    def current_status(self, current_status):
        """Sets the current_status of this ProjectBase.

        *Deprecated: new integrations should prefer the `current_status_update` resource.*

        :param current_status: The current_status of this ProjectBase.
        :type current_status: ProjectStatusResponse
        """

        self._current_status = current_status

    @property
    def current_status_update(self):
        """Gets the current_status_update of this ProjectBase.

        The latest `status_update` posted to this project.

        :return: The current_status_update of this ProjectBase.
        :rtype: StatusUpdateCompact
        """
        return self._current_status_update

    @current_status_update.setter
    def current_status_update(self, current_status_update):
        """Sets the current_status_update of this ProjectBase.

        The latest `status_update` posted to this project.

        :param current_status_update: The current_status_update of this ProjectBase.
        :type current_status_update: StatusUpdateCompact
        """

        self._current_status_update = current_status_update

    @property
    def custom_field_settings(self):
        """Gets the custom_field_settings of this ProjectBase.

        Array of Custom Field Settings (in compact form).

        :return: The custom_field_settings of this ProjectBase.
        :rtype: List[CustomFieldSettingResponse]
        """
        return self._custom_field_settings

    @custom_field_settings.setter
    def custom_field_settings(self, custom_field_settings):
        """Sets the custom_field_settings of this ProjectBase.

        Array of Custom Field Settings (in compact form).

        :param custom_field_settings: The custom_field_settings of this ProjectBase.
        :type custom_field_settings: List[CustomFieldSettingResponse]
        """

        self._custom_field_settings = custom_field_settings

    @property
    def default_view(self):
        """Gets the default_view of this ProjectBase.

        The default view (list, board, calendar, or timeline) of a project.

        :return: The default_view of this ProjectBase.
        :rtype: str
        """
        return self._default_view

    @default_view.setter
    def default_view(self, default_view):
        """Sets the default_view of this ProjectBase.

        The default view (list, board, calendar, or timeline) of a project.

        :param default_view: The default_view of this ProjectBase.
        :type default_view: str
        """
        allowed_values = ["list", "board", "calendar", "timeline"]  # noqa: E501
        if default_view not in allowed_values:
            raise ValueError(
                "Invalid value for `default_view` ({0}), must be one of {1}"
                .format(default_view, allowed_values)
            )

        self._default_view = default_view

    @property
    def due_date(self):
        """Gets the due_date of this ProjectBase.

        *Deprecated: new integrations should prefer the `due_on` field.*

        :return: The due_date of this ProjectBase.
        :rtype: datetime
        """
        return self._due_date

    @due_date.setter
    def due_date(self, due_date):
        """Sets the due_date of this ProjectBase.

        *Deprecated: new integrations should prefer the `due_on` field.*

        :param due_date: The due_date of this ProjectBase.
        :type due_date: datetime
        """

        self._due_date = due_date

    @property
    def due_on(self):
        """Gets the due_on of this ProjectBase.

        The day on which this project is due. This takes a date with format YYYY-MM-DD.

        :return: The due_on of this ProjectBase.
        :rtype: datetime
        """
        return self._due_on

    @due_on.setter
    def due_on(self, due_on):
        """Sets the due_on of this ProjectBase.

        The day on which this project is due. This takes a date with format YYYY-MM-DD.

        :param due_on: The due_on of this ProjectBase.
        :type due_on: datetime
        """

        self._due_on = due_on

    @property
    def html_notes(self):
        """Gets the html_notes of this ProjectBase.

        [Opt In](/docs/input-output-options). The notes of the project with formatting as HTML.

        :return: The html_notes of this ProjectBase.
        :rtype: str
        """
        return self._html_notes

    @html_notes.setter
    def html_notes(self, html_notes):
        """Sets the html_notes of this ProjectBase.

        [Opt In](/docs/input-output-options). The notes of the project with formatting as HTML.

        :param html_notes: The html_notes of this ProjectBase.
        :type html_notes: str
        """

        self._html_notes = html_notes

    @property
    def is_template(self):
        """Gets the is_template of this ProjectBase.

        [Opt In](/docs/input-output-options). *Deprecated - please use a project template endpoint instead (more in [this forum post](https://forum.asana.com/t/a-new-api-for-project-templates/156432)).* Determines if the project is a template.

        :return: The is_template of this ProjectBase.
        :rtype: bool
        """
        return self._is_template

    @is_template.setter
    def is_template(self, is_template):
        """Sets the is_template of this ProjectBase.

        [Opt In](/docs/input-output-options). *Deprecated - please use a project template endpoint instead (more in [this forum post](https://forum.asana.com/t/a-new-api-for-project-templates/156432)).* Determines if the project is a template.

        :param is_template: The is_template of this ProjectBase.
        :type is_template: bool
        """

        self._is_template = is_template

    @property
    def members(self):
        """Gets the members of this ProjectBase.

        Array of users who are members of this project.

        :return: The members of this ProjectBase.
        :rtype: List[UserCompact]
        """
        return self._members

    @members.setter
    def members(self, members):
        """Sets the members of this ProjectBase.

        Array of users who are members of this project.

        :param members: The members of this ProjectBase.
        :type members: List[UserCompact]
        """

        self._members = members

    @property
    def modified_at(self):
        """Gets the modified_at of this ProjectBase.

        The time at which this project was last modified. *Note: This does not currently reflect any changes in associations such as tasks or comments that may have been added or removed from the project.*

        :return: The modified_at of this ProjectBase.
        :rtype: datetime
        """
        return self._modified_at

    @modified_at.setter
    def modified_at(self, modified_at):
        """Sets the modified_at of this ProjectBase.

        The time at which this project was last modified. *Note: This does not currently reflect any changes in associations such as tasks or comments that may have been added or removed from the project.*

        :param modified_at: The modified_at of this ProjectBase.
        :type modified_at: datetime
        """

        self._modified_at = modified_at

    @property
    def notes(self):
        """Gets the notes of this ProjectBase.

        Free-form textual information associated with the project (ie., its description).

        :return: The notes of this ProjectBase.
        :rtype: str
        """
        return self._notes

    @notes.setter
    def notes(self, notes):
        """Sets the notes of this ProjectBase.

        Free-form textual information associated with the project (ie., its description).

        :param notes: The notes of this ProjectBase.
        :type notes: str
        """

        self._notes = notes

    @property
    def public(self):
        """Gets the public of this ProjectBase.

        True if the project is public to its team.

        :return: The public of this ProjectBase.
        :rtype: bool
        """
        return self._public

    @public.setter
    def public(self, public):
        """Sets the public of this ProjectBase.

        True if the project is public to its team.

        :param public: The public of this ProjectBase.
        :type public: bool
        """

        self._public = public

    @property
    def start_on(self):
        """Gets the start_on of this ProjectBase.

        The day on which work for this project begins, or null if the project has no start date. This takes a date with `YYYY-MM-DD` format. *Note: `due_on` or `due_at` must be present in the request when setting or unsetting the `start_on` parameter. Additionally, `start_on` and `due_on` cannot be the same date.*

        :return: The start_on of this ProjectBase.
        :rtype: date
        """
        return self._start_on

    @start_on.setter
    def start_on(self, start_on):
        """Sets the start_on of this ProjectBase.

        The day on which work for this project begins, or null if the project has no start date. This takes a date with `YYYY-MM-DD` format. *Note: `due_on` or `due_at` must be present in the request when setting or unsetting the `start_on` parameter. Additionally, `start_on` and `due_on` cannot be the same date.*

        :param start_on: The start_on of this ProjectBase.
        :type start_on: date
        """

        self._start_on = start_on

    @property
    def workspace(self):
        """Gets the workspace of this ProjectBase.


        :return: The workspace of this ProjectBase.
        :rtype: ProjectBaseAllOfWorkspace
        """
        return self._workspace

    @workspace.setter
    def workspace(self, workspace):
        """Sets the workspace of this ProjectBase.


        :param workspace: The workspace of this ProjectBase.
        :type workspace: ProjectBaseAllOfWorkspace
        """

        self._workspace = workspace
