...
public class SZentraleRef {
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(this.name, szentraleRef.name) &&
        Objects.equals(this.number, szentraleRef.number) &&
        Objects.equals(this.publicPhoneNumber, szentraleRef.publicPhoneNumber);
  }

  @Override
  public int hashCode() {
    return Objects.hash(name, number, publicPhoneNumber);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to SZentraleRef
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SZentraleRef.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in SZentraleRef is not found in the empty JSON string", SZentraleRef.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SZentraleRef.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SZentraleRef` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!SZentraleRef.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'SZentraleRef' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<SZentraleRef> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(SZentraleRef.class));

       return (TypeAdapter<T>) new TypeAdapter<SZentraleRef>() {
           @Override
           public void write(JsonWriter out, SZentraleRef value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public SZentraleRef read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of SZentraleRef given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of SZentraleRef
   * @throws IOException if the JSON string is invalid with respect to SZentraleRef
   */
  public static SZentraleRef fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, SZentraleRef.class);
  }

  /**
   * Convert an instance of SZentraleRef to an JSON string
   *
   * @return JSON string
   */
