# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
import re
from openapi_server import util


class ElectricityFunctionality(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, _class: str=None, id: str=None):
        """ElectricityFunctionality - a model defined in OpenAPI

        :param _class: The _class of this ElectricityFunctionality.
        :param id: The id of this ElectricityFunctionality.
        """
        self.openapi_types = {
            '_class': str,
            'id': str
        }

        self.attribute_map = {
            '_class': 'class',
            'id': 'id'
        }

        self.__class = _class
        self._id = id

    @classmethod
    def from_dict(cls, dikt: dict) -> 'ElectricityFunctionality':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The ElectricityFunctionality of this ElectricityFunctionality.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def _class(self):
        """Gets the _class of this ElectricityFunctionality.


        :return: The _class of this ElectricityFunctionality.
        :rtype: str
        """
        return self.__class

    @_class.setter
    def _class(self, _class):
        """Sets the _class of this ElectricityFunctionality.


        :param _class: The _class of this ElectricityFunctionality.
        :type _class: str
        """
        if _class is None:
            raise ValueError("Invalid value for `_class`, must not be `None`")
        if _class is not None and len(_class) < 2:
            raise ValueError("Invalid value for `_class`, length must be greater than or equal to `2`")
        if _class is not None and not re.search(r'^[A-Z][A-Za-z0-9]+$', _class):
            raise ValueError("Invalid value for `_class`, must be a follow pattern or equal to `/^[A-Z][A-Za-z0-9]+$/`")

        self.__class = _class

    @property
    def id(self):
        """Gets the id of this ElectricityFunctionality.

        Unique identifier of the *Functionality*

        :return: The id of this ElectricityFunctionality.
        :rtype: str
        """
        return self._id

    @id.setter
    def id(self, id):
        """Sets the id of this ElectricityFunctionality.

        Unique identifier of the *Functionality*

        :param id: The id of this ElectricityFunctionality.
        :type id: str
        """
        if id is None:
            raise ValueError("Invalid value for `id`, must not be `None`")
        if id is not None and len(id) > 22:
            raise ValueError("Invalid value for `id`, length must be less than or equal to `22`")
        if id is not None and len(id) < 22:
            raise ValueError("Invalid value for `id`, length must be greater than or equal to `22`")
        if id is not None and not re.search(r'^[A-Za-z0-9-_]{22}$', id):
            raise ValueError("Invalid value for `id`, must be a follow pattern or equal to `/^[A-Za-z0-9-_]{22}$/`")

        self._id = id
