/**
 * ExaVault
 * ExaVaults API allows you to incorporate ExaVaults suite of file transfer and user management tools into your own application.\\nExaVault supports both POST (recommended when requesting large data sets) and GET operations, and requires an API key in order to use.
 *
 * The version of the OpenAPI document: 2.0
 * Contact: support@exavault.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';

/**
 * The AddEmailListRequestBody model module.
 * @module model/AddEmailListRequestBody
 * @version 2.0
 */
class AddEmailListRequestBody {
    /**
     * Constructs a new <code>AddEmailListRequestBody</code>.
     * @alias module:model/AddEmailListRequestBody
     * @param emails {Array.<String>} Array of email addresses to include in the email list. 
     * @param name {String} Name of the email list. 
     */
    constructor(emails, name) { 
        
        AddEmailListRequestBody.initialize(this, emails, name);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj, emails, name) { 
        obj['emails'] = emails;
        obj['name'] = name;
    }

    /**
     * Constructs a <code>AddEmailListRequestBody</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/AddEmailListRequestBody} obj Optional instance to populate.
     * @return {module:model/AddEmailListRequestBody} The populated <code>AddEmailListRequestBody</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new AddEmailListRequestBody();

            if (data.hasOwnProperty('emails')) {
                obj['emails'] = ApiClient.convertToType(data['emails'], ['String']);
            }
            if (data.hasOwnProperty('name')) {
                obj['name'] = ApiClient.convertToType(data['name'], 'String');
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>AddEmailListRequestBody</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>AddEmailListRequestBody</code>.
     */
    static validateJSON(data) {
        // check to make sure all required properties are present in the JSON string
        for (const property of AddEmailListRequestBody.RequiredProperties) {
            if (!data.hasOwnProperty(property)) {
                throw new Error("The required field `" + property + "` is not found in the JSON data: " + JSON.stringify(data));
            }
        }
        // ensure the json data is an array
        if (!Array.isArray(data['emails'])) {
            throw new Error("Expected the field `emails` to be an array in the JSON data but got " + data['emails']);
        }
        // ensure the json data is a string
        if (data['name'] && !(typeof data['name'] === 'string' || data['name'] instanceof String)) {
            throw new Error("Expected the field `name` to be a primitive type in the JSON string but got " + data['name']);
        }

        return true;
    }


}

AddEmailListRequestBody.RequiredProperties = ["emails", "name"];

/**
 * Array of email addresses to include in the email list. 
 * @member {Array.<String>} emails
 */
AddEmailListRequestBody.prototype['emails'] = undefined;

/**
 * Name of the email list. 
 * @member {String} name
 */
AddEmailListRequestBody.prototype['name'] = undefined;






export default AddEmailListRequestBody;

