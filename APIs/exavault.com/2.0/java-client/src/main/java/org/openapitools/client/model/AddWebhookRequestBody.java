...
public class AddWebhookRequestBody {
  ...
  public AddWebhookRequestBody() {
  }

  public AddWebhookRequestBody endpointUrl(URI endpointUrl) {
  ...
  static {
    ...
    openapiRequiredFields.add("endpointUrl");
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to AddWebhookRequestBody
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AddWebhookRequestBody.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in AddWebhookRequestBody is not found in the empty JSON string", AddWebhookRequestBody.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!AddWebhookRequestBody.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AddWebhookRequestBody` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : AddWebhookRequestBody.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!AddWebhookRequestBody.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'AddWebhookRequestBody' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<AddWebhookRequestBody> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(AddWebhookRequestBody.class));

       return (TypeAdapter<T>) new TypeAdapter<AddWebhookRequestBody>() {
           @Override
           public void write(JsonWriter out, AddWebhookRequestBody value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public AddWebhookRequestBody read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of AddWebhookRequestBody given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of AddWebhookRequestBody
   * @throws IOException if the JSON string is invalid with respect to AddWebhookRequestBody
   */
  public static AddWebhookRequestBody fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, AddWebhookRequestBody.class);
  }

  /**
   * Convert an instance of AddWebhookRequestBody to an JSON string
   *
   * @return JSON string
   */
