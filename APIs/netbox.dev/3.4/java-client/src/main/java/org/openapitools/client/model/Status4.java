...
public class Status4 {
   ...
   * @throws IOException if the JSON Element is invalid with respect to Status4
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Status4.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in Status4 is not found in the empty JSON string", Status4.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Status4.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Status4` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : Status4.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!Status4.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'Status4' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<Status4> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(Status4.class));

       return (TypeAdapter<T>) new TypeAdapter<Status4>() {
           @Override
           public void write(JsonWriter out, Status4 value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public Status4 read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of Status4 given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of Status4
   * @throws IOException if the JSON string is invalid with respect to Status4
   */
  public static Status4 fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, Status4.class);
  }

  /**
   * Convert an instance of Status4 to an JSON string
   *
   * @return JSON string
   */
