# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.airflow import Airflow
from openapi_server.models.face import Face
from openapi_server.models.nested_cluster import NestedCluster
from openapi_server.models.nested_device import NestedDevice
from openapi_server.models.nested_device_role import NestedDeviceRole
from openapi_server.models.nested_device_type import NestedDeviceType
from openapi_server.models.nested_ip_address import NestedIPAddress
from openapi_server.models.nested_location import NestedLocation
from openapi_server.models.nested_platform import NestedPlatform
from openapi_server.models.nested_rack import NestedRack
from openapi_server.models.nested_site import NestedSite
from openapi_server.models.nested_tag import NestedTag
from openapi_server.models.nested_tenant import NestedTenant
from openapi_server.models.nested_virtual_chassis import NestedVirtualChassis
from openapi_server.models.status3 import Status3
from openapi_server import util


class Device(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, airflow: Airflow=None, asset_tag: str=None, cluster: NestedCluster=None, comments: str=None, created: datetime=None, custom_fields: object=None, description: str=None, device_role: NestedDeviceRole=None, device_type: NestedDeviceType=None, display: str=None, face: Face=None, id: int=None, last_updated: datetime=None, local_context_data: object=None, location: NestedLocation=None, name: str=None, parent_device: NestedDevice=None, platform: NestedPlatform=None, position: float=None, primary_ip: NestedIPAddress=None, primary_ip4: NestedIPAddress=None, primary_ip6: NestedIPAddress=None, rack: NestedRack=None, serial: str=None, site: NestedSite=None, status: Status3=None, tags: List[NestedTag]=None, tenant: NestedTenant=None, url: str=None, vc_position: int=None, vc_priority: int=None, virtual_chassis: NestedVirtualChassis=None):
        """Device - a model defined in OpenAPI

        :param airflow: The airflow of this Device.
        :param asset_tag: The asset_tag of this Device.
        :param cluster: The cluster of this Device.
        :param comments: The comments of this Device.
        :param created: The created of this Device.
        :param custom_fields: The custom_fields of this Device.
        :param description: The description of this Device.
        :param device_role: The device_role of this Device.
        :param device_type: The device_type of this Device.
        :param display: The display of this Device.
        :param face: The face of this Device.
        :param id: The id of this Device.
        :param last_updated: The last_updated of this Device.
        :param local_context_data: The local_context_data of this Device.
        :param location: The location of this Device.
        :param name: The name of this Device.
        :param parent_device: The parent_device of this Device.
        :param platform: The platform of this Device.
        :param position: The position of this Device.
        :param primary_ip: The primary_ip of this Device.
        :param primary_ip4: The primary_ip4 of this Device.
        :param primary_ip6: The primary_ip6 of this Device.
        :param rack: The rack of this Device.
        :param serial: The serial of this Device.
        :param site: The site of this Device.
        :param status: The status of this Device.
        :param tags: The tags of this Device.
        :param tenant: The tenant of this Device.
        :param url: The url of this Device.
        :param vc_position: The vc_position of this Device.
        :param vc_priority: The vc_priority of this Device.
        :param virtual_chassis: The virtual_chassis of this Device.
        """
        self.openapi_types = {
            'airflow': Airflow,
            'asset_tag': str,
            'cluster': NestedCluster,
            'comments': str,
            'created': datetime,
            'custom_fields': object,
            'description': str,
            'device_role': NestedDeviceRole,
            'device_type': NestedDeviceType,
            'display': str,
            'face': Face,
            'id': int,
            'last_updated': datetime,
            'local_context_data': object,
            'location': NestedLocation,
            'name': str,
            'parent_device': NestedDevice,
            'platform': NestedPlatform,
            'position': float,
            'primary_ip': NestedIPAddress,
            'primary_ip4': NestedIPAddress,
            'primary_ip6': NestedIPAddress,
            'rack': NestedRack,
            'serial': str,
            'site': NestedSite,
            'status': Status3,
            'tags': List[NestedTag],
            'tenant': NestedTenant,
            'url': str,
            'vc_position': int,
            'vc_priority': int,
            'virtual_chassis': NestedVirtualChassis
        }

        self.attribute_map = {
            'airflow': 'airflow',
            'asset_tag': 'asset_tag',
            'cluster': 'cluster',
            'comments': 'comments',
            'created': 'created',
            'custom_fields': 'custom_fields',
            'description': 'description',
            'device_role': 'device_role',
            'device_type': 'device_type',
            'display': 'display',
            'face': 'face',
            'id': 'id',
            'last_updated': 'last_updated',
            'local_context_data': 'local_context_data',
            'location': 'location',
            'name': 'name',
            'parent_device': 'parent_device',
            'platform': 'platform',
            'position': 'position',
            'primary_ip': 'primary_ip',
            'primary_ip4': 'primary_ip4',
            'primary_ip6': 'primary_ip6',
            'rack': 'rack',
            'serial': 'serial',
            'site': 'site',
            'status': 'status',
            'tags': 'tags',
            'tenant': 'tenant',
            'url': 'url',
            'vc_position': 'vc_position',
            'vc_priority': 'vc_priority',
            'virtual_chassis': 'virtual_chassis'
        }

        self._airflow = airflow
        self._asset_tag = asset_tag
        self._cluster = cluster
        self._comments = comments
        self._created = created
        self._custom_fields = custom_fields
        self._description = description
        self._device_role = device_role
        self._device_type = device_type
        self._display = display
        self._face = face
        self._id = id
        self._last_updated = last_updated
        self._local_context_data = local_context_data
        self._location = location
        self._name = name
        self._parent_device = parent_device
        self._platform = platform
        self._position = position
        self._primary_ip = primary_ip
        self._primary_ip4 = primary_ip4
        self._primary_ip6 = primary_ip6
        self._rack = rack
        self._serial = serial
        self._site = site
        self._status = status
        self._tags = tags
        self._tenant = tenant
        self._url = url
        self._vc_position = vc_position
        self._vc_priority = vc_priority
        self._virtual_chassis = virtual_chassis

    @classmethod
    def from_dict(cls, dikt: dict) -> 'Device':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The Device of this Device.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def airflow(self):
        """Gets the airflow of this Device.


        :return: The airflow of this Device.
        :rtype: Airflow
        """
        return self._airflow

    @airflow.setter
    def airflow(self, airflow):
        """Sets the airflow of this Device.


        :param airflow: The airflow of this Device.
        :type airflow: Airflow
        """

        self._airflow = airflow

    @property
    def asset_tag(self):
        """Gets the asset_tag of this Device.

        A unique tag used to identify this device

        :return: The asset_tag of this Device.
        :rtype: str
        """
        return self._asset_tag

    @asset_tag.setter
    def asset_tag(self, asset_tag):
        """Sets the asset_tag of this Device.

        A unique tag used to identify this device

        :param asset_tag: The asset_tag of this Device.
        :type asset_tag: str
        """
        if asset_tag is not None and len(asset_tag) > 50:
            raise ValueError("Invalid value for `asset_tag`, length must be less than or equal to `50`")

        self._asset_tag = asset_tag

    @property
    def cluster(self):
        """Gets the cluster of this Device.


        :return: The cluster of this Device.
        :rtype: NestedCluster
        """
        return self._cluster

    @cluster.setter
    def cluster(self, cluster):
        """Sets the cluster of this Device.


        :param cluster: The cluster of this Device.
        :type cluster: NestedCluster
        """

        self._cluster = cluster

    @property
    def comments(self):
        """Gets the comments of this Device.


        :return: The comments of this Device.
        :rtype: str
        """
        return self._comments

    @comments.setter
    def comments(self, comments):
        """Sets the comments of this Device.


        :param comments: The comments of this Device.
        :type comments: str
        """

        self._comments = comments

    @property
    def created(self):
        """Gets the created of this Device.


        :return: The created of this Device.
        :rtype: datetime
        """
        return self._created

    @created.setter
    def created(self, created):
        """Sets the created of this Device.


        :param created: The created of this Device.
        :type created: datetime
        """

        self._created = created

    @property
    def custom_fields(self):
        """Gets the custom_fields of this Device.


        :return: The custom_fields of this Device.
        :rtype: object
        """
        return self._custom_fields

    @custom_fields.setter
    def custom_fields(self, custom_fields):
        """Sets the custom_fields of this Device.


        :param custom_fields: The custom_fields of this Device.
        :type custom_fields: object
        """

        self._custom_fields = custom_fields

    @property
    def description(self):
        """Gets the description of this Device.


        :return: The description of this Device.
        :rtype: str
        """
        return self._description

    @description.setter
    def description(self, description):
        """Sets the description of this Device.


        :param description: The description of this Device.
        :type description: str
        """
        if description is not None and len(description) > 200:
            raise ValueError("Invalid value for `description`, length must be less than or equal to `200`")

        self._description = description

    @property
    def device_role(self):
        """Gets the device_role of this Device.


        :return: The device_role of this Device.
        :rtype: NestedDeviceRole
        """
        return self._device_role

    @device_role.setter
    def device_role(self, device_role):
        """Sets the device_role of this Device.


        :param device_role: The device_role of this Device.
        :type device_role: NestedDeviceRole
        """
        if device_role is None:
            raise ValueError("Invalid value for `device_role`, must not be `None`")

        self._device_role = device_role

    @property
    def device_type(self):
        """Gets the device_type of this Device.


        :return: The device_type of this Device.
        :rtype: NestedDeviceType
        """
        return self._device_type

    @device_type.setter
    def device_type(self, device_type):
        """Sets the device_type of this Device.


        :param device_type: The device_type of this Device.
        :type device_type: NestedDeviceType
        """
        if device_type is None:
            raise ValueError("Invalid value for `device_type`, must not be `None`")

        self._device_type = device_type

    @property
    def display(self):
        """Gets the display of this Device.


        :return: The display of this Device.
        :rtype: str
        """
        return self._display

    @display.setter
    def display(self, display):
        """Sets the display of this Device.


        :param display: The display of this Device.
        :type display: str
        """

        self._display = display

    @property
    def face(self):
        """Gets the face of this Device.


        :return: The face of this Device.
        :rtype: Face
        """
        return self._face

    @face.setter
    def face(self, face):
        """Sets the face of this Device.


        :param face: The face of this Device.
        :type face: Face
        """

        self._face = face

    @property
    def id(self):
        """Gets the id of this Device.


        :return: The id of this Device.
        :rtype: int
        """
        return self._id

    @id.setter
    def id(self, id):
        """Sets the id of this Device.


        :param id: The id of this Device.
        :type id: int
        """

        self._id = id

    @property
    def last_updated(self):
        """Gets the last_updated of this Device.


        :return: The last_updated of this Device.
        :rtype: datetime
        """
        return self._last_updated

    @last_updated.setter
    def last_updated(self, last_updated):
        """Sets the last_updated of this Device.


        :param last_updated: The last_updated of this Device.
        :type last_updated: datetime
        """

        self._last_updated = last_updated

    @property
    def local_context_data(self):
        """Gets the local_context_data of this Device.


        :return: The local_context_data of this Device.
        :rtype: object
        """
        return self._local_context_data

    @local_context_data.setter
    def local_context_data(self, local_context_data):
        """Sets the local_context_data of this Device.


        :param local_context_data: The local_context_data of this Device.
        :type local_context_data: object
        """

        self._local_context_data = local_context_data

    @property
    def location(self):
        """Gets the location of this Device.


        :return: The location of this Device.
        :rtype: NestedLocation
        """
        return self._location

    @location.setter
    def location(self, location):
        """Sets the location of this Device.


        :param location: The location of this Device.
        :type location: NestedLocation
        """

        self._location = location

    @property
    def name(self):
        """Gets the name of this Device.


        :return: The name of this Device.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name):
        """Sets the name of this Device.


        :param name: The name of this Device.
        :type name: str
        """
        if name is not None and len(name) > 64:
            raise ValueError("Invalid value for `name`, length must be less than or equal to `64`")

        self._name = name

    @property
    def parent_device(self):
        """Gets the parent_device of this Device.


        :return: The parent_device of this Device.
        :rtype: NestedDevice
        """
        return self._parent_device

    @parent_device.setter
    def parent_device(self, parent_device):
        """Sets the parent_device of this Device.


        :param parent_device: The parent_device of this Device.
        :type parent_device: NestedDevice
        """

        self._parent_device = parent_device

    @property
    def platform(self):
        """Gets the platform of this Device.


        :return: The platform of this Device.
        :rtype: NestedPlatform
        """
        return self._platform

    @platform.setter
    def platform(self, platform):
        """Sets the platform of this Device.


        :param platform: The platform of this Device.
        :type platform: NestedPlatform
        """

        self._platform = platform

    @property
    def position(self):
        """Gets the position of this Device.


        :return: The position of this Device.
        :rtype: float
        """
        return self._position

    @position.setter
    def position(self, position):
        """Sets the position of this Device.


        :param position: The position of this Device.
        :type position: float
        """
        if position is not None and position < 0.5:
            raise ValueError("Invalid value for `position`, must be a value greater than or equal to `0.5`")

        self._position = position

    @property
    def primary_ip(self):
        """Gets the primary_ip of this Device.


        :return: The primary_ip of this Device.
        :rtype: NestedIPAddress
        """
        return self._primary_ip

    @primary_ip.setter
    def primary_ip(self, primary_ip):
        """Sets the primary_ip of this Device.


        :param primary_ip: The primary_ip of this Device.
        :type primary_ip: NestedIPAddress
        """

        self._primary_ip = primary_ip

    @property
    def primary_ip4(self):
        """Gets the primary_ip4 of this Device.


        :return: The primary_ip4 of this Device.
        :rtype: NestedIPAddress
        """
        return self._primary_ip4

    @primary_ip4.setter
    def primary_ip4(self, primary_ip4):
        """Sets the primary_ip4 of this Device.


        :param primary_ip4: The primary_ip4 of this Device.
        :type primary_ip4: NestedIPAddress
        """

        self._primary_ip4 = primary_ip4

    @property
    def primary_ip6(self):
        """Gets the primary_ip6 of this Device.


        :return: The primary_ip6 of this Device.
        :rtype: NestedIPAddress
        """
        return self._primary_ip6

    @primary_ip6.setter
    def primary_ip6(self, primary_ip6):
        """Sets the primary_ip6 of this Device.


        :param primary_ip6: The primary_ip6 of this Device.
        :type primary_ip6: NestedIPAddress
        """

        self._primary_ip6 = primary_ip6

    @property
    def rack(self):
        """Gets the rack of this Device.


        :return: The rack of this Device.
        :rtype: NestedRack
        """
        return self._rack

    @rack.setter
    def rack(self, rack):
        """Sets the rack of this Device.


        :param rack: The rack of this Device.
        :type rack: NestedRack
        """

        self._rack = rack

    @property
    def serial(self):
        """Gets the serial of this Device.


        :return: The serial of this Device.
        :rtype: str
        """
        return self._serial

    @serial.setter
    def serial(self, serial):
        """Sets the serial of this Device.


        :param serial: The serial of this Device.
        :type serial: str
        """
        if serial is not None and len(serial) > 50:
            raise ValueError("Invalid value for `serial`, length must be less than or equal to `50`")

        self._serial = serial

    @property
    def site(self):
        """Gets the site of this Device.


        :return: The site of this Device.
        :rtype: NestedSite
        """
        return self._site

    @site.setter
    def site(self, site):
        """Sets the site of this Device.


        :param site: The site of this Device.
        :type site: NestedSite
        """
        if site is None:
            raise ValueError("Invalid value for `site`, must not be `None`")

        self._site = site

    @property
    def status(self):
        """Gets the status of this Device.


        :return: The status of this Device.
        :rtype: Status3
        """
        return self._status

    @status.setter
    def status(self, status):
        """Sets the status of this Device.


        :param status: The status of this Device.
        :type status: Status3
        """

        self._status = status

    @property
    def tags(self):
        """Gets the tags of this Device.


        :return: The tags of this Device.
        :rtype: List[NestedTag]
        """
        return self._tags

    @tags.setter
    def tags(self, tags):
        """Sets the tags of this Device.


        :param tags: The tags of this Device.
        :type tags: List[NestedTag]
        """

        self._tags = tags

    @property
    def tenant(self):
        """Gets the tenant of this Device.


        :return: The tenant of this Device.
        :rtype: NestedTenant
        """
        return self._tenant

    @tenant.setter
    def tenant(self, tenant):
        """Sets the tenant of this Device.


        :param tenant: The tenant of this Device.
        :type tenant: NestedTenant
        """

        self._tenant = tenant

    @property
    def url(self):
        """Gets the url of this Device.


        :return: The url of this Device.
        :rtype: str
        """
        return self._url

    @url.setter
    def url(self, url):
        """Sets the url of this Device.


        :param url: The url of this Device.
        :type url: str
        """

        self._url = url

    @property
    def vc_position(self):
        """Gets the vc_position of this Device.


        :return: The vc_position of this Device.
        :rtype: int
        """
        return self._vc_position

    @vc_position.setter
    def vc_position(self, vc_position):
        """Sets the vc_position of this Device.


        :param vc_position: The vc_position of this Device.
        :type vc_position: int
        """
        if vc_position is not None and vc_position > 255:
            raise ValueError("Invalid value for `vc_position`, must be a value less than or equal to `255`")
        if vc_position is not None and vc_position < 0:
            raise ValueError("Invalid value for `vc_position`, must be a value greater than or equal to `0`")

        self._vc_position = vc_position

    @property
    def vc_priority(self):
        """Gets the vc_priority of this Device.


        :return: The vc_priority of this Device.
        :rtype: int
        """
        return self._vc_priority

    @vc_priority.setter
    def vc_priority(self, vc_priority):
        """Sets the vc_priority of this Device.


        :param vc_priority: The vc_priority of this Device.
        :type vc_priority: int
        """
        if vc_priority is not None and vc_priority > 255:
            raise ValueError("Invalid value for `vc_priority`, must be a value less than or equal to `255`")
        if vc_priority is not None and vc_priority < 0:
            raise ValueError("Invalid value for `vc_priority`, must be a value greater than or equal to `0`")

        self._vc_priority = vc_priority

    @property
    def virtual_chassis(self):
        """Gets the virtual_chassis of this Device.


        :return: The virtual_chassis of this Device.
        :rtype: NestedVirtualChassis
        """
        return self._virtual_chassis

    @virtual_chassis.setter
    def virtual_chassis(self, virtual_chassis):
        """Sets the virtual_chassis of this Device.


        :param virtual_chassis: The virtual_chassis of this Device.
        :type virtual_chassis: NestedVirtualChassis
        """

        self._virtual_chassis = virtual_chassis
