# AvazaApiDocumentation.BillPaymentApi

All URIs are relative to *https://api.avaza.com*

Method | HTTP request | Description
------------- | ------------- | -------------
[**billPaymentGet**](BillPaymentApi.md#billPaymentGet) | **GET** /api/BillPayment | Gets list of Bill Payments
[**billPaymentGetByID**](BillPaymentApi.md#billPaymentGetByID) | **GET** /api/BillPayment/{id} | Gets a Bill Payment by Payment Transaction ID
[**billPaymentPost**](BillPaymentApi.md#billPaymentPost) | **POST** /api/BillPayment | Create new Bill Payment and optionally assign payment allocations to Bills



## billPaymentGet

> BillPaymentList billPaymentGet(opts)

Gets list of Bill Payments

### Example

```javascript
import AvazaApiDocumentation from 'avaza_api_documentation';
let defaultClient = AvazaApiDocumentation.ApiClient.instance;
// Configure OAuth2 access token for authorization: oauth2
let oauth2 = defaultClient.authentications['oauth2'];
oauth2.accessToken = 'YOUR ACCESS TOKEN';

let apiInstance = new AvazaApiDocumentation.BillPaymentApi();
let opts = {
  'updatedAfter': new Date("2013-10-20T19:20:30+01:00"), // Date | 
  'pageSize': 56, // Number | Number of items per page (max 1000)
  'pageNumber': 56 // Number | Page to display. Starts from 1.
};
apiInstance.billPaymentGet(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
  }
});
```

### Parameters


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **updatedAfter** | **Date**|  | [optional] 
 **pageSize** | **Number**| Number of items per page (max 1000) | [optional] 
 **pageNumber** | **Number**| Page to display. Starts from 1. | [optional] 

### Return type

[**BillPaymentList**](BillPaymentList.md)

### Authorization

[oauth2](../README.md#oauth2)

### HTTP request headers

- **Content-Type**: Not defined
- **Accept**: application/json, text/json, application/xml, text/xml


## billPaymentGetByID

> BillPayment billPaymentGetByID(id)

Gets a Bill Payment by Payment Transaction ID

### Example

```javascript
import AvazaApiDocumentation from 'avaza_api_documentation';
let defaultClient = AvazaApiDocumentation.ApiClient.instance;
// Configure OAuth2 access token for authorization: oauth2
let oauth2 = defaultClient.authentications['oauth2'];
oauth2.accessToken = 'YOUR ACCESS TOKEN';

let apiInstance = new AvazaApiDocumentation.BillPaymentApi();
let id = 789; // Number | Invoice Transaction ID Number
apiInstance.billPaymentGetByID(id, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
  }
});
```

### Parameters


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **id** | **Number**| Invoice Transaction ID Number | 

### Return type

[**BillPayment**](BillPayment.md)

### Authorization

[oauth2](../README.md#oauth2)

### HTTP request headers

- **Content-Type**: Not defined
- **Accept**: application/json, text/json, application/xml, text/xml


## billPaymentPost

> BillPayment billPaymentPost(model)

Create new Bill Payment and optionally assign payment allocations to Bills

### Example

```javascript
import AvazaApiDocumentation from 'avaza_api_documentation';
let defaultClient = AvazaApiDocumentation.ApiClient.instance;
// Configure OAuth2 access token for authorization: oauth2
let oauth2 = defaultClient.authentications['oauth2'];
oauth2.accessToken = 'YOUR ACCESS TOKEN';

let apiInstance = new AvazaApiDocumentation.BillPaymentApi();
let model = new AvazaApiDocumentation.NewBillPayment(); // NewBillPayment | 
apiInstance.billPaymentPost(model, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
  }
});
```

### Parameters


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **model** | [**NewBillPayment**](NewBillPayment.md)|  | 

### Return type

[**BillPayment**](BillPayment.md)

### Authorization

[oauth2](../README.md#oauth2)

### HTTP request headers

- **Content-Type**: application/json, text/json, application/xml, text/xml, application/x-www-form-urlencoded
- **Accept**: application/json, text/json, application/xml, text/xml

