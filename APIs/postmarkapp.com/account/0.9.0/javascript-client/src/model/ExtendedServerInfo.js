/**
 * Postmark Account-level API
 * Postmark makes sending and receiving email incredibly easy. The Account-level API allows users to configure all Servers, Domains, and Sender Signatures associated with an Account. 
 *
 * The version of the OpenAPI document: 0.9.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';

/**
 * The ExtendedServerInfo model module.
 * @module model/ExtendedServerInfo
 * @version 0.9.0
 */
class ExtendedServerInfo {
    /**
     * Constructs a new <code>ExtendedServerInfo</code>.
     * 
     * @alias module:model/ExtendedServerInfo
     */
    constructor() { 
        
        ExtendedServerInfo.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>ExtendedServerInfo</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/ExtendedServerInfo} obj Optional instance to populate.
     * @return {module:model/ExtendedServerInfo} The populated <code>ExtendedServerInfo</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new ExtendedServerInfo();

            if (data.hasOwnProperty('ApiTokens')) {
                obj['ApiTokens'] = ApiClient.convertToType(data['ApiTokens'], ['String']);
            }
            if (data.hasOwnProperty('BounceHookUrl')) {
                obj['BounceHookUrl'] = ApiClient.convertToType(data['BounceHookUrl'], 'String');
            }
            if (data.hasOwnProperty('ClickHookUrl')) {
                obj['ClickHookUrl'] = ApiClient.convertToType(data['ClickHookUrl'], 'String');
            }
            if (data.hasOwnProperty('Color')) {
                obj['Color'] = ApiClient.convertToType(data['Color'], 'String');
            }
            if (data.hasOwnProperty('DeliveryHookUrl')) {
                obj['DeliveryHookUrl'] = ApiClient.convertToType(data['DeliveryHookUrl'], 'String');
            }
            if (data.hasOwnProperty('ID')) {
                obj['ID'] = ApiClient.convertToType(data['ID'], 'Number');
            }
            if (data.hasOwnProperty('InboundAddress')) {
                obj['InboundAddress'] = ApiClient.convertToType(data['InboundAddress'], 'String');
            }
            if (data.hasOwnProperty('InboundDomain')) {
                obj['InboundDomain'] = ApiClient.convertToType(data['InboundDomain'], 'String');
            }
            if (data.hasOwnProperty('InboundHash')) {
                obj['InboundHash'] = ApiClient.convertToType(data['InboundHash'], 'String');
            }
            if (data.hasOwnProperty('InboundHookUrl')) {
                obj['InboundHookUrl'] = ApiClient.convertToType(data['InboundHookUrl'], 'String');
            }
            if (data.hasOwnProperty('InboundSpamThreshold')) {
                obj['InboundSpamThreshold'] = ApiClient.convertToType(data['InboundSpamThreshold'], 'Number');
            }
            if (data.hasOwnProperty('Name')) {
                obj['Name'] = ApiClient.convertToType(data['Name'], 'String');
            }
            if (data.hasOwnProperty('OpenHookUrl')) {
                obj['OpenHookUrl'] = ApiClient.convertToType(data['OpenHookUrl'], 'String');
            }
            if (data.hasOwnProperty('PostFirstOpenOnly')) {
                obj['PostFirstOpenOnly'] = ApiClient.convertToType(data['PostFirstOpenOnly'], 'Boolean');
            }
            if (data.hasOwnProperty('RawEmailEnabled')) {
                obj['RawEmailEnabled'] = ApiClient.convertToType(data['RawEmailEnabled'], 'Boolean');
            }
            if (data.hasOwnProperty('ServerLink')) {
                obj['ServerLink'] = ApiClient.convertToType(data['ServerLink'], 'String');
            }
            if (data.hasOwnProperty('SmtpApiActivated')) {
                obj['SmtpApiActivated'] = ApiClient.convertToType(data['SmtpApiActivated'], 'Boolean');
            }
            if (data.hasOwnProperty('TrackLinks')) {
                obj['TrackLinks'] = ApiClient.convertToType(data['TrackLinks'], 'String');
            }
            if (data.hasOwnProperty('TrackOpens')) {
                obj['TrackOpens'] = ApiClient.convertToType(data['TrackOpens'], 'Boolean');
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>ExtendedServerInfo</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>ExtendedServerInfo</code>.
     */
    static validateJSON(data) {
        // ensure the json data is an array
        if (!Array.isArray(data['ApiTokens'])) {
            throw new Error("Expected the field `ApiTokens` to be an array in the JSON data but got " + data['ApiTokens']);
        }
        // ensure the json data is a string
        if (data['BounceHookUrl'] && !(typeof data['BounceHookUrl'] === 'string' || data['BounceHookUrl'] instanceof String)) {
            throw new Error("Expected the field `BounceHookUrl` to be a primitive type in the JSON string but got " + data['BounceHookUrl']);
        }
        // ensure the json data is a string
        if (data['ClickHookUrl'] && !(typeof data['ClickHookUrl'] === 'string' || data['ClickHookUrl'] instanceof String)) {
            throw new Error("Expected the field `ClickHookUrl` to be a primitive type in the JSON string but got " + data['ClickHookUrl']);
        }
        // ensure the json data is a string
        if (data['Color'] && !(typeof data['Color'] === 'string' || data['Color'] instanceof String)) {
            throw new Error("Expected the field `Color` to be a primitive type in the JSON string but got " + data['Color']);
        }
        // ensure the json data is a string
        if (data['DeliveryHookUrl'] && !(typeof data['DeliveryHookUrl'] === 'string' || data['DeliveryHookUrl'] instanceof String)) {
            throw new Error("Expected the field `DeliveryHookUrl` to be a primitive type in the JSON string but got " + data['DeliveryHookUrl']);
        }
        // ensure the json data is a string
        if (data['InboundAddress'] && !(typeof data['InboundAddress'] === 'string' || data['InboundAddress'] instanceof String)) {
            throw new Error("Expected the field `InboundAddress` to be a primitive type in the JSON string but got " + data['InboundAddress']);
        }
        // ensure the json data is a string
        if (data['InboundDomain'] && !(typeof data['InboundDomain'] === 'string' || data['InboundDomain'] instanceof String)) {
            throw new Error("Expected the field `InboundDomain` to be a primitive type in the JSON string but got " + data['InboundDomain']);
        }
        // ensure the json data is a string
        if (data['InboundHash'] && !(typeof data['InboundHash'] === 'string' || data['InboundHash'] instanceof String)) {
            throw new Error("Expected the field `InboundHash` to be a primitive type in the JSON string but got " + data['InboundHash']);
        }
        // ensure the json data is a string
        if (data['InboundHookUrl'] && !(typeof data['InboundHookUrl'] === 'string' || data['InboundHookUrl'] instanceof String)) {
            throw new Error("Expected the field `InboundHookUrl` to be a primitive type in the JSON string but got " + data['InboundHookUrl']);
        }
        // ensure the json data is a string
        if (data['Name'] && !(typeof data['Name'] === 'string' || data['Name'] instanceof String)) {
            throw new Error("Expected the field `Name` to be a primitive type in the JSON string but got " + data['Name']);
        }
        // ensure the json data is a string
        if (data['OpenHookUrl'] && !(typeof data['OpenHookUrl'] === 'string' || data['OpenHookUrl'] instanceof String)) {
            throw new Error("Expected the field `OpenHookUrl` to be a primitive type in the JSON string but got " + data['OpenHookUrl']);
        }
        // ensure the json data is a string
        if (data['ServerLink'] && !(typeof data['ServerLink'] === 'string' || data['ServerLink'] instanceof String)) {
            throw new Error("Expected the field `ServerLink` to be a primitive type in the JSON string but got " + data['ServerLink']);
        }
        // ensure the json data is a string
        if (data['TrackLinks'] && !(typeof data['TrackLinks'] === 'string' || data['TrackLinks'] instanceof String)) {
            throw new Error("Expected the field `TrackLinks` to be a primitive type in the JSON string but got " + data['TrackLinks']);
        }

        return true;
    }


}



/**
 * @member {Array.<String>} ApiTokens
 */
ExtendedServerInfo.prototype['ApiTokens'] = undefined;

/**
 * @member {String} BounceHookUrl
 */
ExtendedServerInfo.prototype['BounceHookUrl'] = undefined;

/**
 * @member {String} ClickHookUrl
 */
ExtendedServerInfo.prototype['ClickHookUrl'] = undefined;

/**
 * @member {String} Color
 */
ExtendedServerInfo.prototype['Color'] = undefined;

/**
 * @member {String} DeliveryHookUrl
 */
ExtendedServerInfo.prototype['DeliveryHookUrl'] = undefined;

/**
 * @member {Number} ID
 */
ExtendedServerInfo.prototype['ID'] = undefined;

/**
 * @member {String} InboundAddress
 */
ExtendedServerInfo.prototype['InboundAddress'] = undefined;

/**
 * @member {String} InboundDomain
 */
ExtendedServerInfo.prototype['InboundDomain'] = undefined;

/**
 * @member {String} InboundHash
 */
ExtendedServerInfo.prototype['InboundHash'] = undefined;

/**
 * @member {String} InboundHookUrl
 */
ExtendedServerInfo.prototype['InboundHookUrl'] = undefined;

/**
 * @member {Number} InboundSpamThreshold
 */
ExtendedServerInfo.prototype['InboundSpamThreshold'] = undefined;

/**
 * @member {String} Name
 */
ExtendedServerInfo.prototype['Name'] = undefined;

/**
 * @member {String} OpenHookUrl
 */
ExtendedServerInfo.prototype['OpenHookUrl'] = undefined;

/**
 * @member {Boolean} PostFirstOpenOnly
 */
ExtendedServerInfo.prototype['PostFirstOpenOnly'] = undefined;

/**
 * @member {Boolean} RawEmailEnabled
 */
ExtendedServerInfo.prototype['RawEmailEnabled'] = undefined;

/**
 * @member {String} ServerLink
 */
ExtendedServerInfo.prototype['ServerLink'] = undefined;

/**
 * @member {Boolean} SmtpApiActivated
 */
ExtendedServerInfo.prototype['SmtpApiActivated'] = undefined;

/**
 * @member {module:model/ExtendedServerInfo.TrackLinksEnum} TrackLinks
 */
ExtendedServerInfo.prototype['TrackLinks'] = undefined;

/**
 * @member {Boolean} TrackOpens
 */
ExtendedServerInfo.prototype['TrackOpens'] = undefined;





/**
 * Allowed values for the <code>TrackLinks</code> property.
 * @enum {String}
 * @readonly
 */
ExtendedServerInfo['TrackLinksEnum'] = {

    /**
     * value: "None"
     * @const
     */
    "None": "None",

    /**
     * value: "HtmlAndTextTracking"
     * @const
     */
    "HtmlAndTextTracking": "HtmlAndTextTracking",

    /**
     * value: "HtmlOnlyTracking"
     * @const
     */
    "HtmlOnlyTracking": "HtmlOnlyTracking",

    /**
     * value: "TextOnlyTracking"
     * @const
     */
    "TextOnlyTracking": "TextOnlyTracking"
};



export default ExtendedServerInfo;

