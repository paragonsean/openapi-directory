/**
 * NamSor API v2
 * NamSor API v2 : enpoints to process personal names (gender, cultural origin or ethnicity) in all alphabets or languages. By default, enpoints use 1 unit per name (ex. Gender), but Ethnicity classification uses 10 to 20 units per name depending on taxonomy. Use GET methods for small tests, but prefer POST methods for higher throughput (batch processing of up to 100 names at a time). Need something you can't find here? We have many more features coming soon. Let us know, we'll do our best to add it! 
 *
 * The version of the OpenAPI document: 2.0.24
 * Contact: contact@namsor.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', process.cwd()+'/src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require(process.cwd()+'/src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.NamSorApiV2);
  }
}(this, function(expect, NamSorApiV2) {
  'use strict';

  var instance;

  beforeEach(function() {
    instance = new NamSorApiV2.RegionISO();
  });

  var getProperty = function(object, getter, property) {
    // Use getter method if present; otherwise, get the property directly.
    if (typeof object[getter] === 'function')
      return object[getter]();
    else
      return object[property];
  }

  var setProperty = function(object, setter, property, value) {
    // Use setter method if present; otherwise, set the property directly.
    if (typeof object[setter] === 'function')
      object[setter](value);
    else
      object[property] = value;
  }

  describe('RegionISO', function() {
    it('should create an instance of RegionISO', function() {
      // uncomment below and update the code to test RegionISO
      //var instance = new NamSorApiV2.RegionISO();
      //expect(instance).to.be.a(NamSorApiV2.RegionISO);
    });

    it('should have the property countryFIPS (base name: "countryFIPS")', function() {
      // uncomment below and update the code to test the property countryFIPS
      //var instance = new NamSorApiV2.RegionISO();
      //expect(instance).to.be();
    });

    it('should have the property countryISO2 (base name: "countryISO2")', function() {
      // uncomment below and update the code to test the property countryISO2
      //var instance = new NamSorApiV2.RegionISO();
      //expect(instance).to.be();
    });

    it('should have the property countryISO3 (base name: "countryISO3")', function() {
      // uncomment below and update the code to test the property countryISO3
      //var instance = new NamSorApiV2.RegionISO();
      //expect(instance).to.be();
    });

    it('should have the property countryName (base name: "countryName")', function() {
      // uncomment below and update the code to test the property countryName
      //var instance = new NamSorApiV2.RegionISO();
      //expect(instance).to.be();
    });

    it('should have the property countryNumCode (base name: "countryNumCode")', function() {
      // uncomment below and update the code to test the property countryNumCode
      //var instance = new NamSorApiV2.RegionISO();
      //expect(instance).to.be();
    });

    it('should have the property region (base name: "region")', function() {
      // uncomment below and update the code to test the property region
      //var instance = new NamSorApiV2.RegionISO();
      //expect(instance).to.be();
    });

    it('should have the property subregion (base name: "subregion")', function() {
      // uncomment below and update the code to test the property subregion
      //var instance = new NamSorApiV2.RegionISO();
      //expect(instance).to.be();
    });

    it('should have the property topregion (base name: "topregion")', function() {
      // uncomment below and update the code to test the property topregion
      //var instance = new NamSorApiV2.RegionISO();
      //expect(instance).to.be();
    });

  });

}));
