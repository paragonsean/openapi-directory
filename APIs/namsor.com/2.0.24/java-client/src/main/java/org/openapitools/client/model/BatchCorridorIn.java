...
public class BatchCorridorIn {
  ...
  private List<CorridorIn> corridorFromTo = new ArrayList<>();
   ...
   * @throws IOException if the JSON Element is invalid with respect to BatchCorridorIn
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!BatchCorridorIn.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!BatchCorridorIn.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BatchCorridorIn` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("corridorFromTo") != null && !jsonObj.get("corridorFromTo").isJsonNull()) {
        ...
        if (jsonArraycorridorFromTo != null) {
          ...
          for (int i = 0; i < jsonArraycorridorFromTo.size(); i++) {
            CorridorIn.validateJsonElement(jsonArraycorridorFromTo.get(i));
          };
        }
      }
