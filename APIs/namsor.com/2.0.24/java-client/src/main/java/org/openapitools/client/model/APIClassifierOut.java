...
public class APIClassifierOut {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(classifierName, learning, probabilityCalibrated, serving, shuttingDown);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to APIClassifierOut
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!APIClassifierOut.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!APIClassifierOut.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `APIClassifierOut` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
