...
public class BatchCorridorOut {
  ...
  private List<CorridorOut> corridorFromTo = new ArrayList<>();
   ...
   * @throws IOException if the JSON Element is invalid with respect to BatchCorridorOut
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!BatchCorridorOut.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!BatchCorridorOut.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BatchCorridorOut` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("corridorFromTo") != null && !jsonObj.get("corridorFromTo").isJsonNull()) {
        ...
        if (jsonArraycorridorFromTo != null) {
          ...
          for (int i = 0; i < jsonArraycorridorFromTo.size(); i++) {
            CorridorOut.validateJsonElement(jsonArraycorridorFromTo.get(i));
          };
        }
      }
  }
