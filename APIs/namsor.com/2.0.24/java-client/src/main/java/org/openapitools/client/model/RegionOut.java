...
public class RegionOut {
  ...
  private List<RegionISO> countriesAndRegions = new ArrayList<>();
   ...
   * @throws IOException if the JSON Element is invalid with respect to RegionOut
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!RegionOut.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!RegionOut.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `RegionOut` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("countriesAndRegions") != null && !jsonObj.get("countriesAndRegions").isJsonNull()) {
        ...
        if (jsonArraycountriesAndRegions != null) {
          ...
          for (int i = 0; i < jsonArraycountriesAndRegions.size(); i++) {
            RegionISO.validateJsonElement(jsonArraycountriesAndRegions.get(i));
          };
        }
      }
  }
