/**
 * Events API
 * 1Password Events API Specification.
 *
 * The version of the OpenAPI document: 1.2.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import Cursor from './Cursor';

/**
 * The CursorCollection model module.
 * @module model/CursorCollection
 * @version 1.2.0
 */
class CursorCollection {
    /**
     * Constructs a new <code>CursorCollection</code>.
     * Common cursor properties for collection responses
     * @alias module:model/CursorCollection
     * @implements module:model/Cursor
     */
    constructor() { 
        Cursor.initialize(this);
        CursorCollection.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>CursorCollection</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/CursorCollection} obj Optional instance to populate.
     * @return {module:model/CursorCollection} The populated <code>CursorCollection</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new CursorCollection();
            Cursor.constructFromObject(data, obj);

            if (data.hasOwnProperty('cursor')) {
                obj['cursor'] = ApiClient.convertToType(data['cursor'], 'String');
            }
            if (data.hasOwnProperty('has_more')) {
                obj['has_more'] = ApiClient.convertToType(data['has_more'], 'Boolean');
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>CursorCollection</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>CursorCollection</code>.
     */
    static validateJSON(data) {
        // ensure the json data is a string
        if (data['cursor'] && !(typeof data['cursor'] === 'string' || data['cursor'] instanceof String)) {
            throw new Error("Expected the field `cursor` to be a primitive type in the JSON string but got " + data['cursor']);
        }

        return true;
    }


}



/**
 * Cursor to fetch more data if available or continue the polling process if required
 * @member {String} cursor
 */
CursorCollection.prototype['cursor'] = undefined;

/**
 * Whether there may still be more data to fetch using the returned cursor. If true, the subsequent request could still be empty.
 * @member {Boolean} has_more
 */
CursorCollection.prototype['has_more'] = undefined;


// Implement Cursor interface:
/**
 * Cursor to fetch more data if available or continue the polling process if required
 * @member {String} cursor
 */
Cursor.prototype['cursor'] = undefined;




export default CursorCollection;

