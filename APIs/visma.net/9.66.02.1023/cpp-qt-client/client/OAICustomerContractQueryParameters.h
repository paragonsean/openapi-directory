/**
 * Visma Net API
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 9.66.02.1023
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAICustomerContractQueryParameters.h
 *
 * 
 */

#ifndef OAICustomerContractQueryParameters_H
#define OAICustomerContractQueryParameters_H

#include <QJsonObject>

#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {

class OAICustomerContractQueryParameters : public OAIObject {
public:
    OAICustomerContractQueryParameters();
    OAICustomerContractQueryParameters(QString json);
    ~OAICustomerContractQueryParameters() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QString getAttributes() const;
    void setAttributes(const QString &attributes);
    bool is_attributes_Set() const;
    bool is_attributes_Valid() const;

    QString getContractTemplate() const;
    void setContractTemplate(const QString &contract_template);
    bool is_contract_template_Set() const;
    bool is_contract_template_Valid() const;

    QString getCustomer() const;
    void setCustomer(const QString &customer);
    bool is_customer_Set() const;
    bool is_customer_Valid() const;

    bool isExpandAttributes() const;
    void setExpandAttributes(const bool &expand_attributes);
    bool is_expand_attributes_Set() const;
    bool is_expand_attributes_Valid() const;

    bool isExpandDetails() const;
    void setExpandDetails(const bool &expand_details);
    bool is_expand_details_Set() const;
    bool is_expand_details_Valid() const;

    bool isExpandSummary() const;
    void setExpandSummary(const bool &expand_summary);
    bool is_expand_summary_Set() const;
    bool is_expand_summary_Valid() const;

    QString getGreaterThanValue() const;
    void setGreaterThanValue(const QString &greater_than_value);
    bool is_greater_than_value_Set() const;
    bool is_greater_than_value_Valid() const;

    QString getLastModifiedDateTime() const;
    void setLastModifiedDateTime(const QString &last_modified_date_time);
    bool is_last_modified_date_time_Set() const;
    bool is_last_modified_date_time_Valid() const;

    QString getLastModifiedDateTimeCondition() const;
    void setLastModifiedDateTimeCondition(const QString &last_modified_date_time_condition);
    bool is_last_modified_date_time_condition_Set() const;
    bool is_last_modified_date_time_condition_Valid() const;

    qint32 getNumberToRead() const;
    void setNumberToRead(const qint32 &number_to_read);
    bool is_number_to_read_Set() const;
    bool is_number_to_read_Valid() const;

    QString getOrderBy() const;
    void setOrderBy(const QString &order_by);
    bool is_order_by_Set() const;
    bool is_order_by_Valid() const;

    qint32 getSkipRecords() const;
    void setSkipRecords(const qint32 &skip_records);
    bool is_skip_records_Set() const;
    bool is_skip_records_Valid() const;

    QString getStatus() const;
    void setStatus(const QString &status);
    bool is_status_Set() const;
    bool is_status_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString m_attributes;
    bool m_attributes_isSet;
    bool m_attributes_isValid;

    QString m_contract_template;
    bool m_contract_template_isSet;
    bool m_contract_template_isValid;

    QString m_customer;
    bool m_customer_isSet;
    bool m_customer_isValid;

    bool m_expand_attributes;
    bool m_expand_attributes_isSet;
    bool m_expand_attributes_isValid;

    bool m_expand_details;
    bool m_expand_details_isSet;
    bool m_expand_details_isValid;

    bool m_expand_summary;
    bool m_expand_summary_isSet;
    bool m_expand_summary_isValid;

    QString m_greater_than_value;
    bool m_greater_than_value_isSet;
    bool m_greater_than_value_isValid;

    QString m_last_modified_date_time;
    bool m_last_modified_date_time_isSet;
    bool m_last_modified_date_time_isValid;

    QString m_last_modified_date_time_condition;
    bool m_last_modified_date_time_condition_isSet;
    bool m_last_modified_date_time_condition_isValid;

    qint32 m_number_to_read;
    bool m_number_to_read_isSet;
    bool m_number_to_read_isValid;

    QString m_order_by;
    bool m_order_by_isSet;
    bool m_order_by_isValid;

    qint32 m_skip_records;
    bool m_skip_records_isSet;
    bool m_skip_records_isValid;

    QString m_status;
    bool m_status_isSet;
    bool m_status_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAICustomerContractQueryParameters)

#endif // OAICustomerContractQueryParameters_H
