...
> BusinessLine patchBusinessLinesId(id, opts)
...
let id = "id_example"; // String | The unique identifier of the business line.
let opts = {
  'businessLineInfoUpdate': new LegalEntityManagementApi.BusinessLineInfoUpdate() // BusinessLineInfoUpdate | 
};
apiInstance.patchBusinessLinesId(id, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
