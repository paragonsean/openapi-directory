...
async def test_get_legal_entities_id_terms_of_service_acceptance_infos(client):
    ...
    response = await client.request(
        method='GET',
        path='/lem/v1/legalEntities/{id}/termsOfServiceAcceptanceInfos'.format(id='id_example'),
        headers=headers,
        )
...
async def test_get_legal_entities_id_terms_of_service_status(client):
    ...
    response = await client.request(
        method='GET',
        path='/lem/v1/legalEntities/{id}/termsOfServiceStatus'.format(id='id_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_patch_legal_entities_id_terms_of_service_termsofservicedocumentid(client):
    ...
    response = await client.request(
        method='PATCH',
        path='/lem/v1/legalEntities/{id}/termsOfService/{termsofservicedocumentid}'.format(id='id_example', termsofservicedocumentid='termsofservicedocumentid_example'),
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_post_legal_entities_id_terms_of_service(client):
    ...
    response = await client.request(
        method='POST',
        path='/lem/v1/legalEntities/{id}/termsOfService'.format(id='id_example'),
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
