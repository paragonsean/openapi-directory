...
async def test_get_legal_entities_id_pci_questionnaires(client):
    ...
    response = await client.request(
        method='GET',
        path='/lem/v3/legalEntities/{id}/pciQuestionnaires'.format(id='id_example'),
        headers=headers,
        )
...
async def test_get_legal_entities_id_pci_questionnaires_pciid(client):
    ...
    response = await client.request(
        method='GET',
        path='/lem/v3/legalEntities/{id}/pciQuestionnaires/{pciid}'.format(id='id_example', pciid='pciid_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_post_legal_entities_id_pci_questionnaires_generate_pci_templates(client):
    ...
    response = await client.request(
        method='POST',
        path='/lem/v3/legalEntities/{id}/pciQuestionnaires/generatePciTemplates'.format(id='id_example'),
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_post_legal_entities_id_pci_questionnaires_sign_pci_templates(client):
    ...
    response = await client.request(
        method='POST',
        path='/lem/v3/legalEntities/{id}/pciQuestionnaires/signPciTemplates'.format(id='id_example'),
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
