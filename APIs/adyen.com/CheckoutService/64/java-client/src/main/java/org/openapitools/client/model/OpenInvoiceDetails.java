...
public class OpenInvoiceDetails {
   ...
   * @throws IOException if the JSON Element is invalid with respect to OpenInvoiceDetails
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!OpenInvoiceDetails.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!OpenInvoiceDetails.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `OpenInvoiceDetails` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
