...
public class ZipDetails {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(clickAndCollect, recurringDetailReference, type);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to ZipDetails
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ZipDetails.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ZipDetails.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ZipDetails` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      if ((jsonObj.get("clickAndCollect") != null && !jsonObj.get("clickAndCollect").isJsonNull()) && !jsonObj.get("clickAndCollect").isJsonPrimitive()) {
