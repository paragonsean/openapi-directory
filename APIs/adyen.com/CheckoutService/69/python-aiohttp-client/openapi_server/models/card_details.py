# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server import util


class CardDetails(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, brand: str=None, checkout_attempt_id: str=None, cupsecureplus_smscode: str=None, cvc: str=None, encrypted_card_number: str=None, encrypted_expiry_month: str=None, encrypted_expiry_year: str=None, encrypted_security_code: str=None, expiry_month: str=None, expiry_year: str=None, funding_source: str=None, holder_name: str=None, network_payment_reference: str=None, number: str=None, recurring_detail_reference: str=None, shopper_notification_reference: str=None, stored_payment_method_id: str=None, three_ds2_sdk_version: str=None, type: str='scheme'):
        """CardDetails - a model defined in OpenAPI

        :param brand: The brand of this CardDetails.
        :param checkout_attempt_id: The checkout_attempt_id of this CardDetails.
        :param cupsecureplus_smscode: The cupsecureplus_smscode of this CardDetails.
        :param cvc: The cvc of this CardDetails.
        :param encrypted_card_number: The encrypted_card_number of this CardDetails.
        :param encrypted_expiry_month: The encrypted_expiry_month of this CardDetails.
        :param encrypted_expiry_year: The encrypted_expiry_year of this CardDetails.
        :param encrypted_security_code: The encrypted_security_code of this CardDetails.
        :param expiry_month: The expiry_month of this CardDetails.
        :param expiry_year: The expiry_year of this CardDetails.
        :param funding_source: The funding_source of this CardDetails.
        :param holder_name: The holder_name of this CardDetails.
        :param network_payment_reference: The network_payment_reference of this CardDetails.
        :param number: The number of this CardDetails.
        :param recurring_detail_reference: The recurring_detail_reference of this CardDetails.
        :param shopper_notification_reference: The shopper_notification_reference of this CardDetails.
        :param stored_payment_method_id: The stored_payment_method_id of this CardDetails.
        :param three_ds2_sdk_version: The three_ds2_sdk_version of this CardDetails.
        :param type: The type of this CardDetails.
        """
        self.openapi_types = {
            'brand': str,
            'checkout_attempt_id': str,
            'cupsecureplus_smscode': str,
            'cvc': str,
            'encrypted_card_number': str,
            'encrypted_expiry_month': str,
            'encrypted_expiry_year': str,
            'encrypted_security_code': str,
            'expiry_month': str,
            'expiry_year': str,
            'funding_source': str,
            'holder_name': str,
            'network_payment_reference': str,
            'number': str,
            'recurring_detail_reference': str,
            'shopper_notification_reference': str,
            'stored_payment_method_id': str,
            'three_ds2_sdk_version': str,
            'type': str
        }

        self.attribute_map = {
            'brand': 'brand',
            'checkout_attempt_id': 'checkoutAttemptId',
            'cupsecureplus_smscode': 'cupsecureplus.smscode',
            'cvc': 'cvc',
            'encrypted_card_number': 'encryptedCardNumber',
            'encrypted_expiry_month': 'encryptedExpiryMonth',
            'encrypted_expiry_year': 'encryptedExpiryYear',
            'encrypted_security_code': 'encryptedSecurityCode',
            'expiry_month': 'expiryMonth',
            'expiry_year': 'expiryYear',
            'funding_source': 'fundingSource',
            'holder_name': 'holderName',
            'network_payment_reference': 'networkPaymentReference',
            'number': 'number',
            'recurring_detail_reference': 'recurringDetailReference',
            'shopper_notification_reference': 'shopperNotificationReference',
            'stored_payment_method_id': 'storedPaymentMethodId',
            'three_ds2_sdk_version': 'threeDS2SdkVersion',
            'type': 'type'
        }

        self._brand = brand
        self._checkout_attempt_id = checkout_attempt_id
        self._cupsecureplus_smscode = cupsecureplus_smscode
        self._cvc = cvc
        self._encrypted_card_number = encrypted_card_number
        self._encrypted_expiry_month = encrypted_expiry_month
        self._encrypted_expiry_year = encrypted_expiry_year
        self._encrypted_security_code = encrypted_security_code
        self._expiry_month = expiry_month
        self._expiry_year = expiry_year
        self._funding_source = funding_source
        self._holder_name = holder_name
        self._network_payment_reference = network_payment_reference
        self._number = number
        self._recurring_detail_reference = recurring_detail_reference
        self._shopper_notification_reference = shopper_notification_reference
        self._stored_payment_method_id = stored_payment_method_id
        self._three_ds2_sdk_version = three_ds2_sdk_version
        self._type = type

    @classmethod
    def from_dict(cls, dikt: dict) -> 'CardDetails':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The CardDetails of this CardDetails.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def brand(self):
        """Gets the brand of this CardDetails.

        Secondary brand of the card. For example: **plastix**, **hmclub**.

        :return: The brand of this CardDetails.
        :rtype: str
        """
        return self._brand

    @brand.setter
    def brand(self, brand):
        """Sets the brand of this CardDetails.

        Secondary brand of the card. For example: **plastix**, **hmclub**.

        :param brand: The brand of this CardDetails.
        :type brand: str
        """

        self._brand = brand

    @property
    def checkout_attempt_id(self):
        """Gets the checkout_attempt_id of this CardDetails.

        The checkout attempt identifier.

        :return: The checkout_attempt_id of this CardDetails.
        :rtype: str
        """
        return self._checkout_attempt_id

    @checkout_attempt_id.setter
    def checkout_attempt_id(self, checkout_attempt_id):
        """Sets the checkout_attempt_id of this CardDetails.

        The checkout attempt identifier.

        :param checkout_attempt_id: The checkout_attempt_id of this CardDetails.
        :type checkout_attempt_id: str
        """

        self._checkout_attempt_id = checkout_attempt_id

    @property
    def cupsecureplus_smscode(self):
        """Gets the cupsecureplus_smscode of this CardDetails.


        :return: The cupsecureplus_smscode of this CardDetails.
        :rtype: str
        """
        return self._cupsecureplus_smscode

    @cupsecureplus_smscode.setter
    def cupsecureplus_smscode(self, cupsecureplus_smscode):
        """Sets the cupsecureplus_smscode of this CardDetails.


        :param cupsecureplus_smscode: The cupsecureplus_smscode of this CardDetails.
        :type cupsecureplus_smscode: str
        """

        self._cupsecureplus_smscode = cupsecureplus_smscode

    @property
    def cvc(self):
        """Gets the cvc of this CardDetails.

        The card verification code. Only collect raw card data if you are [fully PCI compliant](https://docs.adyen.com/development-resources/pci-dss-compliance-guide).

        :return: The cvc of this CardDetails.
        :rtype: str
        """
        return self._cvc

    @cvc.setter
    def cvc(self, cvc):
        """Sets the cvc of this CardDetails.

        The card verification code. Only collect raw card data if you are [fully PCI compliant](https://docs.adyen.com/development-resources/pci-dss-compliance-guide).

        :param cvc: The cvc of this CardDetails.
        :type cvc: str
        """

        self._cvc = cvc

    @property
    def encrypted_card_number(self):
        """Gets the encrypted_card_number of this CardDetails.

        The encrypted card number.

        :return: The encrypted_card_number of this CardDetails.
        :rtype: str
        """
        return self._encrypted_card_number

    @encrypted_card_number.setter
    def encrypted_card_number(self, encrypted_card_number):
        """Sets the encrypted_card_number of this CardDetails.

        The encrypted card number.

        :param encrypted_card_number: The encrypted_card_number of this CardDetails.
        :type encrypted_card_number: str
        """
        if encrypted_card_number is not None and len(encrypted_card_number) > 15000:
            raise ValueError("Invalid value for `encrypted_card_number`, length must be less than or equal to `15000`")

        self._encrypted_card_number = encrypted_card_number

    @property
    def encrypted_expiry_month(self):
        """Gets the encrypted_expiry_month of this CardDetails.

        The encrypted card expiry month.

        :return: The encrypted_expiry_month of this CardDetails.
        :rtype: str
        """
        return self._encrypted_expiry_month

    @encrypted_expiry_month.setter
    def encrypted_expiry_month(self, encrypted_expiry_month):
        """Sets the encrypted_expiry_month of this CardDetails.

        The encrypted card expiry month.

        :param encrypted_expiry_month: The encrypted_expiry_month of this CardDetails.
        :type encrypted_expiry_month: str
        """
        if encrypted_expiry_month is not None and len(encrypted_expiry_month) > 15000:
            raise ValueError("Invalid value for `encrypted_expiry_month`, length must be less than or equal to `15000`")

        self._encrypted_expiry_month = encrypted_expiry_month

    @property
    def encrypted_expiry_year(self):
        """Gets the encrypted_expiry_year of this CardDetails.

        The encrypted card expiry year.

        :return: The encrypted_expiry_year of this CardDetails.
        :rtype: str
        """
        return self._encrypted_expiry_year

    @encrypted_expiry_year.setter
    def encrypted_expiry_year(self, encrypted_expiry_year):
        """Sets the encrypted_expiry_year of this CardDetails.

        The encrypted card expiry year.

        :param encrypted_expiry_year: The encrypted_expiry_year of this CardDetails.
        :type encrypted_expiry_year: str
        """
        if encrypted_expiry_year is not None and len(encrypted_expiry_year) > 15000:
            raise ValueError("Invalid value for `encrypted_expiry_year`, length must be less than or equal to `15000`")

        self._encrypted_expiry_year = encrypted_expiry_year

    @property
    def encrypted_security_code(self):
        """Gets the encrypted_security_code of this CardDetails.

        The encrypted card verification code.

        :return: The encrypted_security_code of this CardDetails.
        :rtype: str
        """
        return self._encrypted_security_code

    @encrypted_security_code.setter
    def encrypted_security_code(self, encrypted_security_code):
        """Sets the encrypted_security_code of this CardDetails.

        The encrypted card verification code.

        :param encrypted_security_code: The encrypted_security_code of this CardDetails.
        :type encrypted_security_code: str
        """
        if encrypted_security_code is not None and len(encrypted_security_code) > 15000:
            raise ValueError("Invalid value for `encrypted_security_code`, length must be less than or equal to `15000`")

        self._encrypted_security_code = encrypted_security_code

    @property
    def expiry_month(self):
        """Gets the expiry_month of this CardDetails.

        The card expiry month. Only collect raw card data if you are [fully PCI compliant](https://docs.adyen.com/development-resources/pci-dss-compliance-guide).

        :return: The expiry_month of this CardDetails.
        :rtype: str
        """
        return self._expiry_month

    @expiry_month.setter
    def expiry_month(self, expiry_month):
        """Sets the expiry_month of this CardDetails.

        The card expiry month. Only collect raw card data if you are [fully PCI compliant](https://docs.adyen.com/development-resources/pci-dss-compliance-guide).

        :param expiry_month: The expiry_month of this CardDetails.
        :type expiry_month: str
        """

        self._expiry_month = expiry_month

    @property
    def expiry_year(self):
        """Gets the expiry_year of this CardDetails.

        The card expiry year. Only collect raw card data if you are [fully PCI compliant](https://docs.adyen.com/development-resources/pci-dss-compliance-guide).

        :return: The expiry_year of this CardDetails.
        :rtype: str
        """
        return self._expiry_year

    @expiry_year.setter
    def expiry_year(self, expiry_year):
        """Sets the expiry_year of this CardDetails.

        The card expiry year. Only collect raw card data if you are [fully PCI compliant](https://docs.adyen.com/development-resources/pci-dss-compliance-guide).

        :param expiry_year: The expiry_year of this CardDetails.
        :type expiry_year: str
        """

        self._expiry_year = expiry_year

    @property
    def funding_source(self):
        """Gets the funding_source of this CardDetails.

        The funding source that should be used when multiple sources are available. For Brazilian combo cards, by default the funding source is credit. To use debit, set this value to **debit**.

        :return: The funding_source of this CardDetails.
        :rtype: str
        """
        return self._funding_source

    @funding_source.setter
    def funding_source(self, funding_source):
        """Sets the funding_source of this CardDetails.

        The funding source that should be used when multiple sources are available. For Brazilian combo cards, by default the funding source is credit. To use debit, set this value to **debit**.

        :param funding_source: The funding_source of this CardDetails.
        :type funding_source: str
        """
        allowed_values = ["credit", "debit"]  # noqa: E501
        if funding_source not in allowed_values:
            raise ValueError(
                "Invalid value for `funding_source` ({0}), must be one of {1}"
                .format(funding_source, allowed_values)
            )

        self._funding_source = funding_source

    @property
    def holder_name(self):
        """Gets the holder_name of this CardDetails.

        The name of the card holder.

        :return: The holder_name of this CardDetails.
        :rtype: str
        """
        return self._holder_name

    @holder_name.setter
    def holder_name(self, holder_name):
        """Sets the holder_name of this CardDetails.

        The name of the card holder.

        :param holder_name: The holder_name of this CardDetails.
        :type holder_name: str
        """

        self._holder_name = holder_name

    @property
    def network_payment_reference(self):
        """Gets the network_payment_reference of this CardDetails.

        The network token reference. This is the [`networkTxReference`](https://docs.adyen.com/api-explorer/#/CheckoutService/latest/post/payments__resParam_additionalData-ResponseAdditionalDataCommon-networkTxReference) from the response to the first payment.

        :return: The network_payment_reference of this CardDetails.
        :rtype: str
        """
        return self._network_payment_reference

    @network_payment_reference.setter
    def network_payment_reference(self, network_payment_reference):
        """Sets the network_payment_reference of this CardDetails.

        The network token reference. This is the [`networkTxReference`](https://docs.adyen.com/api-explorer/#/CheckoutService/latest/post/payments__resParam_additionalData-ResponseAdditionalDataCommon-networkTxReference) from the response to the first payment.

        :param network_payment_reference: The network_payment_reference of this CardDetails.
        :type network_payment_reference: str
        """

        self._network_payment_reference = network_payment_reference

    @property
    def number(self):
        """Gets the number of this CardDetails.

        The card number. Only collect raw card data if you are [fully PCI compliant](https://docs.adyen.com/development-resources/pci-dss-compliance-guide).

        :return: The number of this CardDetails.
        :rtype: str
        """
        return self._number

    @number.setter
    def number(self, number):
        """Sets the number of this CardDetails.

        The card number. Only collect raw card data if you are [fully PCI compliant](https://docs.adyen.com/development-resources/pci-dss-compliance-guide).

        :param number: The number of this CardDetails.
        :type number: str
        """

        self._number = number

    @property
    def recurring_detail_reference(self):
        """Gets the recurring_detail_reference of this CardDetails.

        This is the `recurringDetailReference` returned in the response when you created the token.

        :return: The recurring_detail_reference of this CardDetails.
        :rtype: str
        """
        return self._recurring_detail_reference

    @recurring_detail_reference.setter
    def recurring_detail_reference(self, recurring_detail_reference):
        """Sets the recurring_detail_reference of this CardDetails.

        This is the `recurringDetailReference` returned in the response when you created the token.

        :param recurring_detail_reference: The recurring_detail_reference of this CardDetails.
        :type recurring_detail_reference: str
        """

        self._recurring_detail_reference = recurring_detail_reference

    @property
    def shopper_notification_reference(self):
        """Gets the shopper_notification_reference of this CardDetails.

        The `shopperNotificationReference` returned in the response when you requested to notify the shopper. Used only for recurring payments in India.

        :return: The shopper_notification_reference of this CardDetails.
        :rtype: str
        """
        return self._shopper_notification_reference

    @shopper_notification_reference.setter
    def shopper_notification_reference(self, shopper_notification_reference):
        """Sets the shopper_notification_reference of this CardDetails.

        The `shopperNotificationReference` returned in the response when you requested to notify the shopper. Used only for recurring payments in India.

        :param shopper_notification_reference: The shopper_notification_reference of this CardDetails.
        :type shopper_notification_reference: str
        """

        self._shopper_notification_reference = shopper_notification_reference

    @property
    def stored_payment_method_id(self):
        """Gets the stored_payment_method_id of this CardDetails.

        This is the `recurringDetailReference` returned in the response when you created the token.

        :return: The stored_payment_method_id of this CardDetails.
        :rtype: str
        """
        return self._stored_payment_method_id

    @stored_payment_method_id.setter
    def stored_payment_method_id(self, stored_payment_method_id):
        """Sets the stored_payment_method_id of this CardDetails.

        This is the `recurringDetailReference` returned in the response when you created the token.

        :param stored_payment_method_id: The stored_payment_method_id of this CardDetails.
        :type stored_payment_method_id: str
        """
        if stored_payment_method_id is not None and len(stored_payment_method_id) > 64:
            raise ValueError("Invalid value for `stored_payment_method_id`, length must be less than or equal to `64`")

        self._stored_payment_method_id = stored_payment_method_id

    @property
    def three_ds2_sdk_version(self):
        """Gets the three_ds2_sdk_version of this CardDetails.

        Required for mobile integrations. Version of the 3D Secure 2 mobile SDK.

        :return: The three_ds2_sdk_version of this CardDetails.
        :rtype: str
        """
        return self._three_ds2_sdk_version

    @three_ds2_sdk_version.setter
    def three_ds2_sdk_version(self, three_ds2_sdk_version):
        """Sets the three_ds2_sdk_version of this CardDetails.

        Required for mobile integrations. Version of the 3D Secure 2 mobile SDK.

        :param three_ds2_sdk_version: The three_ds2_sdk_version of this CardDetails.
        :type three_ds2_sdk_version: str
        """
        if three_ds2_sdk_version is not None and len(three_ds2_sdk_version) > 12:
            raise ValueError("Invalid value for `three_ds2_sdk_version`, length must be less than or equal to `12`")

        self._three_ds2_sdk_version = three_ds2_sdk_version

    @property
    def type(self):
        """Gets the type of this CardDetails.

        Default payment method details. Common for scheme payment methods, and for simple payment method details.

        :return: The type of this CardDetails.
        :rtype: str
        """
        return self._type

    @type.setter
    def type(self, type):
        """Sets the type of this CardDetails.

        Default payment method details. Common for scheme payment methods, and for simple payment method details.

        :param type: The type of this CardDetails.
        :type type: str
        """
        allowed_values = ["bcmc", "scheme", "networkToken", "giftcard", "card"]  # noqa: E501
        if type not in allowed_values:
            raise ValueError(
                "Invalid value for `type` ({0}), must be one of {1}"
                .format(type, allowed_values)
            )

        self._type = type
