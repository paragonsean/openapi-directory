...
public class CheckoutThreeDS2Action {
   ...
   * @throws IOException if the JSON Element is invalid with respect to CheckoutThreeDS2Action
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CheckoutThreeDS2Action.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CheckoutThreeDS2Action.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CheckoutThreeDS2Action` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
