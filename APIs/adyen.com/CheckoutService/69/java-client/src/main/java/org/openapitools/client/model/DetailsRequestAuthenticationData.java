...
public class DetailsRequestAuthenticationData {
   ...
   * @throws IOException if the JSON Element is invalid with respect to DetailsRequestAuthenticationData
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DetailsRequestAuthenticationData.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DetailsRequestAuthenticationData.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DetailsRequestAuthenticationData` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
