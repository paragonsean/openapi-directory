...
public class PaymentLinkResponse {
  ...
  public enum RequiredShopperFieldsEnum {
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      String value = jsonElement.getAsString();
      RequiredShopperFieldsEnum.fromValue(value);
    }
  }
  ...
  @Override
  public int hashCode() {
    return Objects.hash(allowedPaymentMethods, amount, applicationInfo, billingAddress, blockedPaymentMethods, countryCode, deliverAt, deliveryAddress, description, expiresAt, id, installmentOptions, lineItems, manualCapture, merchantAccount, merchantOrderReference, metadata, recurringProcessingModel, reference, requiredShopperFields, returnUrl, reusable, riskData, shopperEmail, shopperLocale, shopperName, shopperReference, showRemovePaymentMethodButton, splits, status, store, storePaymentMethodMode, telephoneNumber, themeId, updatedAt, url);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to PaymentLinkResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PaymentLinkResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PaymentLinkResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PaymentLinkResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      // ensure the optional json data is an array if present
      if (jsonObj.get("requiredShopperFields") != null && !jsonObj.get("requiredShopperFields").isJsonNull() && !jsonObj.get("requiredShopperFields").isJsonArray()) {
