...
public class PayWithGoogleDetails {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(fundingSource, googlePayToken, recurringDetailReference, type);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to PayWithGoogleDetails
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PayWithGoogleDetails.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PayWithGoogleDetails.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PayWithGoogleDetails` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
