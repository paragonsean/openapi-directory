...
public class BlikDetails {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(blikCode, recurringDetailReference, type);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to BlikDetails
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!BlikDetails.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!BlikDetails.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BlikDetails` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      if ((jsonObj.get("blikCode") != null && !jsonObj.get("blikCode").isJsonNull()) && !jsonObj.get("blikCode").isJsonPrimitive()) {
