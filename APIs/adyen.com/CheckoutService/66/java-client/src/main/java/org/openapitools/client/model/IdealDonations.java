...
public class IdealDonations {
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(this.issuer, idealDonations.issuer) &&
   ...
   * @throws IOException if the JSON Element is invalid with respect to IdealDonations
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!IdealDonations.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in IdealDonations is not found in the empty JSON string", IdealDonations.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!IdealDonations.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `IdealDonations` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : IdealDonations.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!IdealDonations.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'IdealDonations' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<IdealDonations> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(IdealDonations.class));

       return (TypeAdapter<T>) new TypeAdapter<IdealDonations>() {
           @Override
           public void write(JsonWriter out, IdealDonations value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public IdealDonations read(JsonReader in) throws IOException {
   ...
   * @throws IOException if the JSON string is invalid with respect to IdealDonations
   */
  public static IdealDonations fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, IdealDonations.class);
  }
