...
public class GooglePayDetails {
  ...
  public static final String SERIALIZED_NAME_GOOGLE_PAY_CARD_NETWORK = "googlePayCardNetwork";
  @SerializedName(SERIALIZED_NAME_GOOGLE_PAY_CARD_NETWORK)
  ...
  @Override
  public int hashCode() {
    return Objects.hash(fundingSource, googlePayCardNetwork, googlePayToken, recurringDetailReference, storedPaymentMethodId, type);
  }
  ...
  static {
    ...
    openapiFields.add("googlePayCardNetwork");
   ...
   * @throws IOException if the JSON Element is invalid with respect to GooglePayDetails
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!GooglePayDetails.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in GooglePayDetails is not found in the empty JSON string", GooglePayDetails.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!GooglePayDetails.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `GooglePayDetails` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : GooglePayDetails.openapiRequiredFields) {
      ...
      if ((jsonObj.get("googlePayCardNetwork") != null && !jsonObj.get("googlePayCardNetwork").isJsonNull()) && !jsonObj.get("googlePayCardNetwork").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `googlePayCardNetwork` to be a primitive type in the JSON string but got `%s`", jsonObj.get("googlePayCardNetwork").toString()));
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!GooglePayDetails.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'GooglePayDetails' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<GooglePayDetails> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(GooglePayDetails.class));

       return (TypeAdapter<T>) new TypeAdapter<GooglePayDetails>() {
           @Override
           public void write(JsonWriter out, GooglePayDetails value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public GooglePayDetails read(JsonReader in) throws IOException {
   ...
   * @throws IOException if the JSON string is invalid with respect to GooglePayDetails
   */
  public static GooglePayDetails fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, GooglePayDetails.class);
  }
