...
public class ApplePayDetails {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(applePayToken, fundingSource, recurringDetailReference, type);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to ApplePayDetails
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ApplePayDetails.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ApplePayDetails.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ApplePayDetails` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
