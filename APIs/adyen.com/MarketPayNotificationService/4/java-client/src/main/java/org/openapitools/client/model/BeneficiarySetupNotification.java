...
import org.openapitools.client.model.BeneficiarySetupNotificationContent;
...
public class BeneficiarySetupNotification {
   ...
   * @throws IOException if the JSON Element is invalid with respect to BeneficiarySetupNotification
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!BeneficiarySetupNotification.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in BeneficiarySetupNotification is not found in the empty JSON string", BeneficiarySetupNotification.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!BeneficiarySetupNotification.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BeneficiarySetupNotification` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : BeneficiarySetupNotification.openapiRequiredFields) {
      ...
      if (jsonObj.get("content") != null && !jsonObj.get("content").isJsonNull()) {
        BeneficiarySetupNotificationContent.validateJsonElement(jsonObj.get("content"));
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!BeneficiarySetupNotification.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'BeneficiarySetupNotification' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<BeneficiarySetupNotification> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(BeneficiarySetupNotification.class));

       return (TypeAdapter<T>) new TypeAdapter<BeneficiarySetupNotification>() {
           @Override
           public void write(JsonWriter out, BeneficiarySetupNotification value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public BeneficiarySetupNotification read(JsonReader in) throws IOException {
   ...
   * @throws IOException if the JSON string is invalid with respect to BeneficiarySetupNotification
   */
  public static BeneficiarySetupNotification fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, BeneficiarySetupNotification.class);
  }
