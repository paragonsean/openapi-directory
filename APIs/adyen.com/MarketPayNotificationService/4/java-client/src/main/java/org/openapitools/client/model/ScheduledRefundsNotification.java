...
import org.openapitools.client.model.ScheduledRefundsNotificationContent;
...
public class ScheduledRefundsNotification {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ScheduledRefundsNotification
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ScheduledRefundsNotification.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ScheduledRefundsNotification is not found in the empty JSON string", ScheduledRefundsNotification.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ScheduledRefundsNotification.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ScheduledRefundsNotification` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ScheduledRefundsNotification.openapiRequiredFields) {
      ...
      if (jsonObj.get("content") != null && !jsonObj.get("content").isJsonNull()) {
        ScheduledRefundsNotificationContent.validateJsonElement(jsonObj.get("content"));
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ScheduledRefundsNotification.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ScheduledRefundsNotification' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ScheduledRefundsNotification> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ScheduledRefundsNotification.class));

       return (TypeAdapter<T>) new TypeAdapter<ScheduledRefundsNotification>() {
           @Override
           public void write(JsonWriter out, ScheduledRefundsNotification value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ScheduledRefundsNotification read(JsonReader in) throws IOException {
   ...
   * @throws IOException if the JSON string is invalid with respect to ScheduledRefundsNotification
   */
  public static ScheduledRefundsNotification fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ScheduledRefundsNotification.class);
  }
