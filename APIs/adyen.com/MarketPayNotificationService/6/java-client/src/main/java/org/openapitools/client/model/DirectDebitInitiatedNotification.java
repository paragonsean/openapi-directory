...
public class DirectDebitInitiatedNotification {
   ...
   * @throws IOException if the JSON Element is invalid with respect to DirectDebitInitiatedNotification
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DirectDebitInitiatedNotification.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in DirectDebitInitiatedNotification is not found in the empty JSON string", DirectDebitInitiatedNotification.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DirectDebitInitiatedNotification.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DirectDebitInitiatedNotification` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : DirectDebitInitiatedNotification.openapiRequiredFields) {
      ...
      if (jsonObj.get("content") != null && !jsonObj.get("content").isJsonNull()) {
        DirectDebitInitiatedNotificationContent.validateJsonElement(jsonObj.get("content"));
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!DirectDebitInitiatedNotification.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'DirectDebitInitiatedNotification' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<DirectDebitInitiatedNotification> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(DirectDebitInitiatedNotification.class));

       return (TypeAdapter<T>) new TypeAdapter<DirectDebitInitiatedNotification>() {
           @Override
           public void write(JsonWriter out, DirectDebitInitiatedNotification value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public DirectDebitInitiatedNotification read(JsonReader in) throws IOException {
   ...
   * @throws IOException if the JSON string is invalid with respect to DirectDebitInitiatedNotification
   */
  public static DirectDebitInitiatedNotification fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, DirectDebitInitiatedNotification.class);
  }
