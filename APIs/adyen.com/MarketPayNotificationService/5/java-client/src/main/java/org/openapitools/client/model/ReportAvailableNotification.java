...
import org.openapitools.client.model.ReportAvailableNotificationContent;
...
public class ReportAvailableNotification {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ReportAvailableNotification
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ReportAvailableNotification.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ReportAvailableNotification is not found in the empty JSON string", ReportAvailableNotification.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ReportAvailableNotification.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ReportAvailableNotification` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ReportAvailableNotification.openapiRequiredFields) {
      ...
      if (jsonObj.get("content") != null && !jsonObj.get("content").isJsonNull()) {
        ReportAvailableNotificationContent.validateJsonElement(jsonObj.get("content"));
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ReportAvailableNotification.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ReportAvailableNotification' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ReportAvailableNotification> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ReportAvailableNotification.class));

       return (TypeAdapter<T>) new TypeAdapter<ReportAvailableNotification>() {
           @Override
           public void write(JsonWriter out, ReportAvailableNotification value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ReportAvailableNotification read(JsonReader in) throws IOException {
   ...
   * @throws IOException if the JSON string is invalid with respect to ReportAvailableNotification
   */
  public static ReportAvailableNotification fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ReportAvailableNotification.class);
  }
