...
public class KYCCheckResult {
   ...
   * @throws IOException if the JSON Element is invalid with respect to KYCCheckResult
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!KYCCheckResult.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in KYCCheckResult is not found in the empty JSON string", KYCCheckResult.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!KYCCheckResult.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `KYCCheckResult` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!KYCCheckResult.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'KYCCheckResult' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<KYCCheckResult> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(KYCCheckResult.class));

       return (TypeAdapter<T>) new TypeAdapter<KYCCheckResult>() {
           @Override
           public void write(JsonWriter out, KYCCheckResult value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public KYCCheckResult read(JsonReader in) throws IOException {
   ...
   * @throws IOException if the JSON string is invalid with respect to KYCCheckResult
   */
  public static KYCCheckResult fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, KYCCheckResult.class);
  }
