...
public class AccountHolderStatusChangeNotification {
   ...
   * @throws IOException if the JSON Element is invalid with respect to AccountHolderStatusChangeNotification
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AccountHolderStatusChangeNotification.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in AccountHolderStatusChangeNotification is not found in the empty JSON string", AccountHolderStatusChangeNotification.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!AccountHolderStatusChangeNotification.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AccountHolderStatusChangeNotification` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : AccountHolderStatusChangeNotification.openapiRequiredFields) {
      ...
      if (jsonObj.get("content") != null && !jsonObj.get("content").isJsonNull()) {
        AccountHolderStatusChangeNotificationContent.validateJsonElement(jsonObj.get("content"));
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!AccountHolderStatusChangeNotification.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'AccountHolderStatusChangeNotification' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<AccountHolderStatusChangeNotification> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(AccountHolderStatusChangeNotification.class));

       return (TypeAdapter<T>) new TypeAdapter<AccountHolderStatusChangeNotification>() {
           @Override
           public void write(JsonWriter out, AccountHolderStatusChangeNotification value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public AccountHolderStatusChangeNotification read(JsonReader in) throws IOException {
   ...
   * @throws IOException if the JSON string is invalid with respect to AccountHolderStatusChangeNotification
   */
  public static AccountHolderStatusChangeNotification fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, AccountHolderStatusChangeNotification.class);
  }
