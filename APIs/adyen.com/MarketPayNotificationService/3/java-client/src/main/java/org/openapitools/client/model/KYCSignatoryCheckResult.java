...
public class KYCSignatoryCheckResult {
   ...
   * @throws IOException if the JSON Element is invalid with respect to KYCSignatoryCheckResult
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!KYCSignatoryCheckResult.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in KYCSignatoryCheckResult is not found in the empty JSON string", KYCSignatoryCheckResult.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!KYCSignatoryCheckResult.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `KYCSignatoryCheckResult` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!KYCSignatoryCheckResult.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'KYCSignatoryCheckResult' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<KYCSignatoryCheckResult> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(KYCSignatoryCheckResult.class));

       return (TypeAdapter<T>) new TypeAdapter<KYCSignatoryCheckResult>() {
           @Override
           public void write(JsonWriter out, KYCSignatoryCheckResult value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public KYCSignatoryCheckResult read(JsonReader in) throws IOException {
   ...
   * @throws IOException if the JSON string is invalid with respect to KYCSignatoryCheckResult
   */
  public static KYCSignatoryCheckResult fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, KYCSignatoryCheckResult.class);
  }
