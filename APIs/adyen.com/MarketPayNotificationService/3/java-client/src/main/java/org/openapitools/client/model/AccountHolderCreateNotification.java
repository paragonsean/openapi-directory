...
public class AccountHolderCreateNotification {
   ...
   * @throws IOException if the JSON Element is invalid with respect to AccountHolderCreateNotification
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AccountHolderCreateNotification.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in AccountHolderCreateNotification is not found in the empty JSON string", AccountHolderCreateNotification.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!AccountHolderCreateNotification.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AccountHolderCreateNotification` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : AccountHolderCreateNotification.openapiRequiredFields) {
      ...
      if (jsonObj.get("content") != null && !jsonObj.get("content").isJsonNull()) {
        CreateAccountHolderResponse.validateJsonElement(jsonObj.get("content"));
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!AccountHolderCreateNotification.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'AccountHolderCreateNotification' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<AccountHolderCreateNotification> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(AccountHolderCreateNotification.class));

       return (TypeAdapter<T>) new TypeAdapter<AccountHolderCreateNotification>() {
           @Override
           public void write(JsonWriter out, AccountHolderCreateNotification value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public AccountHolderCreateNotification read(JsonReader in) throws IOException {
   ...
   * @throws IOException if the JSON string is invalid with respect to AccountHolderCreateNotification
   */
  public static AccountHolderCreateNotification fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, AccountHolderCreateNotification.class);
  }
