...
public class SweepConfigurationNotificationData {
   ...
   * @throws IOException if the JSON Element is invalid with respect to SweepConfigurationNotificationData
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SweepConfigurationNotificationData.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SweepConfigurationNotificationData.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SweepConfigurationNotificationData` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("sweep") != null && !jsonObj.get("sweep").isJsonNull()) {
        SweepConfigurationV2.validateJsonElement(jsonObj.get("sweep"));
      }
  }
