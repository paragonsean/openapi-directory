...
public class SupplyDefenseDocumentResponse {
   ...
   * @throws IOException if the JSON Element is invalid with respect to SupplyDefenseDocumentResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SupplyDefenseDocumentResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in SupplyDefenseDocumentResponse is not found in the empty JSON string", SupplyDefenseDocumentResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SupplyDefenseDocumentResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SupplyDefenseDocumentResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : SupplyDefenseDocumentResponse.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!SupplyDefenseDocumentResponse.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'SupplyDefenseDocumentResponse' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<SupplyDefenseDocumentResponse> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(SupplyDefenseDocumentResponse.class));

       return (TypeAdapter<T>) new TypeAdapter<SupplyDefenseDocumentResponse>() {
           @Override
           public void write(JsonWriter out, SupplyDefenseDocumentResponse value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public SupplyDefenseDocumentResponse read(JsonReader in) throws IOException {
   ...
   * @throws IOException if the JSON string is invalid with respect to SupplyDefenseDocumentResponse
   */
  public static SupplyDefenseDocumentResponse fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, SupplyDefenseDocumentResponse.class);
  }
