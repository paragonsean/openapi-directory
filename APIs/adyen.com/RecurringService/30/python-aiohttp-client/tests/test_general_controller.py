...
async def test_post_disable(client):
    ...
    response = await client.request(
        method='POST',
        path='/pal/servlet/Recurring/v30/disable',
        headers=headers,
        json=body,
        )
...
async def test_post_list_recurring_details(client):
    ...
    response = await client.request(
        method='POST',
        path='/pal/servlet/Recurring/v30/listRecurringDetails',
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_post_notify_shopper(client):
    ...
    response = await client.request(
        method='POST',
        path='/pal/servlet/Recurring/v30/notifyShopper',
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_post_schedule_account_updater(client):
    ...
    response = await client.request(
        method='POST',
        path='/pal/servlet/Recurring/v30/scheduleAccountUpdater',
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
