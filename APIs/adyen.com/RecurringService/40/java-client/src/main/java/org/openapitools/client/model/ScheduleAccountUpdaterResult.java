...
public class ScheduleAccountUpdaterResult {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ScheduleAccountUpdaterResult
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ScheduleAccountUpdaterResult.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ScheduleAccountUpdaterResult is not found in the empty JSON string", ScheduleAccountUpdaterResult.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ScheduleAccountUpdaterResult.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ScheduleAccountUpdaterResult` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ScheduleAccountUpdaterResult.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ScheduleAccountUpdaterResult.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ScheduleAccountUpdaterResult' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ScheduleAccountUpdaterResult> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ScheduleAccountUpdaterResult.class));

       return (TypeAdapter<T>) new TypeAdapter<ScheduleAccountUpdaterResult>() {
           @Override
           public void write(JsonWriter out, ScheduleAccountUpdaterResult value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ScheduleAccountUpdaterResult read(JsonReader in) throws IOException {
