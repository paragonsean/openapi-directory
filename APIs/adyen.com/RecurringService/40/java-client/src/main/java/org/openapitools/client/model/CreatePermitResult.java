...
import org.openapitools.client.model.PermitResult;
...
public class CreatePermitResult {
   ...
   * @throws IOException if the JSON Element is invalid with respect to CreatePermitResult
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CreatePermitResult.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CreatePermitResult.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CreatePermitResult` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      if (jsonObj.get("permitResultList") != null && !jsonObj.get("permitResultList").isJsonNull()) {
        ...
        if (jsonArraypermitResultList != null) {
          ...
          for (int i = 0; i < jsonArraypermitResultList.size(); i++) {
            PermitResult.validateJsonElement(jsonArraypermitResultList.get(i));
          };
        }
      }
