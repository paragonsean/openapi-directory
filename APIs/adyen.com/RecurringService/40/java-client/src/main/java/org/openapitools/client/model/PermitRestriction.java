...
public class PermitRestriction {
   ...
   * @throws IOException if the JSON Element is invalid with respect to PermitRestriction
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PermitRestriction.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PermitRestriction.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PermitRestriction` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("singleTransactionLimit") != null && !jsonObj.get("singleTransactionLimit").isJsonNull()) {
        Amount.validateJsonElement(jsonObj.get("singleTransactionLimit"));
      }
  }
