...
public class PermitResult {
   ...
   * @throws IOException if the JSON Element is invalid with respect to PermitResult
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PermitResult.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PermitResult.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PermitResult` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
