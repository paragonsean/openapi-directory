...
public class DisablePermitResult {
   ...
   * @throws IOException if the JSON Element is invalid with respect to DisablePermitResult
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DisablePermitResult.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DisablePermitResult.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DisablePermitResult` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
