...
> DisablePermitResult postDisablePermit(opts)
...
let apiInstance = new AdyenRecurringApi.GeneralApi();
let opts = {
  'disablePermitRequest': new AdyenRecurringApi.DisablePermitRequest() // DisablePermitRequest | 
};
apiInstance.postDisablePermit(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
