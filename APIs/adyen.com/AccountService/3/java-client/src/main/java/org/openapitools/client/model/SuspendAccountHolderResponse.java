...
public class SuspendAccountHolderResponse {
   ...
   * @throws IOException if the JSON Element is invalid with respect to SuspendAccountHolderResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SuspendAccountHolderResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in SuspendAccountHolderResponse is not found in the empty JSON string", SuspendAccountHolderResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SuspendAccountHolderResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SuspendAccountHolderResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!SuspendAccountHolderResponse.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'SuspendAccountHolderResponse' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<SuspendAccountHolderResponse> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(SuspendAccountHolderResponse.class));

       return (TypeAdapter<T>) new TypeAdapter<SuspendAccountHolderResponse>() {
           @Override
           public void write(JsonWriter out, SuspendAccountHolderResponse value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public SuspendAccountHolderResponse read(JsonReader in) throws IOException {
   ...
   * @throws IOException if the JSON string is invalid with respect to SuspendAccountHolderResponse
   */
  public static SuspendAccountHolderResponse fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, SuspendAccountHolderResponse.class);
  }
