...
public class KYCPayoutMethodCheckResult {
   ...
   * @throws IOException if the JSON Element is invalid with respect to KYCPayoutMethodCheckResult
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!KYCPayoutMethodCheckResult.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in KYCPayoutMethodCheckResult is not found in the empty JSON string", KYCPayoutMethodCheckResult.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!KYCPayoutMethodCheckResult.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `KYCPayoutMethodCheckResult` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!KYCPayoutMethodCheckResult.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'KYCPayoutMethodCheckResult' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<KYCPayoutMethodCheckResult> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(KYCPayoutMethodCheckResult.class));

       return (TypeAdapter<T>) new TypeAdapter<KYCPayoutMethodCheckResult>() {
           @Override
           public void write(JsonWriter out, KYCPayoutMethodCheckResult value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public KYCPayoutMethodCheckResult read(JsonReader in) throws IOException {
   ...
   * @throws IOException if the JSON string is invalid with respect to KYCPayoutMethodCheckResult
   */
  public static KYCPayoutMethodCheckResult fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, KYCPayoutMethodCheckResult.class);
  }
