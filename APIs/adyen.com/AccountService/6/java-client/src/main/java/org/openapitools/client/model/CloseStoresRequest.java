...
public class CloseStoresRequest {
   ...
   * @throws IOException if the JSON Element is invalid with respect to CloseStoresRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CloseStoresRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in CloseStoresRequest is not found in the empty JSON string", CloseStoresRequest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CloseStoresRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CloseStoresRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : CloseStoresRequest.openapiRequiredFields) {
      ...
      } else if (!jsonObj.get("stores").isJsonArray()) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!CloseStoresRequest.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'CloseStoresRequest' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<CloseStoresRequest> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(CloseStoresRequest.class));

       return (TypeAdapter<T>) new TypeAdapter<CloseStoresRequest>() {
           @Override
           public void write(JsonWriter out, CloseStoresRequest value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public CloseStoresRequest read(JsonReader in) throws IOException {
   ...
   * @return An instance of CloseStoresRequest
   * @throws IOException if the JSON string is invalid with respect to CloseStoresRequest
   */
  public static CloseStoresRequest fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, CloseStoresRequest.class);
  }
