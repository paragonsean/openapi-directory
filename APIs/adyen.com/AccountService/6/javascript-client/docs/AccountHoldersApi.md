...
> GenericResponse postCloseStores(opts)
...
let apiInstance = new AccountApi.AccountHoldersApi();
let opts = {
  'closeStoresRequest': new AccountApi.CloseStoresRequest() // CloseStoresRequest | 
};
apiInstance.postCloseStores(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
