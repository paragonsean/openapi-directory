...
public class UltimateParentCompany {
   ...
   * @throws IOException if the JSON Element is invalid with respect to UltimateParentCompany
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UltimateParentCompany.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in UltimateParentCompany is not found in the empty JSON string", UltimateParentCompany.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!UltimateParentCompany.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UltimateParentCompany` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("businessDetails") != null && !jsonObj.get("businessDetails").isJsonNull()) {
        UltimateParentCompanyBusinessDetails.validateJsonElement(jsonObj.get("businessDetails"));
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!UltimateParentCompany.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'UltimateParentCompany' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<UltimateParentCompany> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(UltimateParentCompany.class));

       return (TypeAdapter<T>) new TypeAdapter<UltimateParentCompany>() {
           @Override
           public void write(JsonWriter out, UltimateParentCompany value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public UltimateParentCompany read(JsonReader in) throws IOException {
   ...
   * @throws IOException if the JSON string is invalid with respect to UltimateParentCompany
   */
  public static UltimateParentCompany fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, UltimateParentCompany.class);
  }
