...
public class VoidPendingRefundRequest {
  ...
  public VoidPendingRefundRequest additionalData(AdjustAuthorisationRequestAdditionalData additionalData) {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(additionalData, merchantAccount, modificationAmount, originalMerchantReference, originalReference, reference, tenderReference, uniqueTerminalId);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to VoidPendingRefundRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!VoidPendingRefundRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in VoidPendingRefundRequest is not found in the empty JSON string", VoidPendingRefundRequest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!VoidPendingRefundRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `VoidPendingRefundRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : VoidPendingRefundRequest.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!VoidPendingRefundRequest.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'VoidPendingRefundRequest' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<VoidPendingRefundRequest> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(VoidPendingRefundRequest.class));

       return (TypeAdapter<T>) new TypeAdapter<VoidPendingRefundRequest>() {
           @Override
           public void write(JsonWriter out, VoidPendingRefundRequest value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public VoidPendingRefundRequest read(JsonReader in) throws IOException {
