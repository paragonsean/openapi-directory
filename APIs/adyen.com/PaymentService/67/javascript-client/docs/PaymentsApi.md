...
All URIs are relative to *https://pal-test.adyen.com/pal/servlet/Payment/v67*
...
> AuthenticationResultResponse postGetAuthenticationResult(opts)
...
let apiInstance = new AdyenPaymentApi.PaymentsApi();
let opts = {
  'authenticationResultRequest': new AdyenPaymentApi.AuthenticationResultRequest() // AuthenticationResultRequest | 
};
apiInstance.postGetAuthenticationResult(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
