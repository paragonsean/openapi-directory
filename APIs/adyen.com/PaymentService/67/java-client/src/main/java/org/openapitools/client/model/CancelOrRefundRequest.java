...
public class CancelOrRefundRequest {
  ...
  public CancelOrRefundRequest additionalData(AdjustAuthorisationRequestAdditionalData additionalData) {
   ...
   * @throws IOException if the JSON Element is invalid with respect to CancelOrRefundRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CancelOrRefundRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CancelOrRefundRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CancelOrRefundRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
