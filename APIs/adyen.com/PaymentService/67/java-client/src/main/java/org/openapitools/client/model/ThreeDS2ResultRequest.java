...
public class ThreeDS2ResultRequest {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ThreeDS2ResultRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ThreeDS2ResultRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ThreeDS2ResultRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ThreeDS2ResultRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
