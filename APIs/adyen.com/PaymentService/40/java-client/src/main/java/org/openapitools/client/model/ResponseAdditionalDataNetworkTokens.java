...
public class ResponseAdditionalDataNetworkTokens {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ResponseAdditionalDataNetworkTokens
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResponseAdditionalDataNetworkTokens.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ResponseAdditionalDataNetworkTokens.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResponseAdditionalDataNetworkTokens` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      if ((jsonObj.get("networkToken.available") != null && !jsonObj.get("networkToken.available").isJsonNull()) && !jsonObj.get("networkToken.available").isJsonPrimitive()) {
      ...
      if ((jsonObj.get("networkToken.bin") != null && !jsonObj.get("networkToken.bin").isJsonNull()) && !jsonObj.get("networkToken.bin").isJsonPrimitive()) {
      ...
      if ((jsonObj.get("networkToken.tokenSummary") != null && !jsonObj.get("networkToken.tokenSummary").isJsonNull()) && !jsonObj.get("networkToken.tokenSummary").isJsonPrimitive()) {
