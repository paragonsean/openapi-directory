...
public class DetailBalance {
  ...
  public static final String SERIALIZED_NAME_ON_HOLD_BALANCE = "onHoldBalance";
  @SerializedName(SERIALIZED_NAME_ON_HOLD_BALANCE)
  private List<Amount> onHoldBalance = new ArrayList<>();

  public static final String SERIALIZED_NAME_PENDING_BALANCE = "pendingBalance";
  @SerializedName(SERIALIZED_NAME_PENDING_BALANCE)
  private List<Amount> pendingBalance = new ArrayList<>();
  ...
  static {
    ...
    openapiFields.add("onHoldBalance");
      ...
      if (jsonObj.get("balance") != null && !jsonObj.get("balance").isJsonNull()) {
        ...
        if (jsonArraybalance != null) {
          ...
          for (int i = 0; i < jsonArraybalance.size(); i++) {
            Amount.validateJsonElement(jsonArraybalance.get(i));
          };
        }
      }
      if (jsonObj.get("onHoldBalance") != null && !jsonObj.get("onHoldBalance").isJsonNull()) {
        JsonArray jsonArrayonHoldBalance = jsonObj.getAsJsonArray("onHoldBalance");
        if (jsonArrayonHoldBalance != null) {
          // ensure the json data is an array
          if (!jsonObj.get("onHoldBalance").isJsonArray()) {
            throw new IllegalArgumentException(String.format("Expected the field `onHoldBalance` to be an array in the JSON string but got `%s`", jsonObj.get("onHoldBalance").toString()));
          }

          // validate the optional field `onHoldBalance` (array)
          for (int i = 0; i < jsonArrayonHoldBalance.size(); i++) {
            Amount.validateJsonElement(jsonArrayonHoldBalance.get(i));
          };
        }
      }
      if (jsonObj.get("pendingBalance") != null && !jsonObj.get("pendingBalance").isJsonNull()) {
        ...
        if (jsonArraypendingBalance != null) {
          ...
          for (int i = 0; i < jsonArraypendingBalance.size(); i++) {
            Amount.validateJsonElement(jsonArraypendingBalance.get(i));
          };
        }
      }
  }
