...
public class AccountTransactionList {
   ...
   * @throws IOException if the JSON Element is invalid with respect to AccountTransactionList
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AccountTransactionList.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in AccountTransactionList is not found in the empty JSON string", AccountTransactionList.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!AccountTransactionList.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AccountTransactionList` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!AccountTransactionList.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'AccountTransactionList' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<AccountTransactionList> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(AccountTransactionList.class));

       return (TypeAdapter<T>) new TypeAdapter<AccountTransactionList>() {
           @Override
           public void write(JsonWriter out, AccountTransactionList value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public AccountTransactionList read(JsonReader in) throws IOException {
   ...
   * @throws IOException if the JSON string is invalid with respect to AccountTransactionList
   */
  public static AccountTransactionList fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, AccountTransactionList.class);
  }
