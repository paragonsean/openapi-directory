...
public class StoredValueLoadRequest {
   ...
   * @throws IOException if the JSON Element is invalid with respect to StoredValueLoadRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!StoredValueLoadRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!StoredValueLoadRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `StoredValueLoadRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("loadType") != null && !jsonObj.get("loadType").isJsonNull()) && !jsonObj.get("loadType").isJsonPrimitive()) {
      ...
      // validate the optional field `loadType`
      if (jsonObj.get("loadType") != null && !jsonObj.get("loadType").isJsonNull()) {
        LoadTypeEnum.validateJsonElement(jsonObj.get("loadType"));
      }
