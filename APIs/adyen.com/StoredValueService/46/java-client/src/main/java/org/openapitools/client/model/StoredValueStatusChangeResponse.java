...
public class StoredValueStatusChangeResponse {
   ...
   * @throws IOException if the JSON Element is invalid with respect to StoredValueStatusChangeResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!StoredValueStatusChangeResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!StoredValueStatusChangeResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `StoredValueStatusChangeResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
