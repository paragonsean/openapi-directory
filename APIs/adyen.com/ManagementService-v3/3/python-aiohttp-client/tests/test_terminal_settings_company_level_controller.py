...
async def test_get_companies_company_id_terminal_logos(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/companies/{company_id}/terminalLogos'.format(company_id='company_id_example'),
        headers=headers,
        params=params,
        )
...
async def test_get_companies_company_id_terminal_settings(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/companies/{company_id}/terminalSettings'.format(company_id='company_id_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_patch_companies_company_id_terminal_logos(client):
...
async def test_patch_companies_company_id_terminal_settings(client):
