...
async def test_get_merchants_merchant_id_terminal_logos(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/merchants/{merchant_id}/terminalLogos'.format(merchant_id='merchant_id_example'),
        headers=headers,
        params=params,
        )
...
async def test_get_merchants_merchant_id_terminal_settings(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/merchants/{merchant_id}/terminalSettings'.format(merchant_id='merchant_id_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_patch_merchants_merchant_id_terminal_logos(client):
...
async def test_patch_merchants_merchant_id_terminal_settings(client):
