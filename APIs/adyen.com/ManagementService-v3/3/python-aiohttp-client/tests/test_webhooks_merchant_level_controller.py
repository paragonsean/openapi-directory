...
async def test_delete_merchants_merchant_id_webhooks_webhook_id(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/v3/merchants/{merchant_id}/webhooks/{webhook_id}'.format(merchant_id='merchant_id_example', webhook_id='webhook_id_example'),
        headers=headers,
        )
...
async def test_get_merchants_merchant_id_webhooks(client):
    ...
    response = await client.request(
        method='GET',
        path='/v3/merchants/{merchant_id}/webhooks'.format(merchant_id='merchant_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_patch_merchants_merchant_id_webhooks_webhook_id(client):
    ...
    Update a webhook
    """
    body = {"communicationFormat":"http","acceptsUntrustedRootCertificate":True,"active":True,"description":"description","acceptsSelfSignedCertificate":True,"url":"url","password":"password","additionalSettings":{"includeEventCodes":["includeEventCodes","includeEventCodes"],"properties":{"key":True}},"acceptsExpiredCertificate":True,"encryptionProtocol":"HTTP","networkType":"local","populateSoapActionHeader":True,"username":"username"}
    headers = { 
        'Accept': 'application/json',
...
async def test_post_merchants_merchant_id_webhooks_webhook_id_generate_hmac(client):
    ...
    response = await client.request(
        method='POST',
        path='/v3/merchants/{merchant_id}/webhooks/{webhook_id}/generateHmac'.format(merchant_id='merchant_id_example', webhook_id='webhook_id_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_post_merchants_merchant_id_webhooks_webhook_id_test(client):
    ...
    response = await client.request(
        method='POST',
        path='/v3/merchants/{merchant_id}/webhooks/{webhook_id}/test'.format(merchant_id='merchant_id_example', webhook_id='webhook_id_example'),
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
