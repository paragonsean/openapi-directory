...
public class ReleaseUpdateDetails {
  ...
  private TypeEnum type = TypeEnum.RELEASE_UPDATE;
   ...
   * @throws IOException if the JSON Element is invalid with respect to ReleaseUpdateDetails
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ReleaseUpdateDetails.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ReleaseUpdateDetails.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ReleaseUpdateDetails` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
