...
public class TerminalConnectivityEthernet {
   ...
   * @throws IOException if the JSON Element is invalid with respect to TerminalConnectivityEthernet
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!TerminalConnectivityEthernet.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in TerminalConnectivityEthernet is not found in the empty JSON string", TerminalConnectivityEthernet.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!TerminalConnectivityEthernet.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TerminalConnectivityEthernet` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!TerminalConnectivityEthernet.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'TerminalConnectivityEthernet' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<TerminalConnectivityEthernet> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(TerminalConnectivityEthernet.class));

       return (TypeAdapter<T>) new TypeAdapter<TerminalConnectivityEthernet>() {
           @Override
           public void write(JsonWriter out, TerminalConnectivityEthernet value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public TerminalConnectivityEthernet read(JsonReader in) throws IOException {
   ...
   * @throws IOException if the JSON string is invalid with respect to TerminalConnectivityEthernet
   */
  public static TerminalConnectivityEthernet fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, TerminalConnectivityEthernet.class);
  }
