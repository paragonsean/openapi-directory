...
public class Opi {
   ...
   * @throws IOException if the JSON Element is invalid with respect to Opi
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Opi.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Opi.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Opi` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      if ((jsonObj.get("payAtTableStoreNumber") != null && !jsonObj.get("payAtTableStoreNumber").isJsonNull()) && !jsonObj.get("payAtTableStoreNumber").isJsonPrimitive()) {
      ...
      if ((jsonObj.get("payAtTableURL") != null && !jsonObj.get("payAtTableURL").isJsonNull()) && !jsonObj.get("payAtTableURL").isJsonPrimitive()) {
