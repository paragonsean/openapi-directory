...
public class UpdateCompanyWebhookRequest {
   ...
   * @throws IOException if the JSON Element is invalid with respect to UpdateCompanyWebhookRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UpdateCompanyWebhookRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!UpdateCompanyWebhookRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UpdateCompanyWebhookRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("communicationFormat") != null && !jsonObj.get("communicationFormat").isJsonNull()) && !jsonObj.get("communicationFormat").isJsonPrimitive()) {
      ...
      // validate the optional field `communicationFormat`
      if (jsonObj.get("communicationFormat") != null && !jsonObj.get("communicationFormat").isJsonNull()) {
