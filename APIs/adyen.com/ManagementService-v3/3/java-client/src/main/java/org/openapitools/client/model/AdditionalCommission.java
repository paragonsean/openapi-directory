...
public class AdditionalCommission {
   ...
   * @throws IOException if the JSON Element is invalid with respect to AdditionalCommission
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AdditionalCommission.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in AdditionalCommission is not found in the empty JSON string", AdditionalCommission.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!AdditionalCommission.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AdditionalCommission` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!AdditionalCommission.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'AdditionalCommission' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<AdditionalCommission> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(AdditionalCommission.class));

       return (TypeAdapter<T>) new TypeAdapter<AdditionalCommission>() {
           @Override
           public void write(JsonWriter out, AdditionalCommission value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public AdditionalCommission read(JsonReader in) throws IOException {
   ...
   * @throws IOException if the JSON string is invalid with respect to AdditionalCommission
   */
  public static AdditionalCommission fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, AdditionalCommission.class);
  }
