...
public class TerminalOrder {
   ...
   * @throws IOException if the JSON Element is invalid with respect to TerminalOrder
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!TerminalOrder.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!TerminalOrder.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TerminalOrder` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // validate the optional field `billingEntity`
      if (jsonObj.get("billingEntity") != null && !jsonObj.get("billingEntity").isJsonNull()) {
        BillingEntity.validateJsonElement(jsonObj.get("billingEntity"));
      }
      ...
      if (jsonObj.get("shippingLocation") != null && !jsonObj.get("shippingLocation").isJsonNull()) {
        ShippingLocation.validateJsonElement(jsonObj.get("shippingLocation"));
      }
