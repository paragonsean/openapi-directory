...
public class UninstallAndroidCertificateDetails {
  ...
  private TypeEnum type = TypeEnum.UNINSTALL_ANDROID_CERTIFICATE;
   ...
   * @throws IOException if the JSON Element is invalid with respect to UninstallAndroidCertificateDetails
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UninstallAndroidCertificateDetails.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!UninstallAndroidCertificateDetails.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UninstallAndroidCertificateDetails` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
