...
public class InstallAndroidCertificateDetails {
  ...
  private TypeEnum type = TypeEnum.INSTALL_ANDROID_CERTIFICATE;
   ...
   * @throws IOException if the JSON Element is invalid with respect to InstallAndroidCertificateDetails
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!InstallAndroidCertificateDetails.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!InstallAndroidCertificateDetails.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `InstallAndroidCertificateDetails` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
