...
public class TestCompanyWebhookRequest {
   ...
   * @throws IOException if the JSON Element is invalid with respect to TestCompanyWebhookRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!TestCompanyWebhookRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!TestCompanyWebhookRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TestCompanyWebhookRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // ensure the optional json data is an array if present
      if (jsonObj.get("merchantIds") != null && !jsonObj.get("merchantIds").isJsonNull() && !jsonObj.get("merchantIds").isJsonArray()) {
