...
public class CreateCompanyWebhookRequest {
   ...
   * @throws IOException if the JSON Element is invalid with respect to CreateCompanyWebhookRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CreateCompanyWebhookRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CreateCompanyWebhookRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CreateCompanyWebhookRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      // validate the required field `filterMerchantAccountType`
      FilterMerchantAccountTypeEnum.validateJsonElement(jsonObj.get("filterMerchantAccountType"));
      // ensure the required json array is present
      if (jsonObj.get("filterMerchantAccounts") == null) {
        throw new IllegalArgumentException("Expected the field `linkedContent` to be an array in the JSON string but got `null`");
      } else if (!jsonObj.get("filterMerchantAccounts").isJsonArray()) {
