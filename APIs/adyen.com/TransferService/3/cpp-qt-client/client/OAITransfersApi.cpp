...
void OAITransfersApi::initializeServerConfigs() {
    ...
    _serverConfigs.insert("postTransfersTransferIdReturns", defaultConf);
    _serverIndices.insert("postTransfersTransferIdReturns", 0);
}
...
void OAITransfersApi::postTransfersTransferIdReturns(const QString &transfer_id, const ::OpenAPI::OptionalParam<OAIReturnTransferRequest> &oai_return_transfer_request) {
    QString fullPath = QString(_serverConfigs["postTransfersTransferIdReturns"][_serverIndices.value("postTransfersTransferIdReturns")].URL()+"/transfers/{transferId}/returns");
    ...
    if (oai_return_transfer_request.hasValue()){

        
        QByteArray output = oai_return_transfer_request.value().asJson().toUtf8();
        input.request_body.append(output);
    }
    for (auto keyValueIt = _defaultHeaders.keyValueBegin(); keyValueIt != _defaultHeaders.keyValueEnd(); keyValueIt++) {
        input.headers.insert(keyValueIt->first, keyValueIt->second);
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAITransfersApi::postTransfersTransferIdReturnsCallback);
    connect(this, &OAITransfersApi::abortRequestsSignal, worker, &QObject::deleteLater);
    connect(worker, &QObject::destroyed, this, [this]() {
        if (findChildren<OAIHttpRequestWorker*>().count() == 0) {
            Q_EMIT allPendingRequestsCompleted();
...
void OAITransfersApi::postTransfersTransferIdReturnsCallback(OAIHttpRequestWorker *worker) {
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
    ...
    OAIReturnTransferResponse output(QString(worker->response));
    worker->deleteLater();

    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT postTransfersTransferIdReturnsSignal(output);
        Q_EMIT postTransfersTransferIdReturnsSignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT postTransfersTransferIdReturnsSignalE(output, error_type, error_str);
        Q_EMIT postTransfersTransferIdReturnsSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT postTransfersTransferIdReturnsSignalError(output, error_type, error_str);
        Q_EMIT postTransfersTransferIdReturnsSignalErrorFull(worker, error_type, error_str);
    }
}
