...
import org.openapitools.client.model.BankAccountV3AccountIdentification;
...
public class BankAccountV3 {
   ...
   * @throws IOException if the JSON Element is invalid with respect to BankAccountV3
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!BankAccountV3.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!BankAccountV3.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BankAccountV3` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      PartyIdentification.validateJsonElement(jsonObj.get("accountHolder"));
      // validate the required field `accountIdentification`
      BankAccountV3AccountIdentification.validateJsonElement(jsonObj.get("accountIdentification"));
  }
