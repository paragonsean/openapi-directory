...
public class CapitalGrants {
   ...
   * @throws IOException if the JSON Element is invalid with respect to CapitalGrants
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CapitalGrants.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CapitalGrants.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CapitalGrants` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArraygrants.size(); i++) {
        CapitalGrant.validateJsonElement(jsonArraygrants.get(i));
      };
  }
