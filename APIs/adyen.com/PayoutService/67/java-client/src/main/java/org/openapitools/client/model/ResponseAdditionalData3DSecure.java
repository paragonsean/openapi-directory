...
public class ResponseAdditionalData3DSecure {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ResponseAdditionalData3DSecure
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResponseAdditionalData3DSecure.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ResponseAdditionalData3DSecure.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResponseAdditionalData3DSecure` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("scaExemptionRequested") != null && !jsonObj.get("scaExemptionRequested").isJsonNull()) && !jsonObj.get("scaExemptionRequested").isJsonPrimitive()) {
