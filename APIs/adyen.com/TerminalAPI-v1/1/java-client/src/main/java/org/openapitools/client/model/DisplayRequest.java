...
public class DisplayRequest {
  ...
  static {
    ...
    openapiRequiredFields.add("DisplayOutput");
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to DisplayRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DisplayRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in DisplayRequest is not found in the empty JSON string", DisplayRequest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DisplayRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DisplayRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : DisplayRequest.openapiRequiredFields) {
      ...
      // validate the required field `DisplayOutput` (array)
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!DisplayRequest.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'DisplayRequest' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<DisplayRequest> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(DisplayRequest.class));

       return (TypeAdapter<T>) new TypeAdapter<DisplayRequest>() {
           @Override
           public void write(JsonWriter out, DisplayRequest value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public DisplayRequest read(JsonReader in) throws IOException {
   ...
   * @throws IOException if the JSON string is invalid with respect to DisplayRequest
   */
  public static DisplayRequest fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, DisplayRequest.class);
  }
