...
public class POIData {
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(this.poIReconciliationID, poIData.poIReconciliationID) &&
        Objects.equals(this.poITransactionID, poIData.poITransactionID);
  }

  @Override
  public int hashCode() {
    return Objects.hash(poIReconciliationID, poITransactionID);
  }
  ...
  static {
    ...
    openapiRequiredFields.add("POITransactionID");
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to POIData
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!POIData.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in POIData is not found in the empty JSON string", POIData.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!POIData.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `POIData` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : POIData.openapiRequiredFields) {
        if (jsonElement.getAsJsonObject().get(requiredField) == null) {
          throw new IllegalArgumentException(String.format("The required field `%s` is not found in the JSON string: %s", requiredField, jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // validate the required field `POITransactionID`
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!POIData.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'POIData' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<POIData> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(POIData.class));

       return (TypeAdapter<T>) new TypeAdapter<POIData>() {
           @Override
           public void write(JsonWriter out, POIData value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public POIData read(JsonReader in) throws IOException {
   ...
   * @throws IOException if the JSON string is invalid with respect to POIData
   */
  public static POIData fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, POIData.class);
  }
