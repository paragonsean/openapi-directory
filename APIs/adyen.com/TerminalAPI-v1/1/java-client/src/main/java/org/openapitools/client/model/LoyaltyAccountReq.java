...
public class LoyaltyAccountReq {
   ...
   * @throws IOException if the JSON Element is invalid with respect to LoyaltyAccountReq
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!LoyaltyAccountReq.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in LoyaltyAccountReq is not found in the empty JSON string", LoyaltyAccountReq.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!LoyaltyAccountReq.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `LoyaltyAccountReq` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!LoyaltyAccountReq.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'LoyaltyAccountReq' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<LoyaltyAccountReq> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(LoyaltyAccountReq.class));

       return (TypeAdapter<T>) new TypeAdapter<LoyaltyAccountReq>() {
           @Override
           public void write(JsonWriter out, LoyaltyAccountReq value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public LoyaltyAccountReq read(JsonReader in) throws IOException {
   ...
   * @throws IOException if the JSON string is invalid with respect to LoyaltyAccountReq
   */
  public static LoyaltyAccountReq fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, LoyaltyAccountReq.class);
  }
