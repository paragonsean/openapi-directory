...
public class MessageSentTo extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(MessageSentTo.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!MessageSentTo.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'MessageSentTo' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<MessageSentTo>() {
                @Override
                public void write(JsonWriter out, MessageSentTo value) throws IOException {
                ...
                @Override
                public MessageSentTo read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        MessageSentTo ret = new MessageSentTo();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonElement));
                        return ret;
                    }

                    throw new IOException(String.format("Failed deserialization for MessageSentTo: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to MessageSentTo
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            throw new IOException(String.format("The JSON string is invalid for MessageSentTo with oneOf schemas: List<String>, String. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }

    /**
     * Create an instance of MessageSentTo given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of MessageSentTo
     * @throws IOException if the JSON string is invalid with respect to MessageSentTo
     */
    public static MessageSentTo fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, MessageSentTo.class);
    }

    /**
     * Convert an instance of MessageSentTo to an JSON string
     *
     * @return JSON string
     */
