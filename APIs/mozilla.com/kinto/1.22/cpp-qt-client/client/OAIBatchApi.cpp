...
void OAIBatchApi::initializeServerConfigs() {
    ...
    _serverConfigs.insert("batch", defaultConf);
    _serverIndices.insert("batch", 0);
}
...
void OAIBatchApi::batch(const OAIBatchPayloadSchema &oai_batch_payload_schema) {
    QString fullPath = QString(_serverConfigs["batch"][_serverIndices.value("batch")].URL()+"/batch");
...

    {
        ...
        QByteArray output = oai_batch_payload_schema.asJson().toUtf8();
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIBatchApi::batchCallback);
...
void OAIBatchApi::batchCallback(OAIHttpRequestWorker *worker) {
    ...
    OAIBatchResponseBodySchema output(QString(worker->response));
    worker->deleteLater();

    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT batchSignal(output);
        Q_EMIT batchSignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT batchSignalE(output, error_type, error_str);
        Q_EMIT batchSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT batchSignalError(output, error_type, error_str);
        Q_EMIT batchSignalErrorFull(worker, error_type, error_str);
    }
}
