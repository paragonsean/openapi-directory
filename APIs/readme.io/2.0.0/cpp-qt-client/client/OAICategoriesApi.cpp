...
void OAICategoriesApi::initializeServerConfigs() {
    ...
    _serverConfigs.insert("getCategoryDocs", defaultConf);
    _serverIndices.insert("getCategoryDocs", 0);
}
...
void OAICategoriesApi::getCategory(const QString &slug, const QString &x_readme_version) {
    QString fullPath = QString(_serverConfigs["getCategory"][_serverIndices.value("getCategory")].URL()+"/categories/{slug}");
...
void OAICategoriesApi::getCategoryCallback(OAIHttpRequestWorker *worker) {
    ...
    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT getCategorySignal();
        Q_EMIT getCategorySignalFull(worker);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT getCategorySignalE(error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT getCategorySignalError(error_type, error_str);
        Q_EMIT getCategorySignalErrorFull(worker, error_type, error_str);
    }
}

void OAICategoriesApi::getCategoryDocs(const QString &slug, const QString &x_readme_version) {
    QString fullPath = QString(_serverConfigs["getCategoryDocs"][_serverIndices.value("getCategoryDocs")].URL()+"/categories/{slug}/docs");
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAICategoriesApi::getCategoryDocsCallback);
    connect(this, &OAICategoriesApi::abortRequestsSignal, worker, &QObject::deleteLater);
    connect(worker, &QObject::destroyed, this, [this]() {
        if (findChildren<OAIHttpRequestWorker*>().count() == 0) {
            Q_EMIT allPendingRequestsCompleted();
...
void OAICategoriesApi::getCategoryDocsCallback(OAIHttpRequestWorker *worker) {
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
    ...
    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT getCategoryDocsSignal();
        Q_EMIT getCategoryDocsSignalFull(worker);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT getCategoryDocsSignalE(error_type, error_str);
        Q_EMIT getCategoryDocsSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT getCategoryDocsSignalError(error_type, error_str);
        Q_EMIT getCategoryDocsSignalErrorFull(worker, error_type, error_str);
    }
}
