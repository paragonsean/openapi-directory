

# SecurityRights


## Properties

| Name | Type | Description | Notes |
|------------ | ------------- | ------------- | -------------|
|**canAnnotate** | **Boolean** |  |  [optional] |
|**canDelete** | **Boolean** |  |  [optional] |
|**canExecute** | **Boolean** |  |  [optional] |
|**canRead** | **Boolean** |  |  [optional] |
|**canReadData** | **Boolean** |  |  [optional] |
|**canSubscribe** | **Boolean** |  |  [optional] |
|**canSubscribeOthers** | **Boolean** |  |  [optional] |
|**canWrite** | **Boolean** |  |  [optional] |
|**canWriteData** | **Boolean** |  |  [optional] |
|**hasAdmin** | **Boolean** |  |  [optional] |
|**links** | [**SecurityRightsLinks**](SecurityRightsLinks.md) |  |  [optional] |
|**ownerWebId** | **String** |  |  [optional] |
|**rights** | **List&lt;String&gt;** |  |  [optional] |
|**securityItem** | **String** |  |  [optional] |
|**userIdentity** | **String** |  |  [optional] |
|**webException** | [**WebException**](WebException.md) |  |  [optional] |



