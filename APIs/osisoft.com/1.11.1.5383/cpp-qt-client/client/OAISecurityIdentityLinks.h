/**
 * PI Web API 2018 SP1 Swagger Spec
 * Swagger Spec file that describes PI Web API
 *
 * The version of the OpenAPI document: 1.11.1.5383
 * Contact: techsupport@osisoft.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAISecurityIdentityLinks.h
 *
 * 
 */

#ifndef OAISecurityIdentityLinks_H
#define OAISecurityIdentityLinks_H

#include <QJsonObject>

#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {

class OAISecurityIdentityLinks : public OAIObject {
public:
    OAISecurityIdentityLinks();
    OAISecurityIdentityLinks(QString json);
    ~OAISecurityIdentityLinks() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QString getAssetServer() const;
    void setAssetServer(const QString &asset_server);
    bool is_asset_server_Set() const;
    bool is_asset_server_Valid() const;

    QString getSecurity() const;
    void setSecurity(const QString &security);
    bool is_security_Set() const;
    bool is_security_Valid() const;

    QString getSecurityEntries() const;
    void setSecurityEntries(const QString &security_entries);
    bool is_security_entries_Set() const;
    bool is_security_entries_Valid() const;

    QString getSecurityMappings() const;
    void setSecurityMappings(const QString &security_mappings);
    bool is_security_mappings_Set() const;
    bool is_security_mappings_Valid() const;

    QString getSelf() const;
    void setSelf(const QString &self);
    bool is_self_Set() const;
    bool is_self_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString m_asset_server;
    bool m_asset_server_isSet;
    bool m_asset_server_isValid;

    QString m_security;
    bool m_security_isSet;
    bool m_security_isValid;

    QString m_security_entries;
    bool m_security_entries_isSet;
    bool m_security_entries_isValid;

    QString m_security_mappings;
    bool m_security_mappings_isSet;
    bool m_security_mappings_isValid;

    QString m_self;
    bool m_self_isSet;
    bool m_self_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAISecurityIdentityLinks)

#endif // OAISecurityIdentityLinks_H
