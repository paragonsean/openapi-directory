...
public class Boundaries {
  ...
  private List<Boundary> features = new ArrayList<>();
  ...
  public Boundaries features(List<Boundary> features) {
    this.features = features;
    return this;
  }

  public Boundaries addFeaturesItem(Boundary featuresItem) {
  ...
  public Boundaries type(TypeEnum type) {
    this.type = type;
    return this;
  }
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(this.features, boundaries.features) &&
        Objects.equals(this.type, boundaries.type);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to Boundaries
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Boundaries.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in Boundaries is not found in the empty JSON string", Boundaries.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Boundaries.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Boundaries` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : Boundaries.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayfeatures.size(); i++) {
        Boundary.validateJsonElement(jsonArrayfeatures.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!Boundaries.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'Boundaries' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<Boundaries> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(Boundaries.class));

       return (TypeAdapter<T>) new TypeAdapter<Boundaries>() {
           @Override
           public void write(JsonWriter out, Boundaries value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public Boundaries read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of Boundaries given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of Boundaries
   * @throws IOException if the JSON string is invalid with respect to Boundaries
   */
  public static Boundaries fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, Boundaries.class);
  }

  /**
   * Convert an instance of Boundaries to an JSON string
   *
   * @return JSON string
   */
