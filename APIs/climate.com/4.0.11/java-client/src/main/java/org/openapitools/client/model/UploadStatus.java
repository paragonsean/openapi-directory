...
public class UploadStatus {
  ...
  public UploadStatus id(UUID id) {
    this.id = id;
    return this;
  }
  ...
  public UploadStatus status(StatusEnum status) {
    this.status = status;
    return this;
  }
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(this.id, uploadStatus.id) &&
        Objects.equals(this.status, uploadStatus.status);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to UploadStatus
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UploadStatus.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in UploadStatus is not found in the empty JSON string", UploadStatus.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!UploadStatus.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UploadStatus` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : UploadStatus.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!UploadStatus.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'UploadStatus' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<UploadStatus> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(UploadStatus.class));

       return (TypeAdapter<T>) new TypeAdapter<UploadStatus>() {
           @Override
           public void write(JsonWriter out, UploadStatus value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public UploadStatus read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of UploadStatus given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of UploadStatus
   * @throws IOException if the JSON string is invalid with respect to UploadStatus
   */
  public static UploadStatus fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, UploadStatus.class);
  }

  /**
   * Convert an instance of UploadStatus to an JSON string
   *
   * @return JSON string
   */
