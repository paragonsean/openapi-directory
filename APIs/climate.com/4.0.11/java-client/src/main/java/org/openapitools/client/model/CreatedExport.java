...
public class CreatedExport {
  ...
  public CreatedExport() {
  }

  public CreatedExport id(UUID id) {
    this.id = id;
    return this;
  }
  ...
  public boolean equals(Object o) {
    ...
    CreatedExport createdExport = (CreatedExport) o;
    return Objects.equals(this.id, createdExport.id);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to CreatedExport
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CreatedExport.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in CreatedExport is not found in the empty JSON string", CreatedExport.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CreatedExport.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CreatedExport` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : CreatedExport.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!CreatedExport.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'CreatedExport' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<CreatedExport> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(CreatedExport.class));

       return (TypeAdapter<T>) new TypeAdapter<CreatedExport>() {
           @Override
           public void write(JsonWriter out, CreatedExport value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public CreatedExport read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of CreatedExport given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of CreatedExport
   * @throws IOException if the JSON string is invalid with respect to CreatedExport
   */
  public static CreatedExport fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, CreatedExport.class);
  }

  /**
   * Convert an instance of CreatedExport to an JSON string
   *
   * @return JSON string
   */
