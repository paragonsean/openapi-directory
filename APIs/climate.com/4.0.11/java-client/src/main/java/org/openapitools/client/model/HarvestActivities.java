...
import org.openapitools.client.model.HarvestActivitySummary;
...
public class HarvestActivities {
  ...
  private List<HarvestActivitySummary> results = new ArrayList<>();

  public HarvestActivities() {
  }

  public HarvestActivities results(List<HarvestActivitySummary> results) {
    this.results = results;
    return this;
  }

  public HarvestActivities addResultsItem(HarvestActivitySummary resultsItem) {
  ...
  public List<HarvestActivitySummary> getResults() {
    return results;
  }

  public void setResults(List<HarvestActivitySummary> results) {
    this.results = results;
  }
  ...
  public boolean equals(Object o) {
    ...
    HarvestActivities harvestActivities = (HarvestActivities) o;
    return Objects.equals(this.results, harvestActivities.results);
  }
  ...
  public String toString() {
    ...
    sb.append("class HarvestActivities {\n");
   ...
   * @throws IOException if the JSON Element is invalid with respect to HarvestActivities
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!HarvestActivities.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in HarvestActivities is not found in the empty JSON string", HarvestActivities.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!HarvestActivities.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `HarvestActivities` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : HarvestActivities.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayresults.size(); i++) {
        HarvestActivitySummary.validateJsonElement(jsonArrayresults.get(i));
      };
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!HarvestActivities.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'HarvestActivities' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<HarvestActivities> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(HarvestActivities.class));

       return (TypeAdapter<T>) new TypeAdapter<HarvestActivities>() {
           @Override
           public void write(JsonWriter out, HarvestActivities value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public HarvestActivities read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of HarvestActivities given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of HarvestActivities
   * @throws IOException if the JSON string is invalid with respect to HarvestActivities
   */
  public static HarvestActivities fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, HarvestActivities.class);
  }

  /**
   * Convert an instance of HarvestActivities to an JSON string
   *
   * @return JSON string
   */
