...
public class ApplicationActivities {
  ...
  private List<ApplicationActivitySummary> results = new ArrayList<>();

  public ApplicationActivities() {
  }

  public ApplicationActivities results(List<ApplicationActivitySummary> results) {
    this.results = results;
    return this;
  }

  public ApplicationActivities addResultsItem(ApplicationActivitySummary resultsItem) {
  ...
  public List<ApplicationActivitySummary> getResults() {
    return results;
  }

  public void setResults(List<ApplicationActivitySummary> results) {
    this.results = results;
  }
  ...
  public boolean equals(Object o) {
    ...
    ApplicationActivities applicationActivities = (ApplicationActivities) o;
    return Objects.equals(this.results, applicationActivities.results);
  }
  ...
  public String toString() {
    ...
    sb.append("class ApplicationActivities {\n");
   ...
   * @throws IOException if the JSON Element is invalid with respect to ApplicationActivities
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ApplicationActivities.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ApplicationActivities is not found in the empty JSON string", ApplicationActivities.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ApplicationActivities.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ApplicationActivities` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ApplicationActivities.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayresults.size(); i++) {
        ApplicationActivitySummary.validateJsonElement(jsonArrayresults.get(i));
      };
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ApplicationActivities.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ApplicationActivities' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ApplicationActivities> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ApplicationActivities.class));

       return (TypeAdapter<T>) new TypeAdapter<ApplicationActivities>() {
           @Override
           public void write(JsonWriter out, ApplicationActivities value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ApplicationActivities read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of ApplicationActivities given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of ApplicationActivities
   * @throws IOException if the JSON string is invalid with respect to ApplicationActivities
   */
  public static ApplicationActivities fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ApplicationActivities.class);
  }

  /**
   * Convert an instance of ApplicationActivities to an JSON string
   *
   * @return JSON string
   */
