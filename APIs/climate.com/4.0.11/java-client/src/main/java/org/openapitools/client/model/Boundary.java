...
public class Boundary {
  ...
  public Boundary geometry(Geometry geometry) {
  ...
  public Boundary properties(BoundaryProperties properties) {
  ...
  public Boundary type(TypeEnum type) {
    this.type = type;
    return this;
  }
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(this.geometry, boundary.geometry) &&
        Objects.equals(this.id, boundary.id) &&
        Objects.equals(this.properties, boundary.properties) &&
        Objects.equals(this.type, boundary.type);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to Boundary
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Boundary.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in Boundary is not found in the empty JSON string", Boundary.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Boundary.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Boundary` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : Boundary.openapiRequiredFields) {
      ...
      BoundaryProperties.validateJsonElement(jsonObj.get("properties"));
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!Boundary.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'Boundary' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<Boundary> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(Boundary.class));

       return (TypeAdapter<T>) new TypeAdapter<Boundary>() {
           @Override
           public void write(JsonWriter out, Boundary value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public Boundary read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of Boundary given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of Boundary
   * @throws IOException if the JSON string is invalid with respect to Boundary
   */
  public static Boundary fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, Boundary.class);
  }

  /**
   * Convert an instance of Boundary to an JSON string
   *
   * @return JSON string
   */
