...
void OAIAuthApi::getAccessToken(const ::OpenAPI::OptionalParam<OAITokenRequest> &oai_token_request) {
    QString fullPath = QString(_serverConfigs["getAccessToken"][_serverIndices.value("getAccessToken")].URL()+"/token");
    ...
    if (oai_token_request.hasValue()){

        
        QByteArray output = oai_token_request.value().asJson().toUtf8();
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIAuthApi::getAccessTokenCallback);
...
void OAIAuthApi::getAccessTokenCallback(OAIHttpRequestWorker *worker) {
