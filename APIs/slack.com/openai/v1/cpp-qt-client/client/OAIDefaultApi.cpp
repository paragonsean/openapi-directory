...
void OAIDefaultApi::initializeServerConfigs() {
    ...
    _serverConfigs.insert("aiAlphaSearchMessages", defaultConf);
    _serverIndices.insert("aiAlphaSearchMessages", 0);
}
...
void OAIDefaultApi::aiAlphaSearchMessages(const OAISearchRequest &oai_search_request) {
    QString fullPath = QString(_serverConfigs["aiAlphaSearchMessages"][_serverIndices.value("aiAlphaSearchMessages")].URL()+"/ai.alpha.search.messages");
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIDefaultApi::aiAlphaSearchMessagesCallback);
...
void OAIDefaultApi::aiAlphaSearchMessagesCallback(OAIHttpRequestWorker *worker) {
    ...
    OAIAi_alpha_search_messages_200_response output(QString(worker->response));
    worker->deleteLater();

    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT aiAlphaSearchMessagesSignal(output);
        Q_EMIT aiAlphaSearchMessagesSignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT aiAlphaSearchMessagesSignalE(output, error_type, error_str);
        Q_EMIT aiAlphaSearchMessagesSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT aiAlphaSearchMessagesSignalError(output, error_type, error_str);
        Q_EMIT aiAlphaSearchMessagesSignalErrorFull(worker, error_type, error_str);
    }
}
