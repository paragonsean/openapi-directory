...
public class UsersIdentitySchemaInnerAnyOf1User {
   ...
   * @throws IOException if the JSON Element is invalid with respect to UsersIdentitySchemaInnerAnyOf1User
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UsersIdentitySchemaInnerAnyOf1User.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!UsersIdentitySchemaInnerAnyOf1User.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UsersIdentitySchemaInnerAnyOf1User` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
