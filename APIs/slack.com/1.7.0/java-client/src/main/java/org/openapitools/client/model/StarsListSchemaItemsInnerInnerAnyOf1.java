...
public class StarsListSchemaItemsInnerInnerAnyOf1 {
   ...
   * @throws IOException if the JSON Element is invalid with respect to StarsListSchemaItemsInnerInnerAnyOf1
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!StarsListSchemaItemsInnerInnerAnyOf1.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!StarsListSchemaItemsInnerInnerAnyOf1.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `StarsListSchemaItemsInnerInnerAnyOf1` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
