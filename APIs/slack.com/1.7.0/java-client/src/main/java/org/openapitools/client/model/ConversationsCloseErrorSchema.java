...
public class ConversationsCloseErrorSchema {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ConversationsCloseErrorSchema
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ConversationsCloseErrorSchema.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ConversationsCloseErrorSchema.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ConversationsCloseErrorSchema` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
