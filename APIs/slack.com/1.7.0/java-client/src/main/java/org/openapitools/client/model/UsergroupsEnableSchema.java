...
public class UsergroupsEnableSchema {
  ...
  public UsergroupsEnableSchema ok(Boolean ok) {
    this.ok = ok;
    return this;
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to UsergroupsEnableSchema
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UsergroupsEnableSchema.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!UsergroupsEnableSchema.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UsergroupsEnableSchema` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
