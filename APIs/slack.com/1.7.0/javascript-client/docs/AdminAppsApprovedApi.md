# SlackWebApi.AdminAppsApprovedApi
...
> DefaultSuccessTemplate adminAppsApprovedList(token, opts)
...
let apiInstance = new SlackWebApi.AdminAppsApprovedApi();
...
apiInstance.adminAppsApprovedList(token, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
