# SlackWebApi.AdminAppsRequestsApi
...
> DefaultSuccessTemplate adminAppsRequestsList(token, opts)
...
let apiInstance = new SlackWebApi.AdminAppsRequestsApi();
...
apiInstance.adminAppsRequestsList(token, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
