...
class ChangeOrderListRequest {
     ...
     */
    constructor(changeOrders) { 
        
        ChangeOrderListRequest.initialize(this, changeOrders);
    }
     ...
     */
    static initialize(obj, changeOrders) { 
        obj['changeOrders'] = changeOrders;
    }
    ...
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new ChangeOrderListRequest();
    ...
    /**
     * Validates the JSON data with respect to <code>ChangeOrderListRequest</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>ChangeOrderListRequest</code>.
     */
    static validateJSON(data) {
        // check to make sure all required properties are present in the JSON string
        for (const property of ChangeOrderListRequest.RequiredProperties) {
            if (!data.hasOwnProperty(property)) {
                throw new Error("The required field `" + property + "` is not found in the JSON data: " + JSON.stringify(data));
            }
        }
        if (data['changeOrders']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['changeOrders'])) {
                throw new Error("Expected the field `changeOrders` to be an array in the JSON data but got " + data['changeOrders']);
            }
            // validate the optional field `changeOrders` (array)
            for (const item of data['changeOrders']) {
                ChangeOrderListRequestItem.validateJSON(item);
            };
        }
...
ChangeOrderListRequest.RequiredProperties = ["changeOrders"];

/**
 * The change order operations
 * @member {Array.<module:model/ChangeOrderListRequestItem>} changeOrders
 */
ChangeOrderListRequest.prototype['changeOrders'] = undefined;
...
export default ChangeOrderListRequest;
