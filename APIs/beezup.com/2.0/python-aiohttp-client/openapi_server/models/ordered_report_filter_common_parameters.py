# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.beez_up_common_catalog_category_filter import BeezUPCommonCatalogCategoryFilter
from openapi_server.models.order_by_direction import OrderByDirection
from openapi_server.models.performance_indicator_filter import PerformanceIndicatorFilter
from openapi_server.models.performance_indicator_type import PerformanceIndicatorType
from openapi_server.models.report_advanced_filters import ReportAdvancedFilters
from openapi_server.models.report_filter_period_type import ReportFilterPeriodType
from openapi_server import util


class OrderedReportFilterCommonParameters(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, advanced_filters: ReportAdvancedFilters=None, begin_period_utc_date: datetime=None, category_filter: BeezUPCommonCatalogCategoryFilter=None, channel_id: str=None, end_period_utc_date: datetime=None, performance_indicator_filters: List[PerformanceIndicatorFilter]=None, period_type: ReportFilterPeriodType=None, order_by: PerformanceIndicatorType=None, order_by_direction: OrderByDirection=None):
        """OrderedReportFilterCommonParameters - a model defined in OpenAPI

        :param advanced_filters: The advanced_filters of this OrderedReportFilterCommonParameters.
        :param begin_period_utc_date: The begin_period_utc_date of this OrderedReportFilterCommonParameters.
        :param category_filter: The category_filter of this OrderedReportFilterCommonParameters.
        :param channel_id: The channel_id of this OrderedReportFilterCommonParameters.
        :param end_period_utc_date: The end_period_utc_date of this OrderedReportFilterCommonParameters.
        :param performance_indicator_filters: The performance_indicator_filters of this OrderedReportFilterCommonParameters.
        :param period_type: The period_type of this OrderedReportFilterCommonParameters.
        :param order_by: The order_by of this OrderedReportFilterCommonParameters.
        :param order_by_direction: The order_by_direction of this OrderedReportFilterCommonParameters.
        """
        self.openapi_types = {
            'advanced_filters': ReportAdvancedFilters,
            'begin_period_utc_date': datetime,
            'category_filter': BeezUPCommonCatalogCategoryFilter,
            'channel_id': str,
            'end_period_utc_date': datetime,
            'performance_indicator_filters': List[PerformanceIndicatorFilter],
            'period_type': ReportFilterPeriodType,
            'order_by': PerformanceIndicatorType,
            'order_by_direction': OrderByDirection
        }

        self.attribute_map = {
            'advanced_filters': 'advancedFilters',
            'begin_period_utc_date': 'beginPeriodUtcDate',
            'category_filter': 'categoryFilter',
            'channel_id': 'channelId',
            'end_period_utc_date': 'endPeriodUtcDate',
            'performance_indicator_filters': 'performanceIndicatorFilters',
            'period_type': 'periodType',
            'order_by': 'orderBy',
            'order_by_direction': 'orderByDirection'
        }

        self._advanced_filters = advanced_filters
        self._begin_period_utc_date = begin_period_utc_date
        self._category_filter = category_filter
        self._channel_id = channel_id
        self._end_period_utc_date = end_period_utc_date
        self._performance_indicator_filters = performance_indicator_filters
        self._period_type = period_type
        self._order_by = order_by
        self._order_by_direction = order_by_direction

    @classmethod
    def from_dict(cls, dikt: dict) -> 'OrderedReportFilterCommonParameters':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The orderedReportFilterCommonParameters of this OrderedReportFilterCommonParameters.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def advanced_filters(self):
        """Gets the advanced_filters of this OrderedReportFilterCommonParameters.


        :return: The advanced_filters of this OrderedReportFilterCommonParameters.
        :rtype: ReportAdvancedFilters
        """
        return self._advanced_filters

    @advanced_filters.setter
    def advanced_filters(self, advanced_filters):
        """Sets the advanced_filters of this OrderedReportFilterCommonParameters.


        :param advanced_filters: The advanced_filters of this OrderedReportFilterCommonParameters.
        :type advanced_filters: ReportAdvancedFilters
        """
        if advanced_filters is None:
            raise ValueError("Invalid value for `advanced_filters`, must not be `None`")

        self._advanced_filters = advanced_filters

    @property
    def begin_period_utc_date(self):
        """Gets the begin_period_utc_date of this OrderedReportFilterCommonParameters.

        The begin date period you want to get the report. It's required only in case of custom period type !

        :return: The begin_period_utc_date of this OrderedReportFilterCommonParameters.
        :rtype: datetime
        """
        return self._begin_period_utc_date

    @begin_period_utc_date.setter
    def begin_period_utc_date(self, begin_period_utc_date):
        """Sets the begin_period_utc_date of this OrderedReportFilterCommonParameters.

        The begin date period you want to get the report. It's required only in case of custom period type !

        :param begin_period_utc_date: The begin_period_utc_date of this OrderedReportFilterCommonParameters.
        :type begin_period_utc_date: datetime
        """

        self._begin_period_utc_date = begin_period_utc_date

    @property
    def category_filter(self):
        """Gets the category_filter of this OrderedReportFilterCommonParameters.


        :return: The category_filter of this OrderedReportFilterCommonParameters.
        :rtype: BeezUPCommonCatalogCategoryFilter
        """
        return self._category_filter

    @category_filter.setter
    def category_filter(self, category_filter):
        """Sets the category_filter of this OrderedReportFilterCommonParameters.


        :param category_filter: The category_filter of this OrderedReportFilterCommonParameters.
        :type category_filter: BeezUPCommonCatalogCategoryFilter
        """

        self._category_filter = category_filter

    @property
    def channel_id(self):
        """Gets the channel_id of this OrderedReportFilterCommonParameters.

        The channel identifier

        :return: The channel_id of this OrderedReportFilterCommonParameters.
        :rtype: str
        """
        return self._channel_id

    @channel_id.setter
    def channel_id(self, channel_id):
        """Sets the channel_id of this OrderedReportFilterCommonParameters.

        The channel identifier

        :param channel_id: The channel_id of this OrderedReportFilterCommonParameters.
        :type channel_id: str
        """

        self._channel_id = channel_id

    @property
    def end_period_utc_date(self):
        """Gets the end_period_utc_date of this OrderedReportFilterCommonParameters.

        The end date period you want to get the report. It's required only in case of custom period type !

        :return: The end_period_utc_date of this OrderedReportFilterCommonParameters.
        :rtype: datetime
        """
        return self._end_period_utc_date

    @end_period_utc_date.setter
    def end_period_utc_date(self, end_period_utc_date):
        """Sets the end_period_utc_date of this OrderedReportFilterCommonParameters.

        The end date period you want to get the report. It's required only in case of custom period type !

        :param end_period_utc_date: The end_period_utc_date of this OrderedReportFilterCommonParameters.
        :type end_period_utc_date: datetime
        """

        self._end_period_utc_date = end_period_utc_date

    @property
    def performance_indicator_filters(self):
        """Gets the performance_indicator_filters of this OrderedReportFilterCommonParameters.


        :return: The performance_indicator_filters of this OrderedReportFilterCommonParameters.
        :rtype: List[PerformanceIndicatorFilter]
        """
        return self._performance_indicator_filters

    @performance_indicator_filters.setter
    def performance_indicator_filters(self, performance_indicator_filters):
        """Sets the performance_indicator_filters of this OrderedReportFilterCommonParameters.


        :param performance_indicator_filters: The performance_indicator_filters of this OrderedReportFilterCommonParameters.
        :type performance_indicator_filters: List[PerformanceIndicatorFilter]
        """

        self._performance_indicator_filters = performance_indicator_filters

    @property
    def period_type(self):
        """Gets the period_type of this OrderedReportFilterCommonParameters.


        :return: The period_type of this OrderedReportFilterCommonParameters.
        :rtype: ReportFilterPeriodType
        """
        return self._period_type

    @period_type.setter
    def period_type(self, period_type):
        """Sets the period_type of this OrderedReportFilterCommonParameters.


        :param period_type: The period_type of this OrderedReportFilterCommonParameters.
        :type period_type: ReportFilterPeriodType
        """
        if period_type is None:
            raise ValueError("Invalid value for `period_type`, must not be `None`")

        self._period_type = period_type

    @property
    def order_by(self):
        """Gets the order_by of this OrderedReportFilterCommonParameters.


        :return: The order_by of this OrderedReportFilterCommonParameters.
        :rtype: PerformanceIndicatorType
        """
        return self._order_by

    @order_by.setter
    def order_by(self, order_by):
        """Sets the order_by of this OrderedReportFilterCommonParameters.


        :param order_by: The order_by of this OrderedReportFilterCommonParameters.
        :type order_by: PerformanceIndicatorType
        """

        self._order_by = order_by

    @property
    def order_by_direction(self):
        """Gets the order_by_direction of this OrderedReportFilterCommonParameters.


        :return: The order_by_direction of this OrderedReportFilterCommonParameters.
        :rtype: OrderByDirection
        """
        return self._order_by_direction

    @order_by_direction.setter
    def order_by_direction(self, order_by_direction):
        """Sets the order_by_direction of this OrderedReportFilterCommonParameters.


        :param order_by_direction: The order_by_direction of this OrderedReportFilterCommonParameters.
        :type order_by_direction: OrderByDirection
        """

        self._order_by_direction = order_by_direction
