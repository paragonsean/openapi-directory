...
void OAISearchApi::searchPost(const ::OpenAPI::OptionalParam<OAISearchRequestSchema> &search_request_schema) {
    ...
    if (search_request_schema.hasValue()){

        
        QByteArray output = search_request_schema.value().asJson().toUtf8();
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAISearchApi::searchPostCallback);
...
void OAISearchApi::searchPostCallback(OAIHttpRequestWorker *worker) {
    ...
    OAISearchResponseSchema output(QString(worker->response));
    worker->deleteLater();
