...
```javascript
import MasterCardAbuApi from 'master_card_abu_api';

let apiInstance = new MasterCardAbuApi.ABUApi();
let abuRequest = new MasterCardAbuApi.ABU(); // ABU | Request for ABU API
apiInstance.abuPost(abuRequest, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
