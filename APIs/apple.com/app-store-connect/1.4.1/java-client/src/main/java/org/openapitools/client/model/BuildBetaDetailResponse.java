...
public class BuildBetaDetailResponse {
   ...
   * @throws IOException if the JSON Element is invalid with respect to BuildBetaDetailResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!BuildBetaDetailResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in BuildBetaDetailResponse is not found in the empty JSON string", BuildBetaDetailResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!BuildBetaDetailResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BuildBetaDetailResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : BuildBetaDetailResponse.openapiRequiredFields) {
      ...
      BuildBetaDetail.validateJsonElement(jsonObj.get("data"));
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!BuildBetaDetailResponse.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'BuildBetaDetailResponse' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<BuildBetaDetailResponse> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(BuildBetaDetailResponse.class));

       return (TypeAdapter<T>) new TypeAdapter<BuildBetaDetailResponse>() {
           @Override
           public void write(JsonWriter out, BuildBetaDetailResponse value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public BuildBetaDetailResponse read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of BuildBetaDetailResponse given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of BuildBetaDetailResponse
   * @throws IOException if the JSON string is invalid with respect to BuildBetaDetailResponse
   */
  public static BuildBetaDetailResponse fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, BuildBetaDetailResponse.class);
  }

  /**
   * Convert an instance of BuildBetaDetailResponse to an JSON string
   *
   * @return JSON string
   */
