...
public class UserInvitationsResponse {
   ...
   * @throws IOException if the JSON Element is invalid with respect to UserInvitationsResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UserInvitationsResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in UserInvitationsResponse is not found in the empty JSON string", UserInvitationsResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!UserInvitationsResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UserInvitationsResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : UserInvitationsResponse.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArraydata.size(); i++) {
        UserInvitation.validateJsonElement(jsonArraydata.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!UserInvitationsResponse.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'UserInvitationsResponse' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<UserInvitationsResponse> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(UserInvitationsResponse.class));

       return (TypeAdapter<T>) new TypeAdapter<UserInvitationsResponse>() {
           @Override
           public void write(JsonWriter out, UserInvitationsResponse value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public UserInvitationsResponse read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of UserInvitationsResponse given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of UserInvitationsResponse
   * @throws IOException if the JSON string is invalid with respect to UserInvitationsResponse
   */
  public static UserInvitationsResponse fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, UserInvitationsResponse.class);
  }

  /**
   * Convert an instance of UserInvitationsResponse to an JSON string
   *
   * @return JSON string
   */
