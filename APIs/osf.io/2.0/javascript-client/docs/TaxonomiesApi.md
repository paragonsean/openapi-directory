# OsfApiv2Documentation.TaxonomiesApi
...
let apiInstance = new OsfApiv2Documentation.TaxonomiesApi();
apiInstance.taxonomiesList((error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
let apiInstance = new OsfApiv2Documentation.TaxonomiesApi();
let taxonomyId = "taxonomyId_example"; // String | The unique identifier of the taxonomy.
apiInstance.taxonomiesRead(taxonomyId, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
