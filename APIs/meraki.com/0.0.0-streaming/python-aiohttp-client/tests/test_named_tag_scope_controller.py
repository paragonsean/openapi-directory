...
async def test_create_network_sm_target_group(client):
    ...
    response = await client.request(
        method='POST',
        path='/api/v0/networks/{network_id}/sm/targetGroups'.format(network_id='network_id_example'),
        headers=headers,
        json=body,
        )
...
async def test_delete_network_sm_target_group(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/api/v0/networks/{network_id}/sm/targetGroups/{target_group_id}'.format(network_id='network_id_example', target_group_id='target_group_id_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
