...
async def test_create_network_wireless_rf_profile(client):
    ...
    response = await client.request(
        method='POST',
        path='/api/v0/networks/{network_id}/wireless/rfProfiles'.format(network_id='network_id_example'),
        headers=headers,
        json=body,
        )
...
async def test_delete_network_wireless_rf_profile(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/api/v0/networks/{network_id}/wireless/rfProfiles/{rf_profile_id}'.format(network_id='network_id_example', rf_profile_id='rf_profile_id_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
