...
async def test_get_network_bluetooth_client(client):
    ...
    response = await client.request(
        method='GET',
        path='/api/v0/networks/{network_id}/bluetoothClients/{bluetooth_client_id}'.format(network_id='network_id_example', bluetooth_client_id='bluetooth_client_id_example'),
        headers=headers,
        params=params,
        )
...
async def test_get_network_bluetooth_clients(client):
    ...
    response = await client.request(
        method='GET',
        path='/api/v0/networks/{network_id}/bluetoothClients'.format(network_id='network_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
