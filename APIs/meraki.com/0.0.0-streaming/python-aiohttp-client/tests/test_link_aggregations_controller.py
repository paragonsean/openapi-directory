...
async def test_create_network_switch_link_aggregation(client):
    ...
    response = await client.request(
        method='POST',
        path='/api/v0/networks/{network_id}/switch/linkAggregations'.format(network_id='network_id_example'),
        headers=headers,
        json=body,
        )
...
async def test_delete_network_switch_link_aggregation(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/api/v0/networks/{network_id}/switch/linkAggregations/{link_aggregation_id}'.format(network_id='network_id_example', link_aggregation_id='link_aggregation_id_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
