...
> [Object] getDeviceCameraAnalyticsZoneHistory_2(serial, zoneId, opts)
...
apiInstance.getDeviceCameraAnalyticsZoneHistory_2(serial, zoneId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
> [Object] getDeviceCameraAnalyticsZones_2(serial)
...
let apiInstance = new MerakiDashboardApi.ZonesApi();
let serial = "serial_example"; // String | 
apiInstance.getDeviceCameraAnalyticsZones_2(serial, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
