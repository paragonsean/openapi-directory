# MerakiDashboardApi.GetOrganizationDevicesStatuses200Response

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**components** | [**GetOrganizationDevicesStatuses200ResponseComponents**](GetOrganizationDevicesStatuses200ResponseComponents.md) |  | [optional] 
**gateway** | **String** | IP Gateway | [optional] 
**ipType** | **String** | IP Type | [optional] 
**lanIp** | **String** | LAN IP Address | [optional] 
**lastReportedAt** | **String** | Device Last Reported Location | [optional] 
**mac** | **String** | MAC Address | [optional] 
**model** | **String** | Model | [optional] 
**name** | **String** | Device Name | [optional] 
**networkId** | **String** | Network ID | [optional] 
**primaryDns** | **String** | Primary DNS | [optional] 
**productType** | **String** | Product Type | [optional] 
**publicIp** | **String** | Public IP Address | [optional] 
**secondaryDns** | **String** | Secondary DNS | [optional] 
**serial** | **String** | Device Serial Number | [optional] 
**status** | **String** | Device Status | [optional] 
**tags** | **[String]** | Tags | [optional] 


