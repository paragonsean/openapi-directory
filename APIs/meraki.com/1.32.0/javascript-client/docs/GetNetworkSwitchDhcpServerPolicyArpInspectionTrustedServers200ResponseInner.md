# MerakiDashboardApi.GetNetworkSwitchDhcpServerPolicyArpInspectionTrustedServers200ResponseInner

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**ipv4** | [**GetNetworkSwitchDhcpServerPolicyArpInspectionTrustedServers200ResponseInnerIpv4**](GetNetworkSwitchDhcpServerPolicyArpInspectionTrustedServers200ResponseInnerIpv4.md) |  | [optional] 
**mac** | **String** | Mac address of the trusted server. | [optional] 
**trustedServerId** | **String** | ID of the trusted server. | [optional] 
**vlan** | **Number** | Vlan ID of the trusted server. | [optional] 


