...
> [GetOrganizationWebhooksLogs200ResponseInner] getOrganizationWebhooksLogs_2(organizationId, opts)
...
let apiInstance = new MerakiDashboardApi.LogsApi();
...
apiInstance.getOrganizationWebhooksLogs_2(organizationId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
