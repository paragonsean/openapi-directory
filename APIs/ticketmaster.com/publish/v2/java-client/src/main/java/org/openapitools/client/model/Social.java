...
public class Social {
   ...
   * @throws IOException if the JSON Element is invalid with respect to Social
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Social.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Social.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Social` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // validate the optional field `twitter`
      if (jsonObj.get("twitter") != null && !jsonObj.get("twitter").isJsonNull()) {
        Twitter.validateJsonElement(jsonObj.get("twitter"));
      }
  }
