/**
 * CROssBAR Data API
 * # About CROssBAR & data **CROssBAR**: Comprehensive Resource of Biomedical Relations with Deep Learning Applications and Knowledge Graph Representations CROssBAR is a comprehensive system that integrates large-scale biomedical data from various resources e.g UniProt, ChEMBL, Drugbank, EFO, HPO, InterPro & PubChem and stores them in a new NoSQL database, enrich these data with deep learning based prediction of relations between numerous biomedical entities, rigorously analyse the enriched data to obtain biologically meaningful modules and display them to the user via easy to interpret, interactive and heterogeneous knowledge graphs. CROssBAR platform exposes a set of 12 endpoints to query data stored in the CROssBAR database. These endpoints help the user to find data of interest using different parameters provided by the API endpoint. For example, https://www.ebi.ac.uk/tools/crossbar/proteins?accession=A0A023GRW5 -> will provide protein information about accession 'A0A023GRW5' including its interactions, functions, cross-references, variations and more. https://www.ebi.ac.uk/tools/crossbar/activities?moleculeChemblId=CHEMBL465983 -> will provide ChEMBL bio-interactions related information including targets and bio-activity measurements associated with molecule chembl id 'CHEMBL465983'  **Knowledge graphs** Another use case of CROssBAR's API endpoints is in building knowledge graphs. These endpoints can be *weaved* together (output from one API endpoint fed as input to another API endpoint) programmatically to link nodes like protein, disease, drugs etc. as nodes of the graph. The endpoints are designed to be independent from each other which allows users the flexibility to drive biological networks from any facet e.g drug-centric, disease-centric, gene-centric etc. Our service for knowledge graph construction is available at https://crossbar.kansil.org. An example for the part of the background queries on the CROssBAR API during the construction of a knowledge graph,  (with the aim of keeping the example simple, we have only included the processes related to pathways, genes/proteins and drugs/compounds) In this example, we would like to find bio-active compounds (with a pChEMBL value threshold of at least 6.0) & drugs targeting all proteins belonging to \"WNT ligand biogenesis and trafficking\" pathway (based on Reactome pathway annotations). This can be achieved by using endpoints listed on this swagger documentation as illustrated in following steps- Find bio-active compounds (with a pChEMBL value threshold of at least 6.0) & drugs targeting all proteins belonging to \"WNT ligand biogenesis and trafficking\" pathway (based on Reactome annotations) This can be achieved by using endpoints listed on [this swagger documentation](https://www.ebi.ac.uk/tools/crossbar/swagger-ui.html) as illustrated in following steps- 1. Get all proteins from “/proteins” API endpoint which have a reactome pathway name equal to \"WNT ligand biogenesis and trafficking\". 2. From the collection of uniprot protein accessions collected from step 1 above, we query “/targets” API endpoint to obtain the ‘target_chembl_id’s of these proteins. 3. From the collection of target_chembl_ids collected from step 2 above, we query “/activities” API endpoint with pChEMBL value >=6, to obtain the ’molecule_chembl_id’s of the molecules that we need.  4. From the collection of uniprot protein accessions collected from step 1 above, we find out Drug names and ids from the “/drugs” API endpoint that targets our proteins. 5. From the collection of ’molecule_chembl_id’s obtained in step3, we query “/molecules” endpoint to get the compounds that are interacting with the genes/proteins belonging to the “WNT ligand biogenesis and trafficking” pathway.
 *
 * The version of the OpenAPI document: 1.0
 * Contact: crossbar_dev@ebi.ac.uk
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import Ensembl from './Ensembl';
import Go from './Go';
import Interpro from './Interpro';
import Kegg from './Kegg';
import Omim from './Omim';
import OpenTargets from './OpenTargets';
import Orphanet from './Orphanet';
import PDBe from './PDBe';
import Pfam from './Pfam';
import Reactome from './Reactome';

/**
 * The CrossReferences model module.
 * @module model/CrossReferences
 * @version 1.0
 */
class CrossReferences {
    /**
     * Constructs a new <code>CrossReferences</code>.
     * @alias module:model/CrossReferences
     */
    constructor() { 
        
        CrossReferences.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>CrossReferences</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/CrossReferences} obj Optional instance to populate.
     * @return {module:model/CrossReferences} The populated <code>CrossReferences</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new CrossReferences();

            if (data.hasOwnProperty('ensembl')) {
                obj['ensembl'] = ApiClient.convertToType(data['ensembl'], [Ensembl]);
            }
            if (data.hasOwnProperty('go')) {
                obj['go'] = ApiClient.convertToType(data['go'], [Go]);
            }
            if (data.hasOwnProperty('interpro')) {
                obj['interpro'] = ApiClient.convertToType(data['interpro'], [Interpro]);
            }
            if (data.hasOwnProperty('kegg')) {
                obj['kegg'] = ApiClient.convertToType(data['kegg'], [Kegg]);
            }
            if (data.hasOwnProperty('omim')) {
                obj['omim'] = ApiClient.convertToType(data['omim'], [Omim]);
            }
            if (data.hasOwnProperty('openTargets')) {
                obj['openTargets'] = ApiClient.convertToType(data['openTargets'], [OpenTargets]);
            }
            if (data.hasOwnProperty('orphanet')) {
                obj['orphanet'] = ApiClient.convertToType(data['orphanet'], [Orphanet]);
            }
            if (data.hasOwnProperty('pdBe')) {
                obj['pdBe'] = ApiClient.convertToType(data['pdBe'], [PDBe]);
            }
            if (data.hasOwnProperty('pfam')) {
                obj['pfam'] = ApiClient.convertToType(data['pfam'], [Pfam]);
            }
            if (data.hasOwnProperty('reactome')) {
                obj['reactome'] = ApiClient.convertToType(data['reactome'], [Reactome]);
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>CrossReferences</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>CrossReferences</code>.
     */
    static validateJSON(data) {
        if (data['ensembl']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['ensembl'])) {
                throw new Error("Expected the field `ensembl` to be an array in the JSON data but got " + data['ensembl']);
            }
            // validate the optional field `ensembl` (array)
            for (const item of data['ensembl']) {
                Ensembl.validateJSON(item);
            };
        }
        if (data['go']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['go'])) {
                throw new Error("Expected the field `go` to be an array in the JSON data but got " + data['go']);
            }
            // validate the optional field `go` (array)
            for (const item of data['go']) {
                Go.validateJSON(item);
            };
        }
        if (data['interpro']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['interpro'])) {
                throw new Error("Expected the field `interpro` to be an array in the JSON data but got " + data['interpro']);
            }
            // validate the optional field `interpro` (array)
            for (const item of data['interpro']) {
                Interpro.validateJSON(item);
            };
        }
        if (data['kegg']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['kegg'])) {
                throw new Error("Expected the field `kegg` to be an array in the JSON data but got " + data['kegg']);
            }
            // validate the optional field `kegg` (array)
            for (const item of data['kegg']) {
                Kegg.validateJSON(item);
            };
        }
        if (data['omim']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['omim'])) {
                throw new Error("Expected the field `omim` to be an array in the JSON data but got " + data['omim']);
            }
            // validate the optional field `omim` (array)
            for (const item of data['omim']) {
                Omim.validateJSON(item);
            };
        }
        if (data['openTargets']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['openTargets'])) {
                throw new Error("Expected the field `openTargets` to be an array in the JSON data but got " + data['openTargets']);
            }
            // validate the optional field `openTargets` (array)
            for (const item of data['openTargets']) {
                OpenTargets.validateJSON(item);
            };
        }
        if (data['orphanet']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['orphanet'])) {
                throw new Error("Expected the field `orphanet` to be an array in the JSON data but got " + data['orphanet']);
            }
            // validate the optional field `orphanet` (array)
            for (const item of data['orphanet']) {
                Orphanet.validateJSON(item);
            };
        }
        if (data['pdBe']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['pdBe'])) {
                throw new Error("Expected the field `pdBe` to be an array in the JSON data but got " + data['pdBe']);
            }
            // validate the optional field `pdBe` (array)
            for (const item of data['pdBe']) {
                PDBe.validateJSON(item);
            };
        }
        if (data['pfam']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['pfam'])) {
                throw new Error("Expected the field `pfam` to be an array in the JSON data but got " + data['pfam']);
            }
            // validate the optional field `pfam` (array)
            for (const item of data['pfam']) {
                Pfam.validateJSON(item);
            };
        }
        if (data['reactome']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['reactome'])) {
                throw new Error("Expected the field `reactome` to be an array in the JSON data but got " + data['reactome']);
            }
            // validate the optional field `reactome` (array)
            for (const item of data['reactome']) {
                Reactome.validateJSON(item);
            };
        }

        return true;
    }


}



/**
 * @member {Array.<module:model/Ensembl>} ensembl
 */
CrossReferences.prototype['ensembl'] = undefined;

/**
 * @member {Array.<module:model/Go>} go
 */
CrossReferences.prototype['go'] = undefined;

/**
 * @member {Array.<module:model/Interpro>} interpro
 */
CrossReferences.prototype['interpro'] = undefined;

/**
 * @member {Array.<module:model/Kegg>} kegg
 */
CrossReferences.prototype['kegg'] = undefined;

/**
 * @member {Array.<module:model/Omim>} omim
 */
CrossReferences.prototype['omim'] = undefined;

/**
 * @member {Array.<module:model/OpenTargets>} openTargets
 */
CrossReferences.prototype['openTargets'] = undefined;

/**
 * @member {Array.<module:model/Orphanet>} orphanet
 */
CrossReferences.prototype['orphanet'] = undefined;

/**
 * @member {Array.<module:model/PDBe>} pdBe
 */
CrossReferences.prototype['pdBe'] = undefined;

/**
 * @member {Array.<module:model/Pfam>} pfam
 */
CrossReferences.prototype['pfam'] = undefined;

/**
 * @member {Array.<module:model/Reactome>} reactome
 */
CrossReferences.prototype['reactome'] = undefined;






export default CrossReferences;

