...
public class PayorAddressV2 {
   ...
   * @throws IOException if the JSON Element is invalid with respect to PayorAddressV2
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PayorAddressV2.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PayorAddressV2 is not found in the empty JSON string", PayorAddressV2.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PayorAddressV2.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PayorAddressV2` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : PayorAddressV2.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PayorAddressV2.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PayorAddressV2' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PayorAddressV2> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PayorAddressV2.class));

       return (TypeAdapter<T>) new TypeAdapter<PayorAddressV2>() {
           @Override
           public void write(JsonWriter out, PayorAddressV2 value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PayorAddressV2 read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of PayorAddressV2 given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of PayorAddressV2
   * @throws IOException if the JSON string is invalid with respect to PayorAddressV2
   */
  public static PayorAddressV2 fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PayorAddressV2.class);
  }

  /**
   * Convert an instance of PayorAddressV2 to an JSON string
   *
   * @return JSON string
   */
