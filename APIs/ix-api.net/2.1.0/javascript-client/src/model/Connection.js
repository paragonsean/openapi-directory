/**
 * IX-API
 *  This API allows to config/change/delete Internet Exchange services.  # Filters  When querying collections, the provided query parameters are validated. Unknown query parameters are ignored. Providing invalid filter values should yield a validation error. 
 *
 * The version of the OpenAPI document: 2.1.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import ConnectionPartial from './ConnectionPartial';
import Status from './Status';

/**
 * The Connection model module.
 * @module model/Connection
 * @version 2.1.0
 */
class Connection {
    /**
     * Constructs a new <code>Connection</code>.
     * @alias module:model/Connection
     * @implements module:model/ConnectionPartial
     * @param billingAccount {String} An account requires billing_information to be used as a `billing_account`.
     * @param consumingAccount {String} The `id` of the account consuming a service.  Used to be `owning_customer`. 
     * @param id {String} 
     * @param managingAccount {String} The `id` of the account responsible for managing the service via the API. A manager can read and update the state of entities. 
     * @param mode {module:model/Connection.ModeEnum} Sets the mode of the connection. The mode can be:  - `lag_lacp`: connection is build as a LAG with LACP enabled - `lag_static`: connection is build as LAG with static configuration - `flex_ethernet`: connect is build as a FlexEthernet channel - `standalone`: only one port is allowed in this connection without any bundling. 
     * @param name {String} 
     * @param outerVlanEthertypes {Array.<module:model/Connection.OuterVlanEthertypesEnum>} The ethertype of the outer tag in hexadecimal notation. 
     * @param roleAssignments {Array.<String>} A set of `RoleAssignment`s. See the documentation on the specific `required_contact_roles`, `nfc_required_contact_roles` or `nsc_required_contact_roles` on what `RoleAssignment`s to provide. 
     * @param state {module:model/Connection.StateEnum} 
     * @param vlanTypes {Array.<module:model/Connection.VlanTypesEnum>} A list of vlan config types you can configure using this connection.
     */
    constructor(billingAccount, consumingAccount, id, managingAccount, mode, name, outerVlanEthertypes, roleAssignments, state, vlanTypes) { 
        ConnectionPartial.initialize(this);
        Connection.initialize(this, billingAccount, consumingAccount, id, managingAccount, mode, name, outerVlanEthertypes, roleAssignments, state, vlanTypes);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj, billingAccount, consumingAccount, id, managingAccount, mode, name, outerVlanEthertypes, roleAssignments, state, vlanTypes) { 
        obj['billing_account'] = billingAccount;
        obj['consuming_account'] = consumingAccount;
        obj['id'] = id;
        obj['managing_account'] = managingAccount;
        obj['mode'] = mode;
        obj['name'] = name;
        obj['outer_vlan_ethertypes'] = outerVlanEthertypes;
        obj['purchase_order'] = '';
        obj['role_assignments'] = roleAssignments;
        obj['state'] = state;
        obj['vlan_types'] = vlanTypes;
    }

    /**
     * Constructs a <code>Connection</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/Connection} obj Optional instance to populate.
     * @return {module:model/Connection} The populated <code>Connection</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new Connection();
            ConnectionPartial.constructFromObject(data, obj);

            if (data.hasOwnProperty('billing_account')) {
                obj['billing_account'] = ApiClient.convertToType(data['billing_account'], 'String');
            }
            if (data.hasOwnProperty('consuming_account')) {
                obj['consuming_account'] = ApiClient.convertToType(data['consuming_account'], 'String');
            }
            if (data.hasOwnProperty('contract_ref')) {
                obj['contract_ref'] = ApiClient.convertToType(data['contract_ref'], 'String');
            }
            if (data.hasOwnProperty('external_ref')) {
                obj['external_ref'] = ApiClient.convertToType(data['external_ref'], 'String');
            }
            if (data.hasOwnProperty('id')) {
                obj['id'] = ApiClient.convertToType(data['id'], 'String');
            }
            if (data.hasOwnProperty('lacp_timeout')) {
                obj['lacp_timeout'] = ApiClient.convertToType(data['lacp_timeout'], 'String');
            }
            if (data.hasOwnProperty('managing_account')) {
                obj['managing_account'] = ApiClient.convertToType(data['managing_account'], 'String');
            }
            if (data.hasOwnProperty('mode')) {
                obj['mode'] = ApiClient.convertToType(data['mode'], 'String');
            }
            if (data.hasOwnProperty('name')) {
                obj['name'] = ApiClient.convertToType(data['name'], 'String');
            }
            if (data.hasOwnProperty('outer_vlan_ethertypes')) {
                obj['outer_vlan_ethertypes'] = ApiClient.convertToType(data['outer_vlan_ethertypes'], ['String']);
            }
            if (data.hasOwnProperty('ports')) {
                obj['ports'] = ApiClient.convertToType(data['ports'], ['String']);
            }
            if (data.hasOwnProperty('purchase_order')) {
                obj['purchase_order'] = ApiClient.convertToType(data['purchase_order'], 'String');
            }
            if (data.hasOwnProperty('role_assignments')) {
                obj['role_assignments'] = ApiClient.convertToType(data['role_assignments'], ['String']);
            }
            if (data.hasOwnProperty('speed')) {
                obj['speed'] = ApiClient.convertToType(data['speed'], 'Number');
            }
            if (data.hasOwnProperty('state')) {
                obj['state'] = ApiClient.convertToType(data['state'], 'String');
            }
            if (data.hasOwnProperty('status')) {
                obj['status'] = ApiClient.convertToType(data['status'], [Status]);
            }
            if (data.hasOwnProperty('vlan_types')) {
                obj['vlan_types'] = ApiClient.convertToType(data['vlan_types'], ['String']);
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>Connection</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>Connection</code>.
     */
    static validateJSON(data) {
        // check to make sure all required properties are present in the JSON string
        for (const property of Connection.RequiredProperties) {
            if (!data.hasOwnProperty(property)) {
                throw new Error("The required field `" + property + "` is not found in the JSON data: " + JSON.stringify(data));
            }
        }
        // ensure the json data is a string
        if (data['billing_account'] && !(typeof data['billing_account'] === 'string' || data['billing_account'] instanceof String)) {
            throw new Error("Expected the field `billing_account` to be a primitive type in the JSON string but got " + data['billing_account']);
        }
        // ensure the json data is a string
        if (data['consuming_account'] && !(typeof data['consuming_account'] === 'string' || data['consuming_account'] instanceof String)) {
            throw new Error("Expected the field `consuming_account` to be a primitive type in the JSON string but got " + data['consuming_account']);
        }
        // ensure the json data is a string
        if (data['contract_ref'] && !(typeof data['contract_ref'] === 'string' || data['contract_ref'] instanceof String)) {
            throw new Error("Expected the field `contract_ref` to be a primitive type in the JSON string but got " + data['contract_ref']);
        }
        // ensure the json data is a string
        if (data['external_ref'] && !(typeof data['external_ref'] === 'string' || data['external_ref'] instanceof String)) {
            throw new Error("Expected the field `external_ref` to be a primitive type in the JSON string but got " + data['external_ref']);
        }
        // ensure the json data is a string
        if (data['id'] && !(typeof data['id'] === 'string' || data['id'] instanceof String)) {
            throw new Error("Expected the field `id` to be a primitive type in the JSON string but got " + data['id']);
        }
        // ensure the json data is a string
        if (data['lacp_timeout'] && !(typeof data['lacp_timeout'] === 'string' || data['lacp_timeout'] instanceof String)) {
            throw new Error("Expected the field `lacp_timeout` to be a primitive type in the JSON string but got " + data['lacp_timeout']);
        }
        // ensure the json data is a string
        if (data['managing_account'] && !(typeof data['managing_account'] === 'string' || data['managing_account'] instanceof String)) {
            throw new Error("Expected the field `managing_account` to be a primitive type in the JSON string but got " + data['managing_account']);
        }
        // ensure the json data is a string
        if (data['mode'] && !(typeof data['mode'] === 'string' || data['mode'] instanceof String)) {
            throw new Error("Expected the field `mode` to be a primitive type in the JSON string but got " + data['mode']);
        }
        // ensure the json data is a string
        if (data['name'] && !(typeof data['name'] === 'string' || data['name'] instanceof String)) {
            throw new Error("Expected the field `name` to be a primitive type in the JSON string but got " + data['name']);
        }
        // ensure the json data is an array
        if (!Array.isArray(data['outer_vlan_ethertypes'])) {
            throw new Error("Expected the field `outer_vlan_ethertypes` to be an array in the JSON data but got " + data['outer_vlan_ethertypes']);
        }
        // ensure the json data is an array
        if (!Array.isArray(data['ports'])) {
            throw new Error("Expected the field `ports` to be an array in the JSON data but got " + data['ports']);
        }
        // ensure the json data is a string
        if (data['purchase_order'] && !(typeof data['purchase_order'] === 'string' || data['purchase_order'] instanceof String)) {
            throw new Error("Expected the field `purchase_order` to be a primitive type in the JSON string but got " + data['purchase_order']);
        }
        // ensure the json data is an array
        if (!Array.isArray(data['role_assignments'])) {
            throw new Error("Expected the field `role_assignments` to be an array in the JSON data but got " + data['role_assignments']);
        }
        // ensure the json data is a string
        if (data['state'] && !(typeof data['state'] === 'string' || data['state'] instanceof String)) {
            throw new Error("Expected the field `state` to be a primitive type in the JSON string but got " + data['state']);
        }
        if (data['status']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['status'])) {
                throw new Error("Expected the field `status` to be an array in the JSON data but got " + data['status']);
            }
            // validate the optional field `status` (array)
            for (const item of data['status']) {
                Status.validateJSON(item);
            };
        }
        // ensure the json data is an array
        if (!Array.isArray(data['vlan_types'])) {
            throw new Error("Expected the field `vlan_types` to be an array in the JSON data but got " + data['vlan_types']);
        }

        return true;
    }


}

Connection.RequiredProperties = ["billing_account", "consuming_account", "id", "managing_account", "mode", "name", "outer_vlan_ethertypes", "role_assignments", "state", "vlan_types"];

/**
 * An account requires billing_information to be used as a `billing_account`.
 * @member {String} billing_account
 */
Connection.prototype['billing_account'] = undefined;

/**
 * The `id` of the account consuming a service.  Used to be `owning_customer`. 
 * @member {String} consuming_account
 */
Connection.prototype['consuming_account'] = undefined;

/**
 * A reference to a contract. If no specific contract is used, a default MAY be chosen by the implementer. 
 * @member {String} contract_ref
 */
Connection.prototype['contract_ref'] = undefined;

/**
 * Reference field, free to use for the API user.
 * @member {String} external_ref
 */
Connection.prototype['external_ref'] = undefined;

/**
 * @member {String} id
 */
Connection.prototype['id'] = undefined;

/**
 * This sets the LACP Timeout mode. Both ends of the connections need to be configured the same. 
 * @member {module:model/Connection.LacpTimeoutEnum} lacp_timeout
 */
Connection.prototype['lacp_timeout'] = undefined;

/**
 * The `id` of the account responsible for managing the service via the API. A manager can read and update the state of entities. 
 * @member {String} managing_account
 */
Connection.prototype['managing_account'] = undefined;

/**
 * Sets the mode of the connection. The mode can be:  - `lag_lacp`: connection is build as a LAG with LACP enabled - `lag_static`: connection is build as LAG with static configuration - `flex_ethernet`: connect is build as a FlexEthernet channel - `standalone`: only one port is allowed in this connection without any bundling. 
 * @member {module:model/Connection.ModeEnum} mode
 */
Connection.prototype['mode'] = undefined;

/**
 * @member {String} name
 */
Connection.prototype['name'] = undefined;

/**
 * The ethertype of the outer tag in hexadecimal notation. 
 * @member {Array.<module:model/Connection.OuterVlanEthertypesEnum>} outer_vlan_ethertypes
 */
Connection.prototype['outer_vlan_ethertypes'] = undefined;

/**
 * References to the port belonging to this connection. Typically all ports within one connection are distributed over the same device. 
 * @member {Array.<String>} ports
 */
Connection.prototype['ports'] = undefined;

/**
 * Purchase Order ID which will be displayed on the invoice. 
 * @member {String} purchase_order
 * @default ''
 */
Connection.prototype['purchase_order'] = '';

/**
 * A set of `RoleAssignment`s. See the documentation on the specific `required_contact_roles`, `nfc_required_contact_roles` or `nsc_required_contact_roles` on what `RoleAssignment`s to provide. 
 * @member {Array.<String>} role_assignments
 */
Connection.prototype['role_assignments'] = undefined;

/**
 * Shows the total bandwidth of the connection in Mbit/s. 
 * @member {Number} speed
 */
Connection.prototype['speed'] = undefined;

/**
 * @member {module:model/Connection.StateEnum} state
 */
Connection.prototype['state'] = undefined;

/**
 * @member {Array.<module:model/Status>} status
 */
Connection.prototype['status'] = undefined;

/**
 * A list of vlan config types you can configure using this connection.
 * @member {Array.<module:model/Connection.VlanTypesEnum>} vlan_types
 */
Connection.prototype['vlan_types'] = undefined;


// Implement ConnectionPartial interface:
/**
 * An account requires billing_information to be used as a `billing_account`.
 * @member {String} billing_account
 */
ConnectionPartial.prototype['billing_account'] = undefined;
/**
 * The `id` of the account consuming a service.  Used to be `owning_customer`. 
 * @member {String} consuming_account
 */
ConnectionPartial.prototype['consuming_account'] = undefined;
/**
 * A reference to a contract. If no specific contract is used, a default MAY be chosen by the implementer. 
 * @member {String} contract_ref
 */
ConnectionPartial.prototype['contract_ref'] = undefined;
/**
 * Reference field, free to use for the API user.
 * @member {String} external_ref
 */
ConnectionPartial.prototype['external_ref'] = undefined;
/**
 * @member {String} id
 */
ConnectionPartial.prototype['id'] = undefined;
/**
 * This sets the LACP Timeout mode. Both ends of the connections need to be configured the same. 
 * @member {module:model/ConnectionPartial.LacpTimeoutEnum} lacp_timeout
 */
ConnectionPartial.prototype['lacp_timeout'] = undefined;
/**
 * The `id` of the account responsible for managing the service via the API. A manager can read and update the state of entities. 
 * @member {String} managing_account
 */
ConnectionPartial.prototype['managing_account'] = undefined;
/**
 * Sets the mode of the connection. The mode can be:  - `lag_lacp`: connection is build as a LAG with LACP enabled - `lag_static`: connection is build as LAG with static configuration - `flex_ethernet`: connect is build as a FlexEthernet channel - `standalone`: only one port is allowed in this connection without any bundling. 
 * @member {module:model/ConnectionPartial.ModeEnum} mode
 */
ConnectionPartial.prototype['mode'] = undefined;
/**
 * @member {String} name
 */
ConnectionPartial.prototype['name'] = undefined;
/**
 * The ethertype of the outer tag in hexadecimal notation. 
 * @member {Array.<module:model/ConnectionPartial.OuterVlanEthertypesEnum>} outer_vlan_ethertypes
 */
ConnectionPartial.prototype['outer_vlan_ethertypes'] = undefined;
/**
 * References to the port belonging to this connection. Typically all ports within one connection are distributed over the same device. 
 * @member {Array.<String>} ports
 */
ConnectionPartial.prototype['ports'] = undefined;
/**
 * Purchase Order ID which will be displayed on the invoice. 
 * @member {String} purchase_order
 * @default ''
 */
ConnectionPartial.prototype['purchase_order'] = '';
/**
 * A set of `RoleAssignment`s. See the documentation on the specific `required_contact_roles`, `nfc_required_contact_roles` or `nsc_required_contact_roles` on what `RoleAssignment`s to provide. 
 * @member {Array.<String>} role_assignments
 */
ConnectionPartial.prototype['role_assignments'] = undefined;
/**
 * Shows the total bandwidth of the connection in Mbit/s. 
 * @member {Number} speed
 */
ConnectionPartial.prototype['speed'] = undefined;
/**
 * @member {module:model/ConnectionPartial.StateEnum} state
 */
ConnectionPartial.prototype['state'] = undefined;
/**
 * @member {Array.<module:model/Status>} status
 */
ConnectionPartial.prototype['status'] = undefined;
/**
 * A list of vlan config types you can configure using this connection.
 * @member {Array.<module:model/ConnectionPartial.VlanTypesEnum>} vlan_types
 */
ConnectionPartial.prototype['vlan_types'] = undefined;



/**
 * Allowed values for the <code>lacp_timeout</code> property.
 * @enum {String}
 * @readonly
 */
Connection['LacpTimeoutEnum'] = {

    /**
     * value: "slow"
     * @const
     */
    "slow": "slow",

    /**
     * value: "fast"
     * @const
     */
    "fast": "fast"
};


/**
 * Allowed values for the <code>mode</code> property.
 * @enum {String}
 * @readonly
 */
Connection['ModeEnum'] = {

    /**
     * value: "lag_lacp"
     * @const
     */
    "lag_lacp": "lag_lacp",

    /**
     * value: "lag_static"
     * @const
     */
    "lag_static": "lag_static",

    /**
     * value: "flex_ethernet"
     * @const
     */
    "flex_ethernet": "flex_ethernet",

    /**
     * value: "standalone"
     * @const
     */
    "standalone": "standalone"
};


/**
 * Allowed values for the <code>outerVlanEthertypes</code> property.
 * @enum {String}
 * @readonly
 */
Connection['OuterVlanEthertypesEnum'] = {

    /**
     * value: "0x8100"
     * @const
     */
    "0x8100": "0x8100",

    /**
     * value: "0x88a8"
     * @const
     */
    "0x88a8": "0x88a8",

    /**
     * value: "0x9100"
     * @const
     */
    "0x9100": "0x9100"
};


/**
 * Allowed values for the <code>state</code> property.
 * @enum {String}
 * @readonly
 */
Connection['StateEnum'] = {

    /**
     * value: "requested"
     * @const
     */
    "requested": "requested",

    /**
     * value: "allocated"
     * @const
     */
    "allocated": "allocated",

    /**
     * value: "testing"
     * @const
     */
    "testing": "testing",

    /**
     * value: "production"
     * @const
     */
    "production": "production",

    /**
     * value: "production_change_pending"
     * @const
     */
    "production_change_pending": "production_change_pending",

    /**
     * value: "decommission_requested"
     * @const
     */
    "decommission_requested": "decommission_requested",

    /**
     * value: "decommissioned"
     * @const
     */
    "decommissioned": "decommissioned",

    /**
     * value: "archived"
     * @const
     */
    "archived": "archived",

    /**
     * value: "error"
     * @const
     */
    "error": "error",

    /**
     * value: "operator"
     * @const
     */
    "operator": "operator",

    /**
     * value: "scheduled"
     * @const
     */
    "scheduled": "scheduled"
};


/**
 * Allowed values for the <code>vlanTypes</code> property.
 * @enum {String}
 * @readonly
 */
Connection['VlanTypesEnum'] = {

    /**
     * value: "port"
     * @const
     */
    "port": "port",

    /**
     * value: "dot1q"
     * @const
     */
    "dot1q": "dot1q",

    /**
     * value: "qinq"
     * @const
     */
    "qinq": "qinq"
};



export default Connection;

