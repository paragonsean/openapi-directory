from typing import List, Dict
from aiohttp import web

from openapi_server.models.created_result import CreatedResult
from openapi_server.models.error import Error
from openapi_server.models.time_entry import TimeEntry
from openapi_server.models.time_entry_cursor_results import TimeEntryCursorResults
from openapi_server.models.time_entry_numbers_collection import TimeEntryNumbersCollection
from openapi_server import util


async def approve_time_entries_0(request: web.Request, body=None) -> web.Response:
    """Approve a list of Time entries

    Use this endpoint to approve a list of Time entries.

    :param body: 
    :type body: dict | bytes

    """
    body = TimeEntryNumbersCollection.from_dict(body)
    return web.Response(status=200)


async def create_time_entry(request: web.Request, body=None) -> web.Response:
    """Create a single Time entry

    Use this endpoint to create a single Time entry.

    :param body: 
    :type body: dict | bytes

    """
    body = TimeEntry.from_dict(body)
    return web.Response(status=200)


async def delete_time_entry_by_id(request: web.Request, number) -> web.Response:
    """Delete single Time entry

    Use this endpoint to delete a single Time entry by id/number.

    :param number: 
    :type number: int

    """
    return web.Response(status=200)


async def get_all_time_entries(request: web.Request, cursor=None, filter=None) -> web.Response:
    """Retrieve all Time entries

    Use this endpoint to retrieve all Time entries in bulk.  Max number of items returned in a single call is 1000. Use the continuation cursor parameter to set the continuation cursor for retrieval of next set of data. [pagination instructions](#section/Retrieving-data/Pagination)

    :param cursor: 
    :type cursor: str
    :param filter: 
    :type filter: str

    """
    return web.Response(status=200)


async def get_number_of_time_entries(request: web.Request, filter=None) -> web.Response:
    """Retrieve the number of Time entries

    Call this endpoint to get the number of Time entries. You can use a filtering as well.

    :param filter: 
    :type filter: str

    """
    return web.Response(status=200)


async def get_page_of_time_entries(request: web.Request, filter=None, sort=None, page_size=None, skip_pages=None) -> web.Response:
    """Retrieve a page of Time entries

    Use this endpoint to load a page of Time entries.

    :param filter: 
    :type filter: str
    :param sort: 
    :type sort: str
    :param page_size: 
    :type page_size: int
    :param skip_pages: 
    :type skip_pages: int

    """
    return web.Response(status=200)


async def get_time_entry_by_id(request: web.Request, number) -> web.Response:
    """Retrieve single Time entry

    Use this endpoint to load a single Time entry by id/number.

    :param number: 
    :type number: int

    """
    return web.Response(status=200)


async def update_time_entry(request: web.Request, body=None) -> web.Response:
    """Update a single Time entry

    Use this endpoint to update a single Time entry.

    :param body: 
    :type body: dict | bytes

    """
    body = TimeEntry.from_dict(body)
    return web.Response(status=200)
