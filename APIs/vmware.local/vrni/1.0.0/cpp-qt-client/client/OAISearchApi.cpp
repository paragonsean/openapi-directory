...
void OAISearchApi::searchEntities(const ::OpenAPI::OptionalParam<OAISearchRequest> &oai_search_request) {
    QString fullPath = QString(_serverConfigs["searchEntities"][_serverIndices.value("searchEntities")].URL()+"/search");
    ...
    if (oai_search_request.hasValue()){

        
        QByteArray output = oai_search_request.value().asJson().toUtf8();
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAISearchApi::searchEntitiesCallback);
...
void OAISearchApi::searchEntitiesCallback(OAIHttpRequestWorker *worker) {
