# MastodonApiSpecificationHttpsGithubComMastodonMastodon.Mention

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**acct** | **String** | The webfinger acct URI of the mentioned user. Equivalent to &#x60;username&#x60; for local users, or &#x60;username@domain&#x60; for remote users. | 
**id** | **String** | The account id of the mentioned user. Cast from an integer, but not guaranteed to be a number | 
**url** | **String** | The location of the mentioned user&#39;s profile. | 
**username** | **String** | The username of the mentioned user. | 


