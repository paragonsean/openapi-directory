...
async def get_air_traffic(request: web.Request, origin_city_code, period, max=None, fields=None, page_limit=None, page_offset=None, sort=None) -> web.Response:
    ...
    :param period: period when consumers are traveling. * It can be a month only.  * [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format must be used - e.g. 2015-05.  * Period ranges are not supported.  * Only periods from 2011-01 up to previous month are valid.  * Future dates are not supported. 
    :type period: str
    ...
    :param sort: defines on which attribute the sorting will be done: * **analytics.flights.score** - sort destination by flights score (decreasing) * **analytics.travelers.score** - sort destination by traveler&#39;s score (decreasing) 
    :type sort: str
