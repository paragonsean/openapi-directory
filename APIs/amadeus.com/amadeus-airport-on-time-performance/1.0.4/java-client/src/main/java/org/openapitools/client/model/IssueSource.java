...
public class IssueSource {
   ...
   * @throws IOException if the JSON Element is invalid with respect to IssueSource
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!IssueSource.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in IssueSource is not found in the empty JSON string", IssueSource.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!IssueSource.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `IssueSource` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!IssueSource.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'IssueSource' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<IssueSource> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(IssueSource.class));

       return (TypeAdapter<T>) new TypeAdapter<IssueSource>() {
           @Override
           public void write(JsonWriter out, IssueSource value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public IssueSource read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of IssueSource given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of IssueSource
   * @throws IOException if the JSON string is invalid with respect to IssueSource
   */
  public static IssueSource fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, IssueSource.class);
  }

  /**
   * Convert an instance of IssueSource to an JSON string
   *
   * @return JSON string
   */
