...
void OAIAppsApi::initializeServerConfigs() {
    ...
    _serverConfigs.insert("appsIDGet", defaultConf);
    _serverIndices.insert("appsIDGet", 0);
}
...
void OAIAppsApi::appsGet(const ::OpenAPI::OptionalParam<qint32> &offset, const ::OpenAPI::OptionalParam<qint32> &limit) {
...
void OAIAppsApi::appsGetCallback(OAIHttpRequestWorker *worker) {
    ...
    OAIEndpoint_get_apps output(QString(worker->response));
    worker->deleteLater();
...
void OAIAppsApi::appsIDGet(const QList<qint32> &id) {
    QString fullPath = QString(_serverConfigs["appsIDGet"][_serverIndices.value("appsIDGet")].URL()+"/apps/{ID}");
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIAppsApi::appsIDGetCallback);
    connect(this, &OAIAppsApi::abortRequestsSignal, worker, &QObject::deleteLater);
    connect(worker, &QObject::destroyed, this, [this]() {
        if (findChildren<OAIHttpRequestWorker*>().count() == 0) {
            Q_EMIT allPendingRequestsCompleted();
    ...
    connect(_latestWorker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIAppsApi::appsIDGetCallback);
    connect(this, &OAIAppsApi::abortRequestsSignal, _latestWorker, &QObject::deleteLater);
    connect(_latestWorker, &QObject::destroyed, [this](){
        if(findChildren<OAIHttpRequestWorker*>().count() == 0){
            Q_EMIT allPendingRequestsCompleted();
...
void OAIAppsApi::appsIDGetCallback(OAIHttpRequestWorker *worker) {
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
    ...
    OAIEndpoint_get_apps_ID output(QString(worker->response));
    worker->deleteLater();

    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT appsIDGetSignal(output);
        Q_EMIT appsIDGetSignalFull(worker, output);
    } else if(worker->error_type == QNetworkReply::AuthenticationRequiredError){
        connect(&_implicitFlow, SIGNAL(tokenReceived()), this, SLOT(tokenAvailable()));
        QStringList scope;
        scope.append("basic");
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT appsIDGetSignalE(output, error_type, error_str);
        Q_EMIT appsIDGetSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT appsIDGetSignalError(output, error_type, error_str);
        Q_EMIT appsIDGetSignalErrorFull(worker, error_type, error_str);
    }
}
