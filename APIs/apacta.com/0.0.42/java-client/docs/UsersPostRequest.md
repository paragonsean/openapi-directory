

# UsersPostRequest


## Properties

| Name | Type | Description | Notes |
|------------ | ------------- | ------------- | -------------|
|**cityId** | **UUID** |  |  [optional] |
|**costPrice** | **Float** | Cost of salaries |  [optional] |
|**email** | **String** |  |  [optional] |
|**expectedBillableHours** | **Float** |  |  [optional] |
|**extraPrice** | **Float** | Additional cost on this employee (pension, vacation etc.) |  [optional] |
|**firstName** | **String** |  |  |
|**hideAddress** | **Boolean** |  |  [optional] |
|**hidePhone** | **Boolean** |  |  [optional] |
|**initials** | **String** |  |  [optional] |
|**isActive** | **Boolean** |  |  [optional] |
|**languageId** | **UUID** |  |  [optional] |
|**lastName** | **String** |  |  [optional] |
|**mobile** | **String** |  |  [optional] |
|**mobileCountrycode** | **String** |  |  [optional] |
|**password** | **String** |  |  [optional] |
|**phone** | **String** |  |  [optional] |
|**phoneCountrycode** | **String** |  |  [optional] |
|**receiveFormMails** | **Boolean** | If &#x60;true&#x60; the employee will receive an email receipt of every form submitted |  [optional] |
|**roles** | [**ContactsPostRequestContactTypes**](ContactsPostRequestContactTypes.md) |  |  [optional] |
|**salePrice** | **Float** | The price this employee costs per hour when working |  [optional] |
|**streetName** | **String** |  |  [optional] |



