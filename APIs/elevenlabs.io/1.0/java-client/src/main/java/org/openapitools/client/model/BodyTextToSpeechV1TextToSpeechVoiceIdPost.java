...
public class BodyTextToSpeechV1TextToSpeechVoiceIdPost {
   ...
   * @throws IOException if the JSON Element is invalid with respect to BodyTextToSpeechV1TextToSpeechVoiceIdPost
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!BodyTextToSpeechV1TextToSpeechVoiceIdPost.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!BodyTextToSpeechV1TextToSpeechVoiceIdPost.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BodyTextToSpeechV1TextToSpeechVoiceIdPost` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
