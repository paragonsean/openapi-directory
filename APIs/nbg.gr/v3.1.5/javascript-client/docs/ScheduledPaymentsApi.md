# AccountAndTransactionApiSpecificationUk.ScheduledPaymentsApi
...
> OBReadScheduledPayment3 accountsAccountIdScheduledPaymentsGet(accountId, sandboxId, opts)
...
let apiInstance = new AccountAndTransactionApiSpecificationUk.ScheduledPaymentsApi();
...
apiInstance.accountsAccountIdScheduledPaymentsGet(accountId, sandboxId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
