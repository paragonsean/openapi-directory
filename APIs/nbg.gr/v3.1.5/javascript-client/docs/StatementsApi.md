# AccountAndTransactionApiSpecificationUk.StatementsApi
...
> OBReadStatement2 accountsAccountIdStatementsGet(accountId, sandboxId, opts)
...
let apiInstance = new AccountAndTransactionApiSpecificationUk.StatementsApi();
...
let opts = {
  'fromStatementDateTime': new Date("2013-10-20T19:20:30+01:00"), // Date | The UTC ISO 8601 Date Time to filter statements FROM NB Time component is optional - set to 00:00:00 for just Date. If the Date Time contains a timezone, the ASPSP must ignore the timezone component.
  'toStatementDateTime': new Date("2013-10-20T19:20:30+01:00"), // Date | The UTC ISO 8601 Date Time to filter statements TO NB Time component is optional - set to 00:00:00 for just Date. If the Date Time contains a timezone, the ASPSP must ignore the timezone component.
...
apiInstance.accountsAccountIdStatementsGet(accountId, sandboxId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
> File accountsAccountIdStatementsStatementIdFileGet(accountId, statementId, sandboxId, opts)
...
apiInstance.accountsAccountIdStatementsStatementIdFileGet(accountId, statementId, sandboxId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
...
> OBReadStatement2 accountsAccountIdStatementsStatementIdGet(accountId, statementId, sandboxId, opts)
...
apiInstance.accountsAccountIdStatementsStatementIdGet(accountId, statementId, sandboxId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
