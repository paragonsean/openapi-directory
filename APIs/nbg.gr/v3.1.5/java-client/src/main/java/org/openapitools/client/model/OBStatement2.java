...
public class OBStatement2 {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(accountId, creationDateTime, endDateTime, startDateTime, statementId, statementReference, type);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to OBStatement2
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!OBStatement2.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!OBStatement2.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `OBStatement2` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      OBExternalStatementType1Code.validateJsonElement(jsonObj.get("Type"));
  }
