...
import org.openapitools.client.model.OBCreditLine1;
...
public class OBCashBalance1 {
   ...
   * @throws IOException if the JSON Element is invalid with respect to OBCashBalance1
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!OBCashBalance1.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in OBCashBalance1 is not found in the empty JSON string", OBCashBalance1.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!OBCashBalance1.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `OBCashBalance1` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : OBCashBalance1.openapiRequiredFields) {
      ...
      if (jsonObj.get("CreditLine") != null && !jsonObj.get("CreditLine").isJsonNull()) {
        ...
        if (jsonArraycreditLine != null) {
          ...
          for (int i = 0; i < jsonArraycreditLine.size(); i++) {
            OBCreditLine1.validateJsonElement(jsonArraycreditLine.get(i));
          };
        }
      }
      // validate the required field `Type`
      OBBalanceType1Code.validateJsonElement(jsonObj.get("Type"));
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!OBCashBalance1.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'OBCashBalance1' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<OBCashBalance1> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(OBCashBalance1.class));

       return (TypeAdapter<T>) new TypeAdapter<OBCashBalance1>() {
           @Override
           public void write(JsonWriter out, OBCashBalance1 value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public OBCashBalance1 read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of OBCashBalance1 given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of OBCashBalance1
   * @throws IOException if the JSON string is invalid with respect to OBCashBalance1
   */
  public static OBCashBalance1 fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, OBCashBalance1.class);
  }

  /**
   * Convert an instance of OBCashBalance1 to an JSON string
   *
   * @return JSON string
   */
