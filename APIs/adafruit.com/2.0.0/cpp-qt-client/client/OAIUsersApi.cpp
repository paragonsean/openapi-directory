...
void OAIUsersApi::initializeServerConfigs() {
    ...
    "No description provided",
    QMap<QString, OAIServerVariable>()));
    _serverConfigs.insert("currentUser", defaultConf);
    _serverIndices.insert("currentUser", 0);
    _serverConfigs.insert("getCurrentUserThrottle", defaultConf);
    _serverIndices.insert("getCurrentUserThrottle", 0);
}
...
void OAIUsersApi::currentUser() {
    QString fullPath = QString(_serverConfigs["currentUser"][_serverIndices.value("currentUser")].URL()+"/user");
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIUsersApi::currentUserCallback);
...
void OAIUsersApi::currentUserCallback(OAIHttpRequestWorker *worker) {
    ...
    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT currentUserSignal(output);
        Q_EMIT currentUserSignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT currentUserSignalE(output, error_type, error_str);
        Q_EMIT currentUserSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT currentUserSignalError(output, error_type, error_str);
        Q_EMIT currentUserSignalErrorFull(worker, error_type, error_str);
    }
}

void OAIUsersApi::getCurrentUserThrottle(const QString &username) {
    QString fullPath = QString(_serverConfigs["getCurrentUserThrottle"][_serverIndices.value("getCurrentUserThrottle")].URL()+"/{username}/throttle");
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIUsersApi::getCurrentUserThrottleCallback);
    connect(this, &OAIUsersApi::abortRequestsSignal, worker, &QObject::deleteLater);
    connect(worker, &QObject::destroyed, this, [this]() {
        if (findChildren<OAIHttpRequestWorker*>().count() == 0) {
            Q_EMIT allPendingRequestsCompleted();
...
void OAIUsersApi::getCurrentUserThrottleCallback(OAIHttpRequestWorker *worker) {
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
    ...
    OAIGetCurrentUserThrottle_200_response output(QString(worker->response));
    worker->deleteLater();

    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT getCurrentUserThrottleSignal(output);
        Q_EMIT getCurrentUserThrottleSignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT getCurrentUserThrottleSignalE(output, error_type, error_str);
        Q_EMIT getCurrentUserThrottleSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT getCurrentUserThrottleSignalError(output, error_type, error_str);
        Q_EMIT getCurrentUserThrottleSignalErrorFull(worker, error_type, error_str);
    }
}
