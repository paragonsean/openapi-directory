...
public class Picture {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(active, link, resourceKey, sizes, type, uri);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to Picture
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Picture.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Picture.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Picture` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArraysizes.size(); i++) {
        PictureSizesInner.validateJsonElement(jsonArraysizes.get(i));
      };
