/**
 * Vimeo
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 3.4
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', process.cwd()+'/src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require(process.cwd()+'/src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.Vimeo);
  }
}(this, function(expect, Vimeo) {
  'use strict';

  var instance;

  beforeEach(function() {
    instance = new Vimeo.VideoMetadataInteractionsBuy();
  });

  var getProperty = function(object, getter, property) {
    // Use getter method if present; otherwise, get the property directly.
    if (typeof object[getter] === 'function')
      return object[getter]();
    else
      return object[property];
  }

  var setProperty = function(object, setter, property, value) {
    // Use setter method if present; otherwise, set the property directly.
    if (typeof object[setter] === 'function')
      object[setter](value);
    else
      object[property] = value;
  }

  describe('VideoMetadataInteractionsBuy', function() {
    it('should create an instance of VideoMetadataInteractionsBuy', function() {
      // uncomment below and update the code to test VideoMetadataInteractionsBuy
      //var instance = new Vimeo.VideoMetadataInteractionsBuy();
      //expect(instance).to.be.a(Vimeo.VideoMetadataInteractionsBuy);
    });

    it('should have the property currency (base name: "currency")', function() {
      // uncomment below and update the code to test the property currency
      //var instance = new Vimeo.VideoMetadataInteractionsBuy();
      //expect(instance).to.be();
    });

    it('should have the property displayPrice (base name: "display_price")', function() {
      // uncomment below and update the code to test the property displayPrice
      //var instance = new Vimeo.VideoMetadataInteractionsBuy();
      //expect(instance).to.be();
    });

    it('should have the property download (base name: "download")', function() {
      // uncomment below and update the code to test the property download
      //var instance = new Vimeo.VideoMetadataInteractionsBuy();
      //expect(instance).to.be();
    });

    it('should have the property drm (base name: "drm")', function() {
      // uncomment below and update the code to test the property drm
      //var instance = new Vimeo.VideoMetadataInteractionsBuy();
      //expect(instance).to.be();
    });

    it('should have the property link (base name: "link")', function() {
      // uncomment below and update the code to test the property link
      //var instance = new Vimeo.VideoMetadataInteractionsBuy();
      //expect(instance).to.be();
    });

    it('should have the property price (base name: "price")', function() {
      // uncomment below and update the code to test the property price
      //var instance = new Vimeo.VideoMetadataInteractionsBuy();
      //expect(instance).to.be();
    });

    it('should have the property purchaseTime (base name: "purchase_time")', function() {
      // uncomment below and update the code to test the property purchaseTime
      //var instance = new Vimeo.VideoMetadataInteractionsBuy();
      //expect(instance).to.be();
    });

    it('should have the property stream (base name: "stream")', function() {
      // uncomment below and update the code to test the property stream
      //var instance = new Vimeo.VideoMetadataInteractionsBuy();
      //expect(instance).to.be();
    });

    it('should have the property uri (base name: "uri")', function() {
      // uncomment below and update the code to test the property uri
      //var instance = new Vimeo.VideoMetadataInteractionsBuy();
      //expect(instance).to.be();
    });

  });

}));
