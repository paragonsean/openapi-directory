...
> [Channel] getCategoryChannels(category, opts)
...
let apiInstance = new Vimeo.CategoriesChannelsApi();
...
apiInstance.getCategoryChannels(category, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
