/**
 * Amazon Pinpoint
 * Doc Engage API - Amazon Pinpoint API
 *
 * The version of the OpenAPI document: 2016-12-01
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';

/**
 * The MessageResponse model module.
 * @module model/MessageResponse
 * @version 2016-12-01
 */
class MessageResponse {
    /**
     * Constructs a new <code>MessageResponse</code>.
     * Provides information about the results of a request to send a message to an endpoint address.
     * @alias module:model/MessageResponse
     * @param applicationId {String} 
     */
    constructor(applicationId) { 
        
        MessageResponse.initialize(this, applicationId);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj, applicationId) { 
        obj['ApplicationId'] = applicationId;
    }

    /**
     * Constructs a <code>MessageResponse</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/MessageResponse} obj Optional instance to populate.
     * @return {module:model/MessageResponse} The populated <code>MessageResponse</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new MessageResponse();

            if (data.hasOwnProperty('ApplicationId')) {
                obj['ApplicationId'] = ApiClient.convertToType(data['ApplicationId'], 'String');
            }
            if (data.hasOwnProperty('EndpointResult')) {
                obj['EndpointResult'] = ApiClient.convertToType(data['EndpointResult'], Object);
            }
            if (data.hasOwnProperty('RequestId')) {
                obj['RequestId'] = ApiClient.convertToType(data['RequestId'], 'String');
            }
            if (data.hasOwnProperty('Result')) {
                obj['Result'] = ApiClient.convertToType(data['Result'], Object);
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>MessageResponse</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>MessageResponse</code>.
     */
    static validateJSON(data) {
        // check to make sure all required properties are present in the JSON string
        for (const property of MessageResponse.RequiredProperties) {
            if (!data.hasOwnProperty(property)) {
                throw new Error("The required field `" + property + "` is not found in the JSON data: " + JSON.stringify(data));
            }
        }
        // validate the optional field `ApplicationId`
        if (data['ApplicationId']) { // data not null
          String.validateJSON(data['ApplicationId']);
        }
        // validate the optional field `EndpointResult`
        if (data['EndpointResult']) { // data not null
          Object.validateJSON(data['EndpointResult']);
        }
        // validate the optional field `RequestId`
        if (data['RequestId']) { // data not null
          String.validateJSON(data['RequestId']);
        }
        // validate the optional field `Result`
        if (data['Result']) { // data not null
          Object.validateJSON(data['Result']);
        }

        return true;
    }


}

MessageResponse.RequiredProperties = ["ApplicationId"];

/**
 * @member {String} ApplicationId
 */
MessageResponse.prototype['ApplicationId'] = undefined;

/**
 * @member {Object} EndpointResult
 */
MessageResponse.prototype['EndpointResult'] = undefined;

/**
 * @member {String} RequestId
 */
MessageResponse.prototype['RequestId'] = undefined;

/**
 * @member {Object} Result
 */
MessageResponse.prototype['Result'] = undefined;






export default MessageResponse;

