...
public class PolicyAttribute {
   ...
   * @throws IOException if the JSON Element is invalid with respect to PolicyAttribute
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PolicyAttribute.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PolicyAttribute is not found in the empty JSON string", PolicyAttribute.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PolicyAttribute.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PolicyAttribute` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PolicyAttribute.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PolicyAttribute' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PolicyAttribute> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PolicyAttribute.class));

       return (TypeAdapter<T>) new TypeAdapter<PolicyAttribute>() {
           @Override
           public void write(JsonWriter out, PolicyAttribute value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PolicyAttribute read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of PolicyAttribute given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of PolicyAttribute
   * @throws IOException if the JSON string is invalid with respect to PolicyAttribute
   */
  public static PolicyAttribute fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PolicyAttribute.class);
  }

  /**
   * Convert an instance of PolicyAttribute to an JSON string
   *
   * @return JSON string
   */
