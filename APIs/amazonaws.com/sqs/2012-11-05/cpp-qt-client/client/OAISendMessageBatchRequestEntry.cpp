/**
 * Amazon Simple Queue Service
 * <p>Welcome to the <i>Amazon SQS API Reference</i>.</p> <p>Amazon SQS is a reliable, highly-scalable hosted queue for storing messages as they travel between applications or microservices. Amazon SQS moves data between distributed application components and helps you decouple these components.</p> <p>For information on the permissions you need to use this API, see <a href=\"https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-authentication-and-access-control.html\">Identity and access management</a> in the <i>Amazon SQS Developer Guide.</i> </p> <p>You can use <a href=\"http://aws.amazon.com/tools/#sdk\">Amazon Web Services SDKs</a> to access Amazon SQS using your favorite programming language. The SDKs perform tasks such as the following automatically:</p> <ul> <li> <p>Cryptographically sign your service requests</p> </li> <li> <p>Retry requests</p> </li> <li> <p>Handle error responses</p> </li> </ul> <p> <b>Additional information</b> </p> <ul> <li> <p> <a href=\"http://aws.amazon.com/sqs/\">Amazon SQS Product Page</a> </p> </li> <li> <p> <i>Amazon SQS Developer Guide</i> </p> <ul> <li> <p> <a href=\"https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-making-api-requests.html\">Making API Requests</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-message-metadata.html#sqs-message-attributes\">Amazon SQS Message Attributes</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-dead-letter-queues.html\">Amazon SQS Dead-Letter Queues</a> </p> </li> </ul> </li> <li> <p> <a href=\"http://docs.aws.amazon.com/cli/latest/reference/sqs/index.html\">Amazon SQS in the <i>Command Line Interface</i> </a> </p> </li> <li> <p> <i>Amazon Web Services General Reference</i> </p> <ul> <li> <p> <a href=\"https://docs.aws.amazon.com/general/latest/gr/rande.html#sqs_region\">Regions and Endpoints</a> </p> </li> </ul> </li> </ul>
 *
 * The version of the OpenAPI document: 2012-11-05
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAISendMessageBatchRequestEntry.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAISendMessageBatchRequestEntry::OAISendMessageBatchRequestEntry(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAISendMessageBatchRequestEntry::OAISendMessageBatchRequestEntry() {
    this->initializeModel();
}

OAISendMessageBatchRequestEntry::~OAISendMessageBatchRequestEntry() {}

void OAISendMessageBatchRequestEntry::initializeModel() {

    m_id_isSet = false;
    m_id_isValid = false;

    m_message_body_isSet = false;
    m_message_body_isValid = false;

    m_delay_seconds_isSet = false;
    m_delay_seconds_isValid = false;

    m_message_attributes_isSet = false;
    m_message_attributes_isValid = false;

    m_message_system_attributes_isSet = false;
    m_message_system_attributes_isValid = false;

    m_message_deduplication_id_isSet = false;
    m_message_deduplication_id_isValid = false;

    m_message_group_id_isSet = false;
    m_message_group_id_isValid = false;
}

void OAISendMessageBatchRequestEntry::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAISendMessageBatchRequestEntry::fromJsonObject(QJsonObject json) {

    m_id_isValid = ::OpenAPI::fromJsonValue(m_id, json[QString("Id")]);
    m_id_isSet = !json[QString("Id")].isNull() && m_id_isValid;

    m_message_body_isValid = ::OpenAPI::fromJsonValue(m_message_body, json[QString("MessageBody")]);
    m_message_body_isSet = !json[QString("MessageBody")].isNull() && m_message_body_isValid;

    m_delay_seconds_isValid = ::OpenAPI::fromJsonValue(m_delay_seconds, json[QString("DelaySeconds")]);
    m_delay_seconds_isSet = !json[QString("DelaySeconds")].isNull() && m_delay_seconds_isValid;

    m_message_attributes_isValid = ::OpenAPI::fromJsonValue(m_message_attributes, json[QString("MessageAttributes")]);
    m_message_attributes_isSet = !json[QString("MessageAttributes")].isNull() && m_message_attributes_isValid;

    m_message_system_attributes_isValid = ::OpenAPI::fromJsonValue(m_message_system_attributes, json[QString("MessageSystemAttributes")]);
    m_message_system_attributes_isSet = !json[QString("MessageSystemAttributes")].isNull() && m_message_system_attributes_isValid;

    m_message_deduplication_id_isValid = ::OpenAPI::fromJsonValue(m_message_deduplication_id, json[QString("MessageDeduplicationId")]);
    m_message_deduplication_id_isSet = !json[QString("MessageDeduplicationId")].isNull() && m_message_deduplication_id_isValid;

    m_message_group_id_isValid = ::OpenAPI::fromJsonValue(m_message_group_id, json[QString("MessageGroupId")]);
    m_message_group_id_isSet = !json[QString("MessageGroupId")].isNull() && m_message_group_id_isValid;
}

QString OAISendMessageBatchRequestEntry::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAISendMessageBatchRequestEntry::asJsonObject() const {
    QJsonObject obj;
    if (m_id_isSet) {
        obj.insert(QString("Id"), ::OpenAPI::toJsonValue(m_id));
    }
    if (m_message_body_isSet) {
        obj.insert(QString("MessageBody"), ::OpenAPI::toJsonValue(m_message_body));
    }
    if (m_delay_seconds_isSet) {
        obj.insert(QString("DelaySeconds"), ::OpenAPI::toJsonValue(m_delay_seconds));
    }
    if (m_message_attributes.isSet()) {
        obj.insert(QString("MessageAttributes"), ::OpenAPI::toJsonValue(m_message_attributes));
    }
    if (m_message_system_attributes.isSet()) {
        obj.insert(QString("MessageSystemAttributes"), ::OpenAPI::toJsonValue(m_message_system_attributes));
    }
    if (m_message_deduplication_id_isSet) {
        obj.insert(QString("MessageDeduplicationId"), ::OpenAPI::toJsonValue(m_message_deduplication_id));
    }
    if (m_message_group_id_isSet) {
        obj.insert(QString("MessageGroupId"), ::OpenAPI::toJsonValue(m_message_group_id));
    }
    return obj;
}

QString OAISendMessageBatchRequestEntry::getId() const {
    return m_id;
}
void OAISendMessageBatchRequestEntry::setId(const QString &id) {
    m_id = id;
    m_id_isSet = true;
}

bool OAISendMessageBatchRequestEntry::is_id_Set() const{
    return m_id_isSet;
}

bool OAISendMessageBatchRequestEntry::is_id_Valid() const{
    return m_id_isValid;
}

QString OAISendMessageBatchRequestEntry::getMessageBody() const {
    return m_message_body;
}
void OAISendMessageBatchRequestEntry::setMessageBody(const QString &message_body) {
    m_message_body = message_body;
    m_message_body_isSet = true;
}

bool OAISendMessageBatchRequestEntry::is_message_body_Set() const{
    return m_message_body_isSet;
}

bool OAISendMessageBatchRequestEntry::is_message_body_Valid() const{
    return m_message_body_isValid;
}

qint32 OAISendMessageBatchRequestEntry::getDelaySeconds() const {
    return m_delay_seconds;
}
void OAISendMessageBatchRequestEntry::setDelaySeconds(const qint32 &delay_seconds) {
    m_delay_seconds = delay_seconds;
    m_delay_seconds_isSet = true;
}

bool OAISendMessageBatchRequestEntry::is_delay_seconds_Set() const{
    return m_delay_seconds_isSet;
}

bool OAISendMessageBatchRequestEntry::is_delay_seconds_Valid() const{
    return m_delay_seconds_isValid;
}

QMap OAISendMessageBatchRequestEntry::getMessageAttributes() const {
    return m_message_attributes;
}
void OAISendMessageBatchRequestEntry::setMessageAttributes(const QMap &message_attributes) {
    m_message_attributes = message_attributes;
    m_message_attributes_isSet = true;
}

bool OAISendMessageBatchRequestEntry::is_message_attributes_Set() const{
    return m_message_attributes_isSet;
}

bool OAISendMessageBatchRequestEntry::is_message_attributes_Valid() const{
    return m_message_attributes_isValid;
}

QMap OAISendMessageBatchRequestEntry::getMessageSystemAttributes() const {
    return m_message_system_attributes;
}
void OAISendMessageBatchRequestEntry::setMessageSystemAttributes(const QMap &message_system_attributes) {
    m_message_system_attributes = message_system_attributes;
    m_message_system_attributes_isSet = true;
}

bool OAISendMessageBatchRequestEntry::is_message_system_attributes_Set() const{
    return m_message_system_attributes_isSet;
}

bool OAISendMessageBatchRequestEntry::is_message_system_attributes_Valid() const{
    return m_message_system_attributes_isValid;
}

QString OAISendMessageBatchRequestEntry::getMessageDeduplicationId() const {
    return m_message_deduplication_id;
}
void OAISendMessageBatchRequestEntry::setMessageDeduplicationId(const QString &message_deduplication_id) {
    m_message_deduplication_id = message_deduplication_id;
    m_message_deduplication_id_isSet = true;
}

bool OAISendMessageBatchRequestEntry::is_message_deduplication_id_Set() const{
    return m_message_deduplication_id_isSet;
}

bool OAISendMessageBatchRequestEntry::is_message_deduplication_id_Valid() const{
    return m_message_deduplication_id_isValid;
}

QString OAISendMessageBatchRequestEntry::getMessageGroupId() const {
    return m_message_group_id;
}
void OAISendMessageBatchRequestEntry::setMessageGroupId(const QString &message_group_id) {
    m_message_group_id = message_group_id;
    m_message_group_id_isSet = true;
}

bool OAISendMessageBatchRequestEntry::is_message_group_id_Set() const{
    return m_message_group_id_isSet;
}

bool OAISendMessageBatchRequestEntry::is_message_group_id_Valid() const{
    return m_message_group_id_isValid;
}

bool OAISendMessageBatchRequestEntry::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_message_body_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_delay_seconds_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_message_attributes.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_message_system_attributes.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_message_deduplication_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_message_group_id_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAISendMessageBatchRequestEntry::isValid() const {
    // only required properties are required for the object to be considered valid
    return m_id_isValid && m_message_body_isValid && true;
}

} // namespace OpenAPI
