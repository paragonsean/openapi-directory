# AmazonConnectService.StartTaskContactRequest
...
**references** | [**{String: Reference}**](Reference.md) | A formatted URL that is shown to an agent in the Contact Control Panel (CCP). | [optional] 
...
**scheduledTime** | **Date** | The timestamp, in Unix Epoch seconds format, at which to start running the inbound flow. The scheduled time cannot be in the past. It must be within up to 6 days in future.  | [optional] 
