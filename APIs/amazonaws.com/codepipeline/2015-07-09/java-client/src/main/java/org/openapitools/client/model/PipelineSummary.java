...
public class PipelineSummary {
  ...
  public PipelineSummary name(String name) {
    this.name = name;
    return this;
  }
  ...
  public PipelineSummary version(Integer version) {
  ...
  public PipelineSummary created(OffsetDateTime created) {
  ...
  public PipelineSummary updated(OffsetDateTime updated) {
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(this.name, pipelineSummary.name) &&
        Objects.equals(this.version, pipelineSummary.version) &&
        Objects.equals(this.created, pipelineSummary.created) &&
        Objects.equals(this.updated, pipelineSummary.updated);
  }

  @Override
  public int hashCode() {
    return Objects.hash(name, version, created, updated);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to PipelineSummary
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PipelineSummary.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PipelineSummary is not found in the empty JSON string", PipelineSummary.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PipelineSummary.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PipelineSummary` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PipelineSummary.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PipelineSummary' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PipelineSummary> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PipelineSummary.class));

       return (TypeAdapter<T>) new TypeAdapter<PipelineSummary>() {
           @Override
           public void write(JsonWriter out, PipelineSummary value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PipelineSummary read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of PipelineSummary given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of PipelineSummary
   * @throws IOException if the JSON string is invalid with respect to PipelineSummary
   */
  public static PipelineSummary fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PipelineSummary.class);
  }

  /**
   * Convert an instance of PipelineSummary to an JSON string
   *
   * @return JSON string
   */
