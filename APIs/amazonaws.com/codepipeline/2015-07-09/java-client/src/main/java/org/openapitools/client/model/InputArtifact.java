...
public class InputArtifact {
  ...
  public InputArtifact name(String name) {
    this.name = name;
    return this;
  }
  ...
  public boolean equals(Object o) {
    ...
    InputArtifact inputArtifact = (InputArtifact) o;
    return Objects.equals(this.name, inputArtifact.name);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to InputArtifact
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!InputArtifact.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in InputArtifact is not found in the empty JSON string", InputArtifact.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!InputArtifact.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `InputArtifact` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : InputArtifact.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!InputArtifact.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'InputArtifact' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<InputArtifact> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(InputArtifact.class));

       return (TypeAdapter<T>) new TypeAdapter<InputArtifact>() {
           @Override
           public void write(JsonWriter out, InputArtifact value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public InputArtifact read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of InputArtifact given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of InputArtifact
   * @throws IOException if the JSON string is invalid with respect to InputArtifact
   */
  public static InputArtifact fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, InputArtifact.class);
  }

  /**
   * Convert an instance of InputArtifact to an JSON string
   *
   * @return JSON string
   */
