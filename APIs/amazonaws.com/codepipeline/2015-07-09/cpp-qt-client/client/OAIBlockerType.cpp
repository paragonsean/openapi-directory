...
OAIBlockerType::OAIBlockerType(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIBlockerType::OAIBlockerType() {
    this->initializeModel();
}

OAIBlockerType::~OAIBlockerType() {}

void OAIBlockerType::initializeModel() {

    m_value_isSet = false;
    m_value_isValid = false;
    m_value = eOAIBlockerType::INVALID_VALUE_OPENAPI_GENERATED;
}

void OAIBlockerType::fromJson(QString jsonString) {
    
    if ( jsonString.compare("Schedule", Qt::CaseInsensitive) == 0) {
        m_value = eOAIBlockerType::SCHEDULE;
        m_value_isSet = m_value_isValid = true;
    }
}

void OAIBlockerType::fromJsonValue(QJsonValue json) {
fromJson(json.toString());
}

QString OAIBlockerType::asJson() const {
    
    QString val;
    switch (m_value){
        case eOAIBlockerType::SCHEDULE:
            val = "Schedule";
            break;
        default:
            break;
...
QJsonValue OAIBlockerType::asJsonValue() const {
    
    return QJsonValue(asJson());
}


OAIBlockerType::eOAIBlockerType OAIBlockerType::getValue() const {
    return m_value;
}

void OAIBlockerType::setValue(const OAIBlockerType::eOAIBlockerType& value){
    m_value = value;
    m_value_isSet = true;
}
bool OAIBlockerType::isSet() const {
    
    return m_value_isSet;
}

bool OAIBlockerType::isValid() const {
