/**
 * Amazon Interactive Video Service RealTime
 * <p> <b>Introduction</b> </p> <p>The Amazon Interactive Video Service (IVS) real-time API is REST compatible, using a standard HTTP API and an AWS EventBridge event stream for responses. JSON is used for both requests and responses, including errors. </p> <p>Terminology:</p> <ul> <li> <p>A <i>stage</i> is a virtual space where participants can exchange video in real time.</p> </li> <li> <p>A <i>participant token</i> is a token that authenticates a participant when they join a stage.</p> </li> <li> <p>A <i>participant object</i> represents participants (people) in the stage and contains information about them. When a token is created, it includes a participant ID; when a participant uses that token to join a stage, the participant is associated with that participant ID There is a 1:1 mapping between participant tokens and participants.</p> </li> </ul> <p> <b>Resources</b> </p> <p>The following resources contain information about your IVS live stream (see <a href=\"https://docs.aws.amazon.com/ivs/latest/RealTimeUserGuide/getting-started.html\">Getting Started with Amazon IVS Real-Time Streaming</a>):</p> <ul> <li> <p> <b>Stage</b> — A stage is a virtual space where participants can exchange video in real time.</p> </li> </ul> <p> <b>Tagging</b> </p> <p>A <i>tag</i> is a metadata label that you assign to an AWS resource. A tag comprises a <i>key</i> and a <i>value</i>, both set by you. For example, you might set a tag as <code>topic:nature</code> to label a particular video category. See <a href=\"https://docs.aws.amazon.com/general/latest/gr/aws_tagging.html\">Tagging AWS Resources</a> for more information, including restrictions that apply to tags and \"Tag naming limits and requirements\"; Amazon IVS stages has no service-specific constraints beyond what is documented there.</p> <p>Tags can help you identify and organize your AWS resources. For example, you can use the same tag for different resources to indicate that they are related. You can also use tags to manage access (see <a href=\"https://docs.aws.amazon.com/IAM/latest/UserGuide/access_tags.html\">Access Tags</a>).</p> <p>The Amazon IVS real-time API has these tag-related endpoints: <a>TagResource</a>, <a>UntagResource</a>, and <a>ListTagsForResource</a>. The following resource supports tagging: Stage.</p> <p>At most 50 tags can be applied to a resource.</p> <p> <b>Stages Endpoints</b> </p> <ul> <li> <p> <a>CreateParticipantToken</a> — Creates an additional token for a specified stage. This can be done after stage creation or when tokens expire.</p> </li> <li> <p> <a>CreateStage</a> — Creates a new stage (and optionally participant tokens).</p> </li> <li> <p> <a>DeleteStage</a> — Shuts down and deletes the specified stage (disconnecting all participants).</p> </li> <li> <p> <a>DisconnectParticipant</a> — Disconnects a specified participant and revokes the participant permanently from a specified stage.</p> </li> <li> <p> <a>GetParticipant</a> — Gets information about the specified participant token.</p> </li> <li> <p> <a>GetStage</a> — Gets information for the specified stage.</p> </li> <li> <p> <a>GetStageSession</a> — Gets information for the specified stage session.</p> </li> <li> <p> <a>ListParticipantEvents</a> — Lists events for a specified participant that occurred during a specified stage session.</p> </li> <li> <p> <a>ListParticipants</a> — Lists all participants in a specified stage session.</p> </li> <li> <p> <a>ListStages</a> — Gets summary information about all stages in your account, in the AWS region where the API request is processed.</p> </li> <li> <p> <a>ListStageSessions</a> — Gets all sessions for a specified stage.</p> </li> <li> <p> <a>UpdateStage</a> — Updates a stage’s configuration.</p> </li> </ul> <p> <b>Tags Endpoints</b> </p> <ul> <li> <p> <a>ListTagsForResource</a> — Gets information about AWS tags for the specified ARN.</p> </li> <li> <p> <a>TagResource</a> — Adds or updates tags for the AWS resource with the specified ARN.</p> </li> <li> <p> <a>UntagResource</a> — Removes tags from the resource with the specified ARN.</p> </li> </ul>
 *
 * The version of the OpenAPI document: 2020-07-14
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIGetParticipantResponse_participant.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIGetParticipantResponse_participant::OAIGetParticipantResponse_participant(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIGetParticipantResponse_participant::OAIGetParticipantResponse_participant() {
    this->initializeModel();
}

OAIGetParticipantResponse_participant::~OAIGetParticipantResponse_participant() {}

void OAIGetParticipantResponse_participant::initializeModel() {

    m_attributes_isSet = false;
    m_attributes_isValid = false;

    m_first_join_time_isSet = false;
    m_first_join_time_isValid = false;

    m_participant_id_isSet = false;
    m_participant_id_isValid = false;

    m_published_isSet = false;
    m_published_isValid = false;

    m_state_isSet = false;
    m_state_isValid = false;

    m_user_id_isSet = false;
    m_user_id_isValid = false;
}

void OAIGetParticipantResponse_participant::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAIGetParticipantResponse_participant::fromJsonObject(QJsonObject json) {

    m_attributes_isValid = ::OpenAPI::fromJsonValue(m_attributes, json[QString("attributes")]);
    m_attributes_isSet = !json[QString("attributes")].isNull() && m_attributes_isValid;

    m_first_join_time_isValid = ::OpenAPI::fromJsonValue(m_first_join_time, json[QString("firstJoinTime")]);
    m_first_join_time_isSet = !json[QString("firstJoinTime")].isNull() && m_first_join_time_isValid;

    m_participant_id_isValid = ::OpenAPI::fromJsonValue(m_participant_id, json[QString("participantId")]);
    m_participant_id_isSet = !json[QString("participantId")].isNull() && m_participant_id_isValid;

    m_published_isValid = ::OpenAPI::fromJsonValue(m_published, json[QString("published")]);
    m_published_isSet = !json[QString("published")].isNull() && m_published_isValid;

    m_state_isValid = ::OpenAPI::fromJsonValue(m_state, json[QString("state")]);
    m_state_isSet = !json[QString("state")].isNull() && m_state_isValid;

    m_user_id_isValid = ::OpenAPI::fromJsonValue(m_user_id, json[QString("userId")]);
    m_user_id_isSet = !json[QString("userId")].isNull() && m_user_id_isValid;
}

QString OAIGetParticipantResponse_participant::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAIGetParticipantResponse_participant::asJsonObject() const {
    QJsonObject obj;
    if (m_attributes.isSet()) {
        obj.insert(QString("attributes"), ::OpenAPI::toJsonValue(m_attributes));
    }
    if (m_first_join_time_isSet) {
        obj.insert(QString("firstJoinTime"), ::OpenAPI::toJsonValue(m_first_join_time));
    }
    if (m_participant_id_isSet) {
        obj.insert(QString("participantId"), ::OpenAPI::toJsonValue(m_participant_id));
    }
    if (m_published_isSet) {
        obj.insert(QString("published"), ::OpenAPI::toJsonValue(m_published));
    }
    if (m_state.isSet()) {
        obj.insert(QString("state"), ::OpenAPI::toJsonValue(m_state));
    }
    if (m_user_id_isSet) {
        obj.insert(QString("userId"), ::OpenAPI::toJsonValue(m_user_id));
    }
    return obj;
}

QMap OAIGetParticipantResponse_participant::getAttributes() const {
    return m_attributes;
}
void OAIGetParticipantResponse_participant::setAttributes(const QMap &attributes) {
    m_attributes = attributes;
    m_attributes_isSet = true;
}

bool OAIGetParticipantResponse_participant::is_attributes_Set() const{
    return m_attributes_isSet;
}

bool OAIGetParticipantResponse_participant::is_attributes_Valid() const{
    return m_attributes_isValid;
}

QDateTime OAIGetParticipantResponse_participant::getFirstJoinTime() const {
    return m_first_join_time;
}
void OAIGetParticipantResponse_participant::setFirstJoinTime(const QDateTime &first_join_time) {
    m_first_join_time = first_join_time;
    m_first_join_time_isSet = true;
}

bool OAIGetParticipantResponse_participant::is_first_join_time_Set() const{
    return m_first_join_time_isSet;
}

bool OAIGetParticipantResponse_participant::is_first_join_time_Valid() const{
    return m_first_join_time_isValid;
}

QString OAIGetParticipantResponse_participant::getParticipantId() const {
    return m_participant_id;
}
void OAIGetParticipantResponse_participant::setParticipantId(const QString &participant_id) {
    m_participant_id = participant_id;
    m_participant_id_isSet = true;
}

bool OAIGetParticipantResponse_participant::is_participant_id_Set() const{
    return m_participant_id_isSet;
}

bool OAIGetParticipantResponse_participant::is_participant_id_Valid() const{
    return m_participant_id_isValid;
}

bool OAIGetParticipantResponse_participant::getPublished() const {
    return m_published;
}
void OAIGetParticipantResponse_participant::setPublished(const bool &published) {
    m_published = published;
    m_published_isSet = true;
}

bool OAIGetParticipantResponse_participant::is_published_Set() const{
    return m_published_isSet;
}

bool OAIGetParticipantResponse_participant::is_published_Valid() const{
    return m_published_isValid;
}

OAIParticipantState OAIGetParticipantResponse_participant::getState() const {
    return m_state;
}
void OAIGetParticipantResponse_participant::setState(const OAIParticipantState &state) {
    m_state = state;
    m_state_isSet = true;
}

bool OAIGetParticipantResponse_participant::is_state_Set() const{
    return m_state_isSet;
}

bool OAIGetParticipantResponse_participant::is_state_Valid() const{
    return m_state_isValid;
}

QString OAIGetParticipantResponse_participant::getUserId() const {
    return m_user_id;
}
void OAIGetParticipantResponse_participant::setUserId(const QString &user_id) {
    m_user_id = user_id;
    m_user_id_isSet = true;
}

bool OAIGetParticipantResponse_participant::is_user_id_Set() const{
    return m_user_id_isSet;
}

bool OAIGetParticipantResponse_participant::is_user_id_Valid() const{
    return m_user_id_isValid;
}

bool OAIGetParticipantResponse_participant::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_attributes.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_first_join_time_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_participant_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_published_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_state.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_user_id_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAIGetParticipantResponse_participant::isValid() const {
    // only required properties are required for the object to be considered valid
    return true;
}

} // namespace OpenAPI
