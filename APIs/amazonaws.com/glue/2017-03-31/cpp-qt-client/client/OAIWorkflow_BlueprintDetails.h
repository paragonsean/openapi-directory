/**
 * AWS Glue
 * <fullname>Glue</fullname> <p>Defines the public endpoint for the Glue service.</p>
 *
 * The version of the OpenAPI document: 2017-03-31
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIWorkflow_BlueprintDetails.h
 *
 * 
 */

#ifndef OAIWorkflow_BlueprintDetails_H
#define OAIWorkflow_BlueprintDetails_H

#include <QJsonObject>

#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {

class OAIWorkflow_BlueprintDetails : public OAIObject {
public:
    OAIWorkflow_BlueprintDetails();
    OAIWorkflow_BlueprintDetails(QString json);
    ~OAIWorkflow_BlueprintDetails() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QString getBlueprintName() const;
    void setBlueprintName(const QString &blueprint_name);
    bool is_blueprint_name_Set() const;
    bool is_blueprint_name_Valid() const;

    QString getRunId() const;
    void setRunId(const QString &run_id);
    bool is_run_id_Set() const;
    bool is_run_id_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString m_blueprint_name;
    bool m_blueprint_name_isSet;
    bool m_blueprint_name_isValid;

    QString m_run_id;
    bool m_run_id_isSet;
    bool m_run_id_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIWorkflow_BlueprintDetails)

#endif // OAIWorkflow_BlueprintDetails_H
