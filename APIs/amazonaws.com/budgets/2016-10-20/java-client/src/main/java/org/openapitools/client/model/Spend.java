...
public class Spend {
  ...
  public Spend amount(String amount) {
    this.amount = amount;
    return this;
  }
  ...
  public boolean equals(Object o) {
    ...
    Spend spend = (Spend) o;
    return Objects.equals(this.amount, spend.amount) &&
        Objects.equals(this.unit, spend.unit);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to Spend
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Spend.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in Spend is not found in the empty JSON string", Spend.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Spend.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Spend` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : Spend.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!Spend.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'Spend' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<Spend> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(Spend.class));

       return (TypeAdapter<T>) new TypeAdapter<Spend>() {
           @Override
           public void write(JsonWriter out, Spend value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public Spend read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of Spend given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of Spend
   * @throws IOException if the JSON string is invalid with respect to Spend
   */
  public static Spend fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, Spend.class);
  }

  /**
   * Convert an instance of Spend to an JSON string
   *
   * @return JSON string
   */
