/**
 * Amazon CloudHSM
 * <fullname>AWS CloudHSM Service</fullname> <p>This is documentation for <b>AWS CloudHSM Classic</b>. For more information, see <a href=\"http://aws.amazon.com/cloudhsm/faqs-classic/\">AWS CloudHSM Classic FAQs</a>, the <a href=\"https://docs.aws.amazon.com/cloudhsm/classic/userguide/\">AWS CloudHSM Classic User Guide</a>, and the <a href=\"https://docs.aws.amazon.com/cloudhsm/classic/APIReference/\">AWS CloudHSM Classic API Reference</a>.</p> <p> <b>For information about the current version of AWS CloudHSM</b>, see <a href=\"http://aws.amazon.com/cloudhsm/\">AWS CloudHSM</a>, the <a href=\"https://docs.aws.amazon.com/cloudhsm/latest/userguide/\">AWS CloudHSM User Guide</a>, and the <a href=\"https://docs.aws.amazon.com/cloudhsm/latest/APIReference/\">AWS CloudHSM API Reference</a>.</p>
 *
 * The version of the OpenAPI document: 2014-05-30
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIDescribeLunaClientResponse.h
 *
 * 
 */

#ifndef OAIDescribeLunaClientResponse_H
#define OAIDescribeLunaClientResponse_H

#include <QJsonObject>

#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {

class OAIDescribeLunaClientResponse : public OAIObject {
public:
    OAIDescribeLunaClientResponse();
    OAIDescribeLunaClientResponse(QString json);
    ~OAIDescribeLunaClientResponse() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QString getClientArn() const;
    void setClientArn(const QString &client_arn);
    bool is_client_arn_Set() const;
    bool is_client_arn_Valid() const;

    QString getCertificate() const;
    void setCertificate(const QString &certificate);
    bool is_certificate_Set() const;
    bool is_certificate_Valid() const;

    QString getCertificateFingerprint() const;
    void setCertificateFingerprint(const QString &certificate_fingerprint);
    bool is_certificate_fingerprint_Set() const;
    bool is_certificate_fingerprint_Valid() const;

    QString getLastModifiedTimestamp() const;
    void setLastModifiedTimestamp(const QString &last_modified_timestamp);
    bool is_last_modified_timestamp_Set() const;
    bool is_last_modified_timestamp_Valid() const;

    QString getLabel() const;
    void setLabel(const QString &label);
    bool is_label_Set() const;
    bool is_label_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString m_client_arn;
    bool m_client_arn_isSet;
    bool m_client_arn_isValid;

    QString m_certificate;
    bool m_certificate_isSet;
    bool m_certificate_isValid;

    QString m_certificate_fingerprint;
    bool m_certificate_fingerprint_isSet;
    bool m_certificate_fingerprint_isValid;

    QString m_last_modified_timestamp;
    bool m_last_modified_timestamp_isSet;
    bool m_last_modified_timestamp_isValid;

    QString m_label;
    bool m_label_isSet;
    bool m_label_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIDescribeLunaClientResponse)

#endif // OAIDescribeLunaClientResponse_H
