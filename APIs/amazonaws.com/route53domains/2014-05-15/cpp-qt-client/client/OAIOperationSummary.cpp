/**
 * Amazon Route 53 Domains
 * Amazon Route 53 API actions let you register domain names and perform related operations.
 *
 * The version of the OpenAPI document: 2014-05-15
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIOperationSummary.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIOperationSummary::OAIOperationSummary(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIOperationSummary::OAIOperationSummary() {
    this->initializeModel();
}

OAIOperationSummary::~OAIOperationSummary() {}

void OAIOperationSummary::initializeModel() {

    m_operation_id_isSet = false;
    m_operation_id_isValid = false;

    m_status_isSet = false;
    m_status_isValid = false;

    m_type_isSet = false;
    m_type_isValid = false;

    m_submitted_date_isSet = false;
    m_submitted_date_isValid = false;

    m_domain_name_isSet = false;
    m_domain_name_isValid = false;

    m_message_isSet = false;
    m_message_isValid = false;

    m_status_flag_isSet = false;
    m_status_flag_isValid = false;

    m_last_updated_date_isSet = false;
    m_last_updated_date_isValid = false;
}

void OAIOperationSummary::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAIOperationSummary::fromJsonObject(QJsonObject json) {

    m_operation_id_isValid = ::OpenAPI::fromJsonValue(m_operation_id, json[QString("OperationId")]);
    m_operation_id_isSet = !json[QString("OperationId")].isNull() && m_operation_id_isValid;

    m_status_isValid = ::OpenAPI::fromJsonValue(m_status, json[QString("Status")]);
    m_status_isSet = !json[QString("Status")].isNull() && m_status_isValid;

    m_type_isValid = ::OpenAPI::fromJsonValue(m_type, json[QString("Type")]);
    m_type_isSet = !json[QString("Type")].isNull() && m_type_isValid;

    m_submitted_date_isValid = ::OpenAPI::fromJsonValue(m_submitted_date, json[QString("SubmittedDate")]);
    m_submitted_date_isSet = !json[QString("SubmittedDate")].isNull() && m_submitted_date_isValid;

    m_domain_name_isValid = ::OpenAPI::fromJsonValue(m_domain_name, json[QString("DomainName")]);
    m_domain_name_isSet = !json[QString("DomainName")].isNull() && m_domain_name_isValid;

    m_message_isValid = ::OpenAPI::fromJsonValue(m_message, json[QString("Message")]);
    m_message_isSet = !json[QString("Message")].isNull() && m_message_isValid;

    m_status_flag_isValid = ::OpenAPI::fromJsonValue(m_status_flag, json[QString("StatusFlag")]);
    m_status_flag_isSet = !json[QString("StatusFlag")].isNull() && m_status_flag_isValid;

    m_last_updated_date_isValid = ::OpenAPI::fromJsonValue(m_last_updated_date, json[QString("LastUpdatedDate")]);
    m_last_updated_date_isSet = !json[QString("LastUpdatedDate")].isNull() && m_last_updated_date_isValid;
}

QString OAIOperationSummary::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAIOperationSummary::asJsonObject() const {
    QJsonObject obj;
    if (m_operation_id_isSet) {
        obj.insert(QString("OperationId"), ::OpenAPI::toJsonValue(m_operation_id));
    }
    if (m_status.isSet()) {
        obj.insert(QString("Status"), ::OpenAPI::toJsonValue(m_status));
    }
    if (m_type.isSet()) {
        obj.insert(QString("Type"), ::OpenAPI::toJsonValue(m_type));
    }
    if (m_submitted_date_isSet) {
        obj.insert(QString("SubmittedDate"), ::OpenAPI::toJsonValue(m_submitted_date));
    }
    if (m_domain_name_isSet) {
        obj.insert(QString("DomainName"), ::OpenAPI::toJsonValue(m_domain_name));
    }
    if (m_message_isSet) {
        obj.insert(QString("Message"), ::OpenAPI::toJsonValue(m_message));
    }
    if (m_status_flag.isSet()) {
        obj.insert(QString("StatusFlag"), ::OpenAPI::toJsonValue(m_status_flag));
    }
    if (m_last_updated_date_isSet) {
        obj.insert(QString("LastUpdatedDate"), ::OpenAPI::toJsonValue(m_last_updated_date));
    }
    return obj;
}

QString OAIOperationSummary::getOperationId() const {
    return m_operation_id;
}
void OAIOperationSummary::setOperationId(const QString &operation_id) {
    m_operation_id = operation_id;
    m_operation_id_isSet = true;
}

bool OAIOperationSummary::is_operation_id_Set() const{
    return m_operation_id_isSet;
}

bool OAIOperationSummary::is_operation_id_Valid() const{
    return m_operation_id_isValid;
}

OAIOperationStatus OAIOperationSummary::getStatus() const {
    return m_status;
}
void OAIOperationSummary::setStatus(const OAIOperationStatus &status) {
    m_status = status;
    m_status_isSet = true;
}

bool OAIOperationSummary::is_status_Set() const{
    return m_status_isSet;
}

bool OAIOperationSummary::is_status_Valid() const{
    return m_status_isValid;
}

OAIOperationType OAIOperationSummary::getType() const {
    return m_type;
}
void OAIOperationSummary::setType(const OAIOperationType &type) {
    m_type = type;
    m_type_isSet = true;
}

bool OAIOperationSummary::is_type_Set() const{
    return m_type_isSet;
}

bool OAIOperationSummary::is_type_Valid() const{
    return m_type_isValid;
}

QDateTime OAIOperationSummary::getSubmittedDate() const {
    return m_submitted_date;
}
void OAIOperationSummary::setSubmittedDate(const QDateTime &submitted_date) {
    m_submitted_date = submitted_date;
    m_submitted_date_isSet = true;
}

bool OAIOperationSummary::is_submitted_date_Set() const{
    return m_submitted_date_isSet;
}

bool OAIOperationSummary::is_submitted_date_Valid() const{
    return m_submitted_date_isValid;
}

QString OAIOperationSummary::getDomainName() const {
    return m_domain_name;
}
void OAIOperationSummary::setDomainName(const QString &domain_name) {
    m_domain_name = domain_name;
    m_domain_name_isSet = true;
}

bool OAIOperationSummary::is_domain_name_Set() const{
    return m_domain_name_isSet;
}

bool OAIOperationSummary::is_domain_name_Valid() const{
    return m_domain_name_isValid;
}

QString OAIOperationSummary::getMessage() const {
    return m_message;
}
void OAIOperationSummary::setMessage(const QString &message) {
    m_message = message;
    m_message_isSet = true;
}

bool OAIOperationSummary::is_message_Set() const{
    return m_message_isSet;
}

bool OAIOperationSummary::is_message_Valid() const{
    return m_message_isValid;
}

OAIStatusFlag OAIOperationSummary::getStatusFlag() const {
    return m_status_flag;
}
void OAIOperationSummary::setStatusFlag(const OAIStatusFlag &status_flag) {
    m_status_flag = status_flag;
    m_status_flag_isSet = true;
}

bool OAIOperationSummary::is_status_flag_Set() const{
    return m_status_flag_isSet;
}

bool OAIOperationSummary::is_status_flag_Valid() const{
    return m_status_flag_isValid;
}

QDateTime OAIOperationSummary::getLastUpdatedDate() const {
    return m_last_updated_date;
}
void OAIOperationSummary::setLastUpdatedDate(const QDateTime &last_updated_date) {
    m_last_updated_date = last_updated_date;
    m_last_updated_date_isSet = true;
}

bool OAIOperationSummary::is_last_updated_date_Set() const{
    return m_last_updated_date_isSet;
}

bool OAIOperationSummary::is_last_updated_date_Valid() const{
    return m_last_updated_date_isValid;
}

bool OAIOperationSummary::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_operation_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_status.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_type.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_submitted_date_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_domain_name_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_message_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_status_flag.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_last_updated_date_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAIOperationSummary::isValid() const {
    // only required properties are required for the object to be considered valid
    return true;
}

} // namespace OpenAPI
