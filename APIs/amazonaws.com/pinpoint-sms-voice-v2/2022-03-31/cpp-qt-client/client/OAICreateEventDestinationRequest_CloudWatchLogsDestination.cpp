/**
 * Amazon Pinpoint SMS Voice V2
 * <p>Welcome to the <i>Amazon Pinpoint SMS and Voice, version 2 API Reference</i>. This guide provides information about Amazon Pinpoint SMS and Voice, version 2 API resources, including supported HTTP methods, parameters, and schemas.</p> <p>Amazon Pinpoint is an Amazon Web Services service that you can use to engage with your recipients across multiple messaging channels. The Amazon Pinpoint SMS and Voice, version 2 API provides programmatic access to options that are unique to the SMS and voice channels and supplements the resources provided by the Amazon Pinpoint API.</p> <p>If you're new to Amazon Pinpoint, it's also helpful to review the <a href=\"https://docs.aws.amazon.com/pinpoint/latest/developerguide/welcome.html\"> Amazon Pinpoint Developer Guide</a>. The <i>Amazon Pinpoint Developer Guide</i> provides tutorials, code samples, and procedures that demonstrate how to use Amazon Pinpoint features programmatically and how to integrate Amazon Pinpoint functionality into mobile apps and other types of applications. The guide also provides key information, such as Amazon Pinpoint integration with other Amazon Web Services services, and the quotas that apply to use of the service.</p>
 *
 * The version of the OpenAPI document: 2022-03-31
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAICreateEventDestinationRequest_CloudWatchLogsDestination.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAICreateEventDestinationRequest_CloudWatchLogsDestination::OAICreateEventDestinationRequest_CloudWatchLogsDestination(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAICreateEventDestinationRequest_CloudWatchLogsDestination::OAICreateEventDestinationRequest_CloudWatchLogsDestination() {
    this->initializeModel();
}

OAICreateEventDestinationRequest_CloudWatchLogsDestination::~OAICreateEventDestinationRequest_CloudWatchLogsDestination() {}

void OAICreateEventDestinationRequest_CloudWatchLogsDestination::initializeModel() {

    m_iam_role_arn_isSet = false;
    m_iam_role_arn_isValid = false;

    m_log_group_arn_isSet = false;
    m_log_group_arn_isValid = false;
}

void OAICreateEventDestinationRequest_CloudWatchLogsDestination::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAICreateEventDestinationRequest_CloudWatchLogsDestination::fromJsonObject(QJsonObject json) {

    m_iam_role_arn_isValid = ::OpenAPI::fromJsonValue(m_iam_role_arn, json[QString("IamRoleArn")]);
    m_iam_role_arn_isSet = !json[QString("IamRoleArn")].isNull() && m_iam_role_arn_isValid;

    m_log_group_arn_isValid = ::OpenAPI::fromJsonValue(m_log_group_arn, json[QString("LogGroupArn")]);
    m_log_group_arn_isSet = !json[QString("LogGroupArn")].isNull() && m_log_group_arn_isValid;
}

QString OAICreateEventDestinationRequest_CloudWatchLogsDestination::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAICreateEventDestinationRequest_CloudWatchLogsDestination::asJsonObject() const {
    QJsonObject obj;
    if (m_iam_role_arn_isSet) {
        obj.insert(QString("IamRoleArn"), ::OpenAPI::toJsonValue(m_iam_role_arn));
    }
    if (m_log_group_arn_isSet) {
        obj.insert(QString("LogGroupArn"), ::OpenAPI::toJsonValue(m_log_group_arn));
    }
    return obj;
}

QString OAICreateEventDestinationRequest_CloudWatchLogsDestination::getIamRoleArn() const {
    return m_iam_role_arn;
}
void OAICreateEventDestinationRequest_CloudWatchLogsDestination::setIamRoleArn(const QString &iam_role_arn) {
    m_iam_role_arn = iam_role_arn;
    m_iam_role_arn_isSet = true;
}

bool OAICreateEventDestinationRequest_CloudWatchLogsDestination::is_iam_role_arn_Set() const{
    return m_iam_role_arn_isSet;
}

bool OAICreateEventDestinationRequest_CloudWatchLogsDestination::is_iam_role_arn_Valid() const{
    return m_iam_role_arn_isValid;
}

QString OAICreateEventDestinationRequest_CloudWatchLogsDestination::getLogGroupArn() const {
    return m_log_group_arn;
}
void OAICreateEventDestinationRequest_CloudWatchLogsDestination::setLogGroupArn(const QString &log_group_arn) {
    m_log_group_arn = log_group_arn;
    m_log_group_arn_isSet = true;
}

bool OAICreateEventDestinationRequest_CloudWatchLogsDestination::is_log_group_arn_Set() const{
    return m_log_group_arn_isSet;
}

bool OAICreateEventDestinationRequest_CloudWatchLogsDestination::is_log_group_arn_Valid() const{
    return m_log_group_arn_isValid;
}

bool OAICreateEventDestinationRequest_CloudWatchLogsDestination::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_iam_role_arn_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_log_group_arn_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAICreateEventDestinationRequest_CloudWatchLogsDestination::isValid() const {
    // only required properties are required for the object to be considered valid
    return m_iam_role_arn_isValid && m_log_group_arn_isValid && true;
}

} // namespace OpenAPI
