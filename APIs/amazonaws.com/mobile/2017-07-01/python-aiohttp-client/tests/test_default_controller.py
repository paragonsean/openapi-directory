...
async def test_delete_project(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/projects/{project_id}'.format(project_id='project_id_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_describe_bundle(client):
    """Test case for describe_bundle
    ...
    response = await client.request(
        method='GET',
        path='/bundles/{bundle_id}'.format(bundle_id='bundle_id_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_export_bundle(client):
...
async def test_export_project(client):
    ...
    response = await client.request(
        method='POST',
        path='/exports/{project_id}'.format(project_id='project_id_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
