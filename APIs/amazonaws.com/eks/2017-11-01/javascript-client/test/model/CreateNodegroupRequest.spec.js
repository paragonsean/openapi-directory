/**
 * Amazon Elastic Kubernetes Service
 * <p>Amazon Elastic Kubernetes Service (Amazon EKS) is a managed service that makes it easy for you to run Kubernetes on Amazon Web Services without needing to stand up or maintain your own Kubernetes control plane. Kubernetes is an open-source system for automating the deployment, scaling, and management of containerized applications. </p> <p>Amazon EKS runs up-to-date versions of the open-source Kubernetes software, so you can use all the existing plugins and tooling from the Kubernetes community. Applications running on Amazon EKS are fully compatible with applications running on any standard Kubernetes environment, whether running in on-premises data centers or public clouds. This means that you can easily migrate any standard Kubernetes application to Amazon EKS without any code modification required.</p>
 *
 * The version of the OpenAPI document: 2017-11-01
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', process.cwd()+'/src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require(process.cwd()+'/src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.AmazonElasticKubernetesService);
  }
}(this, function(expect, AmazonElasticKubernetesService) {
  'use strict';

  var instance;

  beforeEach(function() {
    instance = new AmazonElasticKubernetesService.CreateNodegroupRequest();
  });

  var getProperty = function(object, getter, property) {
    // Use getter method if present; otherwise, get the property directly.
    if (typeof object[getter] === 'function')
      return object[getter]();
    else
      return object[property];
  }

  var setProperty = function(object, setter, property, value) {
    // Use setter method if present; otherwise, set the property directly.
    if (typeof object[setter] === 'function')
      object[setter](value);
    else
      object[property] = value;
  }

  describe('CreateNodegroupRequest', function() {
    it('should create an instance of CreateNodegroupRequest', function() {
      // uncomment below and update the code to test CreateNodegroupRequest
      //var instance = new AmazonElasticKubernetesService.CreateNodegroupRequest();
      //expect(instance).to.be.a(AmazonElasticKubernetesService.CreateNodegroupRequest);
    });

    it('should have the property nodegroupName (base name: "nodegroupName")', function() {
      // uncomment below and update the code to test the property nodegroupName
      //var instance = new AmazonElasticKubernetesService.CreateNodegroupRequest();
      //expect(instance).to.be();
    });

    it('should have the property scalingConfig (base name: "scalingConfig")', function() {
      // uncomment below and update the code to test the property scalingConfig
      //var instance = new AmazonElasticKubernetesService.CreateNodegroupRequest();
      //expect(instance).to.be();
    });

    it('should have the property diskSize (base name: "diskSize")', function() {
      // uncomment below and update the code to test the property diskSize
      //var instance = new AmazonElasticKubernetesService.CreateNodegroupRequest();
      //expect(instance).to.be();
    });

    it('should have the property subnets (base name: "subnets")', function() {
      // uncomment below and update the code to test the property subnets
      //var instance = new AmazonElasticKubernetesService.CreateNodegroupRequest();
      //expect(instance).to.be();
    });

    it('should have the property instanceTypes (base name: "instanceTypes")', function() {
      // uncomment below and update the code to test the property instanceTypes
      //var instance = new AmazonElasticKubernetesService.CreateNodegroupRequest();
      //expect(instance).to.be();
    });

    it('should have the property amiType (base name: "amiType")', function() {
      // uncomment below and update the code to test the property amiType
      //var instance = new AmazonElasticKubernetesService.CreateNodegroupRequest();
      //expect(instance).to.be();
    });

    it('should have the property remoteAccess (base name: "remoteAccess")', function() {
      // uncomment below and update the code to test the property remoteAccess
      //var instance = new AmazonElasticKubernetesService.CreateNodegroupRequest();
      //expect(instance).to.be();
    });

    it('should have the property nodeRole (base name: "nodeRole")', function() {
      // uncomment below and update the code to test the property nodeRole
      //var instance = new AmazonElasticKubernetesService.CreateNodegroupRequest();
      //expect(instance).to.be();
    });

    it('should have the property labels (base name: "labels")', function() {
      // uncomment below and update the code to test the property labels
      //var instance = new AmazonElasticKubernetesService.CreateNodegroupRequest();
      //expect(instance).to.be();
    });

    it('should have the property taints (base name: "taints")', function() {
      // uncomment below and update the code to test the property taints
      //var instance = new AmazonElasticKubernetesService.CreateNodegroupRequest();
      //expect(instance).to.be();
    });

    it('should have the property tags (base name: "tags")', function() {
      // uncomment below and update the code to test the property tags
      //var instance = new AmazonElasticKubernetesService.CreateNodegroupRequest();
      //expect(instance).to.be();
    });

    it('should have the property clientRequestToken (base name: "clientRequestToken")', function() {
      // uncomment below and update the code to test the property clientRequestToken
      //var instance = new AmazonElasticKubernetesService.CreateNodegroupRequest();
      //expect(instance).to.be();
    });

    it('should have the property launchTemplate (base name: "launchTemplate")', function() {
      // uncomment below and update the code to test the property launchTemplate
      //var instance = new AmazonElasticKubernetesService.CreateNodegroupRequest();
      //expect(instance).to.be();
    });

    it('should have the property updateConfig (base name: "updateConfig")', function() {
      // uncomment below and update the code to test the property updateConfig
      //var instance = new AmazonElasticKubernetesService.CreateNodegroupRequest();
      //expect(instance).to.be();
    });

    it('should have the property capacityType (base name: "capacityType")', function() {
      // uncomment below and update the code to test the property capacityType
      //var instance = new AmazonElasticKubernetesService.CreateNodegroupRequest();
      //expect(instance).to.be();
    });

    it('should have the property version (base name: "version")', function() {
      // uncomment below and update the code to test the property version
      //var instance = new AmazonElasticKubernetesService.CreateNodegroupRequest();
      //expect(instance).to.be();
    });

    it('should have the property releaseVersion (base name: "releaseVersion")', function() {
      // uncomment below and update the code to test the property releaseVersion
      //var instance = new AmazonElasticKubernetesService.CreateNodegroupRequest();
      //expect(instance).to.be();
    });

  });

}));
