/**
 * AWS Elemental MediaLive
 * API for AWS Elemental MediaLive
 *
 * The version of the OpenAPI document: 2017-10-14
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIAudioCodecSettings_AacSettings.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIAudioCodecSettings_AacSettings::OAIAudioCodecSettings_AacSettings(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIAudioCodecSettings_AacSettings::OAIAudioCodecSettings_AacSettings() {
    this->initializeModel();
}

OAIAudioCodecSettings_AacSettings::~OAIAudioCodecSettings_AacSettings() {}

void OAIAudioCodecSettings_AacSettings::initializeModel() {

    m_bitrate_isSet = false;
    m_bitrate_isValid = false;

    m_coding_mode_isSet = false;
    m_coding_mode_isValid = false;

    m_input_type_isSet = false;
    m_input_type_isValid = false;

    m_profile_isSet = false;
    m_profile_isValid = false;

    m_rate_control_mode_isSet = false;
    m_rate_control_mode_isValid = false;

    m_raw_format_isSet = false;
    m_raw_format_isValid = false;

    m_sample_rate_isSet = false;
    m_sample_rate_isValid = false;

    m_spec_isSet = false;
    m_spec_isValid = false;

    m_vbr_quality_isSet = false;
    m_vbr_quality_isValid = false;
}

void OAIAudioCodecSettings_AacSettings::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAIAudioCodecSettings_AacSettings::fromJsonObject(QJsonObject json) {

    m_bitrate_isValid = ::OpenAPI::fromJsonValue(m_bitrate, json[QString("Bitrate")]);
    m_bitrate_isSet = !json[QString("Bitrate")].isNull() && m_bitrate_isValid;

    m_coding_mode_isValid = ::OpenAPI::fromJsonValue(m_coding_mode, json[QString("CodingMode")]);
    m_coding_mode_isSet = !json[QString("CodingMode")].isNull() && m_coding_mode_isValid;

    m_input_type_isValid = ::OpenAPI::fromJsonValue(m_input_type, json[QString("InputType")]);
    m_input_type_isSet = !json[QString("InputType")].isNull() && m_input_type_isValid;

    m_profile_isValid = ::OpenAPI::fromJsonValue(m_profile, json[QString("Profile")]);
    m_profile_isSet = !json[QString("Profile")].isNull() && m_profile_isValid;

    m_rate_control_mode_isValid = ::OpenAPI::fromJsonValue(m_rate_control_mode, json[QString("RateControlMode")]);
    m_rate_control_mode_isSet = !json[QString("RateControlMode")].isNull() && m_rate_control_mode_isValid;

    m_raw_format_isValid = ::OpenAPI::fromJsonValue(m_raw_format, json[QString("RawFormat")]);
    m_raw_format_isSet = !json[QString("RawFormat")].isNull() && m_raw_format_isValid;

    m_sample_rate_isValid = ::OpenAPI::fromJsonValue(m_sample_rate, json[QString("SampleRate")]);
    m_sample_rate_isSet = !json[QString("SampleRate")].isNull() && m_sample_rate_isValid;

    m_spec_isValid = ::OpenAPI::fromJsonValue(m_spec, json[QString("Spec")]);
    m_spec_isSet = !json[QString("Spec")].isNull() && m_spec_isValid;

    m_vbr_quality_isValid = ::OpenAPI::fromJsonValue(m_vbr_quality, json[QString("VbrQuality")]);
    m_vbr_quality_isSet = !json[QString("VbrQuality")].isNull() && m_vbr_quality_isValid;
}

QString OAIAudioCodecSettings_AacSettings::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAIAudioCodecSettings_AacSettings::asJsonObject() const {
    QJsonObject obj;
    if (m_bitrate_isSet) {
        obj.insert(QString("Bitrate"), ::OpenAPI::toJsonValue(m_bitrate));
    }
    if (m_coding_mode.isSet()) {
        obj.insert(QString("CodingMode"), ::OpenAPI::toJsonValue(m_coding_mode));
    }
    if (m_input_type.isSet()) {
        obj.insert(QString("InputType"), ::OpenAPI::toJsonValue(m_input_type));
    }
    if (m_profile.isSet()) {
        obj.insert(QString("Profile"), ::OpenAPI::toJsonValue(m_profile));
    }
    if (m_rate_control_mode.isSet()) {
        obj.insert(QString("RateControlMode"), ::OpenAPI::toJsonValue(m_rate_control_mode));
    }
    if (m_raw_format.isSet()) {
        obj.insert(QString("RawFormat"), ::OpenAPI::toJsonValue(m_raw_format));
    }
    if (m_sample_rate_isSet) {
        obj.insert(QString("SampleRate"), ::OpenAPI::toJsonValue(m_sample_rate));
    }
    if (m_spec.isSet()) {
        obj.insert(QString("Spec"), ::OpenAPI::toJsonValue(m_spec));
    }
    if (m_vbr_quality.isSet()) {
        obj.insert(QString("VbrQuality"), ::OpenAPI::toJsonValue(m_vbr_quality));
    }
    return obj;
}

double OAIAudioCodecSettings_AacSettings::getBitrate() const {
    return m_bitrate;
}
void OAIAudioCodecSettings_AacSettings::setBitrate(const double &bitrate) {
    m_bitrate = bitrate;
    m_bitrate_isSet = true;
}

bool OAIAudioCodecSettings_AacSettings::is_bitrate_Set() const{
    return m_bitrate_isSet;
}

bool OAIAudioCodecSettings_AacSettings::is_bitrate_Valid() const{
    return m_bitrate_isValid;
}

OAIAacCodingMode OAIAudioCodecSettings_AacSettings::getCodingMode() const {
    return m_coding_mode;
}
void OAIAudioCodecSettings_AacSettings::setCodingMode(const OAIAacCodingMode &coding_mode) {
    m_coding_mode = coding_mode;
    m_coding_mode_isSet = true;
}

bool OAIAudioCodecSettings_AacSettings::is_coding_mode_Set() const{
    return m_coding_mode_isSet;
}

bool OAIAudioCodecSettings_AacSettings::is_coding_mode_Valid() const{
    return m_coding_mode_isValid;
}

OAIAacInputType OAIAudioCodecSettings_AacSettings::getInputType() const {
    return m_input_type;
}
void OAIAudioCodecSettings_AacSettings::setInputType(const OAIAacInputType &input_type) {
    m_input_type = input_type;
    m_input_type_isSet = true;
}

bool OAIAudioCodecSettings_AacSettings::is_input_type_Set() const{
    return m_input_type_isSet;
}

bool OAIAudioCodecSettings_AacSettings::is_input_type_Valid() const{
    return m_input_type_isValid;
}

OAIAacProfile OAIAudioCodecSettings_AacSettings::getProfile() const {
    return m_profile;
}
void OAIAudioCodecSettings_AacSettings::setProfile(const OAIAacProfile &profile) {
    m_profile = profile;
    m_profile_isSet = true;
}

bool OAIAudioCodecSettings_AacSettings::is_profile_Set() const{
    return m_profile_isSet;
}

bool OAIAudioCodecSettings_AacSettings::is_profile_Valid() const{
    return m_profile_isValid;
}

OAIAacRateControlMode OAIAudioCodecSettings_AacSettings::getRateControlMode() const {
    return m_rate_control_mode;
}
void OAIAudioCodecSettings_AacSettings::setRateControlMode(const OAIAacRateControlMode &rate_control_mode) {
    m_rate_control_mode = rate_control_mode;
    m_rate_control_mode_isSet = true;
}

bool OAIAudioCodecSettings_AacSettings::is_rate_control_mode_Set() const{
    return m_rate_control_mode_isSet;
}

bool OAIAudioCodecSettings_AacSettings::is_rate_control_mode_Valid() const{
    return m_rate_control_mode_isValid;
}

OAIAacRawFormat OAIAudioCodecSettings_AacSettings::getRawFormat() const {
    return m_raw_format;
}
void OAIAudioCodecSettings_AacSettings::setRawFormat(const OAIAacRawFormat &raw_format) {
    m_raw_format = raw_format;
    m_raw_format_isSet = true;
}

bool OAIAudioCodecSettings_AacSettings::is_raw_format_Set() const{
    return m_raw_format_isSet;
}

bool OAIAudioCodecSettings_AacSettings::is_raw_format_Valid() const{
    return m_raw_format_isValid;
}

double OAIAudioCodecSettings_AacSettings::getSampleRate() const {
    return m_sample_rate;
}
void OAIAudioCodecSettings_AacSettings::setSampleRate(const double &sample_rate) {
    m_sample_rate = sample_rate;
    m_sample_rate_isSet = true;
}

bool OAIAudioCodecSettings_AacSettings::is_sample_rate_Set() const{
    return m_sample_rate_isSet;
}

bool OAIAudioCodecSettings_AacSettings::is_sample_rate_Valid() const{
    return m_sample_rate_isValid;
}

OAIAacSpec OAIAudioCodecSettings_AacSettings::getSpec() const {
    return m_spec;
}
void OAIAudioCodecSettings_AacSettings::setSpec(const OAIAacSpec &spec) {
    m_spec = spec;
    m_spec_isSet = true;
}

bool OAIAudioCodecSettings_AacSettings::is_spec_Set() const{
    return m_spec_isSet;
}

bool OAIAudioCodecSettings_AacSettings::is_spec_Valid() const{
    return m_spec_isValid;
}

OAIAacVbrQuality OAIAudioCodecSettings_AacSettings::getVbrQuality() const {
    return m_vbr_quality;
}
void OAIAudioCodecSettings_AacSettings::setVbrQuality(const OAIAacVbrQuality &vbr_quality) {
    m_vbr_quality = vbr_quality;
    m_vbr_quality_isSet = true;
}

bool OAIAudioCodecSettings_AacSettings::is_vbr_quality_Set() const{
    return m_vbr_quality_isSet;
}

bool OAIAudioCodecSettings_AacSettings::is_vbr_quality_Valid() const{
    return m_vbr_quality_isValid;
}

bool OAIAudioCodecSettings_AacSettings::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_bitrate_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_coding_mode.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_input_type.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_profile.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_rate_control_mode.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_raw_format.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_sample_rate_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_spec.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_vbr_quality.isSet()) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAIAudioCodecSettings_AacSettings::isValid() const {
    // only required properties are required for the object to be considered valid
    return true;
}

} // namespace OpenAPI
