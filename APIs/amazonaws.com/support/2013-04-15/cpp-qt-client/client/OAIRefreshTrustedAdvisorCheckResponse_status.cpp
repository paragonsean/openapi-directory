/**
 * AWS Support
 * <fullname>Amazon Web Services Support</fullname> <p>The <i>Amazon Web Services Support API Reference</i> is intended for programmers who need detailed information about the Amazon Web Services Support operations and data types. You can use the API to manage your support cases programmatically. The Amazon Web Services Support API uses HTTP methods that return results in JSON format.</p> <note> <ul> <li> <p>You must have a Business, Enterprise On-Ramp, or Enterprise Support plan to use the Amazon Web Services Support API. </p> </li> <li> <p>If you call the Amazon Web Services Support API from an account that doesn't have a Business, Enterprise On-Ramp, or Enterprise Support plan, the <code>SubscriptionRequiredException</code> error message appears. For information about changing your support plan, see <a href=\"http://aws.amazon.com/premiumsupport/\">Amazon Web Services Support</a>.</p> </li> </ul> </note> <p>You can also use the Amazon Web Services Support API to access features for <a href=\"http://aws.amazon.com/premiumsupport/trustedadvisor/\">Trusted Advisor</a>. You can return a list of checks and their descriptions, get check results, specify checks to refresh, and get the refresh status of checks.</p> <p>You can manage your support cases with the following Amazon Web Services Support API operations:</p> <ul> <li> <p>The <a>CreateCase</a>, <a>DescribeCases</a>, <a>DescribeAttachment</a>, and <a>ResolveCase</a> operations create Amazon Web Services Support cases, retrieve information about cases, and resolve cases.</p> </li> <li> <p>The <a>DescribeCommunications</a>, <a>AddCommunicationToCase</a>, and <a>AddAttachmentsToSet</a> operations retrieve and add communications and attachments to Amazon Web Services Support cases.</p> </li> <li> <p>The <a>DescribeServices</a> and <a>DescribeSeverityLevels</a> operations return Amazon Web Service names, service codes, service categories, and problem severity levels. You use these values when you call the <a>CreateCase</a> operation.</p> </li> </ul> <p>You can also use the Amazon Web Services Support API to call the Trusted Advisor operations. For more information, see <a href=\"https://docs.aws.amazon.com/\">Trusted Advisor</a> in the <i>Amazon Web Services Support User Guide</i>.</p> <p>For authentication of requests, Amazon Web Services Support uses <a href=\"https://docs.aws.amazon.com/general/latest/gr/signature-version-4.html\">Signature Version 4 Signing Process</a>.</p> <p>For more information about this service and the endpoints to use, see <a href=\"https://docs.aws.amazon.com/awssupport/latest/user/about-support-api.html\">About the Amazon Web Services Support API</a> in the <i>Amazon Web Services Support User Guide</i>.</p>
 *
 * The version of the OpenAPI document: 2013-04-15
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIRefreshTrustedAdvisorCheckResponse_status.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIRefreshTrustedAdvisorCheckResponse_status::OAIRefreshTrustedAdvisorCheckResponse_status(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIRefreshTrustedAdvisorCheckResponse_status::OAIRefreshTrustedAdvisorCheckResponse_status() {
    this->initializeModel();
}

OAIRefreshTrustedAdvisorCheckResponse_status::~OAIRefreshTrustedAdvisorCheckResponse_status() {}

void OAIRefreshTrustedAdvisorCheckResponse_status::initializeModel() {

    m_check_id_isSet = false;
    m_check_id_isValid = false;

    m_status_isSet = false;
    m_status_isValid = false;

    m_millis_until_next_refreshable_isSet = false;
    m_millis_until_next_refreshable_isValid = false;
}

void OAIRefreshTrustedAdvisorCheckResponse_status::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAIRefreshTrustedAdvisorCheckResponse_status::fromJsonObject(QJsonObject json) {

    m_check_id_isValid = ::OpenAPI::fromJsonValue(m_check_id, json[QString("checkId")]);
    m_check_id_isSet = !json[QString("checkId")].isNull() && m_check_id_isValid;

    m_status_isValid = ::OpenAPI::fromJsonValue(m_status, json[QString("status")]);
    m_status_isSet = !json[QString("status")].isNull() && m_status_isValid;

    m_millis_until_next_refreshable_isValid = ::OpenAPI::fromJsonValue(m_millis_until_next_refreshable, json[QString("millisUntilNextRefreshable")]);
    m_millis_until_next_refreshable_isSet = !json[QString("millisUntilNextRefreshable")].isNull() && m_millis_until_next_refreshable_isValid;
}

QString OAIRefreshTrustedAdvisorCheckResponse_status::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAIRefreshTrustedAdvisorCheckResponse_status::asJsonObject() const {
    QJsonObject obj;
    if (m_check_id_isSet) {
        obj.insert(QString("checkId"), ::OpenAPI::toJsonValue(m_check_id));
    }
    if (m_status_isSet) {
        obj.insert(QString("status"), ::OpenAPI::toJsonValue(m_status));
    }
    if (m_millis_until_next_refreshable_isSet) {
        obj.insert(QString("millisUntilNextRefreshable"), ::OpenAPI::toJsonValue(m_millis_until_next_refreshable));
    }
    return obj;
}

QString OAIRefreshTrustedAdvisorCheckResponse_status::getCheckId() const {
    return m_check_id;
}
void OAIRefreshTrustedAdvisorCheckResponse_status::setCheckId(const QString &check_id) {
    m_check_id = check_id;
    m_check_id_isSet = true;
}

bool OAIRefreshTrustedAdvisorCheckResponse_status::is_check_id_Set() const{
    return m_check_id_isSet;
}

bool OAIRefreshTrustedAdvisorCheckResponse_status::is_check_id_Valid() const{
    return m_check_id_isValid;
}

QString OAIRefreshTrustedAdvisorCheckResponse_status::getStatus() const {
    return m_status;
}
void OAIRefreshTrustedAdvisorCheckResponse_status::setStatus(const QString &status) {
    m_status = status;
    m_status_isSet = true;
}

bool OAIRefreshTrustedAdvisorCheckResponse_status::is_status_Set() const{
    return m_status_isSet;
}

bool OAIRefreshTrustedAdvisorCheckResponse_status::is_status_Valid() const{
    return m_status_isValid;
}

qint32 OAIRefreshTrustedAdvisorCheckResponse_status::getMillisUntilNextRefreshable() const {
    return m_millis_until_next_refreshable;
}
void OAIRefreshTrustedAdvisorCheckResponse_status::setMillisUntilNextRefreshable(const qint32 &millis_until_next_refreshable) {
    m_millis_until_next_refreshable = millis_until_next_refreshable;
    m_millis_until_next_refreshable_isSet = true;
}

bool OAIRefreshTrustedAdvisorCheckResponse_status::is_millis_until_next_refreshable_Set() const{
    return m_millis_until_next_refreshable_isSet;
}

bool OAIRefreshTrustedAdvisorCheckResponse_status::is_millis_until_next_refreshable_Valid() const{
    return m_millis_until_next_refreshable_isValid;
}

bool OAIRefreshTrustedAdvisorCheckResponse_status::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_check_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_status_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_millis_until_next_refreshable_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAIRefreshTrustedAdvisorCheckResponse_status::isValid() const {
    // only required properties are required for the object to be considered valid
    return m_check_id_isValid && m_status_isValid && m_millis_until_next_refreshable_isValid && true;
}

} // namespace OpenAPI
