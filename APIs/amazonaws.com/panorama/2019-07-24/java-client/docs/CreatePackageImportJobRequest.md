

# CreatePackageImportJobRequest


## Properties

| Name | Type | Description | Notes |
|------------ | ------------- | ------------- | -------------|
|**clientToken** | **String** | A client token for the package import job. |  |
|**inputConfig** | [**CreatePackageImportJobRequestInputConfig**](CreatePackageImportJobRequestInputConfig.md) |  |  |
|**jobTags** | [**List&lt;JobResourceTags&gt;**](JobResourceTags.md) | Tags for the package import job. |  [optional] |
|**jobType** | [**JobTypeEnum**](#JobTypeEnum) | A job type for the package import job. |  |
|**outputConfig** | [**CreatePackageImportJobRequestOutputConfig**](CreatePackageImportJobRequestOutputConfig.md) |  |  |



## Enum: JobTypeEnum

| Name | Value |
|---- | -----|
| NODE_PACKAGE_VERSION | &quot;NODE_PACKAGE_VERSION&quot; |
| MARKETPLACE_NODE_PACKAGE_VERSION | &quot;MARKETPLACE_NODE_PACKAGE_VERSION&quot; |



