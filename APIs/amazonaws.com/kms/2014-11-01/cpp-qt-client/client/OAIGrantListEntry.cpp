/**
 * AWS Key Management Service
 * <fullname>Key Management Service</fullname> <p>Key Management Service (KMS) is an encryption and key management web service. This guide describes the KMS operations that you can call programmatically. For general information about KMS, see the <a href=\"https://docs.aws.amazon.com/kms/latest/developerguide/\"> <i>Key Management Service Developer Guide</i> </a>.</p> <note> <p>KMS has replaced the term <i>customer master key (CMK)</i> with <i>KMS key</i> and <i>KMS key</i>. The concept has not changed. To prevent breaking changes, KMS is keeping some variations of this term.</p> <p>Amazon Web Services provides SDKs that consist of libraries and sample code for various programming languages and platforms (Java, Ruby, .Net, macOS, Android, etc.). The SDKs provide a convenient way to create programmatic access to KMS and other Amazon Web Services services. For example, the SDKs take care of tasks such as signing requests (see below), managing errors, and retrying requests automatically. For more information about the Amazon Web Services SDKs, including how to download and install them, see <a href=\"http://aws.amazon.com/tools/\">Tools for Amazon Web Services</a>.</p> </note> <p>We recommend that you use the Amazon Web Services SDKs to make programmatic API calls to KMS.</p> <p>If you need to use FIPS 140-2 validated cryptographic modules when communicating with Amazon Web Services, use the FIPS endpoint in your preferred Amazon Web Services Region. For more information about the available FIPS endpoints, see <a href=\"https://docs.aws.amazon.com/general/latest/gr/kms.html#kms_region\">Service endpoints</a> in the Key Management Service topic of the <i>Amazon Web Services General Reference</i>.</p> <p>All KMS API calls must be signed and be transmitted using Transport Layer Security (TLS). KMS recommends you always use the latest supported TLS version. Clients must also support cipher suites with Perfect Forward Secrecy (PFS) such as Ephemeral Diffie-Hellman (DHE) or Elliptic Curve Ephemeral Diffie-Hellman (ECDHE). Most modern systems such as Java 7 and later support these modes.</p> <p> <b>Signing Requests</b> </p> <p>Requests must be signed using an access key ID and a secret access key. We strongly recommend that you do not use your Amazon Web Services account root access key ID and secret access key for everyday work. You can use the access key ID and secret access key for an IAM user or you can use the Security Token Service (STS) to generate temporary security credentials and use those to sign requests. </p> <p>All KMS requests must be signed with <a href=\"https://docs.aws.amazon.com/general/latest/gr/signature-version-4.html\">Signature Version 4</a>.</p> <p> <b>Logging API Requests</b> </p> <p>KMS supports CloudTrail, a service that logs Amazon Web Services API calls and related events for your Amazon Web Services account and delivers them to an Amazon S3 bucket that you specify. By using the information collected by CloudTrail, you can determine what requests were made to KMS, who made the request, when it was made, and so on. To learn more about CloudTrail, including how to turn it on and find your log files, see the <a href=\"https://docs.aws.amazon.com/awscloudtrail/latest/userguide/\">CloudTrail User Guide</a>.</p> <p> <b>Additional Resources</b> </p> <p>For more information about credentials and request signing, see the following:</p> <ul> <li> <p> <a href=\"https://docs.aws.amazon.com/general/latest/gr/aws-security-credentials.html\">Amazon Web Services Security Credentials</a> - This topic provides general information about the types of credentials used to access Amazon Web Services.</p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp.html\">Temporary Security Credentials</a> - This section of the <i>IAM User Guide</i> describes how to create and use temporary security credentials.</p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/general/latest/gr/signature-version-4.html\">Signature Version 4 Signing Process</a> - This set of topics walks you through the process of signing a request using an access key ID and a secret access key.</p> </li> </ul> <p> <b>Commonly Used API Operations</b> </p> <p>Of the API operations discussed in this guide, the following will prove the most useful for most applications. You will likely perform operations other than these, such as creating keys and assigning policies, by using the console.</p> <ul> <li> <p> <a>Encrypt</a> </p> </li> <li> <p> <a>Decrypt</a> </p> </li> <li> <p> <a>GenerateDataKey</a> </p> </li> <li> <p> <a>GenerateDataKeyWithoutPlaintext</a> </p> </li> </ul>
 *
 * The version of the OpenAPI document: 2014-11-01
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIGrantListEntry.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIGrantListEntry::OAIGrantListEntry(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIGrantListEntry::OAIGrantListEntry() {
    this->initializeModel();
}

OAIGrantListEntry::~OAIGrantListEntry() {}

void OAIGrantListEntry::initializeModel() {

    m_key_id_isSet = false;
    m_key_id_isValid = false;

    m_grant_id_isSet = false;
    m_grant_id_isValid = false;

    m_name_isSet = false;
    m_name_isValid = false;

    m_creation_date_isSet = false;
    m_creation_date_isValid = false;

    m_grantee_principal_isSet = false;
    m_grantee_principal_isValid = false;

    m_retiring_principal_isSet = false;
    m_retiring_principal_isValid = false;

    m_issuing_account_isSet = false;
    m_issuing_account_isValid = false;

    m_operations_isSet = false;
    m_operations_isValid = false;

    m_constraints_isSet = false;
    m_constraints_isValid = false;
}

void OAIGrantListEntry::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAIGrantListEntry::fromJsonObject(QJsonObject json) {

    m_key_id_isValid = ::OpenAPI::fromJsonValue(m_key_id, json[QString("KeyId")]);
    m_key_id_isSet = !json[QString("KeyId")].isNull() && m_key_id_isValid;

    m_grant_id_isValid = ::OpenAPI::fromJsonValue(m_grant_id, json[QString("GrantId")]);
    m_grant_id_isSet = !json[QString("GrantId")].isNull() && m_grant_id_isValid;

    m_name_isValid = ::OpenAPI::fromJsonValue(m_name, json[QString("Name")]);
    m_name_isSet = !json[QString("Name")].isNull() && m_name_isValid;

    m_creation_date_isValid = ::OpenAPI::fromJsonValue(m_creation_date, json[QString("CreationDate")]);
    m_creation_date_isSet = !json[QString("CreationDate")].isNull() && m_creation_date_isValid;

    m_grantee_principal_isValid = ::OpenAPI::fromJsonValue(m_grantee_principal, json[QString("GranteePrincipal")]);
    m_grantee_principal_isSet = !json[QString("GranteePrincipal")].isNull() && m_grantee_principal_isValid;

    m_retiring_principal_isValid = ::OpenAPI::fromJsonValue(m_retiring_principal, json[QString("RetiringPrincipal")]);
    m_retiring_principal_isSet = !json[QString("RetiringPrincipal")].isNull() && m_retiring_principal_isValid;

    m_issuing_account_isValid = ::OpenAPI::fromJsonValue(m_issuing_account, json[QString("IssuingAccount")]);
    m_issuing_account_isSet = !json[QString("IssuingAccount")].isNull() && m_issuing_account_isValid;

    m_operations_isValid = ::OpenAPI::fromJsonValue(m_operations, json[QString("Operations")]);
    m_operations_isSet = !json[QString("Operations")].isNull() && m_operations_isValid;

    m_constraints_isValid = ::OpenAPI::fromJsonValue(m_constraints, json[QString("Constraints")]);
    m_constraints_isSet = !json[QString("Constraints")].isNull() && m_constraints_isValid;
}

QString OAIGrantListEntry::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAIGrantListEntry::asJsonObject() const {
    QJsonObject obj;
    if (m_key_id_isSet) {
        obj.insert(QString("KeyId"), ::OpenAPI::toJsonValue(m_key_id));
    }
    if (m_grant_id_isSet) {
        obj.insert(QString("GrantId"), ::OpenAPI::toJsonValue(m_grant_id));
    }
    if (m_name_isSet) {
        obj.insert(QString("Name"), ::OpenAPI::toJsonValue(m_name));
    }
    if (m_creation_date_isSet) {
        obj.insert(QString("CreationDate"), ::OpenAPI::toJsonValue(m_creation_date));
    }
    if (m_grantee_principal_isSet) {
        obj.insert(QString("GranteePrincipal"), ::OpenAPI::toJsonValue(m_grantee_principal));
    }
    if (m_retiring_principal_isSet) {
        obj.insert(QString("RetiringPrincipal"), ::OpenAPI::toJsonValue(m_retiring_principal));
    }
    if (m_issuing_account_isSet) {
        obj.insert(QString("IssuingAccount"), ::OpenAPI::toJsonValue(m_issuing_account));
    }
    if (m_operations.isSet()) {
        obj.insert(QString("Operations"), ::OpenAPI::toJsonValue(m_operations));
    }
    if (m_constraints.isSet()) {
        obj.insert(QString("Constraints"), ::OpenAPI::toJsonValue(m_constraints));
    }
    return obj;
}

QString OAIGrantListEntry::getKeyId() const {
    return m_key_id;
}
void OAIGrantListEntry::setKeyId(const QString &key_id) {
    m_key_id = key_id;
    m_key_id_isSet = true;
}

bool OAIGrantListEntry::is_key_id_Set() const{
    return m_key_id_isSet;
}

bool OAIGrantListEntry::is_key_id_Valid() const{
    return m_key_id_isValid;
}

QString OAIGrantListEntry::getGrantId() const {
    return m_grant_id;
}
void OAIGrantListEntry::setGrantId(const QString &grant_id) {
    m_grant_id = grant_id;
    m_grant_id_isSet = true;
}

bool OAIGrantListEntry::is_grant_id_Set() const{
    return m_grant_id_isSet;
}

bool OAIGrantListEntry::is_grant_id_Valid() const{
    return m_grant_id_isValid;
}

QString OAIGrantListEntry::getName() const {
    return m_name;
}
void OAIGrantListEntry::setName(const QString &name) {
    m_name = name;
    m_name_isSet = true;
}

bool OAIGrantListEntry::is_name_Set() const{
    return m_name_isSet;
}

bool OAIGrantListEntry::is_name_Valid() const{
    return m_name_isValid;
}

QDateTime OAIGrantListEntry::getCreationDate() const {
    return m_creation_date;
}
void OAIGrantListEntry::setCreationDate(const QDateTime &creation_date) {
    m_creation_date = creation_date;
    m_creation_date_isSet = true;
}

bool OAIGrantListEntry::is_creation_date_Set() const{
    return m_creation_date_isSet;
}

bool OAIGrantListEntry::is_creation_date_Valid() const{
    return m_creation_date_isValid;
}

QString OAIGrantListEntry::getGranteePrincipal() const {
    return m_grantee_principal;
}
void OAIGrantListEntry::setGranteePrincipal(const QString &grantee_principal) {
    m_grantee_principal = grantee_principal;
    m_grantee_principal_isSet = true;
}

bool OAIGrantListEntry::is_grantee_principal_Set() const{
    return m_grantee_principal_isSet;
}

bool OAIGrantListEntry::is_grantee_principal_Valid() const{
    return m_grantee_principal_isValid;
}

QString OAIGrantListEntry::getRetiringPrincipal() const {
    return m_retiring_principal;
}
void OAIGrantListEntry::setRetiringPrincipal(const QString &retiring_principal) {
    m_retiring_principal = retiring_principal;
    m_retiring_principal_isSet = true;
}

bool OAIGrantListEntry::is_retiring_principal_Set() const{
    return m_retiring_principal_isSet;
}

bool OAIGrantListEntry::is_retiring_principal_Valid() const{
    return m_retiring_principal_isValid;
}

QString OAIGrantListEntry::getIssuingAccount() const {
    return m_issuing_account;
}
void OAIGrantListEntry::setIssuingAccount(const QString &issuing_account) {
    m_issuing_account = issuing_account;
    m_issuing_account_isSet = true;
}

bool OAIGrantListEntry::is_issuing_account_Set() const{
    return m_issuing_account_isSet;
}

bool OAIGrantListEntry::is_issuing_account_Valid() const{
    return m_issuing_account_isValid;
}

QList OAIGrantListEntry::getOperations() const {
    return m_operations;
}
void OAIGrantListEntry::setOperations(const QList &operations) {
    m_operations = operations;
    m_operations_isSet = true;
}

bool OAIGrantListEntry::is_operations_Set() const{
    return m_operations_isSet;
}

bool OAIGrantListEntry::is_operations_Valid() const{
    return m_operations_isValid;
}

OAIGrantListEntry_Constraints OAIGrantListEntry::getConstraints() const {
    return m_constraints;
}
void OAIGrantListEntry::setConstraints(const OAIGrantListEntry_Constraints &constraints) {
    m_constraints = constraints;
    m_constraints_isSet = true;
}

bool OAIGrantListEntry::is_constraints_Set() const{
    return m_constraints_isSet;
}

bool OAIGrantListEntry::is_constraints_Valid() const{
    return m_constraints_isValid;
}

bool OAIGrantListEntry::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_key_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_grant_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_name_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_creation_date_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_grantee_principal_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_retiring_principal_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_issuing_account_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_operations.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_constraints.isSet()) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAIGrantListEntry::isValid() const {
    // only required properties are required for the object to be considered valid
    return true;
}

} // namespace OpenAPI
