...
public class DisableKeyRotationRequest {
   ...
   * @throws IOException if the JSON Element is invalid with respect to DisableKeyRotationRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DisableKeyRotationRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in DisableKeyRotationRequest is not found in the empty JSON string", DisableKeyRotationRequest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DisableKeyRotationRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DisableKeyRotationRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : DisableKeyRotationRequest.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!DisableKeyRotationRequest.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'DisableKeyRotationRequest' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<DisableKeyRotationRequest> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(DisableKeyRotationRequest.class));

       return (TypeAdapter<T>) new TypeAdapter<DisableKeyRotationRequest>() {
           @Override
           public void write(JsonWriter out, DisableKeyRotationRequest value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public DisableKeyRotationRequest read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of DisableKeyRotationRequest given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of DisableKeyRotationRequest
   * @throws IOException if the JSON string is invalid with respect to DisableKeyRotationRequest
   */
  public static DisableKeyRotationRequest fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, DisableKeyRotationRequest.class);
  }

  /**
   * Convert an instance of DisableKeyRotationRequest to an JSON string
   *
   * @return JSON string
   */
