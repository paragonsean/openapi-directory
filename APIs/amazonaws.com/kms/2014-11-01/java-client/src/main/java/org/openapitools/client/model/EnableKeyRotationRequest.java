...
public class EnableKeyRotationRequest {
   ...
   * @throws IOException if the JSON Element is invalid with respect to EnableKeyRotationRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!EnableKeyRotationRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in EnableKeyRotationRequest is not found in the empty JSON string", EnableKeyRotationRequest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!EnableKeyRotationRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `EnableKeyRotationRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : EnableKeyRotationRequest.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!EnableKeyRotationRequest.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'EnableKeyRotationRequest' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<EnableKeyRotationRequest> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(EnableKeyRotationRequest.class));

       return (TypeAdapter<T>) new TypeAdapter<EnableKeyRotationRequest>() {
           @Override
           public void write(JsonWriter out, EnableKeyRotationRequest value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public EnableKeyRotationRequest read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of EnableKeyRotationRequest given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of EnableKeyRotationRequest
   * @throws IOException if the JSON string is invalid with respect to EnableKeyRotationRequest
   */
  public static EnableKeyRotationRequest fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, EnableKeyRotationRequest.class);
  }

  /**
   * Convert an instance of EnableKeyRotationRequest to an JSON string
   *
   * @return JSON string
   */
