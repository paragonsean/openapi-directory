...
public class DisableKeyRequest {
   ...
   * @throws IOException if the JSON Element is invalid with respect to DisableKeyRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DisableKeyRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in DisableKeyRequest is not found in the empty JSON string", DisableKeyRequest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DisableKeyRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DisableKeyRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : DisableKeyRequest.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!DisableKeyRequest.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'DisableKeyRequest' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<DisableKeyRequest> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(DisableKeyRequest.class));

       return (TypeAdapter<T>) new TypeAdapter<DisableKeyRequest>() {
           @Override
           public void write(JsonWriter out, DisableKeyRequest value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public DisableKeyRequest read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of DisableKeyRequest given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of DisableKeyRequest
   * @throws IOException if the JSON string is invalid with respect to DisableKeyRequest
   */
  public static DisableKeyRequest fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, DisableKeyRequest.class);
  }

  /**
   * Convert an instance of DisableKeyRequest to an JSON string
   *
   * @return JSON string
   */
