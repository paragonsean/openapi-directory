/**
 * Amazon FSx
 * Amazon FSx is a fully managed service that makes it easy for storage and application administrators to launch and use shared file storage.
 *
 * The version of the OpenAPI document: 2018-03-01
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', process.cwd()+'/src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require(process.cwd()+'/src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.AmazonFsx);
  }
}(this, function(expect, AmazonFsx) {
  'use strict';

  var instance;

  beforeEach(function() {
    instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
  });

  var getProperty = function(object, getter, property) {
    // Use getter method if present; otherwise, get the property directly.
    if (typeof object[getter] === 'function')
      return object[getter]();
    else
      return object[property];
  }

  var setProperty = function(object, setter, property, value) {
    // Use setter method if present; otherwise, set the property directly.
    if (typeof object[setter] === 'function')
      object[setter](value);
    else
      object[property] = value;
  }

  describe('UpdateFileSystemResponseFileSystem', function() {
    it('should create an instance of UpdateFileSystemResponseFileSystem', function() {
      // uncomment below and update the code to test UpdateFileSystemResponseFileSystem
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be.a(AmazonFsx.UpdateFileSystemResponseFileSystem);
    });

    it('should have the property ownerId (base name: "OwnerId")', function() {
      // uncomment below and update the code to test the property ownerId
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

    it('should have the property creationTime (base name: "CreationTime")', function() {
      // uncomment below and update the code to test the property creationTime
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

    it('should have the property fileSystemId (base name: "FileSystemId")', function() {
      // uncomment below and update the code to test the property fileSystemId
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

    it('should have the property fileSystemType (base name: "FileSystemType")', function() {
      // uncomment below and update the code to test the property fileSystemType
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

    it('should have the property lifecycle (base name: "Lifecycle")', function() {
      // uncomment below and update the code to test the property lifecycle
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

    it('should have the property failureDetails (base name: "FailureDetails")', function() {
      // uncomment below and update the code to test the property failureDetails
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

    it('should have the property storageCapacity (base name: "StorageCapacity")', function() {
      // uncomment below and update the code to test the property storageCapacity
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

    it('should have the property storageType (base name: "StorageType")', function() {
      // uncomment below and update the code to test the property storageType
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

    it('should have the property vpcId (base name: "VpcId")', function() {
      // uncomment below and update the code to test the property vpcId
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

    it('should have the property subnetIds (base name: "SubnetIds")', function() {
      // uncomment below and update the code to test the property subnetIds
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

    it('should have the property networkInterfaceIds (base name: "NetworkInterfaceIds")', function() {
      // uncomment below and update the code to test the property networkInterfaceIds
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

    it('should have the property dNSName (base name: "DNSName")', function() {
      // uncomment below and update the code to test the property dNSName
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

    it('should have the property kmsKeyId (base name: "KmsKeyId")', function() {
      // uncomment below and update the code to test the property kmsKeyId
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

    it('should have the property resourceARN (base name: "ResourceARN")', function() {
      // uncomment below and update the code to test the property resourceARN
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

    it('should have the property tags (base name: "Tags")', function() {
      // uncomment below and update the code to test the property tags
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

    it('should have the property windowsConfiguration (base name: "WindowsConfiguration")', function() {
      // uncomment below and update the code to test the property windowsConfiguration
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

    it('should have the property lustreConfiguration (base name: "LustreConfiguration")', function() {
      // uncomment below and update the code to test the property lustreConfiguration
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

    it('should have the property administrativeActions (base name: "AdministrativeActions")', function() {
      // uncomment below and update the code to test the property administrativeActions
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

    it('should have the property ontapConfiguration (base name: "OntapConfiguration")', function() {
      // uncomment below and update the code to test the property ontapConfiguration
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

    it('should have the property fileSystemTypeVersion (base name: "FileSystemTypeVersion")', function() {
      // uncomment below and update the code to test the property fileSystemTypeVersion
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

    it('should have the property openZFSConfiguration (base name: "OpenZFSConfiguration")', function() {
      // uncomment below and update the code to test the property openZFSConfiguration
      //var instance = new AmazonFsx.UpdateFileSystemResponseFileSystem();
      //expect(instance).to.be();
    });

  });

}));
