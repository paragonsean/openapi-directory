/**
 * Amazon FSx
 * Amazon FSx is a fully managed service that makes it easy for storage and application administrators to launch and use shared file storage.
 *
 * The version of the OpenAPI document: 2018-03-01
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIUpdateSnapshotResponse_Snapshot.h
 *
 * 
 */

#ifndef OAIUpdateSnapshotResponse_Snapshot_H
#define OAIUpdateSnapshotResponse_Snapshot_H

#include <QJsonObject>

#include "OAILifecycleTransitionReason.h"
#include "OAISnapshotLifecycle.h"
#include "OAITag.h"
#include <QDateTime>
#include <QList>
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {
class OAILifecycleTransitionReason;
class OAITag;

class OAIUpdateSnapshotResponse_Snapshot : public OAIObject {
public:
    OAIUpdateSnapshotResponse_Snapshot();
    OAIUpdateSnapshotResponse_Snapshot(QString json);
    ~OAIUpdateSnapshotResponse_Snapshot() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QString getResourceArn() const;
    void setResourceArn(const QString &resource_arn);
    bool is_resource_arn_Set() const;
    bool is_resource_arn_Valid() const;

    QString getSnapshotId() const;
    void setSnapshotId(const QString &snapshot_id);
    bool is_snapshot_id_Set() const;
    bool is_snapshot_id_Valid() const;

    QString getName() const;
    void setName(const QString &name);
    bool is_name_Set() const;
    bool is_name_Valid() const;

    QString getVolumeId() const;
    void setVolumeId(const QString &volume_id);
    bool is_volume_id_Set() const;
    bool is_volume_id_Valid() const;

    QDateTime getCreationTime() const;
    void setCreationTime(const QDateTime &creation_time);
    bool is_creation_time_Set() const;
    bool is_creation_time_Valid() const;

    OAISnapshotLifecycle getLifecycle() const;
    void setLifecycle(const OAISnapshotLifecycle &lifecycle);
    bool is_lifecycle_Set() const;
    bool is_lifecycle_Valid() const;

    OAILifecycleTransitionReason getLifecycleTransitionReason() const;
    void setLifecycleTransitionReason(const OAILifecycleTransitionReason &lifecycle_transition_reason);
    bool is_lifecycle_transition_reason_Set() const;
    bool is_lifecycle_transition_reason_Valid() const;

    QList<OAITag> getTags() const;
    void setTags(const QList<OAITag> &tags);
    bool is_tags_Set() const;
    bool is_tags_Valid() const;

    QList getAdministrativeActions() const;
    void setAdministrativeActions(const QList &administrative_actions);
    bool is_administrative_actions_Set() const;
    bool is_administrative_actions_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString m_resource_arn;
    bool m_resource_arn_isSet;
    bool m_resource_arn_isValid;

    QString m_snapshot_id;
    bool m_snapshot_id_isSet;
    bool m_snapshot_id_isValid;

    QString m_name;
    bool m_name_isSet;
    bool m_name_isValid;

    QString m_volume_id;
    bool m_volume_id_isSet;
    bool m_volume_id_isValid;

    QDateTime m_creation_time;
    bool m_creation_time_isSet;
    bool m_creation_time_isValid;

    OAISnapshotLifecycle m_lifecycle;
    bool m_lifecycle_isSet;
    bool m_lifecycle_isValid;

    OAILifecycleTransitionReason m_lifecycle_transition_reason;
    bool m_lifecycle_transition_reason_isSet;
    bool m_lifecycle_transition_reason_isValid;

    QList<OAITag> m_tags;
    bool m_tags_isSet;
    bool m_tags_isValid;

    QList m_administrative_actions;
    bool m_administrative_actions_isSet;
    bool m_administrative_actions_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIUpdateSnapshotResponse_Snapshot)

#endif // OAIUpdateSnapshotResponse_Snapshot_H
