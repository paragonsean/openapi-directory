/**
 * AWS Server Migration Service
 * <important> <p> <b>Product update</b> </p> <p>We recommend <a href=\"http://aws.amazon.com/application-migration-service\">Amazon Web Services Application Migration Service</a> (Amazon Web Services MGN) as the primary migration service for lift-and-shift migrations. If Amazon Web Services MGN is unavailable in a specific Amazon Web Services Region, you can use the Server Migration Service APIs through March 2023.</p> </important> <p>Server Migration Service (Server Migration Service) makes it easier and faster for you to migrate your on-premises workloads to Amazon Web Services. To learn more about Server Migration Service, see the following resources:</p> <ul> <li> <p> <a href=\"http://aws.amazon.com/server-migration-service/\">Server Migration Service product page</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/server-migration-service/latest/userguide/\">Server Migration Service User Guide</a> </p> </li> </ul>
 *
 * The version of the OpenAPI document: 2016-10-24
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIConnectorStatus.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIConnectorStatus::OAIConnectorStatus(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIConnectorStatus::OAIConnectorStatus() {
    this->initializeModel();
}

OAIConnectorStatus::~OAIConnectorStatus() {}

void OAIConnectorStatus::initializeModel() {

    m_value_isSet = false;
    m_value_isValid = false;
    m_value = eOAIConnectorStatus::INVALID_VALUE_OPENAPI_GENERATED;
}

void OAIConnectorStatus::fromJson(QString jsonString) {
    
    if ( jsonString.compare("HEALTHY", Qt::CaseInsensitive) == 0) {
        m_value = eOAIConnectorStatus::HEALTHY;
        m_value_isSet = m_value_isValid = true;
    }
    else if ( jsonString.compare("UNHEALTHY", Qt::CaseInsensitive) == 0) {
        m_value = eOAIConnectorStatus::UNHEALTHY;
        m_value_isSet = m_value_isValid = true;
    }
}

void OAIConnectorStatus::fromJsonValue(QJsonValue json) {
fromJson(json.toString());
}

QString OAIConnectorStatus::asJson() const {
    
    QString val;
    switch (m_value){
        case eOAIConnectorStatus::HEALTHY:
            val = "HEALTHY";
            break;
        case eOAIConnectorStatus::UNHEALTHY:
            val = "UNHEALTHY";
            break;
        default:
            break;
    }
    return val;
}

QJsonValue OAIConnectorStatus::asJsonValue() const {
    
    return QJsonValue(asJson());
}


OAIConnectorStatus::eOAIConnectorStatus OAIConnectorStatus::getValue() const {
    return m_value;
}

void OAIConnectorStatus::setValue(const OAIConnectorStatus::eOAIConnectorStatus& value){
    m_value = value;
    m_value_isSet = true;
}
bool OAIConnectorStatus::isSet() const {
    
    return m_value_isSet;
}

bool OAIConnectorStatus::isValid() const {
    // only required properties are required for the object to be considered valid
    return m_value_isValid;
}

} // namespace OpenAPI
