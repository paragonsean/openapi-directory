/**
 * Amazon Appflow
 * <p>Welcome to the Amazon AppFlow API reference. This guide is for developers who need detailed information about the Amazon AppFlow API operations, data types, and errors. </p> <p>Amazon AppFlow is a fully managed integration service that enables you to securely transfer data between software as a service (SaaS) applications like Salesforce, Marketo, Slack, and ServiceNow, and Amazon Web Services like Amazon S3 and Amazon Redshift. </p> <p>Use the following links to get started on the Amazon AppFlow API:</p> <ul> <li> <p> <a href=\"https://docs.aws.amazon.com/appflow/1.0/APIReference/API_Operations.html\">Actions</a>: An alphabetical list of all Amazon AppFlow API operations.</p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/appflow/1.0/APIReference/API_Types.html\">Data types</a>: An alphabetical list of all Amazon AppFlow data types.</p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/appflow/1.0/APIReference/CommonParameters.html\">Common parameters</a>: Parameters that all Query operations can use.</p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/appflow/1.0/APIReference/CommonErrors.html\">Common errors</a>: Client and server errors that all operations can return.</p> </li> </ul> <p>If you're new to Amazon AppFlow, we recommend that you review the <a href=\"https://docs.aws.amazon.com/appflow/latest/userguide/what-is-appflow.html\">Amazon AppFlow User Guide</a>.</p> <p>Amazon AppFlow API users can use vendor-specific mechanisms for OAuth, and include applicable OAuth attributes (such as <code>auth-code</code> and <code>redirecturi</code>) with the connector-specific <code>ConnectorProfileProperties</code> when creating a new connector profile using Amazon AppFlow API operations. For example, Salesforce users can refer to the <a href=\"https://help.salesforce.com/articleView?id=remoteaccess_authenticate.htm\"> <i>Authorize Apps with OAuth</i> </a> documentation.</p>
 *
 * The version of the OpenAPI document: 2020-08-23
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIStartFlowResponse.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIStartFlowResponse::OAIStartFlowResponse(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIStartFlowResponse::OAIStartFlowResponse() {
    this->initializeModel();
}

OAIStartFlowResponse::~OAIStartFlowResponse() {}

void OAIStartFlowResponse::initializeModel() {

    m_flow_arn_isSet = false;
    m_flow_arn_isValid = false;

    m_flow_status_isSet = false;
    m_flow_status_isValid = false;

    m_execution_id_isSet = false;
    m_execution_id_isValid = false;
}

void OAIStartFlowResponse::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAIStartFlowResponse::fromJsonObject(QJsonObject json) {

    m_flow_arn_isValid = ::OpenAPI::fromJsonValue(m_flow_arn, json[QString("flowArn")]);
    m_flow_arn_isSet = !json[QString("flowArn")].isNull() && m_flow_arn_isValid;

    m_flow_status_isValid = ::OpenAPI::fromJsonValue(m_flow_status, json[QString("flowStatus")]);
    m_flow_status_isSet = !json[QString("flowStatus")].isNull() && m_flow_status_isValid;

    m_execution_id_isValid = ::OpenAPI::fromJsonValue(m_execution_id, json[QString("executionId")]);
    m_execution_id_isSet = !json[QString("executionId")].isNull() && m_execution_id_isValid;
}

QString OAIStartFlowResponse::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAIStartFlowResponse::asJsonObject() const {
    QJsonObject obj;
    if (m_flow_arn_isSet) {
        obj.insert(QString("flowArn"), ::OpenAPI::toJsonValue(m_flow_arn));
    }
    if (m_flow_status.isSet()) {
        obj.insert(QString("flowStatus"), ::OpenAPI::toJsonValue(m_flow_status));
    }
    if (m_execution_id_isSet) {
        obj.insert(QString("executionId"), ::OpenAPI::toJsonValue(m_execution_id));
    }
    return obj;
}

QString OAIStartFlowResponse::getFlowArn() const {
    return m_flow_arn;
}
void OAIStartFlowResponse::setFlowArn(const QString &flow_arn) {
    m_flow_arn = flow_arn;
    m_flow_arn_isSet = true;
}

bool OAIStartFlowResponse::is_flow_arn_Set() const{
    return m_flow_arn_isSet;
}

bool OAIStartFlowResponse::is_flow_arn_Valid() const{
    return m_flow_arn_isValid;
}

OAIFlowStatus OAIStartFlowResponse::getFlowStatus() const {
    return m_flow_status;
}
void OAIStartFlowResponse::setFlowStatus(const OAIFlowStatus &flow_status) {
    m_flow_status = flow_status;
    m_flow_status_isSet = true;
}

bool OAIStartFlowResponse::is_flow_status_Set() const{
    return m_flow_status_isSet;
}

bool OAIStartFlowResponse::is_flow_status_Valid() const{
    return m_flow_status_isValid;
}

QString OAIStartFlowResponse::getExecutionId() const {
    return m_execution_id;
}
void OAIStartFlowResponse::setExecutionId(const QString &execution_id) {
    m_execution_id = execution_id;
    m_execution_id_isSet = true;
}

bool OAIStartFlowResponse::is_execution_id_Set() const{
    return m_execution_id_isSet;
}

bool OAIStartFlowResponse::is_execution_id_Valid() const{
    return m_execution_id_isValid;
}

bool OAIStartFlowResponse::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_flow_arn_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_flow_status.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_execution_id_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAIStartFlowResponse::isValid() const {
    // only required properties are required for the object to be considered valid
    return true;
}

} // namespace OpenAPI
