/**
 * Amazon SageMaker Service
 * <p>Provides APIs for creating and managing SageMaker resources. </p> <p>Other Resources:</p> <ul> <li> <p> <a href=\"https://docs.aws.amazon.com/sagemaker/latest/dg/whatis.html#first-time-user\">SageMaker Developer Guide</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/augmented-ai/2019-11-07/APIReference/Welcome.html\">Amazon Augmented AI Runtime API Reference</a> </p> </li> </ul>
 *
 * The version of the OpenAPI document: 2017-07-24
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import BatchDataCaptureConfig from './BatchDataCaptureConfig';
import BatchStrategy from './BatchStrategy';
import DataProcessing from './DataProcessing';
import ExperimentConfig from './ExperimentConfig';
import ModelClientConfig from './ModelClientConfig';
import TransformInput from './TransformInput';
import TransformJob from './TransformJob';
import TransformJobStatus from './TransformJobStatus';
import TransformOutput from './TransformOutput';
import TransformResources from './TransformResources';

/**
 * The TrialComponentSourceDetailTransformJob model module.
 * @module model/TrialComponentSourceDetailTransformJob
 * @version 2017-07-24
 */
class TrialComponentSourceDetailTransformJob {
    /**
     * Constructs a new <code>TrialComponentSourceDetailTransformJob</code>.
     * @alias module:model/TrialComponentSourceDetailTransformJob
     * @implements module:model/TransformJob
     */
    constructor() { 
        TransformJob.initialize(this);
        TrialComponentSourceDetailTransformJob.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>TrialComponentSourceDetailTransformJob</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/TrialComponentSourceDetailTransformJob} obj Optional instance to populate.
     * @return {module:model/TrialComponentSourceDetailTransformJob} The populated <code>TrialComponentSourceDetailTransformJob</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new TrialComponentSourceDetailTransformJob();
            TransformJob.constructFromObject(data, obj);

            if (data.hasOwnProperty('TransformJobName')) {
                obj['TransformJobName'] = ApiClient.convertToType(data['TransformJobName'], 'String');
            }
            if (data.hasOwnProperty('TransformJobArn')) {
                obj['TransformJobArn'] = ApiClient.convertToType(data['TransformJobArn'], 'String');
            }
            if (data.hasOwnProperty('TransformJobStatus')) {
                obj['TransformJobStatus'] = ApiClient.convertToType(data['TransformJobStatus'], TransformJobStatus);
            }
            if (data.hasOwnProperty('FailureReason')) {
                obj['FailureReason'] = ApiClient.convertToType(data['FailureReason'], 'String');
            }
            if (data.hasOwnProperty('ModelName')) {
                obj['ModelName'] = ApiClient.convertToType(data['ModelName'], 'String');
            }
            if (data.hasOwnProperty('MaxConcurrentTransforms')) {
                obj['MaxConcurrentTransforms'] = ApiClient.convertToType(data['MaxConcurrentTransforms'], 'Number');
            }
            if (data.hasOwnProperty('ModelClientConfig')) {
                obj['ModelClientConfig'] = ModelClientConfig.constructFromObject(data['ModelClientConfig']);
            }
            if (data.hasOwnProperty('MaxPayloadInMB')) {
                obj['MaxPayloadInMB'] = ApiClient.convertToType(data['MaxPayloadInMB'], 'Number');
            }
            if (data.hasOwnProperty('BatchStrategy')) {
                obj['BatchStrategy'] = ApiClient.convertToType(data['BatchStrategy'], BatchStrategy);
            }
            if (data.hasOwnProperty('Environment')) {
                obj['Environment'] = ApiClient.convertToType(data['Environment'], Object);
            }
            if (data.hasOwnProperty('TransformInput')) {
                obj['TransformInput'] = TransformInput.constructFromObject(data['TransformInput']);
            }
            if (data.hasOwnProperty('TransformOutput')) {
                obj['TransformOutput'] = TransformOutput.constructFromObject(data['TransformOutput']);
            }
            if (data.hasOwnProperty('TransformResources')) {
                obj['TransformResources'] = TransformResources.constructFromObject(data['TransformResources']);
            }
            if (data.hasOwnProperty('CreationTime')) {
                obj['CreationTime'] = ApiClient.convertToType(data['CreationTime'], 'Date');
            }
            if (data.hasOwnProperty('TransformStartTime')) {
                obj['TransformStartTime'] = ApiClient.convertToType(data['TransformStartTime'], 'Date');
            }
            if (data.hasOwnProperty('TransformEndTime')) {
                obj['TransformEndTime'] = ApiClient.convertToType(data['TransformEndTime'], 'Date');
            }
            if (data.hasOwnProperty('LabelingJobArn')) {
                obj['LabelingJobArn'] = ApiClient.convertToType(data['LabelingJobArn'], 'String');
            }
            if (data.hasOwnProperty('AutoMLJobArn')) {
                obj['AutoMLJobArn'] = ApiClient.convertToType(data['AutoMLJobArn'], 'String');
            }
            if (data.hasOwnProperty('DataProcessing')) {
                obj['DataProcessing'] = DataProcessing.constructFromObject(data['DataProcessing']);
            }
            if (data.hasOwnProperty('ExperimentConfig')) {
                obj['ExperimentConfig'] = ExperimentConfig.constructFromObject(data['ExperimentConfig']);
            }
            if (data.hasOwnProperty('Tags')) {
                obj['Tags'] = ApiClient.convertToType(data['Tags'], Array);
            }
            if (data.hasOwnProperty('DataCaptureConfig')) {
                obj['DataCaptureConfig'] = BatchDataCaptureConfig.constructFromObject(data['DataCaptureConfig']);
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>TrialComponentSourceDetailTransformJob</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>TrialComponentSourceDetailTransformJob</code>.
     */
    static validateJSON(data) {
        // validate the optional field `TransformJobName`
        if (data['TransformJobName']) { // data not null
          String.validateJSON(data['TransformJobName']);
        }
        // validate the optional field `TransformJobArn`
        if (data['TransformJobArn']) { // data not null
          String.validateJSON(data['TransformJobArn']);
        }
        // validate the optional field `TransformJobStatus`
        if (data['TransformJobStatus']) { // data not null
          TransformJobStatus.validateJSON(data['TransformJobStatus']);
        }
        // validate the optional field `FailureReason`
        if (data['FailureReason']) { // data not null
          String.validateJSON(data['FailureReason']);
        }
        // validate the optional field `ModelName`
        if (data['ModelName']) { // data not null
          String.validateJSON(data['ModelName']);
        }
        // validate the optional field `MaxConcurrentTransforms`
        if (data['MaxConcurrentTransforms']) { // data not null
          Number.validateJSON(data['MaxConcurrentTransforms']);
        }
        // validate the optional field `ModelClientConfig`
        if (data['ModelClientConfig']) { // data not null
          ModelClientConfig.validateJSON(data['ModelClientConfig']);
        }
        // validate the optional field `MaxPayloadInMB`
        if (data['MaxPayloadInMB']) { // data not null
          Number.validateJSON(data['MaxPayloadInMB']);
        }
        // validate the optional field `BatchStrategy`
        if (data['BatchStrategy']) { // data not null
          BatchStrategy.validateJSON(data['BatchStrategy']);
        }
        // validate the optional field `Environment`
        if (data['Environment']) { // data not null
          Object.validateJSON(data['Environment']);
        }
        // validate the optional field `TransformInput`
        if (data['TransformInput']) { // data not null
          TransformInput.validateJSON(data['TransformInput']);
        }
        // validate the optional field `TransformOutput`
        if (data['TransformOutput']) { // data not null
          TransformOutput.validateJSON(data['TransformOutput']);
        }
        // validate the optional field `TransformResources`
        if (data['TransformResources']) { // data not null
          TransformResources.validateJSON(data['TransformResources']);
        }
        // validate the optional field `CreationTime`
        if (data['CreationTime']) { // data not null
          Date.validateJSON(data['CreationTime']);
        }
        // validate the optional field `TransformStartTime`
        if (data['TransformStartTime']) { // data not null
          Date.validateJSON(data['TransformStartTime']);
        }
        // validate the optional field `TransformEndTime`
        if (data['TransformEndTime']) { // data not null
          Date.validateJSON(data['TransformEndTime']);
        }
        // validate the optional field `LabelingJobArn`
        if (data['LabelingJobArn']) { // data not null
          String.validateJSON(data['LabelingJobArn']);
        }
        // validate the optional field `AutoMLJobArn`
        if (data['AutoMLJobArn']) { // data not null
          String.validateJSON(data['AutoMLJobArn']);
        }
        // validate the optional field `DataProcessing`
        if (data['DataProcessing']) { // data not null
          DataProcessing.validateJSON(data['DataProcessing']);
        }
        // validate the optional field `ExperimentConfig`
        if (data['ExperimentConfig']) { // data not null
          ExperimentConfig.validateJSON(data['ExperimentConfig']);
        }
        // validate the optional field `Tags`
        if (data['Tags']) { // data not null
          Array.validateJSON(data['Tags']);
        }
        // validate the optional field `DataCaptureConfig`
        if (data['DataCaptureConfig']) { // data not null
          BatchDataCaptureConfig.validateJSON(data['DataCaptureConfig']);
        }

        return true;
    }


}



/**
 * @member {String} TransformJobName
 */
TrialComponentSourceDetailTransformJob.prototype['TransformJobName'] = undefined;

/**
 * @member {String} TransformJobArn
 */
TrialComponentSourceDetailTransformJob.prototype['TransformJobArn'] = undefined;

/**
 * @member {module:model/TransformJobStatus} TransformJobStatus
 */
TrialComponentSourceDetailTransformJob.prototype['TransformJobStatus'] = undefined;

/**
 * @member {String} FailureReason
 */
TrialComponentSourceDetailTransformJob.prototype['FailureReason'] = undefined;

/**
 * @member {String} ModelName
 */
TrialComponentSourceDetailTransformJob.prototype['ModelName'] = undefined;

/**
 * @member {Number} MaxConcurrentTransforms
 */
TrialComponentSourceDetailTransformJob.prototype['MaxConcurrentTransforms'] = undefined;

/**
 * @member {module:model/ModelClientConfig} ModelClientConfig
 */
TrialComponentSourceDetailTransformJob.prototype['ModelClientConfig'] = undefined;

/**
 * @member {Number} MaxPayloadInMB
 */
TrialComponentSourceDetailTransformJob.prototype['MaxPayloadInMB'] = undefined;

/**
 * @member {module:model/BatchStrategy} BatchStrategy
 */
TrialComponentSourceDetailTransformJob.prototype['BatchStrategy'] = undefined;

/**
 * @member {Object} Environment
 */
TrialComponentSourceDetailTransformJob.prototype['Environment'] = undefined;

/**
 * @member {module:model/TransformInput} TransformInput
 */
TrialComponentSourceDetailTransformJob.prototype['TransformInput'] = undefined;

/**
 * @member {module:model/TransformOutput} TransformOutput
 */
TrialComponentSourceDetailTransformJob.prototype['TransformOutput'] = undefined;

/**
 * @member {module:model/TransformResources} TransformResources
 */
TrialComponentSourceDetailTransformJob.prototype['TransformResources'] = undefined;

/**
 * @member {Date} CreationTime
 */
TrialComponentSourceDetailTransformJob.prototype['CreationTime'] = undefined;

/**
 * @member {Date} TransformStartTime
 */
TrialComponentSourceDetailTransformJob.prototype['TransformStartTime'] = undefined;

/**
 * @member {Date} TransformEndTime
 */
TrialComponentSourceDetailTransformJob.prototype['TransformEndTime'] = undefined;

/**
 * @member {String} LabelingJobArn
 */
TrialComponentSourceDetailTransformJob.prototype['LabelingJobArn'] = undefined;

/**
 * @member {String} AutoMLJobArn
 */
TrialComponentSourceDetailTransformJob.prototype['AutoMLJobArn'] = undefined;

/**
 * @member {module:model/DataProcessing} DataProcessing
 */
TrialComponentSourceDetailTransformJob.prototype['DataProcessing'] = undefined;

/**
 * @member {module:model/ExperimentConfig} ExperimentConfig
 */
TrialComponentSourceDetailTransformJob.prototype['ExperimentConfig'] = undefined;

/**
 * @member {Array} Tags
 */
TrialComponentSourceDetailTransformJob.prototype['Tags'] = undefined;

/**
 * @member {module:model/BatchDataCaptureConfig} DataCaptureConfig
 */
TrialComponentSourceDetailTransformJob.prototype['DataCaptureConfig'] = undefined;


// Implement TransformJob interface:
/**
 * @member {String} TransformJobName
 */
TransformJob.prototype['TransformJobName'] = undefined;
/**
 * @member {String} TransformJobArn
 */
TransformJob.prototype['TransformJobArn'] = undefined;
/**
 * @member {module:model/TransformJobStatus} TransformJobStatus
 */
TransformJob.prototype['TransformJobStatus'] = undefined;
/**
 * @member {String} FailureReason
 */
TransformJob.prototype['FailureReason'] = undefined;
/**
 * @member {String} ModelName
 */
TransformJob.prototype['ModelName'] = undefined;
/**
 * @member {Number} MaxConcurrentTransforms
 */
TransformJob.prototype['MaxConcurrentTransforms'] = undefined;
/**
 * @member {module:model/ModelClientConfig} ModelClientConfig
 */
TransformJob.prototype['ModelClientConfig'] = undefined;
/**
 * @member {Number} MaxPayloadInMB
 */
TransformJob.prototype['MaxPayloadInMB'] = undefined;
/**
 * @member {module:model/BatchStrategy} BatchStrategy
 */
TransformJob.prototype['BatchStrategy'] = undefined;
/**
 * @member {Object} Environment
 */
TransformJob.prototype['Environment'] = undefined;
/**
 * @member {module:model/TransformInput} TransformInput
 */
TransformJob.prototype['TransformInput'] = undefined;
/**
 * @member {module:model/TransformOutput} TransformOutput
 */
TransformJob.prototype['TransformOutput'] = undefined;
/**
 * @member {module:model/TransformResources} TransformResources
 */
TransformJob.prototype['TransformResources'] = undefined;
/**
 * @member {Date} CreationTime
 */
TransformJob.prototype['CreationTime'] = undefined;
/**
 * @member {Date} TransformStartTime
 */
TransformJob.prototype['TransformStartTime'] = undefined;
/**
 * @member {Date} TransformEndTime
 */
TransformJob.prototype['TransformEndTime'] = undefined;
/**
 * @member {String} LabelingJobArn
 */
TransformJob.prototype['LabelingJobArn'] = undefined;
/**
 * @member {String} AutoMLJobArn
 */
TransformJob.prototype['AutoMLJobArn'] = undefined;
/**
 * @member {module:model/DataProcessing} DataProcessing
 */
TransformJob.prototype['DataProcessing'] = undefined;
/**
 * @member {module:model/ExperimentConfig} ExperimentConfig
 */
TransformJob.prototype['ExperimentConfig'] = undefined;
/**
 * @member {Array} Tags
 */
TransformJob.prototype['Tags'] = undefined;
/**
 * @member {module:model/BatchDataCaptureConfig} DataCaptureConfig
 */
TransformJob.prototype['DataCaptureConfig'] = undefined;




export default TrialComponentSourceDetailTransformJob;

