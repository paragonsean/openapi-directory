...
public class PartitionKey {
  ...
  public PartitionKey name(String name) {
    this.name = name;
    return this;
  }
  ...
  public boolean equals(Object o) {
    ...
    PartitionKey partitionKey = (PartitionKey) o;
    return Objects.equals(this.name, partitionKey.name);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to PartitionKey
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PartitionKey.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PartitionKey is not found in the empty JSON string", PartitionKey.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PartitionKey.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PartitionKey` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : PartitionKey.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PartitionKey.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PartitionKey' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PartitionKey> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PartitionKey.class));

       return (TypeAdapter<T>) new TypeAdapter<PartitionKey>() {
           @Override
           public void write(JsonWriter out, PartitionKey value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PartitionKey read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of PartitionKey given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of PartitionKey
   * @throws IOException if the JSON string is invalid with respect to PartitionKey
   */
  public static PartitionKey fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PartitionKey.class);
  }

  /**
   * Convert an instance of PartitionKey to an JSON string
   *
   * @return JSON string
   */
