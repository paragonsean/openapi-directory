...
public class TimeToLive {
  ...
  public TimeToLive status(TimeToLiveStatus status) {
    this.status = status;
    return this;
  }
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(this.status, timeToLive.status);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to TimeToLive
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!TimeToLive.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in TimeToLive is not found in the empty JSON string", TimeToLive.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!TimeToLive.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TimeToLive` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : TimeToLive.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!TimeToLive.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'TimeToLive' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<TimeToLive> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(TimeToLive.class));

       return (TypeAdapter<T>) new TypeAdapter<TimeToLive>() {
           @Override
           public void write(JsonWriter out, TimeToLive value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public TimeToLive read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of TimeToLive given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of TimeToLive
   * @throws IOException if the JSON string is invalid with respect to TimeToLive
   */
  public static TimeToLive fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, TimeToLive.class);
  }

  /**
   * Convert an instance of TimeToLive to an JSON string
   *
   * @return JSON string
   */
