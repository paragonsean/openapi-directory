...
async def test_batch_get_findings(client):
    """Test case for batch_get_findings
...
async def test_create_scan(client):
    ...
    """
    body = {"scanName":"","resourceId":{"codeArtifactId":""},"clientToken":"","scanType":"","analysisType":"","tags":""}
    headers = { 
        'Accept': 'application/json',
        'Content-Type': 'application/json',
        'x_amz_content_sha256': 'x_amz_content_sha256_example',
...
async def test_get_findings(client):
    ...
    response = await client.request(
        method='GET',
        path='/findings/{scan_name}'.format(scan_name='scan_name_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_get_scan(client):
    ...
    response = await client.request(
        method='GET',
        path='/scans/{scan_name}'.format(scan_name='scan_name_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_list_findings_metrics(client):
    """Test case for list_findings_metrics
...
async def test_update_account_configuration(client):
    ...
    """
    body = {"encryptionConfig":{"kmsKeyArn":""}}
    headers = { 
        'Accept': 'application/json',
        'Content-Type': 'application/json',
        'x_amz_content_sha256': 'x_amz_content_sha256_example',
