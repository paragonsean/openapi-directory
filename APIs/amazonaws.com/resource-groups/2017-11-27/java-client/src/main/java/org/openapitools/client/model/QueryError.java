...
public class QueryError {
  ...
  public QueryError errorCode(QueryErrorCode errorCode) {
   ...
   * @throws IOException if the JSON Element is invalid with respect to QueryError
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!QueryError.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in QueryError is not found in the empty JSON string", QueryError.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!QueryError.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `QueryError` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // validate the optional field `ErrorCode`
      if (jsonObj.get("ErrorCode") != null && !jsonObj.get("ErrorCode").isJsonNull()) {
        QueryErrorCode.validateJsonElement(jsonObj.get("ErrorCode"));
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!QueryError.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'QueryError' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<QueryError> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(QueryError.class));

       return (TypeAdapter<T>) new TypeAdapter<QueryError>() {
           @Override
  ...
  /**
   * Create an instance of QueryError given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of QueryError
   * @throws IOException if the JSON string is invalid with respect to QueryError
   */
  public static QueryError fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, QueryError.class);
  }

  /**
   * Convert an instance of QueryError to an JSON string
   *
   * @return JSON string
   */
