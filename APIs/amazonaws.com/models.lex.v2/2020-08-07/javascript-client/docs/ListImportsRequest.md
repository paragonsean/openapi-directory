# AmazonLexModelBuildingV2.ListImportsRequest
...
**filters** | [**[ImportFilter]**](ImportFilter.md) | Provides the specification of a filter used to limit the bots in the response to only those that match the filter specification. You can only specify one filter and one string to filter on. | [optional] 
**maxResults** | **Number** | The maximum number of imports to return in each page of results. If there are fewer results than the max page size, only the actual number of results are returned. | [optional] 
