...
|**filters** | [**List&lt;AggregatedUtterancesFilter&gt;**](AggregatedUtterancesFilter.md) | Provides the specification of a filter used to limit the utterances in the response to only those that match the filter specification. You can only specify one filter and one string to filter on. |  [optional] |
|**maxResults** | **Integer** | The maximum number of utterances to return in each page of results. If there are fewer results than the maximum page size, only the actual number of results are returned. If you don&#39;t specify the &lt;code&gt;maxResults&lt;/code&gt; parameter, 1,000 results are returned. |  [optional] |
