/**
 * Amazon Rekognition
 * <p>This is the API Reference for <a href=\"https://docs.aws.amazon.com/rekognition/latest/dg/images.html\">Amazon Rekognition Image</a>, <a href=\"https://docs.aws.amazon.com/rekognition/latest/customlabels-dg/what-is.html\">Amazon Rekognition Custom Labels</a>, <a href=\"https://docs.aws.amazon.com/rekognition/latest/dg/video.html\">Amazon Rekognition Stored Video</a>, <a href=\"https://docs.aws.amazon.com/rekognition/latest/dg/streaming-video.html\">Amazon Rekognition Streaming Video</a>. It provides descriptions of actions, data types, common parameters, and common errors.</p> <p> <b>Amazon Rekognition Image</b> </p> <ul> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_AssociateFaces.html\">AssociateFaces</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_CompareFaces.html\">CompareFaces</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_CreateCollection.html\">CreateCollection</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_CreateUser.html\">CreateUser</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DeleteCollection.html\">DeleteCollection</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DeleteFaces.html\">DeleteFaces</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DeleteUser.html\">DeleteUser</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DescribeCollection.html\">DescribeCollection</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DetectFaces.html\">DetectFaces</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DetectLabels.html\">DetectLabels</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DetectModerationLabels.html\">DetectModerationLabels</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DetectProtectiveEquipment.html\">DetectProtectiveEquipment</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DetectText.html\">DetectText</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DisassociateFaces.html\">DisassociateFaces</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_GetCelebrityInfo.html\">GetCelebrityInfo</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_IndexFaces.html\">IndexFaces</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_ListCollections.html\">ListCollections</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_ListFaces.html\">ListFaces</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_ListFaces.html\">ListUsers</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_RecognizeCelebrities.html\">RecognizeCelebrities</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_SearchFaces.html\">SearchFaces</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_SearchFacesByImage.html\">SearchFacesByImage</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_SearchUsers.html\">SearchUsers</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_SearchUsersByImage.html\">SearchUsersByImage</a> </p> </li> </ul> <p> <b>Amazon Rekognition Custom Labels</b> </p> <ul> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_CopyProjectVersion.html\">CopyProjectVersion</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_CreateDataset.html\">CreateDataset</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_CreateProject.html\">CreateProject</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_CreateProjectVersion.html\">CreateProjectVersion</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DeleteDataset.html\">DeleteDataset</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DeleteProject.html\">DeleteProject</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DeleteProjectPolicy.html\">DeleteProjectPolicy</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DeleteProjectVersion.html\">DeleteProjectVersion</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DescribeDataset.html\">DescribeDataset</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DescribeProjects.html\">DescribeProjects</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DescribeProjectVersions.html\">DescribeProjectVersions</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DetectCustomLabels.html\">DetectCustomLabels</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DistributeDatasetEntries.html\">DistributeDatasetEntries</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_ListDatasetEntries.html\">ListDatasetEntries</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_ListDatasetLabels.html\">ListDatasetLabels</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_ListProjectPolicies.html\">ListProjectPolicies</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_PutProjectPolicy.html\">PutProjectPolicy</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_StartProjectVersion.html\">StartProjectVersion</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_StopProjectVersion.html\">StopProjectVersion</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_UpdateDatasetEntries.html\">UpdateDatasetEntries</a> </p> </li> </ul> <p> <b>Amazon Rekognition Video Stored Video</b> </p> <ul> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_GetCelebrityRecognition.html\">GetCelebrityRecognition</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_GetContentModeration.html\">GetContentModeration</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_GetFaceDetection.html\">GetFaceDetection</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_GetFaceSearch.html\">GetFaceSearch</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_GetLabelDetection.html\">GetLabelDetection</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_GetPersonTracking.html\">GetPersonTracking</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_GetSegmentDetection.html\">GetSegmentDetection</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_GetTextDetection.html\">GetTextDetection</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_StartCelebrityRecognition.html\">StartCelebrityRecognition</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_StartContentModeration.html\">StartContentModeration</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_StartFaceDetection.html\">StartFaceDetection</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_StartFaceSearch.html\">StartFaceSearch</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_StartLabelDetection.html\">StartLabelDetection</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_StartPersonTracking.html\">StartPersonTracking</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_StartSegmentDetection.html\">StartSegmentDetection</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_StartTextDetection.html\">StartTextDetection</a> </p> </li> </ul> <p> <b>Amazon Rekognition Video Streaming Video</b> </p> <ul> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_CreateStreamProcessor.html\">CreateStreamProcessor</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DeleteStreamProcessor.html\">DeleteStreamProcessor</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_DescribeStreamProcessor.html\">DescribeStreamProcessor</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_ListStreamProcessors.html\">ListStreamProcessors</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_StartStreamProcessor.html\">StartStreamProcessor</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_StopStreamProcessor.html\">StopStreamProcessor</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/rekognition/latest/APIReference/API_UpdateStreamProcessor.html\">UpdateStreamProcessor</a> </p> </li> </ul>
 *
 * The version of the OpenAPI document: 2016-06-27
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAICelebrityDetail_Face.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAICelebrityDetail_Face::OAICelebrityDetail_Face(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAICelebrityDetail_Face::OAICelebrityDetail_Face() {
    this->initializeModel();
}

OAICelebrityDetail_Face::~OAICelebrityDetail_Face() {}

void OAICelebrityDetail_Face::initializeModel() {

    m_bounding_box_isSet = false;
    m_bounding_box_isValid = false;

    m_age_range_isSet = false;
    m_age_range_isValid = false;

    m_smile_isSet = false;
    m_smile_isValid = false;

    m_eyeglasses_isSet = false;
    m_eyeglasses_isValid = false;

    m_sunglasses_isSet = false;
    m_sunglasses_isValid = false;

    m_gender_isSet = false;
    m_gender_isValid = false;

    m_beard_isSet = false;
    m_beard_isValid = false;

    m_mustache_isSet = false;
    m_mustache_isValid = false;

    m_eyes_open_isSet = false;
    m_eyes_open_isValid = false;

    m_mouth_open_isSet = false;
    m_mouth_open_isValid = false;

    m_emotions_isSet = false;
    m_emotions_isValid = false;

    m_landmarks_isSet = false;
    m_landmarks_isValid = false;

    m_pose_isSet = false;
    m_pose_isValid = false;

    m_quality_isSet = false;
    m_quality_isValid = false;

    m_confidence_isSet = false;
    m_confidence_isValid = false;

    m_face_occluded_isSet = false;
    m_face_occluded_isValid = false;

    m_eye_direction_isSet = false;
    m_eye_direction_isValid = false;
}

void OAICelebrityDetail_Face::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAICelebrityDetail_Face::fromJsonObject(QJsonObject json) {

    m_bounding_box_isValid = ::OpenAPI::fromJsonValue(m_bounding_box, json[QString("BoundingBox")]);
    m_bounding_box_isSet = !json[QString("BoundingBox")].isNull() && m_bounding_box_isValid;

    m_age_range_isValid = ::OpenAPI::fromJsonValue(m_age_range, json[QString("AgeRange")]);
    m_age_range_isSet = !json[QString("AgeRange")].isNull() && m_age_range_isValid;

    m_smile_isValid = ::OpenAPI::fromJsonValue(m_smile, json[QString("Smile")]);
    m_smile_isSet = !json[QString("Smile")].isNull() && m_smile_isValid;

    m_eyeglasses_isValid = ::OpenAPI::fromJsonValue(m_eyeglasses, json[QString("Eyeglasses")]);
    m_eyeglasses_isSet = !json[QString("Eyeglasses")].isNull() && m_eyeglasses_isValid;

    m_sunglasses_isValid = ::OpenAPI::fromJsonValue(m_sunglasses, json[QString("Sunglasses")]);
    m_sunglasses_isSet = !json[QString("Sunglasses")].isNull() && m_sunglasses_isValid;

    m_gender_isValid = ::OpenAPI::fromJsonValue(m_gender, json[QString("Gender")]);
    m_gender_isSet = !json[QString("Gender")].isNull() && m_gender_isValid;

    m_beard_isValid = ::OpenAPI::fromJsonValue(m_beard, json[QString("Beard")]);
    m_beard_isSet = !json[QString("Beard")].isNull() && m_beard_isValid;

    m_mustache_isValid = ::OpenAPI::fromJsonValue(m_mustache, json[QString("Mustache")]);
    m_mustache_isSet = !json[QString("Mustache")].isNull() && m_mustache_isValid;

    m_eyes_open_isValid = ::OpenAPI::fromJsonValue(m_eyes_open, json[QString("EyesOpen")]);
    m_eyes_open_isSet = !json[QString("EyesOpen")].isNull() && m_eyes_open_isValid;

    m_mouth_open_isValid = ::OpenAPI::fromJsonValue(m_mouth_open, json[QString("MouthOpen")]);
    m_mouth_open_isSet = !json[QString("MouthOpen")].isNull() && m_mouth_open_isValid;

    m_emotions_isValid = ::OpenAPI::fromJsonValue(m_emotions, json[QString("Emotions")]);
    m_emotions_isSet = !json[QString("Emotions")].isNull() && m_emotions_isValid;

    m_landmarks_isValid = ::OpenAPI::fromJsonValue(m_landmarks, json[QString("Landmarks")]);
    m_landmarks_isSet = !json[QString("Landmarks")].isNull() && m_landmarks_isValid;

    m_pose_isValid = ::OpenAPI::fromJsonValue(m_pose, json[QString("Pose")]);
    m_pose_isSet = !json[QString("Pose")].isNull() && m_pose_isValid;

    m_quality_isValid = ::OpenAPI::fromJsonValue(m_quality, json[QString("Quality")]);
    m_quality_isSet = !json[QString("Quality")].isNull() && m_quality_isValid;

    m_confidence_isValid = ::OpenAPI::fromJsonValue(m_confidence, json[QString("Confidence")]);
    m_confidence_isSet = !json[QString("Confidence")].isNull() && m_confidence_isValid;

    m_face_occluded_isValid = ::OpenAPI::fromJsonValue(m_face_occluded, json[QString("FaceOccluded")]);
    m_face_occluded_isSet = !json[QString("FaceOccluded")].isNull() && m_face_occluded_isValid;

    m_eye_direction_isValid = ::OpenAPI::fromJsonValue(m_eye_direction, json[QString("EyeDirection")]);
    m_eye_direction_isSet = !json[QString("EyeDirection")].isNull() && m_eye_direction_isValid;
}

QString OAICelebrityDetail_Face::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAICelebrityDetail_Face::asJsonObject() const {
    QJsonObject obj;
    if (m_bounding_box.isSet()) {
        obj.insert(QString("BoundingBox"), ::OpenAPI::toJsonValue(m_bounding_box));
    }
    if (m_age_range.isSet()) {
        obj.insert(QString("AgeRange"), ::OpenAPI::toJsonValue(m_age_range));
    }
    if (m_smile.isSet()) {
        obj.insert(QString("Smile"), ::OpenAPI::toJsonValue(m_smile));
    }
    if (m_eyeglasses.isSet()) {
        obj.insert(QString("Eyeglasses"), ::OpenAPI::toJsonValue(m_eyeglasses));
    }
    if (m_sunglasses.isSet()) {
        obj.insert(QString("Sunglasses"), ::OpenAPI::toJsonValue(m_sunglasses));
    }
    if (m_gender.isSet()) {
        obj.insert(QString("Gender"), ::OpenAPI::toJsonValue(m_gender));
    }
    if (m_beard.isSet()) {
        obj.insert(QString("Beard"), ::OpenAPI::toJsonValue(m_beard));
    }
    if (m_mustache.isSet()) {
        obj.insert(QString("Mustache"), ::OpenAPI::toJsonValue(m_mustache));
    }
    if (m_eyes_open.isSet()) {
        obj.insert(QString("EyesOpen"), ::OpenAPI::toJsonValue(m_eyes_open));
    }
    if (m_mouth_open.isSet()) {
        obj.insert(QString("MouthOpen"), ::OpenAPI::toJsonValue(m_mouth_open));
    }
    if (m_emotions.isSet()) {
        obj.insert(QString("Emotions"), ::OpenAPI::toJsonValue(m_emotions));
    }
    if (m_landmarks.isSet()) {
        obj.insert(QString("Landmarks"), ::OpenAPI::toJsonValue(m_landmarks));
    }
    if (m_pose.isSet()) {
        obj.insert(QString("Pose"), ::OpenAPI::toJsonValue(m_pose));
    }
    if (m_quality.isSet()) {
        obj.insert(QString("Quality"), ::OpenAPI::toJsonValue(m_quality));
    }
    if (m_confidence_isSet) {
        obj.insert(QString("Confidence"), ::OpenAPI::toJsonValue(m_confidence));
    }
    if (m_face_occluded.isSet()) {
        obj.insert(QString("FaceOccluded"), ::OpenAPI::toJsonValue(m_face_occluded));
    }
    if (m_eye_direction.isSet()) {
        obj.insert(QString("EyeDirection"), ::OpenAPI::toJsonValue(m_eye_direction));
    }
    return obj;
}

OAIFaceDetail_BoundingBox OAICelebrityDetail_Face::getBoundingBox() const {
    return m_bounding_box;
}
void OAICelebrityDetail_Face::setBoundingBox(const OAIFaceDetail_BoundingBox &bounding_box) {
    m_bounding_box = bounding_box;
    m_bounding_box_isSet = true;
}

bool OAICelebrityDetail_Face::is_bounding_box_Set() const{
    return m_bounding_box_isSet;
}

bool OAICelebrityDetail_Face::is_bounding_box_Valid() const{
    return m_bounding_box_isValid;
}

OAIFaceDetail_AgeRange OAICelebrityDetail_Face::getAgeRange() const {
    return m_age_range;
}
void OAICelebrityDetail_Face::setAgeRange(const OAIFaceDetail_AgeRange &age_range) {
    m_age_range = age_range;
    m_age_range_isSet = true;
}

bool OAICelebrityDetail_Face::is_age_range_Set() const{
    return m_age_range_isSet;
}

bool OAICelebrityDetail_Face::is_age_range_Valid() const{
    return m_age_range_isValid;
}

OAIFaceDetail_Smile OAICelebrityDetail_Face::getSmile() const {
    return m_smile;
}
void OAICelebrityDetail_Face::setSmile(const OAIFaceDetail_Smile &smile) {
    m_smile = smile;
    m_smile_isSet = true;
}

bool OAICelebrityDetail_Face::is_smile_Set() const{
    return m_smile_isSet;
}

bool OAICelebrityDetail_Face::is_smile_Valid() const{
    return m_smile_isValid;
}

OAIFaceDetail_Eyeglasses OAICelebrityDetail_Face::getEyeglasses() const {
    return m_eyeglasses;
}
void OAICelebrityDetail_Face::setEyeglasses(const OAIFaceDetail_Eyeglasses &eyeglasses) {
    m_eyeglasses = eyeglasses;
    m_eyeglasses_isSet = true;
}

bool OAICelebrityDetail_Face::is_eyeglasses_Set() const{
    return m_eyeglasses_isSet;
}

bool OAICelebrityDetail_Face::is_eyeglasses_Valid() const{
    return m_eyeglasses_isValid;
}

OAIFaceDetail_Sunglasses OAICelebrityDetail_Face::getSunglasses() const {
    return m_sunglasses;
}
void OAICelebrityDetail_Face::setSunglasses(const OAIFaceDetail_Sunglasses &sunglasses) {
    m_sunglasses = sunglasses;
    m_sunglasses_isSet = true;
}

bool OAICelebrityDetail_Face::is_sunglasses_Set() const{
    return m_sunglasses_isSet;
}

bool OAICelebrityDetail_Face::is_sunglasses_Valid() const{
    return m_sunglasses_isValid;
}

OAIFaceDetail_Gender OAICelebrityDetail_Face::getGender() const {
    return m_gender;
}
void OAICelebrityDetail_Face::setGender(const OAIFaceDetail_Gender &gender) {
    m_gender = gender;
    m_gender_isSet = true;
}

bool OAICelebrityDetail_Face::is_gender_Set() const{
    return m_gender_isSet;
}

bool OAICelebrityDetail_Face::is_gender_Valid() const{
    return m_gender_isValid;
}

OAIFaceDetail_Beard OAICelebrityDetail_Face::getBeard() const {
    return m_beard;
}
void OAICelebrityDetail_Face::setBeard(const OAIFaceDetail_Beard &beard) {
    m_beard = beard;
    m_beard_isSet = true;
}

bool OAICelebrityDetail_Face::is_beard_Set() const{
    return m_beard_isSet;
}

bool OAICelebrityDetail_Face::is_beard_Valid() const{
    return m_beard_isValid;
}

OAIFaceDetail_Mustache OAICelebrityDetail_Face::getMustache() const {
    return m_mustache;
}
void OAICelebrityDetail_Face::setMustache(const OAIFaceDetail_Mustache &mustache) {
    m_mustache = mustache;
    m_mustache_isSet = true;
}

bool OAICelebrityDetail_Face::is_mustache_Set() const{
    return m_mustache_isSet;
}

bool OAICelebrityDetail_Face::is_mustache_Valid() const{
    return m_mustache_isValid;
}

OAIFaceDetail_EyesOpen OAICelebrityDetail_Face::getEyesOpen() const {
    return m_eyes_open;
}
void OAICelebrityDetail_Face::setEyesOpen(const OAIFaceDetail_EyesOpen &eyes_open) {
    m_eyes_open = eyes_open;
    m_eyes_open_isSet = true;
}

bool OAICelebrityDetail_Face::is_eyes_open_Set() const{
    return m_eyes_open_isSet;
}

bool OAICelebrityDetail_Face::is_eyes_open_Valid() const{
    return m_eyes_open_isValid;
}

OAIFaceDetail_MouthOpen OAICelebrityDetail_Face::getMouthOpen() const {
    return m_mouth_open;
}
void OAICelebrityDetail_Face::setMouthOpen(const OAIFaceDetail_MouthOpen &mouth_open) {
    m_mouth_open = mouth_open;
    m_mouth_open_isSet = true;
}

bool OAICelebrityDetail_Face::is_mouth_open_Set() const{
    return m_mouth_open_isSet;
}

bool OAICelebrityDetail_Face::is_mouth_open_Valid() const{
    return m_mouth_open_isValid;
}

QList OAICelebrityDetail_Face::getEmotions() const {
    return m_emotions;
}
void OAICelebrityDetail_Face::setEmotions(const QList &emotions) {
    m_emotions = emotions;
    m_emotions_isSet = true;
}

bool OAICelebrityDetail_Face::is_emotions_Set() const{
    return m_emotions_isSet;
}

bool OAICelebrityDetail_Face::is_emotions_Valid() const{
    return m_emotions_isValid;
}

QList OAICelebrityDetail_Face::getLandmarks() const {
    return m_landmarks;
}
void OAICelebrityDetail_Face::setLandmarks(const QList &landmarks) {
    m_landmarks = landmarks;
    m_landmarks_isSet = true;
}

bool OAICelebrityDetail_Face::is_landmarks_Set() const{
    return m_landmarks_isSet;
}

bool OAICelebrityDetail_Face::is_landmarks_Valid() const{
    return m_landmarks_isValid;
}

OAIFaceDetail_Pose OAICelebrityDetail_Face::getPose() const {
    return m_pose;
}
void OAICelebrityDetail_Face::setPose(const OAIFaceDetail_Pose &pose) {
    m_pose = pose;
    m_pose_isSet = true;
}

bool OAICelebrityDetail_Face::is_pose_Set() const{
    return m_pose_isSet;
}

bool OAICelebrityDetail_Face::is_pose_Valid() const{
    return m_pose_isValid;
}

OAIFaceDetail_Quality OAICelebrityDetail_Face::getQuality() const {
    return m_quality;
}
void OAICelebrityDetail_Face::setQuality(const OAIFaceDetail_Quality &quality) {
    m_quality = quality;
    m_quality_isSet = true;
}

bool OAICelebrityDetail_Face::is_quality_Set() const{
    return m_quality_isSet;
}

bool OAICelebrityDetail_Face::is_quality_Valid() const{
    return m_quality_isValid;
}

float OAICelebrityDetail_Face::getConfidence() const {
    return m_confidence;
}
void OAICelebrityDetail_Face::setConfidence(const float &confidence) {
    m_confidence = confidence;
    m_confidence_isSet = true;
}

bool OAICelebrityDetail_Face::is_confidence_Set() const{
    return m_confidence_isSet;
}

bool OAICelebrityDetail_Face::is_confidence_Valid() const{
    return m_confidence_isValid;
}

OAIFaceDetail_FaceOccluded OAICelebrityDetail_Face::getFaceOccluded() const {
    return m_face_occluded;
}
void OAICelebrityDetail_Face::setFaceOccluded(const OAIFaceDetail_FaceOccluded &face_occluded) {
    m_face_occluded = face_occluded;
    m_face_occluded_isSet = true;
}

bool OAICelebrityDetail_Face::is_face_occluded_Set() const{
    return m_face_occluded_isSet;
}

bool OAICelebrityDetail_Face::is_face_occluded_Valid() const{
    return m_face_occluded_isValid;
}

OAIFaceDetail_EyeDirection OAICelebrityDetail_Face::getEyeDirection() const {
    return m_eye_direction;
}
void OAICelebrityDetail_Face::setEyeDirection(const OAIFaceDetail_EyeDirection &eye_direction) {
    m_eye_direction = eye_direction;
    m_eye_direction_isSet = true;
}

bool OAICelebrityDetail_Face::is_eye_direction_Set() const{
    return m_eye_direction_isSet;
}

bool OAICelebrityDetail_Face::is_eye_direction_Valid() const{
    return m_eye_direction_isValid;
}

bool OAICelebrityDetail_Face::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_bounding_box.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_age_range.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_smile.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_eyeglasses.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_sunglasses.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_gender.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_beard.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_mustache.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_eyes_open.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_mouth_open.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_emotions.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_landmarks.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_pose.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_quality.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_confidence_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_face_occluded.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_eye_direction.isSet()) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAICelebrityDetail_Face::isValid() const {
    // only required properties are required for the object to be considered valid
    return true;
}

} // namespace OpenAPI
