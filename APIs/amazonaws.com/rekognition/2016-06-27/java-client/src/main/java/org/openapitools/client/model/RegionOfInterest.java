...
import org.openapitools.client.model.RegionOfInterestBoundingBox;
...
public class RegionOfInterest {
  ...
  private RegionOfInterestBoundingBox boundingBox;
  ...
  public RegionOfInterest boundingBox(RegionOfInterestBoundingBox boundingBox) {
  ...
  public RegionOfInterestBoundingBox getBoundingBox() {
    return boundingBox;
  }

  public void setBoundingBox(RegionOfInterestBoundingBox boundingBox) {
    this.boundingBox = boundingBox;
  }


  public RegionOfInterest polygon(List polygon) {
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(this.boundingBox, regionOfInterest.boundingBox) &&
        Objects.equals(this.polygon, regionOfInterest.polygon);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to RegionOfInterest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!RegionOfInterest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in RegionOfInterest is not found in the empty JSON string", RegionOfInterest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!RegionOfInterest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `RegionOfInterest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // validate the optional field `BoundingBox`
      if (jsonObj.get("BoundingBox") != null && !jsonObj.get("BoundingBox").isJsonNull()) {
        RegionOfInterestBoundingBox.validateJsonElement(jsonObj.get("BoundingBox"));
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!RegionOfInterest.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'RegionOfInterest' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<RegionOfInterest> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(RegionOfInterest.class));

       return (TypeAdapter<T>) new TypeAdapter<RegionOfInterest>() {
           @Override
           public void write(JsonWriter out, RegionOfInterest value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public RegionOfInterest read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of RegionOfInterest given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of RegionOfInterest
   * @throws IOException if the JSON string is invalid with respect to RegionOfInterest
   */
  public static RegionOfInterest fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, RegionOfInterest.class);
  }

  /**
   * Convert an instance of RegionOfInterest to an JSON string
   *
   * @return JSON string
   */
