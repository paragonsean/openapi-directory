...
public class StreamProcessorDataSharingPreference {
  ...
  public StreamProcessorDataSharingPreference() {
  }

  public StreamProcessorDataSharingPreference optIn(Boolean optIn) {
    this.optIn = optIn;
    return this;
  }
  ...
  public boolean equals(Object o) {
    ...
    StreamProcessorDataSharingPreference streamProcessorDataSharingPreference = (StreamProcessorDataSharingPreference) o;
    return Objects.equals(this.optIn, streamProcessorDataSharingPreference.optIn);
  }
  ...
  public String toString() {
    ...
    sb.append("class StreamProcessorDataSharingPreference {\n");
   ...
   * @throws IOException if the JSON Element is invalid with respect to StreamProcessorDataSharingPreference
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!StreamProcessorDataSharingPreference.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in StreamProcessorDataSharingPreference is not found in the empty JSON string", StreamProcessorDataSharingPreference.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!StreamProcessorDataSharingPreference.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `StreamProcessorDataSharingPreference` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : StreamProcessorDataSharingPreference.openapiRequiredFields) {
        if (jsonElement.getAsJsonObject().get(requiredField) == null) {
          throw new IllegalArgumentException(String.format("The required field `%s` is not found in the JSON string: %s", requiredField, jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // validate the required field `OptIn`
      Boolean.validateJsonElement(jsonObj.get("OptIn"));
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!StreamProcessorDataSharingPreference.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'StreamProcessorDataSharingPreference' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<StreamProcessorDataSharingPreference> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(StreamProcessorDataSharingPreference.class));

       return (TypeAdapter<T>) new TypeAdapter<StreamProcessorDataSharingPreference>() {
           @Override
           public void write(JsonWriter out, StreamProcessorDataSharingPreference value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public StreamProcessorDataSharingPreference read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of StreamProcessorDataSharingPreference given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of StreamProcessorDataSharingPreference
   * @throws IOException if the JSON string is invalid with respect to StreamProcessorDataSharingPreference
   */
  public static StreamProcessorDataSharingPreference fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, StreamProcessorDataSharingPreference.class);
  }

  /**
   * Convert an instance of StreamProcessorDataSharingPreference to an JSON string
   *
   * @return JSON string
   */
