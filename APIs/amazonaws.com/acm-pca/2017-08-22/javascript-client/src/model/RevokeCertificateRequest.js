/**
 * AWS Certificate Manager Private Certificate Authority
 * <p>This is the <i>Amazon Web Services Private Certificate Authority API Reference</i>. It provides descriptions, syntax, and usage examples for each of the actions and data types involved in creating and managing a private certificate authority (CA) for your organization.</p> <p>The documentation for each action shows the API request parameters and the JSON response. Alternatively, you can use one of the Amazon Web Services SDKs to access an API that is tailored to the programming language or platform that you prefer. For more information, see <a href=\"https://aws.amazon.com/tools/#SDKs\">Amazon Web Services SDKs</a>.</p> <p>Each Amazon Web Services Private CA API operation has a quota that determines the number of times the operation can be called per second. Amazon Web Services Private CA throttles API requests at different rates depending on the operation. Throttling means that Amazon Web Services Private CA rejects an otherwise valid request because the request exceeds the operation's quota for the number of requests per second. When a request is throttled, Amazon Web Services Private CA returns a <a href=\"https://docs.aws.amazon.com/privateca/latest/APIReference/CommonErrors.html\">ThrottlingException</a> error. Amazon Web Services Private CA does not guarantee a minimum request rate for APIs. </p> <p>To see an up-to-date list of your Amazon Web Services Private CA quotas, or to request a quota increase, log into your Amazon Web Services account and visit the <a href=\"https://console.aws.amazon.com/servicequotas/\">Service Quotas</a> console.</p>
 *
 * The version of the OpenAPI document: 2017-08-22
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import RevocationReason from './RevocationReason';

/**
 * The RevokeCertificateRequest model module.
 * @module model/RevokeCertificateRequest
 * @version 2017-08-22
 */
class RevokeCertificateRequest {
    /**
     * Constructs a new <code>RevokeCertificateRequest</code>.
     * @alias module:model/RevokeCertificateRequest
     * @param certificateAuthorityArn {String} 
     * @param certificateSerial {String} 
     * @param revocationReason {module:model/RevocationReason} 
     */
    constructor(certificateAuthorityArn, certificateSerial, revocationReason) { 
        
        RevokeCertificateRequest.initialize(this, certificateAuthorityArn, certificateSerial, revocationReason);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj, certificateAuthorityArn, certificateSerial, revocationReason) { 
        obj['CertificateAuthorityArn'] = certificateAuthorityArn;
        obj['CertificateSerial'] = certificateSerial;
        obj['RevocationReason'] = revocationReason;
    }

    /**
     * Constructs a <code>RevokeCertificateRequest</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/RevokeCertificateRequest} obj Optional instance to populate.
     * @return {module:model/RevokeCertificateRequest} The populated <code>RevokeCertificateRequest</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new RevokeCertificateRequest();

            if (data.hasOwnProperty('CertificateAuthorityArn')) {
                obj['CertificateAuthorityArn'] = ApiClient.convertToType(data['CertificateAuthorityArn'], 'String');
            }
            if (data.hasOwnProperty('CertificateSerial')) {
                obj['CertificateSerial'] = ApiClient.convertToType(data['CertificateSerial'], 'String');
            }
            if (data.hasOwnProperty('RevocationReason')) {
                obj['RevocationReason'] = ApiClient.convertToType(data['RevocationReason'], RevocationReason);
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>RevokeCertificateRequest</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>RevokeCertificateRequest</code>.
     */
    static validateJSON(data) {
        // check to make sure all required properties are present in the JSON string
        for (const property of RevokeCertificateRequest.RequiredProperties) {
            if (!data.hasOwnProperty(property)) {
                throw new Error("The required field `" + property + "` is not found in the JSON data: " + JSON.stringify(data));
            }
        }
        // validate the optional field `CertificateAuthorityArn`
        if (data['CertificateAuthorityArn']) { // data not null
          String.validateJSON(data['CertificateAuthorityArn']);
        }
        // validate the optional field `CertificateSerial`
        if (data['CertificateSerial']) { // data not null
          String.validateJSON(data['CertificateSerial']);
        }
        // validate the optional field `RevocationReason`
        if (data['RevocationReason']) { // data not null
          RevocationReason.validateJSON(data['RevocationReason']);
        }

        return true;
    }


}

RevokeCertificateRequest.RequiredProperties = ["CertificateAuthorityArn", "CertificateSerial", "RevocationReason"];

/**
 * @member {String} CertificateAuthorityArn
 */
RevokeCertificateRequest.prototype['CertificateAuthorityArn'] = undefined;

/**
 * @member {String} CertificateSerial
 */
RevokeCertificateRequest.prototype['CertificateSerial'] = undefined;

/**
 * @member {module:model/RevocationReason} RevocationReason
 */
RevokeCertificateRequest.prototype['RevocationReason'] = undefined;






export default RevokeCertificateRequest;

