/**
 * AWS RoboMaker
 * This section provides documentation for the AWS RoboMaker API operations.
 *
 * The version of the OpenAPI document: 2018-06-29
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import Filter from './Filter';

/**
 * The ListRobotApplicationsRequest model module.
 * @module model/ListRobotApplicationsRequest
 * @version 2018-06-29
 */
class ListRobotApplicationsRequest {
    /**
     * Constructs a new <code>ListRobotApplicationsRequest</code>.
     * @alias module:model/ListRobotApplicationsRequest
     */
    constructor() { 
        
        ListRobotApplicationsRequest.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>ListRobotApplicationsRequest</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/ListRobotApplicationsRequest} obj Optional instance to populate.
     * @return {module:model/ListRobotApplicationsRequest} The populated <code>ListRobotApplicationsRequest</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new ListRobotApplicationsRequest();

            if (data.hasOwnProperty('versionQualifier')) {
                obj['versionQualifier'] = ApiClient.convertToType(data['versionQualifier'], 'String');
            }
            if (data.hasOwnProperty('nextToken')) {
                obj['nextToken'] = ApiClient.convertToType(data['nextToken'], 'String');
            }
            if (data.hasOwnProperty('maxResults')) {
                obj['maxResults'] = ApiClient.convertToType(data['maxResults'], 'Number');
            }
            if (data.hasOwnProperty('filters')) {
                obj['filters'] = ApiClient.convertToType(data['filters'], [Filter]);
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>ListRobotApplicationsRequest</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>ListRobotApplicationsRequest</code>.
     */
    static validateJSON(data) {
        // ensure the json data is a string
        if (data['versionQualifier'] && !(typeof data['versionQualifier'] === 'string' || data['versionQualifier'] instanceof String)) {
            throw new Error("Expected the field `versionQualifier` to be a primitive type in the JSON string but got " + data['versionQualifier']);
        }
        // ensure the json data is a string
        if (data['nextToken'] && !(typeof data['nextToken'] === 'string' || data['nextToken'] instanceof String)) {
            throw new Error("Expected the field `nextToken` to be a primitive type in the JSON string but got " + data['nextToken']);
        }
        if (data['filters']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['filters'])) {
                throw new Error("Expected the field `filters` to be an array in the JSON data but got " + data['filters']);
            }
            // validate the optional field `filters` (array)
            for (const item of data['filters']) {
                Filter.validateJSON(item);
            };
        }

        return true;
    }


}



/**
 * The version qualifier of the robot application.
 * @member {String} versionQualifier
 */
ListRobotApplicationsRequest.prototype['versionQualifier'] = undefined;

/**
 * If the previous paginated request did not return all of the remaining results, the response object's <code>nextToken</code> parameter value is set to a token. To retrieve the next set of results, call <code>ListRobotApplications</code> again and assign that token to the request object's <code>nextToken</code> parameter. If there are no remaining results, the previous response object's NextToken parameter is set to null. 
 * @member {String} nextToken
 */
ListRobotApplicationsRequest.prototype['nextToken'] = undefined;

/**
 * When this parameter is used, <code>ListRobotApplications</code> only returns <code>maxResults</code> results in a single page along with a <code>nextToken</code> response element. The remaining results of the initial request can be seen by sending another <code>ListRobotApplications</code> request with the returned <code>nextToken</code> value. This value can be between 1 and 100. If this parameter is not used, then <code>ListRobotApplications</code> returns up to 100 results and a <code>nextToken</code> value if applicable. 
 * @member {Number} maxResults
 */
ListRobotApplicationsRequest.prototype['maxResults'] = undefined;

/**
 * <p>Optional filters to limit results.</p> <p>The filter name <code>name</code> is supported. When filtering, you must use the complete value of the filtered item. You can use up to three filters.</p>
 * @member {Array.<module:model/Filter>} filters
 */
ListRobotApplicationsRequest.prototype['filters'] = undefined;






export default ListRobotApplicationsRequest;

