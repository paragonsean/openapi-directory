/**
 * AWS License Manager
 * License Manager makes it easier to manage licenses from software vendors across multiple Amazon Web Services accounts and on-premises servers.
 *
 * The version of the OpenAPI document: 2018-08-01
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIGrantedLicense.h
 *
 * Describes a license that is granted to a grantee.
 */

#ifndef OAIGrantedLicense_H
#define OAIGrantedLicense_H

#include <QJsonObject>

#include "OAIGrantedLicense_Issuer.h"
#include "OAIGrantedLicense_ReceivedMetadata.h"
#include "OAIGrantedLicense_Validity.h"
#include "OAILicenseStatus.h"
#include "OAILicense_ConsumptionConfiguration.h"
#include <QList>
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {
class OAIGrantedLicense_Issuer;
class OAIGrantedLicense_Validity;
class OAILicense_ConsumptionConfiguration;
class OAIGrantedLicense_ReceivedMetadata;

class OAIGrantedLicense : public OAIObject {
public:
    OAIGrantedLicense();
    OAIGrantedLicense(QString json);
    ~OAIGrantedLicense() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QString getLicenseArn() const;
    void setLicenseArn(const QString &license_arn);
    bool is_license_arn_Set() const;
    bool is_license_arn_Valid() const;

    QString getLicenseName() const;
    void setLicenseName(const QString &license_name);
    bool is_license_name_Set() const;
    bool is_license_name_Valid() const;

    QString getProductName() const;
    void setProductName(const QString &product_name);
    bool is_product_name_Set() const;
    bool is_product_name_Valid() const;

    QString getProductSku() const;
    void setProductSku(const QString &product_sku);
    bool is_product_sku_Set() const;
    bool is_product_sku_Valid() const;

    OAIGrantedLicense_Issuer getIssuer() const;
    void setIssuer(const OAIGrantedLicense_Issuer &issuer);
    bool is_issuer_Set() const;
    bool is_issuer_Valid() const;

    QString getHomeRegion() const;
    void setHomeRegion(const QString &home_region);
    bool is_home_region_Set() const;
    bool is_home_region_Valid() const;

    OAILicenseStatus getStatus() const;
    void setStatus(const OAILicenseStatus &status);
    bool is_status_Set() const;
    bool is_status_Valid() const;

    OAIGrantedLicense_Validity getValidity() const;
    void setValidity(const OAIGrantedLicense_Validity &validity);
    bool is_validity_Set() const;
    bool is_validity_Valid() const;

    QString getBeneficiary() const;
    void setBeneficiary(const QString &beneficiary);
    bool is_beneficiary_Set() const;
    bool is_beneficiary_Valid() const;

    QList getEntitlements() const;
    void setEntitlements(const QList &entitlements);
    bool is_entitlements_Set() const;
    bool is_entitlements_Valid() const;

    OAILicense_ConsumptionConfiguration getConsumptionConfiguration() const;
    void setConsumptionConfiguration(const OAILicense_ConsumptionConfiguration &consumption_configuration);
    bool is_consumption_configuration_Set() const;
    bool is_consumption_configuration_Valid() const;

    QList getLicenseMetadata() const;
    void setLicenseMetadata(const QList &license_metadata);
    bool is_license_metadata_Set() const;
    bool is_license_metadata_Valid() const;

    QString getCreateTime() const;
    void setCreateTime(const QString &create_time);
    bool is_create_time_Set() const;
    bool is_create_time_Valid() const;

    QString getVersion() const;
    void setVersion(const QString &version);
    bool is_version_Set() const;
    bool is_version_Valid() const;

    OAIGrantedLicense_ReceivedMetadata getReceivedMetadata() const;
    void setReceivedMetadata(const OAIGrantedLicense_ReceivedMetadata &received_metadata);
    bool is_received_metadata_Set() const;
    bool is_received_metadata_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString m_license_arn;
    bool m_license_arn_isSet;
    bool m_license_arn_isValid;

    QString m_license_name;
    bool m_license_name_isSet;
    bool m_license_name_isValid;

    QString m_product_name;
    bool m_product_name_isSet;
    bool m_product_name_isValid;

    QString m_product_sku;
    bool m_product_sku_isSet;
    bool m_product_sku_isValid;

    OAIGrantedLicense_Issuer m_issuer;
    bool m_issuer_isSet;
    bool m_issuer_isValid;

    QString m_home_region;
    bool m_home_region_isSet;
    bool m_home_region_isValid;

    OAILicenseStatus m_status;
    bool m_status_isSet;
    bool m_status_isValid;

    OAIGrantedLicense_Validity m_validity;
    bool m_validity_isSet;
    bool m_validity_isValid;

    QString m_beneficiary;
    bool m_beneficiary_isSet;
    bool m_beneficiary_isValid;

    QList m_entitlements;
    bool m_entitlements_isSet;
    bool m_entitlements_isValid;

    OAILicense_ConsumptionConfiguration m_consumption_configuration;
    bool m_consumption_configuration_isSet;
    bool m_consumption_configuration_isValid;

    QList m_license_metadata;
    bool m_license_metadata_isSet;
    bool m_license_metadata_isValid;

    QString m_create_time;
    bool m_create_time_isSet;
    bool m_create_time_isValid;

    QString m_version;
    bool m_version_isSet;
    bool m_version_isValid;

    OAIGrantedLicense_ReceivedMetadata m_received_metadata;
    bool m_received_metadata_isSet;
    bool m_received_metadata_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIGrantedLicense)

#endif // OAIGrantedLicense_H
