/**
 * Amazon CloudSearch
 * <fullname>Amazon CloudSearch Configuration Service</fullname> <p>You use the Amazon CloudSearch configuration service to create, configure, and manage search domains. Configuration service requests are submitted using the AWS Query protocol. AWS Query requests are HTTP or HTTPS requests submitted via HTTP GET or POST with a query parameter named Action.</p> <p>The endpoint for configuration service requests is region-specific: cloudsearch.<i>region</i>.amazonaws.com. For example, cloudsearch.us-east-1.amazonaws.com. For a current list of supported regions and endpoints, see <a href=\"http://docs.aws.amazon.com/general/latest/gr/rande.html#cloudsearch_region\" target=\"_blank\">Regions and Endpoints</a>.</p>
 *
 * The version of the OpenAPI document: 2013-01-01
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAITextArrayOptions.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAITextArrayOptions::OAITextArrayOptions(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAITextArrayOptions::OAITextArrayOptions() {
    this->initializeModel();
}

OAITextArrayOptions::~OAITextArrayOptions() {}

void OAITextArrayOptions::initializeModel() {

    m_default_value_isSet = false;
    m_default_value_isValid = false;

    m_source_fields_isSet = false;
    m_source_fields_isValid = false;

    m_return_enabled_isSet = false;
    m_return_enabled_isValid = false;

    m_highlight_enabled_isSet = false;
    m_highlight_enabled_isValid = false;

    m_analysis_scheme_isSet = false;
    m_analysis_scheme_isValid = false;
}

void OAITextArrayOptions::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAITextArrayOptions::fromJsonObject(QJsonObject json) {

    m_default_value_isValid = ::OpenAPI::fromJsonValue(m_default_value, json[QString("DefaultValue")]);
    m_default_value_isSet = !json[QString("DefaultValue")].isNull() && m_default_value_isValid;

    m_source_fields_isValid = ::OpenAPI::fromJsonValue(m_source_fields, json[QString("SourceFields")]);
    m_source_fields_isSet = !json[QString("SourceFields")].isNull() && m_source_fields_isValid;

    m_return_enabled_isValid = ::OpenAPI::fromJsonValue(m_return_enabled, json[QString("ReturnEnabled")]);
    m_return_enabled_isSet = !json[QString("ReturnEnabled")].isNull() && m_return_enabled_isValid;

    m_highlight_enabled_isValid = ::OpenAPI::fromJsonValue(m_highlight_enabled, json[QString("HighlightEnabled")]);
    m_highlight_enabled_isSet = !json[QString("HighlightEnabled")].isNull() && m_highlight_enabled_isValid;

    m_analysis_scheme_isValid = ::OpenAPI::fromJsonValue(m_analysis_scheme, json[QString("AnalysisScheme")]);
    m_analysis_scheme_isSet = !json[QString("AnalysisScheme")].isNull() && m_analysis_scheme_isValid;
}

QString OAITextArrayOptions::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAITextArrayOptions::asJsonObject() const {
    QJsonObject obj;
    if (m_default_value_isSet) {
        obj.insert(QString("DefaultValue"), ::OpenAPI::toJsonValue(m_default_value));
    }
    if (m_source_fields_isSet) {
        obj.insert(QString("SourceFields"), ::OpenAPI::toJsonValue(m_source_fields));
    }
    if (m_return_enabled_isSet) {
        obj.insert(QString("ReturnEnabled"), ::OpenAPI::toJsonValue(m_return_enabled));
    }
    if (m_highlight_enabled_isSet) {
        obj.insert(QString("HighlightEnabled"), ::OpenAPI::toJsonValue(m_highlight_enabled));
    }
    if (m_analysis_scheme_isSet) {
        obj.insert(QString("AnalysisScheme"), ::OpenAPI::toJsonValue(m_analysis_scheme));
    }
    return obj;
}

QString OAITextArrayOptions::getDefaultValue() const {
    return m_default_value;
}
void OAITextArrayOptions::setDefaultValue(const QString &default_value) {
    m_default_value = default_value;
    m_default_value_isSet = true;
}

bool OAITextArrayOptions::is_default_value_Set() const{
    return m_default_value_isSet;
}

bool OAITextArrayOptions::is_default_value_Valid() const{
    return m_default_value_isValid;
}

QString OAITextArrayOptions::getSourceFields() const {
    return m_source_fields;
}
void OAITextArrayOptions::setSourceFields(const QString &source_fields) {
    m_source_fields = source_fields;
    m_source_fields_isSet = true;
}

bool OAITextArrayOptions::is_source_fields_Set() const{
    return m_source_fields_isSet;
}

bool OAITextArrayOptions::is_source_fields_Valid() const{
    return m_source_fields_isValid;
}

bool OAITextArrayOptions::getReturnEnabled() const {
    return m_return_enabled;
}
void OAITextArrayOptions::setReturnEnabled(const bool &return_enabled) {
    m_return_enabled = return_enabled;
    m_return_enabled_isSet = true;
}

bool OAITextArrayOptions::is_return_enabled_Set() const{
    return m_return_enabled_isSet;
}

bool OAITextArrayOptions::is_return_enabled_Valid() const{
    return m_return_enabled_isValid;
}

bool OAITextArrayOptions::getHighlightEnabled() const {
    return m_highlight_enabled;
}
void OAITextArrayOptions::setHighlightEnabled(const bool &highlight_enabled) {
    m_highlight_enabled = highlight_enabled;
    m_highlight_enabled_isSet = true;
}

bool OAITextArrayOptions::is_highlight_enabled_Set() const{
    return m_highlight_enabled_isSet;
}

bool OAITextArrayOptions::is_highlight_enabled_Valid() const{
    return m_highlight_enabled_isValid;
}

QString OAITextArrayOptions::getAnalysisScheme() const {
    return m_analysis_scheme;
}
void OAITextArrayOptions::setAnalysisScheme(const QString &analysis_scheme) {
    m_analysis_scheme = analysis_scheme;
    m_analysis_scheme_isSet = true;
}

bool OAITextArrayOptions::is_analysis_scheme_Set() const{
    return m_analysis_scheme_isSet;
}

bool OAITextArrayOptions::is_analysis_scheme_Valid() const{
    return m_analysis_scheme_isValid;
}

bool OAITextArrayOptions::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_default_value_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_source_fields_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_return_enabled_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_highlight_enabled_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_analysis_scheme_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAITextArrayOptions::isValid() const {
    // only required properties are required for the object to be considered valid
    return true;
}

} // namespace OpenAPI
