/**
 * Amazon Transcribe Service
 * <p>Amazon Transcribe offers three main types of batch transcription: <b>Standard</b>, <b>Medical</b>, and <b>Call Analytics</b>.</p> <ul> <li> <p> <b>Standard transcriptions</b> are the most common option. Refer to for details.</p> </li> <li> <p> <b>Medical transcriptions</b> are tailored to medical professionals and incorporate medical terms. A common use case for this service is transcribing doctor-patient dialogue into after-visit notes. Refer to for details.</p> </li> <li> <p> <b>Call Analytics transcriptions</b> are designed for use with call center audio on two different channels; if you're looking for insight into customer service calls, use this option. Refer to for details.</p> </li> </ul>
 *
 * The version of the OpenAPI document: 2017-10-26
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import CallAnalyticsJobMedia from './CallAnalyticsJobMedia';
import CallAnalyticsJobSettings from './CallAnalyticsJobSettings';
import CallAnalyticsJobStatus from './CallAnalyticsJobStatus';
import LanguageCode from './LanguageCode';
import MediaFormat from './MediaFormat';
import Transcript from './Transcript';

/**
 * The CallAnalyticsJob model module.
 * @module model/CallAnalyticsJob
 * @version 2017-10-26
 */
class CallAnalyticsJob {
    /**
     * Constructs a new <code>CallAnalyticsJob</code>.
     * &lt;p&gt;Provides detailed information about a Call Analytics job.&lt;/p&gt; &lt;p&gt;To view the job&#39;s status, refer to &lt;code&gt;CallAnalyticsJobStatus&lt;/code&gt;. If the status is &lt;code&gt;COMPLETED&lt;/code&gt;, the job is finished. You can find your completed transcript at the URI specified in &lt;code&gt;TranscriptFileUri&lt;/code&gt;. If the status is &lt;code&gt;FAILED&lt;/code&gt;, &lt;code&gt;FailureReason&lt;/code&gt; provides details on why your transcription job failed.&lt;/p&gt; &lt;p&gt;If you enabled personally identifiable information (PII) redaction, the redacted transcript appears at the location specified in &lt;code&gt;RedactedTranscriptFileUri&lt;/code&gt;.&lt;/p&gt; &lt;p&gt;If you chose to redact the audio in your media file, you can find your redacted media file at the location specified in the &lt;code&gt;RedactedMediaFileUri&lt;/code&gt; field of your response.&lt;/p&gt;
     * @alias module:model/CallAnalyticsJob
     */
    constructor() { 
        
        CallAnalyticsJob.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>CallAnalyticsJob</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/CallAnalyticsJob} obj Optional instance to populate.
     * @return {module:model/CallAnalyticsJob} The populated <code>CallAnalyticsJob</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new CallAnalyticsJob();

            if (data.hasOwnProperty('CallAnalyticsJobName')) {
                obj['CallAnalyticsJobName'] = ApiClient.convertToType(data['CallAnalyticsJobName'], 'String');
            }
            if (data.hasOwnProperty('CallAnalyticsJobStatus')) {
                obj['CallAnalyticsJobStatus'] = ApiClient.convertToType(data['CallAnalyticsJobStatus'], CallAnalyticsJobStatus);
            }
            if (data.hasOwnProperty('LanguageCode')) {
                obj['LanguageCode'] = ApiClient.convertToType(data['LanguageCode'], LanguageCode);
            }
            if (data.hasOwnProperty('MediaSampleRateHertz')) {
                obj['MediaSampleRateHertz'] = ApiClient.convertToType(data['MediaSampleRateHertz'], 'Number');
            }
            if (data.hasOwnProperty('MediaFormat')) {
                obj['MediaFormat'] = ApiClient.convertToType(data['MediaFormat'], MediaFormat);
            }
            if (data.hasOwnProperty('Media')) {
                obj['Media'] = CallAnalyticsJobMedia.constructFromObject(data['Media']);
            }
            if (data.hasOwnProperty('Transcript')) {
                obj['Transcript'] = Transcript.constructFromObject(data['Transcript']);
            }
            if (data.hasOwnProperty('StartTime')) {
                obj['StartTime'] = ApiClient.convertToType(data['StartTime'], 'Date');
            }
            if (data.hasOwnProperty('CreationTime')) {
                obj['CreationTime'] = ApiClient.convertToType(data['CreationTime'], 'Date');
            }
            if (data.hasOwnProperty('CompletionTime')) {
                obj['CompletionTime'] = ApiClient.convertToType(data['CompletionTime'], 'Date');
            }
            if (data.hasOwnProperty('FailureReason')) {
                obj['FailureReason'] = ApiClient.convertToType(data['FailureReason'], 'String');
            }
            if (data.hasOwnProperty('DataAccessRoleArn')) {
                obj['DataAccessRoleArn'] = ApiClient.convertToType(data['DataAccessRoleArn'], 'String');
            }
            if (data.hasOwnProperty('IdentifiedLanguageScore')) {
                obj['IdentifiedLanguageScore'] = ApiClient.convertToType(data['IdentifiedLanguageScore'], 'Number');
            }
            if (data.hasOwnProperty('Settings')) {
                obj['Settings'] = CallAnalyticsJobSettings.constructFromObject(data['Settings']);
            }
            if (data.hasOwnProperty('ChannelDefinitions')) {
                obj['ChannelDefinitions'] = ApiClient.convertToType(data['ChannelDefinitions'], Array);
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>CallAnalyticsJob</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>CallAnalyticsJob</code>.
     */
    static validateJSON(data) {
        // validate the optional field `CallAnalyticsJobName`
        if (data['CallAnalyticsJobName']) { // data not null
          String.validateJSON(data['CallAnalyticsJobName']);
        }
        // validate the optional field `CallAnalyticsJobStatus`
        if (data['CallAnalyticsJobStatus']) { // data not null
          CallAnalyticsJobStatus.validateJSON(data['CallAnalyticsJobStatus']);
        }
        // validate the optional field `LanguageCode`
        if (data['LanguageCode']) { // data not null
          LanguageCode.validateJSON(data['LanguageCode']);
        }
        // validate the optional field `MediaSampleRateHertz`
        if (data['MediaSampleRateHertz']) { // data not null
          Number.validateJSON(data['MediaSampleRateHertz']);
        }
        // validate the optional field `MediaFormat`
        if (data['MediaFormat']) { // data not null
          MediaFormat.validateJSON(data['MediaFormat']);
        }
        // validate the optional field `Media`
        if (data['Media']) { // data not null
          CallAnalyticsJobMedia.validateJSON(data['Media']);
        }
        // validate the optional field `Transcript`
        if (data['Transcript']) { // data not null
          Transcript.validateJSON(data['Transcript']);
        }
        // validate the optional field `StartTime`
        if (data['StartTime']) { // data not null
          Date.validateJSON(data['StartTime']);
        }
        // validate the optional field `CreationTime`
        if (data['CreationTime']) { // data not null
          Date.validateJSON(data['CreationTime']);
        }
        // validate the optional field `CompletionTime`
        if (data['CompletionTime']) { // data not null
          Date.validateJSON(data['CompletionTime']);
        }
        // validate the optional field `FailureReason`
        if (data['FailureReason']) { // data not null
          String.validateJSON(data['FailureReason']);
        }
        // validate the optional field `DataAccessRoleArn`
        if (data['DataAccessRoleArn']) { // data not null
          String.validateJSON(data['DataAccessRoleArn']);
        }
        // validate the optional field `IdentifiedLanguageScore`
        if (data['IdentifiedLanguageScore']) { // data not null
          Number.validateJSON(data['IdentifiedLanguageScore']);
        }
        // validate the optional field `Settings`
        if (data['Settings']) { // data not null
          CallAnalyticsJobSettings.validateJSON(data['Settings']);
        }
        // validate the optional field `ChannelDefinitions`
        if (data['ChannelDefinitions']) { // data not null
          Array.validateJSON(data['ChannelDefinitions']);
        }

        return true;
    }


}



/**
 * @member {String} CallAnalyticsJobName
 */
CallAnalyticsJob.prototype['CallAnalyticsJobName'] = undefined;

/**
 * @member {module:model/CallAnalyticsJobStatus} CallAnalyticsJobStatus
 */
CallAnalyticsJob.prototype['CallAnalyticsJobStatus'] = undefined;

/**
 * @member {module:model/LanguageCode} LanguageCode
 */
CallAnalyticsJob.prototype['LanguageCode'] = undefined;

/**
 * @member {Number} MediaSampleRateHertz
 */
CallAnalyticsJob.prototype['MediaSampleRateHertz'] = undefined;

/**
 * @member {module:model/MediaFormat} MediaFormat
 */
CallAnalyticsJob.prototype['MediaFormat'] = undefined;

/**
 * @member {module:model/CallAnalyticsJobMedia} Media
 */
CallAnalyticsJob.prototype['Media'] = undefined;

/**
 * @member {module:model/Transcript} Transcript
 */
CallAnalyticsJob.prototype['Transcript'] = undefined;

/**
 * @member {Date} StartTime
 */
CallAnalyticsJob.prototype['StartTime'] = undefined;

/**
 * @member {Date} CreationTime
 */
CallAnalyticsJob.prototype['CreationTime'] = undefined;

/**
 * @member {Date} CompletionTime
 */
CallAnalyticsJob.prototype['CompletionTime'] = undefined;

/**
 * @member {String} FailureReason
 */
CallAnalyticsJob.prototype['FailureReason'] = undefined;

/**
 * @member {String} DataAccessRoleArn
 */
CallAnalyticsJob.prototype['DataAccessRoleArn'] = undefined;

/**
 * @member {Number} IdentifiedLanguageScore
 */
CallAnalyticsJob.prototype['IdentifiedLanguageScore'] = undefined;

/**
 * @member {module:model/CallAnalyticsJobSettings} Settings
 */
CallAnalyticsJob.prototype['Settings'] = undefined;

/**
 * @member {Array} ChannelDefinitions
 */
CallAnalyticsJob.prototype['ChannelDefinitions'] = undefined;






export default CallAnalyticsJob;

