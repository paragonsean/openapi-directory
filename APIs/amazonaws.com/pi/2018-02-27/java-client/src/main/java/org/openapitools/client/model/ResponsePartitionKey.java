...
public class ResponsePartitionKey {
  ...
  public ResponsePartitionKey() {
  }

  public ResponsePartitionKey dimensions(Map dimensions) {
  ...
  public boolean equals(Object o) {
    ...
    ResponsePartitionKey responsePartitionKey = (ResponsePartitionKey) o;
    return Objects.equals(this.dimensions, responsePartitionKey.dimensions);
  }
  ...
  public String toString() {
    ...
    sb.append("class ResponsePartitionKey {\n");
   ...
   * @throws IOException if the JSON Element is invalid with respect to ResponsePartitionKey
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResponsePartitionKey.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResponsePartitionKey is not found in the empty JSON string", ResponsePartitionKey.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ResponsePartitionKey.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResponsePartitionKey` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResponsePartitionKey.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResponsePartitionKey.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResponsePartitionKey' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResponsePartitionKey> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResponsePartitionKey.class));

       return (TypeAdapter<T>) new TypeAdapter<ResponsePartitionKey>() {
           @Override
           public void write(JsonWriter out, ResponsePartitionKey value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResponsePartitionKey read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of ResponsePartitionKey given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of ResponsePartitionKey
   * @throws IOException if the JSON string is invalid with respect to ResponsePartitionKey
   */
  public static ResponsePartitionKey fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResponsePartitionKey.class);
  }

  /**
   * Convert an instance of ResponsePartitionKey to an JSON string
   *
   * @return JSON string
   */
