/**
 * AWS MediaTailor
 * <p>Use the AWS Elemental MediaTailor SDKs and CLI to configure scalable ad insertion and linear channels. With MediaTailor, you can assemble existing content into a linear stream and serve targeted ads to viewers while maintaining broadcast quality in over-the-top (OTT) video applications. For information about using the service, including detailed information about the settings covered in this guide, see the <a href=\"https://docs.aws.amazon.com/mediatailor/latest/ug/\">AWS Elemental MediaTailor User Guide</a>.</p> <p>Through the SDKs and the CLI you manage AWS Elemental MediaTailor configurations and channels the same as you do through the console. For example, you specify ad insertion behavior and mapping information for the origin server and the ad decision server (ADS).</p>
 *
 * The version of the OpenAPI document: 2018-04-23
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIPutPlaybackConfiguration_request.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIPutPlaybackConfiguration_request::OAIPutPlaybackConfiguration_request(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIPutPlaybackConfiguration_request::OAIPutPlaybackConfiguration_request() {
    this->initializeModel();
}

OAIPutPlaybackConfiguration_request::~OAIPutPlaybackConfiguration_request() {}

void OAIPutPlaybackConfiguration_request::initializeModel() {

    m_ad_decision_server_url_isSet = false;
    m_ad_decision_server_url_isValid = false;

    m_avail_suppression_isSet = false;
    m_avail_suppression_isValid = false;

    m_bumper_isSet = false;
    m_bumper_isValid = false;

    m_cdn_configuration_isSet = false;
    m_cdn_configuration_isValid = false;

    m_configuration_aliases_isSet = false;
    m_configuration_aliases_isValid = false;

    m_dash_configuration_isSet = false;
    m_dash_configuration_isValid = false;

    m_live_pre_roll_configuration_isSet = false;
    m_live_pre_roll_configuration_isValid = false;

    m_manifest_processing_rules_isSet = false;
    m_manifest_processing_rules_isValid = false;

    m_name_isSet = false;
    m_name_isValid = false;

    m_personalization_threshold_seconds_isSet = false;
    m_personalization_threshold_seconds_isValid = false;

    m_slate_ad_url_isSet = false;
    m_slate_ad_url_isValid = false;

    m_tags_isSet = false;
    m_tags_isValid = false;

    m_transcode_profile_name_isSet = false;
    m_transcode_profile_name_isValid = false;

    m_video_content_source_url_isSet = false;
    m_video_content_source_url_isValid = false;
}

void OAIPutPlaybackConfiguration_request::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAIPutPlaybackConfiguration_request::fromJsonObject(QJsonObject json) {

    m_ad_decision_server_url_isValid = ::OpenAPI::fromJsonValue(m_ad_decision_server_url, json[QString("AdDecisionServerUrl")]);
    m_ad_decision_server_url_isSet = !json[QString("AdDecisionServerUrl")].isNull() && m_ad_decision_server_url_isValid;

    m_avail_suppression_isValid = ::OpenAPI::fromJsonValue(m_avail_suppression, json[QString("AvailSuppression")]);
    m_avail_suppression_isSet = !json[QString("AvailSuppression")].isNull() && m_avail_suppression_isValid;

    m_bumper_isValid = ::OpenAPI::fromJsonValue(m_bumper, json[QString("Bumper")]);
    m_bumper_isSet = !json[QString("Bumper")].isNull() && m_bumper_isValid;

    m_cdn_configuration_isValid = ::OpenAPI::fromJsonValue(m_cdn_configuration, json[QString("CdnConfiguration")]);
    m_cdn_configuration_isSet = !json[QString("CdnConfiguration")].isNull() && m_cdn_configuration_isValid;

    if(json["ConfigurationAliases"].isObject()){
        auto varmap = json["ConfigurationAliases"].toObject().toVariantMap();
        m_configuration_aliases_isValid = true;
        if(varmap.count() > 0){
            for(auto val : varmap.keys()){
                QMap<QString, QString> item;
                auto jval = QJsonValue::fromVariant(varmap.value(val));
                m_configuration_aliases_isValid &= ::OpenAPI::fromJsonValue(item, jval);
                m_configuration_aliases_isSet &= !jval.isNull() && m_configuration_aliases_isValid;
                m_configuration_aliases.insert(m_configuration_aliases.end(), val, item);
            }
        }
    }

    m_dash_configuration_isValid = ::OpenAPI::fromJsonValue(m_dash_configuration, json[QString("DashConfiguration")]);
    m_dash_configuration_isSet = !json[QString("DashConfiguration")].isNull() && m_dash_configuration_isValid;

    m_live_pre_roll_configuration_isValid = ::OpenAPI::fromJsonValue(m_live_pre_roll_configuration, json[QString("LivePreRollConfiguration")]);
    m_live_pre_roll_configuration_isSet = !json[QString("LivePreRollConfiguration")].isNull() && m_live_pre_roll_configuration_isValid;

    m_manifest_processing_rules_isValid = ::OpenAPI::fromJsonValue(m_manifest_processing_rules, json[QString("ManifestProcessingRules")]);
    m_manifest_processing_rules_isSet = !json[QString("ManifestProcessingRules")].isNull() && m_manifest_processing_rules_isValid;

    m_name_isValid = ::OpenAPI::fromJsonValue(m_name, json[QString("Name")]);
    m_name_isSet = !json[QString("Name")].isNull() && m_name_isValid;

    m_personalization_threshold_seconds_isValid = ::OpenAPI::fromJsonValue(m_personalization_threshold_seconds, json[QString("PersonalizationThresholdSeconds")]);
    m_personalization_threshold_seconds_isSet = !json[QString("PersonalizationThresholdSeconds")].isNull() && m_personalization_threshold_seconds_isValid;

    m_slate_ad_url_isValid = ::OpenAPI::fromJsonValue(m_slate_ad_url, json[QString("SlateAdUrl")]);
    m_slate_ad_url_isSet = !json[QString("SlateAdUrl")].isNull() && m_slate_ad_url_isValid;

    m_tags_isValid = ::OpenAPI::fromJsonValue(m_tags, json[QString("tags")]);
    m_tags_isSet = !json[QString("tags")].isNull() && m_tags_isValid;

    m_transcode_profile_name_isValid = ::OpenAPI::fromJsonValue(m_transcode_profile_name, json[QString("TranscodeProfileName")]);
    m_transcode_profile_name_isSet = !json[QString("TranscodeProfileName")].isNull() && m_transcode_profile_name_isValid;

    m_video_content_source_url_isValid = ::OpenAPI::fromJsonValue(m_video_content_source_url, json[QString("VideoContentSourceUrl")]);
    m_video_content_source_url_isSet = !json[QString("VideoContentSourceUrl")].isNull() && m_video_content_source_url_isValid;
}

QString OAIPutPlaybackConfiguration_request::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAIPutPlaybackConfiguration_request::asJsonObject() const {
    QJsonObject obj;
    if (m_ad_decision_server_url_isSet) {
        obj.insert(QString("AdDecisionServerUrl"), ::OpenAPI::toJsonValue(m_ad_decision_server_url));
    }
    if (m_avail_suppression.isSet()) {
        obj.insert(QString("AvailSuppression"), ::OpenAPI::toJsonValue(m_avail_suppression));
    }
    if (m_bumper.isSet()) {
        obj.insert(QString("Bumper"), ::OpenAPI::toJsonValue(m_bumper));
    }
    if (m_cdn_configuration.isSet()) {
        obj.insert(QString("CdnConfiguration"), ::OpenAPI::toJsonValue(m_cdn_configuration));
    }
    if (m_configuration_aliases.size() > 0) {
        
        obj.insert(QString("ConfigurationAliases"), toJsonValue(m_configuration_aliases));
    }
    if (m_dash_configuration.isSet()) {
        obj.insert(QString("DashConfiguration"), ::OpenAPI::toJsonValue(m_dash_configuration));
    }
    if (m_live_pre_roll_configuration.isSet()) {
        obj.insert(QString("LivePreRollConfiguration"), ::OpenAPI::toJsonValue(m_live_pre_roll_configuration));
    }
    if (m_manifest_processing_rules.isSet()) {
        obj.insert(QString("ManifestProcessingRules"), ::OpenAPI::toJsonValue(m_manifest_processing_rules));
    }
    if (m_name_isSet) {
        obj.insert(QString("Name"), ::OpenAPI::toJsonValue(m_name));
    }
    if (m_personalization_threshold_seconds_isSet) {
        obj.insert(QString("PersonalizationThresholdSeconds"), ::OpenAPI::toJsonValue(m_personalization_threshold_seconds));
    }
    if (m_slate_ad_url_isSet) {
        obj.insert(QString("SlateAdUrl"), ::OpenAPI::toJsonValue(m_slate_ad_url));
    }
    if (m_tags.size() > 0) {
        obj.insert(QString("tags"), ::OpenAPI::toJsonValue(m_tags));
    }
    if (m_transcode_profile_name_isSet) {
        obj.insert(QString("TranscodeProfileName"), ::OpenAPI::toJsonValue(m_transcode_profile_name));
    }
    if (m_video_content_source_url_isSet) {
        obj.insert(QString("VideoContentSourceUrl"), ::OpenAPI::toJsonValue(m_video_content_source_url));
    }
    return obj;
}

QString OAIPutPlaybackConfiguration_request::getAdDecisionServerUrl() const {
    return m_ad_decision_server_url;
}
void OAIPutPlaybackConfiguration_request::setAdDecisionServerUrl(const QString &ad_decision_server_url) {
    m_ad_decision_server_url = ad_decision_server_url;
    m_ad_decision_server_url_isSet = true;
}

bool OAIPutPlaybackConfiguration_request::is_ad_decision_server_url_Set() const{
    return m_ad_decision_server_url_isSet;
}

bool OAIPutPlaybackConfiguration_request::is_ad_decision_server_url_Valid() const{
    return m_ad_decision_server_url_isValid;
}

OAIPutPlaybackConfiguration_request_AvailSuppression OAIPutPlaybackConfiguration_request::getAvailSuppression() const {
    return m_avail_suppression;
}
void OAIPutPlaybackConfiguration_request::setAvailSuppression(const OAIPutPlaybackConfiguration_request_AvailSuppression &avail_suppression) {
    m_avail_suppression = avail_suppression;
    m_avail_suppression_isSet = true;
}

bool OAIPutPlaybackConfiguration_request::is_avail_suppression_Set() const{
    return m_avail_suppression_isSet;
}

bool OAIPutPlaybackConfiguration_request::is_avail_suppression_Valid() const{
    return m_avail_suppression_isValid;
}

OAIPutPlaybackConfiguration_request_Bumper OAIPutPlaybackConfiguration_request::getBumper() const {
    return m_bumper;
}
void OAIPutPlaybackConfiguration_request::setBumper(const OAIPutPlaybackConfiguration_request_Bumper &bumper) {
    m_bumper = bumper;
    m_bumper_isSet = true;
}

bool OAIPutPlaybackConfiguration_request::is_bumper_Set() const{
    return m_bumper_isSet;
}

bool OAIPutPlaybackConfiguration_request::is_bumper_Valid() const{
    return m_bumper_isValid;
}

OAIPutPlaybackConfiguration_request_CdnConfiguration OAIPutPlaybackConfiguration_request::getCdnConfiguration() const {
    return m_cdn_configuration;
}
void OAIPutPlaybackConfiguration_request::setCdnConfiguration(const OAIPutPlaybackConfiguration_request_CdnConfiguration &cdn_configuration) {
    m_cdn_configuration = cdn_configuration;
    m_cdn_configuration_isSet = true;
}

bool OAIPutPlaybackConfiguration_request::is_cdn_configuration_Set() const{
    return m_cdn_configuration_isSet;
}

bool OAIPutPlaybackConfiguration_request::is_cdn_configuration_Valid() const{
    return m_cdn_configuration_isValid;
}

QMap<QString, QMap> OAIPutPlaybackConfiguration_request::getConfigurationAliases() const {
    return m_configuration_aliases;
}
void OAIPutPlaybackConfiguration_request::setConfigurationAliases(const QMap<QString, QMap> &configuration_aliases) {
    m_configuration_aliases = configuration_aliases;
    m_configuration_aliases_isSet = true;
}

bool OAIPutPlaybackConfiguration_request::is_configuration_aliases_Set() const{
    return m_configuration_aliases_isSet;
}

bool OAIPutPlaybackConfiguration_request::is_configuration_aliases_Valid() const{
    return m_configuration_aliases_isValid;
}

OAIPutPlaybackConfiguration_request_DashConfiguration OAIPutPlaybackConfiguration_request::getDashConfiguration() const {
    return m_dash_configuration;
}
void OAIPutPlaybackConfiguration_request::setDashConfiguration(const OAIPutPlaybackConfiguration_request_DashConfiguration &dash_configuration) {
    m_dash_configuration = dash_configuration;
    m_dash_configuration_isSet = true;
}

bool OAIPutPlaybackConfiguration_request::is_dash_configuration_Set() const{
    return m_dash_configuration_isSet;
}

bool OAIPutPlaybackConfiguration_request::is_dash_configuration_Valid() const{
    return m_dash_configuration_isValid;
}

OAIPutPlaybackConfiguration_request_LivePreRollConfiguration OAIPutPlaybackConfiguration_request::getLivePreRollConfiguration() const {
    return m_live_pre_roll_configuration;
}
void OAIPutPlaybackConfiguration_request::setLivePreRollConfiguration(const OAIPutPlaybackConfiguration_request_LivePreRollConfiguration &live_pre_roll_configuration) {
    m_live_pre_roll_configuration = live_pre_roll_configuration;
    m_live_pre_roll_configuration_isSet = true;
}

bool OAIPutPlaybackConfiguration_request::is_live_pre_roll_configuration_Set() const{
    return m_live_pre_roll_configuration_isSet;
}

bool OAIPutPlaybackConfiguration_request::is_live_pre_roll_configuration_Valid() const{
    return m_live_pre_roll_configuration_isValid;
}

OAIPutPlaybackConfiguration_request_ManifestProcessingRules OAIPutPlaybackConfiguration_request::getManifestProcessingRules() const {
    return m_manifest_processing_rules;
}
void OAIPutPlaybackConfiguration_request::setManifestProcessingRules(const OAIPutPlaybackConfiguration_request_ManifestProcessingRules &manifest_processing_rules) {
    m_manifest_processing_rules = manifest_processing_rules;
    m_manifest_processing_rules_isSet = true;
}

bool OAIPutPlaybackConfiguration_request::is_manifest_processing_rules_Set() const{
    return m_manifest_processing_rules_isSet;
}

bool OAIPutPlaybackConfiguration_request::is_manifest_processing_rules_Valid() const{
    return m_manifest_processing_rules_isValid;
}

QString OAIPutPlaybackConfiguration_request::getName() const {
    return m_name;
}
void OAIPutPlaybackConfiguration_request::setName(const QString &name) {
    m_name = name;
    m_name_isSet = true;
}

bool OAIPutPlaybackConfiguration_request::is_name_Set() const{
    return m_name_isSet;
}

bool OAIPutPlaybackConfiguration_request::is_name_Valid() const{
    return m_name_isValid;
}

qint32 OAIPutPlaybackConfiguration_request::getPersonalizationThresholdSeconds() const {
    return m_personalization_threshold_seconds;
}
void OAIPutPlaybackConfiguration_request::setPersonalizationThresholdSeconds(const qint32 &personalization_threshold_seconds) {
    m_personalization_threshold_seconds = personalization_threshold_seconds;
    m_personalization_threshold_seconds_isSet = true;
}

bool OAIPutPlaybackConfiguration_request::is_personalization_threshold_seconds_Set() const{
    return m_personalization_threshold_seconds_isSet;
}

bool OAIPutPlaybackConfiguration_request::is_personalization_threshold_seconds_Valid() const{
    return m_personalization_threshold_seconds_isValid;
}

QString OAIPutPlaybackConfiguration_request::getSlateAdUrl() const {
    return m_slate_ad_url;
}
void OAIPutPlaybackConfiguration_request::setSlateAdUrl(const QString &slate_ad_url) {
    m_slate_ad_url = slate_ad_url;
    m_slate_ad_url_isSet = true;
}

bool OAIPutPlaybackConfiguration_request::is_slate_ad_url_Set() const{
    return m_slate_ad_url_isSet;
}

bool OAIPutPlaybackConfiguration_request::is_slate_ad_url_Valid() const{
    return m_slate_ad_url_isValid;
}

QMap<QString, QString> OAIPutPlaybackConfiguration_request::getTags() const {
    return m_tags;
}
void OAIPutPlaybackConfiguration_request::setTags(const QMap<QString, QString> &tags) {
    m_tags = tags;
    m_tags_isSet = true;
}

bool OAIPutPlaybackConfiguration_request::is_tags_Set() const{
    return m_tags_isSet;
}

bool OAIPutPlaybackConfiguration_request::is_tags_Valid() const{
    return m_tags_isValid;
}

QString OAIPutPlaybackConfiguration_request::getTranscodeProfileName() const {
    return m_transcode_profile_name;
}
void OAIPutPlaybackConfiguration_request::setTranscodeProfileName(const QString &transcode_profile_name) {
    m_transcode_profile_name = transcode_profile_name;
    m_transcode_profile_name_isSet = true;
}

bool OAIPutPlaybackConfiguration_request::is_transcode_profile_name_Set() const{
    return m_transcode_profile_name_isSet;
}

bool OAIPutPlaybackConfiguration_request::is_transcode_profile_name_Valid() const{
    return m_transcode_profile_name_isValid;
}

QString OAIPutPlaybackConfiguration_request::getVideoContentSourceUrl() const {
    return m_video_content_source_url;
}
void OAIPutPlaybackConfiguration_request::setVideoContentSourceUrl(const QString &video_content_source_url) {
    m_video_content_source_url = video_content_source_url;
    m_video_content_source_url_isSet = true;
}

bool OAIPutPlaybackConfiguration_request::is_video_content_source_url_Set() const{
    return m_video_content_source_url_isSet;
}

bool OAIPutPlaybackConfiguration_request::is_video_content_source_url_Valid() const{
    return m_video_content_source_url_isValid;
}

bool OAIPutPlaybackConfiguration_request::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_ad_decision_server_url_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_avail_suppression.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_bumper.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_cdn_configuration.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_configuration_aliases.size() > 0) {
            isObjectUpdated = true;
            break;
        }

        if (m_dash_configuration.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_live_pre_roll_configuration.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_manifest_processing_rules.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_name_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_personalization_threshold_seconds_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_slate_ad_url_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_tags.size() > 0) {
            isObjectUpdated = true;
            break;
        }

        if (m_transcode_profile_name_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_video_content_source_url_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAIPutPlaybackConfiguration_request::isValid() const {
    // only required properties are required for the object to be considered valid
    return m_name_isValid && true;
}

} // namespace OpenAPI
