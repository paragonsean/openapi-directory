/**
 * Inspector2
 * Amazon Inspector is a vulnerability discovery service that automates continuous scanning for security vulnerabilities within your Amazon EC2 and Amazon ECR environments.
 *
 * The version of the OpenAPI document: 2020-06-08
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIAtigData.h
 *
 * The Amazon Web Services Threat Intel Group (ATIG) details for a specific vulnerability.
 */

#ifndef OAIAtigData_H
#define OAIAtigData_H

#include <QJsonObject>

#include <QDateTime>
#include <QList>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {

class OAIAtigData : public OAIObject {
public:
    OAIAtigData();
    OAIAtigData(QString json);
    ~OAIAtigData() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QDateTime getFirstSeen() const;
    void setFirstSeen(const QDateTime &first_seen);
    bool is_first_seen_Set() const;
    bool is_first_seen_Valid() const;

    QDateTime getLastSeen() const;
    void setLastSeen(const QDateTime &last_seen);
    bool is_last_seen_Set() const;
    bool is_last_seen_Valid() const;

    QList getTargets() const;
    void setTargets(const QList &targets);
    bool is_targets_Set() const;
    bool is_targets_Valid() const;

    QList getTtps() const;
    void setTtps(const QList &ttps);
    bool is_ttps_Set() const;
    bool is_ttps_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QDateTime m_first_seen;
    bool m_first_seen_isSet;
    bool m_first_seen_isValid;

    QDateTime m_last_seen;
    bool m_last_seen_isSet;
    bool m_last_seen_isValid;

    QList m_targets;
    bool m_targets_isSet;
    bool m_targets_isValid;

    QList m_ttps;
    bool m_ttps_isSet;
    bool m_ttps_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIAtigData)

#endif // OAIAtigData_H
