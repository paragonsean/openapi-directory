/**
 * AWS Step Functions
 * <fullname>Step Functions</fullname> <p>Step Functions is a service that lets you coordinate the components of distributed applications and microservices using visual workflows.</p> <p>You can use Step Functions to build applications from individual components, each of which performs a discrete function, or <i>task</i>, allowing you to scale and change applications quickly. Step Functions provides a console that helps visualize the components of your application as a series of steps. Step Functions automatically triggers and tracks each step, and retries steps when there are errors, so your application executes predictably and in the right order every time. Step Functions logs the state of each step, so you can quickly diagnose and debug any issues.</p> <p>Step Functions manages operations and underlying infrastructure to ensure your application is available at any scale. You can run tasks on Amazon Web Services, your own servers, or any system that has access to Amazon Web Services. You can access and use Step Functions using the console, the Amazon Web Services SDKs, or an HTTP API. For more information about Step Functions, see the <i> <a href=\"https://docs.aws.amazon.com/step-functions/latest/dg/welcome.html\">Step Functions Developer Guide</a> </i>.</p>
 *
 * The version of the OpenAPI document: 2016-11-23
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIActivityStartedEventDetails.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIActivityStartedEventDetails::OAIActivityStartedEventDetails(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIActivityStartedEventDetails::OAIActivityStartedEventDetails() {
    this->initializeModel();
}

OAIActivityStartedEventDetails::~OAIActivityStartedEventDetails() {}

void OAIActivityStartedEventDetails::initializeModel() {

    m_worker_name_isSet = false;
    m_worker_name_isValid = false;
}

void OAIActivityStartedEventDetails::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAIActivityStartedEventDetails::fromJsonObject(QJsonObject json) {

    m_worker_name_isValid = ::OpenAPI::fromJsonValue(m_worker_name, json[QString("workerName")]);
    m_worker_name_isSet = !json[QString("workerName")].isNull() && m_worker_name_isValid;
}

QString OAIActivityStartedEventDetails::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAIActivityStartedEventDetails::asJsonObject() const {
    QJsonObject obj;
    if (m_worker_name_isSet) {
        obj.insert(QString("workerName"), ::OpenAPI::toJsonValue(m_worker_name));
    }
    return obj;
}

QString OAIActivityStartedEventDetails::getWorkerName() const {
    return m_worker_name;
}
void OAIActivityStartedEventDetails::setWorkerName(const QString &worker_name) {
    m_worker_name = worker_name;
    m_worker_name_isSet = true;
}

bool OAIActivityStartedEventDetails::is_worker_name_Set() const{
    return m_worker_name_isSet;
}

bool OAIActivityStartedEventDetails::is_worker_name_Valid() const{
    return m_worker_name_isValid;
}

bool OAIActivityStartedEventDetails::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_worker_name_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAIActivityStartedEventDetails::isValid() const {
    // only required properties are required for the object to be considered valid
    return true;
}

} // namespace OpenAPI
