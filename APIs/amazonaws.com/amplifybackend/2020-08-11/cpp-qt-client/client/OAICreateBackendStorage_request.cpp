/**
 * AmplifyBackend
 * AWS Amplify Admin API
 *
 * The version of the OpenAPI document: 2020-08-11
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAICreateBackendStorage_request.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAICreateBackendStorage_request::OAICreateBackendStorage_request(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAICreateBackendStorage_request::OAICreateBackendStorage_request() {
    this->initializeModel();
}

OAICreateBackendStorage_request::~OAICreateBackendStorage_request() {}

void OAICreateBackendStorage_request::initializeModel() {

    m_backend_environment_name_isSet = false;
    m_backend_environment_name_isValid = false;

    m_resource_config_isSet = false;
    m_resource_config_isValid = false;

    m_resource_name_isSet = false;
    m_resource_name_isValid = false;
}

void OAICreateBackendStorage_request::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAICreateBackendStorage_request::fromJsonObject(QJsonObject json) {

    m_backend_environment_name_isValid = ::OpenAPI::fromJsonValue(m_backend_environment_name, json[QString("backendEnvironmentName")]);
    m_backend_environment_name_isSet = !json[QString("backendEnvironmentName")].isNull() && m_backend_environment_name_isValid;

    m_resource_config_isValid = ::OpenAPI::fromJsonValue(m_resource_config, json[QString("resourceConfig")]);
    m_resource_config_isSet = !json[QString("resourceConfig")].isNull() && m_resource_config_isValid;

    m_resource_name_isValid = ::OpenAPI::fromJsonValue(m_resource_name, json[QString("resourceName")]);
    m_resource_name_isSet = !json[QString("resourceName")].isNull() && m_resource_name_isValid;
}

QString OAICreateBackendStorage_request::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAICreateBackendStorage_request::asJsonObject() const {
    QJsonObject obj;
    if (m_backend_environment_name_isSet) {
        obj.insert(QString("backendEnvironmentName"), ::OpenAPI::toJsonValue(m_backend_environment_name));
    }
    if (m_resource_config.isSet()) {
        obj.insert(QString("resourceConfig"), ::OpenAPI::toJsonValue(m_resource_config));
    }
    if (m_resource_name_isSet) {
        obj.insert(QString("resourceName"), ::OpenAPI::toJsonValue(m_resource_name));
    }
    return obj;
}

QString OAICreateBackendStorage_request::getBackendEnvironmentName() const {
    return m_backend_environment_name;
}
void OAICreateBackendStorage_request::setBackendEnvironmentName(const QString &backend_environment_name) {
    m_backend_environment_name = backend_environment_name;
    m_backend_environment_name_isSet = true;
}

bool OAICreateBackendStorage_request::is_backend_environment_name_Set() const{
    return m_backend_environment_name_isSet;
}

bool OAICreateBackendStorage_request::is_backend_environment_name_Valid() const{
    return m_backend_environment_name_isValid;
}

OAICreateBackendStorage_request_resourceConfig OAICreateBackendStorage_request::getResourceConfig() const {
    return m_resource_config;
}
void OAICreateBackendStorage_request::setResourceConfig(const OAICreateBackendStorage_request_resourceConfig &resource_config) {
    m_resource_config = resource_config;
    m_resource_config_isSet = true;
}

bool OAICreateBackendStorage_request::is_resource_config_Set() const{
    return m_resource_config_isSet;
}

bool OAICreateBackendStorage_request::is_resource_config_Valid() const{
    return m_resource_config_isValid;
}

QString OAICreateBackendStorage_request::getResourceName() const {
    return m_resource_name;
}
void OAICreateBackendStorage_request::setResourceName(const QString &resource_name) {
    m_resource_name = resource_name;
    m_resource_name_isSet = true;
}

bool OAICreateBackendStorage_request::is_resource_name_Set() const{
    return m_resource_name_isSet;
}

bool OAICreateBackendStorage_request::is_resource_name_Valid() const{
    return m_resource_name_isValid;
}

bool OAICreateBackendStorage_request::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_backend_environment_name_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_resource_config.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_resource_name_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAICreateBackendStorage_request::isValid() const {
    // only required properties are required for the object to be considered valid
    return m_backend_environment_name_isValid && m_resource_config_isValid && m_resource_name_isValid && true;
}

} // namespace OpenAPI
