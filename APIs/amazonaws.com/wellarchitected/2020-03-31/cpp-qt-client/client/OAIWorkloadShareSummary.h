/**
 * AWS Well-Architected Tool
 * <fullname>Well-Architected Tool</fullname> <p>This is the <i>Well-Architected Tool API Reference</i>. The WA Tool API provides programmatic access to the <a href=\"http://aws.amazon.com/well-architected-tool\">Well-Architected Tool</a> in the <a href=\"https://console.aws.amazon.com/wellarchitected\">Amazon Web Services Management Console</a>. For information about the Well-Architected Tool, see the <a href=\"https://docs.aws.amazon.com/wellarchitected/latest/userguide/intro.html\">Well-Architected Tool User Guide</a>.</p>
 *
 * The version of the OpenAPI document: 2020-03-31
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIWorkloadShareSummary.h
 *
 * A workload share summary return object.
 */

#ifndef OAIWorkloadShareSummary_H
#define OAIWorkloadShareSummary_H

#include <QJsonObject>

#include "OAIPermissionType.h"
#include "OAIShareStatus.h"
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {

class OAIWorkloadShareSummary : public OAIObject {
public:
    OAIWorkloadShareSummary();
    OAIWorkloadShareSummary(QString json);
    ~OAIWorkloadShareSummary() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QString getShareId() const;
    void setShareId(const QString &share_id);
    bool is_share_id_Set() const;
    bool is_share_id_Valid() const;

    QString getSharedWith() const;
    void setSharedWith(const QString &shared_with);
    bool is_shared_with_Set() const;
    bool is_shared_with_Valid() const;

    OAIPermissionType getPermissionType() const;
    void setPermissionType(const OAIPermissionType &permission_type);
    bool is_permission_type_Set() const;
    bool is_permission_type_Valid() const;

    OAIShareStatus getStatus() const;
    void setStatus(const OAIShareStatus &status);
    bool is_status_Set() const;
    bool is_status_Valid() const;

    QString getStatusMessage() const;
    void setStatusMessage(const QString &status_message);
    bool is_status_message_Set() const;
    bool is_status_message_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString m_share_id;
    bool m_share_id_isSet;
    bool m_share_id_isValid;

    QString m_shared_with;
    bool m_shared_with_isSet;
    bool m_shared_with_isValid;

    OAIPermissionType m_permission_type;
    bool m_permission_type_isSet;
    bool m_permission_type_isValid;

    OAIShareStatus m_status;
    bool m_status_isSet;
    bool m_status_isValid;

    QString m_status_message;
    bool m_status_message_isSet;
    bool m_status_message_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIWorkloadShareSummary)

#endif // OAIWorkloadShareSummary_H
