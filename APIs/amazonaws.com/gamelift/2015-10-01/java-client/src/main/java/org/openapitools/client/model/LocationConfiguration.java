...
public class LocationConfiguration {
  ...
  public boolean equals(Object o) {
    ...
    LocationConfiguration locationConfiguration = (LocationConfiguration) o;
    return Objects.equals(this.location, locationConfiguration.location);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to LocationConfiguration
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!LocationConfiguration.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in LocationConfiguration is not found in the empty JSON string", LocationConfiguration.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!LocationConfiguration.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `LocationConfiguration` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : LocationConfiguration.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!LocationConfiguration.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'LocationConfiguration' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<LocationConfiguration> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(LocationConfiguration.class));

       return (TypeAdapter<T>) new TypeAdapter<LocationConfiguration>() {
           @Override
           public void write(JsonWriter out, LocationConfiguration value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public LocationConfiguration read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of LocationConfiguration given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of LocationConfiguration
   * @throws IOException if the JSON string is invalid with respect to LocationConfiguration
   */
  public static LocationConfiguration fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, LocationConfiguration.class);
  }

  /**
   * Convert an instance of LocationConfiguration to an JSON string
   *
   * @return JSON string
   */
