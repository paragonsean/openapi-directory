/**
 * Amazon GameLift
 * <p>Amazon GameLift provides solutions for hosting session-based multiplayer game servers in the cloud, including tools for deploying, operating, and scaling game servers. Built on Amazon Web Services global computing infrastructure, GameLift helps you deliver high-performance, high-reliability, low-cost game servers while dynamically scaling your resource usage to meet player demand. </p> <p> <b>About Amazon GameLift solutions</b> </p> <p>Get more information on these Amazon GameLift solutions in the <a href=\"https://docs.aws.amazon.com/gamelift/latest/developerguide/\">Amazon GameLift Developer Guide</a>.</p> <ul> <li> <p>Amazon GameLift managed hosting -- Amazon GameLift offers a fully managed service to set up and maintain computing machines for hosting, manage game session and player session life cycle, and handle security, storage, and performance tracking. You can use automatic scaling tools to balance player demand and hosting costs, configure your game session management to minimize player latency, and add FlexMatch for matchmaking.</p> </li> <li> <p>Managed hosting with Realtime Servers -- With Amazon GameLift Realtime Servers, you can quickly configure and set up ready-to-go game servers for your game. Realtime Servers provides a game server framework with core Amazon GameLift infrastructure already built in. Then use the full range of Amazon GameLift managed hosting features, including FlexMatch, for your game.</p> </li> <li> <p>Amazon GameLift FleetIQ -- Use Amazon GameLift FleetIQ as a standalone service while hosting your games using EC2 instances and Auto Scaling groups. Amazon GameLift FleetIQ provides optimizations for game hosting, including boosting the viability of low-cost Spot Instances gaming. For a complete solution, pair the Amazon GameLift FleetIQ and FlexMatch standalone services.</p> </li> <li> <p>Amazon GameLift FlexMatch -- Add matchmaking to your game hosting solution. FlexMatch is a customizable matchmaking service for multiplayer games. Use FlexMatch as integrated with Amazon GameLift managed hosting or incorporate FlexMatch as a standalone service into your own hosting solution.</p> </li> </ul> <p> <b>About this API Reference</b> </p> <p>This reference guide describes the low-level service API for Amazon GameLift. With each topic in this guide, you can find links to language-specific SDK guides and the Amazon Web Services CLI reference. Useful links:</p> <ul> <li> <p> <a href=\"https://docs.aws.amazon.com/gamelift/latest/developerguide/reference-awssdk.html\">Amazon GameLift API operations listed by tasks</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/gamelift/latest/developerguide/gamelift-components.html\"> Amazon GameLift tools and resources</a> </p> </li> </ul>
 *
 * The version of the OpenAPI document: 2015-10-01
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import Compute from './Compute';
import ComputeStatus from './ComputeStatus';
import EC2InstanceType from './EC2InstanceType';
import OperatingSystem from './OperatingSystem';

/**
 * The DescribeComputeOutputCompute model module.
 * @module model/DescribeComputeOutputCompute
 * @version 2015-10-01
 */
class DescribeComputeOutputCompute {
    /**
     * Constructs a new <code>DescribeComputeOutputCompute</code>.
     * @alias module:model/DescribeComputeOutputCompute
     * @implements module:model/Compute
     */
    constructor() { 
        Compute.initialize(this);
        DescribeComputeOutputCompute.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>DescribeComputeOutputCompute</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/DescribeComputeOutputCompute} obj Optional instance to populate.
     * @return {module:model/DescribeComputeOutputCompute} The populated <code>DescribeComputeOutputCompute</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new DescribeComputeOutputCompute();
            Compute.constructFromObject(data, obj);

            if (data.hasOwnProperty('FleetId')) {
                obj['FleetId'] = ApiClient.convertToType(data['FleetId'], 'String');
            }
            if (data.hasOwnProperty('FleetArn')) {
                obj['FleetArn'] = ApiClient.convertToType(data['FleetArn'], 'String');
            }
            if (data.hasOwnProperty('ComputeName')) {
                obj['ComputeName'] = ApiClient.convertToType(data['ComputeName'], 'String');
            }
            if (data.hasOwnProperty('ComputeArn')) {
                obj['ComputeArn'] = ApiClient.convertToType(data['ComputeArn'], 'String');
            }
            if (data.hasOwnProperty('IpAddress')) {
                obj['IpAddress'] = ApiClient.convertToType(data['IpAddress'], 'String');
            }
            if (data.hasOwnProperty('DnsName')) {
                obj['DnsName'] = ApiClient.convertToType(data['DnsName'], 'String');
            }
            if (data.hasOwnProperty('ComputeStatus')) {
                obj['ComputeStatus'] = ApiClient.convertToType(data['ComputeStatus'], ComputeStatus);
            }
            if (data.hasOwnProperty('Location')) {
                obj['Location'] = ApiClient.convertToType(data['Location'], 'String');
            }
            if (data.hasOwnProperty('CreationTime')) {
                obj['CreationTime'] = ApiClient.convertToType(data['CreationTime'], 'Date');
            }
            if (data.hasOwnProperty('OperatingSystem')) {
                obj['OperatingSystem'] = ApiClient.convertToType(data['OperatingSystem'], OperatingSystem);
            }
            if (data.hasOwnProperty('Type')) {
                obj['Type'] = ApiClient.convertToType(data['Type'], EC2InstanceType);
            }
            if (data.hasOwnProperty('GameLiftServiceSdkEndpoint')) {
                obj['GameLiftServiceSdkEndpoint'] = ApiClient.convertToType(data['GameLiftServiceSdkEndpoint'], 'String');
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>DescribeComputeOutputCompute</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>DescribeComputeOutputCompute</code>.
     */
    static validateJSON(data) {
        // validate the optional field `FleetId`
        if (data['FleetId']) { // data not null
          String.validateJSON(data['FleetId']);
        }
        // validate the optional field `FleetArn`
        if (data['FleetArn']) { // data not null
          String.validateJSON(data['FleetArn']);
        }
        // validate the optional field `ComputeName`
        if (data['ComputeName']) { // data not null
          String.validateJSON(data['ComputeName']);
        }
        // validate the optional field `ComputeArn`
        if (data['ComputeArn']) { // data not null
          String.validateJSON(data['ComputeArn']);
        }
        // validate the optional field `IpAddress`
        if (data['IpAddress']) { // data not null
          String.validateJSON(data['IpAddress']);
        }
        // validate the optional field `DnsName`
        if (data['DnsName']) { // data not null
          String.validateJSON(data['DnsName']);
        }
        // validate the optional field `ComputeStatus`
        if (data['ComputeStatus']) { // data not null
          ComputeStatus.validateJSON(data['ComputeStatus']);
        }
        // validate the optional field `Location`
        if (data['Location']) { // data not null
          String.validateJSON(data['Location']);
        }
        // validate the optional field `CreationTime`
        if (data['CreationTime']) { // data not null
          Date.validateJSON(data['CreationTime']);
        }
        // validate the optional field `OperatingSystem`
        if (data['OperatingSystem']) { // data not null
          OperatingSystem.validateJSON(data['OperatingSystem']);
        }
        // validate the optional field `Type`
        if (data['Type']) { // data not null
          EC2InstanceType.validateJSON(data['Type']);
        }
        // validate the optional field `GameLiftServiceSdkEndpoint`
        if (data['GameLiftServiceSdkEndpoint']) { // data not null
          String.validateJSON(data['GameLiftServiceSdkEndpoint']);
        }

        return true;
    }


}



/**
 * @member {String} FleetId
 */
DescribeComputeOutputCompute.prototype['FleetId'] = undefined;

/**
 * @member {String} FleetArn
 */
DescribeComputeOutputCompute.prototype['FleetArn'] = undefined;

/**
 * @member {String} ComputeName
 */
DescribeComputeOutputCompute.prototype['ComputeName'] = undefined;

/**
 * @member {String} ComputeArn
 */
DescribeComputeOutputCompute.prototype['ComputeArn'] = undefined;

/**
 * @member {String} IpAddress
 */
DescribeComputeOutputCompute.prototype['IpAddress'] = undefined;

/**
 * @member {String} DnsName
 */
DescribeComputeOutputCompute.prototype['DnsName'] = undefined;

/**
 * @member {module:model/ComputeStatus} ComputeStatus
 */
DescribeComputeOutputCompute.prototype['ComputeStatus'] = undefined;

/**
 * @member {String} Location
 */
DescribeComputeOutputCompute.prototype['Location'] = undefined;

/**
 * @member {Date} CreationTime
 */
DescribeComputeOutputCompute.prototype['CreationTime'] = undefined;

/**
 * @member {module:model/OperatingSystem} OperatingSystem
 */
DescribeComputeOutputCompute.prototype['OperatingSystem'] = undefined;

/**
 * @member {module:model/EC2InstanceType} Type
 */
DescribeComputeOutputCompute.prototype['Type'] = undefined;

/**
 * @member {String} GameLiftServiceSdkEndpoint
 */
DescribeComputeOutputCompute.prototype['GameLiftServiceSdkEndpoint'] = undefined;


// Implement Compute interface:
/**
 * @member {String} FleetId
 */
Compute.prototype['FleetId'] = undefined;
/**
 * @member {String} FleetArn
 */
Compute.prototype['FleetArn'] = undefined;
/**
 * @member {String} ComputeName
 */
Compute.prototype['ComputeName'] = undefined;
/**
 * @member {String} ComputeArn
 */
Compute.prototype['ComputeArn'] = undefined;
/**
 * @member {String} IpAddress
 */
Compute.prototype['IpAddress'] = undefined;
/**
 * @member {String} DnsName
 */
Compute.prototype['DnsName'] = undefined;
/**
 * @member {module:model/ComputeStatus} ComputeStatus
 */
Compute.prototype['ComputeStatus'] = undefined;
/**
 * @member {String} Location
 */
Compute.prototype['Location'] = undefined;
/**
 * @member {Date} CreationTime
 */
Compute.prototype['CreationTime'] = undefined;
/**
 * @member {module:model/OperatingSystem} OperatingSystem
 */
Compute.prototype['OperatingSystem'] = undefined;
/**
 * @member {module:model/EC2InstanceType} Type
 */
Compute.prototype['Type'] = undefined;
/**
 * @member {String} GameLiftServiceSdkEndpoint
 */
Compute.prototype['GameLiftServiceSdkEndpoint'] = undefined;




export default DescribeComputeOutputCompute;

