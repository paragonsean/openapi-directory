...
async def test_describe_stream(client):
    ...
    """
    body = {"ExclusiveStartShardId":"","Limit":"","StreamArn":""}
    headers = { 
    ...
    response = await client.request(
        method='POST',
        path='/#X-Amz-Target=DynamoDBStreams_20120810.DescribeStream',
        headers=headers,
        json=body,
        )
...
async def test_get_records(client):
    ...
    response = await client.request(
        method='POST',
        path='/#X-Amz-Target=DynamoDBStreams_20120810.GetRecords',
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_get_shard_iterator(client):
    """Test case for get_shard_iterator

    
    """
    body = {"ShardId":"","ShardIteratorType":"","SequenceNumber":"","StreamArn":""}
    headers = { 
        'Accept': 'application/json',
        'Content-Type': 'application/json',
        'x_amz_content_sha256': 'x_amz_content_sha256_example',
    ...
    response = await client.request(
        method='POST',
        path='/#X-Amz-Target=DynamoDBStreams_20120810.GetShardIterator',
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_list_streams(client):
    ...
    """
    body = {"TableName":"","ExclusiveStartStreamArn":"","Limit":""}
    headers = { 
        'Accept': 'application/json',
        'Content-Type': 'application/json',
        'x_amz_content_sha256': 'x_amz_content_sha256_example',
    ...
    response = await client.request(
        method='POST',
        path='/#X-Amz-Target=DynamoDBStreams_20120810.ListStreams',
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
