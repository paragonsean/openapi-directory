/**
 * AWS Marketplace Catalog Service
 * <p>Catalog API actions allow you to manage your entities through list, describe, and update capabilities. An entity can be a product or an offer on AWS Marketplace. </p> <p>You can automate your entity update process by integrating the AWS Marketplace Catalog API with your AWS Marketplace product build or deployment pipelines. You can also create your own applications on top of the Catalog API to manage your products on AWS Marketplace.</p>
 *
 * The version of the OpenAPI document: 2018-09-17
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIStartChangeSet_request.h
 *
 * 
 */

#ifndef OAIStartChangeSet_request_H
#define OAIStartChangeSet_request_H

#include <QJsonObject>

#include "OAIChange.h"
#include "OAITag.h"
#include <QList>
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {
class OAIChange;
class OAITag;

class OAIStartChangeSet_request : public OAIObject {
public:
    OAIStartChangeSet_request();
    OAIStartChangeSet_request(QString json);
    ~OAIStartChangeSet_request() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QString getCatalog() const;
    void setCatalog(const QString &catalog);
    bool is_catalog_Set() const;
    bool is_catalog_Valid() const;

    QList<OAIChange> getChangeSet() const;
    void setChangeSet(const QList<OAIChange> &change_set);
    bool is_change_set_Set() const;
    bool is_change_set_Valid() const;

    QString getChangeSetName() const;
    void setChangeSetName(const QString &change_set_name);
    bool is_change_set_name_Set() const;
    bool is_change_set_name_Valid() const;

    QString getClientRequestToken() const;
    void setClientRequestToken(const QString &client_request_token);
    bool is_client_request_token_Set() const;
    bool is_client_request_token_Valid() const;

    QList<OAITag> getChangeSetTags() const;
    void setChangeSetTags(const QList<OAITag> &change_set_tags);
    bool is_change_set_tags_Set() const;
    bool is_change_set_tags_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString m_catalog;
    bool m_catalog_isSet;
    bool m_catalog_isValid;

    QList<OAIChange> m_change_set;
    bool m_change_set_isSet;
    bool m_change_set_isValid;

    QString m_change_set_name;
    bool m_change_set_name_isSet;
    bool m_change_set_name_isValid;

    QString m_client_request_token;
    bool m_client_request_token_isSet;
    bool m_client_request_token_isValid;

    QList<OAITag> m_change_set_tags;
    bool m_change_set_tags_isSet;
    bool m_change_set_tags_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIStartChangeSet_request)

#endif // OAIStartChangeSet_request_H
