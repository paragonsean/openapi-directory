# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.get_cache_policy_config_result_cache_policy_config import GetCachePolicyConfigResultCachePolicyConfig
from openapi_server import util


class CachePolicy(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, id: str=None, last_modified_time: datetime=None, cache_policy_config: GetCachePolicyConfigResultCachePolicyConfig=None):
        """CachePolicy - a model defined in OpenAPI

        :param id: The id of this CachePolicy.
        :param last_modified_time: The last_modified_time of this CachePolicy.
        :param cache_policy_config: The cache_policy_config of this CachePolicy.
        """
        self.openapi_types = {
            'id': str,
            'last_modified_time': datetime,
            'cache_policy_config': GetCachePolicyConfigResultCachePolicyConfig
        }

        self.attribute_map = {
            'id': 'Id',
            'last_modified_time': 'LastModifiedTime',
            'cache_policy_config': 'CachePolicyConfig'
        }

        self._id = id
        self._last_modified_time = last_modified_time
        self._cache_policy_config = cache_policy_config

    @classmethod
    def from_dict(cls, dikt: dict) -> 'CachePolicy':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The CachePolicy of this CachePolicy.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def id(self):
        """Gets the id of this CachePolicy.


        :return: The id of this CachePolicy.
        :rtype: str
        """
        return self._id

    @id.setter
    def id(self, id):
        """Sets the id of this CachePolicy.


        :param id: The id of this CachePolicy.
        :type id: str
        """
        if id is None:
            raise ValueError("Invalid value for `id`, must not be `None`")

        self._id = id

    @property
    def last_modified_time(self):
        """Gets the last_modified_time of this CachePolicy.


        :return: The last_modified_time of this CachePolicy.
        :rtype: datetime
        """
        return self._last_modified_time

    @last_modified_time.setter
    def last_modified_time(self, last_modified_time):
        """Sets the last_modified_time of this CachePolicy.


        :param last_modified_time: The last_modified_time of this CachePolicy.
        :type last_modified_time: datetime
        """
        if last_modified_time is None:
            raise ValueError("Invalid value for `last_modified_time`, must not be `None`")

        self._last_modified_time = last_modified_time

    @property
    def cache_policy_config(self):
        """Gets the cache_policy_config of this CachePolicy.


        :return: The cache_policy_config of this CachePolicy.
        :rtype: GetCachePolicyConfigResultCachePolicyConfig
        """
        return self._cache_policy_config

    @cache_policy_config.setter
    def cache_policy_config(self, cache_policy_config):
        """Sets the cache_policy_config of this CachePolicy.


        :param cache_policy_config: The cache_policy_config of this CachePolicy.
        :type cache_policy_config: GetCachePolicyConfigResultCachePolicyConfig
        """
        if cache_policy_config is None:
            raise ValueError("Invalid value for `cache_policy_config`, must not be `None`")

        self._cache_policy_config = cache_policy_config
