/**
 * AWS Data Pipeline
 * <p>AWS Data Pipeline configures and manages a data-driven workflow called a pipeline. AWS Data Pipeline handles the details of scheduling and ensuring that data dependencies are met so that your application can focus on processing the data.</p> <p>AWS Data Pipeline provides a JAR implementation of a task runner called AWS Data Pipeline Task Runner. AWS Data Pipeline Task Runner provides logic for common data management scenarios, such as performing database queries and running data analysis using Amazon Elastic MapReduce (Amazon EMR). You can use AWS Data Pipeline Task Runner as your task runner, or you can write your own task runner to provide custom data management.</p> <p>AWS Data Pipeline implements two main sets of functionality. Use the first set to create a pipeline and define data sources, schedules, dependencies, and the transforms to be performed on the data. Use the second set in your task runner application to receive the next task ready for processing. The logic for performing the task, such as querying the data, running data analysis, or converting the data from one format to another, is contained within the task runner. The task runner performs the task assigned to it by the web service, reporting progress to the web service as it does so. When the task is done, the task runner reports the final success or failure of the task to the web service.</p>
 *
 * The version of the OpenAPI document: 2012-10-29
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIQueryObjectsOutput.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIQueryObjectsOutput::OAIQueryObjectsOutput(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIQueryObjectsOutput::OAIQueryObjectsOutput() {
    this->initializeModel();
}

OAIQueryObjectsOutput::~OAIQueryObjectsOutput() {}

void OAIQueryObjectsOutput::initializeModel() {

    m_ids_isSet = false;
    m_ids_isValid = false;

    m_marker_isSet = false;
    m_marker_isValid = false;

    m_has_more_results_isSet = false;
    m_has_more_results_isValid = false;
}

void OAIQueryObjectsOutput::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAIQueryObjectsOutput::fromJsonObject(QJsonObject json) {

    m_ids_isValid = ::OpenAPI::fromJsonValue(m_ids, json[QString("ids")]);
    m_ids_isSet = !json[QString("ids")].isNull() && m_ids_isValid;

    m_marker_isValid = ::OpenAPI::fromJsonValue(m_marker, json[QString("marker")]);
    m_marker_isSet = !json[QString("marker")].isNull() && m_marker_isValid;

    m_has_more_results_isValid = ::OpenAPI::fromJsonValue(m_has_more_results, json[QString("hasMoreResults")]);
    m_has_more_results_isSet = !json[QString("hasMoreResults")].isNull() && m_has_more_results_isValid;
}

QString OAIQueryObjectsOutput::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAIQueryObjectsOutput::asJsonObject() const {
    QJsonObject obj;
    if (m_ids.isSet()) {
        obj.insert(QString("ids"), ::OpenAPI::toJsonValue(m_ids));
    }
    if (m_marker_isSet) {
        obj.insert(QString("marker"), ::OpenAPI::toJsonValue(m_marker));
    }
    if (m_has_more_results_isSet) {
        obj.insert(QString("hasMoreResults"), ::OpenAPI::toJsonValue(m_has_more_results));
    }
    return obj;
}

QList OAIQueryObjectsOutput::getIds() const {
    return m_ids;
}
void OAIQueryObjectsOutput::setIds(const QList &ids) {
    m_ids = ids;
    m_ids_isSet = true;
}

bool OAIQueryObjectsOutput::is_ids_Set() const{
    return m_ids_isSet;
}

bool OAIQueryObjectsOutput::is_ids_Valid() const{
    return m_ids_isValid;
}

QString OAIQueryObjectsOutput::getMarker() const {
    return m_marker;
}
void OAIQueryObjectsOutput::setMarker(const QString &marker) {
    m_marker = marker;
    m_marker_isSet = true;
}

bool OAIQueryObjectsOutput::is_marker_Set() const{
    return m_marker_isSet;
}

bool OAIQueryObjectsOutput::is_marker_Valid() const{
    return m_marker_isValid;
}

bool OAIQueryObjectsOutput::getHasMoreResults() const {
    return m_has_more_results;
}
void OAIQueryObjectsOutput::setHasMoreResults(const bool &has_more_results) {
    m_has_more_results = has_more_results;
    m_has_more_results_isSet = true;
}

bool OAIQueryObjectsOutput::is_has_more_results_Set() const{
    return m_has_more_results_isSet;
}

bool OAIQueryObjectsOutput::is_has_more_results_Valid() const{
    return m_has_more_results_isValid;
}

bool OAIQueryObjectsOutput::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_ids.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_marker_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_has_more_results_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAIQueryObjectsOutput::isValid() const {
    // only required properties are required for the object to be considered valid
    return true;
}

} // namespace OpenAPI
