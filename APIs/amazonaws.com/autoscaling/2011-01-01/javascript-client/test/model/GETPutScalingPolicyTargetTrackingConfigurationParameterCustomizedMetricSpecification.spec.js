/**
 * Auto Scaling
 * <fullname>Amazon EC2 Auto Scaling</fullname> <p>Amazon EC2 Auto Scaling is designed to automatically launch and terminate EC2 instances based on user-defined scaling policies, scheduled actions, and health checks.</p> <p>For more information, see the <a href=\"https://docs.aws.amazon.com/autoscaling/ec2/userguide/\">Amazon EC2 Auto Scaling User Guide</a> and the <a href=\"https://docs.aws.amazon.com/autoscaling/ec2/APIReference/Welcome.html\">Amazon EC2 Auto Scaling API Reference</a>.</p>
 *
 * The version of the OpenAPI document: 2011-01-01
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', process.cwd()+'/src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require(process.cwd()+'/src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.AutoScaling);
  }
}(this, function(expect, AutoScaling) {
  'use strict';

  var instance;

  beforeEach(function() {
    instance = new AutoScaling.GETPutScalingPolicyTargetTrackingConfigurationParameterCustomizedMetricSpecification();
  });

  var getProperty = function(object, getter, property) {
    // Use getter method if present; otherwise, get the property directly.
    if (typeof object[getter] === 'function')
      return object[getter]();
    else
      return object[property];
  }

  var setProperty = function(object, setter, property, value) {
    // Use setter method if present; otherwise, set the property directly.
    if (typeof object[setter] === 'function')
      object[setter](value);
    else
      object[property] = value;
  }

  describe('GETPutScalingPolicyTargetTrackingConfigurationParameterCustomizedMetricSpecification', function() {
    it('should create an instance of GETPutScalingPolicyTargetTrackingConfigurationParameterCustomizedMetricSpecification', function() {
      // uncomment below and update the code to test GETPutScalingPolicyTargetTrackingConfigurationParameterCustomizedMetricSpecification
      //var instance = new AutoScaling.GETPutScalingPolicyTargetTrackingConfigurationParameterCustomizedMetricSpecification();
      //expect(instance).to.be.a(AutoScaling.GETPutScalingPolicyTargetTrackingConfigurationParameterCustomizedMetricSpecification);
    });

    it('should have the property metricName (base name: "MetricName")', function() {
      // uncomment below and update the code to test the property metricName
      //var instance = new AutoScaling.GETPutScalingPolicyTargetTrackingConfigurationParameterCustomizedMetricSpecification();
      //expect(instance).to.be();
    });

    it('should have the property namespace (base name: "Namespace")', function() {
      // uncomment below and update the code to test the property namespace
      //var instance = new AutoScaling.GETPutScalingPolicyTargetTrackingConfigurationParameterCustomizedMetricSpecification();
      //expect(instance).to.be();
    });

    it('should have the property dimensions (base name: "Dimensions")', function() {
      // uncomment below and update the code to test the property dimensions
      //var instance = new AutoScaling.GETPutScalingPolicyTargetTrackingConfigurationParameterCustomizedMetricSpecification();
      //expect(instance).to.be();
    });

    it('should have the property statistic (base name: "Statistic")', function() {
      // uncomment below and update the code to test the property statistic
      //var instance = new AutoScaling.GETPutScalingPolicyTargetTrackingConfigurationParameterCustomizedMetricSpecification();
      //expect(instance).to.be();
    });

    it('should have the property unit (base name: "Unit")', function() {
      // uncomment below and update the code to test the property unit
      //var instance = new AutoScaling.GETPutScalingPolicyTargetTrackingConfigurationParameterCustomizedMetricSpecification();
      //expect(instance).to.be();
    });

    it('should have the property metrics (base name: "Metrics")', function() {
      // uncomment below and update the code to test the property metrics
      //var instance = new AutoScaling.GETPutScalingPolicyTargetTrackingConfigurationParameterCustomizedMetricSpecification();
      //expect(instance).to.be();
    });

  });

}));
