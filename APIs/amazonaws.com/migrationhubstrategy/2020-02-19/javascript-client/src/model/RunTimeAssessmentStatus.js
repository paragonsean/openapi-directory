/**
 * Migration Hub Strategy Recommendations
 * <p><fullname>Migration Hub Strategy Recommendations</fullname> <p>This API reference provides descriptions, syntax, and other details about each of the actions and data types for Migration Hub Strategy Recommendations (Strategy Recommendations). The topic for each action shows the API request parameters and the response. Alternatively, you can use one of the AWS SDKs to access an API that is tailored to the programming language or platform that you're using. For more information, see <a href=\"http://aws.amazon.com/tools/#SDKs\">AWS SDKs</a>.</p></p>
 *
 * The version of the OpenAPI document: 2020-02-19
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
/**
* Enum class RunTimeAssessmentStatus.
* @enum {}
* @readonly
*/
export default class RunTimeAssessmentStatus {
    
        /**
         * value: "dataCollectionTaskToBeScheduled"
         * @const
         */
        "dataCollectionTaskToBeScheduled" = "dataCollectionTaskToBeScheduled";

    
        /**
         * value: "dataCollectionTaskScheduled"
         * @const
         */
        "dataCollectionTaskScheduled" = "dataCollectionTaskScheduled";

    
        /**
         * value: "dataCollectionTaskStarted"
         * @const
         */
        "dataCollectionTaskStarted" = "dataCollectionTaskStarted";

    
        /**
         * value: "dataCollectionTaskStopped"
         * @const
         */
        "dataCollectionTaskStopped" = "dataCollectionTaskStopped";

    
        /**
         * value: "dataCollectionTaskSuccess"
         * @const
         */
        "dataCollectionTaskSuccess" = "dataCollectionTaskSuccess";

    
        /**
         * value: "dataCollectionTaskFailed"
         * @const
         */
        "dataCollectionTaskFailed" = "dataCollectionTaskFailed";

    
        /**
         * value: "dataCollectionTaskPartialSuccess"
         * @const
         */
        "dataCollectionTaskPartialSuccess" = "dataCollectionTaskPartialSuccess";

    

    /**
    * Returns a <code>RunTimeAssessmentStatus</code> enum value from a Javascript object name.
    * @param {Object} data The plain JavaScript object containing the name of the enum value.
    * @return {module:model/RunTimeAssessmentStatus} The enum <code>RunTimeAssessmentStatus</code> value.
    */
    static constructFromObject(object) {
        return object;
    }
}

