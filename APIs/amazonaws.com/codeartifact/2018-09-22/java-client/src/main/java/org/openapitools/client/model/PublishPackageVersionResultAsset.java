...
public class PublishPackageVersionResultAsset {
  ...
  public PublishPackageVersionResultAsset() {
  }

  public PublishPackageVersionResultAsset name(String name) {
    this.name = name;
    return this;
  }
  ...
  public PublishPackageVersionResultAsset size(Integer size) {
    this.size = size;
    return this;
  }
  ...
  public PublishPackageVersionResultAsset hashes(Map hashes) {
    this.hashes = hashes;
    return this;
  }
  ...
  public boolean equals(Object o) {
    ...
    PublishPackageVersionResultAsset publishPackageVersionResultAsset = (PublishPackageVersionResultAsset) o;
    return Objects.equals(this.name, publishPackageVersionResultAsset.name) &&
        Objects.equals(this.size, publishPackageVersionResultAsset.size) &&
        Objects.equals(this.hashes, publishPackageVersionResultAsset.hashes);
  }

  @Override
  public int hashCode() {
    return Objects.hash(name, size, hashes);
  }

  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class PublishPackageVersionResultAsset {\n");
   ...
   * @throws IOException if the JSON Element is invalid with respect to PublishPackageVersionResultAsset
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PublishPackageVersionResultAsset.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PublishPackageVersionResultAsset is not found in the empty JSON string", PublishPackageVersionResultAsset.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PublishPackageVersionResultAsset.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PublishPackageVersionResultAsset` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : PublishPackageVersionResultAsset.openapiRequiredFields) {
      ...
      if (jsonObj.get("hashes") != null && !jsonObj.get("hashes").isJsonNull()) {
        Map.validateJsonElement(jsonObj.get("hashes"));
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PublishPackageVersionResultAsset.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PublishPackageVersionResultAsset' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PublishPackageVersionResultAsset> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PublishPackageVersionResultAsset.class));

       return (TypeAdapter<T>) new TypeAdapter<PublishPackageVersionResultAsset>() {
           @Override
           public void write(JsonWriter out, PublishPackageVersionResultAsset value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PublishPackageVersionResultAsset read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of PublishPackageVersionResultAsset given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of PublishPackageVersionResultAsset
   * @throws IOException if the JSON string is invalid with respect to PublishPackageVersionResultAsset
   */
  public static PublishPackageVersionResultAsset fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PublishPackageVersionResultAsset.class);
  }

  /**
   * Convert an instance of PublishPackageVersionResultAsset to an JSON string
   *
   * @return JSON string
   */
