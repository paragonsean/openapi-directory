...
public class ResourceNotFoundException {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ResourceNotFoundException
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResourceNotFoundException.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ResourceNotFoundException.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResourceNotFoundException` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
