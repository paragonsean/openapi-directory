# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.grpc_retry_policy_per_retry_timeout import GrpcRetryPolicyPerRetryTimeout
from openapi_server import util


class HttpRouteRetryPolicy(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, http_retry_events: List=None, max_retries: int=None, per_retry_timeout: GrpcRetryPolicyPerRetryTimeout=None, tcp_retry_events: List=None):
        """HttpRouteRetryPolicy - a model defined in OpenAPI

        :param http_retry_events: The http_retry_events of this HttpRouteRetryPolicy.
        :param max_retries: The max_retries of this HttpRouteRetryPolicy.
        :param per_retry_timeout: The per_retry_timeout of this HttpRouteRetryPolicy.
        :param tcp_retry_events: The tcp_retry_events of this HttpRouteRetryPolicy.
        """
        self.openapi_types = {
            'http_retry_events': List,
            'max_retries': int,
            'per_retry_timeout': GrpcRetryPolicyPerRetryTimeout,
            'tcp_retry_events': List
        }

        self.attribute_map = {
            'http_retry_events': 'httpRetryEvents',
            'max_retries': 'maxRetries',
            'per_retry_timeout': 'perRetryTimeout',
            'tcp_retry_events': 'tcpRetryEvents'
        }

        self._http_retry_events = http_retry_events
        self._max_retries = max_retries
        self._per_retry_timeout = per_retry_timeout
        self._tcp_retry_events = tcp_retry_events

    @classmethod
    def from_dict(cls, dikt: dict) -> 'HttpRouteRetryPolicy':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The HttpRoute_retryPolicy of this HttpRouteRetryPolicy.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def http_retry_events(self):
        """Gets the http_retry_events of this HttpRouteRetryPolicy.


        :return: The http_retry_events of this HttpRouteRetryPolicy.
        :rtype: List
        """
        return self._http_retry_events

    @http_retry_events.setter
    def http_retry_events(self, http_retry_events):
        """Sets the http_retry_events of this HttpRouteRetryPolicy.


        :param http_retry_events: The http_retry_events of this HttpRouteRetryPolicy.
        :type http_retry_events: List
        """

        self._http_retry_events = http_retry_events

    @property
    def max_retries(self):
        """Gets the max_retries of this HttpRouteRetryPolicy.


        :return: The max_retries of this HttpRouteRetryPolicy.
        :rtype: int
        """
        return self._max_retries

    @max_retries.setter
    def max_retries(self, max_retries):
        """Sets the max_retries of this HttpRouteRetryPolicy.


        :param max_retries: The max_retries of this HttpRouteRetryPolicy.
        :type max_retries: int
        """
        if max_retries is None:
            raise ValueError("Invalid value for `max_retries`, must not be `None`")

        self._max_retries = max_retries

    @property
    def per_retry_timeout(self):
        """Gets the per_retry_timeout of this HttpRouteRetryPolicy.


        :return: The per_retry_timeout of this HttpRouteRetryPolicy.
        :rtype: GrpcRetryPolicyPerRetryTimeout
        """
        return self._per_retry_timeout

    @per_retry_timeout.setter
    def per_retry_timeout(self, per_retry_timeout):
        """Sets the per_retry_timeout of this HttpRouteRetryPolicy.


        :param per_retry_timeout: The per_retry_timeout of this HttpRouteRetryPolicy.
        :type per_retry_timeout: GrpcRetryPolicyPerRetryTimeout
        """
        if per_retry_timeout is None:
            raise ValueError("Invalid value for `per_retry_timeout`, must not be `None`")

        self._per_retry_timeout = per_retry_timeout

    @property
    def tcp_retry_events(self):
        """Gets the tcp_retry_events of this HttpRouteRetryPolicy.


        :return: The tcp_retry_events of this HttpRouteRetryPolicy.
        :rtype: List
        """
        return self._tcp_retry_events

    @tcp_retry_events.setter
    def tcp_retry_events(self, tcp_retry_events):
        """Sets the tcp_retry_events of this HttpRouteRetryPolicy.


        :param tcp_retry_events: The tcp_retry_events of this HttpRouteRetryPolicy.
        :type tcp_retry_events: List
        """

        self._tcp_retry_events = tcp_retry_events
