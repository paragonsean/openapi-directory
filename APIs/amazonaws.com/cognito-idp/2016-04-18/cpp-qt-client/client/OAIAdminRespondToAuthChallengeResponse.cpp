/**
 * Amazon Cognito Identity Provider
 * <p>With the Amazon Cognito user pools API, you can set up user pools and app clients, and authenticate users. To authenticate users from third-party identity providers (IdPs) in this API, you can <a href=\"https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-pools-identity-federation-consolidate-users.html\">link IdP users to native user profiles</a>. Learn more about the authentication and authorization of federated users in the <a href=\"https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-userpools-server-contract-reference.html\">Using the Amazon Cognito user pools API and user pool endpoints</a>.</p> <p>This API reference provides detailed information about API operations and object types in Amazon Cognito. At the bottom of the page for each API operation and object, under <i>See Also</i>, you can learn how to use it in an Amazon Web Services SDK in the language of your choice.</p> <p>Along with resource management operations, the Amazon Cognito user pools API includes classes of operations and authorization models for client-side and server-side user operations. For more information, see <a href=\"https://docs.aws.amazon.com/cognito/latest/developerguide/user-pools-API-operations.html\">Using the Amazon Cognito native and OIDC APIs</a> in the <i>Amazon Cognito Developer Guide</i>.</p> <p>You can also start reading about the <code>CognitoIdentityProvider</code> client in the following SDK guides.</p> <ul> <li> <p> <a href=\"https://docs.aws.amazon.com/cli/latest/reference/cognito-idp/index.html#cli-aws-cognito-idp\">Amazon Web Services Command Line Interface</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/sdkfornet/v3/apidocs/items/CognitoIdentityProvider/TCognitoIdentityProviderClient.html\">Amazon Web Services SDK for .NET</a> </p> </li> <li> <p> <a href=\"https://sdk.amazonaws.com/cpp/api/LATEST/aws-cpp-sdk-cognito-idp/html/class_aws_1_1_cognito_identity_provider_1_1_cognito_identity_provider_client.html\">Amazon Web Services SDK for C++</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/sdk-for-go/api/service/cognitoidentityprovider/#CognitoIdentityProvider\">Amazon Web Services SDK for Go</a> </p> </li> <li> <p> <a href=\"https://sdk.amazonaws.com/java/api/latest/software/amazon/awssdk/services/cognitoidentityprovider/CognitoIdentityProviderClient.html\">Amazon Web Services SDK for Java V2</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/AWSJavaScriptSDK/latest/AWS/CognitoIdentityServiceProvider.html\">Amazon Web Services SDK for JavaScript</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/aws-sdk-php/v3/api/api-cognito-idp-2016-04-18.html\">Amazon Web Services SDK for PHP V3</a> </p> </li> <li> <p> <a href=\"https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/cognito-idp.html\">Amazon Web Services SDK for Python</a> </p> </li> <li> <p> <a href=\"https://docs.aws.amazon.com/sdk-for-ruby/v3/api/Aws/CognitoIdentityProvider/Client.html\">Amazon Web Services SDK for Ruby V3</a> </p> </li> </ul> <p>To get started with an Amazon Web Services SDK, see <a href=\"http://aws.amazon.com/developer/tools/\">Tools to Build on Amazon Web Services</a>. For example actions and scenarios, see <a href=\"https://docs.aws.amazon.com/cognito/latest/developerguide/service_code_examples_cognito-identity-provider.html\">Code examples for Amazon Cognito Identity Provider using Amazon Web Services SDKs</a>.</p>
 *
 * The version of the OpenAPI document: 2016-04-18
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIAdminRespondToAuthChallengeResponse.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIAdminRespondToAuthChallengeResponse::OAIAdminRespondToAuthChallengeResponse(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIAdminRespondToAuthChallengeResponse::OAIAdminRespondToAuthChallengeResponse() {
    this->initializeModel();
}

OAIAdminRespondToAuthChallengeResponse::~OAIAdminRespondToAuthChallengeResponse() {}

void OAIAdminRespondToAuthChallengeResponse::initializeModel() {

    m_challenge_name_isSet = false;
    m_challenge_name_isValid = false;

    m_session_isSet = false;
    m_session_isValid = false;

    m_challenge_parameters_isSet = false;
    m_challenge_parameters_isValid = false;

    m_authentication_result_isSet = false;
    m_authentication_result_isValid = false;
}

void OAIAdminRespondToAuthChallengeResponse::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAIAdminRespondToAuthChallengeResponse::fromJsonObject(QJsonObject json) {

    m_challenge_name_isValid = ::OpenAPI::fromJsonValue(m_challenge_name, json[QString("ChallengeName")]);
    m_challenge_name_isSet = !json[QString("ChallengeName")].isNull() && m_challenge_name_isValid;

    m_session_isValid = ::OpenAPI::fromJsonValue(m_session, json[QString("Session")]);
    m_session_isSet = !json[QString("Session")].isNull() && m_session_isValid;

    m_challenge_parameters_isValid = ::OpenAPI::fromJsonValue(m_challenge_parameters, json[QString("ChallengeParameters")]);
    m_challenge_parameters_isSet = !json[QString("ChallengeParameters")].isNull() && m_challenge_parameters_isValid;

    m_authentication_result_isValid = ::OpenAPI::fromJsonValue(m_authentication_result, json[QString("AuthenticationResult")]);
    m_authentication_result_isSet = !json[QString("AuthenticationResult")].isNull() && m_authentication_result_isValid;
}

QString OAIAdminRespondToAuthChallengeResponse::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAIAdminRespondToAuthChallengeResponse::asJsonObject() const {
    QJsonObject obj;
    if (m_challenge_name.isSet()) {
        obj.insert(QString("ChallengeName"), ::OpenAPI::toJsonValue(m_challenge_name));
    }
    if (m_session_isSet) {
        obj.insert(QString("Session"), ::OpenAPI::toJsonValue(m_session));
    }
    if (m_challenge_parameters.isSet()) {
        obj.insert(QString("ChallengeParameters"), ::OpenAPI::toJsonValue(m_challenge_parameters));
    }
    if (m_authentication_result.isSet()) {
        obj.insert(QString("AuthenticationResult"), ::OpenAPI::toJsonValue(m_authentication_result));
    }
    return obj;
}

OAIChallengeNameType OAIAdminRespondToAuthChallengeResponse::getChallengeName() const {
    return m_challenge_name;
}
void OAIAdminRespondToAuthChallengeResponse::setChallengeName(const OAIChallengeNameType &challenge_name) {
    m_challenge_name = challenge_name;
    m_challenge_name_isSet = true;
}

bool OAIAdminRespondToAuthChallengeResponse::is_challenge_name_Set() const{
    return m_challenge_name_isSet;
}

bool OAIAdminRespondToAuthChallengeResponse::is_challenge_name_Valid() const{
    return m_challenge_name_isValid;
}

QString OAIAdminRespondToAuthChallengeResponse::getSession() const {
    return m_session;
}
void OAIAdminRespondToAuthChallengeResponse::setSession(const QString &session) {
    m_session = session;
    m_session_isSet = true;
}

bool OAIAdminRespondToAuthChallengeResponse::is_session_Set() const{
    return m_session_isSet;
}

bool OAIAdminRespondToAuthChallengeResponse::is_session_Valid() const{
    return m_session_isValid;
}

QMap OAIAdminRespondToAuthChallengeResponse::getChallengeParameters() const {
    return m_challenge_parameters;
}
void OAIAdminRespondToAuthChallengeResponse::setChallengeParameters(const QMap &challenge_parameters) {
    m_challenge_parameters = challenge_parameters;
    m_challenge_parameters_isSet = true;
}

bool OAIAdminRespondToAuthChallengeResponse::is_challenge_parameters_Set() const{
    return m_challenge_parameters_isSet;
}

bool OAIAdminRespondToAuthChallengeResponse::is_challenge_parameters_Valid() const{
    return m_challenge_parameters_isValid;
}

OAIAdminRespondToAuthChallengeResponse_AuthenticationResult OAIAdminRespondToAuthChallengeResponse::getAuthenticationResult() const {
    return m_authentication_result;
}
void OAIAdminRespondToAuthChallengeResponse::setAuthenticationResult(const OAIAdminRespondToAuthChallengeResponse_AuthenticationResult &authentication_result) {
    m_authentication_result = authentication_result;
    m_authentication_result_isSet = true;
}

bool OAIAdminRespondToAuthChallengeResponse::is_authentication_result_Set() const{
    return m_authentication_result_isSet;
}

bool OAIAdminRespondToAuthChallengeResponse::is_authentication_result_Valid() const{
    return m_authentication_result_isValid;
}

bool OAIAdminRespondToAuthChallengeResponse::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_challenge_name.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_session_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_challenge_parameters.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_authentication_result.isSet()) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAIAdminRespondToAuthChallengeResponse::isValid() const {
    // only required properties are required for the object to be considered valid
    return true;
}

} // namespace OpenAPI
