...
public class AttributeType {
  ...
  public AttributeType value(String value) {
    this.value = value;
    return this;
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to AttributeType
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AttributeType.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in AttributeType is not found in the empty JSON string", AttributeType.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!AttributeType.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AttributeType` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : AttributeType.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!AttributeType.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'AttributeType' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<AttributeType> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(AttributeType.class));

       return (TypeAdapter<T>) new TypeAdapter<AttributeType>() {
           @Override
  ...
  /**
   * Create an instance of AttributeType given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of AttributeType
   * @throws IOException if the JSON string is invalid with respect to AttributeType
   */
  public static AttributeType fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, AttributeType.class);
  }

  /**
   * Convert an instance of AttributeType to an JSON string
   *
   * @return JSON string
   */
