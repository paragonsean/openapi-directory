/**
 * Amazon Lex Model Building Service
 * <fullname>Amazon Lex Build-Time Actions</fullname> <p> Amazon Lex is an AWS service for building conversational voice and text interfaces. Use these actions to create, update, and delete conversational bots for new and existing client applications. </p>
 *
 * The version of the OpenAPI document: 2017-04-19
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', process.cwd()+'/src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require(process.cwd()+'/src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.AmazonLexModelBuildingService);
  }
}(this, function(expect, AmazonLexModelBuildingService) {
  'use strict';

  var instance;

  beforeEach(function() {
    instance = new AmazonLexModelBuildingService.LogSettingsResponse();
  });

  var getProperty = function(object, getter, property) {
    // Use getter method if present; otherwise, get the property directly.
    if (typeof object[getter] === 'function')
      return object[getter]();
    else
      return object[property];
  }

  var setProperty = function(object, setter, property, value) {
    // Use setter method if present; otherwise, set the property directly.
    if (typeof object[setter] === 'function')
      object[setter](value);
    else
      object[property] = value;
  }

  describe('LogSettingsResponse', function() {
    it('should create an instance of LogSettingsResponse', function() {
      // uncomment below and update the code to test LogSettingsResponse
      //var instance = new AmazonLexModelBuildingService.LogSettingsResponse();
      //expect(instance).to.be.a(AmazonLexModelBuildingService.LogSettingsResponse);
    });

    it('should have the property logType (base name: "logType")', function() {
      // uncomment below and update the code to test the property logType
      //var instance = new AmazonLexModelBuildingService.LogSettingsResponse();
      //expect(instance).to.be();
    });

    it('should have the property destination (base name: "destination")', function() {
      // uncomment below and update the code to test the property destination
      //var instance = new AmazonLexModelBuildingService.LogSettingsResponse();
      //expect(instance).to.be();
    });

    it('should have the property kmsKeyArn (base name: "kmsKeyArn")', function() {
      // uncomment below and update the code to test the property kmsKeyArn
      //var instance = new AmazonLexModelBuildingService.LogSettingsResponse();
      //expect(instance).to.be();
    });

    it('should have the property resourceArn (base name: "resourceArn")', function() {
      // uncomment below and update the code to test the property resourceArn
      //var instance = new AmazonLexModelBuildingService.LogSettingsResponse();
      //expect(instance).to.be();
    });

    it('should have the property resourcePrefix (base name: "resourcePrefix")', function() {
      // uncomment below and update the code to test the property resourcePrefix
      //var instance = new AmazonLexModelBuildingService.LogSettingsResponse();
      //expect(instance).to.be();
    });

  });

}));
