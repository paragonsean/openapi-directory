...
public class CancelHandshakeRequest {
  ...
  public CancelHandshakeRequest handshakeId(String handshakeId) {
   ...
   * @throws IOException if the JSON Element is invalid with respect to CancelHandshakeRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CancelHandshakeRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in CancelHandshakeRequest is not found in the empty JSON string", CancelHandshakeRequest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CancelHandshakeRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CancelHandshakeRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : CancelHandshakeRequest.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!CancelHandshakeRequest.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'CancelHandshakeRequest' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<CancelHandshakeRequest> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(CancelHandshakeRequest.class));

       return (TypeAdapter<T>) new TypeAdapter<CancelHandshakeRequest>() {
           @Override
           public void write(JsonWriter out, CancelHandshakeRequest value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public CancelHandshakeRequest read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of CancelHandshakeRequest given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of CancelHandshakeRequest
   * @throws IOException if the JSON string is invalid with respect to CancelHandshakeRequest
   */
  public static CancelHandshakeRequest fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, CancelHandshakeRequest.class);
  }

  /**
   * Convert an instance of CancelHandshakeRequest to an JSON string
   *
   * @return JSON string
   */
