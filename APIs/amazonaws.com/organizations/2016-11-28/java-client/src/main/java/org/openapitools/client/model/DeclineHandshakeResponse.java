...
public class DeclineHandshakeResponse {
  ...
  private DeclineHandshakeResponseHandshake handshake;

  public DeclineHandshakeResponse() {
  }

  public DeclineHandshakeResponse handshake(DeclineHandshakeResponseHandshake handshake) {
  ...
  public void setHandshake(DeclineHandshakeResponseHandshake handshake) {
    this.handshake = handshake;
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to DeclineHandshakeResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DeclineHandshakeResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in DeclineHandshakeResponse is not found in the empty JSON string", DeclineHandshakeResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DeclineHandshakeResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DeclineHandshakeResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // validate the optional field `Handshake`
      if (jsonObj.get("Handshake") != null && !jsonObj.get("Handshake").isJsonNull()) {
        DeclineHandshakeResponseHandshake.validateJsonElement(jsonObj.get("Handshake"));
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!DeclineHandshakeResponse.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'DeclineHandshakeResponse' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<DeclineHandshakeResponse> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(DeclineHandshakeResponse.class));

       return (TypeAdapter<T>) new TypeAdapter<DeclineHandshakeResponse>() {
           @Override
           public void write(JsonWriter out, DeclineHandshakeResponse value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public DeclineHandshakeResponse read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of DeclineHandshakeResponse given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of DeclineHandshakeResponse
   * @throws IOException if the JSON string is invalid with respect to DeclineHandshakeResponse
   */
  public static DeclineHandshakeResponse fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, DeclineHandshakeResponse.class);
  }

  /**
   * Convert an instance of DeclineHandshakeResponse to an JSON string
   *
   * @return JSON string
   */
