...
import org.openapitools.client.model.UpdatePolicyResponsePolicy;
...
public class UpdatePolicyResponse {
  ...
  private UpdatePolicyResponsePolicy policy;

  public UpdatePolicyResponse() {
  }

  public UpdatePolicyResponse policy(UpdatePolicyResponsePolicy policy) {
    this.policy = policy;
    return this;
  }
  ...
  public UpdatePolicyResponsePolicy getPolicy() {
    return policy;
  }

  public void setPolicy(UpdatePolicyResponsePolicy policy) {
    this.policy = policy;
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to UpdatePolicyResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UpdatePolicyResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in UpdatePolicyResponse is not found in the empty JSON string", UpdatePolicyResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!UpdatePolicyResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UpdatePolicyResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // validate the optional field `Policy`
      if (jsonObj.get("Policy") != null && !jsonObj.get("Policy").isJsonNull()) {
        UpdatePolicyResponsePolicy.validateJsonElement(jsonObj.get("Policy"));
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!UpdatePolicyResponse.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'UpdatePolicyResponse' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<UpdatePolicyResponse> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(UpdatePolicyResponse.class));

       return (TypeAdapter<T>) new TypeAdapter<UpdatePolicyResponse>() {
           @Override
           public void write(JsonWriter out, UpdatePolicyResponse value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public UpdatePolicyResponse read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of UpdatePolicyResponse given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of UpdatePolicyResponse
   * @throws IOException if the JSON string is invalid with respect to UpdatePolicyResponse
   */
  public static UpdatePolicyResponse fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, UpdatePolicyResponse.class);
  }

  /**
   * Convert an instance of UpdatePolicyResponse to an JSON string
   *
   * @return JSON string
   */
