...
public class DisablePolicyTypeResponse {
  ...
  public DisablePolicyTypeResponse root(DisablePolicyTypeResponseRoot root) {
    this.root = root;
    return this;
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to DisablePolicyTypeResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DisablePolicyTypeResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in DisablePolicyTypeResponse is not found in the empty JSON string", DisablePolicyTypeResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DisablePolicyTypeResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DisablePolicyTypeResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!DisablePolicyTypeResponse.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'DisablePolicyTypeResponse' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<DisablePolicyTypeResponse> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(DisablePolicyTypeResponse.class));

       return (TypeAdapter<T>) new TypeAdapter<DisablePolicyTypeResponse>() {
           @Override
           public void write(JsonWriter out, DisablePolicyTypeResponse value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public DisablePolicyTypeResponse read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of DisablePolicyTypeResponse given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of DisablePolicyTypeResponse
   * @throws IOException if the JSON string is invalid with respect to DisablePolicyTypeResponse
   */
  public static DisablePolicyTypeResponse fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, DisablePolicyTypeResponse.class);
  }

  /**
   * Convert an instance of DisablePolicyTypeResponse to an JSON string
   *
   * @return JSON string
   */
