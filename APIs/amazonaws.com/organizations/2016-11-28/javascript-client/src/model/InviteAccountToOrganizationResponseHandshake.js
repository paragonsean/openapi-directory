/**
 * AWS Organizations
 * <p>Organizations is a web service that enables you to consolidate your multiple Amazon Web Services accounts into an <i>organization</i> and centrally manage your accounts and their resources.</p> <p>This guide provides descriptions of the Organizations operations. For more information about using this service, see the <a href=\"https://docs.aws.amazon.com/organizations/latest/userguide/orgs_introduction.html\">Organizations User Guide</a>.</p> <p> <b>Support and feedback for Organizations</b> </p> <p>We welcome your feedback. Send your comments to <a href=\"mailto:feedback-awsorganizations@amazon.com\">feedback-awsorganizations@amazon.com</a> or post your feedback and questions in the <a href=\"http://forums.aws.amazon.com/forum.jspa?forumID=219\">Organizations support forum</a>. For more information about the Amazon Web Services support forums, see <a href=\"http://forums.aws.amazon.com/help.jspa\">Forums Help</a>.</p> <p> <b>Endpoint to call When using the CLI or the Amazon Web Services SDK</b> </p> <p>For the current release of Organizations, specify the <code>us-east-1</code> region for all Amazon Web Services API and CLI calls made from the commercial Amazon Web Services Regions outside of China. If calling from one of the Amazon Web Services Regions in China, then specify <code>cn-northwest-1</code>. You can do this in the CLI by using these parameters and commands:</p> <ul> <li> <p>Use the following parameter with each command to specify both the endpoint and its region:</p> <p> <code>--endpoint-url https://organizations.us-east-1.amazonaws.com</code> <i>(from commercial Amazon Web Services Regions outside of China)</i> </p> <p>or</p> <p> <code>--endpoint-url https://organizations.cn-northwest-1.amazonaws.com.cn</code> <i>(from Amazon Web Services Regions in China)</i> </p> </li> <li> <p>Use the default endpoint, but configure your default region with this command:</p> <p> <code>aws configure set default.region us-east-1</code> <i>(from commercial Amazon Web Services Regions outside of China)</i> </p> <p>or</p> <p> <code>aws configure set default.region cn-northwest-1</code> <i>(from Amazon Web Services Regions in China)</i> </p> </li> <li> <p>Use the following parameter with each command to specify the endpoint:</p> <p> <code>--region us-east-1</code> <i>(from commercial Amazon Web Services Regions outside of China)</i> </p> <p>or</p> <p> <code>--region cn-northwest-1</code> <i>(from Amazon Web Services Regions in China)</i> </p> </li> </ul> <p> <b>Recording API Requests</b> </p> <p>Organizations supports CloudTrail, a service that records Amazon Web Services API calls for your Amazon Web Services account and delivers log files to an Amazon S3 bucket. By using information collected by CloudTrail, you can determine which requests the Organizations service received, who made the request and when, and so on. For more about Organizations and its support for CloudTrail, see <a href=\"https://docs.aws.amazon.com/organizations/latest/userguide/orgs_incident-response.html#orgs_cloudtrail-integration\">Logging Organizations Events with CloudTrail</a> in the <i>Organizations User Guide</i>. To learn more about CloudTrail, including how to turn it on and find your log files, see the <a href=\"https://docs.aws.amazon.com/awscloudtrail/latest/userguide/what_is_cloud_trail_top_level.html\">CloudTrail User Guide</a>.</p>
 *
 * The version of the OpenAPI document: 2016-11-28
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import ActionType from './ActionType';
import Handshake from './Handshake';
import HandshakeState from './HandshakeState';

/**
 * The InviteAccountToOrganizationResponseHandshake model module.
 * @module model/InviteAccountToOrganizationResponseHandshake
 * @version 2016-11-28
 */
class InviteAccountToOrganizationResponseHandshake {
    /**
     * Constructs a new <code>InviteAccountToOrganizationResponseHandshake</code>.
     * @alias module:model/InviteAccountToOrganizationResponseHandshake
     * @implements module:model/Handshake
     */
    constructor() { 
        Handshake.initialize(this);
        InviteAccountToOrganizationResponseHandshake.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>InviteAccountToOrganizationResponseHandshake</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/InviteAccountToOrganizationResponseHandshake} obj Optional instance to populate.
     * @return {module:model/InviteAccountToOrganizationResponseHandshake} The populated <code>InviteAccountToOrganizationResponseHandshake</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new InviteAccountToOrganizationResponseHandshake();
            Handshake.constructFromObject(data, obj);

            if (data.hasOwnProperty('Id')) {
                obj['Id'] = ApiClient.convertToType(data['Id'], 'String');
            }
            if (data.hasOwnProperty('Arn')) {
                obj['Arn'] = ApiClient.convertToType(data['Arn'], 'String');
            }
            if (data.hasOwnProperty('Parties')) {
                obj['Parties'] = ApiClient.convertToType(data['Parties'], Array);
            }
            if (data.hasOwnProperty('State')) {
                obj['State'] = ApiClient.convertToType(data['State'], HandshakeState);
            }
            if (data.hasOwnProperty('RequestedTimestamp')) {
                obj['RequestedTimestamp'] = ApiClient.convertToType(data['RequestedTimestamp'], 'Date');
            }
            if (data.hasOwnProperty('ExpirationTimestamp')) {
                obj['ExpirationTimestamp'] = ApiClient.convertToType(data['ExpirationTimestamp'], 'Date');
            }
            if (data.hasOwnProperty('Action')) {
                obj['Action'] = ApiClient.convertToType(data['Action'], ActionType);
            }
            if (data.hasOwnProperty('Resources')) {
                obj['Resources'] = ApiClient.convertToType(data['Resources'], Array);
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>InviteAccountToOrganizationResponseHandshake</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>InviteAccountToOrganizationResponseHandshake</code>.
     */
    static validateJSON(data) {
        // validate the optional field `Id`
        if (data['Id']) { // data not null
          String.validateJSON(data['Id']);
        }
        // validate the optional field `Arn`
        if (data['Arn']) { // data not null
          String.validateJSON(data['Arn']);
        }
        // validate the optional field `Parties`
        if (data['Parties']) { // data not null
          Array.validateJSON(data['Parties']);
        }
        // validate the optional field `State`
        if (data['State']) { // data not null
          HandshakeState.validateJSON(data['State']);
        }
        // validate the optional field `RequestedTimestamp`
        if (data['RequestedTimestamp']) { // data not null
          Date.validateJSON(data['RequestedTimestamp']);
        }
        // validate the optional field `ExpirationTimestamp`
        if (data['ExpirationTimestamp']) { // data not null
          Date.validateJSON(data['ExpirationTimestamp']);
        }
        // validate the optional field `Action`
        if (data['Action']) { // data not null
          ActionType.validateJSON(data['Action']);
        }
        // validate the optional field `Resources`
        if (data['Resources']) { // data not null
          Array.validateJSON(data['Resources']);
        }

        return true;
    }


}



/**
 * @member {String} Id
 */
InviteAccountToOrganizationResponseHandshake.prototype['Id'] = undefined;

/**
 * @member {String} Arn
 */
InviteAccountToOrganizationResponseHandshake.prototype['Arn'] = undefined;

/**
 * @member {Array} Parties
 */
InviteAccountToOrganizationResponseHandshake.prototype['Parties'] = undefined;

/**
 * @member {module:model/HandshakeState} State
 */
InviteAccountToOrganizationResponseHandshake.prototype['State'] = undefined;

/**
 * @member {Date} RequestedTimestamp
 */
InviteAccountToOrganizationResponseHandshake.prototype['RequestedTimestamp'] = undefined;

/**
 * @member {Date} ExpirationTimestamp
 */
InviteAccountToOrganizationResponseHandshake.prototype['ExpirationTimestamp'] = undefined;

/**
 * @member {module:model/ActionType} Action
 */
InviteAccountToOrganizationResponseHandshake.prototype['Action'] = undefined;

/**
 * @member {Array} Resources
 */
InviteAccountToOrganizationResponseHandshake.prototype['Resources'] = undefined;


// Implement Handshake interface:
/**
 * @member {String} Id
 */
Handshake.prototype['Id'] = undefined;
/**
 * @member {String} Arn
 */
Handshake.prototype['Arn'] = undefined;
/**
 * @member {Array} Parties
 */
Handshake.prototype['Parties'] = undefined;
/**
 * @member {module:model/HandshakeState} State
 */
Handshake.prototype['State'] = undefined;
/**
 * @member {Date} RequestedTimestamp
 */
Handshake.prototype['RequestedTimestamp'] = undefined;
/**
 * @member {Date} ExpirationTimestamp
 */
Handshake.prototype['ExpirationTimestamp'] = undefined;
/**
 * @member {module:model/ActionType} Action
 */
Handshake.prototype['Action'] = undefined;
/**
 * @member {Array} Resources
 */
Handshake.prototype['Resources'] = undefined;




export default InviteAccountToOrganizationResponseHandshake;

