/**
 * AWS Organizations
 * <p>Organizations is a web service that enables you to consolidate your multiple Amazon Web Services accounts into an <i>organization</i> and centrally manage your accounts and their resources.</p> <p>This guide provides descriptions of the Organizations operations. For more information about using this service, see the <a href=\"https://docs.aws.amazon.com/organizations/latest/userguide/orgs_introduction.html\">Organizations User Guide</a>.</p> <p> <b>Support and feedback for Organizations</b> </p> <p>We welcome your feedback. Send your comments to <a href=\"mailto:feedback-awsorganizations@amazon.com\">feedback-awsorganizations@amazon.com</a> or post your feedback and questions in the <a href=\"http://forums.aws.amazon.com/forum.jspa?forumID=219\">Organizations support forum</a>. For more information about the Amazon Web Services support forums, see <a href=\"http://forums.aws.amazon.com/help.jspa\">Forums Help</a>.</p> <p> <b>Endpoint to call When using the CLI or the Amazon Web Services SDK</b> </p> <p>For the current release of Organizations, specify the <code>us-east-1</code> region for all Amazon Web Services API and CLI calls made from the commercial Amazon Web Services Regions outside of China. If calling from one of the Amazon Web Services Regions in China, then specify <code>cn-northwest-1</code>. You can do this in the CLI by using these parameters and commands:</p> <ul> <li> <p>Use the following parameter with each command to specify both the endpoint and its region:</p> <p> <code>--endpoint-url https://organizations.us-east-1.amazonaws.com</code> <i>(from commercial Amazon Web Services Regions outside of China)</i> </p> <p>or</p> <p> <code>--endpoint-url https://organizations.cn-northwest-1.amazonaws.com.cn</code> <i>(from Amazon Web Services Regions in China)</i> </p> </li> <li> <p>Use the default endpoint, but configure your default region with this command:</p> <p> <code>aws configure set default.region us-east-1</code> <i>(from commercial Amazon Web Services Regions outside of China)</i> </p> <p>or</p> <p> <code>aws configure set default.region cn-northwest-1</code> <i>(from Amazon Web Services Regions in China)</i> </p> </li> <li> <p>Use the following parameter with each command to specify the endpoint:</p> <p> <code>--region us-east-1</code> <i>(from commercial Amazon Web Services Regions outside of China)</i> </p> <p>or</p> <p> <code>--region cn-northwest-1</code> <i>(from Amazon Web Services Regions in China)</i> </p> </li> </ul> <p> <b>Recording API Requests</b> </p> <p>Organizations supports CloudTrail, a service that records Amazon Web Services API calls for your Amazon Web Services account and delivers log files to an Amazon S3 bucket. By using information collected by CloudTrail, you can determine which requests the Organizations service received, who made the request and when, and so on. For more about Organizations and its support for CloudTrail, see <a href=\"https://docs.aws.amazon.com/organizations/latest/userguide/orgs_incident-response.html#orgs_cloudtrail-integration\">Logging Organizations Events with CloudTrail</a> in the <i>Organizations User Guide</i>. To learn more about CloudTrail, including how to turn it on and find your log files, see the <a href=\"https://docs.aws.amazon.com/awscloudtrail/latest/userguide/what_is_cloud_trail_top_level.html\">CloudTrail User Guide</a>.</p>
 *
 * The version of the OpenAPI document: 2016-11-28
 * Contact: mike.ralphson@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAICreateAccountStatus.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAICreateAccountStatus::OAICreateAccountStatus(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAICreateAccountStatus::OAICreateAccountStatus() {
    this->initializeModel();
}

OAICreateAccountStatus::~OAICreateAccountStatus() {}

void OAICreateAccountStatus::initializeModel() {

    m_id_isSet = false;
    m_id_isValid = false;

    m_account_name_isSet = false;
    m_account_name_isValid = false;

    m_state_isSet = false;
    m_state_isValid = false;

    m_requested_timestamp_isSet = false;
    m_requested_timestamp_isValid = false;

    m_completed_timestamp_isSet = false;
    m_completed_timestamp_isValid = false;

    m_account_id_isSet = false;
    m_account_id_isValid = false;

    m_gov_cloud_account_id_isSet = false;
    m_gov_cloud_account_id_isValid = false;

    m_failure_reason_isSet = false;
    m_failure_reason_isValid = false;
}

void OAICreateAccountStatus::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAICreateAccountStatus::fromJsonObject(QJsonObject json) {

    m_id_isValid = ::OpenAPI::fromJsonValue(m_id, json[QString("Id")]);
    m_id_isSet = !json[QString("Id")].isNull() && m_id_isValid;

    m_account_name_isValid = ::OpenAPI::fromJsonValue(m_account_name, json[QString("AccountName")]);
    m_account_name_isSet = !json[QString("AccountName")].isNull() && m_account_name_isValid;

    m_state_isValid = ::OpenAPI::fromJsonValue(m_state, json[QString("State")]);
    m_state_isSet = !json[QString("State")].isNull() && m_state_isValid;

    m_requested_timestamp_isValid = ::OpenAPI::fromJsonValue(m_requested_timestamp, json[QString("RequestedTimestamp")]);
    m_requested_timestamp_isSet = !json[QString("RequestedTimestamp")].isNull() && m_requested_timestamp_isValid;

    m_completed_timestamp_isValid = ::OpenAPI::fromJsonValue(m_completed_timestamp, json[QString("CompletedTimestamp")]);
    m_completed_timestamp_isSet = !json[QString("CompletedTimestamp")].isNull() && m_completed_timestamp_isValid;

    m_account_id_isValid = ::OpenAPI::fromJsonValue(m_account_id, json[QString("AccountId")]);
    m_account_id_isSet = !json[QString("AccountId")].isNull() && m_account_id_isValid;

    m_gov_cloud_account_id_isValid = ::OpenAPI::fromJsonValue(m_gov_cloud_account_id, json[QString("GovCloudAccountId")]);
    m_gov_cloud_account_id_isSet = !json[QString("GovCloudAccountId")].isNull() && m_gov_cloud_account_id_isValid;

    m_failure_reason_isValid = ::OpenAPI::fromJsonValue(m_failure_reason, json[QString("FailureReason")]);
    m_failure_reason_isSet = !json[QString("FailureReason")].isNull() && m_failure_reason_isValid;
}

QString OAICreateAccountStatus::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAICreateAccountStatus::asJsonObject() const {
    QJsonObject obj;
    if (m_id_isSet) {
        obj.insert(QString("Id"), ::OpenAPI::toJsonValue(m_id));
    }
    if (m_account_name_isSet) {
        obj.insert(QString("AccountName"), ::OpenAPI::toJsonValue(m_account_name));
    }
    if (m_state.isSet()) {
        obj.insert(QString("State"), ::OpenAPI::toJsonValue(m_state));
    }
    if (m_requested_timestamp_isSet) {
        obj.insert(QString("RequestedTimestamp"), ::OpenAPI::toJsonValue(m_requested_timestamp));
    }
    if (m_completed_timestamp_isSet) {
        obj.insert(QString("CompletedTimestamp"), ::OpenAPI::toJsonValue(m_completed_timestamp));
    }
    if (m_account_id_isSet) {
        obj.insert(QString("AccountId"), ::OpenAPI::toJsonValue(m_account_id));
    }
    if (m_gov_cloud_account_id_isSet) {
        obj.insert(QString("GovCloudAccountId"), ::OpenAPI::toJsonValue(m_gov_cloud_account_id));
    }
    if (m_failure_reason.isSet()) {
        obj.insert(QString("FailureReason"), ::OpenAPI::toJsonValue(m_failure_reason));
    }
    return obj;
}

QString OAICreateAccountStatus::getId() const {
    return m_id;
}
void OAICreateAccountStatus::setId(const QString &id) {
    m_id = id;
    m_id_isSet = true;
}

bool OAICreateAccountStatus::is_id_Set() const{
    return m_id_isSet;
}

bool OAICreateAccountStatus::is_id_Valid() const{
    return m_id_isValid;
}

QString OAICreateAccountStatus::getAccountName() const {
    return m_account_name;
}
void OAICreateAccountStatus::setAccountName(const QString &account_name) {
    m_account_name = account_name;
    m_account_name_isSet = true;
}

bool OAICreateAccountStatus::is_account_name_Set() const{
    return m_account_name_isSet;
}

bool OAICreateAccountStatus::is_account_name_Valid() const{
    return m_account_name_isValid;
}

OAICreateAccountState OAICreateAccountStatus::getState() const {
    return m_state;
}
void OAICreateAccountStatus::setState(const OAICreateAccountState &state) {
    m_state = state;
    m_state_isSet = true;
}

bool OAICreateAccountStatus::is_state_Set() const{
    return m_state_isSet;
}

bool OAICreateAccountStatus::is_state_Valid() const{
    return m_state_isValid;
}

QDateTime OAICreateAccountStatus::getRequestedTimestamp() const {
    return m_requested_timestamp;
}
void OAICreateAccountStatus::setRequestedTimestamp(const QDateTime &requested_timestamp) {
    m_requested_timestamp = requested_timestamp;
    m_requested_timestamp_isSet = true;
}

bool OAICreateAccountStatus::is_requested_timestamp_Set() const{
    return m_requested_timestamp_isSet;
}

bool OAICreateAccountStatus::is_requested_timestamp_Valid() const{
    return m_requested_timestamp_isValid;
}

QDateTime OAICreateAccountStatus::getCompletedTimestamp() const {
    return m_completed_timestamp;
}
void OAICreateAccountStatus::setCompletedTimestamp(const QDateTime &completed_timestamp) {
    m_completed_timestamp = completed_timestamp;
    m_completed_timestamp_isSet = true;
}

bool OAICreateAccountStatus::is_completed_timestamp_Set() const{
    return m_completed_timestamp_isSet;
}

bool OAICreateAccountStatus::is_completed_timestamp_Valid() const{
    return m_completed_timestamp_isValid;
}

QString OAICreateAccountStatus::getAccountId() const {
    return m_account_id;
}
void OAICreateAccountStatus::setAccountId(const QString &account_id) {
    m_account_id = account_id;
    m_account_id_isSet = true;
}

bool OAICreateAccountStatus::is_account_id_Set() const{
    return m_account_id_isSet;
}

bool OAICreateAccountStatus::is_account_id_Valid() const{
    return m_account_id_isValid;
}

QString OAICreateAccountStatus::getGovCloudAccountId() const {
    return m_gov_cloud_account_id;
}
void OAICreateAccountStatus::setGovCloudAccountId(const QString &gov_cloud_account_id) {
    m_gov_cloud_account_id = gov_cloud_account_id;
    m_gov_cloud_account_id_isSet = true;
}

bool OAICreateAccountStatus::is_gov_cloud_account_id_Set() const{
    return m_gov_cloud_account_id_isSet;
}

bool OAICreateAccountStatus::is_gov_cloud_account_id_Valid() const{
    return m_gov_cloud_account_id_isValid;
}

OAICreateAccountFailureReason OAICreateAccountStatus::getFailureReason() const {
    return m_failure_reason;
}
void OAICreateAccountStatus::setFailureReason(const OAICreateAccountFailureReason &failure_reason) {
    m_failure_reason = failure_reason;
    m_failure_reason_isSet = true;
}

bool OAICreateAccountStatus::is_failure_reason_Set() const{
    return m_failure_reason_isSet;
}

bool OAICreateAccountStatus::is_failure_reason_Valid() const{
    return m_failure_reason_isValid;
}

bool OAICreateAccountStatus::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_account_name_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_state.isSet()) {
            isObjectUpdated = true;
            break;
        }

        if (m_requested_timestamp_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_completed_timestamp_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_account_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_gov_cloud_account_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_failure_reason.isSet()) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAICreateAccountStatus::isValid() const {
    // only required properties are required for the object to be considered valid
    return true;
}

} // namespace OpenAPI
