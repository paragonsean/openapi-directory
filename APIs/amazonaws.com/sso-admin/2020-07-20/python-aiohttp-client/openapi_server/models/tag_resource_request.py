# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server import util


class TagResourceRequest(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, instance_arn: str=None, resource_arn: str=None, tags: List=None):
        """TagResourceRequest - a model defined in OpenAPI

        :param instance_arn: The instance_arn of this TagResourceRequest.
        :param resource_arn: The resource_arn of this TagResourceRequest.
        :param tags: The tags of this TagResourceRequest.
        """
        self.openapi_types = {
            'instance_arn': str,
            'resource_arn': str,
            'tags': List
        }

        self.attribute_map = {
            'instance_arn': 'InstanceArn',
            'resource_arn': 'ResourceArn',
            'tags': 'Tags'
        }

        self._instance_arn = instance_arn
        self._resource_arn = resource_arn
        self._tags = tags

    @classmethod
    def from_dict(cls, dikt: dict) -> 'TagResourceRequest':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The TagResourceRequest of this TagResourceRequest.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def instance_arn(self):
        """Gets the instance_arn of this TagResourceRequest.


        :return: The instance_arn of this TagResourceRequest.
        :rtype: str
        """
        return self._instance_arn

    @instance_arn.setter
    def instance_arn(self, instance_arn):
        """Sets the instance_arn of this TagResourceRequest.


        :param instance_arn: The instance_arn of this TagResourceRequest.
        :type instance_arn: str
        """
        if instance_arn is None:
            raise ValueError("Invalid value for `instance_arn`, must not be `None`")

        self._instance_arn = instance_arn

    @property
    def resource_arn(self):
        """Gets the resource_arn of this TagResourceRequest.


        :return: The resource_arn of this TagResourceRequest.
        :rtype: str
        """
        return self._resource_arn

    @resource_arn.setter
    def resource_arn(self, resource_arn):
        """Sets the resource_arn of this TagResourceRequest.


        :param resource_arn: The resource_arn of this TagResourceRequest.
        :type resource_arn: str
        """
        if resource_arn is None:
            raise ValueError("Invalid value for `resource_arn`, must not be `None`")

        self._resource_arn = resource_arn

    @property
    def tags(self):
        """Gets the tags of this TagResourceRequest.


        :return: The tags of this TagResourceRequest.
        :rtype: List
        """
        return self._tags

    @tags.setter
    def tags(self, tags):
        """Sets the tags of this TagResourceRequest.


        :param tags: The tags of this TagResourceRequest.
        :type tags: List
        """
        if tags is None:
            raise ValueError("Invalid value for `tags`, must not be `None`")

        self._tags = tags
