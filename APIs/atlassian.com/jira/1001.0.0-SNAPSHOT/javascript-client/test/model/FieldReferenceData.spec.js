/**
 * The Jira Cloud platform REST API
 * Jira Cloud platform REST API documentation
 *
 * The version of the OpenAPI document: 1001.0.0-SNAPSHOT
 * Contact: ecosystem@atlassian.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', process.cwd()+'/src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require(process.cwd()+'/src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.TheJiraCloudPlatformRestApi);
  }
}(this, function(expect, TheJiraCloudPlatformRestApi) {
  'use strict';

  var instance;

  beforeEach(function() {
    instance = new TheJiraCloudPlatformRestApi.FieldReferenceData();
  });

  var getProperty = function(object, getter, property) {
    // Use getter method if present; otherwise, get the property directly.
    if (typeof object[getter] === 'function')
      return object[getter]();
    else
      return object[property];
  }

  var setProperty = function(object, setter, property, value) {
    // Use setter method if present; otherwise, set the property directly.
    if (typeof object[setter] === 'function')
      object[setter](value);
    else
      object[property] = value;
  }

  describe('FieldReferenceData', function() {
    it('should create an instance of FieldReferenceData', function() {
      // uncomment below and update the code to test FieldReferenceData
      //var instance = new TheJiraCloudPlatformRestApi.FieldReferenceData();
      //expect(instance).to.be.a(TheJiraCloudPlatformRestApi.FieldReferenceData);
    });

    it('should have the property auto (base name: "auto")', function() {
      // uncomment below and update the code to test the property auto
      //var instance = new TheJiraCloudPlatformRestApi.FieldReferenceData();
      //expect(instance).to.be();
    });

    it('should have the property cfid (base name: "cfid")', function() {
      // uncomment below and update the code to test the property cfid
      //var instance = new TheJiraCloudPlatformRestApi.FieldReferenceData();
      //expect(instance).to.be();
    });

    it('should have the property deprecated (base name: "deprecated")', function() {
      // uncomment below and update the code to test the property deprecated
      //var instance = new TheJiraCloudPlatformRestApi.FieldReferenceData();
      //expect(instance).to.be();
    });

    it('should have the property deprecatedSearcherKey (base name: "deprecatedSearcherKey")', function() {
      // uncomment below and update the code to test the property deprecatedSearcherKey
      //var instance = new TheJiraCloudPlatformRestApi.FieldReferenceData();
      //expect(instance).to.be();
    });

    it('should have the property displayName (base name: "displayName")', function() {
      // uncomment below and update the code to test the property displayName
      //var instance = new TheJiraCloudPlatformRestApi.FieldReferenceData();
      //expect(instance).to.be();
    });

    it('should have the property operators (base name: "operators")', function() {
      // uncomment below and update the code to test the property operators
      //var instance = new TheJiraCloudPlatformRestApi.FieldReferenceData();
      //expect(instance).to.be();
    });

    it('should have the property orderable (base name: "orderable")', function() {
      // uncomment below and update the code to test the property orderable
      //var instance = new TheJiraCloudPlatformRestApi.FieldReferenceData();
      //expect(instance).to.be();
    });

    it('should have the property searchable (base name: "searchable")', function() {
      // uncomment below and update the code to test the property searchable
      //var instance = new TheJiraCloudPlatformRestApi.FieldReferenceData();
      //expect(instance).to.be();
    });

    it('should have the property types (base name: "types")', function() {
      // uncomment below and update the code to test the property types
      //var instance = new TheJiraCloudPlatformRestApi.FieldReferenceData();
      //expect(instance).to.be();
    });

    it('should have the property value (base name: "value")', function() {
      // uncomment below and update the code to test the property value
      //var instance = new TheJiraCloudPlatformRestApi.FieldReferenceData();
      //expect(instance).to.be();
    });

  });

}));
