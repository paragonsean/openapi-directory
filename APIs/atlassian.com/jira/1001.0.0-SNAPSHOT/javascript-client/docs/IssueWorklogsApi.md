# TheJiraCloudPlatformRestApi.IssueWorklogsApi
...
> Worklog addWorklog(issueIdOrKey, worklog, opts)
...
let apiInstance = new TheJiraCloudPlatformRestApi.IssueWorklogsApi();
let issueIdOrKey = "issueIdOrKey_example"; // String | The ID or key the issue.
let worklog = {"comment":{"content":[{"content":[{"text":"I did some work here.","type":"text"}],"type":"paragraph"}],"type":"doc","version":1},"started":"2021-01-17T12:34:00.000+0000","timeSpentSeconds":12000,"visibility":{"identifier":"276f955c-63d7-42c8-9520-92d01dca0625","type":"group"}}; // Worklog | 
let opts = {
  ...
  'overrideEditableFlag': false // Boolean | Whether the worklog entry should be added to the issue even if the issue is not editable, because jira.issue.editable set to false or missing. For example, the issue is closed. Connect and Forge app users with *Administer Jira* [global permission](https://confluence.atlassian.com/x/x4dKLg) can use this flag.
};
apiInstance.addWorklog(issueIdOrKey, worklog, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
let opts = {
  ...
  'overrideEditableFlag': false // Boolean | Whether the work log entry should be added to the issue even if the issue is not editable, because jira.issue.editable set to false or missing. For example, the issue is closed. Connect and Forge app users with admin permission can use this flag.
};
...
> ChangedWorklogs getIdsOfWorklogsDeletedSince(opts)
...
> ChangedWorklogs getIdsOfWorklogsModifiedSince(opts)
...
> PageOfWorklogs getIssueWorklog(issueIdOrKey, opts)
...
let opts = {
  ...
  'startedBefore': 789, // Number | The worklog start date and time, as a UNIX timestamp in milliseconds, before which worklogs are returned.
...
apiInstance.getIssueWorklog(issueIdOrKey, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
 ...
 **startedBefore** | **Number**| The worklog start date and time, as a UNIX timestamp in milliseconds, before which worklogs are returned. | [optional] 
...
> Worklog getWorklog(issueIdOrKey, id, opts)
...
apiInstance.getWorklog(issueIdOrKey, id, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
...
> [Worklog] getWorklogsForIds(worklogIdsRequestBean, opts)
...
let apiInstance = new TheJiraCloudPlatformRestApi.IssueWorklogsApi();
let worklogIdsRequestBean = {"ids":[1,2,5,10]}; // WorklogIdsRequestBean | A JSON object containing a list of worklog IDs.
let opts = {
  'expand': "''" // String | Use [expand](#expansion) to include additional information about worklogs in the response. This parameter accepts `properties` that returns the properties of each worklog.
};
apiInstance.getWorklogsForIds(worklogIdsRequestBean, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
...
> Worklog updateWorklog(issueIdOrKey, id, worklog, opts)
...
let opts = {
  ...
  'newEstimate': "newEstimate_example", // String | The value to set as the issue's remaining time estimate, as days (\\#d), hours (\\#h), or minutes (\\#m or \\#). For example, *2d*. Required when `adjustEstimate` is `new`.
  'expand': "''", // String | Use [expand](#expansion) to include additional information about worklogs in the response. This parameter accepts `properties`, which returns worklog properties.
  'overrideEditableFlag': false // Boolean | Whether the worklog should be added to the issue even if the issue is not editable. For example, because the issue is closed. Connect and Forge app users with *Administer Jira* [global permission](https://confluence.atlassian.com/x/x4dKLg) can use this flag.
};
apiInstance.updateWorklog(issueIdOrKey, id, worklog, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
