# TheJiraCloudPlatformRestApi.GroupAndUserPickerApi
...
> FoundUsersAndGroups findUsersAndGroups(query, opts)
...
let apiInstance = new TheJiraCloudPlatformRestApi.GroupAndUserPickerApi();
...
apiInstance.findUsersAndGroups(query, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
