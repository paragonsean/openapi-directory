...
public class ContainerOfWorkflowSchemeAssociations {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ContainerOfWorkflowSchemeAssociations
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ContainerOfWorkflowSchemeAssociations.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ContainerOfWorkflowSchemeAssociations.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ContainerOfWorkflowSchemeAssociations` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        WorkflowSchemeAssociations.validateJsonElement(jsonArrayvalues.get(i));
      };
  }
