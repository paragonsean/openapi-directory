...
async def test_delete_project_role(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/rest/api/3/role/{id}'.format(id=56),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_get_all_project_roles(client):
...
async def test_get_project_role(client):
    ...
    """
    params = [('excludeInactiveUsers', False)]
    headers = { 
        'Accept': 'application/json',
        'Authorization': 'Bearer special-key',
        'Authorization': 'BasicZm9vOmJhcg==',
...
async def test_get_project_role_by_id(client):
...
async def test_get_project_role_details(client):
    ...
    response = await client.request(
        method='GET',
        path='/rest/api/3/project/{project_id_or_key}/roledetails'.format(project_id_or_key='project_id_or_key_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_get_project_roles(client):
    ...
    response = await client.request(
        method='GET',
        path='/rest/api/3/project/{project_id_or_key}/role'.format(project_id_or_key='project_id_or_key_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
