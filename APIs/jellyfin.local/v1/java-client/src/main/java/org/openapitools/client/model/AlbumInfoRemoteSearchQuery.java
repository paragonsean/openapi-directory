...
import org.openapitools.client.model.AlbumInfo;
...
public class AlbumInfoRemoteSearchQuery {
   ...
   * @throws IOException if the JSON Element is invalid with respect to AlbumInfoRemoteSearchQuery
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AlbumInfoRemoteSearchQuery.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in AlbumInfoRemoteSearchQuery is not found in the empty JSON string", AlbumInfoRemoteSearchQuery.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!AlbumInfoRemoteSearchQuery.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AlbumInfoRemoteSearchQuery` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("SearchInfo") != null && !jsonObj.get("SearchInfo").isJsonNull()) {
        AlbumInfo.validateJsonElement(jsonObj.get("SearchInfo"));
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!AlbumInfoRemoteSearchQuery.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'AlbumInfoRemoteSearchQuery' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<AlbumInfoRemoteSearchQuery> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(AlbumInfoRemoteSearchQuery.class));

       return (TypeAdapter<T>) new TypeAdapter<AlbumInfoRemoteSearchQuery>() {
           @Override
           public void write(JsonWriter out, AlbumInfoRemoteSearchQuery value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public AlbumInfoRemoteSearchQuery read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of AlbumInfoRemoteSearchQuery given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of AlbumInfoRemoteSearchQuery
   * @throws IOException if the JSON string is invalid with respect to AlbumInfoRemoteSearchQuery
   */
  public static AlbumInfoRemoteSearchQuery fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, AlbumInfoRemoteSearchQuery.class);
  }

  /**
   * Convert an instance of AlbumInfoRemoteSearchQuery to an JSON string
   *
   * @return JSON string
   */
