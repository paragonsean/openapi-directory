# JellyfinApi.PlaybackProgressInfo

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**aspectRatio** | **String** |  | [optional] 
**audioStreamIndex** | **Number** | Gets or sets the index of the audio stream. | [optional] 
**brightness** | **Number** |  | [optional] 
**canSeek** | **Boolean** | Gets or sets a value indicating whether this instance can seek. | [optional] 
**isMuted** | **Boolean** | Gets or sets a value indicating whether this instance is muted. | [optional] 
**isPaused** | **Boolean** | Gets or sets a value indicating whether this instance is paused. | [optional] 
**item** | [**BaseItemDto**](BaseItemDto.md) |  | [optional] 
**itemId** | **String** | Gets or sets the item identifier. | [optional] 
**liveStreamId** | **String** | Gets or sets the live stream identifier. | [optional] 
**mediaSourceId** | **String** | Gets or sets the media version identifier. | [optional] 
**nowPlayingQueue** | [**[QueueItem]**](QueueItem.md) |  | [optional] 
**playMethod** | [**PlayMethod**](PlayMethod.md) |  | [optional] 
**playSessionId** | **String** | Gets or sets the play session identifier. | [optional] 
**playbackStartTimeTicks** | **Number** |  | [optional] 
**playlistItemId** | **String** |  | [optional] 
**positionTicks** | **Number** | Gets or sets the position ticks. | [optional] 
**repeatMode** | [**RepeatMode**](RepeatMode.md) |  | [optional] 
**sessionId** | **String** | Gets or sets the session id. | [optional] 
**subtitleStreamIndex** | **Number** | Gets or sets the index of the subtitle stream. | [optional] 
**volumeLevel** | **Number** | Gets or sets the volume level. | [optional] 


