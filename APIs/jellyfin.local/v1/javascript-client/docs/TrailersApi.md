...
> BaseItemDtoQueryResult getTrailers(opts)
...
let apiInstance = new JellyfinApi.TrailersApi();
let opts = {
...
apiInstance.getTrailers(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
