...
> File getLiveHlsStream(itemId, opts)
...
let apiInstance = new JellyfinApi.VideoHlsApi();
...
let opts = {
  ...
  'maxWidth': 56, // Number | Optional. The max width.
  'maxHeight': 56, // Number | Optional. The max height.
  'enableSubtitlesInManifest': true // Boolean | Optional. Whether to enable subtitles in the manifest.
};
apiInstance.getLiveHlsStream(itemId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
 ...
 **maxWidth** | **Number**| Optional. The max width. | [optional] 
 **maxHeight** | **Number**| Optional. The max height. | [optional] 
