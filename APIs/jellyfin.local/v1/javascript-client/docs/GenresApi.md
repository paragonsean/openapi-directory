...
> BaseItemDto getGenre(genreName, opts)
...
let apiInstance = new JellyfinApi.GenresApi();
let genreName = "genreName_example"; // String | The genre name.
let opts = {
  'userId': "userId_example" // String | The user id.
};
apiInstance.getGenre(genreName, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
> BaseItemDtoQueryResult getGenres(opts)
...
apiInstance.getGenres(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
