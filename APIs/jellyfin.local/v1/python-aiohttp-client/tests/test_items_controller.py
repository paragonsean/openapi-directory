...
async def test_get_items_by_user_id(client):
    """Test case for get_items_by_user_id

    Gets items based on a query.
    """
    params = [('maxOfficialRating', 'max_official_rating_example'),
                    ('hasThemeSong', True),
                    ('hasThemeVideo', True),
                    ('hasSubtitles', True),
                    ('hasSpecialFeature', True),
    ...
    response = await client.request(
        method='GET',
        path='/Users/{user_id}/Items'.format(user_id='user_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_get_resume_items(client):
    ...
    response = await client.request(
        method='GET',
        path='/Users/{user_id}/Items/Resume'.format(user_id='user_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
