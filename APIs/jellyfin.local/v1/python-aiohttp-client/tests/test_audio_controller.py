...
async def test_get_audio_stream(client):
    ...
    response = await client.request(
        method='GET',
        path='/Audio/{item_id}/stream'.format(item_id='item_id_example'),
        headers=headers,
        params=params,
        )
...
async def test_get_audio_stream_by_container(client):
    ...
    response = await client.request(
        method='GET',
        path='/Audio/{item_id}/stream.{container}'.format(item_id='item_id_example', container='container_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_head_audio_stream(client):
...
async def test_head_audio_stream_by_container(client):
