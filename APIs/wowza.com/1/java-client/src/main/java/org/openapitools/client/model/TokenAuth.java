...
public class TokenAuth {
   ...
   * @throws IOException if the JSON Element is invalid with respect to TokenAuth
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!TokenAuth.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in TokenAuth is not found in the empty JSON string", TokenAuth.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!TokenAuth.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TokenAuth` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("trusted_shared_secret") != null && !jsonObj.get("trusted_shared_secret").isJsonNull()) && !jsonObj.get("trusted_shared_secret").isJsonPrimitive()) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!TokenAuth.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'TokenAuth' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<TokenAuth> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(TokenAuth.class));

       return (TypeAdapter<T>) new TypeAdapter<TokenAuth>() {
           @Override
           public void write(JsonWriter out, TokenAuth value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public TokenAuth read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of TokenAuth given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of TokenAuth
   * @throws IOException if the JSON string is invalid with respect to TokenAuth
   */
  public static TokenAuth fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, TokenAuth.class);
  }

  /**
   * Convert an instance of TokenAuth to an JSON string
   *
   * @return JSON string
   */
