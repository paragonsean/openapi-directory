...
public class HashOfZones {
   ...
   * @throws IOException if the JSON Element is invalid with respect to HashOfZones
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!HashOfZones.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in HashOfZones is not found in the empty JSON string", HashOfZones.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!HashOfZones.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `HashOfZones` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!HashOfZones.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'HashOfZones' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<HashOfZones> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(HashOfZones.class));

       return (TypeAdapter<T>) new TypeAdapter<HashOfZones>() {
           @Override
           public void write(JsonWriter out, HashOfZones value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public HashOfZones read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of HashOfZones given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of HashOfZones
   * @throws IOException if the JSON string is invalid with respect to HashOfZones
   */
  public static HashOfZones fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, HashOfZones.class);
  }

  /**
   * Convert an instance of HashOfZones to an JSON string
   *
   * @return JSON string
   */
