...
public class GpuUsageMetric {
   ...
   * @throws IOException if the JSON Element is invalid with respect to GpuUsageMetric
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!GpuUsageMetric.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in GpuUsageMetric is not found in the empty JSON string", GpuUsageMetric.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!GpuUsageMetric.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `GpuUsageMetric` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!GpuUsageMetric.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'GpuUsageMetric' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<GpuUsageMetric> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(GpuUsageMetric.class));

       return (TypeAdapter<T>) new TypeAdapter<GpuUsageMetric>() {
           @Override
           public void write(JsonWriter out, GpuUsageMetric value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public GpuUsageMetric read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of GpuUsageMetric given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of GpuUsageMetric
   * @throws IOException if the JSON string is invalid with respect to GpuUsageMetric
   */
  public static GpuUsageMetric fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, GpuUsageMetric.class);
  }

  /**
   * Convert an instance of GpuUsageMetric to an JSON string
   *
   * @return JSON string
   */
