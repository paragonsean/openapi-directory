...
public class HashOfTotals {
   ...
   * @throws IOException if the JSON Element is invalid with respect to HashOfTotals
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!HashOfTotals.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in HashOfTotals is not found in the empty JSON string", HashOfTotals.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!HashOfTotals.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `HashOfTotals` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!HashOfTotals.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'HashOfTotals' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<HashOfTotals> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(HashOfTotals.class));

       return (TypeAdapter<T>) new TypeAdapter<HashOfTotals>() {
           @Override
           public void write(JsonWriter out, HashOfTotals value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public HashOfTotals read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of HashOfTotals given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of HashOfTotals
   * @throws IOException if the JSON string is invalid with respect to HashOfTotals
   */
  public static HashOfTotals fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, HashOfTotals.class);
  }

  /**
   * Convert an instance of HashOfTotals to an JSON string
   *
   * @return JSON string
   */
