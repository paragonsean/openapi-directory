# WowzaStreamingCloudRestApiReferenceDocumentation.ProcessingTimeApi
...
> UsageTimeTranscoders usageTimeTranscodersIndex(opts)
...
let apiInstance = new WowzaStreamingCloudRestApiReferenceDocumentation.ProcessingTimeApi();
let opts = {
...
apiInstance.usageTimeTranscodersIndex(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
