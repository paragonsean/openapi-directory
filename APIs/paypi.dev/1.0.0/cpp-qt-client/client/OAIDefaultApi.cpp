...
void OAIDefaultApi::initializeServerConfigs() {
    ...
    QUrl("https://ev.apis.paypi.dev"),
    "No description provided",
    QMap<QString, OAIServerVariable>()));
    _serverConfigs.insert("checkCodePost", defaultConf);
    _serverIndices.insert("checkCodePost", 0);
    _serverConfigs.insert("sendCodePost", defaultConf);
    _serverIndices.insert("sendCodePost", 0);
}
...
void OAIDefaultApi::checkCodePost(const OAI_checkCode_post_request &oai_check_code_post_request) {
    QString fullPath = QString(_serverConfigs["checkCodePost"][_serverIndices.value("checkCodePost")].URL()+"/checkCode");
...

    {
        ...
        QByteArray output = oai_check_code_post_request.asJson().toUtf8();
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIDefaultApi::checkCodePostCallback);
...
void OAIDefaultApi::checkCodePostCallback(OAIHttpRequestWorker *worker) {
    ...
    OAI_checkCode_post_200_response output(QString(worker->response));
    worker->deleteLater();

    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT checkCodePostSignal(output);
        Q_EMIT checkCodePostSignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT checkCodePostSignalE(output, error_type, error_str);
        Q_EMIT checkCodePostSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT checkCodePostSignalError(output, error_type, error_str);
        Q_EMIT checkCodePostSignalErrorFull(worker, error_type, error_str);
    }
}

void OAIDefaultApi::sendCodePost(const OAI_sendCode_post_request &oai_send_code_post_request) {
    QString fullPath = QString(_serverConfigs["sendCodePost"][_serverIndices.value("sendCodePost")].URL()+"/sendCode");
...

    {
        ...
        QByteArray output = oai_send_code_post_request.asJson().toUtf8();
        input.request_body.append(output);
    }
    for (auto keyValueIt = _defaultHeaders.keyValueBegin(); keyValueIt != _defaultHeaders.keyValueEnd(); keyValueIt++) {
        input.headers.insert(keyValueIt->first, keyValueIt->second);
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIDefaultApi::sendCodePostCallback);
    connect(this, &OAIDefaultApi::abortRequestsSignal, worker, &QObject::deleteLater);
    connect(worker, &QObject::destroyed, this, [this]() {
        if (findChildren<OAIHttpRequestWorker*>().count() == 0) {
            Q_EMIT allPendingRequestsCompleted();
...
void OAIDefaultApi::sendCodePostCallback(OAIHttpRequestWorker *worker) {
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
    ...
    OAI_sendCode_post_200_response output(QString(worker->response));
    worker->deleteLater();

    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT sendCodePostSignal(output);
        Q_EMIT sendCodePostSignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT sendCodePostSignalE(output, error_type, error_str);
        Q_EMIT sendCodePostSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT sendCodePostSignalError(output, error_type, error_str);
        Q_EMIT sendCodePostSignalErrorFull(worker, error_type, error_str);
    }
}
