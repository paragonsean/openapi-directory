...
async def test_analyze_image(client):
    ...
    params = [('visualFeatures', ['[\"Categories\",\"Adult\",\"Tags\",\"Description\",\"Faces\",\"Color\",\"ImageType\",\"Objects\",\"Brands\"]']),
                    ...
                    ('language', en),
                    ('descriptionExclude', ['description_exclude_example'])]
    headers = { 
...
async def test_analyze_image_by_domain(client):
    ...
    response = await client.request(
        method='POST',
        path='/vision/v2.1/models/{model}/analyze'.format(model='Celebrities'),
        headers=headers,
        json=body,
        params=params,
        )
...
async def test_generate_thumbnail(client):
    ...
    response = await client.request(
        method='POST',
        path='/vision/v2.1/generateThumbnail',
        headers=headers,
        json=body,
        params=params,
        )
...
async def test_get_area_of_interest(client):
    ...
    response = await client.request(
        method='POST',
        path='/vision/v2.1/areaOfInterest',
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
