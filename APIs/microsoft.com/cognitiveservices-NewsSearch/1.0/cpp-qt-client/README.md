...
#include <iostream>
#include "../client/OAINewsCategoryApi.h"
...
void Example::exampleFunction1(){
     OAINewsCategoryApi apiInstance;
     
      // Configure API key authorization: apiKeyHeader
      apiInstance.setApiKey("YOUR API KEY NAME","YOUR API KEY");

      QEventLoop loop;
      connect(&apiInstance, &OAINewsCategoryApi::newsCategorySignal, [&]() {
          loop.quit();
      });
      connect(&apiInstance, &OAINewsCategoryApi::newsCategorySignalE, [&](QNetworkReply::NetworkError, QString error_str) {
      ...
      qint32 count = create(); // qint32 | The number of news articles to return in the response. The actual number delivered may be less than requested. The default is 10 and the maximum value is 100. The actual number delivered may be less than requested.You may use this parameter along with the offset parameter to page results. For example, if your user interface displays 20 articles per page, set count to 20 and offset to 0 to get the first page of results. For each subsequent page, increment offset by 20 (for example, 0, 20, 40). It is possible for multiple pages to include some overlap in results. If you do not specify the [category](https://docs.microsoft.com/en-us/rest/api/cognitiveservices/bing-news-api-v7-reference#category) parameter, Bing ignores this parameter.
      ...
      qint32 headline_count = create(); // qint32 | The number of headline articles to return in the response. The default is 12. Specify this parameter only if you do not specify the [category](https://docs.microsoft.com/en-us/rest/api/cognitiveservices/bing-news-api-v7-reference#category) parameter.
      ...
      apiInstance.newsCategory(x_bing_apis_sdkacceptaccept_languageuser_agentx_ms_edge_client_idx_ms_edge_client_ipx_search_locationcccategorycountheadline_countmktoffsetoriginal_imgsafe_searchset_langtext_decorationstext_format);
