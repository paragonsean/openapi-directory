...
async def test_delete_prediction(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/customvision/v2.0/Training/projects/{project_id}/predictions'.format(project_id='64b822c5-8082-4b36-a426-27225f4aa18c'),
        headers=headers,
        params=params,
        )
...
async def test_query_predictions(client):
    ...
    response = await client.request(
        method='POST',
        path='/customvision/v2.0/Training/projects/{project_id}/predictions/query'.format(project_id='bc3f7dad-5544-468c-8573-3ef04d55463e'),
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_quick_test_image(client):
    ...
    response = await client.request(
        method='POST',
        path='/customvision/v2.0/Training/projects/{project_id}/quicktest/image'.format(project_id='64b822c5-8082-4b36-a426-27225f4aa18c'),
        headers=headers,
        data=data,
        params=params,
        )
...
async def test_quick_test_image_url(client):
    ...
    response = await client.request(
        method='POST',
        path='/customvision/v2.0/Training/projects/{project_id}/quicktest/url'.format(project_id='64b822c5-8082-4b36-a426-27225f4aa18c'),
        headers=headers,
        json=body,
        params=params,
        )
