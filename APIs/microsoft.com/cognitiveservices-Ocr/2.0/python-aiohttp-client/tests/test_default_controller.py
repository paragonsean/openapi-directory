...
async def test_batch_read_file(client):
    ...
    response = await client.request(
        method='POST',
        path='/vision/v2.0/read/core/asyncBatchAnalyze',
        headers=headers,
        json=body,
        )
...
async def test_get_read_operation_result(client):
    ...
    response = await client.request(
        method='GET',
        path='/vision/v2.0/read/operations/{operation_id}'.format(operation_id='e56ffa6e-1ee4-4042-bc07-993db706c95f'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_get_text_operation_result(client):
    ...
    response = await client.request(
        method='GET',
        path='/vision/v2.0/textOperations/{operation_id}'.format(operation_id='49a36324-fc4b-4387-aa06-090cfbf0064f'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
