# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.action import Action
from openapi_server.models.response import Response
from openapi_server import util


class Thing(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, url: str=None, adaptive_card: str=None, immediate_action: List[Action]=None, potential_action: List[Action]=None, preferred_clickthrough_url: str=None, read_link: str=None, web_search_url: str=None, id: str=None, type: str=None):
        """Thing - a model defined in OpenAPI

        :param url: The url of this Thing.
        :param adaptive_card: The adaptive_card of this Thing.
        :param immediate_action: The immediate_action of this Thing.
        :param potential_action: The potential_action of this Thing.
        :param preferred_clickthrough_url: The preferred_clickthrough_url of this Thing.
        :param read_link: The read_link of this Thing.
        :param web_search_url: The web_search_url of this Thing.
        :param id: The id of this Thing.
        :param type: The type of this Thing.
        """
        self.openapi_types = {
            'url': str,
            'adaptive_card': str,
            'immediate_action': List[Action],
            'potential_action': List[Action],
            'preferred_clickthrough_url': str,
            'read_link': str,
            'web_search_url': str,
            'id': str,
            'type': str
        }

        self.attribute_map = {
            'url': 'url',
            'adaptive_card': 'adaptiveCard',
            'immediate_action': 'immediateAction',
            'potential_action': 'potentialAction',
            'preferred_clickthrough_url': 'preferredClickthroughUrl',
            'read_link': 'readLink',
            'web_search_url': 'webSearchUrl',
            'id': 'id',
            'type': '_type'
        }

        self._url = url
        self._adaptive_card = adaptive_card
        self._immediate_action = immediate_action
        self._potential_action = potential_action
        self._preferred_clickthrough_url = preferred_clickthrough_url
        self._read_link = read_link
        self._web_search_url = web_search_url
        self._id = id
        self._type = type

    @classmethod
    def from_dict(cls, dikt: dict) -> 'Thing':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The Thing of this Thing.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def url(self):
        """Gets the url of this Thing.

        The URL to get more information about the thing represented by this object.

        :return: The url of this Thing.
        :rtype: str
        """
        return self._url

    @url.setter
    def url(self, url):
        """Sets the url of this Thing.

        The URL to get more information about the thing represented by this object.

        :param url: The url of this Thing.
        :type url: str
        """

        self._url = url

    @property
    def adaptive_card(self):
        """Gets the adaptive_card of this Thing.


        :return: The adaptive_card of this Thing.
        :rtype: str
        """
        return self._adaptive_card

    @adaptive_card.setter
    def adaptive_card(self, adaptive_card):
        """Sets the adaptive_card of this Thing.


        :param adaptive_card: The adaptive_card of this Thing.
        :type adaptive_card: str
        """

        self._adaptive_card = adaptive_card

    @property
    def immediate_action(self):
        """Gets the immediate_action of this Thing.


        :return: The immediate_action of this Thing.
        :rtype: List[Action]
        """
        return self._immediate_action

    @immediate_action.setter
    def immediate_action(self, immediate_action):
        """Sets the immediate_action of this Thing.


        :param immediate_action: The immediate_action of this Thing.
        :type immediate_action: List[Action]
        """

        self._immediate_action = immediate_action

    @property
    def potential_action(self):
        """Gets the potential_action of this Thing.


        :return: The potential_action of this Thing.
        :rtype: List[Action]
        """
        return self._potential_action

    @potential_action.setter
    def potential_action(self, potential_action):
        """Sets the potential_action of this Thing.


        :param potential_action: The potential_action of this Thing.
        :type potential_action: List[Action]
        """

        self._potential_action = potential_action

    @property
    def preferred_clickthrough_url(self):
        """Gets the preferred_clickthrough_url of this Thing.


        :return: The preferred_clickthrough_url of this Thing.
        :rtype: str
        """
        return self._preferred_clickthrough_url

    @preferred_clickthrough_url.setter
    def preferred_clickthrough_url(self, preferred_clickthrough_url):
        """Sets the preferred_clickthrough_url of this Thing.


        :param preferred_clickthrough_url: The preferred_clickthrough_url of this Thing.
        :type preferred_clickthrough_url: str
        """

        self._preferred_clickthrough_url = preferred_clickthrough_url

    @property
    def read_link(self):
        """Gets the read_link of this Thing.

        The URL that returns this resource.

        :return: The read_link of this Thing.
        :rtype: str
        """
        return self._read_link

    @read_link.setter
    def read_link(self, read_link):
        """Sets the read_link of this Thing.

        The URL that returns this resource.

        :param read_link: The read_link of this Thing.
        :type read_link: str
        """

        self._read_link = read_link

    @property
    def web_search_url(self):
        """Gets the web_search_url of this Thing.

        The URL To Bing's search result for this item.

        :return: The web_search_url of this Thing.
        :rtype: str
        """
        return self._web_search_url

    @web_search_url.setter
    def web_search_url(self, web_search_url):
        """Sets the web_search_url of this Thing.

        The URL To Bing's search result for this item.

        :param web_search_url: The web_search_url of this Thing.
        :type web_search_url: str
        """

        self._web_search_url = web_search_url

    @property
    def id(self):
        """Gets the id of this Thing.

        A String identifier.

        :return: The id of this Thing.
        :rtype: str
        """
        return self._id

    @id.setter
    def id(self, id):
        """Sets the id of this Thing.

        A String identifier.

        :param id: The id of this Thing.
        :type id: str
        """

        self._id = id

    @property
    def type(self):
        """Gets the type of this Thing.


        :return: The type of this Thing.
        :rtype: str
        """
        return self._type

    @type.setter
    def type(self, type):
        """Sets the type of this Thing.


        :param type: The type of this Thing.
        :type type: str
        """
        if type is None:
            raise ValueError("Invalid value for `type`, must not be `None`")

        self._type = type
