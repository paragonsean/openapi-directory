...
import org.openapitools.client.model.CreateJiraIssue200ResponseJiraIssue;
...
public class JiraIssue {
  public static final String SERIALIZED_NAME_JIRA_ISSUE = "jiraIssue";
  @SerializedName(SERIALIZED_NAME_JIRA_ISSUE)
  private CreateJiraIssue200ResponseJiraIssue jiraIssue;

  public JiraIssue() {
  }

  public JiraIssue jiraIssue(CreateJiraIssue200ResponseJiraIssue jiraIssue) {
  ...
  public CreateJiraIssue200ResponseJiraIssue getJiraIssue() {
    return jiraIssue;
  }

  public void setJiraIssue(CreateJiraIssue200ResponseJiraIssue jiraIssue) {
    this.jiraIssue = jiraIssue;
  }
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(this.jiraIssue, jiraIssue.jiraIssue);
  }

  @Override
  public int hashCode() {
    return Objects.hash(jiraIssue);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to JiraIssue
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!JiraIssue.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in JiraIssue is not found in the empty JSON string", JiraIssue.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!JiraIssue.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `JiraIssue` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // validate the optional field `jiraIssue`
      if (jsonObj.get("jiraIssue") != null && !jsonObj.get("jiraIssue").isJsonNull()) {
        CreateJiraIssue200ResponseJiraIssue.validateJsonElement(jsonObj.get("jiraIssue"));
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!JiraIssue.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'JiraIssue' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<JiraIssue> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(JiraIssue.class));

       return (TypeAdapter<T>) new TypeAdapter<JiraIssue>() {
           @Override
           public void write(JsonWriter out, JiraIssue value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public JiraIssue read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of JiraIssue given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of JiraIssue
   * @throws IOException if the JSON string is invalid with respect to JiraIssue
   */
  public static JiraIssue fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, JiraIssue.class);
  }

  /**
   * Convert an instance of JiraIssue to an JSON string
   *
   * @return JSON string
   */
