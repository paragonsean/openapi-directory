...
public class PriorityScore {
  ...
  public PriorityScore max(BigDecimal max) {
    this.max = max;
    return this;
  }
  ...
  public PriorityScore min(BigDecimal min) {
    this.min = min;
    return this;
  }
  ...
  public boolean equals(Object o) {
    ...
    PriorityScore priorityScore = (PriorityScore) o;
    return Objects.equals(this.max, priorityScore.max) &&
        Objects.equals(this.min, priorityScore.min);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to PriorityScore
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PriorityScore.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PriorityScore is not found in the empty JSON string", PriorityScore.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PriorityScore.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PriorityScore` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PriorityScore.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PriorityScore' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PriorityScore> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PriorityScore.class));

       return (TypeAdapter<T>) new TypeAdapter<PriorityScore>() {
           @Override
           public void write(JsonWriter out, PriorityScore value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PriorityScore read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of PriorityScore given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of PriorityScore
   * @throws IOException if the JSON string is invalid with respect to PriorityScore
   */
  public static PriorityScore fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PriorityScore.class);
  }

  /**
   * Convert an instance of PriorityScore to an JSON string
   *
   * @return JSON string
   */
