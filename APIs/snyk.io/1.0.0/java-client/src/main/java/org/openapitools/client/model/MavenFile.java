...
public class MavenFile {
  ...
  public MavenFile contents(String contents) {
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(this.contents, mavenFile.contents);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to MavenFile
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!MavenFile.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in MavenFile is not found in the empty JSON string", MavenFile.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!MavenFile.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `MavenFile` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : MavenFile.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!MavenFile.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'MavenFile' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<MavenFile> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(MavenFile.class));

       return (TypeAdapter<T>) new TypeAdapter<MavenFile>() {
           @Override
           public void write(JsonWriter out, MavenFile value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public MavenFile read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of MavenFile given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of MavenFile
   * @throws IOException if the JSON string is invalid with respect to MavenFile
   */
  public static MavenFile fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, MavenFile.class);
  }

  /**
   * Convert an instance of MavenFile to an JSON string
   *
   * @return JSON string
   */
