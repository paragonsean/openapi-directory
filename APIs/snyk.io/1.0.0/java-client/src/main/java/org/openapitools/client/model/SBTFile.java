...
public class SBTFile {
  ...
  public SBTFile contents(String contents) {
  ...
  public boolean equals(Object o) {
    ...
    SBTFile sbTFile = (SBTFile) o;
    return Objects.equals(this.contents, sbTFile.contents);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to SBTFile
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SBTFile.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in SBTFile is not found in the empty JSON string", SBTFile.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SBTFile.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SBTFile` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : SBTFile.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!SBTFile.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'SBTFile' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<SBTFile> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(SBTFile.class));

       return (TypeAdapter<T>) new TypeAdapter<SBTFile>() {
           @Override
           public void write(JsonWriter out, SBTFile value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public SBTFile read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of SBTFile given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of SBTFile
   * @throws IOException if the JSON string is invalid with respect to SBTFile
   */
  public static SBTFile fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, SBTFile.class);
  }

  /**
   * Convert an instance of SBTFile to an JSON string
   *
   * @return JSON string
   */
