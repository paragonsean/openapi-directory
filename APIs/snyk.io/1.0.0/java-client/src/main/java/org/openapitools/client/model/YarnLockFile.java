...
public class YarnLockFile {
  ...
  public YarnLockFile contents(String contents) {
  ...
  public boolean equals(Object o) {
    ...
    YarnLockFile yarnLockFile = (YarnLockFile) o;
    return Objects.equals(this.contents, yarnLockFile.contents);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to YarnLockFile
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!YarnLockFile.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in YarnLockFile is not found in the empty JSON string", YarnLockFile.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!YarnLockFile.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `YarnLockFile` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!YarnLockFile.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'YarnLockFile' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<YarnLockFile> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(YarnLockFile.class));

       return (TypeAdapter<T>) new TypeAdapter<YarnLockFile>() {
           @Override
           public void write(JsonWriter out, YarnLockFile value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public YarnLockFile read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of YarnLockFile given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of YarnLockFile
   * @throws IOException if the JSON string is invalid with respect to YarnLockFile
   */
  public static YarnLockFile fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, YarnLockFile.class);
  }

  /**
   * Convert an instance of YarnLockFile to an JSON string
   *
   * @return JSON string
   */
