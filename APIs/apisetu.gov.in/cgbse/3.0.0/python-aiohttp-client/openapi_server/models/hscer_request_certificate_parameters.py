# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server import util


class HscerRequestCertificateParameters(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, full_name: str=None, examtype: str=None, rollno: str=None, year: str=None):
        """HscerRequestCertificateParameters - a model defined in OpenAPI

        :param full_name: The full_name of this HscerRequestCertificateParameters.
        :param examtype: The examtype of this HscerRequestCertificateParameters.
        :param rollno: The rollno of this HscerRequestCertificateParameters.
        :param year: The year of this HscerRequestCertificateParameters.
        """
        self.openapi_types = {
            'full_name': str,
            'examtype': str,
            'rollno': str,
            'year': str
        }

        self.attribute_map = {
            'full_name': 'FullName',
            'examtype': 'examtype',
            'rollno': 'rollno',
            'year': 'year'
        }

        self._full_name = full_name
        self._examtype = examtype
        self._rollno = rollno
        self._year = year

    @classmethod
    def from_dict(cls, dikt: dict) -> 'HscerRequestCertificateParameters':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The hscer_request_certificateParameters of this HscerRequestCertificateParameters.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def full_name(self):
        """Gets the full_name of this HscerRequestCertificateParameters.

        Full name

        :return: The full_name of this HscerRequestCertificateParameters.
        :rtype: str
        """
        return self._full_name

    @full_name.setter
    def full_name(self, full_name):
        """Sets the full_name of this HscerRequestCertificateParameters.

        Full name

        :param full_name: The full_name of this HscerRequestCertificateParameters.
        :type full_name: str
        """
        if full_name is None:
            raise ValueError("Invalid value for `full_name`, must not be `None`")

        self._full_name = full_name

    @property
    def examtype(self):
        """Gets the examtype of this HscerRequestCertificateParameters.

        Examination Type

        :return: The examtype of this HscerRequestCertificateParameters.
        :rtype: str
        """
        return self._examtype

    @examtype.setter
    def examtype(self, examtype):
        """Sets the examtype of this HscerRequestCertificateParameters.

        Examination Type

        :param examtype: The examtype of this HscerRequestCertificateParameters.
        :type examtype: str
        """
        if examtype is None:
            raise ValueError("Invalid value for `examtype`, must not be `None`")

        self._examtype = examtype

    @property
    def rollno(self):
        """Gets the rollno of this HscerRequestCertificateParameters.

        Roll No

        :return: The rollno of this HscerRequestCertificateParameters.
        :rtype: str
        """
        return self._rollno

    @rollno.setter
    def rollno(self, rollno):
        """Sets the rollno of this HscerRequestCertificateParameters.

        Roll No

        :param rollno: The rollno of this HscerRequestCertificateParameters.
        :type rollno: str
        """
        if rollno is None:
            raise ValueError("Invalid value for `rollno`, must not be `None`")

        self._rollno = rollno

    @property
    def year(self):
        """Gets the year of this HscerRequestCertificateParameters.

        Year

        :return: The year of this HscerRequestCertificateParameters.
        :rtype: str
        """
        return self._year

    @year.setter
    def year(self, year):
        """Sets the year of this HscerRequestCertificateParameters.

        Year

        :param year: The year of this HscerRequestCertificateParameters.
        :type year: str
        """
        if year is None:
            raise ValueError("Invalid value for `year`, must not be `None`")

        self._year = year
