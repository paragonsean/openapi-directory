...
class FixedFee {
     ...
     * @param formula {module:model/FixedFee.FormulaEnum} The price formula determines what algorithm is used to calculate the invoice price based on a few factors, - the quantity in the order (which may be variable if usage pricing, otherwise determined when creating the order) - the price brackets data  To determine which formula is correct, please see the price formula documentation. 
     * @param price {Number} For the very simple price when it's fixed and does not depend on the quantity chosen by customer.  If the price is 0, it's free. 
     */
    constructor(formula, price) { 
        PlanPriceFormula.initialize(this, formula);
        FixedFee.initialize(this, formula, price);
    }
     ...
     */
    static initialize(obj, formula, price) { 
        obj['price'] = price;
    }
...
FixedFee.RequiredProperties = ["price", "formula"];
 ...
 * @member {Number} price
 */
FixedFee.prototype['price'] = undefined;
