

# EMSAllOfCredentials

EMS credentials object.

## Properties

| Name | Type | Description | Notes |
|------------ | ------------- | ------------- | -------------|
|**clientCertificate** | **String** | Client Certificate. |  |
|**clientCertificatePassword** | **String** | Client Certificate password. |  |
|**merchantName** | **String** | Merchant Name for SFTP reconciliation. |  [optional] |
|**password** | **String** | EMS password. |  |
|**privateKey** | **String** | Private Key. |  |
|**privateKeyPassword** | **String** | Private key password. |  |
|**serverCertificate** | **String** | Server Certificate. |  |
|**sftpPrivateKey** | **String** | SFTP reconciliation private key. |  [optional] |
|**storeId** | **String** | EMS store id. |  |
|**userId** | **String** | EMS account id. |  |



