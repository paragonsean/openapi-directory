...
import org.openapitools.client.model.LinodeTypeAddonsBackups;
...
public class LinodeTypeAddons {
  ...
  private LinodeTypeAddonsBackups backups;

  public LinodeTypeAddons() {
  }

  public LinodeTypeAddons backups(LinodeTypeAddonsBackups backups) {
  ...
  public LinodeTypeAddonsBackups getBackups() {
    return backups;
  }

  public void setBackups(LinodeTypeAddonsBackups backups) {
    this.backups = backups;
  }
  ...
  public boolean equals(Object o) {
    ...
    LinodeTypeAddons linodeTypeAddons = (LinodeTypeAddons) o;
    return Objects.equals(this.backups, linodeTypeAddons.backups);
  }
  ...
  public String toString() {
    ...
    sb.append("class LinodeTypeAddons {\n");
   ...
   * @throws IOException if the JSON Element is invalid with respect to LinodeTypeAddons
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!LinodeTypeAddons.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in LinodeTypeAddons is not found in the empty JSON string", LinodeTypeAddons.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!LinodeTypeAddons.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `LinodeTypeAddons` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // validate the optional field `backups`
      if (jsonObj.get("backups") != null && !jsonObj.get("backups").isJsonNull()) {
        LinodeTypeAddonsBackups.validateJsonElement(jsonObj.get("backups"));
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!LinodeTypeAddons.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'LinodeTypeAddons' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<LinodeTypeAddons> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(LinodeTypeAddons.class));

       return (TypeAdapter<T>) new TypeAdapter<LinodeTypeAddons>() {
           @Override
           public void write(JsonWriter out, LinodeTypeAddons value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public LinodeTypeAddons read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of LinodeTypeAddons given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of LinodeTypeAddons
   * @throws IOException if the JSON string is invalid with respect to LinodeTypeAddons
   */
  public static LinodeTypeAddons fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, LinodeTypeAddons.class);
  }

  /**
   * Convert an instance of LinodeTypeAddons to an JSON string
   *
   * @return JSON string
   */
