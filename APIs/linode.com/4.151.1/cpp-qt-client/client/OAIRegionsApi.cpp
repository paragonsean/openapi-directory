...
void OAIRegionsApi::getRegion(const QString &region_id) {
    QString fullPath = QString(_serverConfigs["getRegion"][_serverIndices.value("getRegion")].URL()+"/regions/{regionId}");
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIRegionsApi::getRegionCallback);
...
void OAIRegionsApi::getRegionCallback(OAIHttpRequestWorker *worker) {
...
void OAIRegionsApi::getRegions() {
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIRegionsApi::getRegionsCallback);
    connect(this, &OAIRegionsApi::abortRequestsSignal, worker, &QObject::deleteLater);
    connect(worker, &QObject::destroyed, this, [this]() {
        if (findChildren<OAIHttpRequestWorker*>().count() == 0) {
            Q_EMIT allPendingRequestsCompleted();
...
void OAIRegionsApi::getRegionsCallback(OAIHttpRequestWorker *worker) {
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
    ...
    OAIGetRegions_200_response output(QString(worker->response));
    worker->deleteLater();
