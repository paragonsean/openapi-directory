/**
 * Product Finder API
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 2.2.1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import OtherFeesChargesInner from './OtherFeesChargesInner';

/**
 * The BCAMarketingStateInner model module.
 * @module model/BCAMarketingStateInner
 * @version 2.2.1
 */
class BCAMarketingStateInner {
    /**
     * Constructs a new <code>BCAMarketingStateInner</code>.
     * The marketing state (promotional or regular) of the BCA Product.
     * @alias module:model/BCAMarketingStateInner
     * @param identification {String} Unique and unambiguous identification of a  BCA Product Marketing State.
     * @param marketingState {module:model/BCAMarketingStateInner.MarketingStateEnum} Describes the marketing state (regular or promotional) of the BCA Product
     * @param otherFeesCharges {Array.<module:model/OtherFeesChargesInner>} Contains details of fees and charges which are not associated with either Overdraft or features/benefits
     */
    constructor(identification, marketingState, otherFeesCharges) { 
        
        BCAMarketingStateInner.initialize(this, identification, marketingState, otherFeesCharges);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj, identification, marketingState, otherFeesCharges) { 
        obj['Identification'] = identification;
        obj['MarketingState'] = marketingState;
        obj['OtherFeesCharges'] = otherFeesCharges;
    }

    /**
     * Constructs a <code>BCAMarketingStateInner</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/BCAMarketingStateInner} obj Optional instance to populate.
     * @return {module:model/BCAMarketingStateInner} The populated <code>BCAMarketingStateInner</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new BCAMarketingStateInner();

            if (data.hasOwnProperty('CoreProduct')) {
                obj['CoreProduct'] = ApiClient.convertToType(data['CoreProduct'], Object);
            }
            if (data.hasOwnProperty('CreditInterest')) {
                obj['CreditInterest'] = ApiClient.convertToType(data['CreditInterest'], Object);
            }
            if (data.hasOwnProperty('Eligibility')) {
                obj['Eligibility'] = ApiClient.convertToType(data['Eligibility'], Object);
            }
            if (data.hasOwnProperty('FeaturesAndBenefits')) {
                obj['FeaturesAndBenefits'] = ApiClient.convertToType(data['FeaturesAndBenefits'], Object);
            }
            if (data.hasOwnProperty('FirstMarketedDate')) {
                obj['FirstMarketedDate'] = ApiClient.convertToType(data['FirstMarketedDate'], 'Date');
            }
            if (data.hasOwnProperty('Identification')) {
                obj['Identification'] = ApiClient.convertToType(data['Identification'], 'String');
            }
            if (data.hasOwnProperty('LastMarketedDate')) {
                obj['LastMarketedDate'] = ApiClient.convertToType(data['LastMarketedDate'], 'Date');
            }
            if (data.hasOwnProperty('MarketingState')) {
                obj['MarketingState'] = ApiClient.convertToType(data['MarketingState'], 'String');
            }
            if (data.hasOwnProperty('Notes')) {
                obj['Notes'] = ApiClient.convertToType(data['Notes'], ['String']);
            }
            if (data.hasOwnProperty('OtherFeesCharges')) {
                obj['OtherFeesCharges'] = ApiClient.convertToType(data['OtherFeesCharges'], [OtherFeesChargesInner]);
            }
            if (data.hasOwnProperty('Overdraft')) {
                obj['Overdraft'] = ApiClient.convertToType(data['Overdraft'], Object);
            }
            if (data.hasOwnProperty('PredecessorID')) {
                obj['PredecessorID'] = ApiClient.convertToType(data['PredecessorID'], 'String');
            }
            if (data.hasOwnProperty('StateTenureLength')) {
                obj['StateTenureLength'] = ApiClient.convertToType(data['StateTenureLength'], 'Number');
            }
            if (data.hasOwnProperty('StateTenurePeriod')) {
                obj['StateTenurePeriod'] = ApiClient.convertToType(data['StateTenurePeriod'], 'String');
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>BCAMarketingStateInner</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>BCAMarketingStateInner</code>.
     */
    static validateJSON(data) {
        // check to make sure all required properties are present in the JSON string
        for (const property of BCAMarketingStateInner.RequiredProperties) {
            if (!data.hasOwnProperty(property)) {
                throw new Error("The required field `" + property + "` is not found in the JSON data: " + JSON.stringify(data));
            }
        }
        // ensure the json data is a string
        if (data['Identification'] && !(typeof data['Identification'] === 'string' || data['Identification'] instanceof String)) {
            throw new Error("Expected the field `Identification` to be a primitive type in the JSON string but got " + data['Identification']);
        }
        // ensure the json data is a string
        if (data['MarketingState'] && !(typeof data['MarketingState'] === 'string' || data['MarketingState'] instanceof String)) {
            throw new Error("Expected the field `MarketingState` to be a primitive type in the JSON string but got " + data['MarketingState']);
        }
        // ensure the json data is an array
        if (!Array.isArray(data['Notes'])) {
            throw new Error("Expected the field `Notes` to be an array in the JSON data but got " + data['Notes']);
        }
        if (data['OtherFeesCharges']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['OtherFeesCharges'])) {
                throw new Error("Expected the field `OtherFeesCharges` to be an array in the JSON data but got " + data['OtherFeesCharges']);
            }
            // validate the optional field `OtherFeesCharges` (array)
            for (const item of data['OtherFeesCharges']) {
                OtherFeesChargesInner.validateJSON(item);
            };
        }
        // ensure the json data is a string
        if (data['PredecessorID'] && !(typeof data['PredecessorID'] === 'string' || data['PredecessorID'] instanceof String)) {
            throw new Error("Expected the field `PredecessorID` to be a primitive type in the JSON string but got " + data['PredecessorID']);
        }
        // ensure the json data is a string
        if (data['StateTenurePeriod'] && !(typeof data['StateTenurePeriod'] === 'string' || data['StateTenurePeriod'] instanceof String)) {
            throw new Error("Expected the field `StateTenurePeriod` to be a primitive type in the JSON string but got " + data['StateTenurePeriod']);
        }

        return true;
    }


}

BCAMarketingStateInner.RequiredProperties = ["Identification", "MarketingState", "OtherFeesCharges"];

/**
 * BCA core product details.
 * @member {Object} CoreProduct
 */
BCAMarketingStateInner.prototype['CoreProduct'] = undefined;

/**
 * Details about the interest that may be payable to the BCA account holders
 * @member {Object} CreditInterest
 */
BCAMarketingStateInner.prototype['CreditInterest'] = undefined;

/**
 * Eligibility details for this product i.e. the criteria that an accountholder has to meet in order to be eligible for the BCA product.
 * @member {Object} Eligibility
 */
BCAMarketingStateInner.prototype['Eligibility'] = undefined;

/**
 * Feature And Benefits Details
 * @member {Object} FeaturesAndBenefits
 */
BCAMarketingStateInner.prototype['FeaturesAndBenefits'] = undefined;

/**
 * Marketing state start date
 * @member {Date} FirstMarketedDate
 */
BCAMarketingStateInner.prototype['FirstMarketedDate'] = undefined;

/**
 * Unique and unambiguous identification of a  BCA Product Marketing State.
 * @member {String} Identification
 */
BCAMarketingStateInner.prototype['Identification'] = undefined;

/**
 * Marketing state end date
 * @member {Date} LastMarketedDate
 */
BCAMarketingStateInner.prototype['LastMarketedDate'] = undefined;

/**
 * Describes the marketing state (regular or promotional) of the BCA Product
 * @member {module:model/BCAMarketingStateInner.MarketingStateEnum} MarketingState
 */
BCAMarketingStateInner.prototype['MarketingState'] = undefined;

/**
 * Free text for adding details for marketing state
 * @member {Array.<String>} Notes
 */
BCAMarketingStateInner.prototype['Notes'] = undefined;

/**
 * Contains details of fees and charges which are not associated with either Overdraft or features/benefits
 * @member {Array.<module:model/OtherFeesChargesInner>} OtherFeesCharges
 */
BCAMarketingStateInner.prototype['OtherFeesCharges'] = undefined;

/**
 * Borrowing details
 * @member {Object} Overdraft
 */
BCAMarketingStateInner.prototype['Overdraft'] = undefined;

/**
 * Identifies the marketing state that precedes this marketing state
 * @member {String} PredecessorID
 */
BCAMarketingStateInner.prototype['PredecessorID'] = undefined;

/**
 * The length/duration of a promotional state
 * @member {Number} StateTenureLength
 */
BCAMarketingStateInner.prototype['StateTenureLength'] = undefined;

/**
 * The unit of period (days, weeks, months etc.) of the promotional length
 * @member {module:model/BCAMarketingStateInner.StateTenurePeriodEnum} StateTenurePeriod
 */
BCAMarketingStateInner.prototype['StateTenurePeriod'] = undefined;





/**
 * Allowed values for the <code>MarketingState</code> property.
 * @enum {String}
 * @readonly
 */
BCAMarketingStateInner['MarketingStateEnum'] = {

    /**
     * value: "Promotional"
     * @const
     */
    "Promotional": "Promotional",

    /**
     * value: "Regular"
     * @const
     */
    "Regular": "Regular"
};


/**
 * Allowed values for the <code>StateTenurePeriod</code> property.
 * @enum {String}
 * @readonly
 */
BCAMarketingStateInner['StateTenurePeriodEnum'] = {

    /**
     * value: "Day"
     * @const
     */
    "Day": "Day",

    /**
     * value: "Half Year"
     * @const
     */
    "Half Year": "Half Year",

    /**
     * value: "Month"
     * @const
     */
    "Month": "Month",

    /**
     * value: "Quarter"
     * @const
     */
    "Quarter": "Quarter",

    /**
     * value: "Week"
     * @const
     */
    "Week": "Week",

    /**
     * value: "AcademicTerm"
     * @const
     */
    "AcademicTerm": "AcademicTerm",

    /**
     * value: "Year"
     * @const
     */
    "Year": "Year"
};



export default BCAMarketingStateInner;

