...
async def test_open_banking_v22_personal_current_accounts_get(client):
    ...
    response = await client.request(
        method='GET',
        path='/open-banking/v2.2/personal-current-accounts',
        headers=headers,
        )
...
async def test_x_open_banking_v22_personal_current_accounts_segment_segment_get(client):
    ...
    response = await client.request(
        method='GET',
        path='/x-open-banking/v2.2/personal-current-accounts/segment/{segment}'.format(segment='segment_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
