...
> NAMeasureResponse getmeasure_1(deviceId, scale, type, opts)
...
let apiInstance = new Netatmo.PartnerApi();
...
apiInstance.getmeasure_1(deviceId, scale, type, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
let apiInstance = new Netatmo.PartnerApi();
apiInstance.partnerdevices((error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
