...
async def getmeasure_2(request: web.Request, device_id, scale, type, module_id=None, date_begin=None, date_end=None, limit=None, optimize=None, real_time=None) -> web.Response:
    """getmeasure_2
...
async def getpublicdata(request: web.Request, lat_ne, lon_ne, lat_sw, lon_sw, required_data=None, filter=None) -> web.Response:
    """getpublicdata
    ...
    :type lon_sw: int
    :param required_data: To filter stations based on relevant measurements you want (e.g. rain will only return stations with rain gauges). Default is no filter. You can find all measurements available on the Thermostat page.
    :type required_data: List[str]
    :param filter: True to exclude stations with abnormal temperature measures. Default is false.
    :type filter: bool
