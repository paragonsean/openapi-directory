...
public class DcimPowerPanelsList200Response {
   ...
   * @throws IOException if the JSON Element is invalid with respect to DcimPowerPanelsList200Response
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DcimPowerPanelsList200Response.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DcimPowerPanelsList200Response.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DcimPowerPanelsList200Response` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayresults.size(); i++) {
        PowerPanel.validateJsonElement(jsonArrayresults.get(i));
      };
  }
