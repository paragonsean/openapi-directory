...
public class VirtualizationClusterTypesList200Response {
   ...
   * @throws IOException if the JSON Element is invalid with respect to VirtualizationClusterTypesList200Response
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!VirtualizationClusterTypesList200Response.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!VirtualizationClusterTypesList200Response.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `VirtualizationClusterTypesList200Response` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayresults.size(); i++) {
        ClusterType.validateJsonElement(jsonArrayresults.get(i));
      };
  }
