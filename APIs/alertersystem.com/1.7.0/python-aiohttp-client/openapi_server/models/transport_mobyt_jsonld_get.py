# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.alert_log_jsonld_get_context import AlertLogJsonldGetContext
from openapi_server import util


class TransportMobytJsonldGet(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, context: AlertLogJsonldGetContext=None, id: str=None, type: str=None, created_at: datetime=None, data_segment_code: str=None, id: str=None, mobyt_access_token: str=None, mobyt_from: str=None, mobyt_type_quality: str=None, mobyt_user_key: str=None, partition: str=None, resource_owner: str=None, transport_name: str=None):
        """TransportMobytJsonldGet - a model defined in OpenAPI

        :param context: The context of this TransportMobytJsonldGet.
        :param id: The id of this TransportMobytJsonldGet.
        :param type: The type of this TransportMobytJsonldGet.
        :param created_at: The created_at of this TransportMobytJsonldGet.
        :param data_segment_code: The data_segment_code of this TransportMobytJsonldGet.
        :param id: The id of this TransportMobytJsonldGet.
        :param mobyt_access_token: The mobyt_access_token of this TransportMobytJsonldGet.
        :param mobyt_from: The mobyt_from of this TransportMobytJsonldGet.
        :param mobyt_type_quality: The mobyt_type_quality of this TransportMobytJsonldGet.
        :param mobyt_user_key: The mobyt_user_key of this TransportMobytJsonldGet.
        :param partition: The partition of this TransportMobytJsonldGet.
        :param resource_owner: The resource_owner of this TransportMobytJsonldGet.
        :param transport_name: The transport_name of this TransportMobytJsonldGet.
        """
        self.openapi_types = {
            'context': AlertLogJsonldGetContext,
            'id': str,
            'type': str,
            'created_at': datetime,
            'data_segment_code': str,
            'id': str,
            'mobyt_access_token': str,
            'mobyt_from': str,
            'mobyt_type_quality': str,
            'mobyt_user_key': str,
            'partition': str,
            'resource_owner': str,
            'transport_name': str
        }

        self.attribute_map = {
            'context': '@context',
            'id': '@id',
            'type': '@type',
            'created_at': 'createdAt',
            'data_segment_code': 'dataSegmentCode',
            'id': 'id',
            'mobyt_access_token': 'mobytAccessToken',
            'mobyt_from': 'mobytFrom',
            'mobyt_type_quality': 'mobytTypeQuality',
            'mobyt_user_key': 'mobytUserKey',
            'partition': 'partition',
            'resource_owner': 'resourceOwner',
            'transport_name': 'transportName'
        }

        self._context = context
        self._id = id
        self._type = type
        self._created_at = created_at
        self._data_segment_code = data_segment_code
        self._id = id
        self._mobyt_access_token = mobyt_access_token
        self._mobyt_from = mobyt_from
        self._mobyt_type_quality = mobyt_type_quality
        self._mobyt_user_key = mobyt_user_key
        self._partition = partition
        self._resource_owner = resource_owner
        self._transport_name = transport_name

    @classmethod
    def from_dict(cls, dikt: dict) -> 'TransportMobytJsonldGet':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The TransportMobyt.jsonld-get of this TransportMobytJsonldGet.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def context(self):
        """Gets the context of this TransportMobytJsonldGet.


        :return: The context of this TransportMobytJsonldGet.
        :rtype: AlertLogJsonldGetContext
        """
        return self._context

    @context.setter
    def context(self, context):
        """Sets the context of this TransportMobytJsonldGet.


        :param context: The context of this TransportMobytJsonldGet.
        :type context: AlertLogJsonldGetContext
        """

        self._context = context

    @property
    def id(self):
        """Gets the id of this TransportMobytJsonldGet.


        :return: The id of this TransportMobytJsonldGet.
        :rtype: str
        """
        return self._id

    @id.setter
    def id(self, id):
        """Sets the id of this TransportMobytJsonldGet.


        :param id: The id of this TransportMobytJsonldGet.
        :type id: str
        """

        self._id = id

    @property
    def type(self):
        """Gets the type of this TransportMobytJsonldGet.


        :return: The type of this TransportMobytJsonldGet.
        :rtype: str
        """
        return self._type

    @type.setter
    def type(self, type):
        """Sets the type of this TransportMobytJsonldGet.


        :param type: The type of this TransportMobytJsonldGet.
        :type type: str
        """

        self._type = type

    @property
    def created_at(self):
        """Gets the created_at of this TransportMobytJsonldGet.

        When the resource instance was created. This date-time is in the UTC timezone.

        :return: The created_at of this TransportMobytJsonldGet.
        :rtype: datetime
        """
        return self._created_at

    @created_at.setter
    def created_at(self, created_at):
        """Sets the created_at of this TransportMobytJsonldGet.

        When the resource instance was created. This date-time is in the UTC timezone.

        :param created_at: The created_at of this TransportMobytJsonldGet.
        :type created_at: datetime
        """

        self._created_at = created_at

    @property
    def data_segment_code(self):
        """Gets the data_segment_code of this TransportMobytJsonldGet.

        User-provided string on which to segment and filter data. Max 50 characters.

        :return: The data_segment_code of this TransportMobytJsonldGet.
        :rtype: str
        """
        return self._data_segment_code

    @data_segment_code.setter
    def data_segment_code(self, data_segment_code):
        """Sets the data_segment_code of this TransportMobytJsonldGet.

        User-provided string on which to segment and filter data. Max 50 characters.

        :param data_segment_code: The data_segment_code of this TransportMobytJsonldGet.
        :type data_segment_code: str
        """
        if data_segment_code is not None and len(data_segment_code) > 50:
            raise ValueError("Invalid value for `data_segment_code`, length must be less than or equal to `50`")

        self._data_segment_code = data_segment_code

    @property
    def id(self):
        """Gets the id of this TransportMobytJsonldGet.

        The unique identifier of the resource instance.

        :return: The id of this TransportMobytJsonldGet.
        :rtype: str
        """
        return self._id

    @id.setter
    def id(self, id):
        """Sets the id of this TransportMobytJsonldGet.

        The unique identifier of the resource instance.

        :param id: The id of this TransportMobytJsonldGet.
        :type id: str
        """

        self._id = id

    @property
    def mobyt_access_token(self):
        """Gets the mobyt_access_token of this TransportMobytJsonldGet.

        The access token for the Mobyt service. Stored in encrypted format.

        :return: The mobyt_access_token of this TransportMobytJsonldGet.
        :rtype: str
        """
        return self._mobyt_access_token

    @mobyt_access_token.setter
    def mobyt_access_token(self, mobyt_access_token):
        """Sets the mobyt_access_token of this TransportMobytJsonldGet.

        The access token for the Mobyt service. Stored in encrypted format.

        :param mobyt_access_token: The mobyt_access_token of this TransportMobytJsonldGet.
        :type mobyt_access_token: str
        """
        if mobyt_access_token is None:
            raise ValueError("Invalid value for `mobyt_access_token`, must not be `None`")
        if mobyt_access_token is not None and len(mobyt_access_token) > 1000:
            raise ValueError("Invalid value for `mobyt_access_token`, length must be less than or equal to `1000`")

        self._mobyt_access_token = mobyt_access_token

    @property
    def mobyt_from(self):
        """Gets the mobyt_from of this TransportMobytJsonldGet.

        The sender for the Mobyt service.

        :return: The mobyt_from of this TransportMobytJsonldGet.
        :rtype: str
        """
        return self._mobyt_from

    @mobyt_from.setter
    def mobyt_from(self, mobyt_from):
        """Sets the mobyt_from of this TransportMobytJsonldGet.

        The sender for the Mobyt service.

        :param mobyt_from: The mobyt_from of this TransportMobytJsonldGet.
        :type mobyt_from: str
        """
        if mobyt_from is None:
            raise ValueError("Invalid value for `mobyt_from`, must not be `None`")
        if mobyt_from is not None and len(mobyt_from) > 255:
            raise ValueError("Invalid value for `mobyt_from`, length must be less than or equal to `255`")

        self._mobyt_from = mobyt_from

    @property
    def mobyt_type_quality(self):
        """Gets the mobyt_type_quality of this TransportMobytJsonldGet.

        The quality of your message: 'N' for high, 'L' for medium, 'LL' for low, for the Mobyt service.

        :return: The mobyt_type_quality of this TransportMobytJsonldGet.
        :rtype: str
        """
        return self._mobyt_type_quality

    @mobyt_type_quality.setter
    def mobyt_type_quality(self, mobyt_type_quality):
        """Sets the mobyt_type_quality of this TransportMobytJsonldGet.

        The quality of your message: 'N' for high, 'L' for medium, 'LL' for low, for the Mobyt service.

        :param mobyt_type_quality: The mobyt_type_quality of this TransportMobytJsonldGet.
        :type mobyt_type_quality: str
        """
        if mobyt_type_quality is None:
            raise ValueError("Invalid value for `mobyt_type_quality`, must not be `None`")
        if mobyt_type_quality is not None and len(mobyt_type_quality) > 255:
            raise ValueError("Invalid value for `mobyt_type_quality`, length must be less than or equal to `255`")

        self._mobyt_type_quality = mobyt_type_quality

    @property
    def mobyt_user_key(self):
        """Gets the mobyt_user_key of this TransportMobytJsonldGet.

        The user key for the Mobyt service.

        :return: The mobyt_user_key of this TransportMobytJsonldGet.
        :rtype: str
        """
        return self._mobyt_user_key

    @mobyt_user_key.setter
    def mobyt_user_key(self, mobyt_user_key):
        """Sets the mobyt_user_key of this TransportMobytJsonldGet.

        The user key for the Mobyt service.

        :param mobyt_user_key: The mobyt_user_key of this TransportMobytJsonldGet.
        :type mobyt_user_key: str
        """
        if mobyt_user_key is None:
            raise ValueError("Invalid value for `mobyt_user_key`, must not be `None`")
        if mobyt_user_key is not None and len(mobyt_user_key) > 255:
            raise ValueError("Invalid value for `mobyt_user_key`, length must be less than or equal to `255`")

        self._mobyt_user_key = mobyt_user_key

    @property
    def partition(self):
        """Gets the partition of this TransportMobytJsonldGet.

        The partition that contains this resource instance. The resource cannot be moved to another partition.

        :return: The partition of this TransportMobytJsonldGet.
        :rtype: str
        """
        return self._partition

    @partition.setter
    def partition(self, partition):
        """Sets the partition of this TransportMobytJsonldGet.

        The partition that contains this resource instance. The resource cannot be moved to another partition.

        :param partition: The partition of this TransportMobytJsonldGet.
        :type partition: str
        """
        if partition is None:
            raise ValueError("Invalid value for `partition`, must not be `None`")

        self._partition = partition

    @property
    def resource_owner(self):
        """Gets the resource_owner of this TransportMobytJsonldGet.

        The name of the person who owns this resource.

        :return: The resource_owner of this TransportMobytJsonldGet.
        :rtype: str
        """
        return self._resource_owner

    @resource_owner.setter
    def resource_owner(self, resource_owner):
        """Sets the resource_owner of this TransportMobytJsonldGet.

        The name of the person who owns this resource.

        :param resource_owner: The resource_owner of this TransportMobytJsonldGet.
        :type resource_owner: str
        """

        self._resource_owner = resource_owner

    @property
    def transport_name(self):
        """Gets the transport_name of this TransportMobytJsonldGet.

        The name of the transport.

        :return: The transport_name of this TransportMobytJsonldGet.
        :rtype: str
        """
        return self._transport_name

    @transport_name.setter
    def transport_name(self, transport_name):
        """Sets the transport_name of this TransportMobytJsonldGet.

        The name of the transport.

        :param transport_name: The transport_name of this TransportMobytJsonldGet.
        :type transport_name: str
        """
        if transport_name is None:
            raise ValueError("Invalid value for `transport_name`, must not be `None`")
        if transport_name is not None and len(transport_name) > 255:
            raise ValueError("Invalid value for `transport_name`, length must be less than or equal to `255`")

        self._transport_name = transport_name
