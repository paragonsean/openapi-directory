# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server import util


class TransportEmailGet(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, created_at: datetime=None, data_segment_code: str=None, email_from_address: str=None, email_from_name: str=None, email_password: str=None, email_port: int=None, email_server: str=None, email_username: str=None, id: str=None, partition: str=None, resource_owner: str=None, transport_name: str=None):
        """TransportEmailGet - a model defined in OpenAPI

        :param created_at: The created_at of this TransportEmailGet.
        :param data_segment_code: The data_segment_code of this TransportEmailGet.
        :param email_from_address: The email_from_address of this TransportEmailGet.
        :param email_from_name: The email_from_name of this TransportEmailGet.
        :param email_password: The email_password of this TransportEmailGet.
        :param email_port: The email_port of this TransportEmailGet.
        :param email_server: The email_server of this TransportEmailGet.
        :param email_username: The email_username of this TransportEmailGet.
        :param id: The id of this TransportEmailGet.
        :param partition: The partition of this TransportEmailGet.
        :param resource_owner: The resource_owner of this TransportEmailGet.
        :param transport_name: The transport_name of this TransportEmailGet.
        """
        self.openapi_types = {
            'created_at': datetime,
            'data_segment_code': str,
            'email_from_address': str,
            'email_from_name': str,
            'email_password': str,
            'email_port': int,
            'email_server': str,
            'email_username': str,
            'id': str,
            'partition': str,
            'resource_owner': str,
            'transport_name': str
        }

        self.attribute_map = {
            'created_at': 'createdAt',
            'data_segment_code': 'dataSegmentCode',
            'email_from_address': 'emailFromAddress',
            'email_from_name': 'emailFromName',
            'email_password': 'emailPassword',
            'email_port': 'emailPort',
            'email_server': 'emailServer',
            'email_username': 'emailUsername',
            'id': 'id',
            'partition': 'partition',
            'resource_owner': 'resourceOwner',
            'transport_name': 'transportName'
        }

        self._created_at = created_at
        self._data_segment_code = data_segment_code
        self._email_from_address = email_from_address
        self._email_from_name = email_from_name
        self._email_password = email_password
        self._email_port = email_port
        self._email_server = email_server
        self._email_username = email_username
        self._id = id
        self._partition = partition
        self._resource_owner = resource_owner
        self._transport_name = transport_name

    @classmethod
    def from_dict(cls, dikt: dict) -> 'TransportEmailGet':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The TransportEmail-get of this TransportEmailGet.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def created_at(self):
        """Gets the created_at of this TransportEmailGet.

        When the resource instance was created. This date-time is in the UTC timezone.

        :return: The created_at of this TransportEmailGet.
        :rtype: datetime
        """
        return self._created_at

    @created_at.setter
    def created_at(self, created_at):
        """Sets the created_at of this TransportEmailGet.

        When the resource instance was created. This date-time is in the UTC timezone.

        :param created_at: The created_at of this TransportEmailGet.
        :type created_at: datetime
        """

        self._created_at = created_at

    @property
    def data_segment_code(self):
        """Gets the data_segment_code of this TransportEmailGet.

        User-provided string on which to segment and filter data. Max 50 characters.

        :return: The data_segment_code of this TransportEmailGet.
        :rtype: str
        """
        return self._data_segment_code

    @data_segment_code.setter
    def data_segment_code(self, data_segment_code):
        """Sets the data_segment_code of this TransportEmailGet.

        User-provided string on which to segment and filter data. Max 50 characters.

        :param data_segment_code: The data_segment_code of this TransportEmailGet.
        :type data_segment_code: str
        """
        if data_segment_code is not None and len(data_segment_code) > 50:
            raise ValueError("Invalid value for `data_segment_code`, length must be less than or equal to `50`")

        self._data_segment_code = data_segment_code

    @property
    def email_from_address(self):
        """Gets the email_from_address of this TransportEmailGet.

        The sender email address for the SMTP Email service.

        :return: The email_from_address of this TransportEmailGet.
        :rtype: str
        """
        return self._email_from_address

    @email_from_address.setter
    def email_from_address(self, email_from_address):
        """Sets the email_from_address of this TransportEmailGet.

        The sender email address for the SMTP Email service.

        :param email_from_address: The email_from_address of this TransportEmailGet.
        :type email_from_address: str
        """
        if email_from_address is None:
            raise ValueError("Invalid value for `email_from_address`, must not be `None`")
        if email_from_address is not None and len(email_from_address) > 255:
            raise ValueError("Invalid value for `email_from_address`, length must be less than or equal to `255`")

        self._email_from_address = email_from_address

    @property
    def email_from_name(self):
        """Gets the email_from_name of this TransportEmailGet.

        The sender name for the SMTP Email service.

        :return: The email_from_name of this TransportEmailGet.
        :rtype: str
        """
        return self._email_from_name

    @email_from_name.setter
    def email_from_name(self, email_from_name):
        """Sets the email_from_name of this TransportEmailGet.

        The sender name for the SMTP Email service.

        :param email_from_name: The email_from_name of this TransportEmailGet.
        :type email_from_name: str
        """
        if email_from_name is None:
            raise ValueError("Invalid value for `email_from_name`, must not be `None`")
        if email_from_name is not None and len(email_from_name) > 255:
            raise ValueError("Invalid value for `email_from_name`, length must be less than or equal to `255`")

        self._email_from_name = email_from_name

    @property
    def email_password(self):
        """Gets the email_password of this TransportEmailGet.

        The password for the SMTP Email service. Stored in encrypted format.

        :return: The email_password of this TransportEmailGet.
        :rtype: str
        """
        return self._email_password

    @email_password.setter
    def email_password(self, email_password):
        """Sets the email_password of this TransportEmailGet.

        The password for the SMTP Email service. Stored in encrypted format.

        :param email_password: The email_password of this TransportEmailGet.
        :type email_password: str
        """
        if email_password is None:
            raise ValueError("Invalid value for `email_password`, must not be `None`")
        if email_password is not None and len(email_password) > 1000:
            raise ValueError("Invalid value for `email_password`, length must be less than or equal to `1000`")

        self._email_password = email_password

    @property
    def email_port(self):
        """Gets the email_port of this TransportEmailGet.

        The port for the SMTP Email service.

        :return: The email_port of this TransportEmailGet.
        :rtype: int
        """
        return self._email_port

    @email_port.setter
    def email_port(self, email_port):
        """Sets the email_port of this TransportEmailGet.

        The port for the SMTP Email service.

        :param email_port: The email_port of this TransportEmailGet.
        :type email_port: int
        """
        if email_port is None:
            raise ValueError("Invalid value for `email_port`, must not be `None`")
        if email_port is not None and email_port <= 0:
            raise ValueError("Invalid value for `email_port`, must be a value greater than `0`")

        self._email_port = email_port

    @property
    def email_server(self):
        """Gets the email_server of this TransportEmailGet.

        The server for the SMTP Email service.

        :return: The email_server of this TransportEmailGet.
        :rtype: str
        """
        return self._email_server

    @email_server.setter
    def email_server(self, email_server):
        """Sets the email_server of this TransportEmailGet.

        The server for the SMTP Email service.

        :param email_server: The email_server of this TransportEmailGet.
        :type email_server: str
        """
        if email_server is None:
            raise ValueError("Invalid value for `email_server`, must not be `None`")
        if email_server is not None and len(email_server) > 255:
            raise ValueError("Invalid value for `email_server`, length must be less than or equal to `255`")

        self._email_server = email_server

    @property
    def email_username(self):
        """Gets the email_username of this TransportEmailGet.

        The username for the SMTP Email service.

        :return: The email_username of this TransportEmailGet.
        :rtype: str
        """
        return self._email_username

    @email_username.setter
    def email_username(self, email_username):
        """Sets the email_username of this TransportEmailGet.

        The username for the SMTP Email service.

        :param email_username: The email_username of this TransportEmailGet.
        :type email_username: str
        """
        if email_username is None:
            raise ValueError("Invalid value for `email_username`, must not be `None`")
        if email_username is not None and len(email_username) > 255:
            raise ValueError("Invalid value for `email_username`, length must be less than or equal to `255`")

        self._email_username = email_username

    @property
    def id(self):
        """Gets the id of this TransportEmailGet.

        The unique identifier of the resource instance.

        :return: The id of this TransportEmailGet.
        :rtype: str
        """
        return self._id

    @id.setter
    def id(self, id):
        """Sets the id of this TransportEmailGet.

        The unique identifier of the resource instance.

        :param id: The id of this TransportEmailGet.
        :type id: str
        """

        self._id = id

    @property
    def partition(self):
        """Gets the partition of this TransportEmailGet.

        The partition that contains this resource instance. The resource cannot be moved to another partition.

        :return: The partition of this TransportEmailGet.
        :rtype: str
        """
        return self._partition

    @partition.setter
    def partition(self, partition):
        """Sets the partition of this TransportEmailGet.

        The partition that contains this resource instance. The resource cannot be moved to another partition.

        :param partition: The partition of this TransportEmailGet.
        :type partition: str
        """
        if partition is None:
            raise ValueError("Invalid value for `partition`, must not be `None`")

        self._partition = partition

    @property
    def resource_owner(self):
        """Gets the resource_owner of this TransportEmailGet.

        The name of the person who owns this resource.

        :return: The resource_owner of this TransportEmailGet.
        :rtype: str
        """
        return self._resource_owner

    @resource_owner.setter
    def resource_owner(self, resource_owner):
        """Sets the resource_owner of this TransportEmailGet.

        The name of the person who owns this resource.

        :param resource_owner: The resource_owner of this TransportEmailGet.
        :type resource_owner: str
        """

        self._resource_owner = resource_owner

    @property
    def transport_name(self):
        """Gets the transport_name of this TransportEmailGet.

        The name of the transport.

        :return: The transport_name of this TransportEmailGet.
        :rtype: str
        """
        return self._transport_name

    @transport_name.setter
    def transport_name(self, transport_name):
        """Sets the transport_name of this TransportEmailGet.

        The name of the transport.

        :param transport_name: The transport_name of this TransportEmailGet.
        :type transport_name: str
        """
        if transport_name is None:
            raise ValueError("Invalid value for `transport_name`, must not be `None`")
        if transport_name is not None and len(transport_name) > 255:
            raise ValueError("Invalid value for `transport_name`, length must be less than or equal to `255`")

        self._transport_name = transport_name
