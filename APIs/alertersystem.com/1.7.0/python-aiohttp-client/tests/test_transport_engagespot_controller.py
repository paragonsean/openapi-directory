# coding: utf-8

import pytest
import json
from aiohttp import web

from openapi_server.models.api_transport_engagespot_get_collection200_response import ApiTransportEngagespotGetCollection200Response
from openapi_server.models.transport_engagespot_get import TransportEngagespotGet
from openapi_server.models.transport_engagespot_jsonld_get import TransportEngagespotJsonldGet
from openapi_server.models.transport_engagespot_jsonld_post import TransportEngagespotJsonldPost
from openapi_server.models.transport_engagespot_jsonld_put import TransportEngagespotJsonldPut
from openapi_server.models.transport_engagespot_patch import TransportEngagespotPatch
from openapi_server.models.transport_engagespot_post import TransportEngagespotPost
from openapi_server.models.transport_engagespot_put import TransportEngagespotPut


pytestmark = pytest.mark.asyncio

async def test_api_transport_engagespot_get_collection(client):
    """Test case for api_transport_engagespot_get_collection

    Retrieves the collection of TransportEngagespot resources.
    """
    params = [('page', 1),
                    ('dataSegmentCode', 'data_segment_code_example'),
                    ('dataSegmentCode[]', ['data_segment_code_example']),
                    ('partition', 'partition_example'),
                    ('partition[]', ['partition_example']),
                    ('properties[]', ['properties_example'])]
    headers = { 
        'Accept': 'application/json',
        'Authorization': 'Bearer special-key',
    }
    response = await client.request(
        method='GET',
        path='/api/transport-engagespot',
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')


pytestmark = pytest.mark.asyncio

async def test_api_transport_engagespot_id_delete(client):
    """Test case for api_transport_engagespot_id_delete

    Removes the TransportEngagespot resource.
    """
    headers = { 
        'Authorization': 'Bearer special-key',
    }
    response = await client.request(
        method='DELETE',
        path='/api/transport-engagespot/{id}'.format(id='id_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')


pytestmark = pytest.mark.asyncio

async def test_api_transport_engagespot_id_get(client):
    """Test case for api_transport_engagespot_id_get

    Retrieves a TransportEngagespot resource.
    """
    headers = { 
        'Accept': 'application/json',
        'Authorization': 'Bearer special-key',
    }
    response = await client.request(
        method='GET',
        path='/api/transport-engagespot/{id}'.format(id='id_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')


pytestmark = pytest.mark.asyncio

async def test_api_transport_engagespot_id_patch(client):
    """Test case for api_transport_engagespot_id_patch

    Updates the TransportEngagespot resource.
    """
    body = openapi_server.TransportEngagespotPatch()
    headers = { 
        'Accept': 'application/json',
        'Content-Type': 'application/merge-patch+json',
        'Authorization': 'Bearer special-key',
    }
    response = await client.request(
        method='PATCH',
        path='/api/transport-engagespot/{id}'.format(id='id_example'),
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')


pytestmark = pytest.mark.asyncio

@pytest.mark.skip("Connexion does not support multiple consumes. See https://github.com/zalando/connexion/pull/760")
async def test_api_transport_engagespot_id_put(client):
    """Test case for api_transport_engagespot_id_put

    Replaces the TransportEngagespot resource.
    """
    body = openapi_server.TransportEngagespotPut()
    headers = { 
        'Accept': 'application/json',
        'Content-Type': 'application/json',
        'Authorization': 'Bearer special-key',
    }
    response = await client.request(
        method='PUT',
        path='/api/transport-engagespot/{id}'.format(id='id_example'),
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')


pytestmark = pytest.mark.asyncio

@pytest.mark.skip("Connexion does not support multiple consumes. See https://github.com/zalando/connexion/pull/760")
async def test_api_transport_engagespot_post(client):
    """Test case for api_transport_engagespot_post

    Creates a TransportEngagespot resource.
    """
    body = openapi_server.TransportEngagespotPost()
    headers = { 
        'Accept': 'application/json',
        'Content-Type': 'application/json',
        'Authorization': 'Bearer special-key',
    }
    response = await client.request(
        method='POST',
        path='/api/transport-engagespot',
        headers=headers,
        json=body,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')

