# AlerterSystemApi.HttpMethodCodeApi
...
> [HttpMethodCodeGet] apiHttpMethodCodeGetCollection(opts)
...
let apiInstance = new AlerterSystemApi.HttpMethodCodeApi();
let opts = {
  'page': 1, // Number | The collection page number
  'properties': ["null"] // [String] | Allows you to reduce the response to contain only the properties you need. If your desired property is nested, you can address it using nested arrays. Example: properties[]={propertyName}&properties[]={anotherPropertyName}&properties[{nestedPropertyParent}][]={nestedProperty}
};
apiInstance.apiHttpMethodCodeGetCollection(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
let apiInstance = new AlerterSystemApi.HttpMethodCodeApi();
let id = "id_example"; // String | HttpMethodCode identifier
apiInstance.apiHttpMethodCodeIdGet(id, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
