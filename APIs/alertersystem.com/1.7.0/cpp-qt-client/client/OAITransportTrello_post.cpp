/**
 * Alerter System API
 * <p>This is the <a href=\"/\">Alerter System</a> API playground. More documentation is available at the <a href=\"/help/developers/\">API Help Center</a>.</p><p>The \"Available Authorizations\" in the Authorize popup only applies to this playground web interface. Other <a href=\"/help/developers/authorization/\">authorizations</a> are available for the actual API.</p>
 *
 * The version of the OpenAPI document: 1.7.0
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAITransportTrello_post.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAITransportTrello_post::OAITransportTrello_post(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAITransportTrello_post::OAITransportTrello_post() {
    this->initializeModel();
}

OAITransportTrello_post::~OAITransportTrello_post() {}

void OAITransportTrello_post::initializeModel() {

    m_data_segment_code_isSet = false;
    m_data_segment_code_isValid = false;

    m_partition_isSet = false;
    m_partition_isValid = false;

    m_transport_name_isSet = false;
    m_transport_name_isValid = false;

    m_trello_api_key_isSet = false;
    m_trello_api_key_isValid = false;

    m_trello_api_token_isSet = false;
    m_trello_api_token_isValid = false;

    m_trello_list_id_isSet = false;
    m_trello_list_id_isValid = false;
}

void OAITransportTrello_post::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAITransportTrello_post::fromJsonObject(QJsonObject json) {

    m_data_segment_code_isValid = ::OpenAPI::fromJsonValue(m_data_segment_code, json[QString("dataSegmentCode")]);
    m_data_segment_code_isSet = !json[QString("dataSegmentCode")].isNull() && m_data_segment_code_isValid;

    m_partition_isValid = ::OpenAPI::fromJsonValue(m_partition, json[QString("partition")]);
    m_partition_isSet = !json[QString("partition")].isNull() && m_partition_isValid;

    m_transport_name_isValid = ::OpenAPI::fromJsonValue(m_transport_name, json[QString("transportName")]);
    m_transport_name_isSet = !json[QString("transportName")].isNull() && m_transport_name_isValid;

    m_trello_api_key_isValid = ::OpenAPI::fromJsonValue(m_trello_api_key, json[QString("trelloApiKey")]);
    m_trello_api_key_isSet = !json[QString("trelloApiKey")].isNull() && m_trello_api_key_isValid;

    m_trello_api_token_isValid = ::OpenAPI::fromJsonValue(m_trello_api_token, json[QString("trelloApiToken")]);
    m_trello_api_token_isSet = !json[QString("trelloApiToken")].isNull() && m_trello_api_token_isValid;

    m_trello_list_id_isValid = ::OpenAPI::fromJsonValue(m_trello_list_id, json[QString("trelloListId")]);
    m_trello_list_id_isSet = !json[QString("trelloListId")].isNull() && m_trello_list_id_isValid;
}

QString OAITransportTrello_post::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAITransportTrello_post::asJsonObject() const {
    QJsonObject obj;
    if (m_data_segment_code_isSet) {
        obj.insert(QString("dataSegmentCode"), ::OpenAPI::toJsonValue(m_data_segment_code));
    }
    if (m_partition_isSet) {
        obj.insert(QString("partition"), ::OpenAPI::toJsonValue(m_partition));
    }
    if (m_transport_name_isSet) {
        obj.insert(QString("transportName"), ::OpenAPI::toJsonValue(m_transport_name));
    }
    if (m_trello_api_key_isSet) {
        obj.insert(QString("trelloApiKey"), ::OpenAPI::toJsonValue(m_trello_api_key));
    }
    if (m_trello_api_token_isSet) {
        obj.insert(QString("trelloApiToken"), ::OpenAPI::toJsonValue(m_trello_api_token));
    }
    if (m_trello_list_id_isSet) {
        obj.insert(QString("trelloListId"), ::OpenAPI::toJsonValue(m_trello_list_id));
    }
    return obj;
}

QString OAITransportTrello_post::getDataSegmentCode() const {
    return m_data_segment_code;
}
void OAITransportTrello_post::setDataSegmentCode(const QString &data_segment_code) {
    m_data_segment_code = data_segment_code;
    m_data_segment_code_isSet = true;
}

bool OAITransportTrello_post::is_data_segment_code_Set() const{
    return m_data_segment_code_isSet;
}

bool OAITransportTrello_post::is_data_segment_code_Valid() const{
    return m_data_segment_code_isValid;
}

QString OAITransportTrello_post::getPartition() const {
    return m_partition;
}
void OAITransportTrello_post::setPartition(const QString &partition) {
    m_partition = partition;
    m_partition_isSet = true;
}

bool OAITransportTrello_post::is_partition_Set() const{
    return m_partition_isSet;
}

bool OAITransportTrello_post::is_partition_Valid() const{
    return m_partition_isValid;
}

QString OAITransportTrello_post::getTransportName() const {
    return m_transport_name;
}
void OAITransportTrello_post::setTransportName(const QString &transport_name) {
    m_transport_name = transport_name;
    m_transport_name_isSet = true;
}

bool OAITransportTrello_post::is_transport_name_Set() const{
    return m_transport_name_isSet;
}

bool OAITransportTrello_post::is_transport_name_Valid() const{
    return m_transport_name_isValid;
}

QString OAITransportTrello_post::getTrelloApiKey() const {
    return m_trello_api_key;
}
void OAITransportTrello_post::setTrelloApiKey(const QString &trello_api_key) {
    m_trello_api_key = trello_api_key;
    m_trello_api_key_isSet = true;
}

bool OAITransportTrello_post::is_trello_api_key_Set() const{
    return m_trello_api_key_isSet;
}

bool OAITransportTrello_post::is_trello_api_key_Valid() const{
    return m_trello_api_key_isValid;
}

QString OAITransportTrello_post::getTrelloApiToken() const {
    return m_trello_api_token;
}
void OAITransportTrello_post::setTrelloApiToken(const QString &trello_api_token) {
    m_trello_api_token = trello_api_token;
    m_trello_api_token_isSet = true;
}

bool OAITransportTrello_post::is_trello_api_token_Set() const{
    return m_trello_api_token_isSet;
}

bool OAITransportTrello_post::is_trello_api_token_Valid() const{
    return m_trello_api_token_isValid;
}

QString OAITransportTrello_post::getTrelloListId() const {
    return m_trello_list_id;
}
void OAITransportTrello_post::setTrelloListId(const QString &trello_list_id) {
    m_trello_list_id = trello_list_id;
    m_trello_list_id_isSet = true;
}

bool OAITransportTrello_post::is_trello_list_id_Set() const{
    return m_trello_list_id_isSet;
}

bool OAITransportTrello_post::is_trello_list_id_Valid() const{
    return m_trello_list_id_isValid;
}

bool OAITransportTrello_post::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_data_segment_code_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_partition_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_transport_name_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_trello_api_key_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_trello_api_token_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_trello_list_id_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAITransportTrello_post::isValid() const {
    // only required properties are required for the object to be considered valid
    return m_partition_isValid && m_transport_name_isValid && m_trello_api_key_isValid && m_trello_api_token_isValid && m_trello_list_id_isValid && true;
}

} // namespace OpenAPI
