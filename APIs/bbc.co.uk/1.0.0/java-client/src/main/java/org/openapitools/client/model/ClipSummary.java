...
public class ClipSummary {
  ...
  public ClipSummary ancestors(List<AncestorSummary> ancestors) {
  ...
  public ClipSummary availableVersions(List<AvailableVersions> availableVersions) {
  ...
  public ClipSummary images(List<Image> images) {
    this.images = images;
    return this;
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to ClipSummary
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ClipSummary.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ClipSummary is not found in the empty JSON string", ClipSummary.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ClipSummary.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ClipSummary` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ClipSummary.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ClipSummary.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ClipSummary' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ClipSummary> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ClipSummary.class));

       return (TypeAdapter<T>) new TypeAdapter<ClipSummary>() {
           @Override
           public void write(JsonWriter out, ClipSummary value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ClipSummary read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of ClipSummary given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of ClipSummary
   * @throws IOException if the JSON string is invalid with respect to ClipSummary
   */
  public static ClipSummary fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ClipSummary.class);
  }

  /**
   * Convert an instance of ClipSummary to an JSON string
   *
   * @return JSON string
   */
