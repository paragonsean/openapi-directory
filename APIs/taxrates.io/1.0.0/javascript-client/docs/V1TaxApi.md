...
> TaxRatesByCountryCode200Response taxRatesByCountryCode(opts)
...
let apiInstance = new TaxratesIoApi.V1TaxApi();
let opts = {
...
apiInstance.taxRatesByCountryCode(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
> [TaxRatesByCountryCode200Response] taxRatesByIpAddress(opts)
...
let opts = {
  'domain': "api.taxrates.io", // String | Domain name: api.taxrates.io
  'ip': "86.139.70.49", // String | Customer's IP address
  'filter': "", // String | For US sales tax you can filter the tax type
  'zip': "", // String | For US sales tax a Zipcode must be proivded
  'productCode': "C010" // String | Your can filter your taxes by product code
};
apiInstance.taxRatesByIpAddress(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
