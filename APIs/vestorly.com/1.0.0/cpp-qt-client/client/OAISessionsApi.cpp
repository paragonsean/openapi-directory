...
void OAISessionsApi::login(const QString &username, const QString &password) {
    QString fullPath = QString(_serverConfigs["login"][_serverIndices.value("login")].URL()+"/sessions");
...
void OAISessionsApi::logout(const QString &vestorly_auth, const QString &id) {
    QString fullPath = QString(_serverConfigs["logout"][_serverIndices.value("logout")].URL()+"/sessions/{id}");
...
void OAISessionsApi::logoutCallback(OAIHttpRequestWorker *worker) {
    ...
    OAISessionLogoutResponse output(QString(worker->response));
    worker->deleteLater();
