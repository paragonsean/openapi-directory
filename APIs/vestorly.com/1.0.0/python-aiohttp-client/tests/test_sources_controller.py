...
async def test_create_source(client):
    ...
    """
    source = openapi_server.SourceInput()
...
async def test_find_sources(client):
    """Test case for find_sources
...
async def test_get_source_by_id(client):
    ...
    response = await client.request(
        method='GET',
        path='/api/v2/sources/{id}'.format(id='id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
@pytest.mark.skip("*/* not supported by Connexion. Use application/json instead. See https://github.com/zalando/connexion/pull/760")
async def test_update_source_by_id(client):
