...
void OAIDefaultApi::initializeServerConfigs() {
    ...
    QUrl("https://api.json2video.com/v2"),
    "Version 2 endpoint",
    QMap<QString, OAIServerVariable>()));
    _serverConfigs.insert("getMovies", defaultConf);
    _serverIndices.insert("getMovies", 0);
    _serverConfigs.insert("newMovie", defaultConf);
    _serverIndices.insert("newMovie", 0);
}
...
void OAIDefaultApi::getMovies() {
    QString fullPath = QString(_serverConfigs["getMovies"][_serverIndices.value("getMovies")].URL()+"/movies");
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIDefaultApi::getMoviesCallback);
...
void OAIDefaultApi::getMoviesCallback(OAIHttpRequestWorker *worker) {
    ...
    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT getMoviesSignal();
        Q_EMIT getMoviesSignalFull(worker);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT getMoviesSignalE(error_type, error_str);
        Q_EMIT getMoviesSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT getMoviesSignalError(error_type, error_str);
        Q_EMIT getMoviesSignalErrorFull(worker, error_type, error_str);
    }
}

void OAIDefaultApi::newMovie(const OAIMovie &oai_movie) {
    QString fullPath = QString(_serverConfigs["newMovie"][_serverIndices.value("newMovie")].URL()+"/movies");
...

    {
        ...
        QByteArray output = oai_movie.asJson().toUtf8();
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIDefaultApi::newMovieCallback);
    connect(this, &OAIDefaultApi::abortRequestsSignal, worker, &QObject::deleteLater);
    connect(worker, &QObject::destroyed, this, [this]() {
        if (findChildren<OAIHttpRequestWorker*>().count() == 0) {
            Q_EMIT allPendingRequestsCompleted();
...
void OAIDefaultApi::newMovieCallback(OAIHttpRequestWorker *worker) {
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
    ...
    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT newMovieSignal();
        Q_EMIT newMovieSignalFull(worker);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT newMovieSignalE(error_type, error_str);
        Q_EMIT newMovieSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT newMovieSignalError(error_type, error_str);
        Q_EMIT newMovieSignalErrorFull(worker, error_type, error_str);
    }
}
