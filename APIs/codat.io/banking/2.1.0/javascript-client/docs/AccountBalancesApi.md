...
> AccountBalances listAccountBalances(companyId, connectionId, page, opts)
...
let apiInstance = new BankingApi.AccountBalancesApi();
...
apiInstance.listAccountBalances(companyId, connectionId, page, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
