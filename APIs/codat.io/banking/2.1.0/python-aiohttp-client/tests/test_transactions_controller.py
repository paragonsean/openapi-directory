...
async def test_list_bank_transactions(client):
    ...
    response = await client.request(
        method='GET',
        path='/companies/{company_id}/data/banking-transactions',
        headers=headers,
        params=params,
        )
...
async def test_list_transactions(client):
    ...
    response = await client.request(
        method='GET',
        path='/companies/{company_id}/connections/{connection_id}/data/banking-transactions',
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
