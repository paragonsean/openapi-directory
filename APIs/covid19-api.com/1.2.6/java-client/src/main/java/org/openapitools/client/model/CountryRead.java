...
public class CountryRead {
   ...
   * @throws IOException if the JSON Element is invalid with respect to CountryRead
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CountryRead.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in CountryRead is not found in the empty JSON string", CountryRead.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CountryRead.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CountryRead` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!CountryRead.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'CountryRead' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<CountryRead> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(CountryRead.class));

       return (TypeAdapter<T>) new TypeAdapter<CountryRead>() {
           @Override
           public void write(JsonWriter out, CountryRead value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public CountryRead read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of CountryRead given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of CountryRead
   * @throws IOException if the JSON string is invalid with respect to CountryRead
   */
  public static CountryRead fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, CountryRead.class);
  }

  /**
   * Convert an instance of CountryRead to an JSON string
   *
   * @return JSON string
   */
