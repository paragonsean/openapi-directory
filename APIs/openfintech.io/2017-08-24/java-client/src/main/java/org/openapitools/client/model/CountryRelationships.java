...
import org.openapitools.client.model.CountryTranslations;
...
public class CountryRelationships {
  ...
  public CountryRelationships() {
  }

  public CountryRelationships translations(CountryTranslations translations) {
  ...
  public boolean equals(Object o) {
    ...
    CountryRelationships countryRelationships = (CountryRelationships) o;
    return Objects.equals(this.translations, countryRelationships.translations);
  }
  ...
  public String toString() {
    ...
    sb.append("class CountryRelationships {\n");
   ...
   * @throws IOException if the JSON Element is invalid with respect to CountryRelationships
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CountryRelationships.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in CountryRelationships is not found in the empty JSON string", CountryRelationships.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CountryRelationships.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CountryRelationships` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // validate the optional field `translations`
      if (jsonObj.get("translations") != null && !jsonObj.get("translations").isJsonNull()) {
        CountryTranslations.validateJsonElement(jsonObj.get("translations"));
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!CountryRelationships.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'CountryRelationships' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<CountryRelationships> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(CountryRelationships.class));

       return (TypeAdapter<T>) new TypeAdapter<CountryRelationships>() {
           @Override
           public void write(JsonWriter out, CountryRelationships value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public CountryRelationships read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of CountryRelationships given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of CountryRelationships
   * @throws IOException if the JSON string is invalid with respect to CountryRelationships
   */
  public static CountryRelationships fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, CountryRelationships.class);
  }

  /**
   * Convert an instance of CountryRelationships to an JSON string
   *
   * @return JSON string
   */
