...
void OAICountriesApi::initializeServerConfigs() {
    ...
    _serverConfigs.insert("countriesIdGet", defaultConf);
    _serverIndices.insert("countriesIdGet", 0);
}
...
void OAICountriesApi::countriesGet(const ::OpenAPI::OptionalParam<qint32> &page_number, const ::OpenAPI::OptionalParam<qint32> &page_size, const ::OpenAPI::OptionalParam<QSet<QString>> &filter_region, const ::OpenAPI::OptionalParam<QSet<QString>> &filter_sub_region, const ::OpenAPI::OptionalParam<QSet<QString>> &sort) {
    ...
    if (filter_region.hasValue())
    {
        ...
        queryPrefix = getParamStylePrefix(queryStyle);
        querySuffix = getParamStyleSuffix(queryStyle);
        queryDelimiter = getParamStyleDelimiter(queryStyle, "filter[region]", false);
        if (filter_region.value().size() > 0) {
            if (QString("csv").indexOf("multi") == 0) {
                for (QString t : filter_region.value()) {
                    if (fullPath.indexOf("?") > 0)
                        fullPath.append(queryPrefix);
                    else
                        fullPath.append("?");
                    fullPath.append("filter[region]=").append(::OpenAPI::toStringValue(t));
                }
            } else if (QString("csv").indexOf("ssv") == 0) {
                if (fullPath.indexOf("?") > 0)
                    fullPath.append("&");
                else
                    fullPath.append("?").append(queryPrefix).append("filter[region]").append(querySuffix);
                qint32 count = 0;
                for (QString t : filter_region.value()) {
                    if (count > 0) {
    ...
    if (filter_sub_region.hasValue())
    {
        ...
        queryPrefix = getParamStylePrefix(queryStyle);
        querySuffix = getParamStyleSuffix(queryStyle);
        queryDelimiter = getParamStyleDelimiter(queryStyle, "filter[sub_region]", false);
        if (filter_sub_region.value().size() > 0) {
            if (QString("csv").indexOf("multi") == 0) {
                for (QString t : filter_sub_region.value()) {
                    if (fullPath.indexOf("?") > 0)
                        fullPath.append(queryPrefix);
                    else
                        fullPath.append("?");
                    fullPath.append("filter[sub_region]=").append(::OpenAPI::toStringValue(t));
                }
            } else if (QString("csv").indexOf("ssv") == 0) {
                if (fullPath.indexOf("?") > 0)
                    fullPath.append("&");
                else
                    fullPath.append("?").append(queryPrefix).append("filter[sub_region]").append(querySuffix);
                qint32 count = 0;
                for (QString t : filter_sub_region.value()) {
                    if (count > 0) {
                        fullPath.append((false)? queryDelimiter : QUrl::toPercentEncoding(queryDelimiter));
...
void OAICountriesApi::countriesIdGet(const QString &id) {
    QString fullPath = QString(_serverConfigs["countriesIdGet"][_serverIndices.value("countriesIdGet")].URL()+"/countries/{id}");
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAICountriesApi::countriesIdGetCallback);
    connect(this, &OAICountriesApi::abortRequestsSignal, worker, &QObject::deleteLater);
    connect(worker, &QObject::destroyed, this, [this]() {
        if (findChildren<OAIHttpRequestWorker*>().count() == 0) {
            Q_EMIT allPendingRequestsCompleted();
...
void OAICountriesApi::countriesIdGetCallback(OAIHttpRequestWorker *worker) {
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
    ...
    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT countriesIdGetSignal(output);
        Q_EMIT countriesIdGetSignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT countriesIdGetSignalE(output, error_type, error_str);
        Q_EMIT countriesIdGetSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT countriesIdGetSignalError(output, error_type, error_str);
        Q_EMIT countriesIdGetSignalErrorFull(worker, error_type, error_str);
    }
}
