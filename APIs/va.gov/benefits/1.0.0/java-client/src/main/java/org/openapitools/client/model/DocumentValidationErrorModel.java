...
public class DocumentValidationErrorModel {
  ...
  public DocumentValidationErrorModel() {
  }

  public DocumentValidationErrorModel detail(String detail) {
    this.detail = detail;
    return this;
  }
  ...
  public DocumentValidationErrorModel status(String status) {
    this.status = status;
    return this;
  }
  ...
  public DocumentValidationErrorModel title(String title) {
    this.title = title;
    return this;
  }
  ...
  public boolean equals(Object o) {
    ...
    DocumentValidationErrorModel documentValidationErrorModel = (DocumentValidationErrorModel) o;
    return Objects.equals(this.detail, documentValidationErrorModel.detail) &&
        Objects.equals(this.status, documentValidationErrorModel.status) &&
        Objects.equals(this.title, documentValidationErrorModel.title);
  }
  ...
  public String toString() {
    ...
    sb.append("class DocumentValidationErrorModel {\n");
   ...
   * @throws IOException if the JSON Element is invalid with respect to DocumentValidationErrorModel
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DocumentValidationErrorModel.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in DocumentValidationErrorModel is not found in the empty JSON string", DocumentValidationErrorModel.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DocumentValidationErrorModel.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DocumentValidationErrorModel` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : DocumentValidationErrorModel.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!DocumentValidationErrorModel.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'DocumentValidationErrorModel' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<DocumentValidationErrorModel> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(DocumentValidationErrorModel.class));

       return (TypeAdapter<T>) new TypeAdapter<DocumentValidationErrorModel>() {
           @Override
           public void write(JsonWriter out, DocumentValidationErrorModel value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public DocumentValidationErrorModel read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of DocumentValidationErrorModel given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of DocumentValidationErrorModel
   * @throws IOException if the JSON string is invalid with respect to DocumentValidationErrorModel
   */
  public static DocumentValidationErrorModel fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, DocumentValidationErrorModel.class);
  }

  /**
   * Convert an instance of DocumentValidationErrorModel to an JSON string
   *
   * @return JSON string
   */
