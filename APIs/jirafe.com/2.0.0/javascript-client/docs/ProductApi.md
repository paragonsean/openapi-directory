...
let apiInstance = new JirafeEvents.ProductApi();
let siteId = "siteId_example"; // String | ID site to send the event
let body = new JirafeEvents.Product(); // Product | product json for the event
apiInstance.postProduct(siteId, body, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
