# JirafeEvents.CategoryApi

All URIs are relative to *https://event.jirafe.com/v2*

Method | HTTP request | Description
------------- | ------------- | -------------
[**postCategory**](CategoryApi.md#postCategory) | **POST** /{siteId}/category | Send a category for the given site



## postCategory

> postCategory(siteId, body)
...
let apiInstance = new JirafeEvents.CategoryApi();
let siteId = "siteId_example"; // String | ID site to send the event
let body = new JirafeEvents.Category(); // Category | category json for the event
apiInstance.postCategory(siteId, body, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
 ...
 **body** | [**Category**](Category.md)| category json for the event | 
