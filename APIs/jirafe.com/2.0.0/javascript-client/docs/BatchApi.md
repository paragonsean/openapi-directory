# JirafeEvents.BatchApi

All URIs are relative to *https://event.jirafe.com/v2*

Method | HTTP request | Description
------------- | ------------- | -------------
[**postBatch**](BatchApi.md#postBatch) | **POST** /{siteId}/batch | Send a batch for the given site



## postBatch

> postBatch(siteId, body)
...
let apiInstance = new JirafeEvents.BatchApi();
let siteId = "siteId_example"; // String | ID site to send the event
let body = new JirafeEvents.Batch(); // Batch | batch json of events
apiInstance.postBatch(siteId, body, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
 ...
 **body** | [**Batch**](Batch.md)| batch json of events | 
