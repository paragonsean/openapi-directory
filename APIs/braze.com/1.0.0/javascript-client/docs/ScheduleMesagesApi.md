# BrazeEndpoints.ScheduleMesagesApi
...
> getUpcomingScheduledCampaignsAndCanvases_0(opts)
...
let apiInstance = new BrazeEndpoints.ScheduleMesagesApi();
let opts = {
...
apiInstance.getUpcomingScheduledCampaignsAndCanvases_0(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
> scheduleApiTriggeredCanvases_0(opts)
...
apiInstance.scheduleApiTriggeredCanvases_0(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully.');
