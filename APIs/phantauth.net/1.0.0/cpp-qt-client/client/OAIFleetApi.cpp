...
void OAIFleetApi::initializeServerConfigs() {
    ...
    _serverConfigs.insert("fleetFleetnameGet", defaultConf);
    _serverIndices.insert("fleetFleetnameGet", 0);
}
...
void OAIFleetApi::fleetFleetnameGet(const QString &fleetname) {
    QString fullPath = QString(_serverConfigs["fleetFleetnameGet"][_serverIndices.value("fleetFleetnameGet")].URL()+"/fleet/{fleetname}");
    
    
    {
        QString fleetnamePathParam("{");
        fleetnamePathParam.append("fleetname").append("}");
        ...
        pathDelimiter = getParamStyleDelimiter(pathStyle, "fleetname", false);
        QString paramString = (pathStyle == "matrix") ? pathPrefix+"fleetname"+pathSuffix : pathPrefix;
        fullPath.replace(fleetnamePathParam, paramString+QUrl::toPercentEncoding(::OpenAPI::toStringValue(fleetname)));
    }
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAIFleetApi::fleetFleetnameGetCallback);
...
void OAIFleetApi::fleetFleetnameGetCallback(OAIHttpRequestWorker *worker) {
    ...
    OAI_fleet__fleetname__get_200_response output(QString(worker->response));
    worker->deleteLater();

    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT fleetFleetnameGetSignal(output);
        Q_EMIT fleetFleetnameGetSignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT fleetFleetnameGetSignalE(output, error_type, error_str);
        Q_EMIT fleetFleetnameGetSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT fleetFleetnameGetSignalError(output, error_type, error_str);
        Q_EMIT fleetFleetnameGetSignalErrorFull(worker, error_type, error_str);
    }
}
