...
void OAITeamApi::initializeServerConfigs() {
    ...
    QUrl("https://phantauth.net"),
    "No description provided",
    QMap<QString, OAIServerVariable>()));
    _serverConfigs.insert("teamTeamnameGet", defaultConf);
    _serverIndices.insert("teamTeamnameGet", 0);
}
...
void OAITeamApi::teamTeamnameGet(const QString &teamname) {
    QString fullPath = QString(_serverConfigs["teamTeamnameGet"][_serverIndices.value("teamTeamnameGet")].URL()+"/team/{teamname}");
    
    
    {
        QString teamnamePathParam("{");
        teamnamePathParam.append("teamname").append("}");
        ...
        fullPath.replace(teamnamePathParam, paramString+QUrl::toPercentEncoding(::OpenAPI::toStringValue(teamname)));
    }
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAITeamApi::teamTeamnameGetCallback);
...
void OAITeamApi::teamTeamnameGetCallback(OAIHttpRequestWorker *worker) {
    ...
    OAI_team__teamname__get_200_response output(QString(worker->response));
    worker->deleteLater();

    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT teamTeamnameGetSignal(output);
        Q_EMIT teamTeamnameGetSignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT teamTeamnameGetSignalE(output, error_type, error_str);
        Q_EMIT teamTeamnameGetSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT teamTeamnameGetSignalError(output, error_type, error_str);
        Q_EMIT teamTeamnameGetSignalErrorFull(worker, error_type, error_str);
    }
}
