# LgtmApiSpecification.AnalysesApi
...
> Object getAlerts(analysisId, opts)
...
let apiInstance = new LgtmApiSpecification.AnalysesApi();
let analysisId = "analysisId_example"; // String | The analysis identifier.
let opts = {
  'sarifVersion': "sarifVersion_example", // String | The desired version of the SARIF format. Currently supported versions are `1.0.0`, `2.0.0`, and `2.1.0`.
  'excludedFiles': false // Boolean | Set `true` to include results in files that are excluded from the output by default. This includes results in test code and generated files. For more information, see [File classification](https://lgtm.com/help/lgtm/file-classification).
};
apiInstance.getAlerts(analysisId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
let apiInstance = new LgtmApiSpecification.AnalysesApi();
let analysisId = "analysisId_example"; // String | The analysis identifier.
apiInstance.getAnalysis(analysisId, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
...
let commitId = "commitId_example"; // String | The identifier of a specific commit. Alternatively, use `latest` for the most recent analyzed commit.
apiInstance.getAnalysisForCommit(projectId, commitId, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
...
> Operation requestAnalysis(projectId, commit, opts)
...
let apiInstance = new LgtmApiSpecification.AnalysesApi();
let projectId = 789; // Number | The numeric project identifier.
let commit = "commit_example"; // String | The identifier of the commit to analyze.
let opts = {
  'language': ["null"] // [String] | The language codes of the languages to analyze. For a list of available languages, see [Supported languages](https://lgtm.com/help/lgtm/analysis-faqs#which-languages-are-supported). To specify more than one language, this parameter can be repeated. If no language is specified, all the project's languages will be analyzed. 
};
apiInstance.requestAnalysis(projectId, commit, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
