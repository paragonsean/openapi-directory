# LgtmApiSpecification.CodeReviewsApi
...
> [CodeReview] getCodeReview(reviewId)
...
let apiInstance = new LgtmApiSpecification.CodeReviewsApi();
let reviewId = "reviewId_example"; // String | The identifier for the review (from the `task-result-url`).
apiInstance.getCodeReview(reviewId, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
> Operation requestReview(projectId, base, externalId, body, opts)
 ...
 **externalId** | **Number**| Your reference number for the code review. | 
