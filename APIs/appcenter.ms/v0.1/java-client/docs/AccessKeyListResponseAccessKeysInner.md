

# AccessKeyListResponseAccessKeysInner


## Properties

| Name | Type | Description | Notes |
|------------ | ------------- | ------------- | -------------|
|**createdBy** | **String** | Account name of creator. |  [optional] |
|**createdTime** | **BigDecimal** | Created time of access key |  [optional] |
|**description** | **String** | Description of access key |  [optional] |
|**expires** | **BigDecimal** | Time of expiry of access key |  [optional] |
|**friendlyName** | **String** | Friendly name of access key |  [optional] |
|**id** | **String** | Id of accessKey |  [optional] |
|**isSession** | **Boolean** | Legacy property which indicate if accessKey was created from session |  [optional] |
|**name** | **String** | Key of access key |  [optional] |



