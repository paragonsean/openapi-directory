

# Subscription1

Subscription information

## Properties

| Name | Type | Description | Notes |
|------------ | ------------- | ------------- | -------------|
|**active** | **Boolean** | Is the subscription currently active? |  [optional] |
|**concurrentDevicesLimit** | **Integer** | Customer limits on concurrent devices |  [optional] |
|**daysLeft** | **BigDecimal** | The number of days left in the subscription |  [optional] |
|**endsAt** | **String** | The date the subscription will end or ended |  [optional] |
|**id** | **UUID** | Id of the subscription |  [optional] |
|**runningDevices** | **Integer** | Current device concurrency utilization |  [optional] |
|**startsAt** | **String** | The date the subscription began |  [optional] |
|**tier** | [**SubscriptionTier**](SubscriptionTier.md) |  |  [optional] |



