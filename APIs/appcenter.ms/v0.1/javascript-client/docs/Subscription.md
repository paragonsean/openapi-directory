# AppCenterClient.Subscription

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**active** | **Boolean** | Is the subscription currently active? | [optional] 
**concurrentDevicesLimit** | **Number** | Customer limits on concurrent devices | [optional] 
**daysLeft** | **Number** | The number of days left in the subscription | [optional] 
**endsAt** | **String** | The date the subscription will end or ended | [optional] 
**id** | **String** | Id of the subscription | [optional] 
**runningDevices** | **Number** | Current device concurrency utilization | [optional] 
**startsAt** | **String** | The date the subscription began | [optional] 
**tier** | [**SubscriptionTier**](SubscriptionTier.md) |  | [optional] 


