/**
 * App Center Client
 * Microsoft Visual Studio App Center API
 *
 * The version of the OpenAPI document: v0.1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import AnalyticsDeviceCounts200ResponseDailyInner from './AnalyticsDeviceCounts200ResponseDailyInner';

/**
 * The EventCount model module.
 * @module model/EventCount
 * @version v0.1
 */
class EventCount {
    /**
     * Constructs a new <code>EventCount</code>.
     * @alias module:model/EventCount
     */
    constructor() { 
        
        EventCount.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>EventCount</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/EventCount} obj Optional instance to populate.
     * @return {module:model/EventCount} The populated <code>EventCount</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new EventCount();

            if (data.hasOwnProperty('count')) {
                obj['count'] = ApiClient.convertToType(data['count'], [AnalyticsDeviceCounts200ResponseDailyInner]);
            }
            if (data.hasOwnProperty('previous_total_count')) {
                obj['previous_total_count'] = ApiClient.convertToType(data['previous_total_count'], 'Number');
            }
            if (data.hasOwnProperty('total_count')) {
                obj['total_count'] = ApiClient.convertToType(data['total_count'], 'Number');
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>EventCount</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>EventCount</code>.
     */
    static validateJSON(data) {
        if (data['count']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['count'])) {
                throw new Error("Expected the field `count` to be an array in the JSON data but got " + data['count']);
            }
            // validate the optional field `count` (array)
            for (const item of data['count']) {
                AnalyticsDeviceCounts200ResponseDailyInner.validateJSON(item);
            };
        }

        return true;
    }


}



/**
 * @member {Array.<module:model/AnalyticsDeviceCounts200ResponseDailyInner>} count
 */
EventCount.prototype['count'] = undefined;

/**
 * @member {Number} previous_total_count
 */
EventCount.prototype['previous_total_count'] = undefined;

/**
 * @member {Number} total_count
 */
EventCount.prototype['total_count'] = undefined;






export default EventCount;

