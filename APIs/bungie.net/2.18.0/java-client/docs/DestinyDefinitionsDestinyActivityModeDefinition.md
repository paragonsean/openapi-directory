...
|------------ | ------------- | ------------- | -------------|
|**activityModeCategory** | **Integer** | The type of play being performed in broad terms (PVP, PVE) |  [optional] |
|**activityModeMappings** | **Map&lt;String, Integer&gt;** | If this exists, the mode has specific Activities (referred to by the Key) that should instead map to other Activity Modes when they are played. This was useful in D1 for Private Matches, where we wanted to have Private Matches as an activity mode while still referring to the specific mode being played. |  [optional] |
...
|**modeType** | **Integer** | The Enumeration value for this Activity Mode. Pass this identifier into Stats endpoints to get aggregate stats for this mode. |  [optional] |
|**order** | **Integer** | The relative ordering of activity modes. |  [optional] |
|**parentHashes** | **List&lt;Integer&gt;** | The hash identifiers of the DestinyActivityModeDefinitions that represent all of the \&quot;parent\&quot; modes for this mode. For instance, the Nightfall Mode is also a member of AllStrikes and AllPvE. |  [optional] |
