...
import org.openapitools.client.model.ExposedActionSchema;
...
public class ExposedActionResponseSchema {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ExposedActionResponseSchema
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ExposedActionResponseSchema.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ExposedActionResponseSchema.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ExposedActionResponseSchema` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayresults.size(); i++) {
        ExposedActionSchema.validateJsonElement(jsonArrayresults.get(i));
      };
  }
