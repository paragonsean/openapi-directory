...
public class MediaType {
   ...
   * @throws IOException if the JSON Element is invalid with respect to MediaType
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!MediaType.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in MediaType is not found in the empty JSON string", MediaType.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!MediaType.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `MediaType` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!MediaType.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'MediaType' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<MediaType> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(MediaType.class));

       return (TypeAdapter<T>) new TypeAdapter<MediaType>() {
           @Override
  ...
  /**
   * Create an instance of MediaType given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of MediaType
   * @throws IOException if the JSON string is invalid with respect to MediaType
   */
  public static MediaType fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, MediaType.class);
  }

  /**
   * Convert an instance of MediaType to an JSON string
   *
   * @return JSON string
   */
