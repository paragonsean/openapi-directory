...
> GetJobsResponse jobsAll(xApideckConsumerId, xApideckAppId, opts)
...
```javascript
import AtsApi from 'ats_api';
let defaultClient = AtsApi.ApiClient.instance;
...
let apiInstance = new AtsApi.JobsApi();
...
apiInstance.jobsAll(xApideckConsumerId, xApideckAppId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
> GetJobResponse jobsOne(id, xApideckConsumerId, xApideckAppId, opts)
...
apiInstance.jobsOne(id, xApideckConsumerId, xApideckAppId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
