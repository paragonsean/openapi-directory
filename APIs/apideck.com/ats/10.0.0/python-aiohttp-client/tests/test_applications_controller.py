...
async def test_applications_add(client):
    """Test case for applications_add

    Create Application
    """
    body = {"applicant_id":"12345","stage":{"name":"12345","id":"12345"},"updated_at":"2020-09-30T07:43:32Z","job_id":"12345","updated_by":"12345","created_at":"2020-09-30T07:43:32Z","id":"12345","created_by":"12345","custom_mappings":"{}","status":"open"}
    params = [('raw', False)]
    headers = { 
...
async def test_applications_all(client):
...
async def test_applications_delete(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/ats/applications/{id}'.format(id='id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_applications_one(client):
