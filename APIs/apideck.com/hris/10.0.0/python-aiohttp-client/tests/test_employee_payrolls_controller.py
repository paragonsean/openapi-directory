...
async def test_employee_payrolls_all(client):
    ...
    response = await client.request(
        method='GET',
        path='/hris/payrolls/employees/{employee_id}'.format(employee_id='employee_id_example'),
        headers=headers,
        params=params,
        )
...
async def test_employee_payrolls_one(client):
    ...
    response = await client.request(
        method='GET',
        path='/hris/payrolls/employees/{employee_id}/payrolls/{payroll_id}'.format(payroll_id='payroll_id_example', employee_id='employee_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
