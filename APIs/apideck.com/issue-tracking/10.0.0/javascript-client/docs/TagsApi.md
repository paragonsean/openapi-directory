...
> GetCollectionTagsResponse collectionTagsAll(collectionId, xApideckConsumerId, xApideckAppId, opts)
...
let apiInstance = new IssueTrackingApi.TagsApi();
...
apiInstance.collectionTagsAll(collectionId, xApideckConsumerId, xApideckAppId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
