...
async def test_upload_sessions_add(client):
    """Test case for upload_sessions_add

    Start Upload Session
    """
    body = {"size":1810673,"drive_id":"1234","parent_folder_id":"1234","name":"Documents"}
    params = [('raw', False)]
    headers = { 
...
async def test_upload_sessions_delete(client):
    ...
    response = await client.request(
        method='DELETE',
        path='/file-storage/upload-sessions/{id}'.format(id='id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_upload_sessions_finish(client):
    ...
    response = await client.request(
        method='POST',
        path='/file-storage/upload-sessions/{id}/finish'.format(id='id_example'),
        headers=headers,
        json=body,
        params=params,
        )
...
async def test_upload_sessions_one(client):
...
async def test_upload_sessions_upload(client):
    """Test case for upload_sessions_upload

    Upload part of File to Upload Session
    """
    body = '/path/to/file'
    params = [('part_number', 0),
                    ('raw', False)]
    headers = { 
        'Accept': 'application/json',
        'Content-Type': 'application/json',
