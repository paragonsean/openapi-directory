...
> GetCompanyInfoResponse companyInfoOne(xApideckConsumerId, xApideckAppId, opts)
...
let apiInstance = new AccountingApi.CompanyInfoApi();
...
apiInstance.companyInfoOne(xApideckConsumerId, xApideckAppId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
