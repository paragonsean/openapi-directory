...
public class PostProxyRequest extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(PostProxyRequest.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!PostProxyRequest.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'PostProxyRequest' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<PostProxyRequest>() {
                @Override
                public void write(JsonWriter out, PostProxyRequest value) throws IOException {
                ...
                @Override
                public PostProxyRequest read(JsonReader in) throws IOException {
                    ...
                    try {
                        ...
                        PostProxyRequest ret = new PostProxyRequest();
                    ...
                    throw new IOException(String.format("Failed deserialization for PostProxyRequest: no class matches result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public PostProxyRequest() {
        super("anyOf", Boolean.FALSE);
    }

    public PostProxyRequest(Object o) {
        super("anyOf", Boolean.FALSE);
        setActualInstance(o);
    }
    ...
    public Map<String, Class<?>> getSchemas() {
        return PostProxyRequest.schemas;
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to PostProxyRequest
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        throw new IOException(String.format("The JSON string is invalid for PostProxyRequest with anyOf schemas: Object, String. no class match the result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
    }

    /**
     * Create an instance of PostProxyRequest given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of PostProxyRequest
     * @throws IOException if the JSON string is invalid with respect to PostProxyRequest
     */
    public static PostProxyRequest fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, PostProxyRequest.class);
    }

    /**
     * Convert an instance of PostProxyRequest to an JSON string
     *
     * @return JSON string
     */
