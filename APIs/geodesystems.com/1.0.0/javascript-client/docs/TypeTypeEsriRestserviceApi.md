# GeodesystemsCom443.TypeTypeEsriRestserviceApi
...
> searchTypeEsriRestservice(opts)
...
let apiInstance = new GeodesystemsCom443.TypeTypeEsriRestserviceApi();
let opts = {
...
apiInstance.searchTypeEsriRestservice(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
