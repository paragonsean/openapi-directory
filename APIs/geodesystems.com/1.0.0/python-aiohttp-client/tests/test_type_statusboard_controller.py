...
async def test_search_statusboard(client):
    """Test case for search_statusboard
    ...
    params = [('text', 'text_example'),
                    ...
                    ('max', 56),
                    ('skip', 56),
                    ('search.db_statusboard.what', 'search_db_statusboard_what_example'),
                    ('search.db_statusboard.status', 'search_db_statusboard_status_example')]
    headers = { 
    }
    response = await client.request(
        method='GET',
        path='/repository/search/type/statusboard',
        headers=headers,
        params=params,
        )
