...
async def test_search_bio_dicom_test(client):
    """Test case for search_bio_dicom_test
    ...
    params = [('text', 'text_example'),
                    ...
                    ('max', 56),
                    ('skip', 56),
                    ('search.bio_dicom_test.PatientName', 'search_bio_dicom_test_patient_name_example'),
                    ('search.bio_dicom_test.PatientID', 'search_bio_dicom_test_patient_id_example')]
    headers = { 
    }
    response = await client.request(
        method='GET',
        path='/repository/search/type/bio_dicom_test',
        headers=headers,
        params=params,
        )
