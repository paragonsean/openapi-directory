...
async def test_search_incident(client):
    ...
    params = [('text', 'text_example'),
                    ...
                    ('max', 56),
                    ('skip', 56),
                    ('search.incident.incidenttype', 'search_incident_incidenttype_example'),
                    ('search.incident.cause', 'search_incident_cause_example'),
                    ('search.incident.state', 'search_incident_state_example')]
    headers = { 
    }
    response = await client.request(
        method='GET',
        path='/repository/search/type/incident',
        headers=headers,
        params=params,
        )
