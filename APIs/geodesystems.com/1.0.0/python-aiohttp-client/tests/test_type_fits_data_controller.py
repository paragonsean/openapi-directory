...
async def test_search_fits_data(client):
    """Test case for search_fits_data
    ...
    params = [('text', 'text_example'),
                    ...
                    ('max', 56),
                    ('skip', 56),
                    ('search.fits_data.origin', 'search_fits_data_origin_example'),
                    ('search.fits_data.telescope', 'search_fits_data_telescope_example'),
                    ('search.fits_data.instrument', 'search_fits_data_instrument_example')]
    headers = { 
    }
    response = await client.request(
        method='GET',
        path='/repository/search/type/fits_data',
        headers=headers,
        params=params,
        )
