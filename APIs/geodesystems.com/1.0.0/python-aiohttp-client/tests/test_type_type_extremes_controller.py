...
async def test_search_type_extremes(client):
    """Test case for search_type_extremes
    ...
    params = [('text', 'text_example'),
                    ...
                    ('max', 56),
                    ('skip', 56),
                    ('search.type_extremes.region', 'search_type_extremes_region_example'),
                    ('search.type_extremes.variable', 'search_type_extremes_variable_example')]
    headers = { 
    }
    response = await client.request(
        method='GET',
        path='/repository/search/type/type_extremes',
        headers=headers,
        params=params,
        )
