...
async def test_search_type_hydro_colorado(client):
    """Test case for search_type_hydro_colorado
    ...
    params = [('text', 'text_example'),
                    ...
                    ('max', 56),
                    ('skip', 56),
                    ('search.type_hydro_colorado.site_id', 'search_type_hydro_colorado_site_id_example')]
    headers = { 
    }
    response = await client.request(
        method='GET',
        path='/repository/search/type/type_hydro_colorado',
        headers=headers,
        params=params,
        )
