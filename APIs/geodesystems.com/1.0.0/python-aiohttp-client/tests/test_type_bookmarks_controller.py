...
async def test_search_bookmarks(client):
    ...
    params = [('text', 'text_example'),
                    ...
                    ('max', 56),
                    ('skip', 56),
                    ('search.db_bookmarks.title', 'search_db_bookmarks_title_example'),
                    ('search.db_bookmarks.url', 'search_db_bookmarks_url_example'),
                    ('search.db_bookmarks.category', 'search_db_bookmarks_category_example'),
                    ('search.db_bookmarks.date', 'search_db_bookmarks_date_example')]
    headers = { 
    }
    response = await client.request(
        method='GET',
        path='/repository/search/type/bookmarks',
        headers=headers,
        params=params,
        )
