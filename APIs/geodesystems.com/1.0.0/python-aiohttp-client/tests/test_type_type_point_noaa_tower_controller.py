...
async def test_search_type_point_noaa_tower(client):
    """Test case for search_type_point_noaa_tower
    ...
    params = [('text', 'text_example'),
                    ...
                    ('max', 56),
                    ('skip', 56),
                    ('search.type_point_noaa_tower.site_id', 'search_type_point_noaa_tower_site_id_example')]
    headers = { 
    }
    response = await client.request(
        method='GET',
        path='/repository/search/type/type_point_noaa_tower',
        headers=headers,
        params=params,
        )
