...
async def test_search_wikipage(client):
    """Test case for search_wikipage
    ...
    params = [('text', 'text_example'),
                    ...
                    ('max', 56),
                    ('skip', 56),
                    ('search.wikipage.wikitext', 'search_wikipage_wikitext_example'),
                    ('search.wikipage.category', 'search_wikipage_category_example')]
    headers = { 
    }
    response = await client.request(
        method='GET',
        path='/repository/search/type/wikipage',
        headers=headers,
        params=params,
        )
