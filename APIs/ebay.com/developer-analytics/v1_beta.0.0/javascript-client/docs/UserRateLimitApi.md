...
> RateLimitsResponse getUserRateLimits(opts)
...
let apiInstance = new AnalyticsApi.UserRateLimitApi();
let opts = {
...
apiInstance.getUserRateLimits(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
