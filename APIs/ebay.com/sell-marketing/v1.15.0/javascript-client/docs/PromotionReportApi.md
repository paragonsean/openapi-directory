# MarketingApi.PromotionReportApi
...
> PromotionsReportPagedCollection getPromotionReports(marketplaceId, opts)
...
let apiInstance = new MarketingApi.PromotionReportApi();
...
apiInstance.getPromotionReports(marketplaceId, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
