...
public class LegalAddress {
   ...
   * @throws IOException if the JSON Element is invalid with respect to LegalAddress
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!LegalAddress.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in LegalAddress is not found in the empty JSON string", LegalAddress.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!LegalAddress.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `LegalAddress` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!LegalAddress.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'LegalAddress' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<LegalAddress> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(LegalAddress.class));

       return (TypeAdapter<T>) new TypeAdapter<LegalAddress>() {
           @Override
           public void write(JsonWriter out, LegalAddress value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public LegalAddress read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of LegalAddress given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of LegalAddress
   * @throws IOException if the JSON string is invalid with respect to LegalAddress
   */
  public static LegalAddress fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, LegalAddress.class);
  }

  /**
   * Convert an instance of LegalAddress to an JSON string
   *
   * @return JSON string
   */
