...
public class RecipientAccountReference {
   ...
   * @throws IOException if the JSON Element is invalid with respect to RecipientAccountReference
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!RecipientAccountReference.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in RecipientAccountReference is not found in the empty JSON string", RecipientAccountReference.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!RecipientAccountReference.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `RecipientAccountReference` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!RecipientAccountReference.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'RecipientAccountReference' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<RecipientAccountReference> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(RecipientAccountReference.class));

       return (TypeAdapter<T>) new TypeAdapter<RecipientAccountReference>() {
           @Override
           public void write(JsonWriter out, RecipientAccountReference value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public RecipientAccountReference read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of RecipientAccountReference given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of RecipientAccountReference
   * @throws IOException if the JSON string is invalid with respect to RecipientAccountReference
   */
  public static RecipientAccountReference fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, RecipientAccountReference.class);
  }

  /**
   * Convert an instance of RecipientAccountReference to an JSON string
   *
   * @return JSON string
   */
