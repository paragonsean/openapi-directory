...
public class FulfillmentPolicy {
   ...
   * @throws IOException if the JSON Element is invalid with respect to FulfillmentPolicy
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!FulfillmentPolicy.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in FulfillmentPolicy is not found in the empty JSON string", FulfillmentPolicy.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!FulfillmentPolicy.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `FulfillmentPolicy` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!FulfillmentPolicy.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'FulfillmentPolicy' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<FulfillmentPolicy> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(FulfillmentPolicy.class));

       return (TypeAdapter<T>) new TypeAdapter<FulfillmentPolicy>() {
           @Override
           public void write(JsonWriter out, FulfillmentPolicy value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public FulfillmentPolicy read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of FulfillmentPolicy given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of FulfillmentPolicy
   * @throws IOException if the JSON string is invalid with respect to FulfillmentPolicy
   */
  public static FulfillmentPolicy fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, FulfillmentPolicy.class);
  }

  /**
   * Convert an instance of FulfillmentPolicy to an JSON string
   *
   * @return JSON string
   */
