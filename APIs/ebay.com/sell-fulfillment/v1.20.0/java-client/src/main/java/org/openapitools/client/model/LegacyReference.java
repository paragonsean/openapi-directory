...
public class LegacyReference {
  ...
  static {
    ...
    openapiFields.add("legacyTransactionId");
   ...
   * @throws IOException if the JSON Element is invalid with respect to LegacyReference
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!LegacyReference.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in LegacyReference is not found in the empty JSON string", LegacyReference.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!LegacyReference.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `LegacyReference` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("legacyTransactionId") != null && !jsonObj.get("legacyTransactionId").isJsonNull()) && !jsonObj.get("legacyTransactionId").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `legacyTransactionId` to be a primitive type in the JSON string but got `%s`", jsonObj.get("legacyTransactionId").toString()));
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!LegacyReference.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'LegacyReference' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<LegacyReference> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(LegacyReference.class));

       return (TypeAdapter<T>) new TypeAdapter<LegacyReference>() {
           @Override
           public void write(JsonWriter out, LegacyReference value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public LegacyReference read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of LegacyReference given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of LegacyReference
   * @throws IOException if the JSON string is invalid with respect to LegacyReference
   */
  public static LegacyReference fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, LegacyReference.class);
  }

  /**
   * Convert an instance of LegacyReference to an JSON string
   *
   * @return JSON string
   */
