...
public class InventoryTaskCollection {
   ...
   * @throws IOException if the JSON Element is invalid with respect to InventoryTaskCollection
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!InventoryTaskCollection.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in InventoryTaskCollection is not found in the empty JSON string", InventoryTaskCollection.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!InventoryTaskCollection.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `InventoryTaskCollection` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("tasks") != null && !jsonObj.get("tasks").isJsonNull()) {
        ...
        if (jsonArraytasks != null) {
          ...
          for (int i = 0; i < jsonArraytasks.size(); i++) {
            InventoryTask.validateJsonElement(jsonArraytasks.get(i));
          };
        }
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!InventoryTaskCollection.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'InventoryTaskCollection' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<InventoryTaskCollection> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(InventoryTaskCollection.class));

       return (TypeAdapter<T>) new TypeAdapter<InventoryTaskCollection>() {
           @Override
           public void write(JsonWriter out, InventoryTaskCollection value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public InventoryTaskCollection read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of InventoryTaskCollection given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of InventoryTaskCollection
   * @throws IOException if the JSON string is invalid with respect to InventoryTaskCollection
   */
  public static InventoryTaskCollection fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, InventoryTaskCollection.class);
  }

  /**
   * Convert an instance of InventoryTaskCollection to an JSON string
   *
   * @return JSON string
   */
