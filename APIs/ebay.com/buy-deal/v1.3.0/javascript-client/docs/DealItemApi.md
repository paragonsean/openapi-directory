...
> DealItemSearchResponse getDealItems(X_EBAY_C_MARKETPLACE_ID, opts)
...
let apiInstance = new DealApi.DealItemApi();
...
apiInstance.getDealItems(X_EBAY_C_MARKETPLACE_ID, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
 ...
 **categoryIds** | **String**| The unique identifier of the eBay category for the search. | [optional] 
