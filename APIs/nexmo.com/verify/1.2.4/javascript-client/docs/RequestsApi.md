...
> VerifyRequestWithPSD2200Response verifyRequest(format, apiKey, apiSecret, brand, number, opts)
...
let apiInstance = new VerifyApi.RequestsApi();
...
let brand = "brand_example"; // String | An 18-character alphanumeric string you can use to personalize the verification request SMS body, to help users identify your company or application name. For example: \\\"Your `Acme Inc` PIN is ...\\\"
...
apiInstance.verifyRequest(format, apiKey, apiSecret, brand, number, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
