...
import org.openapitools.client.model.TopEditorsByAbsBytesDiffItemsInnerResultsInnerTopInner;
...
public class TopEditorsByAbsBytesDiffItemsInnerResultsInner {
  ...
  private List<TopEditorsByAbsBytesDiffItemsInnerResultsInnerTopInner> top = new ArrayList<>();
   ...
   * @throws IOException if the JSON Element is invalid with respect to TopEditorsByAbsBytesDiffItemsInnerResultsInner
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!TopEditorsByAbsBytesDiffItemsInnerResultsInner.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!TopEditorsByAbsBytesDiffItemsInnerResultsInner.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TopEditorsByAbsBytesDiffItemsInnerResultsInner` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("top") != null && !jsonObj.get("top").isJsonNull()) {
        ...
        if (jsonArraytop != null) {
          ...
          for (int i = 0; i < jsonArraytop.size(); i++) {
            TopEditorsByAbsBytesDiffItemsInnerResultsInnerTopInner.validateJsonElement(jsonArraytop.get(i));
          };
        }
      }
  }
