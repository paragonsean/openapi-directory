/**
 * Shutterstock API Explorer
 * The Shutterstock API provides access to Shutterstock's library of media, as well as information about customers' accounts and the contributors that provide the media.
 *
 * The version of the OpenAPI document: 1.1.32
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import RemoveCatalogCollectionItem from './RemoveCatalogCollectionItem';

/**
 * The RemoveCatalogCollectionItems model module.
 * @module model/RemoveCatalogCollectionItems
 * @version 1.1.32
 */
class RemoveCatalogCollectionItems {
    /**
     * Constructs a new <code>RemoveCatalogCollectionItems</code>.
     * @alias module:model/RemoveCatalogCollectionItems
     * @param items {Array.<module:model/RemoveCatalogCollectionItem>} 
     */
    constructor(items) { 
        
        RemoveCatalogCollectionItems.initialize(this, items);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj, items) { 
        obj['items'] = items;
    }

    /**
     * Constructs a <code>RemoveCatalogCollectionItems</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/RemoveCatalogCollectionItems} obj Optional instance to populate.
     * @return {module:model/RemoveCatalogCollectionItems} The populated <code>RemoveCatalogCollectionItems</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new RemoveCatalogCollectionItems();

            if (data.hasOwnProperty('items')) {
                obj['items'] = ApiClient.convertToType(data['items'], [RemoveCatalogCollectionItem]);
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>RemoveCatalogCollectionItems</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>RemoveCatalogCollectionItems</code>.
     */
    static validateJSON(data) {
        // check to make sure all required properties are present in the JSON string
        for (const property of RemoveCatalogCollectionItems.RequiredProperties) {
            if (!data.hasOwnProperty(property)) {
                throw new Error("The required field `" + property + "` is not found in the JSON data: " + JSON.stringify(data));
            }
        }
        if (data['items']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['items'])) {
                throw new Error("Expected the field `items` to be an array in the JSON data but got " + data['items']);
            }
            // validate the optional field `items` (array)
            for (const item of data['items']) {
                RemoveCatalogCollectionItem.validateJSON(item);
            };
        }

        return true;
    }


}

RemoveCatalogCollectionItems.RequiredProperties = ["items"];

/**
 * @member {Array.<module:model/RemoveCatalogCollectionItem>} items
 */
RemoveCatalogCollectionItems.prototype['items'] = undefined;






export default RemoveCatalogCollectionItems;

