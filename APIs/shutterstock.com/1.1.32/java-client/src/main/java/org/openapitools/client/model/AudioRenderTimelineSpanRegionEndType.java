...
public class AudioRenderTimelineSpanRegionEndType {
  ...
  public AudioRenderTimelineSpanRegionEndType beat(BigDecimal beat) {
    this.beat = beat;
    return this;
  }
  ...
  @Override
  public int hashCode() {
    return Objects.hash(beat, event, type);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to AudioRenderTimelineSpanRegionEndType
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AudioRenderTimelineSpanRegionEndType.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!AudioRenderTimelineSpanRegionEndType.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AudioRenderTimelineSpanRegionEndType` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
