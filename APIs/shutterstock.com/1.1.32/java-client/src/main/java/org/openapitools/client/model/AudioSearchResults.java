...
public class AudioSearchResults {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(data, message, page, perPage, searchId, totalCount);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to AudioSearchResults
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AudioSearchResults.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!AudioSearchResults.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AudioSearchResults` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
