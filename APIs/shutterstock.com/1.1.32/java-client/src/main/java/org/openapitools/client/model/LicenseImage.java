...
public class LicenseImage {
  ...
  public LicenseImage authCookie(Cookie authCookie) {
  ...
  public LicenseImage price(BigDecimal price) {
    this.price = price;
    return this;
  }
  ...
  @Override
  public int hashCode() {
    return Objects.hash(authCookie, customDimensions, editorialAcknowledgement, format, imageId, metadata, price, searchId, showModal, size, subscriptionId, verificationCode);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to LicenseImage
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!LicenseImage.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!LicenseImage.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `LicenseImage` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("custom_dimensions") != null && !jsonObj.get("custom_dimensions").isJsonNull()) {
        CustomSizeDimensions.validateJsonElement(jsonObj.get("custom_dimensions"));
      }
