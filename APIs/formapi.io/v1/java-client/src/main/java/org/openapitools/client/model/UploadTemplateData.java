...
import org.openapitools.client.model.UploadTemplateDataDocument;
...
public class UploadTemplateData {
   ...
   * @throws IOException if the JSON Element is invalid with respect to UploadTemplateData
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UploadTemplateData.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!UploadTemplateData.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UploadTemplateData` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("document") != null && !jsonObj.get("document").isJsonNull()) {
        UploadTemplateDataDocument.validateJsonElement(jsonObj.get("document"));
      }
