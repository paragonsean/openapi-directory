...
public class CreateHtmlTemplateData {
   ...
   * @throws IOException if the JSON Element is invalid with respect to CreateHtmlTemplateData
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CreateHtmlTemplateData.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CreateHtmlTemplateData.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CreateHtmlTemplateData` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      HtmlTemplateData.validateJsonElement(jsonObj.get("template"));
  }
