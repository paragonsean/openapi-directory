...
import org.openapitools.client.model.CreateSubmissionDataRequestTokenResponseToken;
...
public class CreateSubmissionDataRequestTokenResponse {
   ...
   * @throws IOException if the JSON Element is invalid with respect to CreateSubmissionDataRequestTokenResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CreateSubmissionDataRequestTokenResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CreateSubmissionDataRequestTokenResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CreateSubmissionDataRequestTokenResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      CreateSubmissionDataRequestTokenResponseToken.validateJsonElement(jsonObj.get("token"));
  }
