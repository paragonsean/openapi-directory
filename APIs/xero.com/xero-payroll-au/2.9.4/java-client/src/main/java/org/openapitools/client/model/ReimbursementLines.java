...
public class ReimbursementLines {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ReimbursementLines
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ReimbursementLines.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ReimbursementLines is not found in the empty JSON string", ReimbursementLines.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ReimbursementLines.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ReimbursementLines` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ReimbursementLines.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ReimbursementLines' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ReimbursementLines> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ReimbursementLines.class));

       return (TypeAdapter<T>) new TypeAdapter<ReimbursementLines>() {
           @Override
           public void write(JsonWriter out, ReimbursementLines value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ReimbursementLines read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of ReimbursementLines given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of ReimbursementLines
   * @throws IOException if the JSON string is invalid with respect to ReimbursementLines
   */
  public static ReimbursementLines fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ReimbursementLines.class);
  }

  /**
   * Convert an instance of ReimbursementLines to an JSON string
   *
   * @return JSON string
   */
