...
public class TrackingCategories {
   ...
   * @throws IOException if the JSON Element is invalid with respect to TrackingCategories
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!TrackingCategories.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in TrackingCategories is not found in the empty JSON string", TrackingCategories.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!TrackingCategories.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TrackingCategories` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!TrackingCategories.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'TrackingCategories' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<TrackingCategories> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(TrackingCategories.class));

       return (TypeAdapter<T>) new TypeAdapter<TrackingCategories>() {
           @Override
           public void write(JsonWriter out, TrackingCategories value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public TrackingCategories read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of TrackingCategories given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of TrackingCategories
   * @throws IOException if the JSON string is invalid with respect to TrackingCategories
   */
  public static TrackingCategories fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, TrackingCategories.class);
  }

  /**
   * Convert an instance of TrackingCategories to an JSON string
   *
   * @return JSON string
   */
