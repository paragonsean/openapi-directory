/**
 * Lucidtech API
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 2023-03-23T15:40:40Z
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIPaymentMethods_paymentMethods_inner.h
 *
 * 
 */

#ifndef OAIPaymentMethods_paymentMethods_inner_H
#define OAIPaymentMethods_paymentMethods_inner_H

#include <QJsonObject>

#include "OAIObject.h"
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {

class OAIPaymentMethods_paymentMethods_inner : public OAIObject {
public:
    OAIPaymentMethods_paymentMethods_inner();
    OAIPaymentMethods_paymentMethods_inner(QString json);
    ~OAIPaymentMethods_paymentMethods_inner() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QString getCreatedBy() const;
    void setCreatedBy(const QString &created_by);
    bool is_created_by_Set() const;
    bool is_created_by_Valid() const;

    QString getCreatedTime() const;
    void setCreatedTime(const QString &created_time);
    bool is_created_time_Set() const;
    bool is_created_time_Valid() const;

    QString getDescription() const;
    void setDescription(const QString &description);
    bool is_description_Set() const;
    bool is_description_Valid() const;

    OAIObject getDetails() const;
    void setDetails(const OAIObject &details);
    bool is_details_Set() const;
    bool is_details_Valid() const;

    QString getName() const;
    void setName(const QString &name);
    bool is_name_Set() const;
    bool is_name_Valid() const;

    QString getPaymentMethodId() const;
    void setPaymentMethodId(const QString &payment_method_id);
    bool is_payment_method_id_Set() const;
    bool is_payment_method_id_Valid() const;

    QString getStripePublishableKey() const;
    void setStripePublishableKey(const QString &stripe_publishable_key);
    bool is_stripe_publishable_key_Set() const;
    bool is_stripe_publishable_key_Valid() const;

    QString getStripeSetupIntentSecret() const;
    void setStripeSetupIntentSecret(const QString &stripe_setup_intent_secret);
    bool is_stripe_setup_intent_secret_Set() const;
    bool is_stripe_setup_intent_secret_Valid() const;

    QString getUpdatedBy() const;
    void setUpdatedBy(const QString &updated_by);
    bool is_updated_by_Set() const;
    bool is_updated_by_Valid() const;

    QString getUpdatedTime() const;
    void setUpdatedTime(const QString &updated_time);
    bool is_updated_time_Set() const;
    bool is_updated_time_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString m_created_by;
    bool m_created_by_isSet;
    bool m_created_by_isValid;

    QString m_created_time;
    bool m_created_time_isSet;
    bool m_created_time_isValid;

    QString m_description;
    bool m_description_isSet;
    bool m_description_isValid;

    OAIObject m_details;
    bool m_details_isSet;
    bool m_details_isValid;

    QString m_name;
    bool m_name_isSet;
    bool m_name_isValid;

    QString m_payment_method_id;
    bool m_payment_method_id_isSet;
    bool m_payment_method_id_isValid;

    QString m_stripe_publishable_key;
    bool m_stripe_publishable_key_isSet;
    bool m_stripe_publishable_key_isValid;

    QString m_stripe_setup_intent_secret;
    bool m_stripe_setup_intent_secret_isSet;
    bool m_stripe_setup_intent_secret_isValid;

    QString m_updated_by;
    bool m_updated_by_isSet;
    bool m_updated_by_isValid;

    QString m_updated_time;
    bool m_updated_time_isSet;
    bool m_updated_time_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIPaymentMethods_paymentMethods_inner)

#endif // OAIPaymentMethods_paymentMethods_inner_H
