...
public class Example {
  public static void main(String[] args) {
    ...
    LocalDate month = LocalDate.now(); // LocalDate | The budget month in ISO format (e.g. 2016-12-01) (\"current\" can also be used to specify the current calendar month (UTC))
    String categoryId = "categoryId_example"; // String | The id of the category
    PatchMonthCategoryWrapper patchMonthCategoryWrapper = new PatchMonthCategoryWrapper(); // PatchMonthCategoryWrapper | The category to update.  Only `budgeted` amount can be updated and any other fields specified will be ignored.
    try {
      SaveCategoryResponse result = apiInstance.updateMonthCategory(budgetId, month, categoryId, patchMonthCategoryWrapper);
      System.out.println(result);
    } catch (ApiException e) {
