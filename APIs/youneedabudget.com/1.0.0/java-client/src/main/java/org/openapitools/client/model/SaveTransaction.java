...
public class SaveTransaction {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(accountId, amount, approved, categoryId, cleared, date, flagColor, importId, memo, payeeId, payeeName, subtransactions);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to SaveTransaction
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SaveTransaction.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SaveTransaction.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SaveTransaction` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : SaveTransaction.openapiRequiredFields) {
      ...
      // validate the optional field `cleared`
      if (jsonObj.get("cleared") != null && !jsonObj.get("cleared").isJsonNull()) {
      ...
      if (jsonObj.get("subtransactions") != null && !jsonObj.get("subtransactions").isJsonNull()) {
        ...
        if (jsonArraysubtransactions != null) {
          ...
          for (int i = 0; i < jsonArraysubtransactions.size(); i++) {
            SaveSubTransaction.validateJsonElement(jsonArraysubtransactions.get(i));
          };
        }
      }
  }
