...
public class MessageConversation {
  ...
  public MessageConversation id(String id) {
    this.id = id;
    return this;
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to MessageConversation
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!MessageConversation.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in MessageConversation is not found in the empty JSON string", MessageConversation.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!MessageConversation.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `MessageConversation` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!MessageConversation.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'MessageConversation' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<MessageConversation> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(MessageConversation.class));

       return (TypeAdapter<T>) new TypeAdapter<MessageConversation>() {
           @Override
           public void write(JsonWriter out, MessageConversation value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public MessageConversation read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of MessageConversation given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of MessageConversation
   * @throws IOException if the JSON string is invalid with respect to MessageConversation
   */
  public static MessageConversation fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, MessageConversation.class);
  }

  /**
   * Convert an instance of MessageConversation to an JSON string
   *
   * @return JSON string
   */
