...
public class SetEnvVarValueRequest {
   ...
   * @throws IOException if the JSON Element is invalid with respect to SetEnvVarValueRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SetEnvVarValueRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in SetEnvVarValueRequest is not found in the empty JSON string", SetEnvVarValueRequest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SetEnvVarValueRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SetEnvVarValueRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!SetEnvVarValueRequest.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'SetEnvVarValueRequest' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<SetEnvVarValueRequest> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(SetEnvVarValueRequest.class));

       return (TypeAdapter<T>) new TypeAdapter<SetEnvVarValueRequest>() {
           @Override
           public void write(JsonWriter out, SetEnvVarValueRequest value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public SetEnvVarValueRequest read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of SetEnvVarValueRequest given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of SetEnvVarValueRequest
   * @throws IOException if the JSON string is invalid with respect to SetEnvVarValueRequest
   */
  public static SetEnvVarValueRequest fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, SetEnvVarValueRequest.class);
  }

  /**
   * Convert an instance of SetEnvVarValueRequest to an JSON string
   *
   * @return JSON string
   */
