...
public class BuildLogMsg {
   ...
   * @throws IOException if the JSON Element is invalid with respect to BuildLogMsg
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!BuildLogMsg.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in BuildLogMsg is not found in the empty JSON string", BuildLogMsg.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!BuildLogMsg.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BuildLogMsg` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!BuildLogMsg.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'BuildLogMsg' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<BuildLogMsg> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(BuildLogMsg.class));

       return (TypeAdapter<T>) new TypeAdapter<BuildLogMsg>() {
           @Override
           public void write(JsonWriter out, BuildLogMsg value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public BuildLogMsg read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of BuildLogMsg given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of BuildLogMsg
   * @throws IOException if the JSON string is invalid with respect to BuildLogMsg
   */
  public static BuildLogMsg fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, BuildLogMsg.class);
  }

  /**
   * Convert an instance of BuildLogMsg to an JSON string
   *
   * @return JSON string
   */
