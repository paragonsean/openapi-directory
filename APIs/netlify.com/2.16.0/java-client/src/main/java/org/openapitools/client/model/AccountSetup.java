...
public class AccountSetup {
   ...
   * @throws IOException if the JSON Element is invalid with respect to AccountSetup
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AccountSetup.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in AccountSetup is not found in the empty JSON string", AccountSetup.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!AccountSetup.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AccountSetup` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : AccountSetup.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!AccountSetup.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'AccountSetup' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<AccountSetup> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(AccountSetup.class));

       return (TypeAdapter<T>) new TypeAdapter<AccountSetup>() {
           @Override
           public void write(JsonWriter out, AccountSetup value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public AccountSetup read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of AccountSetup given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of AccountSetup
   * @throws IOException if the JSON string is invalid with respect to AccountSetup
   */
  public static AccountSetup fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, AccountSetup.class);
  }

  /**
   * Convert an instance of AccountSetup to an JSON string
   *
   * @return JSON string
   */
