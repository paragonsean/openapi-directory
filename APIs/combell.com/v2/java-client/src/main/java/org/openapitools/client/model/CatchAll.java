...
public class CatchAll {
  ...
  public CatchAll emailAddresses(List<String> emailAddresses) {
    this.emailAddresses = emailAddresses;
    return this;
  }

  public CatchAll addEmailAddressesItem(String emailAddressesItem) {
  ...
  public boolean equals(Object o) {
    ...
    CatchAll catchAll = (CatchAll) o;
    return Objects.equals(this.emailAddresses, catchAll.emailAddresses);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to CatchAll
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CatchAll.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in CatchAll is not found in the empty JSON string", CatchAll.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CatchAll.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CatchAll` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!CatchAll.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'CatchAll' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<CatchAll> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(CatchAll.class));

       return (TypeAdapter<T>) new TypeAdapter<CatchAll>() {
           @Override
           public void write(JsonWriter out, CatchAll value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public CatchAll read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of CatchAll given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of CatchAll
   * @throws IOException if the JSON string is invalid with respect to CatchAll
   */
  public static CatchAll fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, CatchAll.class);
  }

  /**
   * Convert an instance of CatchAll to an JSON string
   *
   * @return JSON string
   */
