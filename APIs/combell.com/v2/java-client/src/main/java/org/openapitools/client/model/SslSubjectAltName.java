...
import org.openapitools.client.model.SslSubjectAltNameType;
...
public class SslSubjectAltName {
  ...
  private SslSubjectAltNameType type;
  ...
  public SslSubjectAltName type(SslSubjectAltNameType type) {
    this.type = type;
    return this;
  }
  ...
  public SslSubjectAltNameType getType() {
    return type;
  }

  public void setType(SslSubjectAltNameType type) {
    this.type = type;
  }


  public SslSubjectAltName value(String value) {
    this.value = value;
    return this;
  }
  ...
  public boolean equals(Object o) {
    ...
    SslSubjectAltName sslSubjectAltName = (SslSubjectAltName) o;
    return Objects.equals(this.type, sslSubjectAltName.type) &&
        Objects.equals(this.value, sslSubjectAltName.value);
  }
  ...
  public String toString() {
    ...
    sb.append("class SslSubjectAltName {\n");
   ...
   * @throws IOException if the JSON Element is invalid with respect to SslSubjectAltName
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SslSubjectAltName.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in SslSubjectAltName is not found in the empty JSON string", SslSubjectAltName.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SslSubjectAltName.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SslSubjectAltName` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // validate the optional field `type`
      if (jsonObj.get("type") != null && !jsonObj.get("type").isJsonNull()) {
        SslSubjectAltNameType.validateJsonElement(jsonObj.get("type"));
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!SslSubjectAltName.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'SslSubjectAltName' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<SslSubjectAltName> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(SslSubjectAltName.class));

       return (TypeAdapter<T>) new TypeAdapter<SslSubjectAltName>() {
           @Override
           public void write(JsonWriter out, SslSubjectAltName value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public SslSubjectAltName read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of SslSubjectAltName given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of SslSubjectAltName
   * @throws IOException if the JSON string is invalid with respect to SslSubjectAltName
   */
  public static SslSubjectAltName fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, SslSubjectAltName.class);
  }

  /**
   * Convert an instance of SslSubjectAltName to an JSON string
   *
   * @return JSON string
   */
