...
public class AntiSpam {
  ...
  private List<AntiSpamTypes> allowedTypes = new ArrayList<>();
  ...
  public AntiSpam allowedTypes(List<AntiSpamTypes> allowedTypes) {
    this.allowedTypes = allowedTypes;
    return this;
  }

  public AntiSpam addAllowedTypesItem(AntiSpamTypes allowedTypesItem) {
  ...
  public List<AntiSpamTypes> getAllowedTypes() {
    return allowedTypes;
  }

  public void setAllowedTypes(List<AntiSpamTypes> allowedTypes) {
    this.allowedTypes = allowedTypes;
  }


  public AntiSpam type(AntiSpamTypes type) {
    this.type = type;
    return this;
  }
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(this.allowedTypes, antiSpam.allowedTypes) &&
        Objects.equals(this.type, antiSpam.type);
  }

  @Override
  public int hashCode() {
    return Objects.hash(allowedTypes, type);
  }

  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class AntiSpam {\n");
   ...
   * @throws IOException if the JSON Element is invalid with respect to AntiSpam
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AntiSpam.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in AntiSpam is not found in the empty JSON string", AntiSpam.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!AntiSpam.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AntiSpam` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // ensure the optional json data is an array if present
      if (jsonObj.get("allowed_types") != null && !jsonObj.get("allowed_types").isJsonNull() && !jsonObj.get("allowed_types").isJsonArray()) {
        throw new IllegalArgumentException(String.format("Expected the field `allowed_types` to be an array in the JSON string but got `%s`", jsonObj.get("allowed_types").toString()));
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!AntiSpam.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'AntiSpam' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<AntiSpam> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(AntiSpam.class));

       return (TypeAdapter<T>) new TypeAdapter<AntiSpam>() {
           @Override
           public void write(JsonWriter out, AntiSpam value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public AntiSpam read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of AntiSpam given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of AntiSpam
   * @throws IOException if the JSON string is invalid with respect to AntiSpam
   */
  public static AntiSpam fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, AntiSpam.class);
  }

  /**
   * Convert an instance of AntiSpam to an JSON string
   *
   * @return JSON string
   */
