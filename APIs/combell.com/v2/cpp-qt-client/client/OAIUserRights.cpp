...
OAIUserRights::OAIUserRights(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIUserRights::OAIUserRights() {
    this->initializeModel();
}

OAIUserRights::~OAIUserRights() {}

void OAIUserRights::initializeModel() {

    m_value_isSet = false;
    m_value_isValid = false;
    m_value = eOAIUserRights::INVALID_VALUE_OPENAPI_GENERATED;
}

void OAIUserRights::fromJson(QString jsonString) {
    
    if ( jsonString.compare("read_and_write", Qt::CaseInsensitive) == 0) {
        m_value = eOAIUserRights::AND_WRITE;
        m_value_isSet = m_value_isValid = true;
    }
    else if ( jsonString.compare("read_only", Qt::CaseInsensitive) == 0) {
        m_value = eOAIUserRights::ONLY;
        m_value_isSet = m_value_isValid = true;
    }
}

void OAIUserRights::fromJsonValue(QJsonValue json) {
fromJson(json.toString());
}

QString OAIUserRights::asJson() const {
    
    QString val;
    switch (m_value){
        case eOAIUserRights::AND_WRITE:
            val = "read_and_write";
            break;
        case eOAIUserRights::ONLY:
            val = "read_only";
            break;
        default:
            break;
...
QJsonValue OAIUserRights::asJsonValue() const {
    
    return QJsonValue(asJson());
}


OAIUserRights::eOAIUserRights OAIUserRights::getValue() const {
    return m_value;
}

void OAIUserRights::setValue(const OAIUserRights::eOAIUserRights& value){
    m_value = value;
    m_value_isSet = true;
}
bool OAIUserRights::isSet() const {
    
    return m_value_isSet;
}

bool OAIUserRights::isValid() const {
