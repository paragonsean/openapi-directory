...
void OAIAccountsApi::createAccount(const ::OpenAPI::OptionalParam<OAICreateAccount> &oai_create_account) {
    ...
    if (oai_create_account.hasValue()){

        
        QByteArray output = oai_create_account.value().asJson().toUtf8();
...
void OAIAccountsApi::createAccountCallback(OAIHttpRequestWorker *worker) {
    ...
    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT createAccountSignal();
        Q_EMIT createAccountSignalFull(worker);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT createAccountSignalE(error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT createAccountSignalError(error_type, error_str);
        Q_EMIT createAccountSignalErrorFull(worker, error_type, error_str);
    }
}

void OAIAccountsApi::getAccount(const qint32 &account_id, const QString &account_id2) {
    QString fullPath = QString(_serverConfigs["getAccount"][_serverIndices.value("getAccount")].URL()+"/accounts/{accountId}");
    
    
    {
        QString account_id2PathParam("{");
        account_id2PathParam.append("accountId").append("}");
        ...
        fullPath.replace(account_id2PathParam, paramString+QUrl::toPercentEncoding(::OpenAPI::toStringValue(account_id2)));
    }
...
void OAIAccountsApi::getAccountCallback(OAIHttpRequestWorker *worker) {
    ...
    OAIAccountDetail output(QString(worker->response));
    worker->deleteLater();
...
void OAIAccountsApi::getAccounts(const ::OpenAPI::OptionalParam<qint32> &skip, const ::OpenAPI::OptionalParam<qint32> &take, const ::OpenAPI::OptionalParam<OAIAssetType> &asset_type, const ::OpenAPI::OptionalParam<QString> &identifier) {
    QString fullPath = QString(_serverConfigs["getAccounts"][_serverIndices.value("getAccounts")].URL()+"/accounts");
    ...
    if (asset_type.hasValue())
    {
        ...
        if (fullPath.indexOf("?") > 0)
            ...
            fullPath.append("?");
        QString paramString = (queryStyle == "form" && true) ? "" : (queryStyle == "form" && !(true)) ? "asset_type"+querySuffix : "";
        QJsonObject parameter = asset_type.value().asJsonObject();
        qint32 count = 0;
        for(const QString& key : parameter.keys()) {
            ...
            switch(parameter.value(key).type()) {
                ...
                {
                    paramString.append(((queryStyle == "form") ? key : QString("asset_type").append("[").append(key).append("]"))+assignOperator+parameter.value(key).toString());
                    break;
                }
                case QJsonValue::Double:
                {
                    paramString.append(((queryStyle == "form") ? key : QString("asset_type").append("[").append(key).append("]"))+assignOperator+QString::number(parameter.value(key).toDouble()));
                    break;
                }
                case QJsonValue::Bool:
                {
                    paramString.append(((queryStyle == "form") ? key : QString("asset_type").append("[").append(key).append("]"))+assignOperator+QVariant(parameter.value(key).toBool()).toString());
                    break;
                }
                case QJsonValue::Array:
                {
                    paramString.append(((queryStyle == "form") ? key : QString("asset_type").append("[").append(key).append("]"))+assignOperator+QVariant(parameter.value(key).toArray()).toString());
                    break;
                }
                case QJsonValue::Object:
                {
                    paramString.append(((queryStyle == "form") ? key : QString("asset_type").append("[").append(key).append("]"))+assignOperator+QVariant(parameter.value(key).toObject()).toString());
                    break;
                }
