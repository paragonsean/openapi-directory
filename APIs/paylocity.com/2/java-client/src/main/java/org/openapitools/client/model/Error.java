...
public class Error {
  ...
  private List<ErrorOptionsInner> options = new ArrayList<>();
  ...
  @Override
  public int hashCode() {
    return Objects.hash(field, message, options, path);
  }
      ...
      if (jsonObj.get("options") != null && !jsonObj.get("options").isJsonNull()) {
        ...
        if (jsonArrayoptions != null) {
          ...
          for (int i = 0; i < jsonArrayoptions.size(); i++) {
            ErrorOptionsInner.validateJsonElement(jsonArrayoptions.get(i));
          };
        }
      }
