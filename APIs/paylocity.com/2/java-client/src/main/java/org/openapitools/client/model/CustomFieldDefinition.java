...
import org.openapitools.client.model.CustomFieldDefinitionValuesInner;
...
public class CustomFieldDefinition {
  ...
  private List<CustomFieldDefinitionValuesInner> values = new ArrayList<>();
  ...
  @Override
  public int hashCode() {
    return Objects.hash(category, defaultValue, isRequired, label, type, values);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to CustomFieldDefinition
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CustomFieldDefinition.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CustomFieldDefinition.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CustomFieldDefinition` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("values") != null && !jsonObj.get("values").isJsonNull()) {
        ...
        if (jsonArrayvalues != null) {
          ...
          for (int i = 0; i < jsonArrayvalues.size(); i++) {
            CustomFieldDefinitionValuesInner.validateJsonElement(jsonArrayvalues.get(i));
          };
        }
      }
  }
