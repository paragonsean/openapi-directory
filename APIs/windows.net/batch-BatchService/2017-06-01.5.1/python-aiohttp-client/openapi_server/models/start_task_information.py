...
class StartTaskInformation(Model):
    ...
    def __init__(self, end_time: datetime=None, exit_code: int=None, failure_info: TaskFailureInformation=None, last_retry_time: datetime=None, result: TaskExecutionResult=None, retry_count: int=None, start_time: datetime=None, state: str=None):
    ...
    def state(self):
        ...
        Values are:   running - The start task is currently running.  completed - The start task has exited with exit code 0, or the start task has failed and the retry limit has reached, or the start task process did not run due to task preparation errors (such as resource file download failures).
