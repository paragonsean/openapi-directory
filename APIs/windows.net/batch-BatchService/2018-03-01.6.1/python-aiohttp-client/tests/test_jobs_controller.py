...
async def test_job_get_all_lifetime_statistics(client):
...
async def test_job_get_task_counts(client):
    ...
    response = await client.request(
        method='GET',
        path='/jobs/{job_id}/taskcounts'.format(job_id='job_id_example'),
        headers=headers,
        params=params,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
