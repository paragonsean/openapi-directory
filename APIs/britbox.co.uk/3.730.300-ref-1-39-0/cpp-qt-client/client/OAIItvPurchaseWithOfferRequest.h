/**
 * Rocket Services
 * An Orchestration Layer that takes ISL services and packages them in a more targeted way for front-end applications. This in turn makes client integration easier and reduces the complexity and size of front-end applications.  Rocket is also customisable - allowing UI engineers to ‘remix’ the existing back-end services into something that best suits the application they are developing. 
 *
 * The version of the OpenAPI document: 3.730.300-ref-1-39-0
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIItvPurchaseWithOfferRequest.h
 *
 * 
 */

#ifndef OAIItvPurchaseWithOfferRequest_H
#define OAIItvPurchaseWithOfferRequest_H

#include <QJsonObject>

#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {

class OAIItvPurchaseWithOfferRequest : public OAIObject {
public:
    OAIItvPurchaseWithOfferRequest();
    OAIItvPurchaseWithOfferRequest(QString json);
    ~OAIItvPurchaseWithOfferRequest() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QString getCouponId() const;
    void setCouponId(const QString &coupon_id);
    bool is_coupon_id_Set() const;
    bool is_coupon_id_Valid() const;

    QString getPaymentMethodFromToken() const;
    void setPaymentMethodFromToken(const QString &payment_method_from_token);
    bool is_payment_method_from_token_Set() const;
    bool is_payment_method_from_token_Valid() const;

    QString getPaymentMethodId() const;
    void setPaymentMethodId(const QString &payment_method_id);
    bool is_payment_method_id_Set() const;
    bool is_payment_method_id_Valid() const;

    QString getPlanId() const;
    void setPlanId(const QString &plan_id);
    bool is_plan_id_Set() const;
    bool is_plan_id_Valid() const;

    QString getProfileToken() const;
    void setProfileToken(const QString &profile_token);
    bool is_profile_token_Set() const;
    bool is_profile_token_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString m_coupon_id;
    bool m_coupon_id_isSet;
    bool m_coupon_id_isValid;

    QString m_payment_method_from_token;
    bool m_payment_method_from_token_isSet;
    bool m_payment_method_from_token_isValid;

    QString m_payment_method_id;
    bool m_payment_method_id_isSet;
    bool m_payment_method_id_isValid;

    QString m_plan_id;
    bool m_plan_id_isSet;
    bool m_plan_id_isValid;

    QString m_profile_token;
    bool m_profile_token_isSet;
    bool m_profile_token_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIItvPurchaseWithOfferRequest)

#endif // OAIItvPurchaseWithOfferRequest_H
