/**
 * Data API
 * Serves the Clever Data API
 *
 * The version of the OpenAPI document: 1.2.0
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#include "OAIDistrict.h"

#include <QDebug>
#include <QJsonArray>
#include <QJsonDocument>
#include <QObject>

#include "OAIHelpers.h"

namespace OpenAPI {

OAIDistrict::OAIDistrict(QString json) {
    this->initializeModel();
    this->fromJson(json);
}

OAIDistrict::OAIDistrict() {
    this->initializeModel();
}

OAIDistrict::~OAIDistrict() {}

void OAIDistrict::initializeModel() {

    m_id_isSet = false;
    m_id_isValid = false;

    m_mdr_number_isSet = false;
    m_mdr_number_isValid = false;

    m_name_isSet = false;
    m_name_isValid = false;
}

void OAIDistrict::fromJson(QString jsonString) {
    QByteArray array(jsonString.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
}

void OAIDistrict::fromJsonObject(QJsonObject json) {

    m_id_isValid = ::OpenAPI::fromJsonValue(m_id, json[QString("id")]);
    m_id_isSet = !json[QString("id")].isNull() && m_id_isValid;

    m_mdr_number_isValid = ::OpenAPI::fromJsonValue(m_mdr_number, json[QString("mdr_number")]);
    m_mdr_number_isSet = !json[QString("mdr_number")].isNull() && m_mdr_number_isValid;

    m_name_isValid = ::OpenAPI::fromJsonValue(m_name, json[QString("name")]);
    m_name_isSet = !json[QString("name")].isNull() && m_name_isValid;
}

QString OAIDistrict::asJson() const {
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject OAIDistrict::asJsonObject() const {
    QJsonObject obj;
    if (m_id_isSet) {
        obj.insert(QString("id"), ::OpenAPI::toJsonValue(m_id));
    }
    if (m_mdr_number_isSet) {
        obj.insert(QString("mdr_number"), ::OpenAPI::toJsonValue(m_mdr_number));
    }
    if (m_name_isSet) {
        obj.insert(QString("name"), ::OpenAPI::toJsonValue(m_name));
    }
    return obj;
}

QString OAIDistrict::getId() const {
    return m_id;
}
void OAIDistrict::setId(const QString &id) {
    m_id = id;
    m_id_isSet = true;
}

bool OAIDistrict::is_id_Set() const{
    return m_id_isSet;
}

bool OAIDistrict::is_id_Valid() const{
    return m_id_isValid;
}

QString OAIDistrict::getMdrNumber() const {
    return m_mdr_number;
}
void OAIDistrict::setMdrNumber(const QString &mdr_number) {
    m_mdr_number = mdr_number;
    m_mdr_number_isSet = true;
}

bool OAIDistrict::is_mdr_number_Set() const{
    return m_mdr_number_isSet;
}

bool OAIDistrict::is_mdr_number_Valid() const{
    return m_mdr_number_isValid;
}

QString OAIDistrict::getName() const {
    return m_name;
}
void OAIDistrict::setName(const QString &name) {
    m_name = name;
    m_name_isSet = true;
}

bool OAIDistrict::is_name_Set() const{
    return m_name_isSet;
}

bool OAIDistrict::is_name_Valid() const{
    return m_name_isValid;
}

bool OAIDistrict::isSet() const {
    bool isObjectUpdated = false;
    do {
        if (m_id_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_mdr_number_isSet) {
            isObjectUpdated = true;
            break;
        }

        if (m_name_isSet) {
            isObjectUpdated = true;
            break;
        }
    } while (false);
    return isObjectUpdated;
}

bool OAIDistrict::isValid() const {
    // only required properties are required for the object to be considered valid
    return true;
}

} // namespace OpenAPI
