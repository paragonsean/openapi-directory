...
> CompanyDataRoleSearchResultsInterface companyRoleRepositoryV1GetListGet(opts)
...
let apiInstance = new MagentoB2B.CompanyRoleApi();
let opts = {
...
apiInstance.companyRoleRepositoryV1GetListGet(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
> CompanyDataRoleInterface companyRoleRepositoryV1SavePost(opts)
...
apiInstance.companyRoleRepositoryV1SavePost(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
