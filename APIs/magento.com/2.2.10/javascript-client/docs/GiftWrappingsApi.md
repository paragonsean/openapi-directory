...
let apiInstance = new MagentoB2B.GiftWrappingsApi();
let opts = {
...
apiInstance.giftWrappingWrappingRepositoryV1GetListGet(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
apiInstance.giftWrappingWrappingRepositoryV1SavePost(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
