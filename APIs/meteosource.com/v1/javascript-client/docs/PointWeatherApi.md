# InteractiveDocumentationForYourPremiumPlan.PointWeatherApi
...
> AirQualityPointData airQualityAirQualityGet(opts)
...
let apiInstance = new InteractiveDocumentationForYourPremiumPlan.PointWeatherApi();
let opts = {
...
> PointPointData pointPointGet(opts)
...
apiInstance.pointPointGet(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
