# BulkSmsJsonRestApi.AttachmentsApi
...
let apiInstance = new BulkSmsJsonRestApi.AttachmentsApi();
let preSignRequest = new BulkSmsJsonRestApi.PreSignRequest(); // PreSignRequest | Describes the file to upload
apiInstance.rmmPreSignAttachmentPost(preSignRequest, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
