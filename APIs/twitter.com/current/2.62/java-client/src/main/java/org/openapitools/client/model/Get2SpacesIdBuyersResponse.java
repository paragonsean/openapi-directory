...
public class Get2SpacesIdBuyersResponse {
   ...
   * @throws IOException if the JSON Element is invalid with respect to Get2SpacesIdBuyersResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Get2SpacesIdBuyersResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Get2SpacesIdBuyersResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Get2SpacesIdBuyersResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
