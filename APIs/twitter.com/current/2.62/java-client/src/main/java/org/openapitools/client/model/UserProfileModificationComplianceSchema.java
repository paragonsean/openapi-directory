...
public class UserProfileModificationComplianceSchema {
   ...
   * @throws IOException if the JSON Element is invalid with respect to UserProfileModificationComplianceSchema
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UserProfileModificationComplianceSchema.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!UserProfileModificationComplianceSchema.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UserProfileModificationComplianceSchema` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      UserProfileModificationObjectSchema.validateJsonElement(jsonObj.get("user_profile_modification"));
  }
