...
public class UserWithheldComplianceSchema {
   ...
   * @throws IOException if the JSON Element is invalid with respect to UserWithheldComplianceSchema
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UserWithheldComplianceSchema.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!UserWithheldComplianceSchema.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UserWithheldComplianceSchema` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      UserTakedownComplianceSchema.validateJsonElement(jsonObj.get("user_withheld"));
  }
