...
public class Get2ComplianceJobsResponse {
  ...
  private List<ComplianceJob> data = new ArrayList<>();
   ...
   * @throws IOException if the JSON Element is invalid with respect to Get2ComplianceJobsResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Get2ComplianceJobsResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Get2ComplianceJobsResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Get2ComplianceJobsResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("data") != null && !jsonObj.get("data").isJsonNull()) {
        ...
        if (jsonArraydata != null) {
          ...
          for (int i = 0; i < jsonArraydata.size(); i++) {
            ComplianceJob.validateJsonElement(jsonArraydata.get(i));
          };
        }
      }
