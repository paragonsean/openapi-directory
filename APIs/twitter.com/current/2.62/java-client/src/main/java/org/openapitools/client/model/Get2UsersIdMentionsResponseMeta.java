...
public class Get2UsersIdMentionsResponseMeta {
   ...
   * @throws IOException if the JSON Element is invalid with respect to Get2UsersIdMentionsResponseMeta
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Get2UsersIdMentionsResponseMeta.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Get2UsersIdMentionsResponseMeta.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Get2UsersIdMentionsResponseMeta` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
