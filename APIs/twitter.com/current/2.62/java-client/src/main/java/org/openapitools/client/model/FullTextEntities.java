...
public class FullTextEntities {
  ...
  private List<FullTextEntitiesAnnotationsInner> annotations = new ArrayList<>();

  public static final String SERIALIZED_NAME_CASHTAGS = "cashtags";
  @SerializedName(SERIALIZED_NAME_CASHTAGS)
  private List<CashtagEntity> cashtags = new ArrayList<>();
   ...
   * @throws IOException if the JSON Element is invalid with respect to FullTextEntities
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!FullTextEntities.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!FullTextEntities.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `FullTextEntities` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("annotations") != null && !jsonObj.get("annotations").isJsonNull()) {
        ...
        if (jsonArrayannotations != null) {
          ...
          for (int i = 0; i < jsonArrayannotations.size(); i++) {
            FullTextEntitiesAnnotationsInner.validateJsonElement(jsonArrayannotations.get(i));
          };
        }
      }
      if (jsonObj.get("cashtags") != null && !jsonObj.get("cashtags").isJsonNull()) {
        ...
        if (jsonArraycashtags != null) {
          ...
          for (int i = 0; i < jsonArraycashtags.size(); i++) {
            CashtagEntity.validateJsonElement(jsonArraycashtags.get(i));
          };
        }
      }
      if (jsonObj.get("hashtags") != null && !jsonObj.get("hashtags").isJsonNull()) {
        ...
        if (jsonArrayhashtags != null) {
          ...
          for (int i = 0; i < jsonArrayhashtags.size(); i++) {
            HashtagEntity.validateJsonElement(jsonArrayhashtags.get(i));
          };
        }
      }
      if (jsonObj.get("mentions") != null && !jsonObj.get("mentions").isJsonNull()) {
        ...
        if (jsonArraymentions != null) {
          ...
          for (int i = 0; i < jsonArraymentions.size(); i++) {
            MentionEntity.validateJsonElement(jsonArraymentions.get(i));
          };
        }
      }
