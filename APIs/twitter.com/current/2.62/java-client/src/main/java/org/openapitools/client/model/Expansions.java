...
public class Expansions {
   ...
   * @throws IOException if the JSON Element is invalid with respect to Expansions
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Expansions.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Expansions.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Expansions` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("polls") != null && !jsonObj.get("polls").isJsonNull()) {
        ...
        if (jsonArraypolls != null) {
          ...
          for (int i = 0; i < jsonArraypolls.size(); i++) {
            Poll.validateJsonElement(jsonArraypolls.get(i));
          };
        }
      }
      ...
      if (jsonObj.get("tweets") != null && !jsonObj.get("tweets").isJsonNull()) {
        ...
        if (jsonArraytweets != null) {
          ...
          for (int i = 0; i < jsonArraytweets.size(); i++) {
            Tweet.validateJsonElement(jsonArraytweets.get(i));
          };
        }
      }
