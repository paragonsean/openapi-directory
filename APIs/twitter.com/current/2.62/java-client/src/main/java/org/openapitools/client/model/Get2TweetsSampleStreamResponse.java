...
public class Get2TweetsSampleStreamResponse {
   ...
   * @throws IOException if the JSON Element is invalid with respect to Get2TweetsSampleStreamResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Get2TweetsSampleStreamResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Get2TweetsSampleStreamResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Get2TweetsSampleStreamResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
