...
public class Get2UsersByResponse {
   ...
   * @throws IOException if the JSON Element is invalid with respect to Get2UsersByResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Get2UsersByResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Get2UsersByResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Get2UsersByResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
