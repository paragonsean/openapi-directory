...
public class ComplianceJob {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ComplianceJob
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ComplianceJob.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ComplianceJob.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ComplianceJob` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      ComplianceJobStatus.validateJsonElement(jsonObj.get("status"));
      // validate the required field `type`
      ComplianceJobType.validateJsonElement(jsonObj.get("type"));
