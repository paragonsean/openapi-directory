...
public class Get2TweetsSearchAllResponseMeta {
   ...
   * @throws IOException if the JSON Element is invalid with respect to Get2TweetsSearchAllResponseMeta
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Get2TweetsSearchAllResponseMeta.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Get2TweetsSearchAllResponseMeta.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Get2TweetsSearchAllResponseMeta` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
