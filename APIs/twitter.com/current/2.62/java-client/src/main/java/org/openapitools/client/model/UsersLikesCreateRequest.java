...
public class UsersLikesCreateRequest {
   ...
   * @throws IOException if the JSON Element is invalid with respect to UsersLikesCreateRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UsersLikesCreateRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!UsersLikesCreateRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UsersLikesCreateRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
