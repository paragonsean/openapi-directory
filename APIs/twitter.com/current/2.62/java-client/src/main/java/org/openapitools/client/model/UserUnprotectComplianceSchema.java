...
public class UserUnprotectComplianceSchema {
   ...
   * @throws IOException if the JSON Element is invalid with respect to UserUnprotectComplianceSchema
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UserUnprotectComplianceSchema.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!UserUnprotectComplianceSchema.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UserUnprotectComplianceSchema` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      UserComplianceSchema.validateJsonElement(jsonObj.get("user_unprotect"));
  }
