...
public class ListPinnedResponse {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ListPinnedResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ListPinnedResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ListPinnedResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ListPinnedResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
