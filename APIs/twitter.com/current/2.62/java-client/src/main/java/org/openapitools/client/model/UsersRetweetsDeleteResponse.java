...
public class UsersRetweetsDeleteResponse {
   ...
   * @throws IOException if the JSON Element is invalid with respect to UsersRetweetsDeleteResponse
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UsersRetweetsDeleteResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!UsersRetweetsDeleteResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UsersRetweetsDeleteResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
