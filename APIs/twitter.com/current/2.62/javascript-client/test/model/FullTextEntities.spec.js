/**
 * Twitter API v2
 * Twitter API v2 available endpoints
 *
 * The version of the OpenAPI document: 2.62
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', process.cwd()+'/src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require(process.cwd()+'/src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.TwitterApiV2);
  }
}(this, function(expect, TwitterApiV2) {
  'use strict';

  var instance;

  beforeEach(function() {
    instance = new TwitterApiV2.FullTextEntities();
  });

  var getProperty = function(object, getter, property) {
    // Use getter method if present; otherwise, get the property directly.
    if (typeof object[getter] === 'function')
      return object[getter]();
    else
      return object[property];
  }

  var setProperty = function(object, setter, property, value) {
    // Use setter method if present; otherwise, set the property directly.
    if (typeof object[setter] === 'function')
      object[setter](value);
    else
      object[property] = value;
  }

  describe('FullTextEntities', function() {
    it('should create an instance of FullTextEntities', function() {
      // uncomment below and update the code to test FullTextEntities
      //var instance = new TwitterApiV2.FullTextEntities();
      //expect(instance).to.be.a(TwitterApiV2.FullTextEntities);
    });

    it('should have the property annotations (base name: "annotations")', function() {
      // uncomment below and update the code to test the property annotations
      //var instance = new TwitterApiV2.FullTextEntities();
      //expect(instance).to.be();
    });

    it('should have the property cashtags (base name: "cashtags")', function() {
      // uncomment below and update the code to test the property cashtags
      //var instance = new TwitterApiV2.FullTextEntities();
      //expect(instance).to.be();
    });

    it('should have the property hashtags (base name: "hashtags")', function() {
      // uncomment below and update the code to test the property hashtags
      //var instance = new TwitterApiV2.FullTextEntities();
      //expect(instance).to.be();
    });

    it('should have the property mentions (base name: "mentions")', function() {
      // uncomment below and update the code to test the property mentions
      //var instance = new TwitterApiV2.FullTextEntities();
      //expect(instance).to.be();
    });

    it('should have the property urls (base name: "urls")', function() {
      // uncomment below and update the code to test the property urls
      //var instance = new TwitterApiV2.FullTextEntities();
      //expect(instance).to.be();
    });

  });

}));
