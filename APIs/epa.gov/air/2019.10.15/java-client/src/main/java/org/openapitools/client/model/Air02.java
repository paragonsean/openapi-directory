...
public class Air02 {
   ...
   * @throws IOException if the JSON Element is invalid with respect to Air02
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Air02.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in Air02 is not found in the empty JSON string", Air02.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Air02.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Air02` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : Air02.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!Air02.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'Air02' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<Air02> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(Air02.class));

       return (TypeAdapter<T>) new TypeAdapter<Air02>() {
           @Override
           public void write(JsonWriter out, Air02 value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public Air02 read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of Air02 given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of Air02
   * @throws IOException if the JSON string is invalid with respect to Air02
   */
  public static Air02 fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, Air02.class);
  }

  /**
   * Convert an instance of Air02 to an JSON string
   *
   * @return JSON string
   */
