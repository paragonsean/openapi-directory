# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.error import Error
from openapi_server import util


class DeleteCustomerCardResponse(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, errors: List[Error]=None):
        """DeleteCustomerCardResponse - a model defined in OpenAPI

        :param errors: The errors of this DeleteCustomerCardResponse.
        """
        self.openapi_types = {
            'errors': List[Error]
        }

        self.attribute_map = {
            'errors': 'errors'
        }

        self._errors = errors

    @classmethod
    def from_dict(cls, dikt: dict) -> 'DeleteCustomerCardResponse':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The DeleteCustomerCardResponse of this DeleteCustomerCardResponse.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def errors(self):
        """Gets the errors of this DeleteCustomerCardResponse.

        Any errors that occurred during the request.

        :return: The errors of this DeleteCustomerCardResponse.
        :rtype: List[Error]
        """
        return self._errors

    @errors.setter
    def errors(self, errors):
        """Sets the errors of this DeleteCustomerCardResponse.

        Any errors that occurred during the request.

        :param errors: The errors of this DeleteCustomerCardResponse.
        :type errors: List[Error]
        """

        self._errors = errors
