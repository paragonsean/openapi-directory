/**
 * Gateway
 * Gateway is the hub that routes/orchestrates the interaction between consent managers and API bridges. There are 5 categories of APIs; discovery, link, consent flow, data flow and  monitoring. To reflect the consumers of APIs, the above apis are also categorized under cm facing, hiu facing and hip facing  
 *
 * The version of the OpenAPI document: 0.5
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', process.cwd()+'/src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require(process.cwd()+'/src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.Gateway);
  }
}(this, function(expect, Gateway) {
  'use strict';

  var instance;

  beforeEach(function() {
    instance = new Gateway.ConsentArtefactResponseConsentConsentDetail();
  });

  var getProperty = function(object, getter, property) {
    // Use getter method if present; otherwise, get the property directly.
    if (typeof object[getter] === 'function')
      return object[getter]();
    else
      return object[property];
  }

  var setProperty = function(object, setter, property, value) {
    // Use setter method if present; otherwise, set the property directly.
    if (typeof object[setter] === 'function')
      object[setter](value);
    else
      object[property] = value;
  }

  describe('ConsentArtefactResponseConsentConsentDetail', function() {
    it('should create an instance of ConsentArtefactResponseConsentConsentDetail', function() {
      // uncomment below and update the code to test ConsentArtefactResponseConsentConsentDetail
      //var instance = new Gateway.ConsentArtefactResponseConsentConsentDetail();
      //expect(instance).to.be.a(Gateway.ConsentArtefactResponseConsentConsentDetail);
    });

    it('should have the property careContexts (base name: "careContexts")', function() {
      // uncomment below and update the code to test the property careContexts
      //var instance = new Gateway.ConsentArtefactResponseConsentConsentDetail();
      //expect(instance).to.be();
    });

    it('should have the property consentId (base name: "consentId")', function() {
      // uncomment below and update the code to test the property consentId
      //var instance = new Gateway.ConsentArtefactResponseConsentConsentDetail();
      //expect(instance).to.be();
    });

    it('should have the property consentManager (base name: "consentManager")', function() {
      // uncomment below and update the code to test the property consentManager
      //var instance = new Gateway.ConsentArtefactResponseConsentConsentDetail();
      //expect(instance).to.be();
    });

    it('should have the property createdAt (base name: "createdAt")', function() {
      // uncomment below and update the code to test the property createdAt
      //var instance = new Gateway.ConsentArtefactResponseConsentConsentDetail();
      //expect(instance).to.be();
    });

    it('should have the property hiTypes (base name: "hiTypes")', function() {
      // uncomment below and update the code to test the property hiTypes
      //var instance = new Gateway.ConsentArtefactResponseConsentConsentDetail();
      //expect(instance).to.be();
    });

    it('should have the property hip (base name: "hip")', function() {
      // uncomment below and update the code to test the property hip
      //var instance = new Gateway.ConsentArtefactResponseConsentConsentDetail();
      //expect(instance).to.be();
    });

    it('should have the property hiu (base name: "hiu")', function() {
      // uncomment below and update the code to test the property hiu
      //var instance = new Gateway.ConsentArtefactResponseConsentConsentDetail();
      //expect(instance).to.be();
    });

    it('should have the property patient (base name: "patient")', function() {
      // uncomment below and update the code to test the property patient
      //var instance = new Gateway.ConsentArtefactResponseConsentConsentDetail();
      //expect(instance).to.be();
    });

    it('should have the property permission (base name: "permission")', function() {
      // uncomment below and update the code to test the property permission
      //var instance = new Gateway.ConsentArtefactResponseConsentConsentDetail();
      //expect(instance).to.be();
    });

    it('should have the property purpose (base name: "purpose")', function() {
      // uncomment below and update the code to test the property purpose
      //var instance = new Gateway.ConsentArtefactResponseConsentConsentDetail();
      //expect(instance).to.be();
    });

    it('should have the property requester (base name: "requester")', function() {
      // uncomment below and update the code to test the property requester
      //var instance = new Gateway.ConsentArtefactResponseConsentConsentDetail();
      //expect(instance).to.be();
    });

    it('should have the property schemaVersion (base name: "schemaVersion")', function() {
      // uncomment below and update the code to test the property schemaVersion
      //var instance = new Gateway.ConsentArtefactResponseConsentConsentDetail();
      //expect(instance).to.be();
    });

  });

}));
