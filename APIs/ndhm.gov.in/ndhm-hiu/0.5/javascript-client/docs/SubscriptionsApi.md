# HealthRepositoryProviderSpecificationsForHiu.SubscriptionsApi
...
let apiInstance = new HealthRepositoryProviderSpecificationsForHiu.SubscriptionsApi();
...
let subscriptionApprovalNotification = new HealthRepositoryProviderSpecificationsForHiu.SubscriptionApprovalNotification(); // SubscriptionApprovalNotification | 
...
let authorization = "authorization_example"; // String | Access token which was issued after successful login with gateway auth server, which will be sent by gateway to authenticate itself with API bridge.
let X_HIU_ID = "X_HIU_ID_example"; // String | Identifier of the health information user to which the request was intended.
let hIUSubscriptionRequestReceipt = new HealthRepositoryProviderSpecificationsForHiu.HIUSubscriptionRequestReceipt(); // HIUSubscriptionRequestReceipt | 
apiInstance.v05SubscriptionRequestsHiuOnInitPost(authorization, X_HIU_ID, hIUSubscriptionRequestReceipt, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully.');
...
let authorization = "authorization_example"; // String | Access token which was issued after successful login with gateway auth server, which will be sent by gateway to authenticate itself with API bridge.
let X_HIU_ID = "X_HIU_ID_example"; // String | Identifier of the health information user to which the request was intended.
let hIUSubscriptionNotification = new HealthRepositoryProviderSpecificationsForHiu.HIUSubscriptionNotification(); // HIUSubscriptionNotification | 
