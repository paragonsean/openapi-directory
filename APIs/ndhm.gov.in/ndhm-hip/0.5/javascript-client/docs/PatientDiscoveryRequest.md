# HealthRepositoryProviderSpecificationsForHip.PatientDiscoveryRequest

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**patient** | [**PatientDiscoveryRequestPatient**](PatientDiscoveryRequestPatient.md) |  | 
**requestId** | **String** | a nonce, unique for each HTTP request. | 
**timestamp** | **Date** | Date time format in UTC, includes miliseconds YYYY-MM-DDThh:mm:ss.vZ | 
**transactionId** | **String** | correlation-Id for patient discovery and subsequent care context linkage | 


