...
public class PartyId {
   ...
   * @throws IOException if the JSON Element is invalid with respect to PartyId
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PartyId.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PartyId is not found in the empty JSON string", PartyId.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PartyId.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PartyId` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PartyId.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PartyId' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PartyId> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PartyId.class));

       return (TypeAdapter<T>) new TypeAdapter<PartyId>() {
           @Override
           public void write(JsonWriter out, PartyId value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PartyId read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of PartyId given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of PartyId
   * @throws IOException if the JSON string is invalid with respect to PartyId
   */
  public static PartyId fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PartyId.class);
  }

  /**
   * Convert an instance of PartyId to an JSON string
   *
   * @return JSON string
   */
