...
public class AddressId {
   ...
   * @throws IOException if the JSON Element is invalid with respect to AddressId
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AddressId.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in AddressId is not found in the empty JSON string", AddressId.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!AddressId.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AddressId` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!AddressId.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'AddressId' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<AddressId> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(AddressId.class));

       return (TypeAdapter<T>) new TypeAdapter<AddressId>() {
           @Override
           public void write(JsonWriter out, AddressId value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public AddressId read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of AddressId given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of AddressId
   * @throws IOException if the JSON string is invalid with respect to AddressId
   */
  public static AddressId fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, AddressId.class);
  }

  /**
   * Convert an instance of AddressId to an JSON string
   *
   * @return JSON string
   */
